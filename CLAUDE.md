...
prose up to 100K chars/message, tool results 500 (bulky, low recall value),
...
appear in text blocks. A 4MB per-session bound is enforced on both the full
build and the byte-range extension, evicting the *oldest* text on overflow
(recent turns are the recall targets; the caps were corpus-measured — don't
tighten them without remeasuring). Changing extraction means bumping
...
lib/harnesses.js — /shake, /retry, /fresh, /clear, /advisor, /prewalk, /fast,
/extended-context, /vision, /computer, /loop, /goal, /guided-goal, each
verified non-interactive in a real OMP TUI before listing). Their arg model
lives in the descriptor and is validated in `parseHostBuiltin`: `allowedArgs`
exact-matches a curated set, `freeArgs` takes arbitrary single-line text,
`requireArgs` rejects the bare form, and `blockedArgs` rejects the sub-forms
that open a TUI overlay (a bare string blocks the sub-command and everything
under it, `{ arg, exact: true }` only its argument-less spelling — `/goal set
<text>` completes in place where bare `/goal set` opens an editor). Control
characters are refused everywhere, and `/api/commands` serves only
name/description/args, never those rules. Other alternate wrappers do not get
this fallback. OMP 17.3's `ctx.reload()` reloads session/runtime state
...
That value uses an address already listening. Wildcard binds and a ready
loopback alias advertise `127.0.0.1`; a specific bind uses its primary address
until the alias is ready, and keeps that fallback if the alias fails. Merely
creating the alias server object does not establish reachability: its bind is
asynchronous. An operator-provided `PI_DISH_URL` always wins, including after
the alias becomes ready.
...
- **Layout**: every control lives in the field, and the *box* is the field:
  `.composer-box` carries the border, background and focus ring. `.composer-main`
  wraps a transparent borderless textarea above `.composer-tools` — attach and
  dictate on the left, then the context readout (`#sessionContext`, tap = stats)
  and Stop. `.composer-act` is a sibling column, not a child of the rail: Send
  when idle, Steer over Follow-up during a turn, aligned to the rail so the extra
  glyph grows up beside the textarea instead of resizing the box. There is no
  button row under the field; a phone had no width left for one. The rail is a
  real row, not an overlay: it was once absolutely positioned over reserved
  `padding-bottom`, but a textarea's padding scrolls with its text, so any
  mid-scroll position ran lines under the glyphs. Two invariants hold the
  readout still: Stop keeps its slot when idle (`visibility`, not `display`) and
  `.composer-act` is a fixed width in both states. `resize: none` — the native
  grip drew a light square inside the box, and `autosizePromptInput` (160px cap,
  text only) tracks the content. Glyphs are stroke SVG, muted until hover; Stop
  is the only coloured one, Send the only filled one, and the readout stays muted
  until its tier colours it. `test/ui-scenarios/mobile.js` asserts the geometry.
