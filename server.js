...
function materializeLaunchWrapper(descriptor, token) {
  ...
  // The host-package import must happen in THIS file: OMP rewrites bare
  // host-package specifiers only within the extension entry's directory tree,
  // and this wrapper's tree shares nothing with the repo's bridge modules.
  // Without it the bridge's nested host import fails silently and the native
  // AgentSession capture (projection, /btw) never applies.
