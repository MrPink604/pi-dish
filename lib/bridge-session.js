...
const running_tool_calls_1 = require("./running-tool-calls");
const harnesses_1 = require("./harnesses");
const session_key_1 = require("./session-key");
function asRecord(value) { return (0, wire_protocol_1.isRecord)(value) ? value : {}; }
function isRegistryEntry(value) {
    return (0, wire_protocol_1.isRecord)(value) && (0, session_key_1.validSessionId)(value.sessionId)
        && typeof value.socketPath === 'string' && value.socketPath.length > 0;
}
function requireNativeId(value) {
    if (!(0, session_key_1.validSessionId)(value))
        throw new TypeError('Invalid bridge session identity');
    return value;
}
...
function registryHarnessId(input) {
...
const FOREIGN_CONFIG_ROOTS = (0, harnesses_1.listHarnesses)()
...
function withoutWrappedPiClaims(entries) {
    ...
    for (const entry of entries) {
        if (registryHarnessId(entry) === 'pi' || !entry.sessionFile)
            continue;
        wrapped.add(`${Number(entry.pid)}\0${entry.sessionFile}`);
    }
    return entries.filter((entry) => {
        if (registryHarnessId(entry) !== 'pi')
            return true;
        if (!entry.sessionFile)
            return true;
        if (wrapped.has(`${Number(entry.pid)}\0${entry.sessionFile}`))
            return false;
...
function validRegistryClaimShape(input) {
    const entry = asRecord(input);
...
function sameRegistryClaim(left, right) {
    if (!(0, wire_protocol_1.isRecord)(left) || !(0, wire_protocol_1.isRecord)(right))
        return false;
    ...
    return left.sessionId === right.sessionId
        ...
        && (0, util_1.isDeepStrictEqual)(left.wrapper || null, right.wrapper || null)
        && (0, util_1.isDeepStrictEqual)(left.capabilities || null, right.capabilities || null)
...
function pruneRegisteredSession(entry) {
    ...
    if (!(0, wire_protocol_1.isRecord)(entry) || typeof entry.sessionId !== 'string' || !entry.sessionId || path.basename(entry.sessionId) !== entry.sessionId)
        return false;
    ...
    if (!sameRegistryClaim(entry, current))
        return false;
    try {
        fs.unlinkSync(file);
        return true;
...
function pruneUnreachableRegisteredSession(entry, error) {
    ...
    if (typeof code !== 'string' || !['ENOENT', 'ECONNREFUSED', 'ENOTSOCK'].includes(code))
        return false;
...
function pruneMalformedRegistryEntry(file, inspected) {
    try {
        ...
        if (!(0, util_1.isDeepStrictEqual)(current, inspected))
            return false;
        fs.unlinkSync(file);
        return true;
    }
...
function scanRegistry() {
    ...
    for (const name of entries) {
        ...
        if (hasStartTime(entry)) {
            const identity = (0, process_identity_1.processIdentity)(Number(entry.pid));
...
function getRegisteredSessionByNativeId(harnessId, nativeSessionId) {
    const matches = listRegisteredSessions().filter(e => registryHarnessId(e) === harnessId
 ...
 */
class BridgeSession extends events_1.EventEmitter {
    constructor(registryEntry) {
        super();
        if (!isRegistryEntry(registryEntry))
            throw new TypeError('Invalid bridge registry entry');
        ...
        this.sock = null;
        this._nextId = 1;
        this._pending = new pending_requests_1.PendingRequests();
        this.hello = null;
    }
    connect() {
        this.hello = null;
        return new Promise((resolve, reject) => {
            ...
            const connectTimer = setTimeout(() => {
                if (!settled) {
                    fail(new Error('bridge connect timeout'));
                    sock.destroy();
                }
            }, 5000);
            sock.on('connect', () => {
            ...
            sock.on('data', (0, line_splitter_1.createLineSplitter)((line) => {
                let msg;
                try {
                    msg = JSON.parse(line);
                }
    ...
    _handle(input) {
        const frame = (0, wire_protocol_1.decodeBridgeFrame)(input);
        if (!frame)
            return;
        ...
        if (frame.kind !== 'hello' && Number(this.registryClaim.protocolVersion || 1) >= 2 && !this.hello)
            return;
        if (frame.kind === 'hello') {
            ...
            if (!(0, session_key_1.validSessionId)(nextNativeId)) {
            ...
            if (Number(this.registryClaim?.protocolVersion || 1) < 2) {
                if ((0, wire_protocol_1.isRecord)(msg.capabilities))
            ...
            if (msg.name)
                this.name = msg.name;
            if (msg.contextUsage)
                this.contextUsage = msg.contextUsage;
        ...
        if (frame.kind === 'event') {
            ...
            const switchedId = ev === 'session_switch' && (0, session_key_1.validSessionId)(data.sessionId) ? data.sessionId : null;
            ...
            if (ev === 'session_switch' && data.sessionId && !switchedId)
                return;
            (0, running_tool_calls_1.trackRunningToolCalls)(this.runningToolCalls, ev, {
            ...
            else if (ev === 'compaction_start')
                this.compacting = true;
            else if (ev === 'compaction_end')
                this.compacting = false;
            else if (ev === 'queue_update')
                this.queueState = frame.data;
            if (ev === 'extension_ui_request' && data?.method) {
                if (data.method === 'setWidget') {
                    const key = data.widgetKey || 'default';
                    if (Array.isArray(data.widgetLines) && data.widgetLines.length)
                        this.extUIState.widgets.set(key, data);
                    else
                        this.extUIState.widgets.delete(key);
                }
                else if (data.method === 'setStatus') {
                    const key = data.statusKey || 'default';
                    if (data.statusText)
                        this.extUIState.statuses.set(key, data);
                    else
                        this.extUIState.statuses.delete(key);
                }
            ...
            if (switchedId) {
                ...
                if (this.registryClaim) {
                    ...
                    if (data.sessionFile)
                        this.registryClaim.sessionFile = data.sessionFile;
                    if (data.cwd)
                        this.registryClaim.cwd = data.cwd;
                }
            }
        }
    }
    waitForHello({ timeout = 2000 } = {}) {
        ...
        if (!this.alive)
            return Promise.reject(new Error('bridge disconnected before hello'));
        return new Promise((resolve, reject) => {
            let timer;
            const cleanup = () => {
                clearTimeout(timer);
            ...
            const onHello = (hello) => {
                if (!(0, wire_protocol_1.isRecord)(hello))
                    return;
                cleanup();
                resolve(hello);
            };
    ...
    prompt(message, opts = {}) {
        ...
        if (opts.deliverAs)
            params.deliverAs = opts.deliverAs;
    ...
    runCommand(message, deliverAs, opts = {}) {
    ...
    navigateTree(targetId, opts = {}) {
        ...
        if (opts.customInstructions)
            params.customInstructions = opts.customInstructions;
        if (opts.label)
...
async function getBridgeSession(sessionId) {
    ...
    if (!entry)
        throw new Error(`session ${typeof sessionId === 'string' ? sessionId : 'claim'} not registered or ambiguous`);
    const key = connectionKey(entry);
    let promise = connections.get(key);
    if (promise) {
        const sess = await promise;
        if (sess.alive && sameRegistryClaim(sess.registryClaim, entry))
            return sess;
        if (sess.alive)
            sess.close();
    ...
    promise.catch(() => {
        if (connections.get(key) === promise)
            connections.delete(key);
    });
    sess.on('close', () => {
        if (connections.get(key) === promise)
...
module.exports = {
    ...
    BridgeSession,
    pidAlive,
    processIdentity: process_identity_1.processIdentity,
    processIdentityAlive: process_identity_1.processIdentityAlive,
};
