...
function withoutWrappedPiClaims(entries) {
  ...
  if (!wrapped.size) return entries;
  return entries.filter((entry) => registryHarnessId(entry) !== 'pi' || !entry.sessionFile
    || !wrapped.has(`${Number(entry.pid)}\0${entry.sessionFile}`));
}
