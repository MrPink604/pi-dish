...
exports.cacheLifetimeReport = cacheLifetimeReport;
...
function decodeObservations(value) {
    ...
    for (const raw of value) {
        ...
        if (obs.length >= WINDOW_MAX_OBS * 4)
            break;
    }
    return obs;
}
function gapBucket(gap) {
    if (gap < WINDOW_BUCKET_BASE_MS)
        return 0;
    return Math.min(WINDOW_BUCKETS - 1, 1 + Math.floor(Math.log2(gap / WINDOW_BUCKET_BASE_MS)));
}
function evictOldest(window, bucket) {
    let index = -1;
    for (let i = 0; i < window.length; i++) {
        if (bucket !== null && gapBucket(window[i].gap) !== bucket)
            continue;
        if (index < 0 || window[i].t < window[index].t)
            index = i;
    }
    return window.splice(index, 1)[0];
}
/** Admit one probe into a stratified window; returns every probe it evicted (possibly itself). */
function admitObservation(window, obs, now) {
    const cutoff = now - WINDOW_MAX_AGE_MS;
    const evicted = [];
    for (let i = window.length - 1; i >= 0; i--) {
        if (window[i].t < cutoff)
            evicted.push(window.splice(i, 1)[0]);
    }
    if (obs.t < cutoff)
        return [...evicted, obs];
    window.push(obs);
    const counts = new Array(WINDOW_BUCKETS).fill(0);
    for (const o of window)
        counts[gapBucket(o.gap)]++;
    const bucket = gapBucket(obs.gap);
    if (counts[bucket] > WINDOW_BUCKET_OBS)
        evicted.push(evictOldest(window, bucket));
    else if (window.length > WINDOW_MAX_OBS)
        evicted.push(evictOldest(window, counts.indexOf(Math.max(...counts))));
    return evicted;
}
function decodeStore(value) {
    ...
    for (const [key, raw] of Object.entries(rawIdentities)) {
        ...
        const obs = [];
        for (const o of decodeObservations(raw.obs).sort((a, b) => a.t - b.t))
            admitObservation(obs, o, now);
        if (!obs.length)
            continue;
        identities.set(key, {
 ...
 */
function fitGates(fit, rawObservations) {
    if (!fit) {
    ...
    return [
        ...
        { id: 'range', pass: fit.ttlMs >= cache_retention_1.MIN_TTL_MS && fit.ttlMs <= cache_retention_1.MAX_TTL_MS, value: Math.round(fit.ttlMs), need: cache_retention_1.MIN_TTL_MS },
...
function recordObservation(key, obs, now) {
    ...
    for (const dropped of admitObservation(state.obs, obs, now))
...
function extractObservations(entries, file, chain, profileId) {
    ...
    for (const raw of entries) {
        ...
        chain.set(identity, { ts, tier1h: wrote ? (0, session_metadata_1.cacheTier1h)(usage, target.provider === 'anthropic') : (anchor?.tier1h ?? false) });
    }
}
...
function isServedCacheExpiry(expiry) {
 ...
 * replaces guesses.
 */
function applyLearnedCacheExpiry(expiry, config) {
    if (!expiry)
        return null;
    const known = isServedCacheExpiry(expiry) ? expiry : null;
    if (known?.basis === 'fixed')
        return known;
    const target = (0, session_metadata_1.resolveCacheTarget)(splitIdentity(expiry.identity));
    if ((0, cache_retention_1.configuredCacheRetention)(config ?? (0, cache_retention_1.loadCacheRetentionConfig)(), target.provider, target.model))
        return known;
 ...
 */
function cacheLifetimeReport(config = (0, cache_retention_1.loadCacheRetentionConfig)()) {
    const store = getStore();
    const now = Date.now();
    const out = [];
    for (const [key, state] of store.identities) {
        ...
        const fit = ensureFit(key, state, now);
        const active = !!fit && fitActive(fit);
        const override = (0, cache_retention_1.configuredCacheRetention)(config, target.provider, target.model);
        const builtin = (0, session_metadata_1.builtinCacheRetention)(target, tier === '1h');
        let source = 'none';
        let effective = null;
        if (override) {
            source = 'override';
        ...
        let hits = 0, maxHitGapMs = null, minMissGapMs = null, lastAt = 0;
        for (const o of state.obs) {
            if (o.hit) {
                hits++;
                if (maxHitGapMs === null || o.gap > maxHitGapMs)
                    maxHitGapMs = o.gap;
            }
            else if (minMissGapMs === null || o.gap < minMissGapMs)
                minMissGapMs = o.gap;
            if (o.t > lastAt)
                lastAt = o.t;
        }
        out.push({
            ...
            points: [...state.obs].sort((a, b) => a.t - b.t).map(o => [Math.round(o.gap), o.hit ? 1 : 0]),
        });
    }
