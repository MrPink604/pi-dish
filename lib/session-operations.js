...
function createSessionOperations(observations) {
    ...
    function startSessionSpawn(options) {
        ...
        Promise.resolve()
            ...
            const descriptor = (0, harnesses_1.getHarness)(options.harness || 'pi');
            if (!descriptor)
                throw new SessionOperationError(400, `Unknown harness: ${options.harness}`);
            try {
                await launch.validateHarnessPilotSelection(descriptor, { model: options.model, thinking: options.thinking, cwd: options.cwd });
            }
            catch (error) {
                throw new SessionOperationError(errorStatus(error), errorMessage(error));
            }
