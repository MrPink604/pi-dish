...
// v2 indexes tool-call names/args (file paths, bash commands) and raises the
// per-message prose cap; v1 entries reread via the normal bounded backlog.
...
function tryExtendIndexEntry(st, candidate, stats) {
  ...
  if (extension.text && text.text.length < SEARCH_TEXT_SESSION_CAP) {
    text.text = (text.text + ' ' + extension.text).substring(0, SEARCH_TEXT_SESSION_CAP);
  }
