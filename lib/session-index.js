...
// v5 retains Pi's native session-header id and parentSession provenance.
// Older entries cannot gain those fields without rereading source JSONL, so
// the normal bounded backlog rebuilds them just like prior schema upgrades.
const META_SCHEMA_VERSION = 5;
...
function indexFile(st, file, stats) {
  ...
  const records = mineSkillsFromContent(content, {
    ...
    sessionId: path.basename(file, '.jsonl') === 'session' && info.sessionId
      ? info.sessionId : path.basename(file, '.jsonl'),
    skillCtx,
  });
