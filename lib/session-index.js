...
const encodeMeta = (f, e) => ({ f, m: e.mtimeMs, s: e.size, ver: META_SCHEMA_VERSION, p: e.profileId, pv: e.profileVersion, pr: e.pricingRevision, cr: e.cacheRetentionRevision, v: e.info });
...
function indexFile(st, candidate, stats, ctx) {
    ...
    const entries = (0, session_files_js_1.parseSessionEntries)(content);
    ...
    const info = {
        ...
        usage: (0, session_files_js_1.buildIndexedUsageFromEntries)(entries, candidate),
    };
    const search = (0, session_files_js_1.buildSearchIndexFromEntries)(entries);
    (0, session_index_data_1.checkSearchLeaf)(search);
    setEntry(st.meta, st.metaLog, file, { mtimeMs: stats.mtimeMs, size: stats.size, version: META_SCHEMA_VERSION, profileId: candidate.profileId, profileVersion: candidate.profileVersion, pricingRevision: ctx.harnessRevision(candidate.harnessId), cacheRetentionRevision: ctx.cacheConfig.revision, info }, encodeMeta);
...
function tryExtendIndexEntry(st, candidate, stats, ctx) {
    ...
    try {
        ...
        (0, session_files_js_1.extendIndexedUsageFromEntries)(meta.info.usage, entries, candidate);
        const mined = (0, skill_mining_js_1.mineSkillsFromEntries)(entries, {
...
function kickBuilder(st) {
    ...
    const step = () => {
        ...
        st.backlog.delete(file);
        try {
            indexFile(st, work.candidate, work.stats, work.ctx);
        }
