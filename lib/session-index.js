...
const fileHelpers = require('./session-files.js');
const { mineSkillsFromEntries } = require('./skill-mining.js');
const pricing = require('./harness-pricing.js');
function pricingRevision(harnessId) {
    const value = pricing.pricingRevision(harnessId);
    if (typeof value !== 'string')
        throw new TypeError('Invalid pricing revision');
    return value;
}
function searchTextCap(value) {
    if (!(0, session_index_data_1.finite)(value) || value < 0)
        throw new TypeError('Invalid search text cap');
    return value;
}
const SEARCH_TEXT_SESSION_CAP = searchTextCap(fileHelpers.SEARCH_TEXT_SESSION_CAP);
const parseSessionEntries = (content) => (0, session_index_data_1.checkedEntries)(fileHelpers.parseSessionEntries(content));
const buildSearchIndexFromEntries = (entries) => (0, session_index_data_1.checkedSearch)(fileHelpers.buildSearchIndexFromEntries(entries));
const extendSearchIndexFromEntries = (entries, tree, leafId) => {
    const result = fileHelpers.extendSearchIndexFromEntries(entries, tree, leafId);
    return result === null ? null : (0, session_index_data_1.checkedSearch)(result);
};
const buildIndexedUsageFromEntries = (entries, source) => (0, session_index_data_1.checkedUsage)(fileHelpers.buildIndexedUsageFromEntries(entries, source));
const extendIndexedUsageFromEntries = (usage, entries, source) => (0, session_index_data_1.checkedUsage)(fileHelpers.extendIndexedUsageFromEntries(usage, entries, source));
...
function indexFile(st, candidate, stats) {
    ...
    const info = {
        ...(0, session_metadata_1.sessionInfoFromEntries)(entries, stats.mtime, candidate),
    ...
    setEntry(st.meta, st.metaLog, file, { mtimeMs: stats.mtimeMs, size: stats.size, version: META_SCHEMA_VERSION, profileId: candidate.profileId, profileVersion: candidate.profileVersion, pricingRevision: pricingRevision(candidate.harnessId), info }, encodeMeta);
    ...
    const mined = (0, session_index_data_1.checkedSkills)(mineSkillsFromEntries(entries, {
...
function tryExtendIndexEntry(st, candidate, stats) {
    ...
    if (meta.profileId !== candidate.profileId || meta.profileVersion !== candidate.profileVersion)
        return null;
    if (meta.pricingRevision !== pricingRevision(candidate.harnessId))
        return null;
    ...
    if (!extension)
        return null;
    try {
        (0, session_metadata_1.extendSessionInfoFromEntries)(meta.info, entries, stats.mtime, candidate);
...
function kickBuilder(st) {
    ...
    const step = () => {
        ...
        st.backlog.delete(file);
        try {
            indexFile(st, work.candidate, work.stats);
        }
...
function scanSessions(files) {
    ...
    for (const input of files) {
        ...
        if (budget > 0) {
            budget--;
            try {
                infos.set(file, indexFile(st, candidate, stats));
            }
            catch { }
        }
        else {
...
function getSearchText(input) {
    ...
    if (tryExtendIndexEntry(st, candidate, stats))
        return st.text.get(file).text;
    try {
        indexFile(st, candidate, stats);
    }
    catch {
