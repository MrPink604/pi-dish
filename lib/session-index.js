...
function normalize(input) {
    ...
    const identity = (0, session_key_1.resolveSessionRoute)(input.routeId);
    if (identity.harnessId !== input.harnessId || identity.nativeSessionId !== input.nativeSessionId ||
        (0, session_key_1.encodeSessionKey)(identity.harnessId, identity.nativeSessionId) !== input.sessionKey ||
        (0, session_key_1.canonicalSessionId)(input.routeId) !== input.routeId)
...
function indexFile(st, candidate, stats, cacheConfig = (0, cache_retention_1.loadCacheRetentionConfig)()) {
    ...
    const info = {
        ...(0, session_metadata_1.sessionInfoFromEntries)(entries, stats.mtime, candidate, cacheConfig),
    ...
    setEntry(st.meta, st.metaLog, file, { mtimeMs: stats.mtimeMs, size: stats.size, version: META_SCHEMA_VERSION, profileId: candidate.profileId, profileVersion: candidate.profileVersion, pricingRevision: (0, harness_pricing_js_1.pricingRevision)(candidate.harnessId), cacheRetentionRevision: cacheConfig.revision, info }, encodeMeta);
...
function isEntryFresh(st, file, candidate, stats, cacheConfig) {
    ...
    return !!(cached && cached.version === META_SCHEMA_VERSION &&
        ...
        cached.pricingRevision === (0, harness_pricing_js_1.pricingRevision)(candidate.harnessId) &&
 ...
 */
function tryExtendIndexEntry(st, candidate, stats, cacheConfig) {
    const file = candidate.file;
    ...
    if (meta.profileId !== candidate.profileId || meta.profileVersion !== candidate.profileVersion)
        return null;
    if (meta.pricingRevision !== (0, harness_pricing_js_1.pricingRevision)(candidate.harnessId))
        return null;
    if (meta.cacheRetentionRevision !== cacheConfig.revision)
        return null;
    ...
    if (!extension)
        return null;
    try {
        (0, session_metadata_1.extendSessionInfoFromEntries)(meta.info, entries, stats.mtime, candidate, cacheConfig);
...
function kickBuilder(st) {
    ...
    const step = () => {
        ...
        st.backlog.delete(file);
        try {
            indexFile(st, work.candidate, work.stats, work.cacheConfig);
        }
...
function scanSessions(files) {
    ...
    const cacheConfig = (0, cache_retention_1.loadCacheRetentionConfig)();
    for (const input of files) {
        ...
        if (budget > 0) {
            budget--;
            try {
                infos.set(file, indexFile(st, candidate, stats, cacheConfig));
            }
            catch { }
        }
        else {
...
function getSearchText(input) {
    ...
    if (tryExtendIndexEntry(st, candidate, stats, cacheConfig))
        return st.text.get(file).text;
    try {
        indexFile(st, candidate, stats, cacheConfig);
    }
    catch {
