...
// v4 preserves unavailable cost components as null plus per-component
// unavailable-call counts. v3's zero-filled summaries cannot be repaired
// without rereading source JSONL, so the normal bounded backlog rebuilds them.
...
// still detected without it (see lib/skill-mining.js). A version salt bumps
// whenever the roots change so stale-context skill entries re-mine.
...
function setSkillRoots(skillFilePaths) {
  ...
  // Only salt-bump when the root set actually changed, so setting the same
  // inventory twice doesn't force a needless re-mine of the whole corpus.
  const key = [...roots.keys()].sort().join('\n');
  if (key !== (skillCtx._key || '')) {
    skillCtx = { roots, _key: key };
    skillCtxSalt++;
  }
}
...
const encodeMeta = (f, e) => ({ f, m: e.mtimeMs, s: e.size, ver: META_SCHEMA_VERSION, v: e.info });
...
function indexFile(st, file, stats) {
  ...
  info.usage = buildIndexedUsageFromContent(content);
  const search = buildSearchIndexFromContent(content);
  setEntry(st.meta, st.metaLog, file,
    { mtimeMs: stats.mtimeMs, size: stats.size, version: META_SCHEMA_VERSION, info }, encodeMeta);
  ...
  setEntry(st.skills, st.skillsLog, file,
    { mtimeMs: stats.mtimeMs, size: stats.size, version: SKILLS_SCHEMA_VERSION, salt: skillCtxSalt, records }, encodeSkills);
  return info;
}
