...
function createSkillFeatureHandlers(ports) {
    ...
    const skillCoverage = (req, res) => {
        ...
        const mapped = all.filter(r => (r.kind === 'read' || r.kind === 'explicit') &&
            (0, session_index_data_1.finite)(r.ts) && r.ts >= stat.mtimeMs);
        const excludedBeforeMtime = all.filter(r => (r.kind === 'read' || r.kind === 'explicit') &&
            (!(0, session_index_data_1.finite)(r.ts) || r.ts < stat.mtimeMs)).length;
        ...
        if (latest && latest.sessionId) {
            try {
                ...
                if (source)
                    latest = { ...latest, name: (0, session_files_1.getSessionInfo)(source).name || null };
            }
            catch { }
        }
        res.json({
