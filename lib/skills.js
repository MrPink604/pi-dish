...
exports.projectSkillCoverage = projectSkillCoverage;
...
function splitSections(lines) {
    const sections = [];
    let cur = { heading: '(intro)', level: 0, startLine: 1, endLine: 0, lines: [] };
    lines.forEach((line, i) => {
        const m = line.match(/^(#{1,6})\s+(.*)$/);
        if (m) {
            if (cur.lines.length) {
                cur.endLine = cur.startLine + cur.lines.length - 1;
                sections.push(cur);
            }
...
function coveredLines(rec, lineCount) {
    const set = new Set();
    const add = (s, e) => { for (let i = Math.max(1, s); i <= Math.min(lineCount, e); i++)
        set.add(i); };
    if (rec.kind === 'explicit') {
        add(1, lineCount);
        return set;
    }
    if (rec.ranges === 'all') {
        add(1, (0, session_index_data_1.finite)(rec.truncatedTo) ? rec.truncatedTo : lineCount);
        return set;
    }
    if (Array.isArray(rec.ranges)) {
        for (const [s, e] of rec.ranges)
            add(s, e === -1 ? lineCount : e);
    }
    return set;
}
/** Current-file coverage; each mapped read contributes one transient line set. */
function projectSkillCoverage(content, mtimeMs, records) {
    ...
    for (const r of records) {
        if (r.kind === 'targeted')
            targetedTouches++;
        if (r.kind !== 'read' && r.kind !== 'explicit')
            continue;
        if (!(0, session_index_data_1.finite)(r.ts) || r.ts < mtimeMs)
            excludedBeforeMtime++;
        if (!(0, session_index_data_1.finite)(r.ts) || !(r.ts >= mtimeMs))
            continue;
        numMapped++;
        const set = coveredLines(r, lineCount);
        if (set.size < lineCount)
            anyPartial = true;
        for (const ln of set)
            lineHits[ln]++;
    ...
    for (let ln = 1; ln <= lineCount; ln++)
        if (!lineHits[ln])
            unreadChars += lines[ln - 1].length + 1;
    return {
