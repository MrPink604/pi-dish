...
function sanitizePricing(cost) {
  ...
  for (const key of ['input', 'output', 'cacheRead', 'cacheWrite']) {
    if (Number.isFinite(cost[key])) pricing[key] = cost[key];
  }
  return Number.isFinite(pricing.input) && Number.isFinite(pricing.output) ? pricing : null;
}
...
// OMP uses the same v3 transcript entries but keeps a fixed-width title record
// before the session header. Pi's SessionManager requires the header to be the
// first parsed entry, so export a temporary header-first view without touching
// OMP's source file.
async function exportSessionHtml(sessionPath, outputPath, profileId = 'pi-v3') {
  const sdk = await getSDK();
  const mod = await import(
    pathToFileURL(path.join(PKG_ROOT, 'dist', 'core', 'export-html', 'index.js')).href
  );
  let exportPath = sessionPath;
  let tempDir = null;
  try {
    if (profileId === 'omp-v1') {
      ...
      tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pi-dish-omp-export-'));
      exportPath = path.join(tempDir, path.basename(sessionPath));
      fs.writeFileSync(exportPath, lines.slice(headerIndex).join('\n'));
    }
    const sm = sdk.SessionManager.open(exportPath);
    return await mod.exportSessionToHtml(sm, undefined, { outputPath });
  } finally {
