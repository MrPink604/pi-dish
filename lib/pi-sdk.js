...
//
// Runs the CLI asynchronously — execSync here blocked the whole event loop
// (every request, including SSE streams) for the duration of the subprocess
...
function runListModels() {
  if (!_listModelsInFlight) {
    ...
    _listModelsInFlight = new Promise((resolve, reject) => {
      exec(`node "${CLI_PATH}" --list-models 2>&1`, {
        timeout: 10000,
        encoding: 'utf-8',
        env: { ...process.env },
