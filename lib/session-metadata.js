...
function decodeCacheExpiry(value) {
    ...
    if (refreshedAt === null || expiresAt === null || retentionMs === null ||
        ...
        !['fixed', 'minimum', 'estimate'].includes(String(basis)))
        return null;
 ...
 */
function cacheExpiryForMessage(message, previous = null, fallbackTimestamp) {
    ...
    for (let i = 0; i < entries.length; i++) {
        ...
        if (entry.type === 'message' && message?.role === 'assistant' && isRecord(message.usage)) {
            ...
            if (hasCacheActivity(message)) {
                info.cacheExpiry = cacheExpiryForMessage(message.model === undefined ? { ...message, model: info.model } : message, info.cacheExpiry, entry.timestamp);
            }
        }
