...
/**
 * @typedef {'minute'|'hour'|'dayOfMonth'|'month'|'dayOfWeek'} FieldName
 * @typedef {{name: FieldName, min: number, max: number} &
 *   ({names: null, nameOffset?: never} | {names: string[], nameOffset: number})} Field
 * @typedef {Record<FieldName, Set<number>> & {expression: string, domRestricted: boolean, dowRestricted: boolean}} ParsedCron
 */
/** @type {Field[]} */
const FIELDS = [
...
/** @param {Field} field @param {string} token */
...
/** One field → matching values and whether it restricts anything.
 * @param {Field} field @param {string} spec
 */
...
function parseCron(expression) {
  ...
  /** @type {ParsedCron} */
  const parsed = {
    expression: raw, minute: new Set(), hour: new Set(), dayOfMonth: new Set(),
    month: new Set(), dayOfWeek: new Set(), domRestricted: false, dowRestricted: false,
  };
...
/** @param {ParsedCron} parsed @param {Date} date */
...
/** @param {ParsedCron} parsed @param {Date|string|number} date */
...
/** Convenience for callers that only hold the raw string. @param {unknown} expression */
