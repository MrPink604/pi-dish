...
function untrackedEntry(repoPath, relPath) {
...
async function getRepoDiff(repoPath) {
  ...
  const hasHead = status.oid && status.oid !== '(initial)';
  if (hasHead && status.changed.size > 0) {
    try {
      const out = await runGit(repoPath, ['diff', 'HEAD', '--no-color', '--no-ext-diff', '-M', '--']);
      for (const f of splitPatch(out)) {
        const st = status.changed.get(f.path) || status.changed.get(f.oldPath || '');
        repo.files.push({ status: st ? st.status : 'M', ...f });
      }
    } catch (e) {
  ...
  const untracked = status.untracked.slice(0, MAX_UNTRACKED_FILES);
  for (const rel of untracked) {
    const entry = untrackedEntry(repoPath, rel);
    if (entry) repo.files.push(entry);
  }
  if (status.untracked.length > MAX_UNTRACKED_FILES) {
    repo.moreUntracked = status.untracked.length - MAX_UNTRACKED_FILES;
  }
 ...
 */
async function aggregateDiffs(root) {
  if (!isGitAvailable()) return { root, gitAvailable: false, repos: [] };
  const repoPaths = findGitRepos(root);
  const repos = await Promise.all(repoPaths.map(async (abs) => ({
    path: path.relative(root, abs) || '.',
    ...(await getRepoDiff(abs)),
  })));
  repos.sort((a, b) =>
...
module.exports = { isGitAvailable, findGitRepos, splitPatch, parseStatusV2, getRepoDiff, aggregateDiffs };
