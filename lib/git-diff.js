...
function runGit(cwd, args, { timeout = 10000, maxBuffer = 20 * 1024 * 1024 } = {}) {
    return new Promise((resolve, reject) => {
        (0, child_process_1.execFile)('git', ['-c', 'core.quotepath=false', ...args], { cwd, timeout, maxBuffer, encoding: 'utf8' }, (err, stdout, stderr) => {
            if (err) {
                err.stderr = stderr;
                return reject(err);
            }
...
function gitErrorLine(error) {
    const detail = (0, helper_values_1.record)(error) ? error.stderr || error.message || '' : '';
...
function isGitAvailable() {
    if (gitAvailable !== null)
        return gitAvailable;
    try {
        (0, child_process_1.execFileSync)('git', ['--version'], { stdio: 'ignore', timeout: 2000 });
        gitAvailable = true;
    }
    catch {
        gitAvailable = false;
...
function parseStatusV2(out) {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (t.startsWith('# branch.head '))
            result.branch = t.slice('# branch.head '.length);
        else if (t.startsWith('# branch.oid '))
            result.oid = t.slice('# branch.oid '.length);
...
function splitPatch(patchText) {
    ...
    for (const line of String(patchText).split('\n')) {
        ...
        if (line.startsWith('+++ '))
            cur.newPath = line.slice(4).trim();
        else if (line.startsWith('--- '))
            cur.oldPath = line.slice(4).trim();
        else if (line.startsWith('Binary files ') || line.startsWith('GIT binary patch'))
            cur.binary = true;
        else if (line.startsWith('rename from '))
            cur.oldPath = 'a/' + line.slice('rename from '.length);
        else if (line.startsWith('rename to '))
            cur.newPath = 'b/' + line.slice('rename to '.length);
    }
    const strip = (p) => {
        if (!p || p === '/dev/null')
            return null;
 ...
 */
function repoStateVersion(repoPath, statusRaw, status) {
...
function aggregateVersion(root, repoPaths, states) {
    ...
    for (const entry of entries)
        hash.update(`${entry.path}\0${entry.version}\0`);
    return hash.digest('hex');
}
async function getDiffVersion(root) {
    if (!isGitAvailable())
        return null;
...
function repoStateFileCount(state) {
    ...
    if (!state.hasHead)
        for (const filePath of state.status.changed.keys())
...
async function buildRepoDiff(repoPath, state, { includePatches = true } = {}) {
    ...
    for (const f of repo.files) {
        repo.additions += f.additions;
        repo.deletions += f.deletions;
    }
    return repo;
}
...
async function getFilePatch(repoPath, file) {
    ...
    if (abs !== root && !abs.startsWith(root + path.sep))
        return null;
    if (file.status === '?')
        return untrackedEntry(repoPath, file.path);
    const paths = [file.path];
    if (file.oldPath && file.oldPath !== file.path)
        paths.push(file.oldPath);
    const out = await runGit(repoPath, ['diff', 'HEAD', '--no-color', '--no-ext-diff', '-M', '--', ...paths]);
...
async function aggregateDiffs(root, { inlineLimit = null } = {}) {
    if (!isGitAvailable())
        return { root, gitAvailable: false, repos: [], version: null };
    const repoPaths = findGitRepos(root);
    const states = await Promise.all(repoPaths.map(getRepoState));
    ...
    repos.sort((a, b) => Number(b.files.length > 0) - Number(a.files.length > 0) || a.path.localeCompare(b.path));
...
function createDiffView() {
    ...
    return {
        async summary(sessionId, cwd) {
            ...
            const { version, ...data } = await aggregateDiffs(cwd, { inlineLimit: DIFF_INLINE_FILE_LIMIT });
            const snapshot = {
        ...
        async patch(sessionId, cwd, snapshotId, repoPath, filePath) {
            ...
            if (await getDiffVersion(cwd) !== snapshot.version)
                return 'stale';
