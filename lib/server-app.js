...
exports.observeServerStartup = observeServerStartup;
...
const host_health_1 = require("./host-health");
...
const serverStartups = new WeakMap();
function observeServerStartup(initialServer, observer) {
    const startup = serverStartups.get(initialServer);
    if (!startup)
        throw new Error('Server does not belong to a pi-dish startup');
    startup.observers.push(observer);
    if (startup.readyUrl !== null)
        observer.ready(startup.readyUrl);
}
function startServer(rootDirectory) {
    ...
    function observeSessionLifecycle(sess) {
        const dismissAskDialogs = (source) => {
            for (const [id, data] of sess.extUIState.dialogs) {
                if ((data == null ? undefined : property(data, 'method')) !== 'ask')
                    continue;
                (0, extension_ui_state_1.removeExtensionUIDialog)(sess.extUIState, id);
    ...
    const hostHealth = (0, host_health_1.createHostHealth)();
    app.get('/api/host/health', async (_req, res) => {
        try {
            ...
            for (const session of active)
                byHarness[session.harnessId] = (byHarness[session.harnessId] || 0) + 1;
            res.json({
                ...health,
                sessions: {
                    live: active.length,
    ...
    app.get('/api/search', (req, res) => {
        ...
        for (const session of [...active, ...previous]) {
            ...
            ranked.push({ session, text, searchScore: (0, helper_query_js_1.scoreSessionMatch)(parsed, session, text) });
        }
    ...
    app.post('/api/sessions/:id/ui-response', async (req, res) => {
        ...
        try {
            ...
            (0, extension_ui_state_1.removeExtensionUIDialog)(sess.extUIState, requestId);
    ...
    // =========================================================================
    const terminalHandlers = terminal.isTerminalEnabled() ? (0, terminal_handlers_1.createTerminalHandlers)({
        upgradeAuthorized: accessHandlers.upgradeAuthorized,
    ...
    if (process.env.PI_DISH_SHARE_PORT) {
        ...
        shareServer = (0, node_http_1.createServer)(shareApp);
        // Observe without consuming the error: native Node/Electron ownership stays.
        const shareEvents = shareServer;
        shareEvents.on(node_events_1.errorMonitor, reportFailure);
    }
    ...
    function ensureLoopbackAlias(port) {
        ...
        alias.once('listening', () => {
            ...
            console.log(`pi-dish loopback alias at http://${LOOPBACK}:${alias.address().port}`);
            reportReady(alias);
        });
        alias.on('error', (err) => {
    ...
    function startMainListener(main, retrySeconds = 15) {
        main.once('listening', () => {
            if (PORT === 0)
    ...
    if (shareServer) {
        ...
        share.listen(process.env.PI_DISH_SHARE_PORT, shareHost, () => {
            console.log(`pi-dish share listener at http://${shareHost}:${share.address().port}`);
        });
    }
