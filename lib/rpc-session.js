...
const { decodeRPCFrame } = require('./wire-protocol');
...
class RPCSession {
  constructor(id, proc) {
    ...
    this.recoveryObserver = createSessionObserver({
      ...
      snapshot: () => this.sessionFile && this.cwd ? {
        ...
        model: this.state?.model?.provider && this.state?.model?.id
          ? `${this.state.model.provider}/${this.state.model.id}` : null,
  ...
  _canObserveRecovery() {
    ...
    for (const name of entries) {
      ...
      if (claim.recoveryObservation === true && claim.harnessId === 'pi'
        && claim.sessionFile === this.sessionFile && processIdentityAlive(claim)) {
  ...
  _handleMessage(msg) {
    const frame = decodeRPCFrame(msg);
    if (!frame) return;
    ...
    if (msg.type === 'message_start' && msg.message?.role === 'assistant') {
      ...
      if (event.type === 'done') this.streamingAssistantMessage = structuredClone(event.message);
      else if (event.type === 'error') this.streamingAssistantMessage = structuredClone(event.error);
      else if (this.streamingAssistantMessage) {
        const content = this.streamingAssistantMessage.content || (this.streamingAssistantMessage.content = []);
        const index = event.contentIndex;
        if (event.type === 'text_start') content[index] = { type: 'text', text: '' };
        else if (event.type === 'text_delta') {
          if (content[index]?.type !== 'text') content[index] = { type: 'text', text: '' };
          content[index].text += event.delta;
        } else if (event.type === 'text_end') content[index] = { type: 'text', text: event.content };
        else if (event.type === 'thinking_start') content[index] = { type: 'thinking', thinking: '' };
        else if (event.type === 'thinking_delta') {
          if (content[index]?.type !== 'thinking') content[index] = { type: 'thinking', thinking: '' };
          content[index].thinking += event.delta;
        } else if (event.type === 'thinking_end') content[index] = { type: 'thinking', thinking: event.content };
        else if (event.type === 'toolcall_end') content[index] = structuredClone(event.toolCall);
      }
    ...
    trackRunningToolCalls(this.runningToolCalls, msg.type, msg);
    this.recoveryObserver.event(msg.type, msg);
...
async function _initRPCSession(proc, opts = {}) {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      const basenameId = state?.sessionFile ? path.basename(state.sessionFile, '.jsonl') : null;
      const headerId = safeHeaderSessionId(state?.sessionId);
      const realId = basenameId === 'session' && headerId
