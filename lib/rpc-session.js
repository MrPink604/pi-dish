...
class RPCSession {
  constructor(id, proc) {
    ...
    this.proc = proc;
    this.alive = true;
    this.pending = new Map(); // requestId -> { resolve, reject, command }
  ...
  _handleMessage(msg) {
    ...
    if (msg.type === 'response') {
      const pending = msg.id != null ? this.pending.get(msg.id) : null;
      if (pending) {
  ...
  send(command, params = {}, { timeout = 30000 } = {}) {
    return new Promise((resolve, reject) => {
      ...
      const timer = timeout > 0 ? setTimeout(() => {
        if (this.pending.delete(id)) reject(new Error(`RPC ${command} timed out after ${timeout}ms`));
      }, timeout) : null;
      this.pending.set(id, {
        command,
  ...
  async followUp(message) { return this.send('follow_up', { message }); }
