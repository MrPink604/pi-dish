...
function runCatalogCommand(harnessId) {
  ...
  return new Promise((resolve, reject) => {
    execFile(spec.argv[0], [...spec.argv.slice(1), ...descriptor.argv.models], {
...
async function refreshHarnessPricing(harnessId = 'omp', { force = false, now = Date.now() } = {}) {
  ...
  if (!state.inFlight) {
    ...
    state.inFlight = runCatalogCommand(harnessId).then(models => {
...
function pricingRevision(harnessId) {
  return harnessId === 'omp' ? (loadState(harnessId).snapshot?.revision || 'missing') : 'native';
}
