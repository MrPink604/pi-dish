...
function processAlive(pid) {
  if (!pid) return false;
  // A zombie has exited and cannot keep writing the session file, even though
  // kill(pid, 0) continues to report it until its parent reaps it.
  try {
 ...
 * writing one JSONL.
 */
async function killPaneAndWait(socket, paneId, { timeout = 5000 } = {}) {
  const pid = await paneProcessId(socket, paneId);
  if (!pid) {
    if (!await paneExists(socket, paneId)) return; // exited on its own
    throw new Error(`could not identify process for tmux pane ${paneId}`);
  }
  ...
  // actual JSONL writer; fallback is unsafe until both have exited.
  const paneProcesses = processTree(pid);
  const processesStillAlive = () => paneProcesses.filter(processAlive);
  ...
  try {
    ...
    if (await paneExists(socket, paneId) || processesStillAlive().length) throw e;
    return;
  }
  ...
  const paneStillExists = await paneExists(socket, paneId);
  const liveProcesses = processesStillAlive();
  throw new Error(`tmux pane ${paneId} cleanup did not complete (${paneStillExists ? 'pane still exists' : 'pane gone'}, ${liveProcesses.length ? `processes still alive: ${liveProcesses.join(', ')}` : 'processes gone'})`);
}
