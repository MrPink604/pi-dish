...
// Allow a socket only when it lives directly in the tmux tmpdir, or is one of
// the sockets listServers() actually found. The pi-dish server can be exposed
// on 0.0.0.0 — never let a LAN client pass an arbitrary `-S` path through.
async function isSocketAllowed(socket) {
  if (typeof socket !== 'string' || !socket) return false;
  const resolved = path.resolve(socket);
  if (path.dirname(resolved) === path.resolve(tmuxTmpdir())) return true;
  const servers = await listServers();
  return servers.some((s) => path.resolve(s.socket) === resolved);
}
...
async function pruneSpawns(registeredIds = new Set()) {
  const spawns = readSpawns();
  let changed = false;
  for (const [id, entry] of Object.entries(spawns)) {
    if (registeredIds.has(id)) continue;
    if (await paneExists(entry.socket, entry.paneId)) continue;
    delete spawns[id];
    changed = true;
  }
  if (changed) writeSpawns(spawns);
  return spawns;
}
