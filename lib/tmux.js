...
/**
 * Replace the command in one exact pane without changing its pane/window
 * identity. The caller supplies the pane-root birth identity it authorized;
 * re-checking it immediately before respawn prevents a reused pane id from
 * inheriting authority from an older agent.
 */
async function respawnPane({ socket, paneId, cwd, command, env, expectedProcess }) {
  if (!Array.isArray(command) || !command.length) throw new Error('command argv required');
...
// One key chord, no Enter and no literal flag: tmux resolves names like
// "C-M-S-F12" to the terminal encoding the pane's program expects. Unlike
// sendKeys this cannot land in a TUI's composer, so it is safe against a
// pane whose editor already holds an unsent draft.
