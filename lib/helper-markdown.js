...
exports.createMathExtensions = createMathExtensions;
...
function createMathExtensions(getKatex) {
    const blockMath = {
        ...
        renderer(token) {
            const k = getKatex();
            if (!k)
                return `<pre class="math-block"><code>${(0, helper_format_1.escapeHtml)(token.raw)}</code></pre>\n`;
            try {
                return `<div class="math-block">${k.renderToString(token.text, { displayMode: true, throwOnError: false })}</div>\n`;
            }
            catch (e) {
                return `<pre class="math-error"><code>${(0, helper_format_1.escapeHtml)(token.raw)}</code></pre>\n`;
            }
        },
    };
    const inlineMath = {
        ...
        renderer(token) {
            const k = getKatex();
            if (!k)
                return (0, helper_format_1.escapeHtml)(token.raw);
            try {
