...
const registry = {
  ...
  omp: {
    ...
    // web pilot can't do — it blocks the pane-injected tree service command
    // (navigation 504s) and garbles the embedded terminal view. Web-piloted
    // sessions don't need it: provider readiness is surfaced in the model
    // picker instead. OMP_SKIP_SETUP is OMP's own supported skip switch; an
    // explicit value in PI_DISH_OMP_COMMAND still wins.
    ...
    // OMP TUI pane (17.2.15 for /shake, 17.4.0 for the rest): they complete
    // with a status line, no overlay or follow-up interaction. Excluded:
    // /omfg opens a multi-step generation/save flow. WS-G excluded /handoff
    // because its in-pane switch left pi-dish on the old route; switch adoption
    // now closes that gap, but re-evaluating /handoff's host-command UX remains
    // separate work. /hotkeys is terminal-only; /review is a prompt command.
    // /branch, /tree, /debug, /pause open TUI selectors/screens; /plan, /vibe,
    // /goal, /loop are mode toggles whose follow-up UX (plan review, drafts)
    // is TUI-only, so toggling them from a phone strands the user.
    hostBuiltins: [{
      name: 'shake',
