...
function messageFromEntry(entry, candidate, fallbackModel = {}) {
    if (entry.type === 'message' && entry.message) {
        ...
        const estimated = usageCost(candidate, message.provider || fallbackModel.provider, message.responseModel || message.model || fallbackModel.model, message.usage);
        if (usage) {
            if (estimated)
            ...
            if (entry.type === 'model_change') {
                if (candidate?.profileId === 'omp-v1' && typeof entry.model === 'string') {
                    ...
                    if (slash > 0) {
                        ...
                        model = entry.model.slice(slash + 1);
                    }
                    else {
...
function readSessionMessages(filePath) {
    ...
    // JSONL re-parse.
    return statCached(messagesCache, filePath, 8, (fp, _stats, candidate) => parseMessageData(node_fs_1.default.readFileSync(fp, 'utf-8'), candidate)).messages;
}
...
function readSessionMessagesAtLeaf(filePath, leafId) {
    return statCached(messagesCache, filePath, 8, (fp, _stats, candidate) => parseMessageData(node_fs_1.default.readFileSync(fp, 'utf-8'), candidate, leafId), `leaf:${leafId ?? ''}`).messages;
}
function readSessionMessageById(filePath, entryId) {
    return statCached(messagesCache, filePath, 8, (fp, _stats, candidate) => parseMessageData(node_fs_1.default.readFileSync(fp, 'utf-8'), candidate)).byId.get(entryId) || null;
}
/**
...
function computeSessionStats(filePath, _stats, candidate = {}) {
    ...
    for (const line of node_fs_1.default.readFileSync(filePath, 'utf-8').split('\n')) {
        ...
        if (m.role === 'user')
            ...
            addReportedCosts(costBucket, usageCost(candidate, m.provider || provider, m.responseModel || m.model || model, m.usage));
    ...
    for (const raw of entries) {
        ...
        if (e.type === 'model_change') {
            if (profileId === 'omp-v1') {
                ...
                if (typeof e.provider === 'string' && e.provider)
                    provider = e.provider;
                if (typeof e.modelId === 'string' && e.modelId)
                    model = e.modelId;
            }
        }
