...
function statCached(cache, input, max, parse) {
  ...
  const cacheKey = `${filePath}\0${candidate.profileId || 'pi-v3'}\0${candidate.profileVersion ?? 1}\0${pricingRevision(candidate.harnessId)}`;
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
    try {
      ...
      if (!firstUserMsg && entry.type === 'custom_message' &&
          entry.customType === 'session-message') firstUserMsg = entry.content;
      if (entry.timestamp) {
...
function readSessionMessagesAtLeaf(filePath, leafId) {
  ...
  return parseMessageData(fs.readFileSync(candidate.file, 'utf-8'), candidate, leafId).messages;
}
  ...
  for (const entry of entries) {
    try {
      ...
      if (entry.type === 'custom_message' && entry.content &&
        ...
        parts.push(extractTextContent(entry.content).substring(0, 200));
      }
...
function usageCost(candidate, provider, model, usage) {
  ...
  return candidate?.harnessId === 'omp' ? estimated : (estimated || reportedCost(provider, usage));
}
...
function buildIndexedUsageFromContent(content, candidate = {}) {
  ...
  for (const line of content.split('\n')) {
    ...
    const usage = { ...m.usage, cost: usageCost(candidate, p, mid, m.usage) };
