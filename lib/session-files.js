...
function messageFromEntry(entry, candidate, fallbackModel = {}) {
  if (entry.type === 'message' && entry.message) {
    ...
    if (usage && candidate?.harnessId === 'omp') {
 ...
 * so it gets a generous per-message cap — corpus-measured, prose rarely
 * exceeds 2K chars, so this costs little while the old universal 500 left
 * most of a long answer unfindable. Tool results are bulky low-recall dumps
 * (file reads, build output): they stay tight. The session cap bounds one
 * transcript's total text — the whole corpus's text lives in memory
 * (session-index), sized for thousands of sessions.
 */
const SEARCH_TEXT_PROSE_CAP = 10_000;
const SEARCH_TEXT_TOOL_RESULT_CAP = 500;
const SEARCH_TEXT_TOOL_CALL_CAP = 300;
const SEARCH_TEXT_SESSION_CAP = 1_000_000;
...
function searchTextFromEntries(entries, active) {
  ...
  let total = 0;
  const push = (part) => {
    if (!part || total >= SEARCH_TEXT_SESSION_CAP) return;
    if (part.length > SEARCH_TEXT_SESSION_CAP - total) {
      part = part.substring(0, SEARCH_TEXT_SESSION_CAP - total);
    }
    parts.push(part);
    total += part.length + 1;
  };
