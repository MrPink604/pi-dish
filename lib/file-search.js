...
async function searchFiles(basePath, query, limit = 20) {
  ...
  return walkerSearch(basePath, query, limit, { files: true });
}
...
function searchHomeDirs(query, limit = 15) {
  ...
  if (!homeDirCache || Date.now() - homeDirCache.at > DIR_CACHE_MS) {
    homeDirCache = { at: Date.now(), dirs: walkDirs(home, 4) };
  }
...
function walkerSearch(basePath, query, limit, opts) {
  const out = [];
  const stack = [[basePath, 0]];
  let seen = 0;
  while (stack.length && seen < MAX_WALK) {
    const [dir, depth] = stack.pop();
    let entries;
    ...
    for (const e of entries) {
      ...
      seen++;
      if (e.isDirectory()) {
        if (depth + 1 < 6) stack.push([full, depth + 1]);
      } else if (opts.files) {
