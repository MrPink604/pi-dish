...
exports.searchHomeDirs = searchHomeDirs;
exports.getDirChildren = getDirChildren;
exports.isPathCompletionToken = isPathCompletionToken;
...
function loadFff() {
    if (!fffModulePromise) {
        fffModulePromise = (async () => import((0, runtime_resources_1.fffImportSpecifier)(path.resolve(__dirname, '..'))))().catch((e) => {
            console.warn('fff unavailable, falling back to walker:', (0, helper_values_1.record)(e) ? e.message : undefined);
            return null;
        });
    }
...
async function searchFiles(basePath, query, limit = 20) {
    ...
    if (finder) {
        ...
        try {
            ...
            console.warn(`fff fileSearch threw for ${basePath}: ${(0, helper_values_1.record)(e) ? e.message : undefined}`);
        }
    }
...
function searchHomeDirs(query, limit = 15) {
    ...
    return candidates
        .map(c => ({ ...c, score: (0, helper_query_js_1.fuzzyScore)((0, helper_query_js_1.fuzzyMatch)(query, c.short), c.short) }))
 ...
 */
function getDirChildren(inputPath) {
    const resolved = expandHome(String(inputPath || ''));
    if (!path.isAbsolute(resolved)) {
        const e = new Error('an absolute path (or ~) is required');
        e.badRequest = true;
        throw e;
    }
    let entries;
    try {
        ...
        return { path: resolved, dirs: [], error: (0, helper_values_1.record)(e) && typeof e.message === 'string' ? e.message : undefined };
    }
    const dirs = entries
...
function isPathCompletionToken(token) {
    return /^(?:\/|~(?:\/|$)|\.\.?\/)/.test(token);
}
/**
 ...
 */
function completePath(token, { cwd = null, limit = 20 } = {}) {
    const slash = token.lastIndexOf('/');
    const prefix = slash === -1 ? token + '/' : token.slice(0, slash + 1); // bare '~'
    const partial = slash === -1 ? '' : token.slice(slash + 1);
    let dir;
    if (prefix.startsWith('~'))
        dir = path.join(os.homedir(), prefix.slice(1));
    else if (prefix.startsWith('/'))
        dir = prefix;
    else if (cwd)
        dir = path.resolve(cwd, prefix); // ./ and ../ need a session cwd
    else
        return [];
    let entries;
    try {
    ...
    const picked = !partial
        ? candidates
            ...
            .slice(0, limit)
        : candidates
            .map(c => ({ ...c, score: (0, helper_query_js_1.fuzzyScore)((0, helper_query_js_1.fuzzyMatch)(partial, c.name), c.name) }))
            .filter(c => c.score > -Infinity)
            .sort((a, b) => b.score - a.score)
            .slice(0, limit);
...
function walkerSearch(basePath, query, limit) {
    ...
    if (!query)
        return out.slice(0, limit).map(p => ({ path: p, gitStatus: null }));
    return out
        .map(p => ({ path: p, score: (0, helper_query_js_1.fuzzyScore)((0, helper_query_js_1.fuzzyMatch)(query, p), p) }))
        .filter(c => c.score > -Infinity)
        .sort((a, b) => b.score - a.score)
        .slice(0, limit)
