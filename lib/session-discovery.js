...
function candidateForFile(file, workspaceDirName, depth, descriptor) {
    ...
    if (basename !== 'session') {
        ...
        if (!fs.existsSync(parentSession) || !readSessionHeader(parentSession, descriptor.profileId))
            return null;
 ...
 */
function artifactCandidateForFile(file, dirName, depth, descriptor, parentFallback) {
    const header = readSessionHeader(file, descriptor.profileId);
    const id = safeHeaderSessionId(header?.id);
    if (!id)
        return null;
...
function inspectSubsessionExits(parentFile, options = {}) {
    ...
    const { readSessionTailEntry } = require('./session-files');
        ...
        try {
            ...
            while (!capped && (entry = dir.readSync())) {
                ...
                if (entry.isDirectory()) {
                    ...
                    let tail;
                    try {
                        tail = readSessionTailEntry({ file, profileId: descriptor.profileId });
                    }
                    catch { }
                    if (!tail)
