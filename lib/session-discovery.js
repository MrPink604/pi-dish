...
function readSessionHeader(filePath, profileId = 'pi-v3') {
    ...
    try {
        ...
        if (cached && cached.stat.mtimeMs === stat.mtimeMs && cached.stat.size === stat.size &&
            cached.stat.ctimeMs === stat.ctimeMs && cached.stat.dev === stat.dev && cached.stat.ino === stat.ino)
        ...
        for (let start = 0; start < n;) {
            ...
            if ((0, wire_protocol_1.isRecord)(entry) && entry.type === 'session') {
                header = entry;
                break;
            }
            if (profileId !== 'omp-v1')
...
function safeHeaderSessionId(value) {
    return (0, session_key_1.validSessionId)(value) ? value : null;
}
/** The harness/profile identity every discovered candidate carries. */
function decorateCandidate(candidate, descriptor, options = {}) {
    if (!(0, session_key_1.validSessionId)(candidate.id))
        throw new TypeError('Invalid session identity');
    const sessionKey = (0, session_key_1.encodeSessionKey)(descriptor.id, candidate.id);
    return {
        file: candidate.file,
...
function candidateForFile(file, workspaceDirName, depth, descriptor, readHeader = readSessionHeader) {
    ...
    if (basename !== 'session') {
        ...
        if (!descriptor.nestedSubsessions)
            return null;
        const parentSession = `${path.dirname(file)}.jsonl`;
        if (!readHeader(parentSession, descriptor.profileId))
            return null;
 ...
 */
function artifactCandidateForFile(file, dirName, depth, descriptor, parentFallback, readHeader = readSessionHeader) {
    const header = readHeader(file, descriptor.profileId);
    const id = safeHeaderSessionId(header?.id);
    if (!id)
        return null;
...
function artifactParentFallback(walkRoot, dirPath, rootParent) {
    const segments = path.relative(walkRoot, dirPath).split(path.sep);
    if (!segments[segments.length - 1]?.startsWith('sub-'))
        return null;
    if (segments.length === 1)
        return rootParent(null);
    if (segments.length === 2)
        return rootParent(segments[0]);
    return null;
}
/**
...
function discoverSessionCandidates(rootDir, options = {}) {
    const descriptor = options.descriptor || (0, harnesses_1.getHarness)(options.harnessId || 'pi');
    if (!descriptor)
    ...
    const add = (hint) => {
        if (!hint)
            return;
        if (!(0, session_key_1.validSessionId)(hint.id)) {
            skipped += 1;
        ...
        if (excludeIds.has(candidate.nativeSessionId) || ambiguousHeaderIds.has(candidate.nativeSessionId))
            return;
    ...
    if (descriptor.subagentArtifacts) {
        ...
        const walkArtifacts = (dirPath, depth) => {
            if (truncated || depth > DEFAULT_ARTIFACTS_DEPTH)
                return;
            eachEntry(dirPath, (entry) => {
                if (truncated)
                    return;
                ...
                if (entry.isFile() && entry.name.endsWith('.jsonl')) {
                    ...
                    filesSeen += 1;
                    add(artifactCandidateForFile(full, dirName, depth, descriptor, artifactParentFallback(artifactsRoot, dirPath, (topId) => {
                        if (topId === null)
                            return null;
...
function discoverHarnessSessions(descriptors = (0, harnesses_1.listHarnesses)(), options = {}) {
    const candidates = [];
    let truncated = false;
    let skipped = 0;
...
function findSessionCandidate(rootDir, sessionId, options = {}) {
    ...
    if (exact)
        return { candidate: exact, truncated, skipped };
    if (options.allowPartial === false)
        return { candidate: null, truncated, skipped };
...
function discoverSubsessionCandidates(parentFile, options = {}) {
    ...
    if (!descriptor)
        throw new TypeError('Unknown harness descriptor');
    if (typeof parentFile !== 'string' || !parentFile.endsWith('.jsonl'))
        return [];
    if (descriptor.subagentArtifacts)
        return discoverArtifactSubsessions(parentFile, descriptor, options);
    if (!descriptor.nestedSubsessions)
        return [];
    ...
    const walk = (dirPath, depth) => {
        ...
        for (const entry of entries) {
            ...
            if (!candidate || !(0, session_key_1.validSessionId)(candidate.id))
                continue;
            ...
            if (byId.has(candidate.id)) {
                ...
                ambiguous.add(candidate.id);
            }
            if (!ambiguous.has(candidate.id))
                byId.set(candidate.id, decorateCandidate(candidate, descriptor, options));
 ...
 */
function discoverArtifactSubsessions(parentFile, descriptor, options) {
    const maxFiles = positiveInt(options.maxFiles, DEFAULT_SUBSESSION_FILES);
    ...
    let files = 0;
    const walk = (dirPath, depth) => {
        if (depth > DEFAULT_ARTIFACTS_DEPTH || files >= maxFiles)
            return;
        try {
            if (!fs.lstatSync(dirPath).isDirectory())
                return;
        ...
        for (const entry of entries) {
            ...
            if (!entry.isFile() || !entry.name.endsWith('.jsonl'))
                continue;
            files += 1;
            const candidate = artifactCandidateForFile(full, dirName, depth, descriptor, artifactParentFallback(artifactsDir, dirPath, (topId) => topId === null ? parentFile : null));
            if (!candidate)
                continue;
 ...
 */
function inspectSubsessionExits(parentFile, options = {}) {
    const descriptor = options.descriptor || (0, harnesses_1.getHarness)(options.harnessId || 'omp');
    const blockers = [];
    let complete = true;
    const unknown = (reason) => { complete = false; blockers.push(reason); };
    ...
    const validFile = (file) => {
        try {
            ...
            const id = safeHeaderSessionId(header?.id);
            if (!id)
                throw new Error('missing valid session header');
            if (ids.has(id))
                throw new Error('ambiguous session header');
            ids.add(id);
            return true;
        }
        catch (error) {
    ...
    if (!validFile(parentFile))
        return { complete, blockers };
    const walk = (dirPath, depth, optional = false) => {
        if (capped)
            return;
        let dir;
        try {
            ...
            try {
                ...
                if (optional && (0, wire_protocol_1.isRecord)(error) && error.code === 'ENOENT')
                    return;
                throw error;
            }
            ...
            if (depth > maxDepth) {
                capped = true;
                throw new Error('descendant depth limit reached');
            }
            dir = fs.opendirSync(dirPath);
            let entry;
            while (!capped && (entry = dir.readSync())) {
                if (++entries > maxEntries) {
                    capped = true;
                    throw new Error('descendant entry limit reached');
                }
                const file = path.join(dirPath, entry.name);
                if (entry.isSymbolicLink()) {
                    unknown(`Cannot inspect symbolic link ${entry.name}.`);
                    continue;
                }
                if (entry.isDirectory()) {
                    walk(file, depth + 1);
                    ...
                    if (++files > maxFiles) {
                        capped = true;
                        throw new Error('descendant file limit reached');
                    }
                    ...
                    let tail;
                    try {
                        tail = (0, session_files_1.readSessionTailEntry)({ file, profileId: descriptor.profileId });
                    }
                    catch { }
                    if (!tail)
                        unknown(`Cannot determine exit state of ${entry.name}.`);
                    else if (!(0, wire_protocol_1.isRecord)(tail) || tail.type !== 'custom' || tail.customType !== descriptor.sessionExitCustomType) {
            ...
            if (dir)
                try {
                    ...
                    unknown('Cannot finish descendant directory inspection.');
                }
        }
    };
...
function invalidateSessionHeader(file) {
    for (const key of headerCache.keys()) {
        if (key.slice(key.indexOf('\0') + 1) === file)
            headerCache.delete(key);
    }
}
function clearSessionHeaders() { headerCache.clear(); }
module.exports = {
