...
function inspectSubsessionExits(parentFile, options = {}) {
  ...
  const validFile = file => {
    try {
      ...
      const header = readSessionHeader(file, descriptor.profileId);
      if (!safeHeaderSessionId(header?.id)) throw new Error('missing valid session header');
      if (ids.has(header.id)) throw new Error('ambiguous session header');
      ids.add(header.id);
      return true;
    } catch (error) {
      unknown(`Cannot inspect ${path.basename(file)}: ${error.message}`);
      return false;
    }
  };
    ...
    try {
      ...
      while (!capped && (entry = dir.readSync())) {
        ...
        if (entry.isDirectory()) {
          ...
          else if (tail.type !== 'custom' || tail.customType !== descriptor.sessionExitCustomType) {
            blockers.push(`Waiting for ${entry.name} to exit (idle children still count).`);
          }
        }
      }
    } catch (error) {
      unknown(`Cannot completely inspect descendants: ${error.message}`);
    } finally {
