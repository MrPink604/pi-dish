...
exports.normalizeMention = normalizeMention;
exports.extractSessionPaths = extractSessionPaths;
exports.resolveFileMention = resolveFileMention;
exports.readFileForViewer = readFileForViewer;
/**
...
const command_paths_1 = require("./command-paths");
...
function extractSessionPaths(messages, cwd, home = os.homedir()) {
    ...
    messages.forEach((msg, idx) => {
        ...
        for (const block of content) {
            if (!(0, helper_values_1.record)(block) || block.type !== 'toolCall')
                continue;
            ...
            for (const key of PATH_ARG_KEYS) {
                const v = args[key];
                if (typeof v !== 'string' || !v || v.length > 1024)
                    continue;
                ...
                paths.set(abs, idx);
                if (key !== 'cwd')
                    paths.set(path.dirname(abs), idx);
            }
            if (typeof args.command === 'string') {
                for (const m of (0, command_paths_1.matchCommandPaths)(args.command)) {
...
async function resolveFileMention(rawMention, { cwd, messages, home = os.homedir() }) {
    ...
    const allowed = (p) => underCwd(p) || paths.has(p) || byRecency.some(b => p.startsWith(b + path.sep));
    const isFile = (p) => { try {
    ...
    for (const c of [...new Set(candidates)]) {
        if (isFile(c) && allowed(c))
            return { absPath: c, line };
    }
    if (normCwd) {
        const results = await (0, file_search_1.searchFiles)(normCwd, mention, 20).catch(() => []);
        const base = path.basename(mention);
        const hit = results.find(r => r.path === mention || r.path.endsWith('/' + mention) || path.basename(r.path) === base);
        if (hit) {
            const abs = path.resolve(normCwd, hit.path);
            if (isFile(abs) && allowed(abs))
                return { absPath: abs, line };
        }
    }
    return null;
}
 ...
 */
function readFileForViewer(absPath, { imageData = 'base64' } = {}) {
    const stats = fs.statSync(absPath);
    const mime = IMAGE_MIME[path.extname(absPath).toLowerCase()];
    if (mime) {
        if (stats.size > MAX_IMAGE_BYTES)
            return { error: 'Image too large to preview', status: 413 };
        const image = { mimeType: mime };
        if (imageData === 'base64')
            image.data = fs.readFileSync(absPath).toString('base64');
        else if (imageData === 'buffer')
            image.buffer = fs.readFileSync(absPath);
        return {
            image,
