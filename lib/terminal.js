 ...
 *
 * `command` (argv array) runs instead of the login shell — the tmux-attach
 * terminal mode uses it. `env` overlays the inherited environment; keys set
 * to undefined are removed (attaching tmux from a server that itself runs
 * inside tmux needs $TMUX gone or tmux refuses the nested attach). `meta`
 * fields ride on every attach frame sent to clients.
 */
...
function attachClient(sessionId, cwd, ws, opts) {
  ...
  ws.send(JSON.stringify({ type: 'attach', replay: term.buffer, cwd: term.cwd, ...(term.meta || {}) }));
...
function restartTerminal(sessionId, cwd, opts) {
  ...
  const payload = JSON.stringify({ type: 'attach', replay: '', cwd: term.cwd, ...(term.meta || {}) });
