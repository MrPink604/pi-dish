...
test('autocomplete wire data narrows command and file fields without mutating input', () => {
  ...
  const commands = decodeSlashCommands(wire); const command = commands[0]; assert.ok(command); assert.equal(commands.length, 1); assert.equal(command.args, ''); assert.equal(command.description, '');
  const files = decodeFileCompletions([{ path: 'a', isDir: 'yes', gitStatus: '__proto__' }, { path: 1 }]); const file = files[0]; assert.ok(file); assert.equal(files.length, 1); assert.equal(file.isDir, false);
  const source = wire[2]; assert.ok(source); assert.equal(source.source, 'extension');
});
test('session refs use target-host naming and keep exact host-id references unambiguous', () => {
  ...
  const hosts = {
    self: { hostId: 'self', key: 'self', source: 'self' as const, base: '', capabilities: {} },
    peer: { hostId: 'peer', key: 'peer', source: 'fleet' as const, base: 'http://peer', name: 'named-peer', capabilities: {} },
  };
  ...
  const matched = refs.match('same-id'); assert.ok(matched); assert.equal(matched.host, 'peer'); assert.equal(refs.candidates().filter(row => row.id === 'same-id').length, 1);
