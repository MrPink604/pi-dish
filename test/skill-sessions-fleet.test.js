// Generated test/tool from test/skill-sessions-fleet.test.ts; edit that source and run npm run build:tests.
"use strict";
...
test('hosts lists self and every remote, with reachability and capabilities', async () => {
    ...
    const self = (0, test_types_js_1.present)(lines[0]), peerLine = (0, test_types_js_1.present)(lines[1]), downLine = (0, test_types_js_1.present)(lines[2]);
    ...
    assert.match((0, test_types_js_1.present)(lines.at(-1)), /^# Add --host <name>/);
    const parsed = (0, test_types_js_1.record)(JSON.parse((await runCli(['hosts', '--json'])).stdout));
    const hosts = (0, test_types_js_1.records)(parsed.hosts);
    assert.deepEqual(hosts.map(host => host.name), [null, 'peer', 'down']);
    assert.equal((0, test_types_js_1.present)(hosts[0]).self, true);
});
test('--host routes a session command to that host, default stays local', async () => {
    ...
    const shown = (0, test_types_js_1.record)(JSON.parse((await runCli(['show', PEER_SESSION_ID, '--limit', '5', '--host', 'peer'])).stdout));
    assert.equal((0, test_types_js_1.record)(shown.session).id, PEER_SESSION_ID);
    const messages = (0, test_types_js_1.records)(shown.messages), final = (0, test_types_js_1.present)(messages.at(-1));
    assert.equal((0, test_types_js_1.record)((0, test_types_js_1.present)((0, test_types_js_1.records)(final.content)[0])).text, 'peer fixture reply');
...
test('an unknown host names the fleet instead of leaking a bare 404', async () => {
    await assert.rejects(runCli(['list', '--host', 'nope']), (error) => {
        const stderr = String((0, test_types_js_1.record)(error).stderr ?? '');
    ...
    await assert.rejects(runCli(['related', 'no-such-session', '--host', 'peer']), (error) => {
        assert.equal(/unknown host/.test(String((0, test_types_js_1.record)(error).stderr ?? '')), false);
        return true;
    });
});
test('PI_DISH_TOKEN is what gets the CLI through a token-gated server', async () => {
    ...
    await assert.rejects(runCli(['hosts'], { token: null }), (error) => { assert.match(String((0, test_types_js_1.record)(error).stderr ?? ''), /Unauthorized/); return true; });
});
function fakeHub({ rejectProvenance }) {
    ...
    const server = http.createServer((req, res) => {
        ...
        req.on('end', () => {
            ...
            if (req.url === '/api/host')
                return res.end(JSON.stringify({ hostId: 'aaaaaaaa-1111-4111-8111-aaaaaaaaaaaa', label: 'local', capabilities: {} }));
            ...
            if (req.url === '/hosts/peer/api/session-spawns/spawn-1')
                return res.end(JSON.stringify({ status: 'ready', sessionId: 'peer-new-1' }));
            res.statusCode = 404;
...
test('a cross-host spawn attributes the caller host-qualified', async (t) => {
    ...
    if (!address || typeof address === 'string')
        throw new Error('Fake hub lacks TCP address');
    const url = `http://127.0.0.1:${address.port}`;
    const spawned = await runCli(['spawn', '--host', 'peer', '--cwd', '/work', '--json'], { url, token: null, home: os.tmpdir() });
    const result = (0, test_types_js_1.record)(JSON.parse(spawned.stdout));
    assert.equal(result.sessionId, 'peer-new-1');
    assert.equal(result.host, 'peer');
    const create = (0, test_types_js_1.present)(requests.find(request => request.url === '/hosts/peer/api/sessions/new'));
    const body = (0, test_types_js_1.present)(create.body);
...
test('a peer that refuses foreign provenance still gets the session spawned', async (t) => {
    ...
    assert.equal((0, test_types_js_1.record)(JSON.parse(spawned.stdout)).sessionId, 'peer-new-1');
    ...
    assert.equal((0, test_types_js_1.present)((0, test_types_js_1.present)(creates[1]).body).requestedBySessionId, undefined);
});
