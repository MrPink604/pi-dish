// Generated test/tool from test/session-api.test.ts; edit that source and run npm run build:tests.
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const test = require("node:test");
const assert = require("node:assert/strict");
const test_types_js_1 = require("./test-types.js");
const { normalizeModels, sessionForClient, decodeSessionMetadata, decodeSessionList, thinkingResult, decodeModelCatalog, decodeMutationResult, decodeThinkingResult, decodeEnabledModelsResult, decodeSessionRow, decodeSessionMutationPatch, decodeSessionActivityPatch, decodeSessionTranscriptPatch } = require('../lib/session-api');
test('closed session ingress separates authority, endpoint identity and opaque peer extras', () => {
    ...
    const capabilities = (0, test_types_js_1.present)(row.fields.capabilities);
    assert.equal(capabilities.resume, undefined, 'partial capabilities retain legacy fallback');
    assert.equal(capabilities.close, false);
    assert.equal(capabilities.__proto__, false);
...
test('metadata patches preserve presence and permit only their assigned writers', () => {
    ...
    assert.deepEqual(decodeSessionTranscriptPatch({ id: 'new', harnessId: 'other', name: null, cwd: null,
        contextTokens: 0, lastActivity: 0, isActive: false, parentId: 'new', routine: 'new', capabilities: { close: true } }), { name: null, cwd: null, contextTokens: 0, lastActivity: 0, isActive: false });
...
test('client projection preserves feature hints and wire ingress validates control fields', () => {
    ...
    assert.deepEqual(decodeSessionList({ active: [], previous: [], children: [{ id: 'child' }], indexing: true }).children, [{ id: 'child', fields: {}, extras: {} }]);
});
...
test('live native thinking efforts retain exact ladders without inventing unknown levels', () => {
    const normalize = (thinking) => normalizeModels([{ id: 'm', provider: 'omp',
...
test('mutation decoders require success and validate operation-specific results', () => {
    ...
    for (const value of [{}, { success: false }, { success: 'true' }, null])
        assert.throws(() => decodeMutationResult(value));
