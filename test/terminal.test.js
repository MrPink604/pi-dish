// Generated test/tool from test/terminal.test.ts; edit that source and run npm run build:tests.
"use strict";
...
function fakeWebSocket() {
    const candidate = Reflect.construct(WebSocket, [null, undefined, { autoPong: false, closeTimeout: 1000 }]);
    if (!(candidate instanceof WebSocket))
        throw new Error('Failed to construct WebSocket fixture');
    const ws = candidate;
    const sent = [];
    Object.defineProperties(ws, {
        readyState: { configurable: true, value: WebSocket.OPEN },
        send: { configurable: true, value(payload) { sent.push((0, test_types_js_1.record)(JSON.parse(String(payload)))); } },
    ...
    return Object.assign(ws, {
        sent,
        messages(type) { return sent.filter(message => message.type === type); },
        input(data) { ws.emit('message', JSON.stringify({ type: 'input', data })); },
    });
}
...
test('attachClient with a command argv runs it, applies the env overlay, and stamps meta on attach', async () => {
    ...
    const attach = (0, test_types_js_1.present)(ws.messages('attach')[0]);
...
test('reattach replays buffered output after the first client drops', async () => {
    ...
    const attach = (0, test_types_js_1.present)(ws2.messages('attach')[0]);
...
test('ring buffer trims to bufferMax', async () => {
    ...
    const term = (0, test_types_js_1.present)(terminal._terminals.get('lib-ring'));
...
test('idle kill defers while a detached shell is still producing output', async () => {
    ...
    const term = (0, test_types_js_1.present)(terminal._terminals.get('lib-busy'));
...
test('restart frame respawns the shell, keeping the attached client', async () => {
    ...
    const oldPid = (0, test_types_js_1.present)(terminal._terminals.get('lib-restart')).proc.pid;
    ws.emit('message', JSON.stringify({ type: 'restart' }));
    await until(() => ws.messages('attach').length === 2);
    assert.equal((0, test_types_js_1.present)(ws.messages('attach')[1]).replay, '', 'fresh shell attaches with an empty buffer');
    assert.equal(ws.messages('exit').length, 0, 'surviving client gets no exit frame');
    const term = (0, test_types_js_1.present)(terminal._terminals.get('lib-restart'));
...
test('client exit frame is sent when the shell exits', async () => {
    ...
    assert.equal((0, test_types_js_1.present)(ws.messages('exit')[0]).code, 7);
    await until(() => !terminal._terminals.has('lib-exit'));
});
test('GET /api/config reports the terminal feature enabled', async () => {
    const res = await fetch(base + '/api/config');
    const cfg = (0, test_types_js_1.record)(await res.json());
...
test('WS endpoint: end-to-end echo through a real WebSocket', async () => {
    ...
    const frames = [];
    ws.onmessage = event => frames.push((0, test_types_js_1.record)(JSON.parse(String(event.data))));
    await new Promise((resolve, reject) => { ws.onopen = () => resolve(); ws.onerror = reject; });
    await until(() => frames.some(frame => frame.type === 'attach'));
    assert.equal((0, test_types_js_1.present)(frames[0]).cwd, sessionCwd, 'PTY starts at the session cwd');
...
test('WS endpoint: mode=tmux without a locatable pane sends an error frame and closes', async () => {
    ...
    await closed;
    assert.ok(frames.some(frame => frame.type === 'error' && /tmux pane/i.test(frame.error ?? '')), `expected a no-pane error frame, got ${JSON.stringify(frames)}`);
});
test('WS endpoint: native failures preserve absent and function-carried messages', async (t) => {
    ...
    for (const [failure, expected] of [
        ...
        const closed = await new Promise((resolve, reject) => {
            ws.onclose = event => {
                if (!(0, test_types_js_1.record)(event) || typeof event.code !== 'number')
                    reject(new Error('WebSocket close event lacks a code'));
                else
...
test('WS endpoint: mode=tmux attaches a grouped viewer of the owning pane and cleans up', { skip: !tmuxOk, timeout: 30000 }, async () => {
    ...
    (0, node_child_process_1.execFileSync)('tmux', ['-S', sock, '-f', '/dev/null', 'new-session', '-d', '-s', 'owner', '-x', '80', '-y', '24']);
    const paneId = (0, node_child_process_1.execFileSync)('tmux', ['-S', sock, 'list-panes', '-t', 'owner:0', '-F', '#{pane_id}'], { encoding: 'utf8' }).trim();
    // Arithmetic marker (the echoed input can't satisfy the assertion) printed
    // *in the owning pane* — the viewer must render it.
    (0, node_child_process_1.execFileSync)('tmux', ['-S', sock, 'send-keys', '-t', paneId, 'echo pane-marker-$((40+2))', 'Enter']);
    ...
    await new Promise(resolve => bridge.listen(bridgeSock, resolve));
    ...
    try {
        ...
        await until(() => frames.some(frame => frame.type === 'attach'));
        assert.equal((0, test_types_js_1.present)(frames[0]).tmuxPrefix, 'C-b', 'attach frame carries the server prefix (config-less default)');
        ...
        const sessions = (0, node_child_process_1.execFileSync)('tmux', ['-S', sock, 'list-sessions', '-F', '#{session_name}'], { encoding: 'utf8' })
            .trim().split('\n');
        assert.ok(sessions.some(s => s.startsWith('dish-view-')), `viewer session exists (got ${sessions})`);
        assert.equal((0, node_child_process_1.execFileSync)('tmux', ['-S', sock, 'list-clients', '-t', '=owner', '-F', 'x'], { encoding: 'utf8' }).trim(), '', 'no client attached to the owner session itself');
        ...
        await until(() => {
            const left = (0, node_child_process_1.execFileSync)('tmux', ['-S', sock, 'list-sessions', '-F', '#{session_name}'], { encoding: 'utf8' })
                .trim().split('\n');
        ...
        bridge.close();
        try {
            (0, node_child_process_1.execFileSync)('tmux', ['-S', sock, 'kill-server'], { stdio: 'ignore' });
        }
        catch { }
    }
});
