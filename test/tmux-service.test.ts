...
async function waitFor(predicate: () => boolean, label: string, timeout = 5000) {
...
function fixtureEnv(root: string, port: number): NodeJS.ProcessEnv {
  return {
    HOME: root,
...
test('a second supervisor on the same endpoint exits instead of looping', async t => {
  ...
  t.after(() => {
    if (first.pid !== undefined) { try { process.kill(-first.pid, 'SIGKILL'); } catch {} }
    first.kill('SIGKILL');
  });
...
test('tmux service manager ignores the caller tmux socket', t => {
  ...
  const result = spawnSync(path.join(temp, 'scripts', 'pi-dish-tmux.sh'), ['status'], {
    cwd: temp,
    env: {
