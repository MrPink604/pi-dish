...
import { bridgeSupports, sessionCapabilities } from '../lib/session-capabilities.js';
...
test('Pi retains legacy defaults while alternative bridges require exact true', () => {
  ...
  for (const harness of ['pi', 'omp', 'prime'] as const) {
...
test('active projections distinguish absent flags from explicit denials', () => {
  ...
  for (const key of ['prompt', 'tree', 'rename'] as const) assert.equal(deniedPi[key], false);
});
...
test('close and restart advice requires active state and separate ownership inputs', () => {
  for (const harness of ['pi', 'omp', 'prime'] as const) {
    ...
    const unownedFlags: AdvertisedCapabilities & { close: boolean; restart: boolean } = { close: true, restart: true };
    const unowned = sessionCapabilities(harness, unownedFlags, { active: true });
...
test('a registry conflict denies every projected operation, including history and lifecycle', () => {
  const flags = Object.fromEntries(keys.map(key => [key, true] as const));
  for (const harness of ['pi', 'omp', 'prime'] as const) for (const active of [true, false]) {
