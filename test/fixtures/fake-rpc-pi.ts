...
if (!home) throw new Error('Fake RPC fixture requires HOME');
...
let sessionFile: string, sessionId: string;
...
let turnOpen = false;
let abortTurn: (() => void) | null = null; // set while a turn is open
let holdCompaction = false;
const heldCompactions: Array<() => void> = [];

const out = (obj: unknown) => process.stdout.write(JSON.stringify(obj) + '\n');
const respond = (id: string, data: unknown) => out({ type: 'response', id, success: true, data });
const fail = (id: string, error: unknown) => out({ type: 'response', id, success: false, error });
const logCmd = (cmd: unknown) => {
...
function handle(cmd: unknown) {
  logCmd(cmd);
  if (!isRecord(cmd) || typeof cmd.id !== 'string' || typeof cmd.type !== 'string') return;
  const { id, type } = cmd;
  switch (type) {
    case 'get_state': {
    ...
    case 'prompt': {
      ...
      if (!turnOpen) void runTurn(typeof cmd.message === 'string' ? cmd.message : '');
      return;
    }
    case 'steer':
      return respond(id, {});
    ...
    case 'set_model':
      if (typeof cmd.provider !== 'string' || typeof cmd.modelId !== 'string') return fail(id, 'invalid model');
