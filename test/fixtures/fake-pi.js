#!/usr/bin/env node
// Generated test/tool from test/fixtures/fake-pi.ts; edit that source and run npm run build:tests.
"use strict";
...
function main() {
    ...
    if (extensionIndex >= 0 && args[extensionIndex + 1]) {
        try {
            ...
            if (match)
                wrapperToken = JSON.parse(match[1]);
        }
        catch { }
    }
    ...
    if (harnessId === 'prime' && !process.env.PI_FIXTURE_PRIME_WORKER) {
        ...
        const worker = (0, node_child_process_1.spawn)(process.execPath, [__filename, ...args], {
            detached: true,
            stdio: 'ignore',
            env: childEnv,
        });
        worker.unref();
        if (process.env.PI_FIXTURE_PRIME_BROKER)
            return;
        setInterval(() => { }, 1 << 30);
        return;
    }
    ...
    if (process.env.PI_FIXTURE_NOREGISTER) {
        if (process.env.PI_FIXTURE_SURVIVOR_FILE) {
            const child = (0, node_child_process_1.spawn)(process.execPath, [path.join(__dirname, 'idle-child.js'), '--ignore-sighup'], {
                detached: true,
                stdio: 'ignore',
            });
        ...
        if (registerDelay > 0)
            setTimeout(register, registerDelay);
        else
            register();
        setInterval(() => { }, 1 << 30);
    }
