...
if (harnessId === 'omp' && args[0] === 'models' && args.includes('--json')) {
  ...
  const model = (provider: string, id: string, thinking: unknown, name = id) => ({
...
if (harnessId === 'omp' && args[0] === 'config' && args.includes('--json')) {
  ...
  const AGENT_KEYS: Record<string, { type: string; empty: unknown[] | Record<string, unknown> }> = {
  ...
  if (args[1] === 'get') {
    ...
    const values: Record<string, { key: string | undefined; value: unknown; type: string; description: string }> = {
...
if (process.env.PI_FIXTURE_NOREGISTER) {
  if (process.env.PI_FIXTURE_SURVIVOR_FILE) {
    const child = spawn(process.execPath, [path.join(__dirname, 'idle-child.js'), '--ignore-sighup'], {
      detached: true,
      stdio: 'ignore',
    });
