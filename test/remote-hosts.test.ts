...
const packageData: unknown = require('../package.json');
const PKG_VERSION = record(packageData).version;
if (typeof PKG_VERSION !== 'string') throw new Error('Package version is unavailable');
...
function writeSettings(home: string, settings: unknown) {
...
function listenHttp(server: http.Server, port = 0): Promise<number> {
  return new Promise((resolve, reject) => {
    server.once('error', reject);
    server.listen(port, '127.0.0.1', () => {
      server.off('error', reject);
      const address = server.address();
      if (!address || typeof address === 'string') return reject(new Error('HTTP server lacks TCP address'));
...
async function boot(home: string, extraEnv: NodeJS.ProcessEnv = {}): Promise<BootedServer> {
  ...
  children.push(child);
  let stderr = '';
  if (!child.stderr || !child.stdout) throw new Error('Server child missing captured output');
...
const remoteHosts: typeof import('../lib/remote-hosts') = require('../lib/remote-hosts')

function withHome<T>(settings: unknown, fn: (home: string) => T): T {
  const home = makeHome();
...
test('remotes are validated on load and bad entries are skipped, never thrown', () => {
  withHome({
    ...
    assert.deepEqual(remotes.map((remote: unknown) => record(remote).name), ['peer', 'work', 'work2']);
    assert.deepEqual(remotes[0], { name: 'peer', kind: 'direct', origin: 'http://10.0.0.4:3333', token: 'tok' });
    assert.equal(record(remotes[1]).kind, 'ssh');
    assert.equal(record(remotes[1]).remotePort, 3333, 'remotePort defaults to 3333');
    assert.equal(record(remotes[2]).remotePort, 4444);
    assert.equal(record(remoteHosts.getRemote('peer')).origin, 'http://10.0.0.4:3333');
...
test('ssh forwards land on a per-remote socket under the dish run dir', () => {
  withHome({ remotes: [{ name: 'work', sshDest: 'user@box', remotePort: 4321 }] }, (home: string) => {
    ...
    const work = present(remoteHosts.getRemote('work'));
    if (!('sshDest' in work)) throw new Error('Expected SSH remote fixture');
    assert.deepEqual(remoteHosts.sshArgv(work), [
      '-N',
      '-o', 'BatchMode=yes',
  ...
  withHome({ remotes: [{ name: 'bee', sshDest: 'bee', remoteHost: '100.70.163.97' }] }, () => {
    const bee = present(remoteHosts.getRemote('bee'));
    if (!('sshDest' in bee)) throw new Error('Expected SSH remote fixture');
    const argv = remoteHosts.sshArgv(bee);
...
test('probing an unconfigured name reports unknown_host without a request', async () => {
  await withHome({}, async () => {
    ...
    if (result.reachable) throw new Error('unknown host unexpectedly reachable');
...
async function withHomeAsync<T>(settings: unknown, fn: (home: string) => Promise<T>): Promise<T> {
  const home = makeHome();
  writeSettings(home, settings);
  const previous = process.env.HOME;
  process.env.HOME = home;
...
async function withFakeClock<T>(fn: (advance: (milliseconds: number) => void) => Promise<T>): Promise<T> {
  const realNow = Date.now;
  let now = realNow();
  Date.now = () => now;
  try { return await fn((milliseconds: number) => { now += milliseconds; }); } finally { Date.now = realNow; }
}

/** The slot the breaker is currently holding, i.e. which ladder rung it is on. */
function heldSlot(name: string) {
...
test('reachability() is a pure cache read that expires on the ladder', async () => {
  await withHomeAsync({ remotes: [{ name: 'peer', url: 'http://127.0.0.1:1/' }] }, async () => {
    await withFakeClock(async advance => {
      ...
      const down = present(remoteHosts.reachability('peer'));
      assert.equal(down.reachable, false);
      if (down.reachable) throw new Error('failed peer unexpectedly reachable');
      ...
      assert.equal(present(remoteHosts.reachability('peer')).reachable, false);
...
test('a flapping peer climbs the ladder; only sustained health resets it', async () => {
  ...
  closables.push(up);
  const port = await listenHttp(up);
...
let hub!: BootedServer;        // no token: the everyday loopback/tailnet posture
let peer!: BootedServer;
let echo!: http.Server;       // in-process reflector, registered as a second remote
let echoPort = 0;
let downPort = 0;             // nothing listens here
...
test('boot a peer, an echo peer and a hub that knows about them', async () => {
  ...
  echoPort = await listenHttp(echoServer);
  echo = echoServer;
...
test('GET /api/hosts lists self first, then every configured remote', async () => {
  ...
  const responseBody = record(await (await fetch(`${hub.base}/api/hosts`)).json());
  const hosts = records(responseBody.hosts);
  assert.ok(Date.now() - started < 3500, 'the fleet list must not wait on a dead peer');

  const self = present(hosts[0]);
  assert.equal(self.self, true);
  assert.equal(self.name, null);
  assert.equal(self.base, '');
  assert.equal(self.reachable, true);
  assert.match(String(self.hostId), UUID_RE);
  assert.equal(self.label, 'hub');
  assert.equal(self.version, PKG_VERSION);
  assert.equal(record(self.capabilities).sessions, true);

  assert.deepEqual(hosts.slice(1).map(host => host.name), ['peer', 'echo', 'down']);

  const peerDescriptor = record(await (await fetch(`${peer.base}/api/host`)).json());
  const entry = present(hosts.find(host => host.name === 'peer'));
  ...
  // socket/ssh text.
  const down = present(hosts.find(host => host.name === 'down'));
  assert.equal(down.reachable, false);
  assert.equal(down.hostId, undefined);
  assert.match(String(down.error), /^[a-z_]+$/);
...
test('the proxy reaches the peer with the peer\'s own credential', async () => {
  ...
  const body = record(await proxied.json());
  assert.ok(records(body.previous).some(session => session.id === PEER_SESSION_ID), 'the peer\'s fixture session should be listed');

  // The peer's own session list is what came back, not the hub's (empty) one.
  const local = record(await (await fetch(`${hub.base}/api/sessions`)).json());
  assert.equal(records(local.previous).some(session => session.id === PEER_SESSION_ID), false);
  ...
  const page = record(await messages.json());
  assert.equal(records(page.messages).length, 1);
});
...
test('proxied requests carry the peer token and nothing of the caller\'s', async () => {
  ...
  const seen = record(await res.json());

  assert.equal(seen.method, 'POST');
  assert.equal(seen.url, '/api/sessions/abc/prompt?x=1');
  const headers = record(seen.headers);
  assert.equal(headers.authorization, `Bearer ${ECHO_TOKEN}`);
  assert.equal(headers.origin, undefined);
  assert.equal(headers.host, `127.0.0.1:${echoPort}`);
  assert.equal(headers['x-custom'], 'kept');
  assert.equal(headers['content-type'], 'application/json');
  // The body reached the peer as bytes: nothing parsed or re-serialized it.
  assert.deepEqual(JSON.parse(String(seen.body)), { message: 'hello from the hub' });
});
...
test('unknown and malformed host names are bare 404s; peer statuses pass through', async () => {
  ...
  assert.match(String(record(await bad.json()).error), /days must be/);
});
...
test('an unreachable peer fails fast instead of hanging', async () => {
  ...
  assert.match(String(body.error), /down/);
  assert.equal(body.reason, 'connection_refused');
});

test('a proxied terminal upgrade is spliced through to the peer', async (t) => {
  const capabilities = record(record(await (await fetch(`${peer.base}/api/host`)).json()).capabilities);
  ...
  const hubCapabilities = record(record(await (await fetch(`${hub.base}/api/host`)).json()).capabilities);
  assert.equal('terminal' in hubCapabilities, false);

  const upgrade = (urlString: string, headers: http.OutgoingHttpHeaders = {}) => new Promise<number>((resolve, reject) => {
...
const shutdownSignals: Array<readonly [NodeJS.Signals, number]> = [['SIGTERM', 143], ['SIGINT', 130]];
...
test(`the ssh transport forwards through a socket and cleans up on ${signal}`, async () => {
  ...
  const hostBody = record(await (await fetch(`${sshHub.base}/api/hosts`)).json());
  const hosts = records(hostBody.hosts);
  const work = present(hosts.find(host => host.name === 'work'));
  ...
  assert.equal(proxied.status, 200);
  assert.ok(records(record(await proxied.json()).previous).some(session => session.id === PEER_SESSION_ID));

  // The forward is a child of the server and goes down with it.
  const exited = new Promise<number | null>(resolve => sshHub.child.once('exit', code => resolve(code)));
...
test('a hub token gates /hosts/* exactly like /api/*', async () => {
  ...
  assert.ok(records(record(await ok.json()).previous).some(session => session.id === PEER_SESSION_ID));
  assert.equal((await fetch(`${peer.base}/api/sessions`, authed(HUB_TOKEN))).status, 401);

  const hostBody = record(await (await fetch(`${gated.base}/api/hosts`, authed(HUB_TOKEN))).json());
  const hosts = records(hostBody.hosts);
  assert.equal(present(hosts.find(host => host.name === 'peer')).reachable, true);

  // EventSource can't send a header, so the hub's own ticket opens the
  // proxied stream route — and only that route.
  const ticketBody = record(await (await fetch(`${gated.base}/api/auth/ticket`, authed(HUB_TOKEN, {
    method: 'POST',
    headers: { Authorization: `Bearer ${HUB_TOKEN}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ purpose: 'stream' }),
  }))).json());
  const ticket = String(ticketBody.ticket);
...
test('the proxy trips a breaker on a down peer and re-dials when the slot expires', async () => {
  ...
  const body = record(await first.json());
  ...
  assert.equal(record(await shorted.json()).reason, 'connection_refused');
