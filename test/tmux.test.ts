...
const { encodeSessionKey, VERSION: SESSION_KEY_VERSION }: typeof import('../lib/session-key') = require('../lib/session-key');
const { createLineSplitter }: typeof import('../lib/line-splitter') = require('../lib/line-splitter');
const { processIdentityAlive }: typeof import('../lib/process-identity') = require('../lib/process-identity');
import type { ProcessIdentity } from '../lib/contracts.js';
import type { PaneProcessState, SpawnPlacement } from '../lib/tmux.js';

function processEvidence(value: unknown): ProcessIdentity {
...
function placement(value: TestRecord): SpawnPlacement {
...
function recordablePlacement(value: SpawnPlacement) {
  return {
    ...value,
...
const sessionRecovery: typeof import('../lib/session-recovery') = require('../lib/session-recovery');
const bridgeRegistry: typeof import('../lib/bridge-session') = require('../lib/bridge-session');
const sessionOwnership: typeof import('../lib/session-ownership') = require('../lib/session-ownership');
const sessionFiles: typeof import('../lib/session-files') = require('../lib/session-files');

let primeRosterTransform: (rows: TestRecord[]) => unknown = rows => rows;
let primeStopMode = 'stop';
let primeProtocolVersion = 7;
let primeAfterList: () => void = () => {};
let primeAfterKill: () => void = () => {};
const primeKills: unknown[] = [];
let primeConnections = 0;
function primeClaims(): TestRecord[] {
  const dir = path.join(tmpHome, '.pi', 'dish', 'sessions');
  return fs.readdirSync(dir).flatMap(name => {
    try {
      const claim = record(JSON.parse(fs.readFileSync(path.join(dir, name), 'utf8')));
...
const primeDaemon = net.createServer(socket => {
  ...
  socket.on('data', createLineSplitter((line: string) => {
    ...
    const response: TestRecord = { id: envelope.id, type: 'response', command: command.type, success: true };
    if (command.type === 'list') {
      ...
      if (primeStopMode !== 'hang') process.kill(Number(claim.pid), 'SIGTERM');
      primeAfterKill();
...
test('GET /api/tmux/targets lists the running server and its sessions', { skip: !tmuxOk }, async () => {
  ...
  const srv = present(records(body.servers).find(candidate => path.resolve(String(candidate.socket)) === path.resolve(TMUX_SOCKET)));
  assert.ok(srv, 'our tmux socket is listed');
  assert.ok(records(srv.sessions).some(session => session.name === 'work'), 'the "work" session is listed');
...
test('POST /api/sessions/new with a tmux target spawns and returns the registered id', { skip: !tmuxOk }, async () => {
  ...
  for (let i = 0; i < 10 && !active; i++) {
    ...
    active = records(list.body.active).some(session => session.id === body.id);
...
test('OMP spawn omits the launch wrapper when the discovery bridge is installed', { skip: !tmuxOk }, async () => {
  ...
  try {
    ...
    assert.equal(status, 200, JSON.stringify(body));
    const spawn = present(tmux.getSpawn(text(body.id)));
    ...
    const registryFile = fs.readdirSync(registryDir).find(name => {
      const entry = record(JSON.parse(fs.readFileSync(path.join(registryDir, name), 'utf8')));
    ...
    const registry = record(JSON.parse(fs.readFileSync(path.join(registryDir, registryFile), 'utf8')));
    assert.ok(Array.isArray(registry.launchArgs) && !registry.launchArgs.includes('--extension'),
      `launch args carry no --extension: ${JSON.stringify(registry.launchArgs)}`);
...
test('OMP session models gain catalog thinking levels the live registry omits', { skip: !tmuxOk }, async () => {
  ...
  const models = records(await res.json());
  const glm52 = present(models.find(model => model.selector === 'zai/glm-5.2'));
  ...
  const uncatalogued = present(models.find(model => model.selector === 'live-only/uncatalogued'));
...
test('OMP spawn keeps its launch wrapper when the configured agent directory has no discovery bridge', { skip: !tmuxOk }, async () => {
  ...
  try {
    ...
    const registry = JSON.parse(fs.readFileSync(path.join(registryDir, present(registryFile)), 'utf8'));
...
test('OMP launch uses its wrapper descriptor and encoded cross-harness identity', { skip: !tmuxOk }, async () => {
  ...
  const list = await get('/api/sessions?active=1');
  const session = records(list.body.active).find((entry) => entry.id === body.id);
  ...
  const messageSession = record(messages.body.session);
...
test('OMP close kills and verifies only its exact pi-dish-owned pane process tree', { skip: !tmuxOk }, async () => {
  ...
  const spawn = present(tmux.getSpawn(text(created.body.id)));
  ...
  const registry = record(JSON.parse(fs.readFileSync(registryPath, 'utf8')));
  ...
  const siblingSpawn = present(tmux.getSpawn(text(sibling.body.id)));
  ...
  const siblingRegistry = record(JSON.parse(fs.readFileSync(siblingRegistryPath, 'utf8')));
  const siblingProcess = processEvidence(siblingRegistry);

  try {
    const list = await get('/api/sessions?active=1');
    const session = records(list.body.active).find(entry => entry.id === created.body.id);
    assert.equal(session?.closeMode, 'owned-pane');
    assert.equal(record(session?.capabilities).close, true);
    ...
    assert.doesNotThrow(() => process.kill(siblingProcess.pid, 0),
    ...
    try { process.kill(registryProcess.pid, 'SIGTERM'); } catch {}
...
test('POST /restart respawns an owned agent in the exact tmux pane', { skip: !tmuxOk }, async () => {
  ...
  const beforeSpawn = present(tmux.getSpawn(id));
  ...
  const beforeSession = present(records(beforeList.body.active).find(entry => entry.id === id));
  assert.equal(record(beforeSession.capabilities).restart, true);
  ...
  const afterSpawn = present(tmux.getSpawn(id), 'replacement owns a persisted pane claim');
  ...
  const replacement = fs.readdirSync(registryDir)
    .map(name => record(JSON.parse(fs.readFileSync(path.join(registryDir, name), 'utf8'))))
    .find(entry => entry.harnessId === 'omp' && entry.spawnToken === afterSpawn.spawnToken);
  const replacementEntry = present(replacement, 'replacement bridge registration is live');
...
test('OMP close stays unavailable for a live pane pi-dish does not own', { skip: !tmuxOk }, async () => {
  ...
  try {
    ...
    const list = await get('/api/sessions?active=1');
    const session = records(list.body.active).find((entry) => entry.id === created.body.id);
    assert.equal(record(session?.capabilities).close, false);
    assert.equal(record(session?.capabilities).restart, false);
...
test('Prime close stops the owned worker and pane, refusing a replaced pane first', { skip: !tmuxOk }, async () => {
  ...
  const spawn = present(tmux.getSpawn(text(created.body.id)));
  if (typeof spawn.wrapperPath !== 'string') throw new Error('Prime launch lacks its wrapper path');
  assert.ok(fs.existsSync(spawn.wrapperPath),
  ...
  assert.ok(paneProcess.pid && paneProcess.startTime,
  ...
  const registry = record(JSON.parse(fs.readFileSync(path.join(regDir, regFile), 'utf8')));
  const registryProcess = processEvidence(registry);
  ...
  assert.equal(record(session?.capabilities).close, true);
  assert.equal(record(session?.capabilities).restart, true);
...
test('Prime close also stops a worker in the owned client process tree', { skip: !tmuxOk }, async () => {
  ...
  let spawn: TestRecord | null = null;
  let registry: TestRecord | null = null;
  ...
  try {
    ...
    assert.equal(created.status, 200, JSON.stringify(created.body));
    spawn = record(tmux.getSpawn(text(created.body.id)));
    const regDir = path.join(tmpHome, '.pi', 'dish', 'sessions');
    const regFile = fs.readdirSync(regDir).find(name => {
      const entry = record(JSON.parse(fs.readFileSync(path.join(regDir, name), 'utf8')));
    ...
    registry = record(JSON.parse(fs.readFileSync(registryPath, 'utf8')));
    ...
    process.env.PI_DISH_PRIME_COMMAND = `env PI_FIXTURE_HARNESS=prime ${process.execPath} ${FIXTURE}`;
    if (spawn) {
      try { await tmux.killPane(text(spawn.socket), text(spawn.paneId)); } catch {}
    }
    if (registry?.pid) {
      try { process.kill(Number(registry.pid), 'SIGTERM'); } catch {}
    }
    if (registryPath) fs.rmSync(registryPath, { force: true });
  }
});

async function primeCloseFixture(t: test.TestContext) {
  const created = await post('/api/sessions/new', {
    harness: 'prime', target: { type: 'tmux', socket: TMUX_SOCKET, tmuxSession: 'work' },
  });
  assert.equal(created.status, 200, JSON.stringify(created.body));
  const id = text(created.body.id);
  const spawn = present(tmux.getSpawn(id));
  const claim = present(primeClaims().find(entry => entry.spawnToken === spawn.spawnToken));
  t.after(async () => {
    ...
    if (processIdentityAlive(claim)) process.kill(Number(claim.pid), 'SIGTERM');
...
test('Prime restart replaces the worker in the same pane and transcript without stopping its peer', { skip: !tmuxOk }, async t => {
  ...
  const location = present(await tmux.paneLocation(spawn.socket, spawn.paneId));
  ...
  const replacement = present(tmux.getSpawn(id));
  ...
  const worker = present(primeClaims().find(entry => entry.spawnToken === replacement.spawnToken));
...
test('Prime restart rejects unowned, replaced and missing panes and changed supervisor ownership', { skip: !tmuxOk }, async t => {
  ...
  await refused();
  const row = present(records((await get('/api/sessions?active=1')).body.active).find(candidate => candidate.id === id));
  assert.equal(record(row.capabilities).restart, false);
  assert.equal(record(row.capabilities).close, true);
});
...
for (const mode of ['lost-response', 'hang', 'replaced-pane']) {
  test(`Prime restart ${mode} never launches a replacement after an unproved stop or pane`, { skip: !tmuxOk }, async t => {
    ...
    if (mode === 'replaced-pane') primeAfterKill = () => { tmuxCmd(['respawn-pane', '-k', '-t', text(spawn.paneId), 'sleep 3600']); };
    else primeStopMode = mode;
    const killsBefore = primeKills.length;
    const result = await post(`/api/sessions/${encodeURIComponent(id)}/restart`);
    assert.equal(result.status, 500, JSON.stringify(result.body));
    assert.equal(primeKills.length, killsBefore + 1);
    assert.equal(present(tmux.getSpawn(id)).spawnToken, spawn.spawnToken, 'no new launch');
    assert.equal(await tmux.paneExists(spawn.socket, spawn.paneId), true);
...
test('Prime restart excludes competing restart, close and resume requests while replacing its worker', { skip: !tmuxOk }, async t => {
  ...
  assert.notEqual(present(tmux.getSpawn(id)).spawnToken, spawn.spawnToken);
});
...
test('Prime failed replacement stays inspectable and blocks a second writer even after its client exits', { skip: !tmuxOk }, async t => {
  ...
  t.after(() => { if (processIdentityAlive(worker)) process.kill(Number(worker.pid), 'SIGTERM'); });
  await tmux.killPane(spawn.socket, spawn.paneId);
...
for (const mismatch of ['session-id', 'session-file', 'client-pane']) {
  test(`Prime restart rejects replacement ${mismatch} drift`, { skip: !tmuxOk }, async t => {
    ...
    if (mismatch === 'client-pane') {
      tmux.paneProcessIdentity = async (socket: string, paneId: string) => {
...
test('Prime close rejects unowned launches, changed tokens, ambiguous roots and changed roster identities', { skip: !tmuxOk }, async t => {
  ...
  tmux.recordSpawn(id, recordablePlacement(spawn));
  const transforms: Array<(row: TestRecord) => TestRecord> = [
...
test('Prime without worker daemon metadata advertises no close and fails closed', { skip: !tmuxOk }, async t => {
  ...
  const list = await get('/api/sessions?active=1');
  assert.equal(record(records(list.body.active).find(row => row.id === fixture.id)?.capabilities).close, false);
  const killsBefore = primeKills.length;
...
test('/reload falls back to send-keys into the owning tmux pane when the bridge cannot run it', { skip: !tmuxOk }, async () => {
  ...
  for (let i = 0; i < 10 && !sess; i++) {
    const list = await get('/api/sessions?active=1');
    sess = records(list.body.active).find((s) => s.id === id) || null;
...
test('/reload is discovered and sent into a reachable OMP tmux pane', { skip: !tmuxOk }, async () => {
  ...
  const commands = records(await commandsResponse.json());
  assert.ok(commands.some(command => command.name === 'reload'),
...
test('overlapping resume requests share the first target and launch one process', { skip: !tmuxOk }, async () => {
  ...
  const paneCount = (socket: string, target?: string): number => {
    try {
  ...
  try {
    ...
    assert.equal(path.resolve(present(tmux.getSpawn(id)).socket), path.resolve(TMUX_SOCKET),
...
test('explicit tmux resume timeout quarantines its pane until the writer is gone', { skip: !tmuxOk }, async () => {
  ...
  const hiddenSocket = path.join(tmuxTmp, 'pi-dish');
  const paneIds = (socket: string, target?: string): Set<string> => {
    try {
      const args = target ? ['list-panes', '-s', '-t', target, '-F', '#{pane_id}'] : ['list-panes', '-a', '-F', '#{pane_id}'];
  ...
  let paneState: PaneProcessState | null = null;
  ...
  try {
    ...
    [paneId] = added;
    paneState = await tmux.paneProcessState(TMUX_SOCKET, present(paneId));
    ...
    await tmux.killPane(TMUX_SOCKET, present(paneId));
    for (let i = 0; i < 30; i++) {
      const state = await tmux.paneProcessState(TMUX_SOCKET, present(paneId), {
        knownProcesses: present(paneState).knownProcesses,
      });
...
test('POST /api/sessions/new with no target spawns into an independent hidden tmux session', { skip: !tmuxOk }, async () => {
  ...
  const spawn = present(tmux.getSpawn(text(body.id)), 'placement persisted like any tmux spawn');
  assert.equal(path.basename(spawn.socket), 'pi-dish', 'lands on the dedicated pi-dish socket');
  const loc = present(await tmux.paneLocation(spawn.socket, spawn.paneId));
...
test('a second headless spawn creates a separate independent session on the pi-dish server', { skip: !tmuxOk }, async () => {
  ...
  const loc = present(await tmux.paneLocation(socket, spawn.paneId));
...
test('pane-gone cleanup quarantine tracks surviving descendants by pid and starttime', { skip: !tmuxOk }, async () => {
  ...
  let knownProcesses: ProcessIdentity[] = [];
  try {
    ...
    let firstError: TestRecord | null = null;
    await assert.rejects(
      () => tmux.killPaneAndWait(TMUX_SOCKET, paneId, { timeout: 250 }),
      (error: unknown) => { firstError = record(error); return /pane gone.*processes still alive/.test(errorMessage(error)); },
    );
    assert.equal(await tmux.paneExists(TMUX_SOCKET, paneId), false, 'tmux pane is gone');
    knownProcesses = records(record(firstError).remainingProcesses).map(processEvidence);
...
test('runtime location is cached per pid: a rename shows the old name within TTL', { skip: !tmuxOk }, async () => {
  ...
  try {
    ...
    const firstRuntime = record(first.body.runtime);
    ...
    assert.equal(record(second.body.runtime).tmuxSession, 'cachesrc', 'served from cache, not re-resolved');
  } finally {
...
test('legacy coercible registry PID retains tmux runtime and pane discovery', { skip: !tmuxOk }, async () => {
  ...
  try {
    const runtime = present(await owner.describeRuntime(sessionId));
    assert.equal(runtime.kind, 'tmux');
    assert.equal(runtime.tmuxSession, 'work');
    const pane = present(await owner.locatePiPane(sessionId));
...
test('attachPaneArgv builds a grouped viewer; getPrefixKey reads the server prefix', { skip: !tmuxOk }, async () => {
  const paneId = present(tmuxCmd(['list-panes', '-t', 'work:0', '-F', '#{pane_id}']).trim().split('\n')[0]);
  const argv = present(await tmux.attachPaneArgv(TMUX_SOCKET, paneId));
...
test('pruneSpawns preserves a replacement recorded while its pane probe is in flight', () => {
  ...
  try {
    const out = execFileSync(process.execPath, [PRUNE_DRIVER, 'replacement', modulePath], {
      encoding: 'utf8',
...
test('paneExists rejects an exit-0 empty target field and prune removes that stale placement', () => {
  ...
  const modulePath = path.join(__dirname, '..', 'lib', 'tmux.js');
  try {
    const out = execFileSync(process.execPath, [PRUNE_DRIVER, 'stale', modulePath], {
      encoding: 'utf8',
      env: { ...process.env, HOME: home, PATH: `${binDir}${path.delimiter}${process.env.PATH || ''}` },
    });
