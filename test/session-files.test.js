// Generated test/tool from test/session-files.test.ts; edit that source and run npm run build:tests.
"use strict";
...
const userMsg = (text, timestamp) => ({ type: 'message', message: { role: 'user', content: [{ type: 'text', text }], timestamp } });
const assistantMsg = (text, usage) => ({ type: 'message', message: { role: 'assistant', content: [{ type: 'text', text }], usage } });
const messageText = (message) => (0, test_types_js_1.record)((0, test_types_js_1.present)((0, test_types_js_1.records)((0, test_types_js_1.record)(message).content)[0])).text;
...
test('readSessionMessages returns the displayable stream in order', () => {
    ...
    assert.deepEqual(all.map((message) => message.role), ['user', 'assistant', 'toolResult', 'user', 'custom', 'assistant']);
    assert.equal((0, test_types_js_1.record)((0, test_types_js_1.present)((0, test_types_js_1.records)((0, test_types_js_1.record)((0, test_types_js_1.present)(all[3])).content)[0])).text, 'injected', 'session-message custom entries render as user messages');
    assert.equal((0, test_types_js_1.record)((0, test_types_js_1.present)(all[4])).customType, 'other', 'unknown visible custom entries get a generic row');
...
test('OMP entry tolerance renders async/interruption/unknown custom rows without leaking hidden state', () => {
    ...
    assert.deepEqual((0, test_types_js_1.record)((0, test_types_js_1.present)(messages[1])).content, [], 'empty aborted assistant messages remain parseable');
    assert.deepEqual((0, test_types_js_1.record)((0, test_types_js_1.present)(messages[2])).content, [], 'interrupted reasoning is projected only as a marker');
    const jobs = (0, test_types_js_1.records)((0, test_types_js_1.record)((0, test_types_js_1.record)((0, test_types_js_1.present)(messages[3])).details).jobs);
    assert.equal((0, test_types_js_1.record)((0, test_types_js_1.present)(jobs[0])).jobId, 'bg_123');
    assert.equal((0, test_types_js_1.record)((0, test_types_js_1.present)(messages[4])).content, 'A future visible OMP notice', 'unknown visible custom types get a generic row');
...
test('cache TTL policy supports built-in model slugs and live provider overrides', () => {
    ...
    const builtIn = (0, test_types_js_1.present)(SF.readSessionMessages(deepseek)[0].cacheExpiry);
    ...
    assert.equal((0, test_types_js_1.present)(SF.readSessionMessages(anthropic)[0].cacheExpiry).retention, '5m');
    ...
    assert.equal((0, test_types_js_1.present)(SF.readSessionMessages(deepseek)[0].cacheExpiry).retention, '2d', 'model override replaces the built-in without touching the session file');
    assert.equal((0, test_types_js_1.present)(SF.readSessionMessages(anthropic)[0].cacheExpiry).retention, '1h', 'provider override invalidates the cached projection');
    assert.equal((0, test_types_js_1.present)(SF.getSessionInfo(anthropic).cacheExpiry).expiresAt, startedAt + 60 * 60_000);
...
test('sanitizeUsage preserves missing and explicit-zero cost components', () => {
    ...
    assert.equal(SF.sanitizeUsage({ cost: { input: NaN } }), undefined, 'an empty sanitized cost object is omitted rather than implying availability');
});
test('readSessionMessages cache revalidates on append and survives LRU pressure', () => {
    ...
    for (let i = 0; i < 8; i++)
        SF.readSessionMessages(writeSession([userMsg(`filler ${i}`)]));
...
test('readSessionMessages follows the active tree path, not file order', () => {
    ...
    const texts = SF.readSessionMessages(file).map(messageText);
    assert.deepEqual(texts, ['first prompt', 'first answer', 'tried X', 'retry prompt'], 'abandoned branch entries do not render; active path keeps file order');
});
test('readSessionMessagesAtLeaf follows a live leaf that is not the last JSONL entry', () => {
    ...
    assert.deepEqual(selected.map(messageText), ['root', 'shared answer']);
    assert.deepEqual(SF.readSessionMessages(file).map(messageText), ['root', 'shared answer', 'abandoned prompt', 'physical tip'], 'ordinary reads keep deriving the leaf from the physical file tip');
    assert.throws(() => SF.readSessionMessagesAtLeaf(file, 'missing'), /Session tree leaf not found: missing/);
});
test('large snapcompact archives are projected without retaining frame payloads', () => {
    ...
    const compact = parsed.find((entry) => (0, test_types_js_1.record)(entry).type === 'compaction');
    ...
    assert.deepEqual(SF.readSessionMessagesAtLeaf(file, 'u2').map(messageText), ['before snap', 'before answer', 'after snap']);
    ...
    assert.equal(SF.getSessionStats(file).compactions, 1, 'a projected large compaction record is still counted');
    ...
    const unusualCompact = (0, test_types_js_1.record)((0, test_types_js_1.present)(SF.parseSessionEntries(unusual)[0]));
    assert.equal(unusualCompact.summary, 'field serialized after preserveData');
    assert.ok(String((0, test_types_js_1.record)(unusualCompact.preserveData).custom).startsWith(archiveMarker), 'nonstandard field order falls back to full JSON parsing');
});
...
test('buildSearchTextFromContent indexes tool-call names and string args', () => {
    ...
    assert.ok(text.includes('/repo/deep-needle.md'), 'path args outrank bulky args inside the per-call cap');
...
test('buildSearchTextFromContent bounds a whole session\'s text, keeping the newest', () => {
    const entries = [];
    for (let i = 0; i < 50; i++)
        entries.push(userMsg(`msg${i} ` + 'y'.repeat(99_000)));
...
test('indexed usage keeps all-known and explicit-zero costs numeric', () => {
    ...
    const usage = SF.buildIndexedUsageFromContent(content);
    assert.deepEqual(usage.total.costs, { input: 0.1, output: 0.2, cacheRead: 0.03, cacheWrite: 0.04, total: 0.37 });
    assert.deepEqual(usage.total.costUnavailable, { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0 });
});
test('indexed usage treats ZAI plan zeros as unpriced and drops empty failed retries', () => {
    ...
    assert.deepEqual(usage.total.costs, { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0 });
    assert.deepEqual(usage.total.costUnavailable, { input: 1, output: 1, cacheRead: 1, cacheWrite: 1, total: 1 });
...
test('malformed raw counters contribute no tokens while retry and cost policies retain raw usage', () => {
    ...
    assert.strictEqual(SF.extendIndexedUsageFromEntries(extended, SF.parseSessionEntries(lines.slice(3).join('\n'))), extended, 'delta mutates the same borrowed usage');
    assert.deepEqual(extended, usage, 'full and delta projections use the same operand policy');
    const rawUsage = (0, test_types_js_1.record)((0, test_types_js_1.record)((0, test_types_js_1.present)(parsed[2])).message);
    const rawCounters = (0, test_types_js_1.record)(rawUsage.usage);
    assert.equal(rawCounters.input, '8', 'raw usage is not rewritten');
    assert.equal(rawCounters.cacheWrite, Infinity);
});
test('OMP chronology keeps billing continuity separate from display identity and retries', () => {
    ...
    assert.equal((0, test_types_js_1.present)((0, test_types_js_1.present)(usage.models['provider/billed']).tokens).output, 2);
    assert.equal((0, test_types_js_1.present)((0, test_types_js_1.present)(usage.models['provider/selected']).tokens).output, 3);
...
test('session stats do not present ZAI Coding Plan usage as free', () => {
    ...
    assert.deepEqual(stats.costUnavailable, { input: 1, output: 1, cacheRead: 1, cacheWrite: 1, total: 1 });
});
