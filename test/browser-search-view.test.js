// Generated test/tool from test/browser-search-view.test.ts; edit that source and run npm run build:tests.
"use strict";
...
test('search payloads narrow rows and merge with the actual host identity', () => {
    ...
    assert.equal(result.results[0].host, 'real');
    assert.equal(result.results[0].hostLabel, 'Real');
    assert.deepEqual(plain(result.results[0].snippets), ['hit']);
    assert.equal(result.total, 1);
    assert.throws(() => decodeSearchPayload({}), /Invalid search response/);
});
