...
test('extension payloads narrow malformed fields and strip ANSI without mutating the wire object', () => {
  ...
  assert.ok(result); const question = result.questions[0]; assert.ok(question);
  const first = question.options[0], second = question.options[1]; assert.ok(first); assert.ok(second);
  assert.equal(result.title, 'Title'); assert.equal(result.questions.length, 1); assert.equal(first.label, 'First');
  assert.equal(second.preview, 'Preview'); assert.equal(question.recommended, null); assert.equal(question.multi, false);
