// Generated test/tool from test/listener-lifecycle.test.ts; edit that source and run npm run build:tests.
"use strict";
...
function addressOf(server) {
    ...
    if (!address || typeof address === 'string')
        throw new Error('Listener lacks TCP address');
    return address;
}
async function alternateLoopback(t) {
    try {
        ...
        if (!['EADDRNOTAVAIL', 'EAFNOSUPPORT'].includes(String((0, test_types_js_1.record)(error).code ?? '')))
            throw error;
        t.skip('this OS does not provide the alternate loopback address 127.0.0.2');
        return false;
    }
}
function decodeListenerEvent(value) {
    const row = (0, test_types_js_1.record)(value), rawAddress = row.address;
    let address = { address: '', family: '', port: 0 };
    if (rawAddress && typeof rawAddress === 'object') {
        const item = (0, test_types_js_1.record)(rawAddress);
...
function boot(t, overrides = {}) {
    ...
    const child = (0, node_child_process_1.fork)(path.join(__dirname, 'fixtures/listener-server.js'), [], {
...
async function descriptor(host, port) {
    ...
    return (0, test_types_js_1.record)(await response.json());
}
async function stopAndRebind(t, server, signal, listeners) {
    ...
    for (const listener of listeners) {
        if (!listener.address)
            throw new Error('Listener event missing address');
...
for (const explicitUrl of ['', 'https://fixture.invalid/dish']) {
    test(`a delayed alias advertises ${explicitUrl ? 'the explicit URL unchanged' : 'only an address already listening'}`, { timeout: 15000 }, async (t) => {
        ...
        const ready = await server.wait((event) => event.type === 'startup-ready');
...
test('readiness waits for the full app, not a restricted share or an explicit advertised URL', { timeout: 15000 }, async (t) => {
    ...
    await server.wait((event) => event.type === 'main-held');
    const share = await server.wait((event) => event.type === 'listening');
    ...
    const late = await server.wait((event) => event.type === 'late-ready');
...
test('ephemeral readiness follows the replacement listener after the initial bind fails', { timeout: 30000 }, async (t) => {
    ...
    await server.wait((event) => event.type === 'bind-failed');
    assert.equal(server.events.some(event => event.type === 'startup-ready'), false);
    const ready = await server.wait((event) => event.type === 'startup-ready', 20000);
