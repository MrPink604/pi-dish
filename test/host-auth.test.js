// Generated tool from test/host-auth.test.ts; edit that source and run npm run build:tools.
"use strict";
...
async function boot(home, extraEnv = {}) {
    ...
    const env = { ...(0, test_env_js_1.sanitizeTestEnv)(process.env), HOME: home, PORT: String(port), TMUX_TMPDIR: tmuxRoot };
    ...
    for (;;) {
        ...
        if (child.exitCode !== null || child.signalCode !== null)
            throw new Error(`server exited (${child.exitCode ?? child.signalCode}): ${stderr}`);
        try {
    ...
    if (remaining <= 0)
        throw new Error(`server model warmup exceeded startup deadline: ${stderr}`);
    const init = { signal: AbortSignal.timeout(remaining) };
...
test('without a token the API is open and emits no CORS headers', async () => {
    const home = makeHome();
    fs.writeFileSync(path.join(home, '.pi', 'dish', 'settings.json'), JSON.stringify({ allowedOrigins: ['https://dash.example'] }));
    const { base } = await boot(home);
...
test('stream tickets are minted for authed callers only and unlock the SSE route', async () => {
    ...
    const terminalBody = await (await mint(authed(TOKEN, { body: JSON.stringify({ purpose: 'terminal' }) }))).json();
...
test('the terminal WebSocket upgrade is gated by ticket and origin', async (t) => {
    ...
    const hostBody = await (await fetch(`${base}/api/host`)).json();
    ...
    if (!capabilities.terminal)
        return t.skip('node-pty unavailable');
    ...
    const upgrade = (query, headers = {}) => new Promise((resolve, reject) => {
        ...
        req.on('response', (res) => {
            ...
            if (typeof res.statusCode !== 'number')
                reject(new Error('Expected an HTTP response status'));
            else
    ...
    const mintTerminal = async () => {
        const body = await (await fetch(`${base}/api/auth/ticket`, {
            method: 'POST',
...
test('settings responses never carry the token or the origin allowlist', async () => {
    const home = makeHome();
    fs.writeFileSync(path.join(home, '.pi', 'dish', 'token'), TOKEN);
    fs.writeFileSync(path.join(home, '.pi', 'dish', 'settings.json'), JSON.stringify({ allowedOrigins: ['https://dash.example'], hostLabel: 'tycho', monthlyBudgetUsd: 10 }));
    const { base } = await boot(home);
