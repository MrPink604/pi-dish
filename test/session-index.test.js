// Generated test/tool from test/session-index.test.ts; edit that source and run npm run build:tests.
"use strict";
...
function sourceForFile(file) {
    return sourceForIdentity('pi', (0, test_types_js_1.nativeId)(path.basename(file, '.jsonl')), file);
}
...
const userMsg = (text) => ({ type: 'message', message: { role: 'user', content: [{ type: 'text', text }], timestamp: '2026-07-01T10:00:00.000Z' } });
...
const messageText = (message) => String((0, test_types_js_1.record)((0, test_types_js_1.present)((0, test_types_js_1.records)((0, test_types_js_1.record)(message).content)[0])).text ?? '').toLowerCase();
const readRecords = (file) => fs.readFileSync(file, 'utf8').trim().split('\n').map(line => (0, test_types_js_1.record)(JSON.parse(line)));
function lastLiveRecord(values, file) {
    for (let index = values.length - 1; index >= 0; index--) {
        const value = values[index];
        if (value && value.f === file && !value.del)
            return value;
    }
    throw new Error(`Missing persisted record for ${file}`);
}
test('scanSessions indexes files and revalidates on append', () => {
    ...
    assert.equal((0, test_types_js_1.present)(infos.get(file)).messageCount, 1);
    assert.equal((0, test_types_js_1.present)(infos.get(file)).name, 'first question');
    fs.appendFileSync(file, JSON.stringify(userMsg('second question')) + '\n');
    ({ infos } = index.scanSessions([sourceForFile(file)]));
    assert.equal((0, test_types_js_1.present)(infos.get(file)).messageCount, 2, 'appended file re-indexed');
});
test('cache TTL settings invalidate indexed expiry without a session append', () => {
    ...
    assert.equal((0, test_types_js_1.present)((0, test_types_js_1.present)(index.scanSessions([source]).infos.get(file)).cacheExpiry).retention, '5m');
    try {
        ...
        const updated = (0, test_types_js_1.present)(index.scanSessions([source]).infos.get(file));
        assert.equal((0, test_types_js_1.present)(updated.cacheExpiry).retention, '1h');
        assert.equal((0, test_types_js_1.present)(updated.cacheExpiry).expiresAt, startedAt + 60 * 60_000);
    }
    finally {
...
test('OMP candidates retain their combined model in indexed usage', () => {
    ...
    const candidate = sourceForIdentity('omp', (0, test_types_js_1.nativeId)('omp-usage'), file);
    const { infos, indexing } = index.scanSessions([candidate]);
    assert.equal(indexing, false);
    const usage = (0, test_types_js_1.present)(infos.get(file)).usage;
...
test('index persists: a zero-budget scan after state reset still serves entries', () => {
    ...
    assert.equal((0, test_types_js_1.present)(infos.get(file)).messageCount, 1, 'served from disk, not re-parsed');
    assert.equal((0, test_types_js_1.present)(infos.get(file)).sessionId, 'persisted-core-id', 'core header id persisted');
    assert.equal((0, test_types_js_1.present)(infos.get(file)).parentSession, '/sessions/native-parent.jsonl', 'native lineage persisted');
    assert.ok((0, test_types_js_1.present)(infos.get(file)).lastActivity instanceof Date, 'lastActivity revived as Date');
...
test('versionless metadata is queued for reindex instead of being served stale', async () => {
    ...
    await waitFor(() => withBudget(0, () => index.scanSessions([sourceForFile(file)])).indexing === false, 'schema migration reindex');
    const fresh = (0, test_types_js_1.present)(withBudget(0, () => index.scanSessions([sourceForFile(file)])).infos.get(file));
...
test('schema-3 zero-filled usage migrates through the bounded reindex backlog', async () => {
    ...
    await waitFor(() => withBudget(0, () => index.scanSessions([sourceForFile(file)])).indexing === false, 'cost availability schema migration');
    const fresh = (0, test_types_js_1.present)(withBudget(0, () => index.scanSessions([sourceForFile(file)])).infos.get(file));
    assert.deepEqual(fresh.usage.total.costs, { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0.25 });
    assert.deepEqual(fresh.usage.total.costUnavailable, { input: 1, output: 1, cacheRead: 1, cacheWrite: 1, total: 0 });
    index.resetForTests();
    ...
    assert.equal((0, test_types_js_1.present)(persisted.infos.get(file)).usage.total.costs.input, 0);
});
test('schema-5 ZAI plan usage is reindexed instead of remaining falsely free', async () => {
    ...
    assert.equal(first.indexing, true);
    await waitFor(() => withBudget(0, () => index.scanSessions([sourceForFile(file)])).indexing === false, 'ZAI pricing schema migration');
    const fresh = (0, test_types_js_1.present)(withBudget(0, () => index.scanSessions([sourceForFile(file)])).infos.get(file));
...
test('versionless search text migrates through the bounded indexing backlog', async () => {
    ...
    await waitFor(() => withBudget(0, () => index.scanSessions([sourceForFile(file)])).indexing === false, 'search schema migration reindex');
...
test('zero sync budget queues a backlog that the background build drains', async () => {
    ...
    assert.equal(first.indexing, true);
    await waitFor(() => withBudget(0, () => index.scanSessions(files.map(sourceForFile))).indexing === false, 'background build to drain');
...
test('a queued file appended before its build is indexed once at its current size', async () => {
    ...
    assert.equal((0, test_types_js_1.present)(result.infos.get(file)).messageCount, 2, 'the append must not be counted again from a stale queued size');
});
...
test('sync budget bounds per-scan parsing; the rest lands via the builder', async () => {
    ...
    assert.equal(first.indexing, true);
    await waitFor(() => withBudget(0, () => index.scanSessions(files.map(sourceForFile))).indexing === false, 'builder to finish the remaining files');
...
test('getSearchText extends from the appended byte range', () => {
    ...
    assert.equal(index.getSearchText(sourceForFile(path.join(sessionsDir, 'missing.jsonl'))), '', 'missing file degrades to empty');
});
test('the byte-range extension honors the session text cap, keeping the newest', () => {
    const big = 'y'.repeat(99_000);
    const entries = [];
    for (let i = 0; i < 45; i++)
        entries.push(userMsg(`cap${i} ${big}`));
    ...
    assert.ok(extended.length <= sessionFiles.SEARCH_TEXT_SESSION_CAP, 'a streaming append cannot grow the text past the cap one delta at a time');
    assert.ok(extended.includes('overflow_marker_zulu'), 'the appended (newest) turn is searchable; overflow evicts old text instead');
...
test('a pre-v2 text entry is re-indexed to pick up tool-call args', () => {
    ...
    fs.writeFileSync(textLog, fs.readFileSync(textLog, 'utf-8').split('\n').map(line => {
        if (!line || !line.includes(JSON.stringify(file)))
            return line;
    ...
    assert.ok(index.getSearchText(sourceForFile(file)).includes('migration_needle'), 'the stale-version entry was rebuilt with the v2 extraction');
});
test('getSearchText rebuilds when an append changes the active tree branch', () => {
    ...
    assert.ok(index.getSearchText(sourceForFile(file)).includes('secret_abandoned'), 'the original leaf is searchable before navigation');
    ...
    assert.ok(transcript.every((part) => text.includes(part)), `indexed text agrees with the active transcript: ${transcript.join(', ')}`);
    index.resetForTests();
    const persisted = withBudget(0, () => index.scanSessions([sourceForFile(file)]));
...
test('log compaction keeps the text log near its live size', () => {
    ...
    assert.ok(logSize < 500_000, `text log should be compacted near one live entry (~50KB), got ${logSize}`);
    // And the compacted log still round-trips.
    const { infos } = withBudget(0, () => index.scanSessions([sourceForFile(file)]));
    assert.equal((0, test_types_js_1.present)(infos.get(file)).messageCount, 145);
});
...
test('append extension matches a full re-index (meta, usage, text, skills)', () => {
    ...
    const extInfo = (0, test_types_js_1.present)(extended.infos.get(fileA));
    const fileB = writeSession([...base, ...delta]);
    const fullInfo = (0, test_types_js_1.present)(index.scanSessions([sourceForFile(fileB)]).infos.get(fileB));
    const comparableKeys = ['model', 'name', 'messageCount', 'contextTokens', 'cwd', 'sessionId', 'parentSession'];
    ...
    const stripSession = (value) => { const row = (0, test_types_js_1.record)(value); const rest = { ...row }; delete rest.sessionId; return rest; };
    const recsA = index.getSkillActivations({ skill: SKILL }).filter((value) => (0, test_types_js_1.record)(value).sessionId === path.basename(fileA, '.jsonl'));
    const recsB = index.getSkillActivations({ skill: SKILL }).filter((value) => (0, test_types_js_1.record)(value).sessionId === path.basename(fileB, '.jsonl'));
...
test('getSessionInfo serves the session-files shape and extends on append', () => {
    ...
    assert.throws(() => index.getSessionInfo(sourceForFile(path.join(sessionsDir, 'missing.jsonl'))), 'unreadable file throws like session-files.getSessionInfo');
});
test('an index populated before skills mining re-mines its files (upgrade path)', () => {
    ...
    assert.deepEqual((0, test_types_js_1.record)((0, test_types_js_1.present)(recs[0])).ranges, [[5, 14]]);
});
test('index APIs require explicit sources and never infer generic-file identity', () => {
    ...
    // Deliberate runtime-boundary probes: invalid values never enter typed callers.
    assert.throws(() => Reflect.apply(index.scanSessions, index, [[invalidSource]]), /SessionSource/);
    assert.throws(() => Reflect.apply(index.getSessionInfo, index, [invalidSource]), /SessionSource/);
    assert.throws(() => Reflect.apply(index.getSearchText, index, [invalidSource]), /SessionSource/);
    const source = sourceForIdentity('pi', (0, test_types_js_1.nativeId)('resolved-identity'), file);
    const info = (0, test_types_js_1.present)(index.scanSessions([source]).infos.get(file));
...
test('invalid current-schema metadata is stale and rebuilt through the bounded backlog', async () => {
    ...
    const persisted = readRecords(log);
    const valid = lastLiveRecord(persisted, file);
    const validValue = (0, test_types_js_1.record)(valid.v);
    fs.appendFileSync(log, JSON.stringify({ ...valid, v: { ...validValue, name: { malformed: true } } }) + '\n');
...
test('persisted numeric activity and missing legacy continuity preserve warm reads', () => {
    ...
    const source = sourceForFile(file);
    const initial = (0, test_types_js_1.present)(index.scanSessions([source]).infos.get(file));
    index.resetForTests();
    const log = path.join(indexDir, 'meta.ndjson');
    const persisted = readRecords(log);
    const valid = lastLiveRecord(persisted, file), validValue = (0, test_types_js_1.record)(valid.v);
    const legacyUsage = { ...(0, test_types_js_1.record)(validValue.usage) };
    delete legacyUsage.state;
    fs.appendFileSync(log, JSON.stringify({ ...valid, v: {
            ...validValue, name: '', contextTokens: 0, lastActivity: 0, usage: legacyUsage,
        } }) + '\n');
    const warm = withBudget(0, () => index.scanSessions([source]));
    assert.equal(warm.indexing, false);
    const info = (0, test_types_js_1.present)(warm.infos.get(file));
    ...
    assert.strictEqual((0, test_types_js_1.present)(index.scanSessions([source]).infos.get(file)), info, 'scan metadata remains borrowed');
});
test('malformed consumed persisted usage buckets rebuild instead of entering a warm scan', async () => {
    ...
    const valid = lastLiveRecord(persisted, file);
    const mutations = [
        usage => { (0, test_types_js_1.record)((0, test_types_js_1.record)((0, test_types_js_1.record)(usage).total).tokens).input = '3'; },
        usage => { (0, test_types_js_1.record)((0, test_types_js_1.record)((0, test_types_js_1.record)(usage).total).costs).total = null; },
        usage => { (0, test_types_js_1.record)((0, test_types_js_1.record)((0, test_types_js_1.record)(usage).total).costUnavailable).input = {}; },
        usage => { (0, test_types_js_1.record)((0, test_types_js_1.record)(usage).total).measured = '0'; },
        usage => { (0, test_types_js_1.record)((0, test_types_js_1.record)((0, test_types_js_1.record)(usage).days).unknown).durationMs = false; },
        usage => { (0, test_types_js_1.record)((0, test_types_js_1.record)((0, test_types_js_1.record)((0, test_types_js_1.record)(usage).days).unknown).tokens).output = null; },
        usage => { (0, test_types_js_1.record)((0, test_types_js_1.record)((0, test_types_js_1.record)(usage).models)['test/model']).provider = 42; },
        usage => { (0, test_types_js_1.record)((0, test_types_js_1.record)((0, test_types_js_1.record)((0, test_types_js_1.record)((0, test_types_js_1.record)(usage).models)['test/model']).days).unknown).costs = []; },
        usage => { (0, test_types_js_1.record)((0, test_types_js_1.record)((0, test_types_js_1.record)(usage).models)['test/model']).calls = '1'; },
        usage => { (0, test_types_js_1.record)((0, test_types_js_1.record)((0, test_types_js_1.record)((0, test_types_js_1.record)((0, test_types_js_1.record)((0, test_types_js_1.record)(usage).models)['test/model']).days).unknown).costUnavailable).total = '1'; },
    ];
    for (const mutate of mutations) {
        const broken = structuredClone(valid), brokenValue = (0, test_types_js_1.record)(broken.v);
        mutate((0, test_types_js_1.record)(brokenValue.usage));
        ...
        const rebuilt = (0, test_types_js_1.present)(index.scanSessions([source]).infos.get(file)).usage;
...
test('persisted sparse day and model buckets retain omitted zero fields on warm reads', () => {
    ...
    const log = path.join(indexDir, 'meta.ndjson');
    const persisted = readRecords(log);
    const valid = lastLiveRecord(persisted, file), validValue = (0, test_types_js_1.record)(valid.v), persistedUsage = (0, test_types_js_1.record)(validValue.usage);
    (0, test_types_js_1.record)(persistedUsage.days).unknown = { calls: 1, tokens: { output: 2 }, costs: { total: 0 } };
    (0, test_types_js_1.record)(persistedUsage.models)['test/model'] = { provider: 'test', model: 'model', days: { unknown: {} } };
    delete persistedUsage.state;
    fs.appendFileSync(log, JSON.stringify(valid) + '\n');
    const warm = withBudget(0, () => index.scanSessions([source]));
    assert.equal(warm.indexing, false);
    const usage = (0, test_types_js_1.present)(warm.infos.get(file)).usage;
...
const indexedHarnesses = ['pi', 'omp'];
for (const harnessId of indexedHarnesses) {
    test(`${harnessId} malformed metadata preserves indexed usage and skill continuity on full reads and append`, () => {
        ...
        const file = writeSession([...valid, ...malformed]);
        const source = sourceForIdentity(harnessId, (0, test_types_js_1.nativeId)(path.basename(file, '.jsonl')), file);
        const first = index.scanSessions([source]);
        assert.equal(first.indexing, false);
        const info = (0, test_types_js_1.present)(first.infos.get(file));
        ...
        const activations = () => index.getSkillActivations({ skill }).filter((value) => (0, test_types_js_1.record)(value).sessionId === identity);
        assert.equal(activations().length, 1, 'relative skill read keeps its valid cwd');
        assert.equal((0, test_types_js_1.record)((0, test_types_js_1.present)(activations()[0])).model, 'openai/gpt-known');
        assert.equal((0, test_types_js_1.record)((0, test_types_js_1.present)(activations()[0])).entryId, null, 'malformed entry provenance does not drop a valid skill read');
        ...
        const next = (0, test_types_js_1.present)(extended.infos.get(file));
        ...
        assert.ok(activations().every((value) => { const row = (0, test_types_js_1.record)(value); return row.cwd === cwd && row.model === 'openai/gpt-known'; }));
    });
}
