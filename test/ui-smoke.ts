 ...
 */
import scenarios = require('./ui-scenarios/index.js');
import type { UiRegistryState, UiScenarioContext } from './ui-scenarios/contracts.js';
import type { SessionEntry } from '../src/browser/session-state.js';
import { chromium, type ConsoleMessage, type Dialog, type Page, type Request as PlaywrightRequest, type Response as PlaywrightResponse, type Route } from 'playwright';
import { once } from 'node:events';
import http = require('node:http');
import type { IncomingMessage, ServerResponse } from 'node:http';
import { spawn, type ChildProcess, execFileSync } from 'node:child_process';
import { isRecord } from '../lib/wire-protocol.js';
import { errorMessage, record, records } from './test-types.js';
...
const selectedScenario: ScenarioName | null = isScenarioName(requestedScenario) ? requestedScenario : null;
...
const appendEntry = (entry: unknown): void => fs.appendFileSync(sessionFile, JSON.stringify(entry) + '\n');
...
const bridge = net.createServer((sock) => {
  ...
  sock.on('data', (chunk) => {
    ...
    while ((idx = buf.indexOf('\n')) >= 0) {
      ...
      let message: unknown; try { message = JSON.parse(line); } catch { continue; }
      if (isRecord(message)) handleCommand(sock, message);
    }
  });
});

function respond(sock: Socket, id: unknown, data: unknown): void {
...
let liveQueue: { steering: unknown[]; followUp: unknown[] } = { steering: [], followUp: [] };
let runCommandCount = 0;
function setQueue(queue: { steering: unknown[]; followUp: unknown[] }): void { liveQueue = queue; emit('queue_update', liveQueue); }

function handleCommand(sock: Socket, msg: Record<string, unknown>): void {
  ...
  switch (msg.command) {
    ...
    case 'set_session_name':
      ...
      if (typeof msg.name !== 'string') throw new Error('set_session_name requires a string name');
...
let lastUIResponse: Record<string, unknown> | null = null;
let fakeCompacting = false;
let bufferedPrompt: Record<string, unknown> | null = null;
...
function streamTurn(userText: unknown, images: unknown): void {
...
const sttRequests: unknown[] = [];
const sttEndpoint = http.createServer(async (req: IncomingMessage, res: ServerResponse) => {
  try {
    const chunks: Buffer[] = [];
    ...
    sttRequests.push({ error: errorMessage(error) });
  }
  res.writeHead(200, { 'Content-Type': 'application/json' });
  res.end(JSON.stringify({ text: 'transcribed hello' }));
});
const dishSettingsFile = path.join(tmpHome, '.pi', 'dish', 'settings.json');
function writeSttSettings(block: unknown): void {
  const parsed: unknown = fs.existsSync(dishSettingsFile) ? JSON.parse(fs.readFileSync(dishSettingsFile, 'utf8')) : {};
  if (!isRecord(parsed)) throw new Error('Fixture settings must be an object');
  if (block) parsed.stt = block; else delete parsed.stt;
...
function writeRemoteFixture(mainOrigin: string): void {
...
async function startRemoteHost(): Promise<{ child: ChildProcess; base: string }> {
  const serverPath = path.join(__dirname, '..', 'server.js');
  const child = spawn(process.execPath, [REMOTE_SERVER_HOST, serverPath], {
    env: {
      ...process.env,
      HOME: remoteHome,
      PORT: '0',
  ...
  if (!child.stdout) throw new Error('Remote host stdout is unavailable');
  let buffered = '';
...
let registryState: UiRegistryState = {
...
function writeRegistry(patch: Partial<UiRegistryState> = {}): void {
  registryState = { ...registryState, ...patch };
  fs.writeFileSync(path.join(registryDir, `${SESSION_ID}.json`), JSON.stringify(registryState));
}

let remoteHost: { child: ChildProcess; base: string } | null = null;
(async () => {
  bridge.listen(socketPath);
  await once(bridge, 'listening');
  writeRegistry();
  sttEndpoint.listen(0, '127.0.0.1');
  await once(sttEndpoint, 'listening');
  const sttAddress = sttEndpoint.address();
  if (sttAddress === null || typeof sttAddress === 'string') throw new Error('Expected STT TCP address');
  writeSttSettings({
    url: `http://127.0.0.1:${sttAddress.port}/v1/audio/transcriptions`,
    model: 'whisper-1',
  });

  const server: http.Server = require('../server.js');
  if (!server.listening) await once(server, 'listening');
  const serverAddress = server.address();
  if (serverAddress === null || typeof serverAddress === 'string') throw new Error('Expected server TCP address');
  ...
  const errors: string[] = [];
  const watch = async (page: Page, tag: string): Promise<void> => {
    await installFixtureApp(page);
    page.on('pageerror', (error: Error) => errors.push(`${tag} pageerror: ${error.message}`));
  ...
  try {
    ...
    const scenarioContext = (): UiScenarioContext => ({ path, tmpHome, desktop, check, fs, registryState, SKILL_SESSION_ID, CWD, BETA_ID, SESSION_ID, base, browser, watch, emit });
    if (selectedScenario) {
      await desktop.goto(base, { waitUntil: 'networkidle' });
      await desktop.click('#tabAll');
      await desktop.evaluate((id: string) => fixtureApp.features.sessionView.select(id), SESSION_ID);
    ...
    check(!coldResources.some((pathname: string) =>
    ...
    check(await desktop.locator('.message.custom-message.interrupted').count() === 1 &&
      await desktop.locator('.message.custom-message.interrupted').textContent().then((t: string | null) => !!t?.includes('Interrupted')),
    ...
    await desktop.waitForFunction((id: string) => fixtureApp.features.sessionState.currentSession?.id === id, SWITCH_ID, { timeout: 5000 });
    ...
    await desktop.waitForFunction((id: string) => fixtureApp.features.sessionState.currentSession?.id === id, SESSION_ID, { timeout: 5000 });
    ...
    check(!(await histGroup.evaluate((el: HTMLDetailsElement) => el.open)), 'tool-group is collapsed by default');
    ...
    if (histLabel === null) throw new Error('Missing histLabel');
    ...
    check((await imgResult.locator('.tool-result-meta').allTextContents()).some((t: string) => t.trim() === 'image'),
      'tool-result header meta flags the image');
    check(await imgResult.locator('details.tool-result-details').evaluate((el: HTMLDetailsElement) => el.open),
      'image tool result is open by default');
    await histGroup.evaluate((el: HTMLDetailsElement) => { el.open = true; });
    ...
    await desktop.waitForFunction(() => {
      const img = document.querySelector<HTMLImageElement>('.message.tool-result img.msg-image');
      return img?.complete && img.naturalWidth === 1;
    }, { timeout: 3000 });
    check(true, 'resource-backed historical image renders successfully');
    await histGroup.evaluate((el: HTMLDetailsElement) => { el.open = false; });
    ...
    check(await desktop.evaluate(() => {
      ...
      if (el === null) throw new Error('Missing el');
    ...
    check(finals.some((t: string) => t.includes('Streamed reply with')), 'final assistant message rendered');
    ...
    check(await desktop.evaluate(() =>
      !fixtureElement(document.getElementById('sessionWorking'), "document.getElementById('sessionWorking')").classList.contains('active')),
    ...
    await desktop.waitForFunction(() =>
      fixtureElement(document.querySelector('.diagram-block .code-copy-btn'), "document.querySelector('.diagram-block .code-copy-btn')").textContent === '✓',
      null, { timeout: 3000 });
    ...
    check(await desktop.evaluate(() =>
      fixtureElement(document.querySelector('.diagram-lightbox .diagram-stage svg'), "document.querySelector('.diagram-lightbox .diagram-stage svg')").getBoundingClientRect().width) > zoomed.width,
    ...
    check(detailRows.some((t: string) => t.includes('Estimated input') && t.includes('Unavailable')),
      'response details label an omitted component unavailable');
    check(detailRows.some((t: string) => t.includes('Estimated total') && t.includes('~$0.0010')),
    ...
    desktop.once('dialog', (d: Dialog) => d.accept());
    ...
    if (copiedRef === null) throw new Error('Missing copiedRef');
    ...
    if (statsText === null) throw new Error('Missing statsText');
    ...
    if (filePath === null) throw new Error('Missing filePath');
    ...
    await desktop.waitForSelector('#fileView .markdown-body h1', { timeout: 5000 });
    check(await desktop.evaluate(() => fixtureElement(document.getElementById('messages'), "document.getElementById('messages')").offsetParent === null),
    ...
    if (shownPath === null) throw new Error('Missing shownPath');
    ...
    if (rawHref === null) throw new Error('Missing rawHref');
    ...
    const rawResponse = await desktop.evaluate(async () => {
      const response = await fetch(fixtureElement(document.querySelector<HTMLAnchorElement>('#fileViewRaw'), '#fileViewRaw').href);
    ...
    await desktop.evaluate(() => {
      const paragraph = [...document.querySelectorAll<HTMLElement>('#fileViewBody p')]
    ...
    if (bubbleBox === null) throw new Error('Missing bubbleBox');
    ...
    if (narrowBubbleBox === null) throw new Error('Missing narrowBubbleBox');
    ...
    await desktop.click('#commentSendBtn');
    await desktop.waitForSelector('#commentBubble', { state: 'hidden', timeout: 5000 });
    const editedComments = await desktop.evaluate(async (id: string) => {
      const res = await fetch(`/api/comments/index?sessionId=${encodeURIComponent(id)}`);
      const body: unknown = await res.json();
      if (typeof body !== 'object' || body === null || !('comments' in body) || !Array.isArray(body.comments)) throw new Error('Invalid comment index');
      return body.comments.map((comment) =>
        typeof comment === 'object' && comment !== null && 'bodyPreview' in comment ? comment.bodyPreview : null);
    }, SESSION_ID);
    ...
    await desktop.waitForFunction(() =>
      document.querySelectorAll('#fileViewBody mark.comment-mark').length === 0
        && fixtureElement(document.getElementById('fileViewComments'), "document.getElementById('fileViewComments')").style.display === 'none',
      { timeout: 5000 });
    ...
    if (!pageLink) throw new Error('Published page link is missing');
    ...
    if (!pageHref) throw new Error('Page artifact link is missing');
    ...
    await desktop.waitForFunction(() =>
      fixtureElement(document.getElementById('fileViewPage'), "document.getElementById('fileViewPage')").style.display === 'none', { timeout: 5000 });
    ...
    await desktop.keyboard.press('Escape');
    await desktop.waitForSelector('#fileView', { state: 'hidden', timeout: 2000 });
    await desktop.waitForFunction(() => fixtureElement(document.getElementById('messages'), "document.getElementById('messages')").offsetParent !== null,
      { timeout: 2000 });
    ...
    const diffFiles = await desktop.evaluate(() =>
      [...document.querySelectorAll<HTMLDetailsElement>('.diff-file')].map((el) => ({
        status: fixtureElement(el.querySelector<HTMLElement>('.diff-status'), '.diff-status').textContent,
        path: fixtureElement(el.querySelector<HTMLElement>('.diff-file-path'), '.diff-file-path').textContent,
        open: el.open,
      })));
    ...
    await desktop.evaluate(() => {
      const line = [...document.querySelectorAll<HTMLElement>('.diff-line.diff-add')]
    ...
    await desktop.click('#commentSendBtn');
    await desktop.waitForSelector('#commentBubble', { state: 'hidden', timeout: 5000 });
    const diffComments = record(await (await fetch(`${base}/api/comments/index?sessionId=${SESSION_ID}`)).json());
    const diffComment = records(diffComments.comments)
      .find(comment => record(comment.target).kind === 'diff');
    check(diffComments.total === 1 && record(record(diffComment).target).anchor !== undefined
      && record(record(record(diffComment).target).anchor).newStart === 2,
    ...
    if (userBox === null) throw new Error('Missing userBox');
    const feedBox = await wide.locator('.messages').boundingBox();
    if (feedBox === null) throw new Error('Missing feedBox');
    ...
    await desktop.route('**/api/harnesses', async (route: Route) => {
      await route.fulfill({
        status: 200,
    ...
    await desktop.route(/\/api\/models\?harness=omp(?:&|$)/, async (route: Route) => {
    ...
    let ompGlobalRoles: Record<string, string> = { default: 'zai/glm-4.7-flash', smol: 'zai/glm-5.2', advisor: 'zai/glm-4.7-flash:max' };
    await desktop.route(/\/api\/harnesses\/omp\/config(?:\?|$)/, async (route: Route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
    ...
    let modelRolesPatch: Record<string, unknown> | undefined;
    const readModelRolesPatch = () => modelRolesPatch;
    await desktop.route('**/api/harnesses/omp/model-roles', async (route: Route) => {
      modelRolesPatch = record(route.request().postDataJSON());
      const roles = record(modelRolesPatch.roles);
      for (const [role, value] of Object.entries(roles)) {
        if (value === null) delete ompGlobalRoles[role];
        else if (typeof value === 'string') ompGlobalRoles[role] = value;
      }
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
    ...
    let agentsPatch: Record<string, unknown> | undefined;
    const readAgentsPatch = () => agentsPatch;
    await desktop.route(/\/api\/harnesses\/omp\/agents(?:\?|$)/, async (route: Route) => {
      if (route.request().method() === 'PUT') {
        agentsPatch = record(route.request().postDataJSON());
        const agents = record(agentsPatch.agents);
        for (const [name, value] of Object.entries(agents)) {
          ...
          else if (typeof settings.model === 'string' && settings.model) ompAgentSettings.modelOverrides[name] = settings.model;
          for (const key of ['prewalk', 'advisor'] as const) {
            if (settings[key] === null) delete ompAgentSettings[key][name];
            else if (typeof settings[key] === 'boolean') ompAgentSettings[key][name] = settings[key];
          }
        }
      }
    ...
    check(await desktop.evaluate(() => fixtureElement(document.getElementById('harnessSettingsModal'), "document.getElementById('harnessSettingsModal')").style.display === 'none') &&
      await desktop.evaluate(() => fixtureElement(document.querySelector('.main'), "document.querySelector('.main')").classList.contains('new-session-open')),
    ...
    const planLevels = await desktop.locator('.model-role-level[data-role="plan"] option')
      .evaluateAll(options => options.map(option => {
        if (!(option instanceof HTMLOptionElement)) throw new Error('Expected model role option');
        return option.value;
      }));
    ...
    await desktop.waitForFunction(() =>
      fixtureElement(document.getElementById('harnessSettingsModal'), "document.getElementById('harnessSettingsModal')").style.display === 'none');
    check(JSON.stringify(record(readModelRolesPatch()).roles) === JSON.stringify({
      ...
      `(got ${JSON.stringify(record(readModelRolesPatch()).roles)})`);
    check(JSON.stringify(record(readAgentsPatch()).agents) === JSON.stringify({
      reviewer: { disabled: true }, scout: { model: null, prewalk: false },
    }), `saving PUTs only the changed agent settings (got ${JSON.stringify(record(readAgentsPatch()).agents)})`);
    await desktop.waitForFunction(() =>
      fixtureElement(document.getElementById('nsHarnessRoles'), "document.getElementById('nsHarnessRoles')").textContent.includes('plan zai/glm-5.2'));
    ...
    check(opts.some((t: string) => t.includes('workspace/proj-alpha')), 'proj-alpha found by fuzzy dir search');
    ...
    await desktop.waitForFunction((cwd: string) => fixtureApp.features.modelCatalog.scope?.cwd === cwd, CWD, { timeout: 5000 });
    ...
    let asyncSpawnBody: Record<string, unknown> | undefined;
    const readAsyncSpawnBody = () => asyncSpawnBody;
    await desktop.route('**/api/sessions/new', async (route: Route) => {
      asyncSpawnBody = record(route.request().postDataJSON());
    ...
    await desktop.route('**/api/session-spawns/ui-spawn-1', async (route: Route) => {
    ...
    const submittedSpawn = record(readAsyncSpawnBody());
    ...
    check(!(await desktop.evaluate(() => fixtureElement(document.querySelector('.main'), "document.querySelector('.main')").classList.contains('new-session-open'))),
      'takeover closes in favor of the provisional pane');
    check(await desktop.locator('.session-item.starting').textContent().then((text: string | null) => !!text?.includes('Starting Oh My Pi')),
      'the provisional row identifies the selected harness before registration');
    check(await desktop.locator('.session-item.starting').evaluate((el: HTMLElement) => el.classList.contains('active')) &&
    ...
    check(await desktop.evaluate((key: string) => localStorage.getItem(fixtureApp.features.composerDrafts.draftKey(key)), provisionalKey) === startupDraft,
    ...
    await desktop.waitForFunction(({ id, draft }) => fixtureApp.features.sessionState.currentSession?.id === id &&
      fixtureInput(document.getElementById('promptInput'), "document.getElementById('promptInput')").value === draft,
    ...
    spawnResult = 'error';
    await desktop.waitForFunction(() => fixtureElement(document.getElementById('sessionName'), "document.getElementById('sessionName')").textContent === 'Session failed to start',
      null, { timeout: 3000 });
    check(await desktop.inputValue('#promptInput') === failedDraft &&
      await desktop.locator('#btnSend').isDisabled(),
      'failed spawn keeps its provisional draft accessible');
    await desktop.evaluate(async (id: string) => {
    ...
    await desktop.waitForFunction(() => fixtureElement(document.getElementById('sessionName'), "document.getElementById('sessionName')").textContent === 'renamed live',
      null, { timeout: 5000 });
    ...
    await desktop.evaluate(async (b64: string) => {
    ...
    const promptImages = records(record(lastPrompt).images);
    const promptImage = record(promptImages[0]);
    check(promptImages.length === 1 && promptImage.data === PNG_B64 && promptImage.mimeType === 'image/png',
    ...
    await dictation.waitForFunction(
      () => fixtureInput(document.getElementById('promptInput'), "document.getElementById('promptInput')").value.includes('transcribed hello'),
      { timeout: 20000 });
    ...
    await desktop.waitForFunction(() =>
      fixtureInput(document.getElementById('promptInput'), "document.getElementById('promptInput')").value.includes('steer me now'), { timeout: 5000 });
    ...
    await desktop.waitForFunction((t: string) =>
    ...
    check(!(await desktop.evaluate(() => /Compacting context/.test(document.getElementById('status')?.textContent || ''))),
      'working prose stays off the status line');
    ...
    check(!(await desktop.evaluate((id: string) => fixtureApp.features.sessionActivity.isAborting(fixtureApp.ports.composerDrafts.keyForSession(id)), SESSION_ID)),
    ...
    check(await desktop.evaluate(() => fixtureInput(document.getElementById('promptInput'), "document.getElementById('promptInput')").value) === '/compact',
    ...
    flushBufferedPrompt();
    await desktop.waitForFunction((n: number) =>
    ...
    const termText = () => desktop.evaluate(() => {
      ...
      if (!state) throw new Error('Terminal state is unavailable');
      const buffer = state.term.buffer.active;
      let out = '';
      for (let i = 0; i < buffer.length; i++) out += buffer.getLine(i)?.translateToString(true) + '\n';
      return out;
    });
    await desktop.waitForFunction(() => fixtureElement(document.getElementById('terminalStatus'), "document.getElementById('terminalStatus')").textContent === '',
    ...
    if (cwdLabel === null) throw new Error('Missing cwdLabel');
    ...
    check(await desktop.evaluate(() => fixtureElement(document.getElementById('terminalPanel'), "document.getElementById('terminalPanel')").style.display === 'none'),
    ...
    if (termHandleBox === null) throw new Error('Missing termHandleBox');
    ...
    if (await desktop.evaluate(() => fixtureApp.ports.routinesController.config().tmux)) {
      ...
      await desktop.waitForFunction(() =>
        /tmux pane/i.test(fixtureElement(document.getElementById('terminalStatus'), "document.getElementById('terminalStatus')").textContent), { timeout: 5000 });
    ...
    await desktop.waitForFunction(() =>
      [...document.querySelectorAll<HTMLElement>('.ext-ui-widget')].some((el) => el.dataset.widgetKey === 'todo'),
      { timeout: 3000 });
    await desktop.evaluate(() => { fixtureElement(document.querySelector<HTMLElement>('[data-widget-key="todo"]'), '[data-widget-key="todo"]').dataset.marker = 'kept'; });
    ...
    check(await desktop.evaluate(() => {
      const els = [...document.querySelectorAll<HTMLElement>('[data-widget-key="todo"]')];
      return els.length === 1 && els[0]?.dataset.marker === 'kept'
        && !els[0].classList.contains('hidden')
        && fixtureElement(els[0].querySelector<HTMLElement>('.ext-ui-widget-body'), '.ext-ui-widget-body').textContent === 'task one\ntask two';
    ...
    await desktop.evaluate(() => { fixtureElement(document.querySelector<HTMLElement>('.ext-ui-status-badge'), '.ext-ui-status-badge').dataset.marker = 'kept'; });
    emit('extension_ui_request', { method: 'setStatus', statusKey: 'procs' });
    await desktop.waitForTimeout(50);
    emit('extension_ui_request', { method: 'setStatus', statusKey: 'procs', statusText: '2 running' });
    await desktop.waitForTimeout(700); // past the removal grace
    check(await desktop.evaluate(() => {
      const els = [...document.querySelectorAll<HTMLElement>('.ext-ui-status-badge')];
    ...
    check(await desktop.evaluate(() =>
      getComputedStyle(fixtureElement(document.getElementById('extUiStatusToggle'), '#extUiStatusToggle')).display === 'none'),
    ...
    check(await desktop.evaluate(() =>
      !fixtureElement(document.getElementById('extUiStatuses'), "document.getElementById('extUiStatuses')").classList.contains('collapsed')),
    ...
    check(await desktop.evaluate(() =>
      !fixtureElement(document.querySelector('.input-area'), "document.querySelector('.input-area')").classList.contains('ext-dialog-takeover') &&
      getComputedStyle(fixtureElement(document.getElementById('promptInput'), '#promptInput')).display !== 'none'),
    ...
    check(await desktop.evaluate(() =>
      fixtureElement(document.querySelector('#extUiDialogs .ext-ui-dialog-modal'), "document.querySelector('#extUiDialogs .ext-ui-dialog-modal')").classList.contains('minimized')),
    ...
    await desktop.waitForFunction(() =>
      !fixtureElement(document.querySelector('#extUiDialogs .ext-ui-dialog-modal'), "document.querySelector('#extUiDialogs .ext-ui-dialog-modal')").classList.contains('minimized'), { timeout: 5000 });
    await desktop.click('[data-action="submit-ask"]');
    await desktop.waitForFunction(() => !document.getElementById('extUiDialogs'), { timeout: 5000 });
    const uiResponse = record(lastUIResponse); const uiValue = record(uiResponse.value);
    const uiResults = records(uiValue.results); const firstUiResult = record(uiResults[0]);
    const selectedOptions = firstUiResult.selectedOptions;
    check(uiResponse.requestId === 'dlg-ask-1' && uiValue.kind === 'submit'
      && Array.isArray(selectedOptions) && selectedOptions[0] === 'Beta',
    ...
    await desktop.evaluate((b: string) => fixtureApp.features.sessionView.select(b, { forceTranscriptReload: true }), SESSION2_ID);
    await desktop.waitForFunction((b: string) => fixtureApp.features.sessionState.currentSession?.id === b &&
    ...
    const rapidSelection = await desktop.evaluate((b: string) => ({
    ...
    await desktop.evaluate((a: string) => fixtureApp.features.sessionView.select(a, { forceTranscriptReload: true }), SESSION_ID);
    await desktop.waitForSelector('#messages .message.assistant', { timeout: 5000 });
    await desktop.evaluate((a: string) => {
      const realFetch = window.fetch.bind(window);
      window.__auditRealFetch = realFetch;
      let held = false;
      window.fetch = (input, init) => {
    ...
    await desktop.evaluate(({ a, image }) => {
      ...
      if (!composerKey) throw new Error('Composer key is unavailable');
      ...
      window.__auditFailedPromptId = document.querySelector<HTMLElement>('#messages [data-client-prompt-id]:last-child')?.dataset.clientPromptId;
    ...
    const failedOwnership = await desktop.evaluate((a: string) => ({
    ...
    await desktop.evaluate((a: string) => {
      ...
      const row = document.querySelector<HTMLElement>('.queue-item');
      if (row === null) throw new Error('Missing row');
      window.__auditQueueAssociation = row.dataset.clientPromptId;
      const edit = fixtureElement(row.querySelector<HTMLElement>('.queue-item-edit'), '.queue-item-edit');
      ...
      fixtureApp.features.promptDelivery.render({ followUp: ['duplicate buffered prompt'] });
      window.__auditRemainingQueueAssociation = document.querySelector<HTMLElement>('.queue-item')?.dataset.clientPromptId;
    }, SESSION_ID);
    ...
    const queueOwnership = await desktop.evaluate((a: string) => ({
      currentText: fixtureInput(document.getElementById('promptInput'), "document.getElementById('promptInput')").value,
      originDraft: localStorage.getItem(fixtureApp.features.composerDrafts.draftKey(a)),
    ...
    await desktop.evaluate(async () => {
      ...
      fixtureApp.features.apiTransport.request = async (host, url, options) => {
        if (url === '/api/models/enabled') {
          if (typeof options?.body !== 'string') throw new Error('Enabled-model fixture expected a JSON body');
          const body: unknown = JSON.parse(options.body);
          if (!body || typeof body !== 'object' || Array.isArray(body)) {
            throw new Error('Enabled-model fixture expected an object body');
          }
          window.__auditEnabledBody = body as Record<string, unknown>;
    ...
    await desktop.evaluate((a: string) => { fixtureApp.features.appModels.load(a); return fixtureApp.features.sessionView.select(a, { forceTranscriptReload: true }); }, SESSION_ID);
    ...
    await desktop.evaluate(() => {
      ...
      const heldFonts = new Promise<void>(resolve => { releaseFonts = resolve; });
      if (!releaseFonts) throw new Error('Font readiness resolver was not initialized');
    ...
    check(await desktop.evaluate(() => !fixtureApp.features.terminalController.state && window.__auditTerminalUrls.length === 0 &&
      fixtureElement(document.getElementById('terminalPanel'), "document.getElementById('terminalPanel')").style.display === 'none'),
    ...
    const terminalOwnership = await desktop.evaluate(({ a, b }) => ({
      ...
      currentId: fixtureApp.features.sessionState.currentSession?.id,
      hasAUrl: window.__auditTerminalUrls.some((url: string) => url.includes(encodeURIComponent(a))),
      hasBUrl: window.__auditTerminalUrls.some((url: string) => url.includes(encodeURIComponent(b))),
    ...
    await desktop.waitForSelector(`.session-item[data-id="${registryState.sessionId}"] .session-item-tags`, { timeout: 5000 });
    {
      ...
      const rows = await desktop.locator(rowSel).evaluate((el: HTMLElement) =>
        [...el.querySelectorAll<HTMLElement>(':scope > *')].map((child) => child.className));
      ...
      check(await desktop.locator(`${rowSel} .session-item-meta`).textContent()
        .then((text: string | null) => !/msgs/.test(text || '')), 'the message count is gone from the card');
    ...
    check(await desktop.locator('#btnTree').count() === 0, 'the tree button is gone from the session header');
    {
      ...
      const before = await desktop.locator('#sidebar').evaluate((el: HTMLElement) => el.offsetWidth);
      ...
      await desktop.mouse.up();
      const after = await desktop.locator('#sidebar').evaluate((el: HTMLElement) => el.offsetWidth);
      ...
      check(await desktop.locator('#sidebar').evaluate((el: HTMLElement) => el.offsetWidth) === before &&
    ...
    check(await desktop.evaluate(() =>
      fixtureElement(document.querySelector('.sidebar-filter'), "document.querySelector('.sidebar-filter')").classList.contains('searching')),
    ...
    await desktop.waitForFunction(() =>
      !fixtureElement(document.querySelector('.sidebar-filter'), "document.querySelector('.sidebar-filter')").classList.contains('searching'), null, { timeout: 5000 });
    ...
    await desktop.waitForFunction((ids: string) =>
      JSON.stringify([...document.querySelectorAll<HTMLElement>('#sessionList .session-item')].map((el) => el.dataset.id)) === ids,
    ...
    await desktop.click('#tabActive');
    await desktop.waitForSelector(`.session-item[data-id="${registryState.sessionId}"] .session-item-snippet`, { timeout: 5000 });
    check(await desktop.evaluate(() => fixtureInput(document.getElementById('filterInput'), "document.getElementById('filterInput')").value) === 'existing',
    ...
    await desktop.waitForFunction((betaId: unknown) =>
    ...
    check(await desktop.evaluate(() => fixtureInput(document.getElementById('filterInput'), "document.getElementById('filterInput')").value) === '',
    ...
    await desktop.click(`.session-item[data-id="${BETA_ID}"]`);
    await desktop.waitForFunction((count: number) =>
    ...
    await desktop.evaluate((id: string) => fixtureApp.features.sessionView.select(id), BETA_ID);
    await desktop.waitForSelector('#sessionRelations .session-relation-chip', { timeout: 5000 });
    check((await desktop.locator('#sessionRelations').textContent() ?? '').includes('Subagents'),
    ...
    await desktop.waitForFunction((id: string) => fixtureApp.features.sessionState.currentSession?.id === id, registryState.sessionId, { timeout: 5000 });
    check(true, 'the peek pane Open action navigates to the available peer session');
    const relationRaceOwner = await desktop.evaluate(async (nextId: string) => {
      const originalLoad = fixtureApp.features.sidebarLists.load;
      let release: (() => void) | undefined;
      fixtureApp.features.sidebarLists.load = () => new Promise<void>(resolve => { release = resolve; });
      try {
        ...
        if (!release) throw new Error('Relation load release was not initialized');
        release();
        await pending;
        const current = fixtureApp.features.sessionState.currentSession;
        if (!current) throw new Error('Current session is unavailable');
        return current.id;
      } finally {
    ...
    await desktop.evaluate((id: string) => fixtureApp.features.sessionView.select(id), registryState.sessionId);
    await desktop.waitForFunction((id: string) => fixtureApp.features.sessionState.currentSession?.id === id, registryState.sessionId);
    ...
    await desktop.route('**/api/sessions/*/lineage', async (route: Route) => {
    ...
    await desktop.keyboard.press('Escape');
    await desktop.waitForFunction(
      () => !fixtureElement(document.querySelector('.main'), "document.querySelector('.main')").classList.contains('subagents-open'),
      null, { timeout: 3000 });
    ...
    check(await desktop.locator('.main').evaluate((el: HTMLElement) => !el.classList.contains('subagents-open')),
      'navigation closes the subagents takeover');
    await desktop.unroute('**/api/sessions/*/lineage');
    await desktop.evaluate((id: string) => fixtureApp.features.sessionView.select(id), registryState.sessionId);
    await desktop.waitForFunction((id: string) => fixtureApp.features.sessionState.currentSession?.id === id &&
      !fixtureElement(document.getElementById('sessionRelations'), "document.getElementById('sessionRelations')").textContent.includes('overflow-'),
    ...
    await desktop.waitForFunction(() =>
      !fixtureElement(document.querySelector('.scope-chip[data-name="Beta only"]'), 'document.querySelector(\'.scope-chip[data-name="Beta only"]\')').classList.contains('active'));
    ...
    await desktop.waitForFunction((id: string) =>
      ...
      BETA_ID, { timeout: 10000 });
    check(!await desktop.evaluate(() =>
      fixtureElement(document.querySelector('.search-count-line'), "document.querySelector('.search-count-line')").textContent.includes('showing the 100 best matches')),
    ...
    await desktop.waitForFunction(() =>
      fixtureElement(document.querySelector('.main'), "document.querySelector('.main')").classList.contains('search-open'), null, { timeout: 5000 });
    check(await desktop.evaluate(() => fixtureInput(document.getElementById('searchViewInput'), "document.getElementById('searchViewInput')").value) === 'beta',
      'full-search chip carries the sidebar query into the takeover');
    check(await desktop.evaluate(() => fixtureElement(document.getElementById('sessionView'), "document.getElementById('sessionView')").offsetParent === null),
    ...
    const countText = await desktop.evaluate((id: string) =>
    ...
    await desktop.waitForFunction((id: string) =>
      fixtureInput(document.getElementById('searchViewInput'), "document.getElementById('searchViewInput')").value.includes('is:active') &&
    ...
    check(!(await desktop.evaluate(() => fixtureInput(document.getElementById('searchViewInput'), "document.getElementById('searchViewInput')").value)).includes('is:active'),
    ...
    check(await desktop.evaluate(() => fixtureInput(document.getElementById('searchInput'), "document.getElementById('searchInput')").value) === 'cedar maple',
    ...
    check(await desktop.evaluate(() =>
      ...
      fixtureElement(document.getElementById('searchCount'), "document.getElementById('searchCount')").textContent !== 'no matches'),
      'distributed-term click-through lands on a relevant transcript message');
    check(await desktop.evaluate(() => fixtureElement(document.getElementById('sessionName'), "document.getElementById('sessionName')").textContent) === 'beta question',
    ...
    const preOpacity = await desktop.$eval(`${closeRowSel} .session-close-btn`, (el: HTMLElement) => getComputedStyle(el).opacity);
    check(preOpacity === '0', `close button hidden until hover (opacity ${preOpacity})`);
    await desktop.hover(closeRowSel);
    await desktop.waitForFunction((sel: string) => {
      const button = fixtureElement(document.querySelector<HTMLElement>(sel), sel);
    ...
    await desktop.waitForFunction((sel: string) => !document.querySelector(sel),
    ...
    let closePostUrl: string | null = null;
    const readClosePostUrl = () => closePostUrl;
    let resolveClosePost: (() => void) | undefined;
    const closePostFired = new Promise<void>(resolve => { resolveClosePost = resolve; });
    await desktop.route('**/api/sessions/*/close', async (route: Route) => {
      closePostUrl = route.request().url();
      if (!resolveClosePost) throw new Error('Close route resolver was not initialized');
      resolveClosePost();
    ...
    check(typeof postedCloseUrl === 'string' && postedCloseUrl.includes(`/api/sessions/${SESSION_ID}/close`),
      `second tap fires POST /close for the row (got ${JSON.stringify(postedCloseUrl)})`);
    ...
    const dummyGone = new Promise<void>(resolve => dummy.on('exit', (_code: number, sig: NodeJS.Signals | null) => { dummySignal = sig; resolve(); }));
    ...
    const runtimeRow = await desktop.evaluate(() => {
      const row = [...document.querySelectorAll<HTMLElement>('#statsBody tr')]
        .find((tr) => tr.querySelector('.stats-key')?.textContent === 'Running in');
      return row ? fixtureElement(row.querySelector<HTMLElement>('.stats-val'), '.stats-val').textContent : null;
    });
    ...
    const rpcRestartId = await desktop.evaluate(async (id: string) => {
    ...
    await desktop.waitForFunction((id: string) => fixtureApp.features.sessionState.currentSession?.id === id && fixtureApp.features.sessionState.currentSession.isActive,
    ...
    const keys = await desktop.evaluate((id: string) => {
    ...
    const remoteDescriptorBody = record(await fetch(remoteHost.base + '/api/host').then(response => response.json()));
    if (typeof remoteDescriptorBody.hostId !== 'string') throw new Error('Remote host descriptor is invalid');
    const remoteDescriptor = remoteDescriptorBody as typeof remoteDescriptorBody & { hostId: string };
    ...
    multi.on('request', (request: PlaywrightRequest) => { if (request.url().startsWith(remoteBase)) remoteRequests.push(request.url()); });
    ...
    const localRecoveryBefore = record(await fetch(base + '/api/settings').then(response => response.json()));
    ...
    const remoteRecovery = record(await fetch(remoteHost.base + '/api/settings', {
      headers: { Authorization: 'Bearer ' + REMOTE_TOKEN },
    }).then(response => response.json()));
    const localRecoveryAfter = record(await fetch(base + '/api/settings').then(response => response.json()));
    ...
    const descriptorRoute = async (route: Route) => route.fulfill({
      json: { ...remoteDescriptor, capabilities: { ...(isRecord(remoteDescriptor.capabilities) ? remoteDescriptor.capabilities : {}), recovery: advertisesRecovery } },
    });
    ...
    const beforeFleetSettings = record(JSON.parse(fs.readFileSync(dishSettingsFile, 'utf8')));
    ...
    const [proxySave] = await Promise.all([
      multi.waitForResponse((response: PlaywrightResponse) => response.url() === base + '/hosts/tycho/api/settings' && response.request().method() === 'PUT'),
      multi.click('#saveRecoveryMode'),
    ]);
    const proxiedRecovery = record(await fetch(remoteHost.base + '/api/settings', {
      headers: { Authorization: 'Bearer ' + REMOTE_TOKEN },
    }).then(response => response.json()));
    ...
    const groupKeys = await multi.evaluate((cwd: string) => [...document.querySelectorAll<HTMLElement>('.workspace-group-header')]
    ...
    const sections = await multi.evaluate(() => [...document.querySelectorAll<HTMLElement>('#sessionList > .host-section')]
      .map((section) => ({
        name: section.querySelector('.host-section-name')?.textContent || null,
        key: section.querySelector<HTMLElement>('.host-section-header')?.dataset.hostSection || null,
        color: section.style.getPropertyValue('--host-color'),
        dot: getComputedStyle(fixtureElement(section.querySelector<HTMLElement>('.host-section-dot'), '.host-section-dot')).backgroundColor,
    ...
    const remoteRefResolution = await multi.evaluate((remoteId: string) => {
      ...
      const current = fixtureApp.features.sessionState.currentSession;
      if (!current) throw new Error('Current remote session is unavailable');
      ...
      const publish = (extra: SessionEntry | null) => {
        const parts = new Map<string | null, { hostId: string | null; active: SessionEntry[]; previous: SessionEntry[] }>();
        for (const kind of ['active', 'previous'] as const) {
    ...
    const noteList = (request: PlaywrightRequest) => { if (/\/api\/sessions(\?|$)/.test(request.url())) listReqs.push(request.url()); };
    ...
    await multi.waitForFunction(() => fixtureInput(document.getElementById('searchViewInput'), "document.getElementById('searchViewInput')").value === 'host:tycho',
      { timeout: 10000 });
    await multi.waitForFunction((id: string) => [...document.querySelectorAll<HTMLElement>('.search-result')]
    ...
    await multi.evaluate(() => fixtureApp.features.sidebarLists.load(undefined, { withPrevious: true }));
    const seenKeys = await multi.evaluate((id: string) => {
      const host = fixtureApp.ports.hostDiscovery.hosts().find((candidate) => !candidate.self);
      if (!host) throw new Error('Remote host is unavailable');
    ...
    if (!selfId) throw new Error('Self host identity is unavailable');
    ...
    await multi.evaluate(() => fixtureApp.features.sidebarLists.load(undefined, { withPrevious: true }));
    check(await multi.evaluate((host: string) => fixtureApp.features.sessionState.currentSession?.host === host, remoteId),
    ...
    await multi.evaluate((id: string) => fixtureApp.features.sessionView.select(id, { host: 'missing-host' }), collisionId);
    ...
    const collisionMutations: readonly [action: 'model' | 'thinking' | 'rename', field: 'model' | 'thinkingLevel' | 'name', value: string][] = [
    ...
    for (const [action, field, value] of collisionMutations) {
      ...
      let receiveRequest: ((route: Route) => void) | undefined;
      const received = new Promise<Route>(resolve => { receiveRequest = resolve; });
      await multi.route(endpoint, (route: Route) => {
        if (!receiveRequest) throw new Error('Collision route receiver is unavailable');
      ...
      const before = await multi.evaluate(({ id, host, field }) => {
        ...
        if (!session) throw new Error('Collision self session is unavailable');
        return session[field];
      }, { id: collisionId, host: selfId, field });
      await multi.evaluate(({ action, value }) => {
        if (action === 'rename') { fixtureApp.features.sessionControls.startRename(); fixtureInput(document.getElementById('sessionNameInput'), '#sessionNameInput').value = value; }
      ...
      const outcome = await multi.evaluate(({ id, self, remote, field }) => {
        ...
        if (!current || !selfSession || !remoteSession) throw new Error('Collision sessions are unavailable');
    ...
    check(closeResponse.request().method() === 'POST' &&
      await multi.evaluate((host: string) => fixtureApp.features.sessionState.currentSession?.host === host, selfId),
    ...
    const remoteGone = once(remoteProcess.child, 'exit');
    ...
    const offlineSection = await multi.evaluate(() => {
      const host = fixtureApp.ports.hostDiscovery.hosts().find((candidate) => !candidate.self);
      if (!host) throw new Error('Offline host is unavailable');
      const header = [...document.querySelectorAll<HTMLElement>('.host-section-header')]
        .find((candidate) => candidate.dataset.hostSection === 'host:' + host.hostId);
      const section = header?.closest<HTMLElement>('.host-section');
      return {
    ...
    failures++;
    console.error('  ✘ smoke test crashed:', errorMessage(error));
    ...
    try {
      const rpcSessions: typeof import('../lib/rpc-session') = require('../lib/rpc-session');
      for (const rpc of rpcSessions.getAllRPCSessions()) rpc.kill();
    } catch {}
