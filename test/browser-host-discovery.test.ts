import type { ApiRequest, HostTarget, RequestOptions } from '../src/browser/api-client.js';
import type { EffectiveHost } from '../src/browser/host-catalog.js';
import type { HostConnectionEvent } from '../src/browser/host-connections.js';
import type { HostDescriptor, HostDiscoveryOptions, HostDiscoverySource } from '../src/browser/host-discovery.js';
...
const descriptor = (hostId = 'peer', label = 'Peer'): HostDescriptor => ({ hostId, label, version: null, capabilities: { terminal: true } });
...
interface TestHost extends EffectiveHost {
  base: string;
  hostId: string | null;
  token?: string;
}
...
type DiscoveryEvent = [string, ...unknown[]];
function fixture(overrides: Partial<HostDiscoveryOptions> = {}) {
  let sources: TestSource[] = [{ base: '/hosts/peer', hostId: 'peer', token: 'fixture' }];
  let hosts: TestHost[] = [{ ...present(sources[0]), key: 'peer', source: 'user', hostId: 'peer' }];
  let now = 100000;
  const calls: PendingCall[] = [], events: DiscoveryEvent[] = [];
  const waiters = new Map<number, (call: PendingCall) => void>();
  const pending: ApiRequest = (host, url, options = {}) => new Promise<Response>((resolve, reject) => {
    const call = { host, url, options, resolve, reject };
    const index = calls.length;
    calls.push(call);
    waiters.get(index)?.(call);
  });
  const waitForCall = (index: number): Promise<PendingCall> => calls[index] ? Promise.resolve(present(calls[index]))
  ...
  return { discovery, calls, events, waitForCall, getSources: () => sources, getHosts: () => hosts,
    setSources: (value: TestSource[]) => { sources = value; }, setHosts: (value: TestHost[]) => { hosts = value; },
...
test('self identity accepts only the latest request and preserves older-server fallback', async () => {
  ...
  present(calls[1]).resolve(response(descriptor('self-new')));
  await current;
  present(calls[0]).resolve(response(descriptor('self-old')));
  await old;
  assert.deepEqual(events.map(event => [event[0], record(event[1]).hostId]), [['self', 'self-new']]);
  const missing = discovery.loadIdentity();
  present(calls[2]).resolve(response(null, 404));
  await missing;
...
for (const failure of [false, true]) test(`a superseded descriptor ${failure ? '401' : 'success'} cannot replace newer metadata or connection state`, async () => {
  ...
  present(calls[1]).resolve(response(descriptor('peer', 'New')));
  await current;
  present(calls[0]).resolve(response(descriptor('other', 'Old'), failure ? 401 : 200));
  await old;
  assert.equal(present(discovery.descriptor('peer')).label, 'New');
...
test('removed and re-added sources cannot inherit the old descriptor request', async () => {
  ...
  present(calls[0]).resolve(response(descriptor('wrong')));
  await old;
  assert.equal(events.length, 0);
  assert.equal(discovery.descriptor('wrong'), undefined);
  assert.equal(present(getSources()[0]).hostId, 'peer');
});
...
test('in-place credential changes retire requests and routing was captured before awaiting', async () => {
  ...
  const old = discovery.identify(true);
  present(getSources()[0]).token = present(getHosts()[0]).token = 'new-fixture';
  assert.equal(record(present(calls[0]).host).token, 'fixture');
  assert.equal(record(present(calls[0]).host).base, '/hosts/peer');
  assert.equal(present(calls[0]).options.timeoutMs, 8000);
  assert.equal(Object.isFrozen(present(calls[0]).host), true);
  present(calls[0]).reject(new Error('old failure'));
  await old;
  assert.equal(events.length, 0);
});
...
test('an entry hidden by a new effective route cannot publish a descriptor', async () => {
  ...
  const old = discovery.identify(true);
  setHosts([{ ...present(getHosts()[0]), source: 'fleet', base: '/hosts/new-route' }]);
  present(calls[0]).resolve(response(descriptor()));
  await old;
  assert.equal(events.length, 0);
});
...
test('ownership is rechecked after delayed response body decoding', async () => {
  ...
  Object.defineProperty(delayed, 'json', { value: () => new Promise<unknown>(resolve => {
    finishBody = resolve;
    markBodyStarted();
  }) });
  present(calls[0]).resolve(delayed);
  await bodyStarted;
  setSources([]);
...
test('current descriptor failures are isolated and remembered descriptors skip non-refresh reads', async () => {
  ...
  present(calls[0]).resolve(response(null, 401));
  await pending;
  assert.equal(events[0]?.[2], 'blocked');
  pending = discovery.identify();
  present(calls[1]).resolve(response(null, 503));
  await pending;
  assert.equal(record(events[1]?.[2]).type, 'failure');
  pending = discovery.identify();
  present(calls[2]).resolve(response({ hostId: 3 }));
  await pending;
  ...
  present(calls[3]).resolve(response(descriptor()));
  await pending;
...
test('fleet results reject old completions, isolate malformed rows and preserve refresh timing', async () => {
  ...
  assert.equal(present(calls[0]).options.timeoutMs, 10000);
  present(calls[1]).resolve(response({ hosts: [null, 7, [], { self: true, label: 'Self' }, { base: '/hosts/new' }] }));
  await current;
  present(calls[0]).resolve(response({ hosts: [{ base: '/hosts/old' }] }));
  await old;
  assert.deepEqual(events.map(event => event[0]), ['fleet', 'afterFleet']);
  const fleet = record(events[0]?.[1]), fleetHosts = Array.isArray(fleet.hosts) ? fleet.hosts : [];
  assert.equal(fleetHosts.length, 1);
  assert.equal(record(fleetHosts[0]).base, '/hosts/new');
...
test('a failed newer fleet refresh preserves the render owed by the current published list', async () => {
  ...
  const old = discovery.loadFleet();
  present(calls[0]).resolve(response({ hosts: [] }));
  assert.equal((await waitForCall(1)).url, '/api/host');
  const current = discovery.loadFleet();
  present(calls[2]).resolve(response(null, 404));
  await current;
  present(calls[1]).resolve(response(descriptor()));
  await old;
...
test('a persistence failure after learning identity still reports failure for the owned source', async () => {
  const { discovery, calls, events } = fixture({
    onIdentified: (host: EffectiveHost, source: HostDiscoverySource, data: HostDescriptor) => {
      source.hostId = host.hostId = data.hostId;
      throw new Error('storage unavailable');
    },
  });
  const pending = discovery.identify(true);
  present(calls[0]).resolve(response(descriptor('new-identity')));
  await pending;
  assert.equal(events.length, 1);
  assert.equal(events[0]?.[0], 'connection');
  const failure = record(events[0]?.[2]);
  assert.equal(failure.type, 'failure');
  assert.equal(record(failure.error).message, 'storage unavailable');
});
...
test('a superseded fleet waiter stays pending until the latest load settles', async () => {
  ...
  assert.equal(events.length, 0);
  present(calls[1]).resolve(response({ hosts: [{ base: '/hosts/current' }] }));
  await Promise.all([startup, replacement]);
  assert.equal(ready, true);
  const published = record(events[0]?.[1]), publishedHosts = Array.isArray(published.hosts) ? published.hosts : [];
  assert.equal(record(publishedHosts[0]).base, '/hosts/current');
});
...
test('fleet waiters follow repeated replacements without allowing old publication callbacks', async () => {
  ...
  present(calls[2]).resolve(response({ hosts: [{ base: '/hosts/superseded' }] }));
  present(calls[3]).resolve(response({ hosts: [{ base: '/hosts/current' }] }));
  await waitForCall(4); // Third publication starts the replacement descriptor.
  present(calls[1]).resolve(response(descriptor('peer', 'Old')));
  await new Promise<void>(resolve => setImmediate(resolve));
  assert.equal(ready, false);
  assert.equal(events.filter(event => event[0] === 'afterFleet').length, 0);
  present(calls[4]).resolve(response(descriptor('peer', 'Current')));
...
test('saving a catalog preserves unrelated source owners while removing changed or extra fields', async () => {
  const { discovery, calls, events, getSources, setSources } = fixture();
  const unchanged = present(getSources()[0]);
  const current = discovery.identify(true);
  ...
  assert.equal(record(getSources()[1]).capabilities, undefined);
  assert.notEqual(getSources()[2], whitespace);
  assert.equal(present(getSources()[2]).label, 'Space');
  present(calls[0]).resolve(response(descriptor()));
  await current;
