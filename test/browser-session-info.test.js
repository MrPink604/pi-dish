// Generated test/tool from test/browser-session-info.test.ts; edit that source and run npm run build:tests.
"use strict";
...
test('session stats retain unavailable prices and narrow context and runtime metadata', () => {
    ...
    assert.equal(stats.runtime.pid, null);
    assert.equal(stats.runtime.windowIndex, 0);
...
test('published artifact payloads reject error envelopes and malformed identities', () => {
    assert.equal(decodeSessionShare({ error: 'bad', path: '/share' }), null);
    assert.equal(decodeSessionShare({}), null);
    const share = decodeSessionShare({ url: 'https://fixture.invalid/shared' });
    assert.ok(share);
    assert.equal(share.url, 'https://fixture.invalid/shared');
    ...
    assert.equal(pages[0].missing, false);
});
