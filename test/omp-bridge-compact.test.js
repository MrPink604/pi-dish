// Generated test/tool from test/omp-bridge-compact.test.ts; edit that source and run npm run build:tests.
"use strict";
...
test('OMP lifecycle blocks pending background deliveries and fails closed on unknown job state', async () => {
    ...
    try {
        ...
        const backgroundWork = async (jobs) => {
            ...
            return (0, test_types_js_1.record)((0, test_types_js_1.record)(await session.send('get_state')).lifecycle).backgroundWork;
        };
        ...
        assert.equal(await backgroundWork({ ...empty, delivery: { queued: 0 } }), null, 'partial data is not evidence that background work finished');
        fs.writeFileSync(host.lifecycleFile, '{torn');
        assert.equal((0, test_types_js_1.record)((0, test_types_js_1.record)(await session.send('get_state')).lifecycle).backgroundWork, null, 'a failed runtime read does not permit maintenance');
    }
    finally {
        session.close();
        await host.close();
...
test('OMP bridge projects the native ask tool through extension UI', async () => {
    ...
    try {
        ...
        const option = (0, test_types_js_1.record)((0, test_types_js_1.present)((0, test_types_js_1.records)(question.options)[0]));
        assert.equal(option.description, 'Ship the current build.');
        const answer = {
            kind: 'submit',
            results: [{
                    id: 'deploy',
        ...
        if (typeof request.id !== 'string')
            throw new Error('ask request lacks an id');
        await session.respondExtensionUI(request.id, { value: answer });
        const received = await waitFor(() => {
            try {
                return JSON.parse(fs.readFileSync(host.askResultFile, 'utf8'));
            }
            catch {
                return null;
            }
...
test('OMP bridge retires an ask request when local presentation throws', async () => {
    ...
    try {
        ...
        assert.equal(requests.some(request => request.method === 'ask'), false, 'failed ask is absent from bridge replay');
    }
    finally {
        session.close();
        await host.close();
...
test('OMP bridge projects native todos, plan mode, and prewalk as extension UI', async () => {
    ...
    try {
        ...
        const widget = (0, test_types_js_1.present)(requests.find(request => request.method === 'setWidget'));
        ...
        assert.equal((0, test_types_js_1.present)(requests.find(request => request.statusKey === 'planmode')).statusText, 'Plan mode · parallel');
        assert.equal((0, test_types_js_1.present)(requests.find(request => request.statusKey === 'prewalk')).statusText, 'Prewalk → zai/glm-5.2 · high');
...
test('OMP bridge uses a nested subagent header id instead of its local agent filename', async () => {
    ...
    try {
        ...
        assert.equal(path.basename(typeof host.claim.sessionFile === 'string' ? host.claim.sessionFile : ''), 'Explore.jsonl');
    }
    finally {
        await host.close();
    }
...
test('OMP bridge compact operation uses public ctx.compact and forwards lifecycle events', async () => {
    ...
    const session = new BridgeSession(host.claim);
    try {
        const capabilities = (0, test_types_js_1.record)(host.claim.capabilities);
        assert.equal(capabilities.compact, true);
        assert.equal(capabilities.shareSnapshot, true);
        await session.connect();
        assert.deepEqual(await session.getShareSnapshot(), {
            systemPrompt: 'effective fake OMP system prompt',
            tools: [{ name: 'read', description: 'Read a file' }],
        });
        const commands = (0, test_types_js_1.record)(await session.getCommands());
        assert.equal((0, test_types_js_1.records)(commands.commands).some(command => (0, test_types_js_1.record)(command).name === 'compact'), true, 'compact control is advertised when the operation is available');
        const events = [];
        session.on('compaction_start', (data) => events.push(['start', data]));
        session.on('compaction_end', (data) => events.push(['end', data]));
        const result = (0, test_types_js_1.record)(await session.compact('retain decisions'));
        assert.match(String(result.info), /started/i);
        await new Promise((resolve, reject) => {
        ...
        assert.equal((0, test_types_js_1.record)(JSON.parse(fs.readFileSync(host.callFile, 'utf8'))).instructions, 'retain decisions');
...
test('OMP bridge releases the compaction gate when the host swallows a failure', async () => {
    ...
    try {
        ...
        const result = (0, test_types_js_1.record)(await session.compact('will be refused'));
        assert.match(String(result.info), /started/i);
        ...
        assert.match(String((0, test_types_js_1.record)((0, test_types_js_1.present)(events[0])).errorMessage), /finished without compacting/i);
        assert.equal(session.compacting, false);
        // The gate is genuinely down: a new compact attempt reaches the host
        // instead of being refused as already-in-progress.
        const again = (0, test_types_js_1.record)(await session.compact());
        assert.match(String(again.info), /started/i);
    }
    finally {
        session.close();
        await host.close();
...
test('OMP bridge degrades compact capability when the host context lacks ctx.compact', async () => {
    ...
    const session = new BridgeSession(host.claim);
    try {
        assert.equal((0, test_types_js_1.record)(host.claim.capabilities).compact, false);
        await session.connect();
        const commands = (0, test_types_js_1.record)(await session.getCommands());
        assert.equal((0, test_types_js_1.records)(commands.commands).some(command => (0, test_types_js_1.record)(command).name === 'compact'), false, 'compact control is hidden when the host lacks the operation');
