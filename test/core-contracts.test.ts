...
import { canonicalSessionId, encodeSessionKey, resolveSessionRoute, validSessionId } from '../lib/session-key';
import { getHostId, getHostLabel } from '../lib/host-identity';
import { readStore, writeStore } from '../lib/dish-store';
import { PendingRequests } from '../lib/pending-requests';
import { createLineSplitter } from '../lib/line-splitter';
import { processIdentity, processIdentityAlive, inspectProcessAncestry } from '../lib/process-identity';

test('canonical route ids retain legacy Pi bytes and distinguish alternate harnesses', () => {
  const native = 'same-native:id';
  assert.ok(validSessionId(native));
  ...
  assert.throws(() => canonicalSessionId(`${omp}=`));
...
test('request timeout and socket failure reject and release their pending entries', async () => {
  ...
  const failedWrite = assert.rejects(pending.track('write'), (error: unknown) => error === writeFailure);
  pending.fail('write', writeFailure);
  await failedWrite;
  const disconnected = new Error('disconnected');
  const first = assert.rejects(pending.track('a'), (error: unknown) => error === disconnected);
  const second = assert.rejects(pending.track('b'), (error: unknown) => error === disconnected);
