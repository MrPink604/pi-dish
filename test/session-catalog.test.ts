...
import type { SessionInfo } from '../lib/session-metadata-contracts.js';
import type { SessionSource, DiscoveryCandidate } from '../lib/session-source-contracts.js';
import type { CatalogSession } from '../lib/session-catalog-contracts.js';
import type { HarnessId, NativeSessionId } from '../lib/contracts.js';
...
const { sessionCapabilities }: typeof import('../lib/session-capabilities') = require('../lib/session-capabilities')
const { encodeSessionKey }: typeof import('../lib/session-key') = require('../lib/session-key')
const { sourceForIdentity }: typeof import('../lib/session-source') = require('../lib/session-source')
const { sessionForClient }: typeof import('../lib/session-api') = require('../lib/session-api')
const {
...
}: typeof import('../lib/session-catalog') = require('../lib/session-catalog')

const options = {
  harnesses: new Map([registry.pi, registry.omp, registry.prime].map(harness => [harness.id, harness])),
  contextWindowForModel: () => 1000,
  canonicalPath: (file: string | null) => file ? path.resolve(file) : null,
  directoryExists: () => false,
};
function source(harnessId: HarnessId = 'pi', nativeSessionId: NativeSessionId = checkedNativeId('native'), extras: Partial<SessionSource> = {}): SessionSource {
...
function context(src: SessionSource, metadata: SessionInfo | null = info(), overrides: Record<string, unknown> = {}) {
...
function history(src: SessionSource, metadata: SessionInfo = info(), overrides: Record<string, unknown> = {}) {
  const candidate: DiscoveryCandidate = { ...src, dirName: '--workspace--', depth: 0, identitySource: 'basename' };
...
test('registered catalog projection retains live usage, metadata fallback and lifecycle advice', () => {
  const src = source('omp', checkedNativeId('child'), { parentSession: '/catalog-fixture/parent.jsonl' });
  ...
  assert.equal(row.id, encodeSessionKey('omp', checkedNativeId('child')));
...
test('history and live children share projection while preserving naming, cwd fallback and resume gates', () => {
  const src = source('omp', checkedNativeId('header-child'), { file: '/catalog-fixture/parent/Explore.jsonl', parentSession: '/catalog-fixture/parent.jsonl' });
  ...
  assert.equal(present(row.capabilities).resume, false);
  ...
  const existing = { ...options, directoryExists: (cwd: string) => cwd === '/workspace' };
...
test('catalog deduplicates live/history, preserves active-only child ordering and discovery flags', () => {
  ...
  const child = history(source('omp', checkedNativeId('child')), info({ lastActivity: new Date('2026-02-01') }), { liveChild: true });
  const older = history(source('prime', checkedNativeId('older')), info({ lastActivity: new Date('2025-01-01') }));
  ...
  assert.deepEqual(full.list.map((row: CatalogSession) => row.nativeSessionId), ['native', 'child', 'older']);
  assert.equal(present(full.active[0]).name, 'Bridge');
  assert.equal(present(present(full.previous[0]).capabilities).resume, false);
  ...
  assert.deepEqual(activeOnly.children.map((row: CatalogSession) => row.nativeSessionId), ['child']);
});

test('native canonical lineage precedes launch hints, self parents are suppressed and routines remain annotations', () => {
  const parent = source('pi', checkedNativeId('parent'));
  const child = source('omp', checkedNativeId('child'));
  const other = source('prime', checkedNativeId('other'));
  const detached = source('pi', checkedNativeId('detached'));
  const self = source('pi', checkedNativeId('self'));
  ...
  const row = present(result.byId.get(child.routeId));
  ...
  assert.equal(present(result.byId.get(detached.routeId)).parentSource, 'pi-dish-launch');
  assert.equal(present(result.byId.get(detached.routeId)).familyParentId, null);
  assert.equal(present(result.byId.get(self.routeId)).parentId, null);
  assert.equal(present(result.byId.get(parent.routeId)).routine, undefined);
  assert.equal(present(result.byId.get(other.routeId)).parentId, null);
...
test('store annotation ingress rejects malformed consumed values and folds encoded Pi aliases', () => {
  const parents = decodeLaunchParents({ [encodeSessionKey('pi', checkedNativeId('child'))]: { sourceSessionId: encodeSessionKey('pi', checkedNativeId('parent')) },
  ...
  const routines = decodeRoutineAnnotations(new Map([
    [encodeSessionKey('pi', checkedNativeId('child')), { routineName: '', routineId: '', id: '' }],
