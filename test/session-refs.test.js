// Generated test/tool from test/session-refs.test.ts; edit that source and run npm run build:tests.
"use strict";
...
const { expandSessionRefs } = require('../lib/session-refs.js');
const { splitSessionRefContext } = require('../lib/helper-refs.js');
...
test('an unfleeted, unhinted host part is dropped', () => {
    ...
    assert.equal(expandSessionRefs(`#${PEER_HOST}:remote-session-1`, null, d), `#${PEER_HOST}:remote-session-1`);
});
...
test('every resolvable ref in one prompt lands in a single block', () => {
    const d = deps();
    const out = expandSessionRefs('diff #local-session-aaaa1111 against #tycho/8f3ab2c1, ignore #nosuchref', null, d);
    ...
    assert.equal(splitSessionRefContext(out).text, 'diff #local-session-aaaa1111 against #tycho/8f3ab2c1, ignore #nosuchref');
});
