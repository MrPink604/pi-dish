...
import { encodeSessionKey, decodeSessionKey, resolveSessionRoute, validSessionId, VERSION } from '../lib/session-key';
import { registry, getHarness, resolveLaunchSpec } from '../lib/harnesses';
import { discoverSessionCandidates, discoverHarnessSessions } from '../lib/session-discovery';
import { getSessionInfo } from '../lib/session-files';
...
test('canonical identities round-trip strictly and legacy routes belong to Pi', () => {
  ...
  for (const bad of ['', VERSION, `${VERSION}!!!!`, encodeSessionKey('pi', shortId) + 'x']) assert.throws(() => decodeSessionKey(bad));
  assert.throws(() => resolveSessionRoute(`${VERSION}!!!!`),
    'a malformed encoded route never falls back to Pi');
  for (const id of ['', '../x', 'x'.repeat(201)]) assert.throws(() => { Reflect.apply(encodeSessionKey, undefined, ['pi', id]); });
  assert.throws(() => { Reflect.apply(encodeSessionKey, undefined, ['other', 'x']); });
});

test('harness lookup accepts only registered string names', () => {
  for (const id of ['pi', 'omp', 'prime']) {
    assert.ok(id === 'pi' || id === 'omp' || id === 'prime');
    assert.equal(getHarness(id), registry[id]);
  }
  for (const id of ['constructor', '__proto__', 'toString', ['pi'], null, 42, { toString: () => 'pi' }]) {
    assert.equal(getHarness(id), null);
    assert.throws(() => { Reflect.apply(encodeSessionKey, undefined, [id, 'native']); });
  }
});
...
test('encoded session routes reject inherited and coerced harness names', () => {
  for (const id of ['constructor', '__proto__', 'toString', ['pi'], null, 42]) {
    ...
    assert.throws(() => resolveSessionRoute(key));
  }
});
