import { present, record, records, type TestRecord } from './test-types.js';
/**
...
const { getAllRPCSessions }: typeof import('../lib/rpc-session') = require('../lib/rpc-session')
...
const get = (requestPath: string) => request('GET', requestPath);
const post = (requestPath: string, body?: unknown) => request('POST', requestPath, body);
const put = (requestPath: string, body?: unknown) => request('PUT', requestPath, body);
const del = (requestPath: string) => request('DELETE', requestPath);
...
async function waitFor<T>(predicate: () => T | Promise<T>, { timeout = 20000, label = 'condition' } = {}): Promise<NonNullable<T>> {
...
const invocation = async (id: unknown): Promise<TestRecord> =>
  record((await get(`/api/routine-invocations/${text(id)}`)).body.invocation);
const waitForStatus = (id: unknown, ...statuses: string[]) => waitFor(async () => {
...
test('the host advertises the routines capability', async () => {
  ...
  assert.equal(record(body.capabilities).routines, true);
});
...
test('CRUD: create, list, read, update, delete', async () => {
  ...
  const routine = record(created.body.routine);
  ...
  const row = present(records(list.body.routines).find(candidate => candidate.id === routine.id));
  ...
  assert.equal(record((await get('/api/routines/crud-routine')).body.routine).id, routine.id);
  assert.equal(record((await get(`/api/routines/${routine.id}`)).body.routine).id, routine.id);
  ...
  assert.equal(record(renamed.body.routine).promptVersion, 1);
  const bumped = await put('/api/routines/crud-routine', { prompt: 'Pong.' });
  assert.equal(bumped.status, 200);
  assert.equal(record(bumped.body.routine).promptVersion, 2);
  assert.equal(records(record(bumped.body.routine).versions).length, 2);
...
test('pilot errors precede definition errors without laundering untrusted selections', async () => {
  ...
  assert.equal(records((await get('/api/routines')).body.routines).some(row => row.name === invalid.name), false);
});
...
test('routine pilot updates distinguish absent selections from explicit null', async () => {
  ...
  const resource = `/api/routines/${record(created.body.routine).id}`;
  const unchanged = await put(resource, { description: 'same pilot' });
  assert.equal(unchanged.status, 200);
  assert.equal(record(unchanged.body.routine).model, 'zai/glm-5.2');
  assert.equal(record(unchanged.body.routine).thinking, 'high');
  const cleared = await put(resource, { model: null, thinking: null });
  assert.equal(cleared.status, 200);
  assert.equal(Object.hasOwn(record(cleared.body.routine), 'model'), false);
  assert.equal(Object.hasOwn(record(cleared.body.routine), 'thinking'), false);
  ...
  assert.equal(record((await get(resource)).body.routine).cwd, tmpHome);
  await del(resource);
});

test('invoke spawns a session, delivers the prompt with the input block, completes and closes', async () => {
  const routine = record((await post('/api/routines', definition({
  ...
  assert.equal(record(started.body.invocation).status, 'starting');
  assert.equal(record(started.body.invocation).trigger, 'invoke');
  assert.equal(record(started.body.invocation).delivery, 'prompt');
  assert.equal(record(started.body.invocation).source, 'nightly-cron');
  assert.equal(record(started.body.invocation).version, 1);
  const id = record(started.body.invocation).id;

  const running = await waitForStatus(id, 'running', 'completed');
  assert.ok(running.sessionId, 'the invocation names its session');

  const delivered = record(await waitFor(() => prompts().length > before && prompts().at(-1),
  ...
  assert.ok(!getAllRPCSessions().some((session: unknown) => {
    const row = record(session);
    return row.id === closed.sessionId && row.alive;
  }), 'the spawned pi child is gone');

  // Stats reflect the run.
  const row = present(records((await get('/api/routines')).body.routines).find(candidate => candidate.id === routine.id));
  const stats = record(row.stats);
  assert.equal(stats.invocations, 1);
  assert.equal(stats.running, 0);
  assert.equal(record(stats.lastInvocation).id, id);
...
test('a routine session is stamped and findable with routine: while it lives', async () => {
  ...
  const invoked = record(body.invocation);
  ...
  const listed = await waitFor(async () => {
    ...
    return records(res.body.active).find((s) => s.id === sessionId) || null;
  ...
  assert.ok(records(found.body.active).some((s) => s.id === sessionId), 'routine: matches the session');
  const excluded = await get('/api/sessions?active=1&q=-routine%3Astamped-routine');
  assert.ok(!records(excluded.body.active).some((s) => s.id === sessionId), '-routine: excludes it');
  const other = await get('/api/sessions?active=1&q=routine%3Asomething-else');
  assert.ok(!records(other.body.active).some((s) => s.id === sessionId));

  // Advanced search speaks the same grammar over the same stamp.
  const search = await get('/api/search?q=routine%3Astamped-routine');
  assert.ok(records(search.body.results).some((s) => s.id === sessionId), 'advanced search matches too');
  ...
  await waitFor(async () => {
    ...
    return records(res.body.results).some((s) => s.id === sessionId) ? res : null;
  }, { label: 'the closed routine run to become searchable history' });
  const hidden = await get('/api/search?hideAutomation=1');
  assert.ok(!records(hidden.body.results).some((s) => s.id === sessionId),
  ...
  assert.ok(records(asked.body.results).some((s) => s.id === sessionId),
  ...
  assert.ok(records(scoped.body.results).some((s) => s.id === sessionId),
...
test('a busy routine skips with 409, then steers once onBusy says so', async () => {
  ...
  const running = await waitForStatus(record(first.body.invocation).id, 'running');
  ...
  assert.equal(record(refused.body.invocation).id, record(first.body.invocation).id);
  assert.equal(records((await get(`/api/routines/${routine.id}/invocations`)).body.invocations).length, 1,
    'the refusal is not recorded');

  const steering = await put(`/api/routines/${routine.id}`, { onBusy: 'steer' });
  assert.equal(record(steering.body.routine).onBusy, 'steer');
  ...
  assert.equal(record(steered.body.invocation).delivery, 'steer');
  const delivered = record(await waitFor(() => steers().length > before && steers().at(-1),
  ...
  assert.equal((await invocation(record(steered.body.invocation).id)).sessionId, running.sessionId,
    'the steer went into the running session, not a new one');

  // Both invocations settle at the turn that follows.
  await waitForStatus(record(first.body.invocation).id, 'completed', 'interrupted');
  await waitForStatus(record(steered.body.invocation).id, 'completed', 'interrupted');
...
test('continue mode prompts the same session on the second run', async () => {
  ...
  const firstSession = record(first.body.invocation).sessionId;
  assert.ok(firstSession);
  await waitForStatus(record(first.body.invocation).id, 'completed');

  const before = prompts().length;
  const second = await post(`/api/routines/${routine.id}/invoke?wait=1`, {});
  assert.equal(record(second.body.invocation).sessionId, firstSession, 'the same session is reused');
  await waitFor(() => prompts().length > before, { label: 'the second prompt' });
  const done = await waitForStatus(record(second.body.invocation).id, 'completed');
  ...
  assert.equal(records(page.body.invocations).length, 1);
  assert.equal(records(page.body.invocations)[0].id, record(second.body.invocation).id, 'newest first');
  ...
  assert.equal(records(older.body.invocations)[0].id, record(first.body.invocation).id);
...
test('minIntervalSec answers 429 with a retry hint and records nothing', async () => {
  ...
  assert.equal(record(limited.body.lastInvocation).id, record(first.body.invocation).id);
  ...
  await waitForStatus(record(first.body.invocation).id, 'completed');
  await post(`/api/sessions/${record(first.body.invocation).sessionId}/close`, {});
  await del(`/api/routines/${routine.id}`);
});

test('oversized input is a 413 and a bad source a 400, neither recorded', async () => {
  const routine = record((await post('/api/routines', definition({ name: 'guard-routine' }))).body.routine)
  ...
  assert.equal(records((await get(`/api/routines/${routine.id}/invocations`)).body.invocations).length, 0);
...
test('deleting a routine keeps its invocations readable', async () => {
  ...
  const id = record(run.body.invocation).id;
  await waitForStatus(id, 'completed');
  await post(`/api/sessions/${record(run.body.invocation).sessionId}/close`, {});

  const removed = await del(`/api/routines/${routine.id}`);
  assert.deepEqual(removed.body, { success: true, invocations: 1 });
  const kept = await get(`/api/routine-invocations/${id}`);
  assert.equal(kept.status, 200);
  assert.equal(record(kept.body.invocation).routineName, 'ledger-routine', 'the denormalized name survives');
...
test('saved raw launch selections retain native argv, cwd failure order and falsy fallback', async () => {
  const routine = record((await post('/api/routines', definition({ name: 'raw-launch-routine', prompt: 'slow: verify raw launch selections' }))).body.routine)
  const file = path.join(tmpHome, '.pi', 'dish', 'routines.json');
  const saved = JSON.parse(fs.readFileSync(file, 'utf8'));
  Object.assign(record(record(record(saved).routines)[text(routine.id)]), { model: 42, thinking: ['one', 'two'] });
  ...
  const child = present(started.find(row => path.basename(text(row.sessionFile), '.jsonl') === record(first.body.invocation).sessionId));
  assert.deepEqual(child.args, ['--mode', 'rpc', '--model', '42', '--thinking', 'one,two']);
  await waitFor(async () => (await invocation(record(first.body.invocation).id)).closed);

  record(record(saved).routines)[text(routine.id)] = { ...record(record(record(saved).routines)[text(routine.id)]), cwd: 42 };
  fs.writeFileSync(file, JSON.stringify(saved));
  const before = fs.readFileSync(START_LOG, 'utf8');
  const failed = await post(`/api/routines/${routine.id}/invoke?wait=1`, {});
  assert.equal(failed.status, 200);
  assert.equal(record(failed.body.invocation).status, 'errored');
  assert.equal(record(failed.body.invocation).sessionId, null);
  assert.equal(fs.readFileSync(START_LOG, 'utf8'), before, 'cwd failure must precede native launch');

  record(record(saved).routines)[text(routine.id)] = { ...record(record(record(saved).routines)[text(routine.id)]), cwd: 0 };
  fs.writeFileSync(file, JSON.stringify(saved));
  const fallback = await post(`/api/routines/${routine.id}/invoke?wait=1`, {});
  await waitForStatus(record(fallback.body.invocation).id, 'completed');
  const lastLine = present(fs.readFileSync(START_LOG, 'utf8').trim().split('\n').at(-1));
  const last = record(JSON.parse(lastLine));
  const header = record(JSON.parse(present(fs.readFileSync(text(last.sessionFile), 'utf8').split('\n')[0])));
  assert.equal(header.cwd, tmpHome);
  await waitFor(async () => (await invocation(record(fallback.body.invocation).id)).closed);
  await del(`/api/routines/${routine.id}`);
});

test('raw saved continuation identity falls back to a fresh session without rewriting history', async () => {
  const routine = record((await post('/api/routines', definition({ name: 'raw-ledger-routine', mode: 'continue', prompt: 'slow: verify raw continuation identity' }))).body.routine)
  ...
  const completed = await waitForStatus(record(run.body.invocation).id, 'completed');
