// Generated test/tool from test/browser-routines.test.ts; edit that source and run npm run build:tests.
"use strict";
...
test('routine decoding retains literal prompt versions and stamps immutable host endpoints', () => {
    ...
    assert.equal(routine.host, 'peer');
    assert.equal(routine.onBusy, 'skip');
    assert.equal(routine.prompt, 'Text <xml>');
    assert.equal(routine.versions.length, 1);
    assert.ok(routine.stats.lastInvocation);
    assert.equal(routine.stats.lastInvocation.id, 'run');
    host.base = '/replacement';
    assert.equal(routine.endpoint.base, '/hosts/peer');
    assert.equal(Object.isFrozen(routine.endpoint), true);
...
test('routine invocation decoding rejects malformed rows and cursor types', () => {
    ...
    assert.equal(data.invocations.length, 1);
    assert.equal(data.invocations[0].durationMs, null);
