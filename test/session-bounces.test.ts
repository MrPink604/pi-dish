...
import { createSessionBounces, createSessionBounceRuntime, lifecycleBlockers } from '../lib/session-bounces';
import type { SessionBounceInspection, SessionBounceOperationPorts, SessionBounceOwnershipPorts, SessionBounceQueueOptions } from '../lib/session-bounces';
import { inspectSubsessionExits } from '../lib/session-discovery';
import { BridgeSession } from '../lib/bridge-session';
import type { BridgeRegistryEntry, ProcessIdentity } from '../lib/contracts';
import type { PaneBounceAuthority } from '../lib/session-ownership';
import type { SessionOperationOutcome } from '../lib/session-operations';
import { validSessionId } from '../lib/session-key';
import tmux = require('../lib/tmux');
...
function fixture(overrides: Partial<SessionBounceQueueOptions<{ generation: number }>> = {}) {
  const runtime = { generation: 1, blockers: ['Working'], eligible: true };
  const actions: number[] = [];
  const scheduler = createSessionBounces<{ generation: number }>({
...
test('cancel wins while safety inspection is awaiting, but never interrupts executing work', async () => {
  const inspection = deferred<SessionBounceInspection>();
...
test('unknown activity, dialogs mid-turn, queued input and background jobs all block idle safety', () => {
  ...
  const live = new BridgeSession({ sessionId, socketPath: '/unused-lifecycle' });
  live.alive = true;
...
function productionFixture(t: TestContext) {
  const sessionId = 'parent';
  assert.ok(validSessionId(sessionId));
  const claim = { sessionId, socketPath: '/unused-original', sessionFile: '/parent.jsonl',
    harnessId: 'pi', pid: 42, startTime: '100', bridgeInstanceId: 'original', capabilities: { guardedReload: true } } satisfies BridgeRegistryEntry;
  const paneProcess: ProcessIdentity = { pid: 41, startTime: '90' };
  const live = new BridgeSession(claim);
  live.alive = true;
  const state = { turnInProgress: false, compacting: false,
    lifecycle: { idle: true, pendingMessages: false, pendingDialogs: 0, backgroundWork: false } };
  const actions: string[] = [];
  const authority: PaneBounceAuthority = { sessionId: 'parent', harnessId: 'pi', sessionFile: claim.sessionFile, rpc: null, reg: claim,
    spawn: { socket: '/unused-tmux', paneId: '%1', paneProcess } };
  t.mock.method(tmux, 'paneProcessIdentity', async () => paneProcess);
  t.mock.method(live, 'send', async (command: Parameters<InstanceType<typeof BridgeSession>['send']>[0]) => {
    if (command === 'get_state') return state;
    assert.equal(command, 'guarded_reload');
    actions.push('reload');
    return {};
  });
  const ownership: SessionBounceOwnershipPorts = {
    captureBounceAuthority: () => authority,
    bounceIdentityFailure: () => null,
    getLiveSession: async () => live,
    refreshRegisteredSession: () => claim,
  };
  const operations: SessionBounceOperationPorts = {
...
test('production execution rechecks activity after the last await and releases execution exclusion on waiting', async t => {
  ...
  const restart = t.mock.method(operations, 'restartSession', async (
    _id: Parameters<SessionBounceOperationPorts['restartSession']>[0],
    options: Parameters<SessionBounceOperationPorts['restartSession']>[1],
...
test('production restart distinguishes pre-action refusal from a stopped replacement with the same status', async t => {
  ...
  t.mock.method(operations, 'restartSession', async (): Promise<SessionOperationOutcome> => ({ kind: 'no-action', status: 409, error: 'Ownership changed' }));
  scheduler.enqueue('restart', ['parent']);
  await scheduler.tick();
  assert.equal((await scheduler.list())[0].targets[0].status, 'skipped');
  t.mock.method(operations, 'restartSession', async (): Promise<SessionOperationOutcome> => ({
...
function treeFixture(t: TestContext) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'dish-bounce-'));
  t.after(() => fs.rmSync(dir, { recursive: true, force: true }));
  const write = (relative: string, id: string, exited = false) => {
