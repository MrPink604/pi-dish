...
import type { SessionRefDependencies, SessionRefSummary } from '../lib/session-refs.js';

const { expandSessionRefs }: typeof import('../lib/session-refs.js') = require('../lib/session-refs.js');
const { splitSessionRefContext }: typeof import('../lib/helper-refs.js') = require('../lib/helper-refs.js');

const SELF_HOST = '0f9c1a2b-3c4d-5e6f-7a8b-9c0d1e2f3a4b';
const PEER_HOST = '11112222-3333-4444-5555-666677778888';

const LOCAL: Record<string, SessionRefSummary & { id: string }> = {
...
function deps(overrides: Partial<SessionRefDependencies> = {}) {
  const calls = { resolveLocal: 0, fleetNames: 0 };
  const base: SessionRefDependencies & { calls: typeof calls } = {
    calls,
    selfHostId: SELF_HOST,
    resolveLocal: (id: string, exactOnly: boolean) => {
      calls.resolveLocal++;
...
const refsIn = (text: string) => splitSessionRefContext(text).refs;
