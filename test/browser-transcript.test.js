// Generated test/tool from test/browser-transcript.test.ts; edit that source and run npm run build:tests.
"use strict";
...
test('transcript pages require message arrays and narrow cursors without accepting wire identity', () => {
    ...
    assert.equal(data.lastIndex, 1);
    assert.equal(data.totalMessages, null);
    assert.equal(data.hasMore, false);
    assert.equal(Object.hasOwn(data.session, 'id'), false);
    assert.equal(Object.hasOwn(data.session, 'host'), false);
    assert.equal(data.session.name, 'kept');
});
