...
import type { HostConnectionRecord } from '../src/browser/host-connections.js';
import type { HelperSession, Timestamp } from '../src/core/helper-types.js';
import type { UsageDay, UsageLimit, UsageLimitEntry, UsageLimitReport } from '../src/browser/shared-helper-types.js';
...
const H: typeof import('../src/browser/shared-helpers.js') = require('../public/helpers.js');
...
assertBrowserApiContext(browserContext);
const B = browserContext.PiDishBrowser;
const property = (value: unknown, key: string): unknown => record(value)[key];
const arrayProperty = (value: unknown, key: string): unknown[] => {
  const result = property(value, key);
  return Array.isArray(result) ? result : [];
};
let workspaceSessionId = 0;
const workspaceSession = (cwd: string | null, lastActivity: Timestamp | null) =>
  ({ id: `workspace-${++workspaceSessionId}`, cwd, lastActivity });
function renderMarkdown(marked: Marked, source: string): string {
  const result = marked.parse(source);
  if (typeof result !== 'string') throw new Error('Expected synchronous marked output');
  return result;
}
...
test('groupByWorkspace groups by cwd and sorts by recency', () => {
  ...
  assert.deepEqual(groups.map((group: unknown) => Array.isArray(group) ? group[0] : undefined), ['~', '/b', '/a']);
  // within /a, newest first
  const groupA = Array.isArray(groups[2]) && Array.isArray(groups[2][1]) ? groups[2][1] : [];
  assert.deepEqual(groupA.map((session: unknown) => property(session, 'lastActivity')),
...
test('groupByWorkspace sinks collapsed groups below expanded ones', () => {
  ...
  assert.deepEqual(groups.map((group: unknown) => Array.isArray(group) ? group[0] : undefined), ['/c', '/a', '/b']);
});
...
test('buildWorkspaceTree shows a shared prefix once with distinguishing tails as children', () => {
  ...
  assert.deepEqual(tree[0].children.map((child: unknown) => [
...
test('buildWorkspaceTree flattens multi-segment chains below a divergence point', () => {
  ...
  assert.deepEqual(tree[0].children.map((c: unknown) => property(c, 'label')), ['deep/nested/proj', 'other']);
});
...
test('buildWorkspaceTree keeps unrelated workspaces as flat single nodes', () => {
  ...
  assert.deepEqual(tree.map((node: unknown) => [property(node, 'label'), property(node, 'path'), arrayProperty(node, 'children').length]), [
    ['~', '~', 0],
    ['~/app', '/home/u/app', 0],
    ['/etc/x', '/etc/x', 0],
  ]);
  assert.equal(present(present(tree[1]).sessions).length, 1);
});
...
test('buildWorkspaceTree keeps sessions living at a prefix that also has children', () => {
  ...
  assert.equal(tree.length, 1);
  const root = present(tree[0]);
  assert.equal(root.path, '/w/app');
  assert.equal(present(root.sessions).length, 1);
...
test('buildWorkspaceTree sinks collapsed nodes below expanded siblings per level', () => {
  ...
  assert.deepEqual(tree[0].children.map((c: unknown) => property(c, 'label')), ['older', 'newest']);
});
...
test('buildWorkspaceTree hoists a bare home root — ~/x groups stay top-level', () => {
  ...
  assert.deepEqual(tree.map((node: unknown) => property(node, 'label')), ['~/workspace', '~/src/dotfiles']);
  assert.deepEqual(tree[0].children.map((c: unknown) => property(c, 'label')), ['a', 'b']);
});
...
test('buildSessionFamilies keeps parents first and sorts whole blocks by newest descendant', () => {
  ...
  assert.deepEqual(roots.map((root: unknown) => property(property(root, 'session'), 'id')), ['other-cwd', 'parent', 'standalone']);
  assert.equal(roots[1].activity, Date.parse('2026-01-05T00:00:00Z'));
  assert.equal(roots[1].size, 3);
  assert.deepEqual(roots[1].children.map((child: unknown) => property(property(child, 'session'), 'id')), ['child']);
  assert.deepEqual(H.flattenSessionFamilies([roots[1]]).map((s: unknown) => property(s, 'id')), ['parent', 'child', 'grandchild']);
});
...
test('session family cycles degrade to standalone roots', () => {
  ...
  assert.deepEqual(roots.map((root: unknown) => property(property(root, 'session'), 'id')), ['b', 'a']);
  assert.ok(roots.every((root: unknown) => arrayProperty(root, 'children').length === 0));
});
...
test('partitionPinnedFamilies pins and orders the whole family from any member id', () => {
  ...
  assert.deepEqual(pinned.map((root: unknown) => property(property(root, 'session'), 'id')), ['p']);
  assert.deepEqual(rest.map((root: unknown) => property(property(root, 'session'), 'id')), ['other']);

  const activeFragment = H.buildSessionFamilies([
    { id: 'c', parentId: 'p', familyParentId: 'p', cwd: '/w', lastActivity: 2 },
  ]);
  assert.deepEqual(H.partitionPinnedFamilies(activeFragment, ['p'])[0].map((root: unknown) => property(property(root, 'session'), 'id')), ['c'], 'missing inactive parent aliases its visible child fragment');
...
test('partitionPinnedFamilies matches pins by host + session key', () => {
  ...
  assert.deepEqual(H.partitionPinnedFamilies(families, ['hostA c'])[0]
    .map((root: unknown) => `${property(property(root, 'session'), 'host')} ${property(property(root, 'session'), 'cwd')}`), ['hostA /w']);
  ...
  assert.deepEqual(H.partitionPinnedFamilies(fragment, ['hostA p'])[0].map((root: unknown) => property(property(root, 'session'), 'id')), ['c'], 'missing parent aliases within its own host');
...
test('sortRelations ranks singular lineage links ahead of child lists, stable within a kind', () => {
  const rel = (kind: string, id: string) => ({ kind, session: { id } });
  const input = [
  ...
  assert.deepEqual(sorted.map((r: unknown) => property(property(r, 'session'), 'id')), ['p', 'sf', 'c1', 'c2', 'c3', 'sh1', 'm']);
...
test('groupRelations groups by kind in rank order and skips nothing', () => {
  ...
  assert.deepEqual(present(groups[1]).relations.map(relation => relation.session.id), ['c1', 'c2']);
  assert.deepEqual(H.groupRelations(undefined), []);
  const kindless: unknown = [{ session: { id: 'x' } }];
  const generic = Reflect.apply(H.groupRelations, H, [kindless]);
  assert.equal(present(generic[0]).kind, 'related', 'kind-less relations fall into a generic group');
});

test('partitionPinned splits in pinned order and skips unknown ids', () => {
  const list = [{ id: 'a' }, { id: 'b' }, { id: 'c' }];
  const [pinned, rest] = H.partitionPinned(list, ['c', 'gone', 'a']);
  assert.deepEqual(pinned.map((s: unknown) => property(s, 'id')), ['c', 'a']);
  assert.deepEqual(rest.map((s: unknown) => property(s, 'id')), ['b']);
...
test('applyLocalFilter requires every token across name/cwd/model/id', () => {
  ...
  assert.deepEqual(H.applyLocalFilter(list, 'webapp gpt').map((s: unknown) => property(s, 'id')), ['s1']);
...
test('evaluateSessionQuery: fields scope, negation is metadata-only, content widens plain terms', () => {
  ...
  const q = (str: string, content?: string) => H.evaluateSessionQuery(H.parseSessionQuery(str, new Date('2026-07-21T12:00:00').getTime()), s, content);
...
test('evaluateSessionQuery: is:active tests liveness, not substrings', () => {
  ...
  const q = (str: string, s: HelperSession) => H.evaluateSessionQuery(H.parseSessionQuery(str), s);
...
test('host: composes with is:active, since: and plain terms', () => {
  ...
  const q = (str: string) => H.evaluateSessionQuery(H.parseSessionQuery(str, now), s);
...
test('is:automation tests routine provenance; queryAsksForAutomation is positive-only', () => {
  ...
  const asks = (str: string) => H.queryAsksForAutomation(H.parseSessionQuery(str));
...
test('stripQueryField removes a field\'s tokens and leaves the rest tokenized as parsed', () => {
  const strip = (q: string | null | undefined) => H.stripQueryField(q, 'host');
  ...
  assert.deepEqual(rest.terms, full.terms.filter((term: unknown) => property(term, 'field') !== 'host'));
});
...
test('applyHostTerms filters on host terms alone, ignoring the rest of the query', () => {
  ...
  assert.deepEqual(H.applyHostTerms(list, 'host:beelink').map((s: unknown) => property(s, 'id')), ['s1']);
  assert.deepEqual(H.applyHostTerms(list, '-host:beelink').map((s: unknown) => property(s, 'id')), ['s2', 's3']);
  assert.deepEqual(H.applyHostTerms(list, 'host:uuid-3').map((s: unknown) => property(s, 'id')), ['s3']);
  // Non-host terms are the server's job on this path — they must not narrow.
  assert.deepEqual(H.applyHostTerms(list, 'host:beelink zzz name:nope').map((s: unknown) => property(s, 'id')), ['s1']);
...
test('applyLocalFilter understands the query grammar', () => {
  ...
  assert.deepEqual(H.applyLocalFilter(list, '-name:subagent').map((s: unknown) => property(s, 'id')), ['s2']);
  assert.deepEqual(H.applyLocalFilter(list, 'cwd:webapp').map((s: unknown) => property(s, 'id')), ['s1']);
  assert.equal(H.applyLocalFilter(list, ''), list);
});

test('scoreSessionMatch: distinct-keyword coverage beats repeating one keyword', () => {
  const s = (over: Record<string, unknown> = {}) => ({ name: 'n', cwd: '/c', model: 'm', id: 'i', ...over });
...
test('scoreSessionMatch: only positive plain terms score', () => {
  ...
  const zero = (q: string) => assert.equal(H.scoreSessionMatch(H.parseSessionQuery(q), s, 'alpha alpha'), 0, q);
  zero('name:alpha');
  zero('-bravo');
  zero('since:7d');
  zero('is:active');
...
test('applyLocalFilter orders matches by relevance, then recency', () => {
  ...
  assert.deepEqual(H.applyLocalFilter(list, 'alpha').map((s: unknown) => property(s, 'id')), ['s3', 's2', 's1']);
  // No positive plain term → the incoming order stands.
  assert.deepEqual(H.applyLocalFilter(list, 'cwd:api').map((s: unknown) => property(s, 'id')), ['s2', 's3']);
});
...
test('groupSessionsByDate buckets by recency with undated sunk last', () => {
  ...
  assert.deepEqual(buckets.map((bucket: unknown) => property(bucket, 'key')), ['today', 'yesterday', 'lastweek', 'm:2026-05', 'undated']);
  assert.deepEqual(buckets[0].sessions.map((s: unknown) => property(s, 'id')), ['today', 'today2']);
...
test('groupSessionsByDate: Monday belongs to This week, Sunday before to Last week', () => {
  ...
  assert.deepEqual(buckets.map((bucket: unknown) => property(bucket, 'key')), ['yesterday', 'lastweek']);
});
...
test('findPathTokens picks file mentions out of prose, skipping URLs and word pairs', () => {
  ...
  const tokens = H.findPathTokens(text).map((token: unknown) => property(token, 'token'));
...
test('aggregateUsageWeekly chunks from the end and merges model rows by ref', () => {
  const mkDay = (day: string, cost: number, ref: string): UsageDay => ({
    day, calls: 1,
  ...
  const old = present(weeks[0]), recent = present(weeks[1]);
...
test('mergeUsageLimits collapses agreeing rows and qualifies differing ones', () => {
  const now = 1700000000000;
  const limit = (usedFraction: number, resetsAt: number | null, extra: Partial<UsageLimit> = {}): UsageLimit => ({
    label: '7 days', windowLabel: '7 days', resetsAt, usedFraction, ...extra,
  });
  const entry = (hostLabel: string, reports: readonly UsageLimitReport[]): UsageLimitEntry => {
    const payload = { generatedAt: now, harnesses: [{ harness: 'omp', label: 'Oh My Pi', reports }] };
    return { hostLabel, payload };
  };
  const report = (limits: UsageLimitReport['limits'], { planType = 'pro', fetchedAt = now, provider = 'fakeprov' } = {}): UsageLimitReport =>
  ...
  const shared = present(merged.reports[0]), sharedLimit = present(shared.limits[0]);
  ...
  const differing = present(merged.reports[0]);
  ...
  const split = present(merged.reports[0]);
  ...
  assert.equal(present(present(merged.reports[0]).limits[0]).hosts, null, 'the surviving row needs no qualifier');
});
test('usageLimitsHtml renders the merged provider quota view', () => {
  const now = 1700000000000;
  type LimitHarness = NonNullable<NonNullable<UsageLimitEntry['payload']>['harnesses']>[number];
  const entry = (hostLabel: string, harnesses: readonly LimitHarness[]): UsageLimitEntry => {
    const payload = { generatedAt: now, harnesses };
    return { hostLabel, payload };
  };
  const omp = (limits: UsageLimitReport['limits'], planType = 'max'): LimitHarness => ({
    harness: 'omp', label: 'Oh My Pi',
    reports: [{ provider: 'anthropic', planType, fetchedAt: now, limits }],
  });
  const base: UsageLimit[] = [
  ...
  const two = H.usageLimitsHtml([
    ...
    entry('host-b', [omp([{ ...present(base[0]) }, { ...present(base[1]), usedFraction: 0.2 }])]),
  ], { now });
  const fiveHour = present(two.match(/5 hours<\/span><small>42% used[^<]*<\/small>/))[0];
...
test('OMP_MODEL_ROLES lists the canonical roles in harness order', () => {
  assert.deepEqual(H.OMP_MODEL_ROLES.map((r: unknown) => property(r, 'key')),
    ['default', 'smol', 'slow', 'vision', 'plan', 'designer', 'commit', 'tiny', 'task', 'advisor']);
  assert.ok(H.OMP_MODEL_ROLES.every((role: unknown) => property(role, 'name') && property(role, 'description')));
});
...
test('buildModelRoleRows keeps global values editable and flags project overrides', () => {
  ...
  const def = present(rows.find(row => row.key === 'default'));
  ...
  const vision = present(rows.find(row => row.key === 'vision'));
  assert.equal(vision.value, '');
  assert.equal(vision.override, 'anthropic/claude-opus-4');

  const custom = present(rows.at(-1));
  assert.deepEqual(
...
test('buildModelRoleRows tolerates missing or malformed records', () => {
  ...
  assert.ok(rows.every((r: unknown) => property(r, 'value') === '' && property(r, 'override') === null));
...
test('sessionRef degrades to the bare prefix rather than throwing', () => {
  ...
  const invalidSession: unknown = { id: 42 };
...
test('sessionRefAliases derives the native id and uuid tail from the id alone', () => {
  ...
  assert.equal(present(H.decodeRouteSessionId(OMP_A)).harnessId, 'omp');
});

test('resolveSessionRefAmong resolves native ids and uuid prefixes, not just route ids', () => {
  const hit = (ref: string) => H.resolveSessionRefAmong(CORPUS, ref).session?.name;
...
test('resolveSessionRefAmong keeps route-id resolution ahead of aliases', () => {
  ...
  assert.equal(present(H.resolveSessionRefAmong(shadow, '01a07264').session).name, 'literal id');
  ...
  assert.deepEqual(ambiguous.matches.map((s: unknown) => property(s, 'name')), ['orchestrator', 'reviewer']);
  ...
  assert.equal(present(H.resolveSessionRefAmong(CORPUS, OMP_A, { exactOnly: true }).session).name, 'orchestrator');
...
test('shortSessionRef prefers the uuid tail and widens only when it must', () => {
  ...
  for (const session of CORPUS) {
    ...
    assert.equal(present(H.resolveSessionRefAmong(CORPUS, ref).session).name, session.name, ref);
  }
...
test('parseSessionRefTokens finds every ref form and dedupes them', () => {
  ...
  assert.deepEqual(H.parseSessionRefTokens(text).map((token: unknown) => property(token, 'ref')), [
    '8f3ab2c1',
    'tycho/8f3ab2c1',
...
test('parseSessionRefTokens trims sentence punctuation but never id characters', () => {
  ...
  assert.deepEqual(H.parseSessionRefTokens('resume #2026-08- now').map((token: unknown) => property(token, 'ref')), ['2026-08-']);
  assert.deepEqual(H.parseSessionRefTokens('resume #8f3ab2c1.').map((token: unknown) => property(token, 'ref')), ['8f3ab2c1']);
  assert.deepEqual(H.parseSessionRefTokens('resume #8f3ab2c1:').map((token: unknown) => property(token, 'ref')), ['8f3ab2c1']);
  assert.deepEqual(H.parseSessionRefTokens('resume #tycho/').map((token: unknown) => property(token, 'ref')), ['tycho']);
});
...
test('parseSessionRefTokens leaves markdown, short tokens and glued hashes alone', () => {
  ...
  assert.deepEqual(H.parseSessionRefTokens('`#abcdefgh`').map((token: unknown) => property(token, 'ref')), []);
  assert.deepEqual(H.parseSessionRefTokens('(#abcdefgh)').map((token: unknown) => property(token, 'ref')), ['abcdefgh']);
});
...
test('appendSessionRefContext is a no-op without resolved entries', () => {
  ...
  const unresolvedEntries: unknown = [{ name: 'x' }];
...
test('session ref fields cannot break out of the block', () => {
  ...
  assert.equal(present(block.match(/<session-refs>/g)).length, 1);
  assert.equal(present(block.match(/<\/session-refs>/g)).length, 1);
...
test('searchSessionsForRef ranks name matches over cwd, live over historical', () => {
  ...
  assert.deepEqual(H.searchSessionsForRef(list, 'index').map((r: unknown) => property(property(r, 'session'), 'id')), ['aaaa1111', 'cccc3333']);
  assert.deepEqual(H.searchSessionsForRef(list, 'ttr').map((r: unknown) => property(property(r, 'session'), 'id')), ['bbbb2222']);
  // A pasted prefix finds its own session even when the name says nothing.
  assert.deepEqual(H.searchSessionsForRef(list, 'cccc').map((r: unknown) => property(property(r, 'session'), 'id')), ['cccc3333']);
  // No query is "most recent, live first".
  assert.deepEqual(H.searchSessionsForRef(list, '').map((r: unknown) => property(property(r, 'session'), 'id')),
  ...
  assert.deepEqual(H.searchSessionsForRef(omp, '01a070d2').map((r: unknown) => property(property(r, 'session'), 'id')), [OMP_A]);
  assert.deepEqual(H.searchSessionsForRef(omp, '2026-09-05T09').map((r: unknown) => property(property(r, 'session'), 'id')), [OMP_A]);
  assert.deepEqual(H.searchSessionsForRef(omp, '~sk1_WyJvbXAi').map((r: unknown) => property(property(r, 'session'), 'id')), [OMP_A]);
...
test('mergeHostEntries puts self first and keys on hostId, then base', () => {
  ...
  assert.deepEqual(hosts.map((h: unknown) => property(h, 'key')), ['self-id', 'tycho-id', 'http://ganymede:3333']);
  ...
  assert.equal(hosts.filter((h: unknown) => property(h, 'hostId') === 'self-id').length, 1,
...
test('mergeHostEntries drops garbage bases instead of guessing', () => {
  ...
  assert.deepEqual(hosts.map((host: unknown) => property(host, 'base')), ['', 'http://ok:1']);
});

// --- per-host capability gating -------------------------------------------

test('hostSupportsTerminal reads the owning host\'s advertised capabilities', () => {
  const remote = (capabilities: Readonly<Record<string, boolean | undefined>>) =>
    ({ hostId: 'tycho-id', base: 'http://tycho:3333', capabilities });
...
test('hostSupportsTerminal falls back to /api/config for self only', () => {
  ...
  const legacySelf: unknown = { base: '', capabilities: null };
  assert.equal(Reflect.apply(H.hostSupportsTerminal, H, [legacySelf, { terminal: true }]), true,
...
test('sttUnavailableReason reports the first blocking condition, secure origin first', () => {
  ...
  const insecure = present(H.sttUnavailableReason({ ...ok, isSecureContext: false, hasGetUserMedia: false, hasMediaRecorder: false }));
  ...
  assert.equal(present(H.sttUnavailableReason()).code, 'insecure');
  assert.match(present(H.sttUnavailableReason({ isSecureContext: false })).message, /this origin/);
});
...
const hostState = (previous: HostConnectionRecord | null, event: unknown, at: number): HostConnectionRecord =>
  present(B.hostConnReduce(previous, event, at));

test('hostConnReduce climbs the backoff ladder one rung per failure', () => {
  const fail = (previous: HostConnectionRecord | null, at: number) =>
    hostState(previous, { type: 'failure', error: new Error('EHOSTUNREACH') }, at);
  let s = fail(null, T0);
...
test('hostConnReduce success clears the error but keeps the rung until it settles', () => {
  const down = hostState(hostState(null, { type: 'failure', error: 'x' }, T0),
...
test('mergeUsageSummaries sums totals, days, and per-model day buckets', () => {
  ...
  const merged = present(H.mergeUsageSummaries([
    { hostId: 'host-a', hostLabel: 'laptop', summary: a },
    { hostId: 'host-b', hostLabel: 'tycho', summary: b },
  ]));
  const totals = present(merged.totals), tokens = present(totals.tokens), costs = present(totals.costs);
  const headline = present(present(merged.headlineCosts).month);
  const headlineBuckets = present(present(merged.headlineCostsByBucket).month);
  const daily = present(merged.daily);
  assert.equal(totals.calls, 4);
  assert.equal(tokens.input, 20);
  assert.ok(Math.abs(present(costs.total) - 0.6) < 1e-9);
  assert.equal(headline.toFixed(2), '0.60', 'KPI headlines sum');
  assert.ok(Math.abs(present(headlineBuckets.total) - 0.6) < 1e-9 &&
    Math.abs(present(headlineBuckets.input) - 0.2) < 1e-9,
  ...
  const shared = present(daily.find(day => day.day === '2026-08-21'));
  assert.equal(shared.calls, 4);
  assert.equal(present(shared.models).length, 1, 'the same model ref on two hosts is one series');
  assert.ok(Math.abs((present(present(shared.models)[0]).cost ?? 0) - 0.6) < 1e-9);
...
test('mergeUsageSummaries keeps per-host workspaces and sessions apart, merges models', () => {
  ...
  const groups = present(merged.groups);
  const workspaces = present(groups.workspaces), sessions = present(groups.sessions), models = present(groups.models);
  ...
  assert.equal(present(models[0]).calls, 4);
});
...
test('mergeUsageSummaries preserves known subtotals and unavailable counts', () => {
  ...
  const totals = present(merged.totals), costs = present(totals.costs);
  assert.ok(Math.abs(present(costs.total) - 0.7) < 1e-9);
  assert.equal(totals.priced, false);
  assert.equal(totals.unpricedCalls, 3);
  assert.ok(Math.abs(present(present(merged.headlineCosts).month) - 0.7) < 1e-9);
  assert.equal(present(merged.headlineCostUnavailable).month, 3);
...
test('mergeUsageSummaries ranks merged groups by the requested metric', () => {
  ...
  const byCost = present(H.mergeUsageSummaries([
    { hostId: 'a', summary: usagePayload() }, { hostId: 'b', summary: small },
  ]));
  assert.deepEqual(present(present(byCost.groups).models).map(model => model.key), ['anthropic/opus', 'zai/glm']);

  const byTokens = present(H.mergeUsageSummaries([
...
test('assignHostColor rotates the chart slots by first-seen order', () => {
  ...
  for (const key of ['a', 'b', 'c', 'd', 'e', 'f', 'g']) {
    const res: ReturnType<typeof H.assignHostColor> = H.assignHostColor(order, key, {});
    order = res.order;
...
test('sortHostSections puts self first, then labels alphabetically', () => {
  ...
  assert.deepEqual(H.sortHostSections(hosts).map((host: unknown) => property(host, 'hostId')), ['s', 'b', 'a', 'z']);
...
test('sortHostSections breaks label ties on id so the order never jitters', () => {
  ...
  assert.deepEqual(H.sortHostSections(hosts).map((host: unknown) => property(host, 'hostId')), ['a', 'b']);
