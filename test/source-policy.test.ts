...
test('source inventory rejects unclassified bodies, declaration facades and stale exceptions', async () => {
  ...
  const result = classifyExecutableInventory(
    ...
    new Set(['lib/owned.js']),
    new Set(['src/owned.ts', 'test/support.d.ts', 'types/facade.d.ts']),
    new Set(['test/support.d.ts', 'test/broken-support.d.ts']),
    new Set(['docs/history.js', 'docs/missing-history.js']),
  );
...
test('repository inventory follows tracked paths and effective Git attributes', async t => {
  // The policy API is ESM while generated node:test files are CommonJS.
  const {
    isExecutableFamilyPath,
    repositoryPaths,
    shellInventoryPaths,
    validateLinguist,
  } = await import('../scripts/check-source-policy.mjs');
  ...
  execFileSync('git', ['add', '.amp/rogue.js', '.agents/setup', '.gitattributes', 'authored.js',
  ...
  fs.writeFileSync(path.join(dir, 'nested/.gitattributes'), '');
  const falseAndUnspecified: string[] = [];
...
test('named shell inventory enforces declared sets, index modes and worktree state', async t => {
  ...
  execFileSync('git', ['add', '.agents/setup', 'service.sh'], { cwd: dir });
  ...
  execFileSync('git', ['add', '.agents/setup'], { cwd: dir });
  assert.deepEqual(validateShellInventory(dir, repositoryPaths(dir), declared), {
    paths: ['.agents/setup', 'service.sh'],
    failures: [
  ...
  execFileSync('git', ['add', 'rogue'], { cwd: dir });
  assert.deepEqual(validateShellInventory(dir, repositoryPaths(dir), declared), {
    paths: ['.agents/setup', 'rogue', 'service.sh'],
    failures: ['rogue: unclassified tracked shell path'],
  });
  assert.deepEqual(validateShellInventory(
    dir,
    repositoryPaths(dir),
    new Set([...declared, 'missing.sh']),
  ).failures, [
