// Generated test/tool from test/omp-bridge-session-switch.test.ts; edit that source and run npm run build:tests.
"use strict";
...
async function waitFor(read, label, timeout = 5000) {
    ...
    throw new Error(`Timed out waiting for ${label}${lastError ? `: ${(0, test_types_js_1.errorMessage)(lastError)}` : ''}`);
}
    ...
    try {
        await waitFor(() => {
            if (child.exitCode !== null)
                throw new Error(`host exited ${child.exitCode}: ${stderr}`);
...
test('OMP bridge adopts session_switch identity in place and suppresses same-id reloads', { skip: !bunAvailable }, async () => {
    ...
    try {
        ...
        const updated = await waitFor(() => {
            const entry = (0, test_types_js_1.record)(JSON.parse(fs.readFileSync(host.registryPath, 'utf8')));
...
test('Pi descriptor does not subscribe to preserved-runner session_switch events', { skip: !bunAvailable }, async () => {
    ...
    try {
        ...
        await delay(100);
        const unchanged = (0, test_types_js_1.record)(JSON.parse(fs.readFileSync(host.registryPath, 'utf8')));
