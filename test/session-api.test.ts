...
const { normalizeModels, sessionForClient, decodeSessionMetadata, decodeSessionList,
  ...
  decodeSessionRow, decodeSessionMutationPatch, decodeSessionActivityPatch, decodeSessionTranscriptPatch }: typeof import('../lib/session-api') = require('../lib/session-api')
...
test('closed session ingress separates authority, endpoint identity and opaque peer extras', () => {
  ...
  const capabilities = present(row.fields.capabilities);
