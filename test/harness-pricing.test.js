// Generated tool from test/harness-pricing.test.ts; edit that source and run npm run build:tools.
'use strict';
...
test('OMP catalog command is cached, persisted, and prices tokens including cache reads', async () => {
    ...
    assert.equal(pricing.estimateUsageCost('pi', 'zai', 'glm-4.7-flash', { input: 1_000_000 }), undefined, 'Pi never reads the OMP catalog');
...
test('Pi registry rates are cached and backfill zero-cost JSONL usage', async (t) => {
    ...
    assert.equal(pricing.estimateUsageCost('pi', 'zai', 'glm-5.3', { input: 1_000_000 }), undefined, 'zero-rate ZAI subscription entries remain unpriced rather than free');
...
test('stale last-known OMP catalog survives refresh failure; missing catalog stays unpriced', async (t) => {
    ...
    assert.ok(combined);
    assert.ok(Number.isFinite(combined.total), 'combined OMP selectors match provider/id catalog keys');
...
test('OMP subscription-plan zero rates stay unpriced while free tiers remain free', () => {
    ...
    try {
        ...
        assert.equal(pricing.estimateUsageCost('omp', 'kimi-code', 'k3', { input: 1_000_000 }), undefined, 'zero-rate Kimi subscription entries are unpriced, not free');
        assert.equal(pricing.estimateUsageCost('omp', 'google-antigravity', 'gemini-3.7-flash', { input: 1_000_000 }), undefined, 'zero-rate Antigravity subscription entries are unpriced, not free');
        assert.deepEqual(pricing.estimateUsageCost('omp', 'opencode-zen', 'mimo-v2.5-free', { input: 1_000_000 }), { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0 }, 'genuinely free tiers keep their authoritative $0');
        const pricedPlan = pricing.estimateUsageCost('omp', 'kimi-code', 'k3-priced', { input: 1_000_000 });
        assert.ok(pricedPlan);
        assert.equal(pricedPlan.total, 3, 'a rate-card override prices the same provider normally');
        ...
        assert.equal(usage.models['kimi-code/k3'].costUnavailable.total, 1, 'plan-reported zeros count as unavailable, not free');
...
test('OMP models.yml rate card prices dead ids and beats catalog rows', () => {
    ...
    try {
        ...
        const overrideOnly = pricing.estimateUsageCost('omp', 'google-antigravity', 'gemini-3.7-flash-tiered', { input: 1_000_000, output: 1_000_000 });
        assert.ok(overrideOnly);
        assert.equal(overrideOnly.total, 4.5, 'an override-only id OMP drops from its catalog still prices');
        const overridden = pricing.estimateUsageCost('omp', 'zai', 'glm-4.7-flash', { input: 1_000_000, output: 1_000_000 });
        assert.ok(overridden);
        assert.equal(overridden.total, 3, 'a user rate-card row wins over the catalog row');
        ...
        assert.notEqual(pricing.pricingRevision('omp'), revision, 'rate-card edits shift the pricing revision');
        // A broken file degrades to no overrides rather than breaking pricing.
        fs.writeFileSync(modelsYml, 'providers: [unclosed\n');
        const fallback = pricing.estimateUsageCost('omp', 'zai', 'glm-4.7-flash', { input: 1_000_000, output: 1_000_000 });
        assert.ok(fallback);
        assert.ok(Math.abs(fallback.total - 2.8) < 1e-9, 'broken models.yml falls back to the plain catalog');
    }
    finally {
        process.env.HOME = tmp;
        pricing.resetForTests();
