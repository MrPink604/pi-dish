// Generated test/tool from test/browser-api.test.ts; edit that source and run npm run build:tests.
"use strict";
...
test('browser transport snapshots host routing and preserves supported header/signal forms', async () => {
    ...
    endpoint.base = 'https://other.invalid';
    endpoint.token = 'other';
    await pending;
    const request = (0, test_types_js_1.present)(seen[0]), headers = new Headers(request.options.headers);
...
test('endpoint-explicit mutations reach the captured route and decode server results', async (t) => {
    ...
    const server = http.createServer(async (req, res) => {
        ...
        const body = (0, test_types_js_1.record)(JSON.parse(Buffer.concat(chunks).toString()));
        ...
        if (req.url === '/peer/api/sessions/session%2Fone/model' && typeof body.modelId === 'string')
            state.model = body.modelId;
        else if (req.url === '/peer/api/sessions/session%2Fone/thinking' && typeof body.level === 'string')
            state.thinking = body.level === 'max' ? 'high' : body.level;
        else if (req.url === '/peer/api/sessions/session%2Fone/rename' && typeof body.name === 'string')
            state.name = body.name;
        else {
            res.writeHead(404);
            res.end('{"error":"wrong endpoint"}');
            return;
        }
    ...
    if (!address || typeof address === 'string')
        throw new Error('Fixture server lacks TCP address');
    ...
    await assert.rejects(renameSession(transport.request, { ...original, token: 'bad' }, 'session/one', 'Denied'), error => (0, test_types_js_1.record)(error).status === 401 && (0, test_types_js_1.record)(error).message === 'denied');
...
test('typed reads reject malformed successes and retain HTTP error status', async () => {
    ...
    await assert.rejects(api.list(null, '/api/sessions'), error => (0, test_types_js_1.record)(error).status === 401 && (0, test_types_js_1.record)(error).message === 'denied');
    ...
    assert.equal(Object.keys((0, test_types_js_1.record)(generic)).length, 0);
});
test('session list ingress separates opaque extras and omits malformed presentation values', async () => {
    ...
    const row = (0, test_types_js_1.present)(list.active[0]);
    ...
    assert.equal((0, test_types_js_1.record)(row.extras.custom).model, 42);
    assert.equal((0, test_types_js_1.record)(row.extras.fields).model, 'forged');
