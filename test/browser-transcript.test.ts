...
test('transcript pages require message arrays and narrow cursors without accepting wire identity', () => {
  ...
  assert.equal(Object.hasOwn(data.session, 'id'), false); assert.equal(Object.hasOwn(data.session, 'host'), false); assert.equal(data.session.name, 'kept');
});
test('transcript decoding preserves authoritative indexed empty assistant entries', () => {
  ...
  const message = data.messages[0]; assert.ok(message); assert.equal(data.messages.length, 1); assert.equal(message.index, 3); assert.ok(Array.isArray(message.content)); assert.equal(message.content.length, 0);
});
