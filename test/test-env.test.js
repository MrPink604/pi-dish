// Generated tool from test/test-env.test.ts; edit that source and run npm run build:tools.
"use strict";
...
test('sanitizeTestEnv pins the bind target and drops the operator deployment', () => {
    const env = (0, test_env_js_1.sanitizeTestEnv)(OPERATOR_ENV);
...
test('sanitizeTestEnv puts a user-installed bun on PATH, but never replaces one', () => {
    ...
    try {
        ...
        const added = (0, test_env_js_1.sanitizeTestEnv)({ ...OPERATOR_ENV, HOME: home });
        ...
        const untouched = (0, test_env_js_1.sanitizeTestEnv)({ ...OPERATOR_ENV, HOME: home, PATH: already });
        assert.equal(untouched.PATH, already, 'a bun already on PATH wins');
        assert.equal((0, test_env_js_1.sanitizeTestEnv)(OPERATOR_ENV).PATH, '/usr/bin', 'no bun installed, no PATH change');
    }
    finally {
