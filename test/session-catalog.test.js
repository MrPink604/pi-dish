...
test('registered catalog projection carries the bridge-reported ask block', () => {
    const src = source('omp');
    const blocked = buildActiveSession(registeredSessionObservation({ sessionFile: src.file, askPending: true }, context(src, null, { source: null })), options);
    assert.equal(blocked.askPending, true);
    const idle = buildActiveSession(registeredSessionObservation({ sessionFile: src.file, askPending: 'yes' }, context(src, null, { source: null })), options);
...
test('rpc catalog projection derives the ask block from pending extension dialogs', () => {
    ...
    const blocked = buildActiveSession(rpcSessionObservation({ sessionFile: src.file, extUIState: { dialogs } }, context(src, null)), options);
    assert.equal(blocked.askPending, true);
