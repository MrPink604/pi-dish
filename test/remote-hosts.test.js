// Generated test/tool from test/remote-hosts.test.ts; edit that source and run npm run build:tests.
"use strict";
...
const PKG_VERSION = (0, test_types_js_1.record)(packageData).version;
if (typeof PKG_VERSION !== 'string')
    ...
    return new Promise((resolve, reject) => {
        ...
        probe.listen(0, '127.0.0.1', () => {
            ...
            if (!address || typeof address === 'string')
                return reject(new Error('Port probe lacks TCP address'));
            probe.close(error => error ? reject(error) : resolve(address.port));
        });
    });
}
function listenHttp(server, port = 0) {
    return new Promise((resolve, reject) => {
        ...
        server.listen(port, '127.0.0.1', () => {
            ...
            if (!address || typeof address === 'string')
                return reject(new Error('HTTP server lacks TCP address'));
...
async function boot(home, extraEnv = {}) {
    ...
    const child = (0, node_child_process_1.spawn)(process.execPath, [SERVER], { env, stdio: ['ignore', 'pipe', 'pipe'] });
    children.push(child);
    let stderr = '';
    if (!child.stderr || !child.stdout)
        throw new Error('Server child missing captured output');
    ...
    for (;;) {
        ...
        if (Date.now() > deadline)
            throw new Error(`server did not come up: ${stderr}`);
...
test('remotes are validated on load and bad entries are skipped, never thrown', () => {
    withHome({
        ...
        assert.deepEqual(remotes.map((remote) => (0, test_types_js_1.record)(remote).name), ['peer', 'work', 'work2']);
        assert.deepEqual(remotes[0], { name: 'peer', kind: 'direct', origin: 'http://10.0.0.4:3333', token: 'tok' });
        assert.equal((0, test_types_js_1.record)(remotes[1]).kind, 'ssh');
        assert.equal((0, test_types_js_1.record)(remotes[1]).remotePort, 3333, 'remotePort defaults to 3333');
        assert.equal((0, test_types_js_1.record)(remotes[2]).remotePort, 4444);
        assert.equal((0, test_types_js_1.record)(remoteHosts.getRemote('peer')).origin, 'http://10.0.0.4:3333');
    ...
    process.env.HOME = home;
    try {
        assert.deepEqual(remoteHosts.listRemotes(), []);
    }
    finally {
        process.env.HOME = previous;
    }
...
test('ssh forwards land on a per-remote socket under the dish run dir', () => {
    withHome({ remotes: [{ name: 'work', sshDest: 'user@box', remotePort: 4321 }] }, (home) => {
        ...
        const work = (0, test_types_js_1.present)(remoteHosts.getRemote('work'));
        if (!('sshDest' in work))
            throw new Error('Expected SSH remote fixture');
    ...
    withHome({ remotes: [{ name: 'bee', sshDest: 'bee', remoteHost: '100.70.163.97' }] }, () => {
        const bee = (0, test_types_js_1.present)(remoteHosts.getRemote('bee'));
        if (!('sshDest' in bee))
            throw new Error('Expected SSH remote fixture');
...
test('probing an unconfigured name reports unknown_host without a request', async () => {
    await withHome({}, async () => {
        ...
        if (result.reachable)
            throw new Error('unknown host unexpectedly reachable');
...
async function withFakeClock(fn) {
    ...
    Date.now = () => now;
    try {
        return await fn((milliseconds) => { now += milliseconds; });
    }
    finally {
        Date.now = realNow;
    }
...
test('reachability() is a pure cache read that expires on the ladder', async () => {
    await withHomeAsync({ remotes: [{ name: 'peer', url: 'http://127.0.0.1:1/' }] }, async () => {
        await withFakeClock(async (advance) => {
            ...
            const down = (0, test_types_js_1.present)(remoteHosts.reachability('peer'));
            assert.equal(down.reachable, false);
            if (down.reachable)
                throw new Error('failed peer unexpectedly reachable');
            ...
            assert.equal((0, test_types_js_1.present)(remoteHosts.reachability('peer')).reachable, false);
...
test('GET /api/hosts lists self first, then every configured remote', async () => {
    ...
    const responseBody = (0, test_types_js_1.record)(await (await fetch(`${hub.base}/api/hosts`)).json());
    const hosts = (0, test_types_js_1.records)(responseBody.hosts);
    assert.ok(Date.now() - started < 3500, 'the fleet list must not wait on a dead peer');
    const self = (0, test_types_js_1.present)(hosts[0]);
    ...
    assert.equal((0, test_types_js_1.record)(self.capabilities).sessions, true);
    assert.deepEqual(hosts.slice(1).map(host => host.name), ['peer', 'echo', 'down']);
    const peerDescriptor = (0, test_types_js_1.record)(await (await fetch(`${peer.base}/api/host`)).json());
    const entry = (0, test_types_js_1.present)(hosts.find(host => host.name === 'peer'));
    ...
    // socket/ssh text.
    const down = (0, test_types_js_1.present)(hosts.find(host => host.name === 'down'));
    assert.equal(down.reachable, false);
...
test('the proxy reaches the peer with the peer\'s own credential', async () => {
    ...
    const body = (0, test_types_js_1.record)(await proxied.json());
    assert.ok((0, test_types_js_1.records)(body.previous).some(session => session.id === PEER_SESSION_ID), 'the peer\'s fixture session should be listed');
    // The peer's own session list is what came back, not the hub's (empty) one.
    const local = (0, test_types_js_1.record)(await (await fetch(`${hub.base}/api/sessions`)).json());
    assert.equal((0, test_types_js_1.records)(local.previous).some(session => session.id === PEER_SESSION_ID), false);
    ...
    const page = (0, test_types_js_1.record)(await messages.json());
    assert.equal((0, test_types_js_1.records)(page.messages).length, 1);
});
test('proxied requests carry the peer token and nothing of the caller\'s', async () => {
    ...
    const seen = (0, test_types_js_1.record)(await res.json());
    assert.equal(seen.method, 'POST');
    assert.equal(seen.url, '/api/sessions/abc/prompt?x=1');
    const headers = (0, test_types_js_1.record)(seen.headers);
...
test('unknown and malformed host names are bare 404s; peer statuses pass through', async () => {
    ...
    assert.match(String((0, test_types_js_1.record)(await bad.json()).error), /days must be/);
});
...
test('an unreachable peer fails fast instead of hanging', async () => {
    ...
    const body = (0, test_types_js_1.record)(await res.json());
...
test('a proxied terminal upgrade is spliced through to the peer', async (t) => {
    const capabilities = (0, test_types_js_1.record)((0, test_types_js_1.record)(await (await fetch(`${peer.base}/api/host`)).json()).capabilities);
    if (!capabilities.terminal)
        return t.skip('node-pty unavailable on the peer');
    // The hub itself has no terminal: a proxied PTY lives on the peer.
    const hubCapabilities = (0, test_types_js_1.record)((0, test_types_js_1.record)(await (await fetch(`${hub.base}/api/host`)).json()).capabilities);
...
const shutdownSignals = [['SIGTERM', 143], ['SIGINT', 130]];
for (const [signal, exitCode] of shutdownSignals) {
    test(`the ssh transport forwards through a socket and cleans up on ${signal}`, async () => {
        ...
        const hostBody = (0, test_types_js_1.record)(await (await fetch(`${sshHub.base}/api/hosts`)).json());
        const hosts = (0, test_types_js_1.records)(hostBody.hosts);
        const work = (0, test_types_js_1.present)(hosts.find(host => host.name === 'work'));
        ...
        assert.equal(proxied.status, 200);
        assert.ok((0, test_types_js_1.records)((0, test_types_js_1.record)(await proxied.json()).previous).some(session => session.id === PEER_SESSION_ID));
...
test('a hub token gates /hosts/* exactly like /api/*', async () => {
    ...
    assert.ok((0, test_types_js_1.records)((0, test_types_js_1.record)(await ok.json()).previous).some(session => session.id === PEER_SESSION_ID));
    assert.equal((await fetch(`${peer.base}/api/sessions`, authed(HUB_TOKEN))).status, 401);
    const hostBody = (0, test_types_js_1.record)(await (await fetch(`${gated.base}/api/hosts`, authed(HUB_TOKEN))).json());
    const hosts = (0, test_types_js_1.records)(hostBody.hosts);
    assert.equal((0, test_types_js_1.present)(hosts.find(host => host.name === 'peer')).reachable, true);
    // EventSource can't send a header, so the hub's own ticket opens the
    // proxied stream route — and only that route.
    const ticketBody = (0, test_types_js_1.record)(await (await fetch(`${gated.base}/api/auth/ticket`, authed(HUB_TOKEN, {
        method: 'POST',
...
test('the proxy trips a breaker on a down peer and re-dials when the slot expires', async () => {
    ...
    const body = (0, test_types_js_1.record)(await first.json());
    ...
    assert.equal((0, test_types_js_1.record)(await shorted.json()).reason, 'connection_refused');
