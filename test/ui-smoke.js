#!/usr/bin/env node
// Generated test/tool from test/ui-smoke.ts; edit that source and run npm run build:tests.
"use strict";
 ...
 */
const scenarios = require("./ui-scenarios/index.js");
...
const requestedScenario = args[0] === '--scenario' && args.length === 2 ? args[1] : null;
if (args.length && !isScenarioName(requestedScenario)) {
    throw new Error('Usage: npm run test:ui -- [--list | --scenario <name>]');
}
const selectedScenario = isScenarioName(requestedScenario) ? requestedScenario : null;
...
const appendEntry = (entry) => fs.appendFileSync(sessionFile, JSON.stringify(entry) + '\n');
...
function setQueue(queue) { liveQueue = queue; emit('queue_update', liveQueue); }
function handleCommand(sock, msg) {
    // Dialog answer from the web UI (bridge-session command form).
    if (msg.command === 'extension_ui_response') {
        lastUIResponse = msg;
    ...
    switch (msg.command) {
        ...
        case 'prompt':
            ...
            if (fakeCompacting) {
                bufferedPrompt = msg;
                return respond(sock, msg.id, { queued: true });
            }
            respond(sock, msg.id, {});
        ...
        case 'set_session_name':
            ...
            if (typeof msg.name !== 'string')
                throw new Error('set_session_name requires a string name');
...
function streamTurn(userText, images) {
    ...
    const userContent = [{ type: 'text', text: userText }, ...imageList];
    emit('turn_start', {});
    ...
    appendEntry({ type: 'message', message: echoedUser });
...
const sttRequests = [];
const sttEndpoint = http.createServer(async (req, res) => {
    try {
        const chunks = [];
        ...
        sttRequests.push({ error: (0, test_types_js_1.errorMessage)(error) });
    }
...
function writeSttSettings(block) {
    const parsed = fs.existsSync(dishSettingsFile) ? JSON.parse(fs.readFileSync(dishSettingsFile, 'utf8')) : {};
    if (!(0, wire_protocol_js_1.isRecord)(parsed))
        throw new Error('Fixture settings must be an object');
    if (block)
        parsed.stt = block;
    else
        delete parsed.stt;
    fs.writeFileSync(dishSettingsFile, JSON.stringify(parsed, null, 2) + '\n');
}
// --- assertions ---------------------------------------------------------------
let failures = 0;
...
const REMOTE_SERVER_HOST = path.join(__dirname, 'fixtures', 'remote-server-host.js');
const IDLE_CHILD = path.join(__dirname, 'fixtures', 'idle-child.js');
function writeRemoteFixture(mainOrigin) {
    ...
    fs.writeFileSync(path.join(dishDir, 'settings.json'), JSON.stringify({ allowedOrigins: [mainOrigin], label: 'tycho' }));
...
async function startRemoteHost() {
    ...
    const child = (0, node_child_process_1.spawn)(process.execPath, [REMOTE_SERVER_HOST, serverPath], {
        env: {
            ...process.env,
            HOME: remoteHome,
            PORT: '0',
    ...
    if (!child.stdout)
        throw new Error('Remote host stdout is unavailable');
    let buffered = '';
    ...
    throw new Error('remote host exited before startup: ' + buffered);
}
...
(async () => {
    ...
    await (0, node_events_1.once)(bridge, 'listening');
    writeRegistry();
    sttEndpoint.listen(0, '127.0.0.1');
    await (0, node_events_1.once)(sttEndpoint, 'listening');
    const sttAddress = sttEndpoint.address();
    if (sttAddress === null || typeof sttAddress === 'string')
        throw new Error('Expected STT TCP address');
    writeSttSettings({
        url: `http://127.0.0.1:${sttAddress.port}/v1/audio/transcriptions`,
        model: 'whisper-1',
    });
    ...
    if (serverAddress === null || typeof serverAddress === 'string')
        throw new Error('Expected server TCP address');
    ...
    const watch = async (page, tag) => {
        ...
        page.on('console', (message) => {
            ...
            if (message.type() === 'error' && !message.text().includes('Failed to load resource')) {
                errors.push(`${tag} console: ${message.text()}`);
            }
        });
    };
    try {
        ...
        if (selectedScenario) {
            ...
            if (selectedScenario === 'usage')
                await fetch(base + '/api/usage-summary?days=30');
            ...
            const coldResources = await desktop.evaluate(() => performance.getEntriesByType('resource').map(entry => new URL(entry.name).pathname));
            check(!coldResources.some((pathname) => /\/vendor\/(?:katex|highlight|hljs-theme|xterm)/.test(pathname)), 'cold page transfers no transcript/terminal vendor assets');
            check(!coldResources.some((pathname) => /^\/api\/(?:cwds|tmux\/targets|harnesses|models|commands)$/.test(pathname)), 'cold page skips new-session and composer catalogs');
            await desktop.click('.session-item');
            await desktop.waitForSelector('.message.assistant');
            check(await desktop.locator('.message .markdown-body strong').first().textContent() === 'answer', 'historical markdown rendered');
            ...
            check(await desktop.locator('.message.custom-message.interrupted').count() === 1 &&
                await desktop.locator('.message.custom-message.interrupted').textContent().then((t) => !!t?.includes('Interrupted')), 'interrupted-thinking renders a divider without its hidden reasoning');
            check(!(await desktop.locator('#messages').textContent() ?? '').includes('private interrupted reasoning'), 'interrupted-thinking reasoning stays hidden');
            ...
            check(await desktop.locator('#thinkingDropdown .thinking-option').count() >= 2, 'thinking dropdown lists selectable levels');
            check(!!thinkingBox && !!badgeBox && Math.abs(thinkingBox.x - badgeBox.x) < 2 &&
                thinkingBox.y >= badgeBox.y, 'thinking dropdown is anchored under its badge on desktop');
            await desktop.click('#sessionThinking');
            await desktop.waitForSelector('#thinkingDropdown', { state: 'hidden' });
            check(!(await desktop.locator('#thinkingDropdown').isVisible()), 'clicking the badge again closes the thinking dropdown');
            ...
            await desktop.waitForFunction(() => document.getElementById('messages')?.textContent.includes('switched transcript answer'), { timeout: 5000 });
            check(!(await desktop.locator('#messages').textContent() ?? '').includes('existing answer'), 'new route renders only its own history, not the old transcript cache');
            check(await desktop.evaluate(() => parseSessionKey(localStorage.getItem('pi-dish-session')).sessionId) === SWITCH_ID, 'client follows session_switch to the new route');
            ...
            await desktop.waitForFunction(() => document.getElementById('messages')?.textContent.includes('existing answer'), { timeout: 5000 });
            ...
            if (histLabel === null)
                throw new Error('Missing histLabel');
            ...
            check((await imgResult.locator('.tool-result-meta').allTextContents()).some((t) => t.trim() === 'image'), 'tool-result header meta flags the image');
            check(await imgResult.locator('details.tool-result-details').evaluate((el) => el.open), 'image tool result is open by default');
            ...
            check(await imgResult.locator('.tool-result-details img.msg-image').count() === 1, 'image tool result renders one img.msg-image inside the details');
            const historicalImage = imgResult.locator('img.msg-image');
            const imgSrc = await historicalImage.getAttribute('src');
            check(imgSrc === `/api/sessions/${SESSION_ID}/messages/ui-img1/images/1`, `historical image URL uses its stable JSONL entry id (got ${imgSrc})`);
            ...
            await desktop.waitForFunction(() => document.querySelector('.live-tool-output')?.textContent.includes('h'), { timeout: 5000 });
            check(await desktop.locator('.live-tool-summary').isVisible() &&
                await desktop.locator('.live-tool-summary').textContent() === 'echo hi', 'Bash command remains visible once output starts flowing');
            ...
            check(await desktop.evaluate(() => [...document.querySelectorAll('.message.user')]
                .filter(el => el.textContent.includes('ping from smoke test')).length === 1), 'prompt echo suppressed (single user bubble mid-turn)');
            ...
            await desktop.evaluate(() => { fixtureElement(document.getElementById('messages'), "document.getElementById('messages')").scrollTop = 0; });
            ...
            await desktop.waitForFunction(() => [...document.querySelectorAll('.message.assistant[data-msg-index]')]
            ...
            check(await desktop.locator('.message.assistant[data-streaming="true"]').count() === 0, 'streaming placeholder cleaned up');
            await desktop.waitForTimeout(300); // let the simulated late OMP repeats land
            check(await desktop.evaluate(() => [...document.querySelectorAll('.message.assistant')]
                .filter(el => el.textContent.includes('Streamed reply with')).length === 1), 'repeated assistant completion remains a single rendered response');
            check(await desktop.locator('.message.assistant[data-streaming="true"]').count() === 0, 'late message_update resurrects no streaming bubble');
            check(await desktop.evaluate(() => !fixtureElement(document.getElementById('sessionWorking'), "document.getElementById('sessionWorking')").classList.contains('active')), 'late message_update does not re-arm the working badge');
            // The JSONL catch-up supersedes the live panel and folds this turn's
            // tool activity into a second collapsed group.
            check(await desktop.locator('details.live-tool-panel').count() === 0, 'live tool panel removed once authoritative messages land');
            check(await desktop.locator('details.tool-group').count() === 2, 'streamed turn tool activity folded into its own group');
            ...
            await desktop.waitForFunction(() => document.querySelector('.code-block:not(.diagram-block) code')?.classList.contains('hljs'), { timeout: 5000 });
            check(true, 'first fenced code block lazily loads syntax highlighting');
            await codeBlock.locator('.code-copy-btn').click();
            await desktop.waitForFunction(() => [...document.querySelectorAll('.code-copy-btn')].some(b => b.textContent === '✓'), { timeout: 2000 });
            ...
            check(await desktop.locator('.session-item-status.working').count() === 0, 'working dot cleared after the turn');
            ...
            await desktop.waitForFunction(() => document.querySelectorAll('.diagram-block[data-diagram-state="rendered"] svg').length === 2, null, { timeout: 30000 });
            check(true, 'tagged and sniffed diagram fences both rendered to SVG');
            const diagram = desktop.locator('.diagram-block').first();
            check((await diagram.locator('svg').textContent() ?? '').includes('EUP processor'), 'flowchart node labels rendered inside the SVG');
            check(!(await diagram.locator('pre').isVisible()), 'diagram source hidden behind the rendered SVG');
            ...
            await desktop.waitForFunction(() => fixtureElement(document.querySelector('.diagram-block .code-copy-btn'), "document.querySelector('.diagram-block .code-copy-btn')").textContent === '✓', null, { timeout: 3000 });
            const copiedDiagram = await desktop.evaluate(() => navigator.clipboard.readText());
            check(copiedDiagram.startsWith('flowchart LR'), `copy button yields the mermaid source (got ${JSON.stringify(copiedDiagram.slice(0, 20))})`);
            await diagram.locator('.diagram-source-btn').click();
            check(await diagram.locator('pre').isVisible() &&
                !(await diagram.locator('.diagram-render').isVisible()), 'source toggle swaps the SVG for the mermaid source');
            await diagram.locator('.diagram-source-btn').click();
            ...
            const zoomed = await desktop.evaluate(() => ({
                width: fixtureElement(document.querySelector('.diagram-lightbox .diagram-stage svg'), "document.querySelector('.diagram-lightbox .diagram-stage svg')").getBoundingClientRect().width,
                label: fixtureElement(document.querySelector('.diagram-zoom-label'), "document.querySelector('.diagram-zoom-label')").textContent,
            }));
            check(zoomed.width > 0 && /^\d+%$/.test(zoomed.label), `zoom overlay shows the diagram at a stated scale (got ${JSON.stringify(zoomed)})`);
            await desktop.locator('.diagram-zoom-bar button[title="Zoom in"]').click();
            check(await desktop.evaluate(() => fixtureElement(document.querySelector('.diagram-lightbox .diagram-stage svg'), "document.querySelector('.diagram-lightbox .diagram-stage svg')").getBoundingClientRect().width) > zoomed.width, 'zoom-in enlarges the diagram');
            await desktop.keyboard.press('Escape');
            check(await desktop.locator('.diagram-lightbox').count() === 0, 'Escape closes the zoom overlay');
            ...
            check((await desktop.locator('[data-tool-call-id="completion-only"]').textContent() ?? '').includes('complete output'), 'completion-only end creates a finished panel');
            check(await desktop.locator('[data-tool-call-id="completion-only"]').count() === 1, 'repeated tool completion stays a single panel');
            ...
            check((await desktop.locator('[data-tool-call-id="completion-only"]').textContent() ?? '').includes('late background update'), 'post-cleanup update recreates and reopens the panel');
            emit('tool_execution_end', { toolCallId: 'completion-only', toolName: 'Bash',
                args: { command: 'echo complete' }, result: { content: [{ type: 'text', text: 'late final output' }] }, isError: false });
            await desktop.waitForSelector('details.live-tool-panel.complete[data-tool-call-id="completion-only"]', { timeout: 3000 });
            check((await desktop.locator('[data-tool-call-id="completion-only"]').textContent() ?? '').includes('late final output'), 'late background end finalizes the recreated panel');
            ...
            check(await desktop.locator('[data-tool-call-id="dedupe-start"]').count() === 1, 'repeated start for a known toolCallId stays deduped');
            await desktop.evaluate(() => fixtureApp.features.liveToolsController.clear(document.getElementById('messages')));
            ...
            check(await desktop.locator('.message.custom-message.async-result:not([data-msg-index])').count() === 1, 'cumulative live custom updates upsert one row');
            ...
            check(!(await desktop.locator('#messages').textContent() ?? '').includes('hidden live custom content'), 'unknown hidden live custom messages follow the documented skip');
            ...
            check(await advisorCard.locator('.advisor-severity.sev-concern').textContent() === 'concern', 'advisor card shows its severity chip');
            check((await advisorCard.locator('.advisor-label').textContent() ?? '').trim() === 'Advisor', 'unnamed advisor renders the bare label');
            check(await advisorCard.locator('.advisor-note .markdown-body code').count() === 1
                && !(await advisorCard.textContent() ?? '').includes('<advisory'), 'advisor note renders as markdown with the XML wrapper stripped');
            // Advisor notes are conversation context (like branch summaries), so focus
            // mode must leave them on screen.
            await desktop.evaluate(() => fixtureElement(document.getElementById('messages'), "document.getElementById('messages')").classList.add('focus-mode'));
            check(await advisorCard.isVisible(), 'advisor card survives focus mode');
            await desktop.evaluate(() => fixtureElement(document.getElementById('messages'), "document.getElementById('messages')").classList.remove('focus-mode'));
            ...
            check((await advisorBatch.locator('.advisor-label').textContent() ?? '').includes('security'), 'named advisor renders its name in the header');
            check(await advisorBatch.locator('.advisor-note').count() === 2
                ...
                && await advisorBatch.locator('.advisor-note .advisor-severity.sev-blocker').count() === 1, 'a multi-note batch is one card with a chip per note');
            ...
            check(detailRows.some((t) => t.includes('Estimated input') && t.includes('Unavailable')), 'response details label an omitted component unavailable');
            check(detailRows.some((t) => t.includes('Estimated total') && t.includes('~$0.0010')), 'response details keep a known total beside unavailable components');
            await desktop.keyboard.press('Escape');
            ...
            await desktop.waitForFunction(() => /Message share link copied/.test(document.getElementById('status')?.textContent || ''), { timeout: 5000 });
            const msgLink = await desktop.evaluate(() => navigator.clipboard.readText());
            check(/\/share\/[A-Za-z0-9_-]+\?targetId=ui-a1$/.test(msgLink), `clipboard holds the share deep link (got ${JSON.stringify(msgLink)})`);
            ...
            check(menuItems.length === 2 && menuItems[0].includes('Copy session ref'), `context menu offers the ref item (got ${JSON.stringify(menuItems)})`);
            check(menuItems[0].includes(expectedRefPrefix), `ref item previews the bare id prefix (got ${JSON.stringify(menuItems[0])})`);
            const copiedRef = await desktop.locator('#sessionMenu .context-menu-item').first().getAttribute('data-copy');
            if (copiedRef === null)
                throw new Error('Missing copiedRef');
            check(copiedRef.startsWith(expectedRefPrefix), `ref widens its prefix only as far as same-host collisions require (got ${JSON.stringify(copiedRef)})`);
            ...
            await desktop.waitForFunction(() => !!document.querySelector('#sessionMenu .context-menu-item.copied'), { timeout: 2000 });
            check(await desktop.evaluate(() => navigator.clipboard.readText()) === copiedRef, 'ref landed on the clipboard');
            ...
            if (statsText === null)
                throw new Error('Missing statsText');
            check(/30 tok\/s avg/.test(statsText), 'stats modal shows the session average speed');
            check(statsText.includes(`Ref${copiedRef}`), `stats modal carries the session ref row (got ${JSON.stringify(statsText.slice(0, 40))})`);
            check(statsText.includes('Estimated total~$0.0010*') && statsText.includes('input ~$0*'), 'mixed session stats preserve a known subtotal and mark omitted spend with the partial-estimate suffix');
            ...
            await fileBtn.click();
            await desktop.waitForFunction(() => [...document.querySelectorAll('.stats-copy')].some(b => b.classList.contains('copied')), { timeout: 2000 });
            ...
            const findingsLink = desktop.locator('.message.assistant .markdown-body code.file-link', { hasText: 'findings.md' });
            check(await findingsLink.count() === 1, 'backticked mention linkified');
            check(await desktop.locator('.message.assistant .markdown-body span.file-link', { hasText: 'README.md' }).count() === 1, 'plain-prose mention linkified');
            ...
            const notesLink = desktop.locator('.message.assistant .markdown-body a.file-link', { hasText: 'Full notes' });
            ...
            check(await notesLink.getAttribute('data-file-path') === 'deep/nest/findings.md', 'href preserved as the file mention');
            check(await desktop.locator('.message.assistant .markdown-body a:not(.file-link)', { hasText: 'the repo' }).count() === 1, 'external link untouched');
            await notesLink.click();
            await desktop.waitForSelector('#fileView .markdown-body h1', { timeout: 5000 });
            check((await desktop.locator('#fileViewPath').textContent() ?? '').includes('deep/nest/findings.md'), 'markdown link opened the viewer on the linked path');
            await desktop.keyboard.press('Escape');
            await desktop.waitForSelector('#sessionView.file-open', { state: 'detached', timeout: 3000 });
            await findingsLink.click();
            await desktop.waitForSelector('#fileView .markdown-body h1', { timeout: 5000 });
            check(await desktop.evaluate(() => fixtureElement(document.getElementById('messages'), "document.getElementById('messages')").offsetParent === null), 'transcript hidden while the file view is open');
            check(await desktop.locator('#fileViewTitle').textContent() === 'findings.md', 'viewer titled by filename');
            const shownPath = await desktop.locator('#fileViewPath').textContent();
            if (shownPath === null)
                throw new Error('Missing shownPath');
            check(shownPath.includes('deep/nest/findings.md'), `bare mention resolved to the deep tool-written path (got ${JSON.stringify(shownPath)})`);
            check(await desktop.locator('#fileView .markdown-body h1').textContent() === 'deep findings', 'markdown file renders rendered');
            const rawHref = await desktop.locator('#fileViewRaw').getAttribute('href');
            if (rawHref === null)
                throw new Error('Missing rawHref');
            check(rawHref.includes(`/api/sessions/${SESSION_ID}/file/content?path=`), `viewer exposes a raw file link (got ${JSON.stringify(rawHref)})`);
            const rawResponse = await desktop.evaluate(async () => {
                const response = await fetch(fixtureElement(document.querySelector('#fileViewRaw'), '#fileViewRaw').href);
            ...
            check(rawResponse.status === 200 && /^text\/plain/.test(rawResponse.type || '')
                && rawResponse.text.includes('# deep findings'), `raw link serves markdown source as plain text (got ${JSON.stringify(rawResponse)})`);
            // Select rendered prose and save an anchored comment. This must not send
            // a prompt or initiate an agent turn.
            await desktop.evaluate(() => {
                const paragraph = [...document.querySelectorAll('#fileViewBody p')]
                    .find((el) => el.textContent.includes('hello from deep'));
                const node = fixtureElement(fixtureElement(paragraph, 'findings paragraph').firstChild, 'findings text');
            ...
            if (bubbleBox === null)
                throw new Error('Missing bubbleBox');
            check(bubbleBox.x >= 0 && bubbleBox.y >= 0
                && bubbleBox.x + bubbleBox.width <= 1280 && bubbleBox.y + bubbleBox.height <= 800, `comment bubble is clamped within the viewport (got ${JSON.stringify(bubbleBox)})`);
            check(await desktop.evaluate(() => document.activeElement !== document.getElementById('commentBody')), 'pointer selection opens without stealing focus from selection-to-copy');
            await desktop.setViewportSize({ width: 420, height: 500 });
            await desktop.waitForFunction(() => {
                const box = fixtureElement(document.getElementById('commentBubble'), "document.getElementById('commentBubble')").getBoundingClientRect();
                return box.left >= 0 && box.top >= 0 && box.right <= innerWidth && box.bottom <= innerHeight;
            }, { timeout: 2000 });
            const narrowBubbleBox = await desktop.locator('#commentBubble').boundingBox();
            if (narrowBubbleBox === null)
                throw new Error('Missing narrowBubbleBox');
            check(narrowBubbleBox.x >= 0 && narrowBubbleBox.y >= 0
                && narrowBubbleBox.x + narrowBubbleBox.width <= 420
                && narrowBubbleBox.y + narrowBubbleBox.height <= 500, `comment bubble remains clamped after resize (got ${JSON.stringify(narrowBubbleBox)})`);
            ...
            await desktop.waitForSelector('#commentBubble', { state: 'visible', timeout: 2000 });
            check(await desktop.evaluate(() => document.activeElement === document.getElementById('commentBody')), 'keyboard selection opens and focuses the comment composer');
            check(await desktop.evaluate(() => {
                const root = document.createElement('div');
                root.textContent = 'before  selected  after';
                document.body.append(root);
                const range = document.createRange();
                const text = fixtureElement(root.firstChild, 'anchor fixture text');
            ...
            check(await desktop.locator('#commentBody').inputValue() === 'Make this finding more specific.', 'another capture attempt does not discard an open comment draft');
            ...
            check(fileComments.total === 1 && fileComments.comments[0].target.kind === 'file', 'file selection persisted as one anchored comment');
            ...
            check(await desktop.evaluate(() => fixtureElement(document.querySelector('#fileViewBody mark.comment-mark'), "document.querySelector('#fileViewBody mark.comment-mark')")
            ...
            await desktop.waitForSelector('#commentBubble', { state: 'visible', timeout: 2000 });
            check(await desktop.locator('#commentBubbleTitle').textContent() === 'Edit comment', 'clicking a mark opens the bubble in edit mode');
            check(await desktop.locator('#commentBody').inputValue() === 'Make this finding more specific.', 'the editor prefills the saved body');
            ...
            const editedComments = await desktop.evaluate(async (id) => {
                ...
                if (typeof body !== 'object' || body === null || !('comments' in body) || !Array.isArray(body.comments))
                    throw new Error('Invalid comment index');
                return body.comments.map((comment) => typeof comment === 'object' && comment !== null && 'bodyPreview' in comment ? comment.bodyPreview : null);
            }, SESSION_ID);
            check(editedComments.length === 1 && editedComments[0] === 'Name the exact function.', `editing rewrote the stored comment (got ${JSON.stringify(editedComments)})`);
            await desktop.waitForSelector('#fileViewBody mark.comment-mark', { timeout: 5000 });
            await desktop.click('#fileViewBody mark.comment-mark');
            await desktop.waitForSelector('#commentBubble', { state: 'visible', timeout: 2000 });
            await desktop.click('#commentDeleteBtn');
            check(await desktop.locator('#commentDeleteBtn').textContent() === 'Delete?', 'the first Delete tap only arms the confirm');
            await desktop.click('#commentDeleteBtn');
            await desktop.waitForFunction(() => document.querySelectorAll('#fileViewBody mark.comment-mark').length === 0
                && fixtureElement(document.getElementById('fileViewComments'), "document.getElementById('fileViewComments')").style.display === 'none', { timeout: 5000 });
            check(true, 'the second tap deletes the comment and clears its mark and chip');
            check((await (await fetch(`${base}/api/comments/index?sessionId=${SESSION_ID}`)).json()).total === 0, 'the deleted comment left the agent-facing index');
            ...
            if (!pageLink)
                throw new Error('Published page link is missing');
            ...
            check(pageRes.status === 200 && publishedHtml.includes('<h1>deep findings</h1>')
                ...
                && !publishedHtml.includes('artifact-comments.js'), 'published page uses the standalone file renderer without comments');
            // Shared-artifacts badge: the page plus the share link created earlier.
            console.log('artifacts:');
            await desktop.waitForFunction(() => document.getElementById('artifactCount')?.textContent === '2', { timeout: 5000 });
            ...
            check(artifactLabels.includes('findings.md') && artifactLabels.includes('Read-only transcript'), `artifacts modal lists the page and the share link (got ${JSON.stringify(artifactLabels)})`);
            const pageHref = await desktop.locator('#artifactsModal .artifact-link').first().getAttribute('href');
            if (!pageHref)
                throw new Error('Page artifact link is missing');
            ...
            await desktop.waitForFunction(() => fixtureElement(document.getElementById('fileViewPage'), "document.getElementById('fileViewPage')").style.display === 'none', { timeout: 5000 });
            check((await fetch(pageLink)).status === 404, 'unpublish revokes the public URL');
            await desktop.waitForFunction(() => document.getElementById('artifactCount')?.textContent === '1', { timeout: 5000 });
            check(true, 'artifacts badge drops the revoked page (share link remains)');
            await desktop.keyboard.press('Escape');
            await desktop.waitForSelector('#fileView', { state: 'hidden', timeout: 2000 });
            await desktop.waitForFunction(() => fixtureElement(document.getElementById('messages'), "document.getElementById('messages')").offsetParent !== null, { timeout: 2000 });
            ...
            check(await desktop.evaluate(() => fixtureElement(document.getElementById('messages'), "document.getElementById('messages')").offsetParent === null), 'transcript hidden while the diff view is open');
            check(await desktop.locator('.diff-repo-path').first().textContent() === 'repo-x', 'repo under the cwd discovered and titled by relative path');
            const diffFiles = await desktop.evaluate(() => [...document.querySelectorAll('.diff-file')].map((el) => ({
                status: fixtureElement(el.querySelector('.diff-status'), '.diff-status').textContent,
                path: fixtureElement(el.querySelector('.diff-file-path'), '.diff-file-path').textContent,
                open: el.open,
            })));
            check(diffFiles.some(f => f.path === 'zeta.txt' && f.status === 'M'), `modified file listed with status M (got ${JSON.stringify(diffFiles)})`);
            check(diffFiles.some(f => f.path === 'zulu.txt' && f.status === '?'), 'untracked file listed with status ?');
            check(diffFiles.every(f => f.open), 'small changeset opens patches by default');
            check(await desktop.evaluate(() => [...document.querySelectorAll('.diff-line.diff-add')].some((el) => el.textContent === '+two')), 'modified patch renders its added line');
            check(await desktop.evaluate(() => [...document.querySelectorAll('.diff-line.diff-add')].some((el) => el.textContent === '+brand new')), 'untracked patch is synthesized and rendered');
            await desktop.evaluate(() => {
                const line = [...document.querySelectorAll('.diff-line.diff-add')]
                    .find((el) => el.textContent === '+two');
                const range = document.createRange();
                range.selectNodeContents(fixtureElement(line, 'modified diff line'));
                const selection = window.getSelection();
                if (selection === null)
                    throw new Error('Missing selection');
                selection.removeAllRanges();
            ...
            await desktop.click('#commentSendBtn');
            await desktop.waitForSelector('#commentBubble', { state: 'hidden', timeout: 5000 });
            const diffComments = (0, test_types_js_1.record)(await (await fetch(`${base}/api/comments/index?sessionId=${SESSION_ID}`)).json());
            const diffComment = (0, test_types_js_1.records)(diffComments.comments)
                .find(comment => (0, test_types_js_1.record)(comment.target).kind === 'diff');
            check(diffComments.total === 1 && (0, test_types_js_1.record)((0, test_types_js_1.record)(diffComment).target).anchor !== undefined
                && (0, test_types_js_1.record)((0, test_types_js_1.record)((0, test_types_js_1.record)(diffComment).target).anchor).newStart === 2, 'diff selection persisted with its new-side line anchor');
            await desktop.waitForSelector('#diffViewBody .diff-line.comment-line', { timeout: 5000 });
            check(await desktop.evaluate(() => [...document.querySelectorAll('#diffViewBody .diff-line.comment-line')]
                .every((el) => el.textContent === '+two')), 'the diff comment tints exactly its anchored row');
            check(await desktop.locator('#diffViewComments').textContent() === '💬 1', 'diff view header chips the open comment count');
            await desktop.keyboard.press('Escape');
            await desktop.waitForFunction(() => fixtureElement(document.getElementById('messages'), "document.getElementById('messages')").offsetParent !== null, { timeout: 2000 });
            check(await desktop.locator('#btnDiff.active').count() === 0, 'Escape closes the diff view and restores the transcript');
            ...
            for (let i = 0; i < 5; i++)
                fs.writeFileSync(path.join(REPO, `lazy-${i}.txt`), `lazy line ${i}\n`);
            await desktop.click('#btnDiff');
            await desktop.waitForFunction(() => document.querySelectorAll('.diff-file').length === 7, { timeout: 5000 });
            ...
            await desktop.waitForFunction(() => [...document.querySelectorAll('.diff-line.diff-add')].some(el => el.textContent === '+lazy line 3'), { timeout: 5000 });
            check(await desktop.locator('.diff-line').count() === 2, 'expanding one file renders only its hunk and added line');
            await desktop.evaluate(() => {
                const line = [...document.querySelectorAll('.diff-line.diff-add')]
                    .find((el) => el.textContent === '+lazy line 3');
                const range = document.createRange();
                range.selectNodeContents(fixtureElement(line, 'lazy diff line'));
                const selection = window.getSelection();
                if (selection === null)
                    throw new Error('Missing selection');
                selection.removeAllRanges();
            ...
            if (userBox === null)
                throw new Error('Missing userBox');
            const feedBox = await wide.locator('.messages').boundingBox();
            if (feedBox === null)
                throw new Error('Missing feedBox');
            const leftGap = userBox.x - feedBox.x;
            const rightGap = (feedBox.x + feedBox.width) - (userBox.x + userBox.width);
            check(leftGap > 100 && Math.abs(leftGap - rightGap) < 40, `wide viewport centers the column (gaps ${Math.round(leftGap)}/${Math.round(rightGap)})`);
            await wide.close();
            ...
            await desktop.keyboard.press('Tab');
            check(await desktop.inputValue('#promptInput') === '@~/workspace/', 'accepting a dir appends a slash and keeps completing');
            ...
            check(deepVal === '@~/workspace/proj-alpha/README.md ', `drilled mention inserted (got ${JSON.stringify(deepVal)})`);
            await desktop.fill('#promptInput', '');
            ...
            check((await desktop.locator('#sessionRelations').textContent() ?? '').includes('Subagents'), 'selected session exposes the session family tree link');
            ...
            let modelRolesPatch;
            const readModelRolesPatch = () => modelRolesPatch;
            await desktop.route('**/api/harnesses/omp/model-roles', async (route) => {
                modelRolesPatch = (0, test_types_js_1.record)(route.request().postDataJSON());
                const roles = (0, test_types_js_1.record)(modelRolesPatch.roles);
                for (const [role, value] of Object.entries(roles)) {
                    if (value === null)
                        delete ompGlobalRoles[role];
                    else if (typeof value === 'string')
                        ompGlobalRoles[role] = value;
                }
                await route.fulfill({
                    status: 200,
                    contentType: 'application/json',
            ...
            let agentsPatch;
            const readAgentsPatch = () => agentsPatch;
            await desktop.route(/\/api\/harnesses\/omp\/agents(?:\?|$)/, async (route) => {
                if (route.request().method() === 'PUT') {
                    agentsPatch = (0, test_types_js_1.record)(route.request().postDataJSON());
                    const agents = (0, test_types_js_1.record)(agentsPatch.agents);
                    for (const [name, value] of Object.entries(agents)) {
                        const settings = (0, test_types_js_1.record)(value);
                        if (settings.disabled === true)
                            ompAgentSettings.disabled.push(name);
                        if (settings.disabled === false) {
                            ompAgentSettings.disabled = ompAgentSettings.disabled.filter(entry => entry !== name);
                        }
                        if (settings.model === null)
                            delete ompAgentSettings.modelOverrides[name];
                        else if (typeof settings.model === 'string' && settings.model)
                            ompAgentSettings.modelOverrides[name] = settings.model;
                        for (const key of ['prewalk', 'advisor']) {
                            if (settings[key] === null)
                                delete ompAgentSettings[key][name];
                            else if (typeof settings[key] === 'boolean')
                                ompAgentSettings[key][name] = settings[key];
                        }
                    }
                }
            ...
            check(await desktop.locator('#harnessAgentsBody .hs-agent-row').count() === 3, 'every discovered task agent gets a row');
            check(await desktop.isChecked('.hs-agent-enabled[data-agent="scout"]') &&
                !(await desktop.isChecked('.hs-agent-enabled[data-agent="sonic"]')), 'the enable toggle initializes from the global disabled list');
            check((await desktop.locator('.hs-agent-row[data-agent="scout"]').textContent() ?? '').includes('project override'), 'an agent the project disables here is flagged as a project override');
            check(await desktop.inputValue('.hs-agent-model[data-agent="scout"]') === 'zai/glm-5.2' &&
                ...
                await desktop.inputValue('.hs-agent-prewalk[data-agent="reviewer"]') === '', 'per-agent model, advisor and prewalk controls initialize from the global records');
            ...
            check(await desktop.locator('.model-role-row').count() >= 10, 'every canonical OMP role gets a row');
            check(await desktop.inputValue('.model-role-select[data-role="smol"]') === 'zai/glm-5.2', 'role selects initialize from the global record');
            check(await desktop.inputValue('.model-role-select[data-role="vision"]') === '', 'a project-only assignment leaves the global select unset');
            check((await desktop.locator('.model-role-row[data-role="vision"]').textContent() ?? '').includes('project override'), 'a differing effective value is flagged as a project override');
            ...
            check(await desktop.inputValue('.model-role-select[data-role="advisor"]') === 'zai/glm-4.7-flash', 'a level-pinned role parses its model out of the stored ref');
            check(await desktop.inputValue('.model-role-level[data-role="advisor"]') === 'max' &&
                (await desktop.locator('.model-role-level[data-role="advisor"] option[value="max"]').textContent()) === '(current) max', 'an off-ladder stored level stays selectable as (current)');
            check(await desktop.inputValue('.model-role-level[data-role="smol"]') === '', 'a bare role ref initializes the level select to inherit');
            await desktop.keyboard.press('Escape');
            check(await desktop.evaluate(() => fixtureElement(document.getElementById('harnessSettingsModal'), "document.getElementById('harnessSettingsModal')").style.display === 'none') &&
                await desktop.evaluate(() => fixtureElement(document.querySelector('.main'), "document.querySelector('.main')").classList.contains('new-session-open')), 'Escape closes the settings modal only, not the takeover underneath');
            ...
            const planLevels = await desktop.locator('.model-role-level[data-role="plan"] option')
                ...
                if (!(option instanceof HTMLOptionElement))
                    throw new Error('Expected model role option');
                return option.value;
            }));
            check(JSON.stringify(planLevels) === JSON.stringify(['', 'off', 'auto', 'high', 'max']), `switching the role's model rebuilds the level ladder (got ${JSON.stringify(planLevels)})`);
            ...
            await desktop.waitForFunction(() => fixtureElement(document.getElementById('harnessSettingsModal'), "document.getElementById('harnessSettingsModal')").style.display === 'none');
            check(JSON.stringify((0, test_types_js_1.record)(readModelRolesPatch()).roles) === JSON.stringify({
                ...
                `(got ${JSON.stringify((0, test_types_js_1.record)(readModelRolesPatch()).roles)})`);
            check(JSON.stringify((0, test_types_js_1.record)(readAgentsPatch()).agents) === JSON.stringify({
            ...
            await desktop.waitForFunction(() => [...document.querySelectorAll('#nsTree .ns-tree-name')].some(el => el.textContent === 'workspace'), null, { timeout: 5000 });
            ...
            await desktop.waitForFunction(() => [...document.querySelectorAll('#nsTree .ns-tree-name')].some(el => el.textContent === 'proj-alpha'), null, { timeout: 5000 });
            ...
            let asyncSpawnBody;
            const readAsyncSpawnBody = () => asyncSpawnBody;
            await desktop.route('**/api/sessions/new', async (route) => {
                asyncSpawnBody = (0, test_types_js_1.record)(route.request().postDataJSON());
            ...
            const submittedSpawn = (0, test_types_js_1.record)(readAsyncSpawnBody());
            check(submittedSpawn.async === true, 'takeover spawn opts into asynchronous spawning');
            check(submittedSpawn.name === 'UI named session', 'POST body carries the chosen session name');
            check(submittedSpawn.cwd === CWD, `POST body carries the chosen cwd (got ${JSON.stringify(submittedSpawn.cwd)})`);
            check(submittedSpawn.harness === 'omp', 'POST body routes the spawn through the selected alternative harness');
            check(submittedSpawn.model === 'zai/glm-4.7-flash', 'POST body carries the chosen OMP model');
            check(submittedSpawn.thinking === 'minimal', 'POST body carries a thinking level valid for that model');
            check(!(await desktop.evaluate(() => fixtureElement(document.querySelector('.main'), "document.querySelector('.main')").classList.contains('new-session-open'))), 'takeover closes in favor of the provisional pane');
            check(await desktop.locator('.session-item.starting').textContent().then((text) => !!text?.includes('Starting Oh My Pi')), 'the provisional row identifies the selected harness before registration');
            check(await desktop.locator('.session-item.starting').evaluate((el) => el.classList.contains('active')) &&
                await desktop.locator('#sessionName').textContent() === 'Starting session…', 'provisional session pane opens immediately');
            check(await desktop.locator('#sessionRelations .session-relation-chip').count() === 0 &&
                !(await desktop.locator('#sessionRelations').isVisible()), 'provisional pane clears the previous session relation chips');
            ...
            check(await desktop.inputValue('#promptInput') === startupDraft &&
                (await desktop.locator('#status').textContent() ?? '').includes('still starting'), 'typing and Enter preserve the prompt while Pi starts');
            ...
            check(await desktop.evaluate((key) => localStorage.getItem(fixtureApp.features.composerDrafts.draftKey(key)), provisionalKey) === startupDraft, 'provisional composer owns its draft before registration');
            spawnResult = 'ready';
            ...
            await desktop.waitForFunction(({ id, draft }) => fixtureApp.features.sessionState.currentSession?.id === id &&
                fixtureInput(document.getElementById('promptInput'), "document.getElementById('promptInput')").value === draft, { id: SESSION_ID, draft: startupDraft }, { timeout: 3000 });
            ...
            check(migratedDraft.provisional === null && migratedDraft.session === startupDraft &&
                !(await desktop.locator('#btnSend').isDisabled()), 'draft transfers to the registered session and Send becomes ready');
            await desktop.fill('#promptInput', '');
            ...
            spawnResult = 'error';
            await desktop.waitForFunction(() => fixtureElement(document.getElementById('sessionName'), "document.getElementById('sessionName')").textContent === 'Session failed to start', null, { timeout: 3000 });
            check(await desktop.inputValue('#promptInput') === failedDraft &&
                await desktop.locator('#btnSend').isDisabled(), 'failed spawn keeps its provisional draft accessible');
            ...
            await desktop.waitForFunction(() => fixtureElement(document.getElementById('sessionName'), "document.getElementById('sessionName')").textContent === 'renamed live', null, { timeout: 5000 });
            check(true, 'header shows new name');
            await desktop.waitForFunction(() => document.querySelector('.session-item-name')?.textContent === 'renamed live', null, { timeout: 5000 });
            ...
            const promptImages = (0, test_types_js_1.records)((0, test_types_js_1.record)(lastPrompt).images);
            const promptImage = (0, test_types_js_1.record)(promptImages[0]);
            check(promptImages.length === 1 && promptImage.data === PNG_B64 && promptImage.mimeType === 'image/png', 'bridge received the image payload intact');
            // Wait for the turn to finish and the JSONL catch-up to land: the
            // authoritative user message must still show the image.
            await desktop.waitForFunction(() => [...document.querySelectorAll('.message.user[data-msg-index]')]
                .some((el) => el.textContent.includes('describe the screenshot') && el.querySelector('img.msg-image')), { timeout: 8000 });
            ...
            check(await dictation.locator('#btnMic').getAttribute('aria-disabled') === null, 'mic is enabled on a configured host over a secure origin (127.0.0.1)');
            ...
            await dictation.click('#btnMic');
            await dictation.waitForFunction(() => fixtureInput(document.getElementById('promptInput'), "document.getElementById('promptInput')").value.includes('transcribed hello'), { timeout: 20000 });
            check(true, 'the transcript lands in the composer');
            check((await dictation.inputValue('#promptInput')).startsWith('note: transcribed hello'), 'it is inserted at the caret, keeping what was already typed');
            const sttRequest = (0, test_types_js_1.record)(sttRequests[0]);
            check(sttRequests.length === 1 && typeof sttRequest.filename === 'string' && /^audio\.[a-z0-9]+$/.test(sttRequest.filename) &&
                sttRequest.model === 'whisper-1', `the endpoint saw one named audio upload with the configured model (got ${JSON.stringify(sttRequests)})`);
            ...
            check(await dictation.locator('#composerNote').isVisible() === false, 'a successful take leaves no composer note behind');
            ...
            await dictation.click('.session-item');
            await dictation.waitForSelector('.message.assistant');
            await dictation.waitForFunction(() => document.getElementById('btnMic')?.style.display === 'none', { timeout: 5000 });
            check(await dictation.locator('#btnMic').isVisible() === false, 'the mic is hidden once no host advertises stt');
            ...
            check(await desktop.locator('.queue-item-text').first().textContent() === 'steer me now', 'steered message appears as a strip row');
            ...
            await desktop.waitForFunction(() => fixtureInput(document.getElementById('promptInput'), "document.getElementById('promptInput')").value.includes('steer me now'), { timeout: 5000 });
            check(true, 'Edit returns the queued text to the composer');
            await desktop.waitForFunction(() => fixtureElement(document.getElementById('queuePanel'), "document.getElementById('queuePanel')").style.display === 'none', { timeout: 5000 });
            ...
            await desktop.waitForFunction((t) => [...document.querySelectorAll('.message.user')].some((el) => el.textContent.includes(t)), deliveredText, { timeout: 5000 });
            ...
            await desktop.waitForFunction(() => [...document.querySelectorAll('.queue-item-text')].some((el) => el.textContent === 'replay me later'), { timeout: 5000 });
            ...
            await desktop.waitForFunction(() => document.getElementById('status')?.textContent === 'Stopping...', { timeout: 3000 });
            await desktop.waitForTimeout(100); // HTTP acknowledgement has landed
            check(await desktop.evaluate(() => fixtureApp.features.sessionActivity.turn), 'abort HTTP acknowledgement does not clear turn state');
            await desktop.fill('#promptInput', 'must wait for abort boundary');
            await desktop.press('#promptInput', 'Enter');
            check(await desktop.inputValue('#promptInput') === 'must wait for abort boundary', 'send during abort remains in the composer');
            ...
            check(await desktop.locator('details.live-tool-panel').count() === 0, 'agent_end catch-up removes the aborted turn live tool panel');
            await desktop.fill('#promptInput', '');
            ...
            await desktop.waitForFunction(() => /Working \d+:\d\d · Bash/.test(document.querySelector('#sessionWorking .spinner-text')?.textContent || ''), { timeout: 3000 });
            check(true, 'working badge shows elapsed timer and running tool');
            await desktop.waitForFunction(() => /Working \d+:0[1-9]/.test(document.querySelector('#sessionWorking .spinner-text')?.textContent || ''), { timeout: 4000 });
            ...
            await desktop.waitForFunction(() => document.getElementById('status')?.textContent === 'Compacted (was 152.3k tokens)', { timeout: 3000 });
            ...
            check(!(await desktop.evaluate((id) => fixtureApp.features.sessionActivity.isAborting(fixtureApp.ports.composerDrafts.keyForSession(id)), SESSION_ID)), 'compaction_end clears a compaction-only abort gate even on failure');
            ...
            await desktop.click('#btnSend');
            await desktop.waitForFunction(() => document.getElementById('status')?.textContent === 'Compaction already in progress', { timeout: 3000 });
            check(true, 'second /compact is refused while compacting');
            check(await desktop.evaluate(() => fixtureInput(document.getElementById('promptInput'), "document.getElementById('promptInput')").value) === '/compact', 'refused /compact keeps the composer text');
            await desktop.fill('#promptInput', '');
            const repliesBefore = await desktop.evaluate(() => [...document.querySelectorAll('.message.assistant')].filter((m) => /Streamed reply/.test(m.textContent)).length);
            await desktop.fill('#promptInput', '');
            await desktop.type('#promptInput', 'send after compaction');
            await desktop.click('#btnSend');
            await desktop.waitForFunction(() => /Queued — will send when compaction finishes/.test(document.getElementById('status')?.textContent || ''), { timeout: 3000 });
            ...
            flushBufferedPrompt();
            await desktop.waitForFunction((n) => [...document.querySelectorAll('.message.assistant')].filter((m) => /Streamed reply/.test(m.textContent)).length > n, repliesBefore, { timeout: 6000 });
            check(true, 'queued prompt is delivered as a turn after compaction ends');
            await desktop.waitForFunction(() => document.querySelector('#sessionWorking .spinner-text')?.textContent === 'Working', { timeout: 6000 });
            ...
            emit('turn_start', {});
            await desktop.waitForFunction(() => /^Working/.test(document.querySelector('#sessionWorking .spinner-text')?.textContent || ''), { timeout: 3000 });
            ...
            await desktop.waitForFunction(() => document.getElementById('status')?.textContent === 'Compacted: 152.3k → ~30.5k tokens', { timeout: 3000 });
            ...
            await desktop.waitForFunction(() => [...document.querySelectorAll('.message.user')].some((el) => el.textContent.includes('pre-branch marker')), { timeout: 5000 });
            ...
            await desktop.waitForFunction(() => [...document.querySelectorAll('.ext-ui-status-badge')].some((el) => el.textContent === 'stream reconnected'), { timeout: 5000 });
            ...
            check(await desktop.evaluate(() => document.fonts.check('12px "Symbols Nerd Font Mono"')), 'Nerd Font symbols fallback loaded (p10k prompt glyphs)');
            const termText = () => desktop.evaluate(() => {
                const state = fixtureApp.features.terminalController.state;
                if (!state)
            ...
            await desktop.waitForFunction(() => fixtureElement(document.getElementById('terminalStatus'), "document.getElementById('terminalStatus')").textContent === '', { timeout: 5000 }); // attach frame landed
            ...
            if (cwdLabel === null)
                throw new Error('Missing cwdLabel');
            ...
            check(await desktop.evaluate(() => fixtureElement(document.getElementById('terminalPanel'), "document.getElementById('terminalPanel')").style.display === 'none'), 'panel hidden on close');
            await desktop.click('#btnTerminal');
            await desktop.waitForFunction(() => {
                const rows = document.querySelector('#terminalPanel .xterm');
                return rows && rows.textContent.includes('term-smoke-42');
            ...
            const heightBefore = await desktop.evaluate(() => fixtureElement(document.getElementById('terminalPanel'), "document.getElementById('terminalPanel')").offsetHeight);
            const termHandleBox = await desktop.locator('#terminalResizeHandle').boundingBox();
            if (termHandleBox === null)
                throw new Error('Missing termHandleBox');
            ...
            const heightAfter = await desktop.evaluate(() => fixtureElement(document.getElementById('terminalPanel'), "document.getElementById('terminalPanel')").offsetHeight);
            check(heightAfter > heightBefore + 80, `drag handle grows the panel (${heightBefore}px -> ${heightAfter}px)`);
            check(await desktop.evaluate(() => localStorage.getItem('pi-dish-terminal-size') !== null), 'resized height persists to localStorage');
            ...
            if (await desktop.evaluate(() => fixtureApp.ports.routinesController.config().tmux)) {
                check((await desktop.locator('#termModeBtn').textContent() ?? '').includes('pi tmux'), 'mode button offers the tmux pane view');
                await desktop.click('#termModeBtn');
                await desktop.waitForFunction(() => /tmux pane/i.test(fixtureElement(document.getElementById('terminalStatus'), "document.getElementById('terminalStatus')").textContent), { timeout: 5000 });
                check(true, 'pane-less session surfaces the no-tmux-pane error');
                await desktop.click('#termModeBtn'); // back to shell
                await desktop.waitForFunction(() => fixtureElement(document.getElementById('terminalStatus'), "document.getElementById('terminalStatus')").textContent === '', { timeout: 5000 });
            ...
            await desktop.waitForFunction(() => [...document.querySelectorAll('.ext-ui-widget')].some((el) => el.dataset.widgetKey === 'todo'), { timeout: 3000 });
            await desktop.evaluate(() => { fixtureElement(document.querySelector('[data-widget-key="todo"]'), '[data-widget-key="todo"]').dataset.marker = 'kept'; });
            ...
            check(await desktop.evaluate(() => {
                ...
                return els.length === 1 && els[0]?.dataset.marker === 'kept'
                    ...
                    && fixtureElement(els[0].querySelector('.ext-ui-widget-body'), '.ext-ui-widget-body').textContent === 'task one\ntask two';
            ...
            check(await desktop.evaluate(() => document.querySelector('[data-widget-key="todo"]')?.classList.contains('hidden') === true), 'widget fades once the grace elapses without a re-set');
            emit('extension_ui_request', { method: 'setWidget', widgetKey: 'todo', widgetLines: ['task one', 'task two'] });
            await desktop.waitForTimeout(50);
            check(await desktop.evaluate(() => {
                const els = [...document.querySelectorAll('[data-widget-key="todo"]')];
            ...
            await desktop.evaluate(() => { fixtureElement(document.querySelector('.ext-ui-status-badge'), '.ext-ui-status-badge').dataset.marker = 'kept'; });
            ...
            await desktop.waitForFunction(() => [...document.querySelectorAll('.ext-ui-status-badge')].some((el) => el.textContent === 'temporary'), { timeout: 3000 });
            ...
            check(await desktop.evaluate(() => getComputedStyle(fixtureElement(document.getElementById('extUiStatusToggle'), '#extUiStatusToggle')).display === 'none'), 'status toggle hidden while nothing is clipped');
            emit('extension_ui_request', { method: 'setStatus', statusKey: 'goal', statusText: `Goal · ${'a very long objective line '.repeat(12)}` });
            await desktop.waitForFunction(() => getComputedStyle(fixtureElement(document.getElementById('extUiStatusToggle'), '#extUiStatusToggle')).display !== 'none', { timeout: 3000 });
            check(true, 'status toggle appears once the collapsed strip clips');
            await desktop.click('#extUiStatusToggle');
            check(await desktop.evaluate(() => !fixtureElement(document.getElementById('extUiStatuses'), "document.getElementById('extUiStatuses')").classList.contains('collapsed')), 'status toggle expands the clipped strip');
            await desktop.click('#extUiStatusToggle');
            check(await desktop.evaluate(() => fixtureElement(document.getElementById('extUiStatuses'), "document.getElementById('extUiStatuses')").classList.contains('collapsed')), 'status toggle collapses the strip again');
            emit('extension_ui_request', { method: 'setStatus', statusKey: 'goal' });
            await desktop.waitForFunction(() => document.querySelectorAll('.ext-ui-status-badge').length === 1, { timeout: 3000 });
            bridge2.listen(socket2Path);
            await (0, node_events_1.once)(bridge2, 'listening');
            registerSession2();
            ...
            await desktop.waitForFunction(() => [...document.querySelectorAll('.ext-ui-widget-body')].some((el) => el.textContent === 'deploy #7 running'), { timeout: 5000 });
            ...
            await desktop.waitForFunction(() => [...document.querySelectorAll('.ext-ui-widget-body')].some((el) => el.textContent === 'proc one\nproc two'), { timeout: 5000 });
            check(await desktop.locator('.ext-ui-widget').count() === 1, 'switch-back replays only session 1 widget');
            check(await desktop.locator('.ext-ui-status-badge').textContent() === '2 running', 'status badge replayed on switch-back');
            ...
            check(await desktop.evaluate(() => !fixtureElement(document.querySelector('.input-area'), "document.querySelector('.input-area')").classList.contains('ext-dialog-takeover') &&
                getComputedStyle(fixtureElement(document.getElementById('promptInput'), '#promptInput')).display !== 'none'), 'composer stays usable while the dialog is expanded');
            ...
            check(await desktop.locator('#extUiDialogs .ext-ui-dialog-modal').count() === 1, 're-emitted ask payload does not stack a duplicate card');
            ...
            check(await desktop.evaluate(() => fixtureElement(document.querySelector('#extUiDialogs .ext-ui-dialog-modal'), "document.querySelector('#extUiDialogs .ext-ui-dialog-modal')").classList.contains('minimized')), 'minimizing backgrounds the dialog to a slim bar');
            ...
            await desktop.click(`.session-item[data-id="${registryState.sessionId}"]`);
            await desktop.waitForSelector('#extUiDialogs .ext-ui-dialog-modal', { timeout: 5000 });
            check(await desktop.evaluate(() => fixtureElement(document.querySelector('#extUiDialogs .ext-ui-dialog-modal'), "document.querySelector('#extUiDialogs .ext-ui-dialog-modal')").classList.contains('minimized')), 'dialog re-docked still backgrounded after switch-back');
            check(await desktop.locator('.ext-ui-ask-option.selected').count() === 1, 'selection survives the session switch');
            // Expand via the bar and submit: the answer posts to the session.
            await desktop.click('#extUiDialogs .ext-ui-dialog-modal');
            await desktop.waitForFunction(() => !fixtureElement(document.querySelector('#extUiDialogs .ext-ui-dialog-modal'), "document.querySelector('#extUiDialogs .ext-ui-dialog-modal')").classList.contains('minimized'), { timeout: 5000 });
            await desktop.click('[data-action="submit-ask"]');
            await desktop.waitForFunction(() => !document.getElementById('extUiDialogs'), { timeout: 5000 });
            const uiResponse = (0, test_types_js_1.record)(lastUIResponse);
            const uiValue = (0, test_types_js_1.record)(uiResponse.value);
            const uiResults = (0, test_types_js_1.records)(uiValue.results);
            const firstUiResult = (0, test_types_js_1.record)(uiResults[0]);
            const selectedOptions = firstUiResult.selectedOptions;
            check(uiResponse.requestId === 'dlg-ask-1' && uiValue.kind === 'submit'
                && Array.isArray(selectedOptions) && selectedOptions[0] === 'Beta', `submit posts the picked option to the bridge (got ${JSON.stringify(lastUIResponse)})`);
            check(await desktop.evaluate(() => getComputedStyle(fixtureElement(document.getElementById('promptInput'), '#promptInput')).display !== 'none'), 'composer usable after submit');
            ...
            await desktop.waitForFunction((b) => fixtureApp.features.sessionState.currentSession?.id === b &&
                [...document.querySelectorAll('#messages .message')].some((el) => el.textContent.includes('second session')), SESSION2_ID, { timeout: 5000 });
            ...
            const rapidSelection = await desktop.evaluate((b) => ({
                ...
                text: fixtureElement(document.getElementById('messages'), "document.getElementById('messages')").textContent,
            ...
            check(rapidSelection.currentId === SESSION2_ID && rapidSelection.streamUrl.includes(`/${SESSION2_ID}/stream`) &&
                rapidSelection.text.includes('second session') && !rapidSelection.text.includes('existing answer'), 'stale A selection cannot replace B stream or transcript');
            ...
            check(!(await desktop.locator('#messages').textContent() ?? '').includes('STALE CATCH-UP MARKER'), 'same-session force reload invalidates the older catch-up generation');
            ...
            const statsOwner = await desktop.evaluate(() => ({
                ...
                text: fixtureElement(document.getElementById('statsBody'), "document.getElementById('statsBody')").textContent,
                visible: fixtureElement(document.getElementById('statsModal'), "document.getElementById('statsModal')").style.display !== 'none',
            }));
            check(statsOwner.visible && statsOwner.sessionId === SESSION2_ID && !statsOwner.text.includes('STALE-A-STATS'), 'stale stats response cannot combine session A data with session B actions');
            ...
            check(await desktop.locator('#fileViewTitle').textContent() === 'findings.md' &&
                !(await desktop.locator('#fileViewBody').textContent() ?? '').includes('STALE FILE'), 'stale file response cannot overwrite the latest open file');
            ...
            check(!(await desktop.locator('#diffViewRoot').textContent() ?? '').includes('STALE-DIFF-ROOT') &&
                await desktop.locator('.diff-repo').count() > 0, 'stale diff response cannot overwrite the latest diff view');
            ...
            await desktop.waitForFunction(() => document.querySelectorAll('.diff-file').length === 7, { timeout: 5000 });
            await desktop.locator('.diff-file').filter({ hasText: 'lazy-4.txt' }).locator('summary').click();
            await desktop.waitForFunction(() => [...document.querySelectorAll('.diff-line.diff-add')].some((el) => el.textContent === '+lazy line 4'), { timeout: 5000 });
            await desktop.evaluate(() => window.__releaseAuditPatch());
            await desktop.waitForTimeout(50);
            check(!(await desktop.locator('#diffViewBody').textContent() ?? '').includes('STALE PATCH'), 'stale deferred patch cannot mutate a reopened diff view');
            await desktop.evaluate(() => { fixtureApp.features.fileViews.closeDiff(); window.fetch = window.__auditRealFetch; });
            ...
            await desktop.evaluate(({ a, image }) => {
                const composerKey = fixtureApp.features.composerDrafts.key;
                if (!composerKey)
                    throw new Error('Composer key is unavailable');
                fixtureApp.features.composerDrafts.images.replace(composerKey, [{ data: image, mimeType: 'image/png' }]);
            ...
            const failedOwnership = await desktop.evaluate((a) => ({
                currentText: fixtureInput(document.getElementById('promptInput'), "document.getElementById('promptInput')").value,
                ...
                pendingMatch: window.__auditFailedPromptId ? fixtureApp.features.promptDelivery.has(window.__auditFailedPromptId) : false,
            }), SESSION_ID);
            check(!failedOwnership.currentText.includes('failed prompt from A') &&
                ...
                !failedOwnership.pendingMatch, 'failed prompt restores text/images only to A and removes its optimistic association');
            await desktop.evaluate((a) => fixtureApp.features.sessionView.select(a, { forceTranscriptReload: true }), SESSION_ID);
            check(await desktop.inputValue('#promptInput') === 'failed prompt from A' &&
                await desktop.locator('#attachmentStrip .attachment-thumb').count() === 1, 'originating session restores the failed payload when revisited');
            await desktop.evaluate((a) => {
                fixtureInput(document.getElementById('promptInput'), "document.getElementById('promptInput')").value = '';
            ...
            await desktop.evaluate((a) => {
                ...
                const edit = fixtureElement(row.querySelector('.queue-item-edit'), '.queue-item-edit');
                window.__auditQueueEdit = fixtureApp.features.promptDelivery.edit(edit);
            ...
            check(await desktop.evaluate(() => window.__auditQueueAssociation) === 'audit-buffered-first', 'buffered queue row carries its stable optimistic prompt id');
            check(await desktop.evaluate(() => window.__auditRemainingQueueAssociation) === 'audit-buffered-second', 'queue update before cancel acknowledgement preserves duplicate prompt association');
            await desktop.evaluate((b) => fixtureApp.features.sessionView.select(b, { forceTranscriptReload: true }), SESSION2_ID);
            await desktop.evaluate(async () => {
            ...
            check(!queueOwnership.currentText.includes('duplicate buffered prompt') &&
                queueOwnership.originDraft === 'duplicate buffered prompt', 'queue edit completion restores A without touching B composer/draft');
            check(!queueOwnership.firstPending && queueOwnership.secondPending &&
                queueOwnership.firstRemoved && queueOwnership.secondRetained, 'queue edit removes only the associated bubble and echo suppression');
            await desktop.evaluate((a) => {
            ...
            await desktop.evaluate(async () => {
                ...
                fixtureApp.features.apiTransport.request = async (host, url, options) => {
                    if (url === '/api/models/enabled') {
                        if (typeof options?.body !== 'string')
                            throw new Error('Enabled-model fixture expected a JSON body');
                        const body = JSON.parse(options.body);
                        if (!body || typeof body !== 'object' || Array.isArray(body)) {
                            throw new Error('Enabled-model fixture expected an object body');
                        }
                        window.__auditEnabledBody = body;
                        return new Response(JSON.stringify({ success: true, enabledModels: window.__auditEnabledBody.enabledIds }));
                    }
            ...
            check(JSON.stringify(await desktop.evaluate(() => window.__auditEnabledBody?.enabledIds)) ===
                JSON.stringify(['audit/kept']), 'enabled-model debounce persists the edit-time ID snapshot');
            ...
            await desktop.evaluate(() => {
                ...
                if (!releaseFonts)
                    throw new Error('Font readiness resolver was not initialized');
            ...
            check(await desktop.evaluate(() => !fixtureApp.features.terminalController.state && window.__auditTerminalUrls.length === 0 &&
                fixtureElement(document.getElementById('terminalPanel'), "document.getElementById('terminalPanel')").style.display === 'none'), 'terminal does not open or connect while A font readiness is held');
            await desktop.evaluate((b) => fixtureApp.features.sessionView.select(b, { forceTranscriptReload: true }), SESSION2_ID);
            await desktop.evaluate(() => {
            ...
            check(terminalOwnership.currentId === SESSION2_ID && terminalOwnership.owner === SESSION2_ID &&
                terminalOwnership.urls.length === 1 && !terminalOwnership.hasAUrl && terminalOwnership.hasBUrl, `stale A terminal open cannot connect for or interfere with B (got ${JSON.stringify(terminalOwnership)})`);
            await desktop.evaluate(() => {
            ...
            await desktop.waitForSelector(`.session-item[data-id="${registryState.sessionId}"] .session-item-tags`, { timeout: 5000 });
            {
                ...
                const rows = await desktop.locator(rowSel).evaluate((el) => [...el.querySelectorAll(':scope > *')].map((child) => child.className));
                ...
                check(await desktop.locator(`${rowSel} .session-item-meta`).textContent()
                    .then((text) => !/msgs/.test(text || '')), 'the message count is gone from the card');
                check(await desktop.locator(`${rowSel} .session-item-tags .harness-badge`).count() === 1, 'row 3 carries the harness badge');
                check(await desktop.locator(`${rowSel} .session-item-header .harness-badge`).count() === 0, 'the title row is left to the title');
                ...
                await desktop.evaluate(() => {
                    ...
                    const select = fixtureInput(document.getElementById('sidebarContextMetric'), '#sidebarContextMetric');
                    select.value = 'tokens';
            ...
            check(await desktop.locator('.sidebar-header select').count() === 0, 'the theme picker has left the sidebar header');
            check(await desktop.locator('.sidebar-filter .filter-search-btn').count() === 1, 'the all-sessions search lives in the filter row');
            ...
            check(await desktop.locator('#btnTree').count() === 0, 'the tree button is gone from the session header');
            {
                ...
                check(after === before + 120 && String(after) === stored, `dragging the edge widens the sidebar and persists it (got ${before}→${after}, stored ${stored})`);
                ...
                check(await desktop.locator('#sidebar').evaluate((el) => el.offsetWidth) === before &&
                    await desktop.evaluate(() => localStorage.getItem('pi-dish-sidebar-width')) === null, 'double-clicking the handle restores the default width');
            }
            ...
            check(await desktop.evaluate(() => fixtureElement(document.querySelector('.sidebar-filter'), "document.querySelector('.sidebar-filter')").classList.contains('searching')), 'search shows busy indicator from the first keystroke');
            ...
            check((await snippet.locator('mark').count()) >= 2, 'content match shows a snippet with the tokens highlighted');
            await desktop.waitForFunction(() => !fixtureElement(document.querySelector('.sidebar-filter'), "document.querySelector('.sidebar-filter')").classList.contains('searching'), null, { timeout: 5000 });
            check(true, 'busy indicator clears once results land');
            check(await desktop.evaluate(() => document.querySelectorAll('#sessionList .session-item').length) === 1, 'non-matching sessions filtered out');
            ...
            await desktop.waitForFunction((ids) => JSON.stringify([...document.querySelectorAll('#sessionList .session-item')].map((el) => el.dataset.id)) === ids, JSON.stringify([RANK_ID, BETA_ID]), { timeout: 5000 });
            check(true, 'search results rank the name match above the newer content match');
            check(await desktop.locator('#sessionList .workspace-group-header').count() === 0, 'ranked search list drops the grouping headers');
            check(await desktop.locator('#sessionList .session-item .session-item-cwd').count() === 2, 'ranked rows keep their workspace hint');
            await desktop.fill('#filterInput', '');
            await desktop.waitForFunction(() => document.querySelectorAll('#sessionList .session-item').length >= 3, null, { timeout: 5000 });
            check(await desktop.locator('.session-item-snippet').count() === 0, 'clearing the query drops snippets and restores the list');
            check(await desktop.locator('#sessionList .workspace-group-header').count() > 0, 'clearing the query restores the grouped view');
            ...
            await desktop.click('#tabActive');
            await desktop.waitForSelector(`.session-item[data-id="${registryState.sessionId}"] .session-item-snippet`, { timeout: 5000 });
            check(await desktop.evaluate(() => fixtureInput(document.getElementById('filterInput'), "document.getElementById('filterInput')").value) === 'existing', 'query survives tab switches and keeps matching on both tabs');
            await desktop.fill('#filterInput', '');
            await desktop.click('#tabAll');
            await desktop.waitForFunction(() => document.querySelectorAll('#sessionList .session-item').length >= 3, null, { timeout: 5000 });
            ...
            await desktop.waitForFunction((betaId) => document.querySelectorAll('#sessionList .session-item').length >= 2 &&
            ...
            check(await desktop.evaluate(() => document.querySelector('.scope-chip.active')?.textContent) === 'No beta', 'saved scope renders as an active chip');
            check(await desktop.evaluate(() => fixtureInput(document.getElementById('filterInput'), "document.getElementById('filterInput')").value) === '', 'saving a scope clears the typed query it absorbed');
            ...
            await desktop.waitForFunction((betaId) => !document.querySelector(`.session-item[data-id="${betaId}"]`) &&
                document.querySelector('.scope-hidden-note')?.textContent === '2 hidden by scopes', BETA_ID, { timeout: 5000 });
            ...
            check(await desktop.evaluate(() => localStorage.getItem('pi-dish-active-scopes')) === JSON.stringify(['No beta']), 'active scope persisted to localStorage');
            const serverFilters = await fetch(`${base}/api/settings`).then(r => r.json());
            check(JSON.stringify(serverFilters.savedFilters) === JSON.stringify([{ name: 'No beta', query: '-beta' }]), 'scope definition persisted server-side');
            ...
            await desktop.waitForFunction(() => [...document.querySelectorAll('.workspace-group-label')].some(el => el.textContent === 'Today'), null, { timeout: 5000 });
            check(await desktop.evaluate(() => localStorage.getItem('pi-dish-sidebar-view')) === 'recent', 'view choice persisted');
            check(await desktop.evaluate(() => document.querySelectorAll('#sessionList .session-item').length ===
                document.querySelectorAll('#sessionList .session-item .session-item-cwd').length), 'recent-view rows all show their workspace');
            check(await desktop.locator('.workspace-new-btn').count() === 0, 'date headers carry no per-workspace + button');
            ...
            check(await desktop.locator('.session-segment.collapsed .session-item').count() === 0, 'collapsed date bucket hides its sessions');
            await desktop.click('.workspace-group-header[data-cwd="date:today"]');
            ...
            await desktop.evaluate(() => fixtureApp.ports.appBindings.actions.closeSettingsModal(new Event('click'), document.body));
            ...
            await desktop.click('#viewToggle');
            await desktop.waitForFunction(() => [...document.querySelectorAll('.workspace-group-label')].some(el => el.textContent === 'proj-alpha'), null, { timeout: 5000 });
            ...
            await desktop.waitForFunction(() => document.querySelectorAll('#messages [data-msg-index]').length === 50, null, { timeout: 5000 });
            await desktop.evaluate(() => { fixtureElement(document.getElementById('messages'), "document.getElementById('messages')").scrollTop = 0; });
            await desktop.waitForFunction(() => document.querySelectorAll('#messages [data-msg-index]').length >= 100, null, { timeout: 5000 });
            check(true, 'scrolling to the top implicitly loads the previous transcript page');
            await desktop.evaluate(() => { fixtureElement(document.getElementById('messages'), "document.getElementById('messages')").scrollTop = 0; });
            await desktop.waitForFunction(() => document.querySelector('#messages [data-msg-index="0"]'), null, { timeout: 5000 });
            ...
            check(await desktop.locator('#messages [data-msg-index]').count() === loadedBetaCount, 'returning to the latest message keeps intentionally loaded history');
            await desktop.click(`.session-item[data-id="${registryState.sessionId}"]`);
            await desktop.waitForSelector('#messages .message.assistant');
            await desktop.click(`.session-item[data-id="${BETA_ID}"]`);
            await desktop.waitForFunction((count) => document.querySelectorAll('#messages [data-msg-index]').length === count, loadedBetaCount, { timeout: 5000 });
            check(await desktop.locator('#messages [data-msg-index="0"]').count() === 1, 'briefly switching sessions preserves the earliest loaded history');
            await desktop.click(`.session-item[data-id="${registryState.sessionId}"]`);
            await desktop.waitForSelector('#messages .message.assistant');
            console.log('related-session navigation:');
            await desktop.evaluate((id) => fixtureApp.features.sessionView.select(id), BETA_ID);
            await desktop.waitForSelector('#sessionRelations .session-relation-chip', { timeout: 5000 });
            check((await desktop.locator('#sessionRelations').textContent() ?? '').includes('Subagents'), 'a session with family renders the subagents link');
            ...
            await desktop.waitForFunction(() => fixtureElement(document.getElementById('subagentsDetailName'), "document.getElementById('subagentsDetailName')").textContent.length > 0, null, { timeout: 5000 });
            ...
            const relationRaceOwner = await desktop.evaluate(async (nextId) => {
                ...
                try {
                    ...
                    if (!release)
                        throw new Error('Relation load release was not initialized');
                    release();
                    await pending;
                    const current = fixtureApp.features.sessionState.currentSession;
                    if (!current)
            ...
            await desktop.evaluate((id) => fixtureApp.features.sessionView.select(id), BETA_ID);
            await desktop.waitForSelector('#sessionRelations .session-relation-chip', { timeout: 5000 });
            check((await desktop.locator('#sessionRelations').textContent() ?? '').includes('Subagents · 10'), 'the link counts every family member, closed children included');
            await desktop.click('#sessionRelations .session-relation-chip');
            await desktop.waitForSelector('#subagentsTree .lineage-row', { timeout: 5000 });
            check(await desktop.locator('#subagentsTree .lineage-row').count() === 11, 'the tree lists every relation, including closed children');
            check((await desktop.locator('#subagentsView').textContent() ?? '').includes('closed-child-5'), 'closed children render in the tree');
            check(await desktop.locator('#subagentsTree .lineage-row .live-dot').count() === 5, 'the tree marks each live session');
            check(await desktop.locator('#subagentsTree .lineage-row.lineage-current').count() === 1, 'the tree marks the current session');
            await desktop.keyboard.press('Escape');
            await desktop.waitForFunction(() => !fixtureElement(document.querySelector('.main'), "document.querySelector('.main')").classList.contains('subagents-open'), null, { timeout: 3000 });
            check(true, 'Escape closes the subagents takeover');
            relationFixtureMode = 'active';
            await desktop.evaluate((id) => fixtureApp.features.sessionView.select(id), BETA_ID);
            await desktop.waitForSelector('#sessionRelations .session-relation-chip', { timeout: 5000 });
            check((await desktop.locator('#sessionRelations').textContent() ?? '').includes('Subagents · 26'), 'a large live fan-out still fits the single link');
            await desktop.click('#sessionRelations .session-relation-chip');
            await desktop.waitForSelector('#subagentsTree .lineage-row', { timeout: 5000 });
            check(await desktop.locator('#subagentsTree .lineage-row').count() === 27, 'the tree lists the complete fan-out');
            await desktop.keyboard.press('Escape');
            ...
            check(await desktop.locator('.main').evaluate((el) => !el.classList.contains('subagents-open')), 'navigation closes the subagents takeover');
            ...
            await desktop.waitForFunction((id) => fixtureApp.features.sessionState.currentSession?.id === id &&
                !fixtureElement(document.getElementById('sessionRelations'), "document.getElementById('sessionRelations')").textContent.includes('overflow-'), registryState.sessionId, { timeout: 5000 });
            await desktop.click('#tabActive');
            await desktop.waitForTimeout(200);
            ...
            await desktop.waitForFunction(() => !fixtureElement(document.querySelector('.scope-chip[data-name="Beta only"]'), 'document.querySelector(\'.scope-chip[data-name="Beta only"]\')').classList.contains('active'));
            await desktop.evaluate(() => fixtureApp.features.searchViewController.open('boundary-contract'));
            await desktop.waitForFunction((id) => document.querySelectorAll('.search-result').length === 100 &&
                !document.querySelector(`.search-result[data-id="${id}"]`) &&
                document.querySelector('.search-count-line')?.textContent.includes('showing the 100 best matches'), BETA_ID, { timeout: 10000 });
            ...
            await desktop.click('.scope-chip[data-name="Beta only"]');
            await desktop.waitForSelector('.scope-chip[data-name="Beta only"].active', { timeout: 5000 });
            check(await desktop.evaluate(() => localStorage.getItem('pi-dish-active-scopes')) === JSON.stringify(['Beta only']), 'advanced-search active scope state remains device-local');
            await desktop.evaluate(() => fixtureApp.features.searchViewController.open('boundary-contract'));
            // Scope the note lookup to the takeover: the sidebar behind it renders
            // its own `.scope-hidden-note` for the same active scope.
            await desktop.waitForFunction((id) => document.querySelectorAll('.search-result').length === 1 &&
                document.querySelector(`.search-result[data-id="${id}"]`) &&
                document.querySelector('#searchViewBody .scope-hidden-note')?.textContent === '100 hidden by scopes', BETA_ID, { timeout: 10000 });
            check(!await desktop.evaluate(() => fixtureElement(document.querySelector('.search-count-line'), "document.querySelector('.search-count-line')").textContent.includes('showing the 100 best matches')), 'scoped total and cap messaging describe the post-scope result set');
            await desktop.evaluate(() => fixtureApp.features.searchViewController.close());
            await desktop.fill('#filterInput', 'beta');
            await desktop.waitForSelector('.search-open-chip', { timeout: 2000 });
            await desktop.click('.search-open-chip');
            await desktop.waitForFunction(() => fixtureElement(document.querySelector('.main'), "document.querySelector('.main')").classList.contains('search-open'), null, { timeout: 5000 });
            check(await desktop.evaluate(() => fixtureInput(document.getElementById('searchViewInput'), "document.getElementById('searchViewInput')").value) === 'beta', 'full-search chip carries the sidebar query into the takeover');
            check(await desktop.evaluate(() => fixtureElement(document.getElementById('sessionView'), "document.getElementById('sessionView')").offsetParent === null), 'session view hidden while search is open');
            ...
            const countText = await desktop.evaluate((id) => document.querySelector(`.search-result[data-id="${id}"] .search-result-count`)?.textContent, BETA_ID);
            check(countText === '2 matches', `occurrence count rendered (got "${countText}")`);
            // The Active-only facet rewrites the query text (is:active) and filters.
            await desktop.click('#searchFacetActive');
            await desktop.waitForFunction((id) => fixtureInput(document.getElementById('searchViewInput'), "document.getElementById('searchViewInput')").value.includes('is:active') &&
            ...
            check(!(await desktop.evaluate(() => fixtureInput(document.getElementById('searchViewInput'), "document.getElementById('searchViewInput')").value)).includes('is:active'), 'toggling the facet off removes its token, keeping the text terms');
            ...
            check(await desktop.evaluate(() => fixtureInput(document.getElementById('searchInput'), "document.getElementById('searchInput')").value) === 'cedar maple', 'click-through hands distributed terms to the explicit in-session mode');
            check(await desktop.evaluate(() => document.querySelector('mark.search-mark')?.textContent.toLowerCase() === 'maple' &&
                fixtureElement(document.getElementById('searchCount'), "document.getElementById('searchCount')").textContent !== 'no matches'), 'distributed-term click-through lands on a relevant transcript message');
            check(await desktop.evaluate(() => fixtureElement(document.getElementById('sessionName'), "document.getElementById('sessionName')").textContent) === 'beta question', 'click-through opened the matched session');
            ...
            check(await desktop.evaluate(() => localStorage.getItem('pi-dish-active-scopes')) === JSON.stringify([]), 'toggling the saved scope inactive remains device-local');
            await desktop.fill('#filterInput', '');
            ...
            await desktop.waitForFunction((sel) => {
                const button = fixtureElement(document.querySelector(sel), sel);
                return parseFloat(getComputedStyle(button).opacity) > 0;
            ...
            check(await desktop.locator(`${closeRowSel} .session-close-btn.confirm`).count() === 1, 'first tap arms the danger confirm state');
            check(await desktop.evaluate(() => document.querySelector('.session-item.active')?.dataset.id || null) === selectedBefore, 'confirm tap does not select the row');
            ...
            check(await desktop.locator(`${closeRowSel} .session-close-btn.confirm`).count() === 1, 'list re-render preserves the armed confirm');
            // The armed state auto-reverts after ~3s.
            await desktop.waitForFunction((sel) => !document.querySelector(sel), `${closeRowSel} .session-close-btn.confirm`, { timeout: 6000 });
            ...
            const readClosePostUrl = () => closePostUrl;
            let resolveClosePost;
            const closePostFired = new Promise(resolve => { resolveClosePost = resolve; });
            await desktop.route('**/api/sessions/*/close', async (route) => {
                closePostUrl = route.request().url();
                if (!resolveClosePost)
                    throw new Error('Close route resolver was not initialized');
                resolveClosePost();
            ...
            await Promise.race([closePostFired, desktop.waitForTimeout(5000)]);
            const postedCloseUrl = readClosePostUrl();
            check(typeof postedCloseUrl === 'string' && postedCloseUrl.includes(`/api/sessions/${SESSION_ID}/close`), `second tap fires POST /close for the row (got ${JSON.stringify(postedCloseUrl)})`);
            ...
            const dummy = spawn(process.execPath, [IDLE_CHILD], { stdio: 'ignore' });
            let dummySignal = null;
            const dummyGone = new Promise(resolve => dummy.on('exit', (_code, sig) => { dummySignal = sig; resolve(); }));
            ...
            check(await desktop.locator('#sessionRestartBtn').count() === 0, 'externally launched Pi keeps Close but does not offer pane restart');
            const runtimeRow = await desktop.evaluate(() => {
                ...
                return row ? fixtureElement(row.querySelector('.stats-val'), '.stats-val').textContent : null;
            });
            check(runtimeRow === `terminal · pid ${dummy.pid}`, `stats modal shows where the session runs (got ${JSON.stringify(runtimeRow)})`);
            desktop.once('dialog', (d) => d.accept());
            ...
            const rpcRestartId = await desktop.evaluate(async (id) => {
                await PiDishBrowser.sendJson(fixtureApp.features.apiTransport.request, fixtureApp.features.hostDirectory.self.hostId, `/api/sessions/${encodeURIComponent(id)}/resume`, {});
            ...
            await desktop.waitForFunction((id) => fixtureApp.features.sessionState.currentSession?.id === id && fixtureApp.features.sessionState.currentSession.isActive, rpcRestartId, { timeout: 10000 });
            await desktop.click('#sessionContext');
            ...
            check(/^pi-dish server \(headless\) · pid \d+$/.test(beforeRestartRuntime || ''), `RPC restart starts from a managed child (got ${JSON.stringify(beforeRestartRuntime)})`);
            desktop.once('dialog', (d) => d.accept());
            await desktop.click('#sessionRestartBtn');
            await desktop.waitForFunction((id) => {
                const current = fixtureApp.features.sessionState.currentSession;
                return current?.id === id && current.isActive &&
                    fixtureElement(document.getElementById('statsModal'), "document.getElementById('statsModal')").style.display === 'none';
            ...
            check(afterRestartPid > 1 && afterRestartPid !== beforeRestartPid, `restart replaced the RPC child in place (${beforeRestartPid} → ${afterRestartPid})`);
            desktop.once('dialog', (d) => d.accept());
            await desktop.click('#sessionCloseBtn');
            await desktop.waitForSelector('#resumeBar', { state: 'visible', timeout: 10000 });
            ...
            const keys = await desktop.evaluate((id) => {
                ...
                if (!fixtureApp.features.hostDirectory.self.hostId)
                    fixtureApp.features.hostDirectory.setSelf({ hostId, label: null, version: null, capabilities: null });
                ...
                fixtureApp.ports.hostDiscovery.onSelf({ ...fixtureApp.features.hostDirectory.self, hostId });
                const key = sessionKey(hostId, id);
                const bareLeft = Object.keys(localStorage).filter((k) => /^pi-dish-(draft|history|terminal-mode)-/.test(k) && !k.includes(' ') && !k.includes('spawn:'));
                const freshState = PiDishBrowser.createSessionState({
                    getSelfHostId: () => hostId, getHostLabel: fixtureApp.ports.sessionState.getHostLabel,
            ...
            check(keys.draft === 'bare draft' && keys.history === '["bare prompt"]' && keys.mode === 'tmux', 'per-session drafts/history/terminal mode migrate to composite keys with their values');
            ...
            check(keys.pinned === JSON.stringify([keys.key]) && keys.expanded === JSON.stringify([keys.key]), 'pins and expanded families re-key to host + session');
            check(keys.selected === keys.key, 'the restored-session key carries its host');
            check(keys.derivedDraftKey === 'pi-dish-draft-' + keys.key &&
                keys.derivedTerminalKey === 'pi-dish-terminal-mode-' + keys.key, 'the live key helpers resolve to exactly what the migration wrote');
            ...
            const remoteProcess = remoteHost;
            const remoteDescriptorBody = (0, test_types_js_1.record)(await fetch(remoteHost.base + '/api/host').then(response => response.json()));
            if (typeof remoteDescriptorBody.hostId !== 'string')
                throw new Error('Remote host descriptor is invalid');
            const remoteDescriptor = remoteDescriptorBody;
                ...
                window.fixtureSessionListPatch = (id, fields, host) => {
                    ...
                    for (const kind of ['active', 'previous']) {
                        for (const row of fixtureApp.features.sessionState.sessions[kind]) {
                            ...
                            const part = fixtureElement(parts.get(key), 'host session partition');
            ...
            multi.on('request', (request) => { if (request.url().startsWith(remoteBase))
            ...
            check(remoteRequests.some(url => url.endsWith('/api/host/health')), 'the Fleet view reads the peer\'s capacity from the peer itself');
            ...
            const localRecoveryBefore = (0, test_types_js_1.record)(await fetch(base + '/api/settings').then(response => response.json()));
            await multi.selectOption('#recoveryMode', 'restore');
            await multi.click('#saveRecoveryMode');
            await multi.waitForFunction(() => document.getElementById('recoverySettingsStatus')?.textContent?.startsWith('Saved on') === true);
            const remoteRecovery = (0, test_types_js_1.record)(await fetch(remoteHost.base + '/api/settings', {
                headers: { Authorization: 'Bearer ' + REMOTE_TOKEN },
            }).then(response => response.json()));
            const localRecoveryAfter = (0, test_types_js_1.record)(await fetch(base + '/api/settings').then(response => response.json()));
            check(remoteRecovery.recoveryMode === 'restore' && localRecoveryAfter.recoveryMode === localRecoveryBefore.recoveryMode, 'recovery settings on a saved remote host persist there without changing the entry host');
            await multi.click('#openRecoveryReport');
            await multi.waitForFunction(() => document.getElementById('recoveryViewBody')?.textContent?.includes('No recorded sessions') === true);
            check(await multi.inputValue('#recoveryReportHost') === remoteDescriptor.hostId &&
                !!(await multi.textContent('#recoveryViewBody'))?.includes('Restore open sessions'), 'the recovery report follows the selected remote host');
            ...
            const descriptorRoute = async (route) => route.fulfill({
                json: { ...remoteDescriptor, capabilities: { ...((0, wire_protocol_js_1.isRecord)(remoteDescriptor.capabilities) ? remoteDescriptor.capabilities : {}), recovery: advertisesRecovery } },
            });
            ...
            check(await multi.locator(`#recoverySettingsHost option[value="${remoteDescriptor.hostId}"]`).count() === 0 &&
                !!(await multi.textContent('#recoveryUnavailableHosts'))?.includes('tycho'), 'an older peer is named with upgrade guidance instead of silently omitted');
            advertisesRecovery = true;
            await multi.evaluate(() => fixtureApp.features.hostDiscovery.loadFleet());
            check(await multi.locator(`#recoverySettingsHost option[value="${remoteDescriptor.hostId}"]`).count() === 1 &&
                await multi.inputValue('#recoveryMode') === 'continue', 'a capability refresh adds a remote recovery host without discarding an unsaved selection');
            ...
            const beforeFleetSettings = (0, test_types_js_1.record)(JSON.parse(fs.readFileSync(dishSettingsFile, 'utf8')));
            ...
            const [proxySave] = await Promise.all([
                multi.waitForResponse((response) => response.url() === base + '/hosts/tycho/api/settings' && response.request().method() === 'PUT'),
                multi.click('#saveRecoveryMode'),
            ]);
            const proxiedRecovery = (0, test_types_js_1.record)(await fetch(remoteHost.base + '/api/settings', {
                headers: { Authorization: 'Bearer ' + REMOTE_TOKEN },
            }).then(response => response.json()));
            check(proxySave.ok() && proxiedRecovery.recoveryMode === 'off' &&
                await multi.locator(`#recoverySettingsHost option[value="${remoteDescriptor.hostId}"]`).count() === 1, 'a fleet peer is configured through the main entrypoint proxy and appears only once');
            ...
            check(await multi.locator(`.session-item[data-id="${SESSION_ID}"]`).count() === 1, 'both hosts\' sessions are in one merged list');
            const remoteRow = multi.locator(`.session-item[data-id="${REMOTE_SESSION_ID}"]`);
            check((await remoteRow.getAttribute('data-host')) !== null, 'merged rows carry the host that served them');
            const groupKeys = await multi.evaluate((cwd) => [...document.querySelectorAll('.workspace-group-header')]
                .map((header) => header.dataset.cwd || '').filter((key) => key.endsWith(cwd)), CWD);
            check(groupKeys.length === 2 && new Set(groupKeys).size === 2 && groupKeys.every((k) => k.includes(' ')), `the same cwd on two hosts stays two host-qualified workspace groups (got ${JSON.stringify(groupKeys)})`);
            ...
            const sections = await multi.evaluate(() => [...document.querySelectorAll('#sessionList > .host-section')]
                ...
                dot: getComputedStyle(fixtureElement(section.querySelector('.host-section-dot'), '.host-section-dot')).backgroundColor,
                count: section.querySelector('.host-section-count')?.textContent || null,
            })));
            const selfLabel = await multi.evaluate(() => {
                const self = fixtureApp.ports.hostDiscovery.hosts()[0];
                return hostDisplayLabel(fixtureApp.features.hostView(self));
            });
            check(sections.length === 2 && sections[0].name === selfLabel && sections[1].name === 'tycho', `the workspace view is sectioned by host, self first (got ${JSON.stringify(sections.map((s) => s.name))})`);
            check(sections.every((s) => /^var\(--chart-\d\)$/.test(s.color)) &&
                new Set(sections.map((s) => s.color)).size === 2 &&
                new Set(sections.map((s) => s.dot)).size === 2, `each host section wears its own chart-slot color (got ${JSON.stringify(sections.map((s) => [s.color, s.dot]))})`);
            check(sections.every((s) => Number(s.count) > 0), `host headings count their sessions (got ${JSON.stringify(sections.map((s) => s.count))})`);
            const chipsInTrees = await multi.evaluate(() => document.querySelectorAll('#sessionList .workspace-group-header .host-chip').length);
            ...
            if (!remoteKey)
                throw new Error('Remote host section key is unavailable');
            await multi.evaluate(() => fixtureElement(document.querySelectorAll('.host-section-header')[1], 'remote host heading').click());
            ...
            const collapsed = await multi.evaluate(() => {
                const section = fixtureElement(document.querySelectorAll('#sessionList > .host-section')[1], 'remote host section');
                return {
            ...
            check(collapsed.klass && collapsed.body === 0 && collapsed.stored.includes(remoteKey), `collapsing a host section hides its tree and persists as ${remoteKey} (got ${JSON.stringify(collapsed)})`);
            await multi.evaluate(() => fixtureElement(document.querySelectorAll('.host-section-header')[1], 'remote host heading').click());
            await multi.waitForFunction(() => document.querySelectorAll('#sessionList > .host-section')[1]?.querySelector('.session-item'));
            check(await multi.evaluate(() => fixtureElement(document.querySelectorAll('#sessionList > .host-section')[1], 'remote host section').querySelectorAll('.session-item').length > 0), 'expanding the host section brings its tree back');
            ...
            check(recentChips.length > 0 && recentChips.every((c) => c.dot && c.color) && chipColors.length === 2, `Recent rows carry a color-dotted host chip per host (got ${JSON.stringify(chipColors)})`);
            check(await multi.evaluate(() => document.querySelectorAll('.host-section').length) === 0, 'the Recent view stays interleaved — no host sections');
            await multi.evaluate(() => fixtureApp.features.sidebarQuery.toggleView());
            ...
            await multi.waitForFunction(() => [...document.querySelectorAll('#sessionList > .host-section')]
                .find(section => section.querySelector('.host-section-name')?.textContent === 'tycho')?.style.getPropertyValue('--host-color') === '#d33682');
            const picked = await multi.evaluate((before) => {
                ...
                return {
                    ...
                    reset: !fixtureElement(document.querySelectorAll('#hostsList .host-color-reset')[1], 'remote color reset').classList.contains('hidden'),
                };
            }, beforeColors);
            check(picked.before.every((v) => /^#[0-9a-f]{6}$/.test(v)), `auto colors resolve to concrete hex for the picker (got ${JSON.stringify(picked.before)})`);
            check(picked.sectionColor === '#d33682' && Object.values(picked.stored).includes('#d33682') && picked.reset, `a picked color overrides that host everywhere and offers a reset (got ${JSON.stringify(picked)})`);
            await multi.evaluate(() => fixtureElement(document.querySelectorAll('#hostsList .host-color-reset')[1], 'remote color reset').click());
            await multi.waitForFunction(() => /^var\(--chart-\d\)$/.test([...document.querySelectorAll('#sessionList > .host-section')]
            ...
            check(/^var\(--chart-\d\)$/.test(afterReset.color) && Object.keys(afterReset.stored).length === 0, `reset returns the host to its automatic color (got ${JSON.stringify(afterReset)})`);
            await multi.keyboard.press('Escape');
            await multi.click(`.session-item[data-id="${REMOTE_SESSION_ID}"]`);
            await multi.waitForFunction(() => document.getElementById('messages')?.textContent.includes('remote host answer'), { timeout: 10000 });
            check(remoteRequests.some((u) => u.includes(`/api/sessions/${REMOTE_SESSION_ID}/messages`)), 'the remote session\'s transcript is fetched from its own host');
            check(await multi.locator('#sessionHost').isVisible() &&
                (await multi.locator('#sessionHost').textContent()) === 'tycho', 'the session header names the host');
            const remoteRefResolution = await multi.evaluate((remoteId) => {
                ...
                const current = fixtureApp.features.sessionState.currentSession;
                if (!current)
                    throw new Error('Current remote session is unavailable');
                const shadow = { ...current, host: fixtureApp.features.hostDirectory.self.hostId };
                const originalLists = fixtureApp.features.sessionState.sessions;
                const publish = (extra) => {
                    const parts = new Map();
                    for (const kind of ['active', 'previous']) {
                        for (const row of originalLists[kind]) {
                            const key = row.host || null;
                            if (!parts.has(key))
                                parts.set(key, { hostId: key, active: [], previous: [] });
                            fixtureElement(parts.get(key), 'session partition')[kind].push(row);
                        }
                    }
                    if (extra) {
                        const key = extra.host || null;
                        if (!parts.has(key))
                            parts.set(key, { hostId: key, active: [], previous: [] });
                        fixtureElement(parts.get(key), 'shadow partition').previous.push(extra);
                    }
                    fixtureApp.features.sessionState.setSessionLists([...parts.values()]);
                };
                publish(shadow);
                const candidates = fixtureApp.features.sessionReferences.candidates().filter((session) => session.id === remoteId)
                    .map((session) => fixtureApp.features.sessionReferences.hostId(session));
                publish(null);
                return { bareHost: bare?.host || null, currentHost: current.host, candidates };
            }, REMOTE_SESSION_ID);
            check(remoteRefResolution.bareHost === remoteRefResolution.currentHost, `a bare transcript #ref resolves on its owning remote host (got ${JSON.stringify(remoteRefResolution)})`);
            check(remoteRefResolution.candidates.length === 1 &&
                remoteRefResolution.candidates[0] !== remoteRefResolution.currentHost, 'the ref picker excludes only the current host+session pair when ids collide');
            ...
            const termGating = await multi.evaluate(() => {
                const self = fixtureApp.ports.hostDiscovery.hosts()[0];
                const remote = fixtureApp.ports.hostDiscovery.hosts().find((host) => !host.self);
                return {
                    self: hostSupportsTerminal(self ? fixtureApp.features.hostView(self) : null, fixtureApp.ports.routinesController.config()),
                    remote: hostSupportsTerminal(remote ? fixtureApp.features.hostView(remote) : null, fixtureApp.ports.routinesController.config()),
                    remoteCaps: !!(remote && remote.capabilities),
                    button: fixtureElement(document.getElementById('btnTerminal'), "document.getElementById('btnTerminal')").style.display,
                };
            });
            check(termGating.self === true && termGating.remoteCaps && termGating.remote === false
                && termGating.button === 'none', `terminal gating follows the session's host, not the entry host (got ${JSON.stringify(termGating)})`);
            ...
            const listReqs = [];
            const noteList = (request) => { if (/\/api\/sessions(\?|$)/.test(request.url()))
                listReqs.push(request.url()); };
            multi.on('request', noteList);
            await multi.evaluate(() => {
                const input = fixtureInput(document.getElementById('filterInput'), '#filterInput');
            ...
            check(await multi.locator(`.session-item[data-id="${REMOTE_SESSION_ID}"]`).count() === 1, 'host: keeps the named host\'s rows and drops the others');
            ...
            check(listReqs.length > 0 && listReqs.every((u) => u.startsWith(remoteBase)), `host: prunes the fan-out to the named host (got ${JSON.stringify(listReqs)})`);
            check(listReqs.every((u) => !/host(%3A|:)/i.test(u)), `the host: term is stripped before the wire (got ${JSON.stringify(listReqs)})`);
            await multi.evaluate(() => {
                fixtureInput(document.getElementById('filterInput'), "document.getElementById('filterInput')").value = '';
                fixtureApp.features.sidebarQuery.onInput();
            });
            ...
            check(facetHosts.length === 3 && facetHosts.includes('tycho'), `the host facet offers every known host (got ${JSON.stringify(facetHosts)})`);
            await multi.selectOption('#searchFacetHost', 'tycho');
            await multi.waitForFunction(() => fixtureInput(document.getElementById('searchViewInput'), "document.getElementById('searchViewInput')").value === 'host:tycho', { timeout: 10000 });
            await multi.waitForFunction((id) => [...document.querySelectorAll('.search-result')]
                .every((row) => row.dataset.id !== id), SESSION_ID, { timeout: 10000 });
            check(await multi.locator(`.search-result[data-id="${REMOTE_SESSION_ID}"]`).count() === 1, 'the host facet narrows the results to that host');
            await multi.keyboard.press('Escape');
            ...
            const seenKeys = await multi.evaluate((id) => {
                const host = fixtureApp.ports.hostDiscovery.hosts().find((candidate) => !candidate.self);
                if (!host)
                    throw new Error('Remote host is unavailable');
                return { keys: Object.keys(JSON.parse(localStorage.getItem('pi-dish-seen') || '{}')), want: host.hostId + ' ' + id };
            }, REMOTE_SESSION_ID);
            check(seenKeys.keys.includes(seenKeys.want), `the seen map records the remote session under its own host (got ${JSON.stringify(seenKeys.keys)})`);
            ...
            check(hostOptions.length === 2 && hostOptions.includes('tycho'), `the new-session takeover offers both hosts (got ${JSON.stringify(hostOptions)})`);
            await multi.keyboard.press('Escape');
            ...
            if (!selfId)
                throw new Error('Self host identity is unavailable');
            ...
            check((await multi.locator('#messages').textContent() ?? '').includes('COLLISION SELF TRANSCRIPT'), 'the self-host collision renders its own transcript');
            ...
            check((await multi.locator('#messages').textContent() ?? '').includes('COLLISION REMOTE TRANSCRIPT') &&
                !(await multi.locator('#messages').textContent() ?? '').includes('COLLISION SELF TRANSCRIPT'), 'selecting the peer with the same id cannot restore the self-host transcript');
            await multi.evaluate(() => fixtureApp.features.sidebarLists.load(undefined, { withPrevious: true }));
            check(await multi.evaluate((host) => fixtureApp.features.sessionState.currentSession?.host === host, remoteId), 'polling preserves the selected host when ids collide');
            const selectedCollision = multi.locator(`.session-item.active[data-id="${collisionId}"]`);
            check(await selectedCollision.count() === 1 && await selectedCollision.getAttribute('data-host') === remoteId, 'only the selected host row is highlighted');
            await multi.evaluate((id) => fixtureApp.features.sessionView.select(id, { host: 'missing-host' }), collisionId);
            check(await multi.evaluate(({ id, host }) => !fixtureApp.features.sessionState.findSession(id, 'missing-host') && fixtureApp.features.sessionState.currentSession?.host === host, { id: collisionId, host: remoteId }), 'a missing host-qualified session never falls back to another host');
            await multi.evaluate(({ id, host }) => fixtureApp.features.sessionView.select(id, { host }), { id: collisionId, host: selfId });
            check((await multi.locator('#messages').textContent() ?? '').includes('COLLISION SELF TRANSCRIPT') &&
                !(await multi.locator('#messages').textContent() ?? '').includes('COLLISION REMOTE TRANSCRIPT'), 'switching back restores only the owning host\'s cached transcript');
            ...
            for (const [action, field, value] of collisionMutations) {
                await multi.evaluate(({ id, host }) => fixtureApp.features.sessionView.select(id, { host }), { id: collisionId, host: remoteId });
                ...
                await multi.route(endpoint, (route) => {
                    if (!receiveRequest)
                        throw new Error('Collision route receiver is unavailable');
                    receiveRequest(route);
                });
                const sent = multi.waitForRequest(endpoint, { timeout: 10000 });
                const before = await multi.evaluate(({ id, host, field }) => {
                    const session = fixtureApp.features.sessionState.findSession(id, host);
                    if (!session)
                        throw new Error('Collision self session is unavailable');
                    return session[field];
                }, { id: collisionId, host: selfId, field });
                await multi.evaluate(({ action, value }) => {
                    if (action === 'rename') {
                        fixtureApp.features.sessionControls.startRename();
                        fixtureInput(document.getElementById('sessionNameInput'), '#sessionNameInput').value = value;
                    }
                ...
                const outcome = await multi.evaluate(({ id, self, remote, field }) => {
                    const current = fixtureApp.features.sessionState.currentSession;
                    const selfSession = fixtureApp.features.sessionState.findSession(id, self);
                    const remoteSession = fixtureApp.features.sessionState.findSession(id, remote);
                    if (!current || !selfSession || !remoteSession)
                        throw new Error('Collision sessions are unavailable');
                    return { selectedHost: current.host, selected: current[field], self: selfSession[field], remote: remoteSession[field] };
                }, { id: collisionId, self: selfId, remote: remoteId, field });
                check(outcome.selectedHost === selfId && outcome.selected === before && outcome.self === before && outcome.remote === value, `delayed ${action} completion updates only the originating host`);
            ...
            await multi.evaluate(({ id, hosts }) => {
                for (const host of hosts)
                    window.fixtureSessionListPatch(id, { isActive: true, capabilities: { close: true } }, host);
            ...
            check(await selfClose.evaluate(el => el.classList.contains('confirm')) && !await remoteClose.evaluate(el => el.classList.contains('confirm')), 'the close confirmation is scoped to its host');
            await remoteClose.click();
            await remoteClose.and(multi.locator('.confirm')).waitFor();
            check(await remoteClose.evaluate(el => el.classList.contains('confirm')) && !await selfClose.evaluate(el => el.classList.contains('confirm')), 'the first click on another host arms a new confirmation');
            ...
            check(closeResponse.request().method() === 'POST' &&
                await multi.evaluate((host) => fixtureApp.features.sessionState.currentSession?.host === host, selfId), 'closing the remote row sends to the peer and preserves the self-host selection');
            ...
            const remoteGone = (0, node_events_1.once)(remoteProcess.child, 'exit');
            remoteProcess.child.kill('SIGKILL');
            await remoteGone;
            remoteHost = null;
            await multi.evaluate(() => fixtureApp.features.sidebarLists.load(undefined, { withPrevious: true }));
            ...
            check(await multi.locator(`.session-item[data-id="${SESSION_ID}"]`).count() === 1 &&
                await multi.locator(`.session-item[data-id="${SESSION_ID}"].stale-host`).count() === 0, 'the reachable host\'s rows are untouched by the dead one');
            const offlineSection = await multi.evaluate(() => {
                const host = fixtureApp.ports.hostDiscovery.hosts().find((candidate) => !candidate.self);
                if (!host)
                    throw new Error('Offline host is unavailable');
                const header = [...document.querySelectorAll('.host-section-header')]
                    .find((candidate) => candidate.dataset.hostSection === 'host:' + host.hostId);
            ...
            check(offlineSection.state === 'backoff' && offlineSection.offline &&
                offlineSection.note === 'unreachable' && offlineSection.rows > 0, `the dead host's section heading says why its rows are stale (got ${JSON.stringify(offlineSection)})`);
            await multi.close();
        }
        ...
        failures++;
        console.error('  ✘ smoke test crashed:', (0, test_types_js_1.errorMessage)(error));
        if (error instanceof Error && error.stack)
            console.error(error.stack);
        if (errors.length)
            console.error('  collected page errors:', errors.join(' | '));
    }
    finally {
        await browser.close();
        server.close();
        ...
        try {
            const rpcSessions = require('../lib/rpc-session');
            for (const rpc of rpcSessions.getAllRPCSessions())
                rpc.kill();
        }
        catch { }
