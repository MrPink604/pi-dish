...
const mobile: UiScenario = async ({ browser, watch, base, check, emit, SESSION_ID }) => {
  ...
  const primary = await mobile.evaluate(() => {
    ...
    if (harness === null) throw new Error('Missing harness');
    ...
    const shown = [...row.querySelectorAll<HTMLElement>(':scope > *')].filter(el => el.offsetParent !== null);
    return {
  ...
  const chipRow = await mobile.evaluate(() => {
    ...
    if (row === null) throw new Error('Missing row');
    const first = [...row.querySelectorAll<HTMLElement>(':scope > *')].find(el => el.offsetParent !== null);
    return {
      first: first?.id,
  ...
  const ctxRight = () => mobile.evaluate(() =>
    Math.round(fixtureElement(document.getElementById('sessionContext'), "document.getElementById('sessionContext')").getBoundingClientRect().right));
  const boxHeight = () => mobile.evaluate(() =>
    Math.round(fixtureElement(document.querySelector('.composer-box'), "document.querySelector('.composer-box')").getBoundingClientRect().height));
  ...
  const composer = await mobile.evaluate(() => {
    ...
    const inBox = (rect: DOMRect) => rect.left >= box.left && rect.right <= box.right &&
      rect.top >= box.top && rect.bottom <= box.bottom;
    const visible = [...document.querySelectorAll<HTMLElement>('.composer-box button')]
  ...
  const headerHeight = () => mobile.evaluate(() =>
    fixtureElement(document.querySelector('.session-header'), "document.querySelector('.session-header')").getBoundingClientRect().height);
  ...
  const collapsedStatus = await mobile.evaluate(() => {
    ...
    if (badge === null) throw new Error('Missing badge');
  ...
  await mobile.waitForFunction(() =>
    fixtureElement(document.getElementById('extUiStatuses'), "document.getElementById('extUiStatuses')").style.display === 'none', { timeout: 5000 });
  ...
  if (overlay === null) throw new Error('Missing overlay');
  ...
  check(await mobile.evaluate(() => fixtureElement(document.getElementById('routinesDetail'), "document.getElementById('routinesDetail')").offsetParent === null),
  ...
  check(await mobile.evaluate(() => fixtureElement(document.getElementById('routinesList'), "document.getElementById('routinesList')").offsetParent === null &&
    fixtureElement(document.getElementById('routinesDetail'), "document.getElementById('routinesDetail')").offsetParent !== null),
  ...
  await mobile.waitForFunction(() => fixtureElement(document.getElementById('terminalStatus'), "document.getElementById('terminalStatus')").textContent === '',
    { timeout: 5000 });
  ...
  check(await mobile.evaluate(() => fixtureElement(document.getElementById('termKeyCtrl'), "document.getElementById('termKeyCtrl')").classList.contains('latched')),
    'ctrl key latches');
