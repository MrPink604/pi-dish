// Generated test/tool from test/ui-scenarios/models.ts; edit that source and run npm run build:tests.
"use strict";
const models = async ({ path, tmpHome, desktop, check, fs }) => {
    ...
    check(await desktop.locator('.model-option[title="test/other-model"].disabled').count() === 1, 'toggled model renders as disabled');
    await desktop.waitForTimeout(700); // debounced save
    check(JSON.stringify(readSettings().enabledModels) === JSON.stringify(['test/smoke-model', 'test/third-model']), 'enabledModels persisted to pi settings.json');
    ...
    check(await desktop.locator('.model-option.disabled').count() === 3, 'provider toggle disables every model in the section');
    await desktop.click('.model-group-toggle');
    check(await desktop.locator('.model-option.disabled').count() === 0, 'provider toggle re-enables the section');
