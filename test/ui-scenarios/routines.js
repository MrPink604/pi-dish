// Generated test/tool from test/ui-scenarios/routines.ts; edit that source and run npm run build:tests.
"use strict";
const routines = async ({ desktop, check, CWD }) => {
    ...
    check(await desktop.evaluate(() => fixtureElement(document.getElementById('sessionView'), "document.getElementById('sessionView')").offsetParent === null), 'session view hidden while the routines takeover is open');
    check(await desktop.evaluate(() => document.querySelectorAll('.host-chip').length === 0 ||
        !document.querySelector('.routines-list .host-chip')), 'no host chrome in the routines list on a single host');
    ...
    check(await desktop.inputValue('#rtCron') === '0 9 * * 1-5', 'a schedule preset writes into the cron input');
    ...
    check(await desktop.locator('#rtError').textContent() === '', `routine created without a validation error (got "${await desktop.locator('#rtError').textContent()}")`);
    const rowText = await desktop.locator('.rt-row').first().textContent();
    check(!!rowText?.includes('smoke-routine'), 'the new routine appears in the list');
    const scheduleText = await desktop.locator('.rt-row-sched').first().textContent();
    check(!!scheduleText?.includes('0 9 * * 1-5'), 'the list row shows the cron expression');
    const curlText = await desktop.locator('#rtCurl').textContent();
    check(!!curlText?.includes('/api/routines/') && curlText.includes('"input"'), 'the invoke box shows a curl carrying an input body');
    // A changed prompt appends a version; nothing else does.
    await desktop.fill('#rtPrompt', 'smoke routine prompt v2');
    await desktop.click('#rtSaveBtn');
    await desktop.waitForFunction(() => document.querySelector('.rt-version-badge')?.textContent === 'v2', null, { timeout: 10000 });
    ...
    check(await desktop.locator('.rt-version-row').count() === 2, 'the versions block lists both prompt versions');
    await desktop.click('.rt-version-restore[data-restore="1"]');
    check(await desktop.inputValue('#rtPrompt') === 'smoke routine prompt v1', 'restore writes the older prompt back into the editor (a save would make it a new version)');
    await desktop.fill('#rtPrompt', 'smoke routine prompt v2');
    ...
    await desktop.waitForFunction(() => /completed/.test(document.querySelector('.rt-table tbody tr')?.textContent || ''), null, { timeout: 40000 });
    const runRow = (await desktop.locator('.rt-table tbody tr').first().innerText()).replace(/\s+/g, ' ');
    check(runRow.includes('v2') && runRow.includes('invoke') && runRow.includes('prompt'), `the run records its version, trigger and delivery (got "${runRow}")`);
    ...
    check(await desktop.locator('.rt-table tbody tr').count() === 0 &&
        await desktop.locator('.rt-filter-chip').count() === 1, 'clicking a version row filters the runs table to that version');
    await desktop.click('.rt-filter-clear');
    await desktop.waitForFunction(() => document.querySelectorAll('.rt-table tbody tr').length === 1, null, { timeout: 5000 });
    ...
    await desktop.evaluate(() => {
        fixtureInput(document.querySelector('#filterInput'), '#filterInput').value = 'routine:smoke-routine';
        fixtureApp.features.sidebarQuery.onInput();
    });
    await desktop.waitForSelector('.routine-chip', { timeout: 10000 });
    check(!!(await desktop.locator('.routine-chip').first().textContent())?.includes('smoke-routine'), 'the routine\'s session wears a ⏱ chip in the sidebar');
    check(await desktop.evaluate((id) => !!document.querySelector(`.session-item[data-id="${id}"] .routine-chip`), routineSessionId), 'the chip sits on the row of the session the run actually used');
    ...
    await desktop.evaluate(() => {
        fixtureInput(document.querySelector('#filterInput'), '#filterInput').value = '';
        fixtureApp.features.sidebarQuery.onInput();
    });
    ...
    await desktop.waitForFunction(() => fixtureElement(document.querySelector('.main'), "document.querySelector('.main')").classList.contains('usage-open') &&
        !fixtureElement(document.querySelector('.main'), "document.querySelector('.main')").classList.contains('routines-open'), null, { timeout: 5000 });
    check(true, 'opening the usage view closes the routines takeover');
    await desktop.keyboard.press('Escape');
    await desktop.evaluate(() => fixtureApp.features.routinesController.open());
    await desktop.waitForSelector('.main.routines-open', { timeout: 5000 });
    await desktop.keyboard.press('Escape');
    await desktop.waitForFunction(() => !fixtureElement(document.querySelector('.main'), "document.querySelector('.main')").classList.contains('routines-open'), null, { timeout: 3000 });
