...
const sidebar: UiScenario = async ({ desktop, registryState, check, SKILL_SESSION_ID, CWD, BETA_ID, SESSION_ID }) => {
  ...
  const familyRootId = await desktop.locator(`.session-item[data-id="${registryState.sessionId}"]`)
    .evaluate(el => el.closest<HTMLElement>('.session-family-root')?.dataset.familyId);
  ...
  check(await desktop.evaluate((id) =>
    JSON.parse(localStorage.getItem('pi-dish-expanded-session-families') || '[]')
      .some((key: string) => parseSessionKey(key).sessionId === id), registryState.sessionId),
  ...
  await desktop.evaluate(() => {
    ...
    if (!restore) throw new Error('Missing sidebar create restore callback');
  ...
  await desktop.waitForFunction((want) =>
    JSON.stringify([...document.querySelectorAll<HTMLElement>('.pinned-segment > .session-family-root')].map((el) => el.dataset.familyId)) === want,
  ...
  check(JSON.stringify(await desktop.evaluate(() => JSON.parse(localStorage.getItem('pi-dish-pinned-sessions') || '[]')
    .map((key: string) => parseSessionKey(key).sessionId))) ===
  ...
  await desktop.waitForFunction((id) => document.querySelector<HTMLElement>('.pinned-segment .session-item')?.dataset.id === id,
