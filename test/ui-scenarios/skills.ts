...
const skills: UiScenario = async ({ desktop, check }) => {
  ...
  await desktop.waitForFunction(() =>
    fixtureElement(document.querySelector('.main'), "document.querySelector('.main')").classList.contains('skills-open') &&
  ...
  await desktop.evaluate(() => {
    const row = [...document.querySelectorAll<HTMLElement>('.sk-row')]
      .find(candidate => fixtureElement(candidate.querySelector<HTMLElement>('.sk-name'), '.sk-name').textContent.includes('smoke-skill'));
  ...
  await desktop.waitForFunction(() =>
    fixtureElement(document.querySelector('.main'), "document.querySelector('.main')").classList.contains('new-session-open'), null, { timeout: 5000 });
  check(await desktop.evaluate(() => (fixtureInput(document.querySelector<HTMLInputElement>('#newSessionCwd'), '#newSessionCwd').value || '').includes('smoke-skill')),
  ...
  await desktop.waitForFunction(() => !fixtureElement(document.querySelector('.main'), "document.querySelector('.main')").classList.contains('new-session-open'),
    null, { timeout: 2000 });
