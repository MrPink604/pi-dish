// Generated test/tool from test/ui-scenarios/fleet.ts; edit that source and run npm run build:tests.
"use strict";
...
async function layout(page) {
    return page.evaluate(() => {
        ...
        return {
            ...
            overflowing: [...document.querySelectorAll('#fleetView .fleet-host, #fleetView .fleet-meter, #fleetView .preference-row, #fleetView #bounceView')]
                .filter(el => el.getBoundingClientRect().right > width + 1).map(el => el.className || el.id),
        };
    });
}
const fleet = async ({ desktop, browser, watch, base, check }) => {
    ...
    check(wide.cards >= 1 && wide.meters.every(count => count >= 2), `each host card shows capacity meters (got ${JSON.stringify(wide.meters)})`);
    ...
    check(wide.pageScroll <= wide.width && wide.bodyScroll <= 0 && wide.overflowing.length === 0, `desktop Fleet view has no horizontal overflow (got ${JSON.stringify(wide)})`);
    check(await desktop.locator('#fleetConnections #hostsList').isVisible() &&
        await desktop.locator('#fleetView #openBounceAgents').isVisible(), 'connections and bounce controls render below the host cards');
    ...
    try {
        ...
        check(!(await phone.locator('.sidebar').evaluate(el => el.classList.contains('open'))), 'opening Fleet from the drawer closes the drawer');
        await phone.click('#openBounceAgents');
        await phone.waitForSelector('#bounceMode', { state: 'visible' });
        const narrow = await layout(phone);
        check(narrow.cards >= 1 && narrow.meters.every(count => count >= 2), `mobile host cards show capacity meters (got ${JSON.stringify(narrow.meters)})`);
        check(narrow.pageScroll <= narrow.width && narrow.bodyScroll <= 0 && narrow.overflowing.length === 0, `mobile Fleet view has no horizontal overflow (got ${JSON.stringify(narrow)})`);
