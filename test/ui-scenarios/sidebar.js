// Generated test/tool from test/ui-scenarios/sidebar.ts; edit that source and run npm run build:tests.
"use strict";
const sidebar = async ({ desktop, registryState, check, SKILL_SESSION_ID, CWD, BETA_ID, SESSION_ID }) => {
    ...
    check(await desktop.locator(`.session-item[data-id="${SKILL_SESSION_ID}"]`).count() === 0, 'same-workspace child session is grouped under its parent and collapsed by default');
    ...
    check(JSON.stringify(familyIds) === JSON.stringify([registryState.sessionId, SKILL_SESSION_ID]), 'expanded family keeps the parent first with its child directly beneath');
    check(await desktop.evaluate((id) => JSON.parse(localStorage.getItem('pi-dish-expanded-session-families') || '[]')
        .some((key) => parseSessionKey(key).sessionId === id), registryState.sessionId), 'family expansion persists device-locally');
    await desktop.click(`.session-item[data-id="${registryState.sessionId}"] .session-family-toggle`);
    ...
    check(await desktop.locator(`.session-item[data-id="${registryState.sessionId}"] .session-item-status.working`).count() === 1, 'collapsed parent surfaces a working child status');
    ...
    check(await desktop.locator(`.session-item[data-id="${registryState.sessionId}"] .session-family-toggle`)
        .getAttribute('aria-expanded') === 'true', 'selecting a collapsed child reveals its ancestor and active row');
    ...
    await desktop.click(`.session-item[data-id="${registryState.sessionId}"] .session-family-toggle`);
    await desktop.waitForFunction((id) => !document.querySelector(`.session-item[data-id="${id}"]`), SKILL_SESSION_ID);
    const groupLabels = () => desktop.evaluate(() => [...document.querySelectorAll('.session-segment:not(.pinned-segment) .workspace-group-label')]
        .map((el) => el.textContent));
    await desktop.waitForFunction(() => document.querySelectorAll('.session-segment').length >= 3, null, { timeout: 5000 });
    ...
    check(labelsBefore[1] === 'proj-alpha' && labelsBefore[2] === 'proj-beta', 'children show distinguishing tails, newest first');
    ...
    check(await desktop.locator('.session-segment.collapsed .session-item').count() === 0, 'collapsed group hides its sessions');
    check(await desktop.evaluate(() => JSON.parse(localStorage.getItem('pi-dish-collapsed-groups') || '[]').length) === 1, 'collapse persisted to localStorage');
    ...
    check(JSON.stringify(await groupLabels()) === JSON.stringify(labelsBefore), 'expanding restores the original order');
    ...
    await desktop.waitForSelector('.session-segment.collapsed', { timeout: 2000 });
    check(await desktop.evaluate(() => document.querySelectorAll('#sessionList .session-item').length) === 0, 'collapsed prefix node hides all descendant sessions');
    await desktop.click('.session-segment.collapsed .workspace-group-header');
    await desktop.waitForFunction(() => !document.querySelector('.session-segment.collapsed'), null, { timeout: 2000 });
    ...
    check(await desktop.evaluate(() => window.__newSessionCwd) === CWD, 'header + button targets the node cwd');
    await desktop.evaluate(() => {
        const restore = window.restoreSidebarCreate;
        if (!restore)
            throw new Error('Missing sidebar create restore callback');
        fixtureApp.ports.sidebarControls.create = restore;
        delete window.restoreSidebarCreate;
    });
    check(await desktop.locator('.session-segment.collapsed').count() === 0, 'header + button does not toggle collapse');
    ...
    check(await desktop.evaluate(() => document.querySelector('#sessionList .session-segment')?.classList.contains('pinned-segment')), 'pinned section renders at the top');
    check(await desktop.locator(`.pinned-segment .session-item[data-id="${SKILL_SESSION_ID}"]`).count() === 0, 'pinned family remains collapsed by default');
    await pinToggle(BETA_ID);
    await desktop.waitForFunction(() => document.querySelectorAll('.pinned-segment > .session-family-root').length === 2, null, { timeout: 2000 });
    check(await desktop.locator('.pinned-segment .session-drag-handle').count() === 2, 'pinned families carry one drag handle each');
    check(await desktop.locator('.pinned-segment .session-item-cwd').count() === 2, 'pinned rows show their workspace');
    ...
    await desktop.waitForFunction((want) => JSON.stringify([...document.querySelectorAll('.pinned-segment > .session-family-root')].map((el) => el.dataset.familyId)) === want, JSON.stringify([BETA_ID, registryState.sessionId]), { timeout: 2000 });
    const draggedFamilyIds = await desktop.locator(`.pinned-segment .session-family-root[data-family-id="${registryState.sessionId}"] .session-item`)
        .evaluateAll(rows => rows.map(row => row.dataset.id));
    check(JSON.stringify(draggedFamilyIds) === JSON.stringify([registryState.sessionId, SKILL_SESSION_ID]), 'drag handle moves the expanded family as one block');
    ...
    await desktop.waitForFunction((id) => document.querySelector('.pinned-segment .session-item')?.dataset.id === id, SESSION_ID, { timeout: 5000 });
    check(await desktop.locator(`.pinned-segment .session-item[data-id="${SKILL_SESSION_ID}"]`).count() === 0, 'filtered child pin restores the whole family in its collapsed state');
    ...
    await desktop.fill('#filterInput', '');
    await desktop.waitForFunction(() => document.querySelectorAll('.pinned-segment > .session-family-root').length === 2, null, { timeout: 5000 });
    await pinToggle(BETA_ID);
    await pinToggle(SESSION_ID);
    await desktop.waitForFunction(() => !document.querySelector('.pinned-segment'), null, { timeout: 2000 });
};
