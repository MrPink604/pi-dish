// Generated test/tool from test/ui-scenarios/mobile.ts; edit that source and run npm run build:tests.
"use strict";
const mobile = async ({ browser, watch, base, check, emit, SESSION_ID }) => {
    ...
    check(!(await mobile.locator('.sidebar').evaluate(el => el.classList.contains('open'))), 'drawer closes after picking a session');
    ...
    check(relationLayout.chips === 1 && relationLayout.text.includes('Subagents') && relationLayout.height <= 32, `the family-tree link stays a single compact mobile chip (got ${JSON.stringify(relationLayout)})`);
    ...
    check(title && model && title.y + title.height <= model.y, 'session title sits above the model selector');
    ...
    const primary = await mobile.evaluate(() => {
        ...
        host.style.display = '';
        host.textContent = 'tycho';
        harness.style.display = '';
        harness.textContent = '◆';
        fixtureElement(document.getElementById('sessionThinking'), "document.getElementById('sessionThinking')").style.display = '';
        const model = document.getElementById('sessionModel');
        if (model === null)
            throw new Error('Missing model');
        model.textContent = 'anthropic/claude-opus-4-5-20260101 ▾';
        const shown = [...row.querySelectorAll(':scope > *')].filter(el => el.offsetParent !== null);
        return {
    ...
    check(primary.ids.includes('sessionHost') && primary.ids.includes('sessionHarness') &&
        primary.ids.includes('sessionThinking') && primary.overflowing === 0, `host, harness and reasoning fit beside the model (got ${JSON.stringify(primary)})`);
    check(primary.clipped.length <= 1 && (primary.clipped[0] || 'sessionModel').includes('odel'), `only the model chip gives up width (got ${JSON.stringify(primary.clipped)})`);
    check(primary.shrinkable.length === 1 && primary.shrinkable[0].includes('model'), `only the model chip may shrink (got ${JSON.stringify(primary.shrinkable)})`);
    const chipRow = await mobile.evaluate(() => {
        const row = document.getElementById('sessionChips');
        if (row === null)
            throw new Error('Missing row');
        const first = [...row.querySelectorAll(':scope > *')].find(el => el.offsetParent !== null);
        return {
            first: first?.id,
    ...
    check(chipRow.first === 'sessionWorkingMobile' && chipRow.text === 'idle' && chipRow.scrolls, `the chip row scrolls and leads with run state (got ${JSON.stringify(chipRow)})`);
    ...
    check(!statusVisibility.working && statusVisibility.error, `the run chip carries turn state alone; only errors add prose (got ${JSON.stringify(statusVisibility)})`);
    ...
    const ctxRight = () => mobile.evaluate(() => Math.round(fixtureElement(document.getElementById('sessionContext'), "document.getElementById('sessionContext')").getBoundingClientRect().right));
    const boxHeight = () => mobile.evaluate(() => Math.round(fixtureElement(document.querySelector('.composer-box'), "document.querySelector('.composer-box')").getBoundingClientRect().height));
    ...
    const composer = await mobile.evaluate(() => {
        const box = fixtureElement(document.querySelector('.composer-box'), "document.querySelector('.composer-box')").getBoundingClientRect();
        const text = fixtureElement(document.getElementById('promptInput'), "document.getElementById('promptInput')").getBoundingClientRect();
        const tools = fixtureElement(document.querySelector('.composer-tools'), "document.querySelector('.composer-tools')").getBoundingClientRect();
        const steer = fixtureElement(document.getElementById('btnSteer'), "document.getElementById('btnSteer')").getBoundingClientRect();
        const follow = fixtureElement(document.getElementById('btnFollowUp'), "document.getElementById('btnFollowUp')").getBoundingClientRect();
        const inBox = (rect) => rect.left >= box.left && rect.right <= box.right &&
    ...
    check(composer.outside.length === 0 && composer.textBottom <= composer.railTop, `the control strip sits inside the field, below the text (got ${JSON.stringify(composer)})`);
    check(composer.ids.includes('btnFollowUp') && composer.ids.includes('btnSteer') &&
        composer.overflowing.length === 0, `steer and follow-up stay reachable mid-turn (got ${JSON.stringify(composer)})`);
    check(composer.steerBottom <= composer.followTop &&
        Math.abs(composer.followBottom - composer.toolsBottom) <= 2, `in-turn actions stack up from the tools baseline (got ${JSON.stringify(composer)})`);
    check(await boxHeight() === heightIdle, `the field keeps its idle height when a turn starts (idle ${heightIdle})`);
    check(await ctxRight() === ctxIdle, `the context readout keeps its slot when a turn starts (idle ${ctxIdle})`);
    ...
    const headerHeight = () => mobile.evaluate(() => fixtureElement(document.querySelector('.session-header'), "document.querySelector('.session-header')").getBoundingClientRect().height);
    ...
    check(collapsedStatus.height <= 24 && collapsedStatus.clipped && withStatus - baseHeader <= 30, `collapsed status stays one clipped line (got ${JSON.stringify(collapsedStatus)}, header ${baseHeader}→${withStatus})`);
    ...
    await mobile.waitForFunction(() => fixtureElement(document.getElementById('extUiStatuses'), "document.getElementById('extUiStatuses')").style.display === 'none', { timeout: 5000 });
    ...
    await mobile.waitForFunction(() => !fixtureElement(document.getElementById('sidebar'), "document.getElementById('sidebar')").classList.contains('open'));
    ...
    check(await mobile.evaluate(() => fixtureElement(document.getElementById('routinesDetail'), "document.getElementById('routinesDetail')").offsetParent === null), 'mobile routines opens on the list, detail stacked out of view');
    await mobile.click('.rt-row');
    await mobile.waitForSelector('.routines-view.detail-open #rtName', { timeout: 5000 });
    check(await mobile.evaluate(() => fixtureElement(document.getElementById('routinesList'), "document.getElementById('routinesList')").offsetParent === null &&
        fixtureElement(document.getElementById('routinesDetail'), "document.getElementById('routinesDetail')").offsetParent !== null), 'tapping a routine swaps the list for the detail');
    await mobile.click('.rt-back');
    check(await mobile.evaluate(() => fixtureElement(document.getElementById('routinesList'), "document.getElementById('routinesList')").offsetParent !== null), 'the ‹ back control returns to the routine list');
    ...
    await mobile.waitForFunction(() => fixtureElement(document.getElementById('terminalStatus'), "document.getElementById('terminalStatus')").textContent === '', { timeout: 5000 });
    ...
    check(await mobile.evaluate(() => fixtureElement(document.getElementById('termKeyCtrl'), "document.getElementById('termKeyCtrl')").classList.contains('latched')), 'ctrl key latches');
    await mobile.keyboard.type('c');
    check(await mobile.evaluate(() => !fixtureElement(document.getElementById('termKeyCtrl'), "document.getElementById('termKeyCtrl')").classList.contains('latched')), 'latch clears after the next key');
