// Generated test/tool from test/browser-bounce.test.ts; edit that source and run npm run build:tests.
"use strict";
...
test('bounce previews narrow eligibility and copy valid blocker text', () => {
    ...
    const result = decodeBouncePreview(wire);
    const blockers = wire.targets[2]?.blockers;
    assert.ok(blockers);
    blockers.push('new');
