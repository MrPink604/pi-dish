...
import type * as Routines from '../lib/routines.js';
import type * as Runner from '../lib/routine-runner.js';
import type * as Handlers from '../lib/routine-handlers.js';
import type * as Bridge from '../lib/bridge-session.js';
import type * as SessionKeys from '../lib/session-key.js';
import type * as Protocol from '../lib/wire-protocol.js';
...
const store: typeof Routines = require('../lib/routines.js');
const { createRoutineRunner, STARTING_WATCHDOG_MS }: typeof Runner = require('../lib/routine-runner.js');
const { BridgeSession }: typeof Bridge = require('../lib/bridge-session.js');
const { validSessionId }: typeof SessionKeys = require('../lib/session-key.js');
const { isRecord }: typeof Protocol = require('../lib/wire-protocol.js');
const runners = new Set<Runner.RoutineRunner>();
test.after(() => {
  for (const runner of runners) runner.stop();
  fs.rmSync(tmpHome, { recursive: true, force: true });
  if (originalHome === undefined) delete process.env.HOME;
  else process.env.HOME = originalHome;
  if (originalCloseGrace === undefined) delete process.env.PI_DISH_ROUTINE_CLOSE_GRACE_MS;
  else process.env.PI_DISH_ROUTINE_CLOSE_GRACE_MS = originalCloseGrace;
});
...
type DefinitionPatch = Partial<Pick<Routines.Routine, 'name' | 'cwd' | 'prompt' | 'description' | 'harness' | 'mode' | 'onBusy' | 'thinking' | 'minIntervalSec' | 'schedule' | 'enabled'>>;
const definition = (over: DefinitionPatch = {}) => ({
  name: 'nightly-review',
  cwd: '/tmp/work',
...
function existingInvocation(id: unknown) {
  const entry = store.getInvocation(id);
  assert.ok(entry);
  return entry;
}
function updateRoutine(ref: unknown, patch: unknown) {
...
test('validation rejects bad names, cwds, prompts, enums and cron', () => {
  const bad: Array<[DefinitionPatch, string]> = [
    [{ name: 'Nightly' }, 'name'],
    [{ name: '-leading' }, 'name'],
...
test('only a prompt change bumps the version; the 50-entry cap keeps the current one', () => {
  ...
  for (let i = 3; i <= 60; i++) current = updateRoutine(routine.id, { prompt: `v${i}` });
  assert.equal(current.promptVersion, 60);
  assert.ok(Array.isArray(current.versions));
  assert.equal(current.versions.length, store.MAX_VERSIONS);
  const newest: unknown = current.versions[current.versions.length - 1];
  const oldest: unknown = current.versions[0];
...
test('the ledger is newest-first, capped, and indexable by session id', () => {
  ...
  const many: Array<{ id: string; routineId: string; startedAt: number; status: string }> = [];
...
test('admission owns its original input and rejects forged size receipts as raw input', () => {
  ...
  const prototypeForgery: unknown = Object.assign(Object.create(Object.getPrototypeOf(admission)), { input: huge, size: 0 });
...
class FakeSession extends BridgeSession {
  prompts: Array<{ text: string; opts: NonNullable<Parameters<InstanceType<typeof BridgeSession>['prompt']>[1]> }> = [];
  steers: string[] = [];
  names: string[] = [];
  ...
  override async prompt(text: string, opts: NonNullable<Parameters<InstanceType<typeof BridgeSession>['prompt']>[1]> = {}) {
    this.prompts.push({ text, opts }); this.turnInProgress = true;
  }
  override async steer(text: string) { this.steers.push(text); }
  override async setName(name: string) { this.names.push(name); }
...
function existingSession(sessions: ReadonlyMap<unknown, FakeSession>, id: unknown): FakeSession {
  const session = sessions.get(id);
  assert.ok(session);
  return session;
}

function harness(overrides: Partial<Runner.RoutineRunnerPorts> = {}) {
  const sessions = new Map<unknown, FakeSession>();
  const closed: unknown[] = [];
  const created: Array<Parameters<Runner.RoutineRunnerPorts['createSession']>[0]> = [];
  const resumed: unknown[] = [];
  let nextId = 1;
  const deps: Runner.RoutineRunnerPorts = {
    store,
...
test('an unparseable schedule is inert rather than fatal', async () => {
  ...
  const raw: unknown = JSON.parse(fs.readFileSync(path.join(dishDir, 'routines.json'), 'utf8'));
...
test('restart recovery reconciles starting, live-idle, live-working and gone runs', async () => {
  ...
  const live = new Map<unknown, FakeSession>();
...
test('admission measures the first toJSON value without normalizing later persistence', () => {
  ...
  let value: unknown = { note: 'admission' };
  ...
  try {
    ...
    assert.deepEqual(existingInvocation(invocation.id).input, later,
...
test('HTTP input admission retains JSON 413 versus native serialization rejection ownership', async () => {
  const { createRoutineHandlers }: typeof Handlers = require('../lib/routine-handlers.js');
  const routine = store.createRoutine(definition());
  const { runner } = harness();
  ...
  const injectInput: Handlers.RoutineHandler = (req, _res, next) => {
    req.body = { input: injectedInput, source: 42 };
    next();
  };
  const forwardInvocation: Handlers.RoutineHandler = (req, res, next) => {
  ...
  const observeNativeFailure: express.ErrorRequestHandler = (error: unknown, _req, res, _next) => {
