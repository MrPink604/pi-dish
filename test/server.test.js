// Generated test/tool from test/server.test.ts; edit that source and run npm run build:tests.
"use strict";
...
for (const key of ['PI_DISH_URL', 'PI_DISH_SHARE_PORT', 'PI_DISH_SHARE_HOST',
    'PI_DISH_SHARE_BASE_URL', 'PI_DISH_TERMINAL'])
...
const SERVER_URL_HOST = path.join(__dirname, 'fixtures', 'server-url-host.js');
...
const OMP_ROUTE_ID = encodeSessionKey('omp', (0, test_types_js_1.nativeId)(OMP_SESSION_ID));
...
fs.writeFileSync(path.join(sessionDir, `${REAL_CWD_ID}.jsonl`), [
...
fs.writeFileSync(path.join(bogusDir, `${NO_CWD_ID}.jsonl`), JSON.stringify({ type: 'message', message: { role: 'user', content: [{ type: 'text', text: 'headerless fixture' }], timestamp: '2026-07-04T12:00:01.000Z' } }) + '\n');
...
const OMP_BLOB_ROUTE_ID = encodeSessionKey('omp', (0, test_types_js_1.nativeId)(OMP_BLOB_SESSION_ID));
...
function responseObject(value) {
    return (0, test_types_js_1.record)(value);
}
function responseArray(value) {
    return (0, test_types_js_1.records)(value);
}
let base = '';
test.before(async () => {
    if (!server.listening)
...
const get = async (requestPath) => {
    ...
    return { status: response.status, body: responseObject(await response.json()) };
};
const getArray = async (requestPath) => {
    const response = await fetch(base + requestPath);
    return { status: response.status, body: responseArray(await response.json()) };
};
...
const put = async (requestPath, body) => {
    const response = await fetch(base + requestPath, {
...
const patch = async (requestPath, body) => {
    const response = await fetch(base + requestPath, {
...
const del = async (requestPath, body) => {
    const response = await fetch(base + requestPath, body === undefined ? { method: 'DELETE' } : {
...
const rawGet = (requestPath, headers = {}) => new Promise((resolve, reject) => {
    const req = http.get(base + requestPath, { headers }, res => {
        const chunks = [];
...
function usageRow(value) {
    const row = (0, test_types_js_1.record)(value);
    return {
        ...row,
        id: text(row.id),
        key: text(row.key),
        ...
        models: Array.isArray(row.models) ? row.models.map(usageRow) : [],
        unpricedCalls: Number(row.unpricedCalls ?? 0),
        priced: Boolean(row.priced),
        cost: Number(row.cost ?? 0),
        costs: numberRecord(row.costs),
        costUnavailable: numberRecord(row.costUnavailable),
    };
}
function usageSummary(value) {
    const summary = (0, test_types_js_1.record)(value), groups = (0, test_types_js_1.record)(summary.groups);
    return {
        ...summary,
        range: text(summary.range),
        sort: text(summary.sort),
        models: Array.isArray(summary.models) ? summary.models.map(text) : [],
        unpricedModelCalls: Number(summary.unpricedModelCalls ?? 0),
        totals: usageRow(summary.totals),
        groups: {
            ...groups,
            models: Array.isArray(groups.models) ? groups.models.map(usageRow) : [],
            workspaces: Array.isArray(groups.workspaces) ? groups.workspaces.map(usageRow) : [],
            sessions: Array.isArray(groups.sessions) ? groups.sessions.map(usageRow) : [],
        },
        daily: Array.isArray(summary.daily) ? summary.daily.map(usageRow) : [],
        headlineCosts: numberRecord(summary.headlineCosts),
        headlineCostsByBucket: Object.fromEntries(Object.entries((0, test_types_js_1.record)(summary.headlineCostsByBucket)).map(([key, row]) => [key, numberRecord(row)])),
        headlineCostUnavailable: numberRecord(summary.headlineCostUnavailable),
    };
}
test('GET /api/sessions lists the fixture session with derived metadata', async () => {
    const { status, body } = await get('/api/sessions');
    assert.equal(status, 200);
    const sess = (0, test_types_js_1.present)((0, test_types_js_1.records)(body.previous).find((s) => s.id === SESSION_ID));
...
test('GET /api/sessions client view omits server-only routing metadata', async () => {
    ...
    const nested = (0, test_types_js_1.present)((0, test_types_js_1.records)(body.previous).find((s) => s.id === NESTED_SESSION_ID));
...
test('malformed historical metadata falls back before client projection', async () => {
    ...
    try {
        ...
        assert.ok((0, test_types_js_1.records)(full.body.previous).some((row) => row.id === badId), 'fixture reaches the projection');
        const client = await get('/api/sessions?view=client');
        assert.equal(client.status, 200);
        assert.ok((0, test_types_js_1.records)(client.body.previous).some((row) => row.id === SESSION_ID));
        assert.equal((0, test_types_js_1.records)(client.body.previous).find((row) => row.id === badId)?.name, badId.slice(0, 8));
        assert.equal((0, test_types_js_1.records)(client.body.previous).find((row) => row.id === badId)?.model, 'unknown');
    }
    finally {
...
test('encoded alternative-harness routes never fall back to a partial native id', async () => {
    ...
    const partial = await get(`/api/sessions/${encodeURIComponent(encodeSessionKey('omp', (0, test_types_js_1.nativeId)('omp-prefix')))}/stats`);
...
test('OMP sibling-directory subagents are addressable and appear as session relations', async () => {
    ...
    const childRouteId = encodeSessionKey('omp', (0, test_types_js_1.nativeId)(childNativeId));
    ...
    const child = (0, test_types_js_1.present)((0, test_types_js_1.records)(listed.body.previous).find((session) => session.id === childRouteId));
    ...
    assert.ok((0, test_types_js_1.records)(childRelated.body.relations).some((relation) => relation.kind === 'parent' && relation.source === 'omp-subsession-layout' && (0, test_types_js_1.record)(relation.session).id === OMP_ROUTE_ID));
    const parentRelated = await get(`/api/sessions/${encodeURIComponent(OMP_ROUTE_ID)}/related`);
    assert.ok((0, test_types_js_1.records)(parentRelated.body.relations).some((relation) => relation.kind === 'child' && relation.source === 'omp-subsession-layout' && (0, test_types_js_1.record)(relation.session).id === childRouteId));
    const messages = await get(`/api/sessions/${encodeURIComponent(childRouteId)}/messages?limit=10`);
    assert.equal(messages.status, 200);
    const firstMessage = (0, test_types_js_1.present)((0, test_types_js_1.records)(messages.body.messages)[0]);
    assert.equal((0, test_types_js_1.present)((0, test_types_js_1.records)(firstMessage.content)[0]).text, 'Inspect the OMP code');
});
test('live subagent sessions surface under their live parent on the Active tab', async () => {
    ...
    const runningRoute = encodeSessionKey('omp', (0, test_types_js_1.nativeId)(runningId));
    const disposedRoute = encodeSessionKey('omp', (0, test_types_js_1.nativeId)(disposedId));
    ...
    try {
        ...
        const children = (0, test_types_js_1.records)(activeOnly.body.children).map((session) => session.id);
        ...
        const running = (0, test_types_js_1.present)((0, test_types_js_1.records)(activeOnly.body.children).find((session) => session.id === runningRoute));
        assert.equal(running.subagentLive, true);
        assert.equal(running.isActive, false, 'its parent owns it — pi-dish cannot drive it');
        assert.equal((0, test_types_js_1.record)(running.capabilities).prompt, false);
        assert.equal((0, test_types_js_1.record)(running.capabilities).resume, false, 'resuming would append to a file the parent holds');
        assert.equal(running.familyParentId, OMP_ROUTE_ID, 'nests under the live parent in the sidebar');
        assert.equal(running.name, 'Running', 'the row is labelled by its agent handle, not the shared delegation prompt');
        const full = await get('/api/sessions');
        assert.deepEqual(full.body.children, [], 'a full list already carries them in previous');
        const fullRunning = (0, test_types_js_1.present)((0, test_types_js_1.records)(full.body.previous).find((session) => session.id === runningRoute));
        assert.equal(fullRunning.subagentLive, true, 'the All tab marks the same row live');
        assert.equal((0, test_types_js_1.record)(fullRunning.capabilities).resume, false);
        const fullDisposed = (0, test_types_js_1.present)((0, test_types_js_1.records)(full.body.previous).find((session) => session.id === disposedRoute));
        assert.ok(fullDisposed, 'the disposed subagent stays in history');
        assert.ok(!fullDisposed.subagentLive);
        assert.equal((0, test_types_js_1.record)(fullDisposed.capabilities).resume, true);
        ...
        assert.match(text(refused.body.error), /subagent still loaded in its parent/);
    }
    finally {
...
test('GET /api/host/health reports capacity plus live sessions and subagents from the catalog', async () => {
    ...
    const cpu = (0, test_types_js_1.record)(body.cpu);
    ...
    const memory = (0, test_types_js_1.record)(body.memory);
    ...
    if (body.disk !== null) {
        const disk = (0, test_types_js_1.record)(body.disk);
    ...
    const baseline = (0, test_types_js_1.record)(body.sessions);
    for (const key of ['live', 'working', 'waiting', 'subagents'])
        assert.ok(numeric(baseline[key]), key);
    ...
    try {
        ...
        const sessions = (0, test_types_js_1.record)(during.body.sessions);
        const active = await get('/api/sessions?active=1');
        assert.equal(sessions.live, (0, test_types_js_1.records)(active.body.active).length, 'live matches the Active tab');
        assert.equal(sessions.live, Number(baseline.live) + 1);
        assert.equal(sessions.subagents, (0, test_types_js_1.records)(active.body.children).length);
        assert.ok(Number(sessions.subagents) >= 1, 'the running subagent is counted');
        assert.ok(Number((0, test_types_js_1.record)(sessions.byHarness).omp) >= 1, 'live sessions are grouped by harness');
    }
    finally {
        fs.rmSync(registryPath, { force: true });
    ...
    assert.equal((0, test_types_js_1.record)(after.body.sessions).live, baseline.live, 'an unregistered session stops counting');
});
test('fresh live routes distinguish missing history, then use the session file before indexing', async () => {
    ...
    const routeId = encodeSessionKey('omp', (0, test_types_js_1.nativeId)(nativeId));
    ...
    try {
        ...
        assert.match(text(tree.body.error), /no reachable live bridge for tree reads/i);
...
test('a wrapper bridge claiming a session hides the stock pi bridge riding in the same process', async () => {
    ...
    const ompRouteId = encodeSessionKey('omp', (0, test_types_js_1.nativeId)(nativeId));
    ...
    try {
        ...
        assert.ok((0, test_types_js_1.records)(active.body.active).some((s) => s.harnessId === 'pi' && s.nativeSessionId === nativeId), 'lone stock bridge claim is listed');
        ...
        assert.ok(!(0, test_types_js_1.records)(active.body.active).some((s) => s.nativeSessionId === nativeId), 'a pi claim on a wrapper-corpus file is hidden even without a competing wrapper claim');
        ...
        invalidateRegistryCache();
        active = await get('/api/sessions?active=1');
        const claimed = (0, test_types_js_1.records)(active.body.active).filter((s) => s.nativeSessionId === nativeId);
        assert.equal(claimed.length, 1, `exactly one row for the doubly-claimed session: ${JSON.stringify(claimed.map(session => session.harnessId))}`);
...
test('historical alternative-harness IDs associate pages and workspace choices canonically', async () => {
    ...
    const cwds = await getArray('/api/cwds');
...
test('alternative harnesses do not decode Pi workspace directory names as cwd', async () => {
    ...
    fs.writeFileSync(path.join(misleadingDir, `${nativeId}.jsonl`), JSON.stringify({ type: 'message', message: { role: 'user', content: 'no cwd header' } }) + '\n');
    const sessions = await get('/api/sessions');
    const routeId = encodeSessionKey('omp', (0, test_types_js_1.nativeId)(nativeId));
    const listed = (0, test_types_js_1.present)((0, test_types_js_1.records)(sessions.body.previous).find((entry) => entry.id === routeId));
...
test('nested generic sessions use the core header id and remain fully addressable', async () => {
    const listed = await get('/api/sessions');
    const nested = (0, test_types_js_1.present)((0, test_types_js_1.records)(listed.body.previous).find((s) => s.id === NESTED_SESSION_ID));
    ...
    const filteredChild = (0, test_types_js_1.present)((0, test_types_js_1.records)(filtered.body.previous).find((s) => s.id === NESTED_SESSION_ID));
    assert.equal(filteredChild.familyParentId, SESSION_ID, 'filtered child retains stable family identity when the parent row is absent');
    const messages = await get(`/api/sessions/${NESTED_SESSION_ID}/messages?limit=10`);
    assert.equal(messages.status, 200);
    const firstMessage = (0, test_types_js_1.present)((0, test_types_js_1.records)(messages.body.messages)[0]);
    assert.equal((0, test_types_js_1.present)((0, test_types_js_1.records)(firstMessage.content)[0]).text, 'nested peer question');
    const related = await get(`/api/sessions/${NESTED_SESSION_ID}/related`);
    assert.equal(related.status, 200);
    const parent = (0, test_types_js_1.present)((0, test_types_js_1.records)(related.body.relations).find((r) => r.kind === 'parent' && r.source === 'pi-session-header'));
    assert.equal((0, test_types_js_1.record)(parent.session).id, SESSION_ID);
    const inverse = await get(`/api/sessions/${SESSION_ID}/related`);
    const child = (0, test_types_js_1.present)((0, test_types_js_1.records)(inverse.body.relations).find((r) => r.kind === 'child' && r.source === 'pi-session-header'));
    assert.equal((0, test_types_js_1.record)(child.session).id, NESTED_SESSION_ID);
});
test('stale native parent paths do not attach by basename alone', async () => {
    ...
    try {
        const listed = await get('/api/sessions');
        const stale = (0, test_types_js_1.present)((0, test_types_js_1.records)(listed.body.previous).find((s) => s.id === id));
        assert.ok(stale);
...
test('alien JSONL basenames are skipped without breaking sessions or usage APIs', async () => {
    ...
    try {
        ...
        assert.equal((0, test_types_js_1.records)(listed.body.previous).some((session) => session.sessionFile === alien), false);
...
test('a newly ambiguous nested header id invalidates route lookup', async () => {
    ...
    try {
        ...
        assert.equal(routed.body.totalMessages, 0, 'route access revalidates generic ambiguity before the next full list scan');
        const listed = await get('/api/sessions');
        assert.equal((0, test_types_js_1.records)(listed.body.previous).some((s) => s.id === NESTED_SESSION_ID), false);
    }
    finally {
...
test('pi-dish launch provenance adds neutral related-session navigation', async () => {
    ...
    const listed = await get('/api/sessions');
    const launched = (0, test_types_js_1.present)((0, test_types_js_1.records)(listed.body.previous).find((s) => s.id === REAL_CWD_ID));
    ...
    assert.ok((0, test_types_js_1.records)(source.body.relations).some((r) => r.kind === 'startedHere' && (0, test_types_js_1.record)(r.session).id === REAL_CWD_ID));
    const child = await get(`/api/sessions/${REAL_CWD_ID}/related`);
    assert.ok((0, test_types_js_1.records)(child.body.relations).some((r) => r.kind === 'startedFrom' && (0, test_types_js_1.record)(r.session).id === SESSION_ID));
});
test('lineage endpoint assembles the recursive family tree', async () => {
    ...
    try {
        ...
        assert.equal((0, test_types_js_1.record)(res.body.session).id, rootId);
        assert.equal(res.body.members, 3);
        assert.equal(res.body.truncated, false);
        const tree = (0, test_types_js_1.record)(res.body.tree);
        assert.equal((0, test_types_js_1.record)(tree.session).id, rootId, 'tree roots at the top ancestor');
        assert.equal(tree.edge, null);
        const children = (0, test_types_js_1.records)(tree.children);
        assert.equal(children.length, 1);
        const child = (0, test_types_js_1.present)(children[0]), grandchildren = (0, test_types_js_1.records)(child.children);
        assert.equal((0, test_types_js_1.record)(child.session).id, childId);
        assert.deepEqual(child.edge, { kind: 'child', source: 'pi-session-header' });
        assert.equal((0, test_types_js_1.record)((0, test_types_js_1.present)(grandchildren[0]).session).id, gcId, 'grandchild nests under the child');
        assert.equal((0, test_types_js_1.record)(child.session).turnInProgress, false, 'summaries carry busy state for the signal UI');
        assert.ok((0, test_types_js_1.record)(child.session).capabilities, 'summaries carry capability advice');
        assert.equal((0, test_types_js_1.record)((0, test_types_js_1.record)(child.session).capabilities).prompt, false, 'an inactive relative cannot be prompted');
        assert.equal(typeof (0, test_types_js_1.record)((0, test_types_js_1.record)(child.session).capabilities).resume, 'boolean');
        const fromGc = await get(`/api/sessions/${gcId}/lineage`);
        const fromGcTree = (0, test_types_js_1.record)(fromGc.body.tree);
        assert.equal((0, test_types_js_1.record)(fromGcTree.session).id, rootId, 'a descendant sees the same root');
        assert.equal((0, test_types_js_1.record)(fromGc.body.session).id, gcId);
        const missing = await get('/api/sessions/no-such-lineage-session/lineage');
        assert.equal(missing.status, 404);
    }
    finally {
        for (const file of [rootFile, childFile, gcFile])
            fs.rmSync(file, { force: true });
        await get('/api/sessions');
    }
});
test('lineage terminates on cyclic parent headers', async () => {
    ...
    const writeRow = (file, id, parentSession) => fs.writeFileSync(file, JSON.stringify({ type: 'session', version: 3, id, cwd: '/home/user/proj', parentSession }) + '\n');
...
test('lineage marks pi-dish launch edges and respects the node cap', async () => {
    ...
    try {
        ...
        const fullTree = (0, test_types_js_1.record)(full.body.tree);
        const childNode = (0, test_types_js_1.present)((0, test_types_js_1.records)(fullTree.children).find(node => (0, test_types_js_1.record)(node.session).id === childId));
        assert.ok(childNode, 'launched session joins the tree below its source');
        const childEdge = (0, test_types_js_1.record)(childNode.edge);
        assert.equal(childEdge.kind, 'startedHere');
        assert.equal(childEdge.source, 'pi-dish-launch');
        ...
        if (cap === undefined)
            delete process.env.PI_DISH_LINEAGE_NODE_CAP;
        else
            process.env.PI_DISH_LINEAGE_NODE_CAP = cap;
...
test('prime RLM subagents join the catalog and the lineage tree', async () => {
    ...
    const rootRoute = encodeSessionKey('prime', (0, test_types_js_1.nativeId)(rootNative));
    const childRoute = encodeSessionKey('prime', (0, test_types_js_1.nativeId)(childNative));
    const gcRoute = encodeSessionKey('prime', (0, test_types_js_1.nativeId)(gcNative));
    try {
        const listed = await get('/api/sessions');
        const child = (0, test_types_js_1.present)((0, test_types_js_1.records)(listed.body.previous).find((session) => session.id === childRoute));
        assert.ok(child, 'prime subagent is listed from the artifacts tree');
        assert.equal(child.parentId, rootRoute);
        const gc = (0, test_types_js_1.present)((0, test_types_js_1.records)(listed.body.previous).find((session) => session.id === gcRoute));
        ...
        const lineageTree = (0, test_types_js_1.record)(lineage.body.tree);
        const lineageChild = (0, test_types_js_1.present)((0, test_types_js_1.records)(lineageTree.children)[0]);
        const lineageGrandchild = (0, test_types_js_1.present)((0, test_types_js_1.records)(lineageChild.children)[0]);
        assert.equal((0, test_types_js_1.record)(lineageTree.session).id, rootRoute);
        assert.equal((0, test_types_js_1.record)(lineageChild.session).id, childRoute);
        assert.equal((0, test_types_js_1.record)(lineageGrandchild.session).id, gcRoute);
        assert.equal(lineage.body.members, 3);
        const childRelated = await get(`/api/sessions/${encodeURIComponent(childRoute)}/related`);
        assert.ok((0, test_types_js_1.records)(childRelated.body.relations).some((relation) => relation.kind === 'parent' && (0, test_types_js_1.record)(relation.session).id === rootRoute), 'the flat relations endpoint sees prime parents too');
    }
    finally {
...
test('live prime subagents surface under their live parent via the display entry', async () => {
    ...
    const runningRoute = encodeSessionKey('prime', (0, test_types_js_1.nativeId)(runningNative));
    const doneRoute = encodeSessionKey('prime', (0, test_types_js_1.nativeId)(doneNative));
    ...
    try {
        ...
        const running = (0, test_types_js_1.present)((0, test_types_js_1.records)(activeOnly.body.children).find((session) => session.id === runningRoute));
        assert.equal(running.subagentLive, true);
        assert.equal(running.parentId, encodeSessionKey('prime', (0, test_types_js_1.nativeId)(rootNative)));
        const related = await get(`/api/sessions/${encodeURIComponent(encodeSessionKey('prime', (0, test_types_js_1.nativeId)(rootNative)))}/related`);
        const runningRelation = (0, test_types_js_1.present)((0, test_types_js_1.records)(related.body.relations).find((relation) => (0, test_types_js_1.record)(relation.session).id === runningRoute));
        assert.equal((0, test_types_js_1.record)(runningRelation.session).subagentLive, true, 'relation summaries carry the live-subagent flag');
    }
    finally {
        fs.rmSync(registryPath, { force: true });
        fs.rmSync(primeDir, { recursive: true, force: true });
...
test('cwd falls back to the dir-name decode only when the decoded path exists', async () => {
    ...
    const sess = (0, test_types_js_1.present)((0, test_types_js_1.records)(body.previous).find((s) => s.id === NO_CWD_ID));
...
test('GET /api/sessions?q= filters on message content', async () => {
    ...
    const sess = (0, test_types_js_1.present)((0, test_types_js_1.records)(hit.body.previous).find((s) => s.id === SESSION_ID));
    assert.ok(sess);
    // Content matches carry a snippet showing why the row is in the results…
    assert.ok(text(sess.searchSnippet).includes('bravo'), `snippet shows the hit: ${sess.searchSnippet}`);
    const miss = await get('/api/sessions?q=zzz-not-there');
    assert.ok(!(0, test_types_js_1.records)(miss.body.previous).some((s) => s.id === SESSION_ID));
    // …metadata matches (name/cwd/model/id) don't need one.
    const byName = await get('/api/sessions?q=hello');
    const metaMatch = (0, test_types_js_1.present)((0, test_types_js_1.records)(byName.body.previous).find((s) => s.id === SESSION_ID));
...
test('GET /api/sessions?active=1&q= content-matches registered sessions', async () => {
    ...
    try {
        ...
        const sess = (0, test_types_js_1.present)((0, test_types_js_1.records)(hit.body.active).find((s) => s.id === ID));
        ...
        assert.ok(!(0, test_types_js_1.records)(miss.body.active).some((s) => s.id === ID), 'non-matching query filters it out');
    }
    finally {
...
test('GET /api/sessions?q= speaks the filter grammar: negation, fields, dates', async () => {
    ...
    assert.ok((0, test_types_js_1.records)(negContent.body.previous).some((s) => s.id === SESSION_ID), 'content-only word must not exclude via negation');
    const negName = await get(`/api/sessions?q=${encodeURIComponent('-name:hello')}`);
    assert.ok(!(0, test_types_js_1.records)(negName.body.previous).some((s) => s.id === SESSION_ID), 'name negation excludes');
    ...
    assert.ok((0, test_types_js_1.records)(byCwd.body.previous).some((s) => s.id === SESSION_ID));
    const byWrongField = await get(`/api/sessions?q=${encodeURIComponent('name:proj')}`);
    assert.ok(!(0, test_types_js_1.records)(byWrongField.body.previous).some((s) => s.id === SESSION_ID));
    ...
    assert.ok((0, test_types_js_1.records)(inRange.body.previous).some((s) => s.id === DATED_ID));
    const tooOld = await get(`/api/sessions?q=${encodeURIComponent('dated since:2026-07-01')}`);
    assert.ok(!(0, test_types_js_1.records)(tooOld.body.previous).some((s) => s.id === DATED_ID));
    ...
    const sess = (0, test_types_js_1.present)((0, test_types_js_1.records)(combo.body.previous).find((s) => s.id === SESSION_ID));
    assert.ok(text(sess.searchSnippet).includes('bravo'), 'snippet from the positive term');
});
...
test('GET /api/sessions?q= ranks by relevance, recency only as tiebreaker', async () => {
    ...
    const ids = (0, test_types_js_1.records)(body.previous).map(session => session.id).filter(id => id === RANK_NAME_ID || id === RANK_CONTENT_ID);
    assert.deepEqual(ids, [RANK_NAME_ID, RANK_CONTENT_ID], 'the name match outranks the newer session that only repeats the word');
    const named = (0, test_types_js_1.present)((0, test_types_js_1.records)(body.previous).find((s) => s.id === RANK_NAME_ID));
    const shouted = (0, test_types_js_1.present)((0, test_types_js_1.records)(body.previous).find((s) => s.id === RANK_CONTENT_ID));
    assert.ok(Number(named.searchScore) > Number(shouted.searchScore), `scores exposed to the client (${named.searchScore} vs ${shouted.searchScore})`);
    ...
    assert.ok(text(shouted.searchSnippet).includes('orchid'));
    ...
    const metaTimes = (0, test_types_js_1.records)(meta.body.previous).map(session => new Date(text(session.lastActivity) || 0).getTime());
    assert.ok(metaTimes.every((timestamp, index) => index === 0 || timestamp <= (0, test_types_js_1.present)(metaTimes[index - 1])), 'recency order');
    assert.equal((0, test_types_js_1.present)((0, test_types_js_1.records)(meta.body.previous)[0]).searchScore, undefined);
});
test('GET /api/search returns flat results with multi-snippets and match counts', async () => {
    ...
    assert.ok(Number(body.total) >= 1);
    const sess = (0, test_types_js_1.present)((0, test_types_js_1.records)(body.results).find((s) => s.id === SESSION_ID));
    assert.ok(sess, 'fixture session in results');
    assert.ok(Number(sess.matchCount) >= 2, `content occurrences counted (got ${sess.matchCount})`);
    const snippets = Array.isArray(sess.snippets) ? sess.snippets.map(text) : [];
    assert.ok(snippets.length >= 1 && snippets.every(snippet => snippet.includes('alpha')), 'every snippet shows the token');
    ...
    const metaSess = (0, test_types_js_1.present)((0, test_types_js_1.records)(byCwd.body.results).find((s) => s.id === SESSION_ID));
    assert.ok(metaSess);
    ...
    assert.ok(!(0, test_types_js_1.records)(neg.body.results).some((s) => s.id === SESSION_ID));
    const liveOnly = await get(`/api/search?q=${encodeURIComponent('alpha is:active')}`);
    assert.ok(!(0, test_types_js_1.records)(liveOnly.body.results).some((s) => s.id === SESSION_ID), 'historical session excluded by is:active');
    ...
    const rankIds = (0, test_types_js_1.records)(ranked.body.results).map(session => session.id);
    assert.deepEqual(rankIds, [RANK_NAME_ID, RANK_CONTENT_ID]);
    const rankedResults = (0, test_types_js_1.records)(ranked.body.results);
    assert.ok(Number((0, test_types_js_1.present)(rankedResults[0]).searchScore) > Number((0, test_types_js_1.present)(rankedResults[1]).searchScore), 'result objects carry the score they were ranked by');
    assert.ok((0, test_types_js_1.present)(rankedResults[0]).matchCount === 1 && Number((0, test_types_js_1.present)(rankedResults[1]).matchCount) > 50, `occurrence counts stay independent of the ranking ${JSON.stringify(rankedResults.map(result => [result.id, result.matchCount]))}`);
    // Empty query browses everything, recency-first.
    const all = await get('/api/search?q=');
    assert.ok((0, test_types_js_1.records)(all.body.results).length >= 3);
    const times = (0, test_types_js_1.records)(all.body.results).map(session => new Date(text(session.lastActivity) || 0).getTime());
    assert.ok(times.every((timestamp, index) => index === 0 || timestamp <= (0, test_types_js_1.present)(times[index - 1])), 'recency order');
});
test('GET /api/search applies saved scope before the 100-result cap', async (t) => {
    ...
    t.after(() => {
        if (oldBudget === undefined)
            delete process.env.PI_DISH_INDEX_SYNC_BUDGET;
        else
            process.env.PI_DISH_INDEX_SYNC_BUDGET = oldBudget;
    ...
    for (let i = 0; i < 100; i++) {
        writeCapSession(`2026-07-02T00-00-${String(i).padStart(2, '0')}-scope-filler`, '/workspace/scope-other', new Date(Date.parse('2026-07-02T00:00:00.000Z') + i * 1000).toISOString());
    }
...
test('GET /messages returns the tail with indexes', async () => {
    ...
    assert.deepEqual((0, test_types_js_1.records)(body.messages).map((m) => m.index), [0, 1, 2, 3, 4]);
    assert.deepEqual((0, test_types_js_1.records)(body.messages).map((m) => m.role), ['user', 'assistant', 'toolResult', 'user', 'assistant']);
    // Entry id + generation stats ride the message (share deep links, tok/s).
    const last = (0, test_types_js_1.present)((0, test_types_js_1.records)(body.messages)[4]);
...
test('GET /messages honors limit / before / after cursors', async () => {
    ...
    assert.deepEqual((0, test_types_js_1.records)(tail.body.messages).map((m) => m.index), [3, 4]);
    assert.equal(tail.body.hasMore, true);
    const older = await get(`/api/sessions/${SESSION_ID}/messages?limit=2&before=3`);
    assert.deepEqual((0, test_types_js_1.records)(older.body.messages).map((m) => m.index), [1, 2]);
    assert.equal(older.body.hasMore, true);
    const catchup = await get(`/api/sessions/${SESSION_ID}/messages?after=2`);
    assert.deepEqual((0, test_types_js_1.records)(catchup.body.messages).map((m) => m.index), [3, 4]);
    ...
    assert.deepEqual((0, test_types_js_1.records)(bogus.body.messages).map((m) => m.index), [3, 4]);
    assert.ok((0, test_types_js_1.records)(bogus.body.messages).every((m) => Number.isFinite(m.index)), 'indexes stay numeric');
});
test('GET /messages moves historical image bytes to a cacheable resource', async () => {
    const { body } = await get(`/api/sessions/${SESSION_ID}/messages`);
    const messages = (0, test_types_js_1.records)(body.messages);
    const thirdMessage = (0, test_types_js_1.present)(messages[2]);
    const image = (0, test_types_js_1.present)((0, test_types_js_1.records)(thirdMessage.content).find(block => block.type === 'image'));
    assert.ok(image.url, 'message payload carries an image resource URL');
    assert.match(text(image.url), /\/messages\/img00001\/images\/1$/, 'resource identity is the stable JSONL entry id');
    assert.equal(image.data, undefined, 'base64 bytes are not duplicated into chat JSON');
    const res = await fetch(base + text(image.url));
...
test('OMP blob-store image refs resolve to the stored bytes', async () => {
    ...
    const images = (0, test_types_js_1.records)(body.messages).flatMap(message => (0, test_types_js_1.records)(message.content))
        .filter(block => block.type === 'image');
    assert.equal(images.length, 2);
    assert.ok(images.every(block => block.url && block.data === undefined), 'blob refs are projected to resource URLs, never sent to the client as bogus base64');
    const ok = await fetch(base + text((0, test_types_js_1.present)(images[0]).url));
    assert.equal(ok.status, 200);
    assert.equal(ok.headers.get('content-type'), 'image/png');
    assert.deepEqual(Buffer.from(await ok.arrayBuffer()), TINY_PNG, 'bytes come from ~/.omp/agent/blobs, not from base64-decoding the ref');
    const missing = await fetch(base + text((0, test_types_js_1.present)(images[1]).url));
...
test('historical image resource stays stable after active branch navigation', async () => {
    ...
    const image = (0, test_types_js_1.present)((0, test_types_js_1.records)(before.body.messages).flatMap(message => (0, test_types_js_1.records)(message.content))
        .find(block => block.type === 'image'));
    assert.match(text(image.url), /\/messages\/image001\/images\/0$/);
    const first = await fetch(base + text(image.url));
    ...
    assert.equal((0, test_types_js_1.records)(after.body.messages).some(message => (0, test_types_js_1.records)(message.content).some(block => block.type === 'image')), false, 'image entry is no longer on the active branch');
    const second = await fetch(base + text(image.url));
    assert.equal(second.status, 200, 'old lazy resource URL remains addressable');
    assert.deepEqual(Buffer.from(await second.arrayBuffer()), TINY_PNG, 'the stable URL still resolves to the original entry bytes');
});
test('large chat JSON negotiates gzip and materially reduces wire bytes', async () => {
    ...
    assert.ok(gzip.body.length < identity.body.length * 0.5, `expected at least 50% savings (${identity.body.length} -> ${gzip.body.length})`);
});
...
test('GET /search requires all tokens to match within one message', async () => {
    ...
    assert.deepEqual((0, test_types_js_1.records)(both.body.matches).map((m) => m.index), [3]);
...
test('GET /search mode=any returns relevant messages for session-wide advanced matches', async () => {
    ...
    assert.ok((0, test_types_js_1.records)(advanced.body.results).some((s) => s.id === SESSION_ID), 'advanced search keeps session-wide AND semantics');
...
test('GET /api/dirs fuzzy-finds directories under $HOME', async () => {
    const { status, body } = await getArray('/api/dirs?q=alpha');
    assert.equal(status, 200);
    const hit = (0, test_types_js_1.present)(body.find(directory => directory.path === realCwd));
    assert.equal(hit.short, '~/workspace/proj-alpha');
    // typo tolerance comes from in-order char matching
    const fuzzy = await getArray('/api/dirs?q=wkspalpha');
    assert.ok(fuzzy.body.some(directory => directory.path === realCwd));
});
test('GET /api/dirs/children lists subdirs, excluding dotdirs and heavies', async () => {
    ...
    const dirs = (0, test_types_js_1.records)(body.dirs);
    const names = dirs.map(directory => directory.name);
    assert.deepEqual(names, ['alpha', 'beta'], 'dotdir + node_modules + file excluded, sorted');
    assert.equal((0, test_types_js_1.present)(dirs[0]).path, path.join(treeRoot, 'alpha'));
    // ~ expands to $HOME
    const home = await get('/api/dirs/children?path=' + encodeURIComponent('~'));
    assert.equal(home.status, 200);
    assert.equal(home.body.path, tmpHome);
    assert.ok((0, test_types_js_1.records)(home.body.dirs).some((d) => d.name === 'treeroot'));
    // non-absolute → 400
...
test('GET /files fuzzy-searches the session cwd', async () => {
    ...
    assert.ok((0, test_types_js_1.records)(body.files).some((f) => f.path === 'src/main.js'));
});
test('GET /files completes absolute paths outside the session cwd', async () => {
    ...
    assert.equal(status, 200);
    const dirHit = (0, test_types_js_1.present)((0, test_types_js_1.records)(body.files).find((f) => f.path === path.join(tmpHome, 'scratch')));
    ...
    const fileHit = (0, test_types_js_1.present)((0, test_types_js_1.records)(body.files).find((f) => f.path === path.join(tmpHome, 'scratch', 'notes.md')));
...
test('GET /files completes ~ and ../ tokens, preserving the typed form', async () => {
    ...
    assert.ok((0, test_types_js_1.records)(body.files).some((f) => f.path === '~/scratch/notes.md'));
    // dotfiles hidden unless the partial starts with a dot
    ({ body } = await get(`/api/sessions/${REAL_CWD_ID}/files?q=${encodeURIComponent('~/')}`));
    assert.ok(!(0, test_types_js_1.records)(body.files).some((f) => text(f.path).includes('/.')), 'no dotfiles for a bare listing');
    ({ body } = await get(`/api/sessions/${REAL_CWD_ID}/files?q=${encodeURIComponent('~/.p')}`));
    assert.ok((0, test_types_js_1.records)(body.files).some((f) => f.path === '~/.pi' && f.isDir));
    // ../ resolves against the session cwd
    ({ body } = await get(`/api/sessions/${REAL_CWD_ID}/files?q=${encodeURIComponent('../proj-al')}`));
    assert.ok((0, test_types_js_1.records)(body.files).some((f) => f.path === '../proj-alpha' && f.isDir));
});
...
test('GET /file returns image metadata while a resource route serves the bytes', async () => {
    ...
    assert.equal(status, 200);
    assert.equal((0, test_types_js_1.record)(body.image).mimeType, 'image/png');
    assert.ok((0, test_types_js_1.record)(body.image).url, 'viewer JSON carries a resource URL');
    assert.equal((0, test_types_js_1.record)(body.image).data, undefined, 'viewer JSON does not carry base64 bytes');
    const res = await fetch(base + text((0, test_types_js_1.record)(body.image).url));
    assert.equal(res.status, 200);
    assert.equal(res.headers.get('content-type'), 'image/png');
    assert.deepEqual(Buffer.from(await res.arrayBuffer()), TINY_PNG);
});
...
test('GET /diff aggregates uncommitted changes across repos under the session cwd', async (t) => {
    ...
    const entry = (0, test_types_js_1.present)((0, test_types_js_1.records)(body.repos).find((r) => r.path === 'repo-x'));
    assert.ok(entry, 'repo under the cwd is discovered');
    assert.equal(entry.branch, 'main');
    const byPath = new Map((0, test_types_js_1.records)(entry.files).map(file => [text(file.path), file]));
    assert.equal((0, test_types_js_1.record)(byPath.get('a.txt')).status, 'M');
    assert.ok(text((0, test_types_js_1.record)(byPath.get('a.txt')).patch).includes('+two'));
    assert.equal((0, test_types_js_1.record)(byPath.get('new.txt')).status, '?');
    assert.ok(text((0, test_types_js_1.record)(byPath.get('new.txt')).patch).includes('+fresh'), 'untracked files get synthesized patches');
});
test('large diff summaries defer collapsed patches and serve one on demand', async (t) => {
    ...
    git(repo, 'init', '-q', '-b', 'main');
    for (let i = 0; i < 7; i++)
        fs.writeFileSync(path.join(repo, `file-${i}.txt`), `old ${i}\n`);
    git(repo, 'add', '-A');
    git(repo, 'commit', '-q', '-m', 'init');
    for (let i = 0; i < 7; i++)
        fs.writeFileSync(path.join(repo, `file-${i}.txt`), `old ${i}\nnew ${i}\n`);
    const { body } = await get(`/api/sessions/${REAL_CWD_ID}/diff`);
    const entry = (0, test_types_js_1.present)((0, test_types_js_1.records)(body.repos).find((r) => r.path === 'repo-big'));
    assert.ok(entry, 'large repo is present in the summary');
    const entryFiles = (0, test_types_js_1.records)(entry.files);
    assert.equal(entryFiles.length, 7);
    assert.ok(entryFiles.every(file => file.patch === undefined && file.patchDeferred === true), 'collapsed patch text is absent from the summary payload');
    const query = new URLSearchParams({
        repo: 'repo-big', path: 'file-3.txt', snapshot: text(body.snapshotId),
    });
    const patch = await get(`/api/sessions/${REAL_CWD_ID}/diff/patch?${query}`);
    assert.equal(patch.status, 200);
    assert.match(text(patch.body.patch), /\+new 3/);
...
test('POST /api/pages publishes a file and /page/:token serves it live from disk', async () => {
    ...
    const res = await fetch(base + text(body.path));
    assert.equal(res.status, 200);
    ...
    assert.match(await (await fetch(base + text(body.path))).text(), /<h1>the revised plan<\/h1>/);
    ...
    const list = await getArray(`/api/pages?sessionId=${REAL_CWD_ID}`);
    assert.ok(list.body.some(page => page.token === body.token && page.missing === false));
    const revoked = await del(`/api/pages/${body.token}`);
    assert.equal(revoked.body.revoked, true);
    assert.equal((await fetch(base + text(body.path))).status, 404);
});
test('file-viewer pages render as standalone read-only documents', async () => {
    ...
    const response = await fetch(base + text(published.body.path));
    ...
    assert.doesNotMatch(html, /artifact-comments|commentBubble|app\.js/, 'the standalone renderer has no app or commenting UI');
    ...
    assert.match(await (await fetch(base + text(published.body.path))).text(), /<h2>Revised file<\/h2>/);
    const image = await post('/api/pages', {
        path: path.join(realCwd, 'preview.png'),
        sessionId: REAL_CWD_ID,
        renderer: 'file',
    });
    const imageHtml = await (await fetch(base + text(image.body.path))).text();
    const imageMatch = (0, test_types_js_1.present)(imageHtml.match(/<img[^>]+src="([^"]+)"/));
    const imageSrc = (0, test_types_js_1.present)(imageMatch[1]).replace(/&amp;/g, '&');
...
test('POST /api/pages rejects malformed or unknown explicit sessionIds without creating a page', async () => {
    ...
    for (const sessionId of [null, '', { arbitrary: true }, 'x'.repeat(513)]) {
        ...
        assert.match(text(invalid.body.error), /sessionId.*non-empty string.*512/);
    }
    const unknown = await post('/api/pages', { path: artifact, sessionId: 'missing-page-session' });
    assert.equal(unknown.status, 404);
    assert.match(text(unknown.body.error), /known active or historical session/);
    ...
    const list = await getArray('/api/pages');
...
test('POST /api/pages with an invalid explicit sessionId cannot mutate an existing page association', async () => {
    ...
    const list = await getArray('/api/pages');
    const preserved = (0, test_types_js_1.present)(list.body.find(page => page.token === published.body.token));
    assert.ok(preserved);
...
test('comments support unpaginated indexing, selected reads, and acknowledgment', async () => {
    ...
    const indexedComments = (0, test_types_js_1.records)(index.body.comments);
    assert.deepEqual(indexedComments.map(comment => comment.id), [first.body.id, second.body.id]);
    const indexedFirst = (0, test_types_js_1.present)(indexedComments[0]);
    assert.equal(indexedFirst.bodyPreview, 'Clarify this heading.');
    assert.equal(indexedFirst.sessionId, REAL_CWD_ID, 'index entries carry the session so a page overlay can read/edit them');
    const indexedAnchor = (0, test_types_js_1.record)((0, test_types_js_1.record)(indexedFirst.target).anchor);
    assert.equal(indexedAnchor.quotePreview, 'alpha');
    assert.equal(indexedAnchor.prefix, undefined, 'index omits full anchor context');
    const selected = await post('/api/comments/get', {
        sessionId: REAL_CWD_ID, ids: [second.body.id, first.body.id],
    });
    assert.equal(selected.status, 200);
    const selectedComments = (0, test_types_js_1.records)(selected.body.comments);
    assert.deepEqual(selectedComments.map(comment => comment.id), [second.body.id, first.body.id], 'the agent can fetch any inferred group in its requested order without acking earlier comments');
    assert.equal((0, test_types_js_1.record)((0, test_types_js_1.record)((0, test_types_js_1.present)(selectedComments[1]).target).anchor).prefix, '# ', 'selected fetch returns the full anchor');
    ...
    assert.deepEqual((0, test_types_js_1.records)(indexAfterAck.body.comments).map((c) => c.id), [second.body.id]);
...
test('open comments are editable and deletable, acknowledged ones are not', async () => {
    ...
    assert.equal((await patch(`/api/comments/${editable.body.id}`, { sessionId: SESSION_ID, body: 'hijacked' })).status, 403);
    assert.equal((await patch('/api/comments/nope', { sessionId: REAL_CWD_ID, body: 'ghost' })).status, 404);
    assert.equal((await patch(`/api/comments/${editable.body.id}`, { sessionId: REAL_CWD_ID, body: '   ' })).status, 400, 'an emptied body is not an edit');
    const edited = await patch(`/api/comments/${editable.body.id}`, { sessionId: REAL_CWD_ID, body: 'Second wording.' });
    assert.equal(edited.status, 200);
    assert.equal(edited.body.body, 'Second wording.');
    assert.ok(Number(edited.body.updatedAt) >= Number(edited.body.createdAt), 'an edit stamps updatedAt');
    const readBack = await post('/api/comments/get', { sessionId: REAL_CWD_ID, ids: [editable.body.id] });
    assert.equal((0, test_types_js_1.present)((0, test_types_js_1.records)(readBack.body.comments)[0]).body, 'Second wording.');
    ...
    assert.equal((await del(`/api/comments/${disposable.body.id}`, { sessionId: SESSION_ID })).status, 403);
    assert.deepEqual((await del(`/api/comments/${disposable.body.id}`, { sessionId: REAL_CWD_ID })).body, { ok: true });
    const afterDelete = await get(`/api/comments/index?sessionId=${REAL_CWD_ID}`);
    assert.ok(!(0, test_types_js_1.records)(afterDelete.body.comments).some((c) => c.id === disposable.body.id), 'a deleted comment leaves the open index');
    assert.equal((await del(`/api/comments/${disposable.body.id}`, { sessionId: REAL_CWD_ID })).status, 404);
    ...
    const lateEdit = await patch(`/api/comments/${editable.body.id}`, { sessionId: REAL_CWD_ID, body: 'Too late.' });
    assert.equal(lateEdit.status, 409);
    assert.equal(lateEdit.body.error, 'comment already acknowledged');
    assert.equal((await del(`/api/comments/${editable.body.id}`, { sessionId: REAL_CWD_ID })).status, 409);
});
test('published-page comments inherit the page session and artifact identity', async () => {
    ...
    assert.equal((0, test_types_js_1.record)(created.body.target).root, artifact);
    const selected = await post('/api/comments/get', { sessionId: REAL_CWD_ID, ids: [created.body.id] });
    assert.deepEqual((0, test_types_js_1.records)(selected.body.comments).map((c) => c.id), [created.body.id]);
    ...
    const pageComments = (0, test_types_js_1.records)(byToken.body.comments);
    assert.deepEqual(pageComments.map(comment => comment.id), [created.body.id]);
    assert.equal((0, test_types_js_1.present)(pageComments[0]).sessionId, REAL_CWD_ID);
    ...
    assert.equal((await get(`/api/comments/index?pageToken=${page.body.token}`)).body.total, 0, 'the page index tracks open comments only');
...
test('encoded Pi aliases share the canonical raw Pi comment identity', async () => {
    const alias = encodeSessionKey('pi', (0, test_types_js_1.nativeId)(REAL_CWD_ID));
    const created = await post('/api/comments', {
...
test('publishing without a sessionId infers the most specific containing cwd', async () => {
    ...
    try {
        ...
        const inner = await getArray(`/api/pages?sessionId=${INNER_ID}`);
        assert.ok(inner.body.some(page => page.token === body.token), 'the page routes to the deepest containing session cwd');
        await del(`/api/pages/${body.token}`);
    }
    finally {
...
test('inferred alternative-harness pages and comments keep the encoded harness identity', async () => {
    ...
    fs.writeFileSync(path.join(registryDir, `${OMP_SESSION_ID}.json`), JSON.stringify({
        ...
        startTime: (0, test_types_js_1.present)(processIdentity(process.pid)).startTime,
...
test('the session list forwards the registry compacting flag', async () => {
    ...
    try {
        const { body } = await get('/api/sessions');
        const sess = (0, test_types_js_1.present)((0, test_types_js_1.records)(body.active).find((s) => s.id === ID));
...
test('OMP /compact routes through the advertised bridge operation and rejects overlap', async () => {
    ...
    const bridge = net.createServer(sock => {
        ...
        sock.on('data', chunk => {
            for (const line of chunk.toString().trim().split('\n')) {
                if (!line)
                    continue;
                const command = (0, test_types_js_1.record)(JSON.parse(line));
    ...
    try {
        ...
        assert.deepEqual(received.filter(command => command.command === 'compact').map(command => command.instructions), ['retain decisions'], 'only one compact operation reaches the bridge');
    }
    finally {
        fs.rmSync(registryPath, { force: true });
        invalidateRegistryCache();
...
test('OMP /compact is denied when the live bridge does not advertise it', async () => {
    ...
    try {
        ...
        assert.match(text(result.body.error), /does not support compaction/i);
...
test('OMP /btw requires the advertised capability and passes the bridge answer through', async () => {
    ...
    try {
        ...
        assert.match(text(bare.body.error), /usage: \/btw/);
        ...
        claim.capabilities = { commands: true, btw: false };
        fs.writeFileSync(registryPath, JSON.stringify(claim));
        invalidateRegistryCache();
        ...
        assert.match(text(unsupported.body.error), /does not support \/btw/i);
...
test('OMP command discovery includes pane-backed host commands and API aliases stay capability-first', async () => {
    ...
    const advertised = [
        ...['tree', 'fork', 'new', 'resume', 'session', 'settings', 'export', 'share', 'copy',
            'login', 'logout', 'scoped-models', 'hotkeys', 'quit'].map(name => ({ name, source: 'builtin', supported: false })),
    ...
    const injected = [];
    try {
        const unownedCommands = await getArray(`/api/commands?sessionId=${encodeURIComponent(routeId)}`);
        assert.equal(unownedCommands.status, 200, JSON.stringify(unownedCommands.body));
        assert.deepEqual(unownedCommands.body.map(command => command.name), ['tree', 'compact', 'model', 'name', 'abort', 'skill:review', 'daily', 'dish-push']);
        ...
        tmux.sendKeys = async (socket, paneId, keyText) => { injected.push({ socket, paneId, text: keyText }); };
        const commands = await getArray(`/api/commands?sessionId=${encodeURIComponent(routeId)}`);
        ...
        assert.deepEqual(commands.body.map(command => command.name), ['tree', 'compact', 'model', 'name', 'abort', 'skill:review', 'daily', 'dish-push',
            ...hostNames]);
        assert.deepEqual(commands.body.slice(-hostNames.length).map(({ name, source, supported }) => ({ name, source, supported })), hostNames.map(name => ({ name, source: 'host', supported: true })));
        assert.equal((0, test_types_js_1.present)(commands.body.find(command => command.name === 'shake')).args, '[images]');
        assert.equal((0, test_types_js_1.present)(commands.body.find(command => command.name === 'guided-goal')).args, '<rough objective>');
        // The descriptor's validation rules stay server-side.
        for (const command of commands.body) {
            assert.deepEqual(Object.keys(command).filter(key => ['allowedArgs', 'blockedArgs', 'freeArgs', 'requireArgs'].includes(key)), [], `internal arg rules leaked on /${command.name}`);
        }
        ...
        assert.match(text(retryArgs.body.error), /invalid arguments/i);
        ...
        assert.match(text(badArgs.body.error), /invalid arguments/i);
        ...
        assert.match(text(goalBare.body.error), /requires arguments/i);
        for (const args of ['drop', 'drop it all', 'set', 'budget']) {
            ...
            assert.match(text(blocked.body.error), /only available in the Oh My Pi terminal UI/i);
        }
        const goalNewline = await send('/goal ship it\nrm -rf /');
        assert.equal(goalNewline.status, 400, JSON.stringify(goalNewline.body));
        assert.match(text(goalNewline.body.error), /invalid arguments/i);
        const advisorBad = await send('/advisor sometimes');
        assert.equal(advisorBad.status, 400, JSON.stringify(advisorBad.body));
        assert.match(text(advisorBad.body.error), /expected on or off or status or dump or dump raw/);
        ...
        assert.ok(injected.slice(4).every(entry => entry.socket === '/fake/owned-omp.sock' && entry.paneId === '%88'));
        ...
        const stampedCommands = await getArray(`/api/commands?sessionId=${encodeURIComponent(routeId)}`);
        ...
        assert.match(text(noPane.body.error), /reachable Oh My Pi tmux pane/i);
        ...
        assert.deepEqual(received.filter(command => command.command === 'set_model').map(command => command.model), ['zai/glm-4.7-flash', 'zai/glm-4.5-flash']);
    }
    finally {
...
test('PI_DISH_URL served to spawned agents always answers', async () => {
    ...
    for (const candidate of ['::1', ipv4]) {
        if (candidate && await canBind(candidate)) {
            host = candidate;
            break;
        }
    }
    if (!host)
        return; // no non-loopback-shaped address on this machine
...
test('PUT /api/models/enabled preserves a concurrent pi settings write', async (t) => {
    ...
    const sdkRequire = createRequire(path.join(__dirname, '..', 'node_modules', '@earendil-works', 'pi-coding-agent', 'package.json'));
    ...
    t.mock.method(lockfile, 'lockSync', function (file, options) {
        try {
            ...
            if (file === settingsFile && release && (0, test_types_js_1.record)(error).code === 'ELOCKED') {
...
test('GET /stats aggregates tokens, cost, and message counts from the JSONL', async () => {
    ...
    assert.ok(Math.abs(Number(body.cost) - 0.05) < 1e-9);
    ...
    assert.equal((0, test_types_js_1.record)(body.contextUsage).tokens, 1234);
...
test('usage summary preserves cost availability through every grouping and filter', async () => {
    ...
    const baselineToday = usageSummary((await get('/api/usage-summary?days=1')).body);
    const refs = ['audit/paid', 'audit/free', 'audit/total-only', 'audit/mixed', 'audit/old-paid', 'audit/dateless-paid'];
    const filterUrl = (models, days = 'all', sort = 'cost') => `/api/usage-summary?days=${days}&sort=${sort}&models=${encodeURIComponent(models)}`;
    const entries = [
    ...
    try {
        ...
        assert.equal(knownToday.totals.costs.input, 0.5, 'a total-only call is omitted from the known component subtotal');
        ...
        assert.ok(Math.abs(knownToday.headlineCosts.days7 - baselineToday.headlineCosts.days7 - 1.7) < 1e-9, 'old and dateless usage stay out of the recent headline');
        const free = usageSummary((await get(filterUrl('audit/free'))).body);
        assert.equal(free.totals.costs.total, 0, 'explicit-zero/free totals stay numeric');
        assert.deepEqual(free.totals.costUnavailable, { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0 });
        assert.equal((0, test_types_js_1.present)(free.groups.models.find(model => model.key === 'audit/free')).priced, true, 'explicit zero is available even without catalog inference');
        const totalOnly = usageSummary((await get(filterUrl('audit/total-only'))).body);
        ...
        const all = usageSummary((await get(filterUrl(refs.join(',')))).body);
        ...
        const workspace = (0, test_types_js_1.present)(all.groups.workspaces.find(row => row.key === '/workspace/usage'));
        const session = (0, test_types_js_1.present)(all.groups.sessions.find(row => row.id === usageId));
        ...
        const mixed = (0, test_types_js_1.present)(all.groups.models.find(model => model.key === 'audit/mixed'));
        ...
        assert.equal(all.daily.some(day => day.day === 'unknown'), false, 'dateless usage stays out of daily series');
        const todayEntry = (0, test_types_js_1.present)(usageSummary((await get(filterUrl(refs.join(','), '7'))).body).daily.at(-1));
        assert.ok(Math.abs(todayEntry.costs.total - 1.7) < 1e-9);
        const mixedDay = (0, test_types_js_1.present)(todayEntry.models.find(model => model.ref === 'audit/mixed'));
        assert.equal(mixedDay.cost, 0.2);
        assert.equal(mixedDay.costUnavailable.total, 1);
        assert.ok(todayEntry.models.some(model => model.ref === 'audit/paid' && model.cost === 1), 'daily model data keeps known totals beside unavailable ones');
        const thirty = usageSummary((await get(filterUrl(refs.join(','), '30'))).body);
        assert.equal(thirty.daily.length, 30);
        assert.ok(thirty.daily.some(day => day.models.some(model => model.ref === 'audit/old-paid' && model.cost === 2)), '10-day-old usage must land on its own day');
        assert.ok(all.daily.length >= 11 && all.daily.length <= 365, 'all-time daily spans from the earliest dated day, capped at a year');
        const afterMixedToday = usageSummary((await get('/api/usage-summary?days=1')).body);
        assert.ok(Math.abs(afterMixedToday.headlineCosts.today - baselineToday.headlineCosts.today - 1.7) < 1e-9, 'headline preserves its known subtotal when one call lacks a total');
        ...
        const modelPos = (body, ref) => body.groups.models.findIndex(model => model.key === ref);
        assert.ok(modelPos(all, 'audit/dateless-paid') < modelPos(all, 'audit/paid'));
        assert.ok(modelPos(all, 'audit/mixed') < modelPos(all, 'audit/free'), 'a partial positive subtotal sorts ahead of explicit zero');
        const byTokens = usageSummary((await get(filterUrl(refs.join(','), 'all', 'tokens'))).body);
        ...
        const paid = usageSummary((await get(filterUrl('audit/paid'))).body);
        assert.deepEqual(paid.models, ['audit/paid']);
        assert.equal(paid.totals.calls, 1);
        assert.equal(paid.totals.costs.total, 1);
        assert.ok(paid.groups.models.some(model => model.key === 'audit/dateless-paid'), 'facet list keeps deselected models');
        const paidSession = (0, test_types_js_1.present)(paid.groups.sessions.find(row => row.id === usageId));
        assert.ok(paidSession.calls === 1 && paidSession.costs.total === 1, 'session group carries only the filtered model usage');
        const wsPaid = (0, test_types_js_1.present)(paid.groups.workspaces.find(row => row.key === '/workspace/usage'));
        assert.equal(wsPaid.calls, 1, 'workspace group carries only filtered usage');
        assert.ok(paid.daily.every(day => day.models.every(model => model.ref === 'audit/paid')), 'daily per-model breakdowns are filtered');
        assert.equal(paid.headlineCosts.today, afterMixedToday.headlineCosts.today, 'headline KPIs stay global under model filters');
        const pair = usageSummary((await get(filterUrl('audit/paid,audit/old-paid'))).body);
        assert.equal(pair.totals.calls, 2);
        assert.equal(pair.totals.costs.total, 3);
        const unavailable = usageSummary((await get(filterUrl('audit/mixed'))).body);
        ...
        assert.equal(unavailable.groups.sessions.find(row => row.id === usageId)?.unpricedCalls, 1);
...
test('usage limits come from the harness CLI, sanitized and failure-tolerant', async () => {
    ...
    const omp = (0, test_types_js_1.present)((0, test_types_js_1.records)(body.harnesses).find((h) => h.harness === 'omp'));
    ...
    const reports = (0, test_types_js_1.records)(omp.reports);
    assert.equal(reports.length, 1, 'the limits-less provider report is dropped');
    const report = (0, test_types_js_1.present)(reports[0]);
    assert.equal(report.provider, 'fakeprov');
    assert.equal(report.planType, 'pro');
    const limits = (0, test_types_js_1.records)(report.limits);
    assert.equal(limits.length, 1, 'the malformed limit is dropped');
    const limit = (0, test_types_js_1.present)(limits[0]);
...
test('usage limits degrade to an error entry when the harness command fails', async () => {
    ...
    try {
        const { status, body } = await get('/api/usage-limits');
        assert.equal(status, 200);
        assert.equal((0, test_types_js_1.records)(body.harnesses).length, 0, 'an uninstalled harness is skipped, not an error — the capability check agrees');
        assert.equal('usageLimits' in (0, test_types_js_1.record)((await get('/api/host')).body.capabilities), false);
    }
    finally {
        if (previous === undefined)
            delete process.env.PI_DISH_OMP_COMMAND;
        else
            process.env.PI_DISH_OMP_COMMAND = previous;
    }
});
...
function withSttSettings(block) {
    ...
    return () => {
        if (before === null)
            fs.rmSync(DISH_SETTINGS_FILE, { force: true });
        else
            fs.writeFileSync(DISH_SETTINGS_FILE, before);
    };
}
const postAudio = async (requestPath, contentType, body) => {
    const requestBody = typeof body === 'string' ? body : Uint8Array.from(body);
    const response = await fetch(base + requestPath, {
        method: 'POST', headers: { 'Content-Type': contentType }, body: requestBody,
    });
...
test('speech to text is invisible and refused while unconfigured', async () => {
    ...
    try {
        ...
        assert.equal('stt' in (0, test_types_js_1.record)((await get('/api/host')).body.capabilities), false, 'absent capability is how a client knows not to show the mic');
        const refused = await postAudio('/api/stt', 'audio/webm', Buffer.alloc(2048, 1));
        assert.equal(refused.status, 503);
        assert.match(text(refused.body.error), /not configured/);
    }
    finally {
        restore();
    }
...
test('POST /api/stt relays audio to the configured endpoint and keeps its credentials server-side', async () => {
    ...
    if (!upstreamAddress || typeof upstreamAddress === 'string')
        throw new Error('STT upstream lacks TCP address');
    const url = `http://127.0.0.1:${upstreamAddress.port}/v1/audio/transcriptions`;
    ...
    try {
        ...
        assert.deepEqual(await postAudio('/api/stt', 'audio/webm;codecs=opus', audio), { status: 200, body: { text: 'hello world' } });
        ...
        assert.equal((0, test_types_js_1.record)((await get('/api/host')).body.capabilities).stt, true);
        ...
        assert.equal(JSON.parse(fs.readFileSync(DISH_SETTINGS_FILE, 'utf8')).stt.url, url, 'PUT /api/settings ignores an stt key in the body');
    }
    finally {
        restore();
...
test('POST /api/stt reports an upstream failure and validates its own body', async () => {
    ...
    const upstreamAddress = upstream.address();
    if (!upstreamAddress || typeof upstreamAddress === 'string')
        throw new Error('STT failure upstream lacks TCP address');
    const restore = withSttSettings({ url: `http://127.0.0.1:${upstreamAddress.port}/v1/audio/transcriptions`, apiKey: 'nope' });
    try {
        const failed = await postAudio('/api/stt', 'audio/webm', Buffer.alloc(2048, 3));
        assert.equal(failed.status, 502);
        assert.match(text(failed.body.error), /401/);
        assert.match(text(failed.body.error), /bad key/);
        const empty = await postAudio('/api/stt', 'audio/webm', Buffer.alloc(0));
        assert.equal(empty.status, 400);
        assert.match(text(empty.body.error), /audio body required/);
        ...
        assert.match(text(wrongType.body.error), /unsupported audio type text\/plain/);
    }
    finally {
        restore();
        await new Promise((resolve, reject) => upstream.close(error => error ? reject(error) : resolve()));
...
test('recovery mode and exclusions are partial host settings with validated values', async () => {
    ...
    assert.equal((0, test_types_js_1.record)((await get('/api/host')).body.capabilities).recovery, true);
...
test('POST endpoints validate input and reject inactive sessions', async () => {
    ...
    const badImage = await post(`/api/sessions/${SESSION_ID}/prompt`, { images: [{ mimeType: 'image/png', data: 'not valid base64!' }] });
    assert.equal(badImage.status, 400);
    // Well-formed base64 survives sanitizing and reaches the (dead) session.
    const okImage = await post(`/api/sessions/${SESSION_ID}/prompt`, { images: [{ mimeType: 'image/png', data: 'aGVsbG8=' }] });
...
test('POST /close prunes a reused registry PID without signaling the new process', async () => {
    ...
    fs.mkdirSync(registryDir, { recursive: true });
    const child = spawn(process.execPath, [IDLE_CHILD], { stdio: 'ignore' });
    const actual = (0, test_types_js_1.present)(processIdentity(child.pid));
...
test('POST /close SIGTERMs the registry pid; /stats reports where it runs', async () => {
    ...
    fs.writeFileSync(sockStub, '');
    const identity = (0, test_types_js_1.present)(processIdentity(child.pid));
    ...
    try {
        ...
        assert.equal((0, test_types_js_1.record)(stats.body.runtime).kind, 'tmux');
        assert.equal((0, test_types_js_1.record)(stats.body.runtime).server, 'work');
        assert.equal((0, test_types_js_1.record)(stats.body.runtime).pid, child.pid);
        assert.equal((0, test_types_js_1.record)(stats.body.runtime).tmuxSession, null);
...
test('POST /close refuses an unreachable legacy registry PID without signaling it', async () => {
    ...
    try {
        ...
        assert.ok((0, test_types_js_1.records)(listed.body.active).some((session) => session.id === id), 'a live legacy PID remains visible without a birth marker');
        const closed = await post(`/api/sessions/${id}/close`, {});
        assert.equal(closed.status, 409, JSON.stringify(closed.body));
        assert.match(text(closed.body.error), /without a successful bridge identity handshake/);
...
test('POST /close preserves a legacy claim when a stale pooled hello does not match', async () => {
    ...
    try {
        const closed = await post(`/api/sessions/${id}/close`, {});
        assert.equal(closed.status, 409, JSON.stringify(closed.body));
        assert.match(text(closed.body.error), /did not prove the registered session and PID/);
...
test('POST /branch on a user message returns its text and persists the leaf move', async () => {
    ...
    const active = new Set(Array.isArray(tree.body.activePathIds) ? tree.body.activePathIds : []);
    ...
    const texts = (0, test_types_js_1.records)(msgs.body.messages).map(message => text((0, test_types_js_1.records)(message.content)[0]?.text));
    assert.ok(texts.includes('first prompt') && texts.includes('first answer'), 'active path still renders');
    assert.ok(!texts.includes('second prompt') && !texts.includes('second answer'), 'abandoned branch messages are gone from the transcript');
});
...
test('branch_summary entries surface in /messages as branchSummary role', async () => {
    ...
    assert.equal(status, 200);
    const bs = (0, test_types_js_1.present)((0, test_types_js_1.records)(body.messages).find((m) => m.role === 'branchSummary'));
    assert.ok(bs, 'branch summary appears in the message stream');
    assert.equal(text((0, test_types_js_1.records)(bs.content)[0]?.text), 'Explored **X**; conclusion Y.');
});
test('inactive OMP tree routes return capability errors without calling the Pi SDK', async () => {
    ...
    try {
        ...
        assert.match(text(tree.body.error), /inactive Oh My Pi session/i);
        ...
        assert.match(text(navigate.body.error), /inactive Oh My Pi session/i);
...
test('OMP transcript loading does not wait for the optional pricing catalog', async (t) => {
    ...
    assert.ok((0, test_types_js_1.records)(result.body.messages).length > 0);
});
test('live OMP tree routes use tree_read/tree_navigate and the transcript follows the bridge leaf', async () => {
    ...
    tmux.findPaneByPid = async () => ({ socket: '/fake/omp-tmux.sock', paneId: '%42', tmuxSession: 'work', windowIndex: 0, windowName: 'fixture' });
    tmux.sendKeys = async (socket, paneId, keyText) => {
        serviceCommands.push({ socket, paneId, text: keyText });
        const queued = (0, test_types_js_1.present)(pendingNavigation);
        const { sock, command } = queued;
        pendingNavigation = null;
        leafId = text(command.targetId);
        sock.write(JSON.stringify({
            type: 'response', id: command.id, command: command.command, success: true,
    ...
    try {
        ...
        const wireNavigate = (0, test_types_js_1.present)(received.find(command => command.command === 'tree_navigate'));
        ...
        assert.deepEqual((0, test_types_js_1.records)(messages.body.messages).map(message => text((0, test_types_js_1.records)(message.content)[0]?.text)), ['OMP shared prompt']);
...
test('live OMP tree navigation presses the advertised service chord and surfaces bridge refusals', async () => {
        ...
        sock.on('data', (chunk) => {
            ...
            while ((newline = buffer.indexOf('\n')) >= 0) {
                ...
                if (!line.trim())
                    continue;
                const command = (0, test_types_js_1.record)(JSON.parse(line));
                if (command.command !== 'tree_navigate')
                    continue;
                if (mode === 'refuse') {
                    sock.write(JSON.stringify({
    ...
    const pressed = [];
    tmux.findPaneByPid = async () => ({ socket: '/fake/omp-tmux.sock', paneId: '%42', tmuxSession: 'work', windowIndex: 0, windowName: 'fixture' });
    tmux.sendKeys = async (socket, paneId, keyText) => { typed.push({ socket, paneId, text: keyText }); };
    tmux.sendKey = async (socket, paneId, key) => {
        pressed.push({ socket, paneId, key });
        const { sock, command } = (0, test_types_js_1.present)(pendingNavigation);
        pendingNavigation = null;
        sock.write(JSON.stringify({
            type: 'response', id: command.id, command: command.command, success: true,
    ...
    try {
        ...
        assert.equal(refused.status, 409, JSON.stringify(refused.body));
        assert.equal(refused.body.error, 'cannot navigate the tree while a turn is in progress', 'a bridge refusal wins over the acknowledgement timeout that would otherwise mask it');
...
test('POST /branch on a live bridge session forwards navigate_tree', async () => {
    ...
    const bridge = net.createServer((sock) => {
        ...
        sock.on('data', (chunk) => {
            ...
            while ((i = buf.indexOf('\n')) >= 0) {
                ...
                if (!line.trim())
                    continue;
                const cmd = (0, test_types_js_1.record)(JSON.parse(line));
                received.push(cmd);
    ...
    await new Promise(resolve => setTimeout(resolve, 600)); // registry scan memo TTL
    try {
        const ok = await post(`/api/sessions/${BRIDGE_ID}/branch`, { entryId: 'e2', summarize: true, customInstructions: 'focus on files' });
        assert.equal(ok.status, 200);
        assert.equal(ok.body.editorText, 'from bridge');
        const nav = (0, test_types_js_1.present)(received.find(command => command.command === 'navigate_tree'));
        ...
        assert.match(text(blocked.body.error), /dish-push/);
    }
    finally {
...
test('GET /api/themes lists built-ins plus valid user theme files', async () => {
    ...
    assert.equal(status, 200);
    const themes = (0, test_types_js_1.records)(body.themes);
    assert.deepEqual(themes.filter(theme => theme.builtin).map(theme => theme.id), ['solarized', 'graphite']);
    const custom = (0, test_types_js_1.present)(themes.find(theme => theme.id === 'mytheme'));
    assert.deepEqual(custom.tokens, { '--bg-dark': '#101010', '--accent': 'rgb(1, 2, 3)' }, 'invalid keys/values are dropped');
    assert.ok(!themes.some(theme => theme.id === 'broken'), 'unparseable file is skipped');
});
test('GET /api/themes without a themes dir serves the built-ins', async () => {
    ...
    const { status, body } = await get('/api/themes');
    assert.equal(status, 200);
    assert.deepEqual((0, test_types_js_1.records)(body.themes).map(theme => theme.id), ['solarized', 'graphite']);
});
// --- Public share links --------------------------------------------------
...
test('POST /share is idempotent and 404s for an unknown session', async () => {
    ...
    const piAlias = encodeSessionKey('pi', (0, test_types_js_1.nativeId)(TREE_ID));
...
test('GET /share/:token renders the exported HTML inline; unknown token 404s', async () => {
    ...
    const dataMatch = (0, test_types_js_1.present)(html.match(/id="session-data"[^>]*>([^<]+)</));
    const payload = Buffer.from((0, test_types_js_1.present)(dataMatch[1]), 'base64').toString('utf8');
...
test('OMP sessions can create and render public share links', async () => {
    ...
    const res = await fetch(`${base}${text(created.body.path)}`);
    assert.equal(res.status, 200);
    const html = await res.text();
    assert.ok(html.includes('Native OMP fixture export'), 'sharing uses the OMP renderer');
    const dataMatch = (0, test_types_js_1.present)(html.match(/id="session-data"[^>]*>([^<]+)</));
    const payload = (0, test_types_js_1.record)(JSON.parse(Buffer.from((0, test_types_js_1.present)(dataMatch[1]), 'base64').toString('utf8')));
    assert.equal((0, test_types_js_1.record)(payload.header).id, OMP_SESSION_ID);
    assert.ok((0, test_types_js_1.records)(payload.entries).some(entry => entry.id === 'omp-a1'), 'OMP transcript entries reach the export');
    assert.ok((0, test_types_js_1.records)(payload.entries).some(entry => entry.type === 'title'), 'OMP pre-header records are preserved');
    assert.equal(fs.readFileSync(ompSessionFile, 'utf8').split('\n')[0], JSON.stringify({ type: 'title', title: 'Exact OMP fixture' }), 'sharing does not rewrite the OMP session');
});
test('live OMP shares include the effective system prompt and active tools', async () => {
    ...
    const bridge = net.createServer(sock => {
        ...
        sock.on('data', chunk => {
            ...
            for (;;) {
                ...
                if (newline < 0)
                    break;
                const command = (0, test_types_js_1.record)(JSON.parse(pending.slice(0, newline)));
    ...
    try {
        const created = await post(`/api/sessions/${encodeURIComponent(OMP_ROUTE_ID)}/share`, {});
        const shared = await fetch(`${base}${text(created.body.path)}`);
...
test('OMP custom /share imports and serves its exact native live HTML', async () => {
    ...
    const result = (0, test_types_js_1.record)(await imported.json());
...
test('dedicated share listener serves shared sessions, pages, and standalone file styles', async () => {
    ...
    const sharePort = await new Promise((resolve, reject) => {
        const srv = net.createServer();
        srv.once('error', reject);
        srv.listen(0, '127.0.0.1', () => {
            const address = srv.address();
            if (!address || typeof address === 'string')
                return reject(new Error('Share port probe lacks TCP address'));
            srv.close(error => error ? reject(error) : resolve(address.port));
        });
    });
    ...
    try {
        ...
        assert.equal((await fetch(`${shareBase}/style.css`)).status, 200, 'standalone file pages can load their shared renderer styles');
...
test('session caches pick up JSONL appends (mtime/size revalidation)', async () => {
    ...
    fs.appendFileSync(SESSION_FILE, JSON.stringify({ type: 'message', message: { role: 'user', content: [{ type: 'text', text: 'zulu addendum' }], timestamp: '2026-07-04T10:00:06.000Z' } }) + '\n');
    ...
    const sess = (0, test_types_js_1.present)((0, test_types_js_1.records)(list.body.previous).find((s) => s.id === SESSION_ID));
    ...
    assert.ok((0, test_types_js_1.records)(listSearch.body.previous).some((s) => s.id === SESSION_ID), 'list search text refreshed');
    const stats = await get(`/api/sessions/${SESSION_ID}/stats`);
...
test('OMP session_switch re-keys the owned pane, active route, history source, and old-route SSE', async () => {
    ...
    const oldRouteId = encodeSessionKey('omp', (0, test_types_js_1.nativeId)(oldNativeId));
    const newRouteId = encodeSessionKey('omp', (0, test_types_js_1.nativeId)(newNativeId));
    ...
    try {
        await stream.waitFor((event) => eventName(event) === 'init');
        ...
        const switchEvent = await stream.waitFor((event) => eventName(event) === 'session_switch');
        const switchData = eventData(switchEvent);
        assert.equal(switchData.sessionId, newRouteId);
        assert.equal(switchData.previousSessionId, oldRouteId);
        assert.equal(switchData.nativeSessionId, newNativeId);
        assert.equal(tmux.getSpawn(oldRouteId), null);
        assert.equal(tmux.getSpawn(newRouteId)?.paneId, '%91');
        const listed = await get('/api/sessions');
        assert.ok((0, test_types_js_1.records)(listed.body.active).some((session) => session.id === newRouteId), 'new route is active');
        assert.ok((0, test_types_js_1.records)(listed.body.previous).some((session) => session.id === oldRouteId), 'old route remains historical');
        assert.equal((0, test_types_js_1.records)(listed.body.active).some((session) => session.id === oldRouteId), false);
...
test('SSE replays remembered extension UI state to new connections', async () => {
    ...
    const emit = (event, data) => {
        for (const socket of bridgeSocks)
    ...
    try {
        ...
        await s1.waitFor((e) => eventName(e) === 'init');
        assert.equal(s1.events.some((e) => eventData(e).id === 'stale-ask'), false, 'idle ask replay is discarded before the first SSE subscriber attaches');
        emit('turn_start', {});
        ...
        await s1.waitFor((e) => eventName(e) === 'extension_ui_request' && eventData(e).method === 'confirm');
        await s1.waitFor((e) => eventName(e) === 'extension_ui_request' && eventData(e).method === 'ask');
        await s1.waitFor((e) => eventName(e) === 'tool_execution_update' && eventData(e).toolCallId === 'replay-tool');
        const interrupted = await s1.waitFor((e) => eventName(e) === 'message_end' && (0, test_types_js_1.record)(eventData(e).message).customType === 'interrupted-thinking');
        assert.deepEqual((0, test_types_js_1.record)(eventData(interrupted).message).content, [], 'hidden interrupted reasoning is projected to a marker');
        await s1.waitFor((e) => eventName(e) === 'message_end' && (0, test_types_js_1.record)(eventData(e).message).customType === 'visible-sentinel');
        assert.equal(s1.events.some((e) => eventName(e) === 'message_end' && (0, test_types_js_1.record)(eventData(e).message).customType === 'private-host-state'), false, 'other hidden custom messages are explicitly skipped');
        s1.close();
        ...
        const replayStart = await s2.waitFor((e) => eventName(e) === 'tool_execution_start' && eventData(e).toolCallId === 'replay-tool');
        const replayUpdate = await s2.waitFor((e) => eventName(e) === 'tool_execution_update' && eventData(e).toolCallId === 'replay-tool');
        assert.deepEqual(eventData(replayStart), {
            toolCallId: 'replay-tool', toolName: 'Bash', args: { command: 'sleep 12' }, startedAt: 123,
        });
        const partialResult = (0, test_types_js_1.record)(eventData(replayUpdate).partialResult);
        assert.equal(text((0, test_types_js_1.records)(partialResult.content)[0]?.text), 'still running');
        assert.ok(s2.events.findIndex((e) => eventName(e) === 'init') <
            s2.events.findIndex((e) => eventName(e) === 'tool_execution_start'), 'tool replay follows init');
        const widget = await s2.waitFor((e) => eventName(e) === 'extension_ui_request' && eventData(e).method === 'setWidget');
        assert.deepEqual(eventData(widget).widgetLines, ['one', 'two']);
        assert.equal(eventData(widget).widgetKey, 'procs');
        await s2.waitFor((e) => eventName(e) === 'extension_ui_request' && eventData(e).id === 'dlg1');
        const ask = await s2.waitFor((e) => eventName(e) === 'extension_ui_request' && eventData(e).id === 'ask1');
        assert.equal((0, test_types_js_1.record)((0, test_types_js_1.records)(eventData(ask).questions)[0]).question, 'Which release?');
        ...
        emit('turn_end', {});
        await s2.waitFor((e) => eventName(e) === 'tool_execution_end' && eventData(e).toolCallId === 'replay-tool');
        await s2.waitFor((e) => eventName(e) === 'extension_ui_resolved' && eventData(e).id === 'ask1');
        s2.close();
        ...
        await s3.waitFor((e) => eventName(e) === 'init');
        emit('extension_ui_request', { method: 'notify', message: 'sentinel' });
        await s3.waitFor((e) => eventName(e) === 'extension_ui_request' && eventData(e).method === 'notify');
        const extEvents = (0, test_types_js_1.records)(s3.events).filter((e) => eventName(e) === 'extension_ui_request');
        assert.equal(extEvents.length, 1, 'cleared widget / resolved dialog must not be replayed');
        assert.equal((0, test_types_js_1.records)(s3.events).some(e => text(e.event).startsWith('tool_execution_')), false, 'completed tools are absent from later reconnect snapshots');
        s3.close();
    }
    finally {
        fs.rmSync(path.join(registryDir, `${BRIDGE_ID}.json`), { force: true });
...
test('/reload maps a run_command socket teardown to success (old-bridge race)', async () => {
    ...
    await new Promise(resolve => setTimeout(resolve, 600)); // registry memo TTL
    try {
        const { status, body } = await post(`/api/sessions/${BRIDGE_ID}/command`, { message: '/reload' });
        assert.equal(status, 200, JSON.stringify(body));
        assert.match(text(body.info), /reload/i);
    }
    finally {
        fs.rmSync(path.join(registryDir, `${BRIDGE_ID}.json`), { force: true });
        await new Promise((resolve, reject) => bridge.close(error => error ? reject(error) : resolve()));
...
test('startup omits feature assets and compresses large static text', async () => {
    ...
    assert.doesNotMatch(html, /vendor\/xterm(?:-addon-fit)?\.(?:js|css)/, 'terminal assets must not be part of the initial document');
    assert.doesNotMatch(html, /vendor\/(?:highlight\.js|hljs-theme\.min\.css)/, 'syntax-highlighting assets must not be part of the initial document');
    assert.doesNotMatch(html, /vendor\/katex\.min\.(?:js|css)/, 'math-rendering assets must not be part of the initial document');
...
test('GET /api/skills discovers the fixture skill with mined usage rollups', async () => {
    ...
    assert.ok(Number((0, test_types_js_1.record)(body.summary).discovered) >= 1);
    const demo = (0, test_types_js_1.present)((0, test_types_js_1.records)(body.skills).find(skill => skill.name === 'demo'));
    ...
    assert.ok(Number(demo.bodyTokensEst) > 0);
    assert.ok(Number(demo.catalogTokensEst) > 0);
    // Mined activations: one explicit + one read + one targeted grep.
    const usage = (0, test_types_js_1.record)(demo.usage), kindSplit = (0, test_types_js_1.record)(usage.kindSplit);
    assert.equal(kindSplit.read, 1);
    assert.equal(kindSplit.explicit, 1);
    assert.equal(kindSplit.targeted, 1);
    assert.equal(usage.total, 3);
    assert.equal(Array.isArray(usage.weeks12) ? usage.weeks12.length : 0, 12);
    assert.ok(Number(usage.lastUsedTs) > 0);
...
test('disable-model-invocation skills would be manual with zero advertised cost', async () => {
    ...
    const skills = (0, test_types_js_1.records)(body.skills);
    const advertisedTokens = skills.filter(skill => skill.advertised).reduce((total, skill) => total + Number(skill.catalogTokensEst), 0);
    const manualTokens = skills.filter(skill => !skill.advertised).reduce((total, skill) => total + Number(skill.catalogTokensEst), 0);
...
test('GET /api/skills/coverage maps ranged reads since mtime; targeted are touches', async () => {
    ...
    assert.ok(Number(body.unreadTokensEst) > 0, 'the unread sections carry an estimate');
    const sections = (0, test_types_js_1.records)(body.sections);
    const cold = sections.filter(section => section.neverRead);
    assert.ok(cold.length >= 2, 'Sections B and C are never read');
    assert.ok(sections.some(section => section.reads === 1), 'the read touches at least one section');
    assert.equal((0, test_types_js_1.record)(body.latest).sessionId, SKILLS_SESSION_ID, 'latest activation deep-links its session');
    assert.equal(Array.isArray(body.weeks26) ? body.weeks26.length : 0, 26);
});
...
const waitForSessions = async (ids, tries = 60) => {
    for (let i = 0; i < tries; i++) {
        const { body } = await get('/api/sessions');
        const rows = [...(0, test_types_js_1.records)(body.active), ...(0, test_types_js_1.records)(body.previous)];
        const have = new Set(rows.map(session => session.id));
...
test('GET /api/sessions/resolve resolves exact ids and unique prefixes', async () => {
    ...
    assert.equal(exact.status, 200, JSON.stringify(exact.body));
    assert.equal((0, test_types_js_1.record)(exact.body.session).id, SESSION_ID);
    // The literal path must not be captured by a parameterized /:id route.
    assert.notEqual((0, test_types_js_1.record)(exact.body.session).id, 'resolve');
    const prefix = await get('/api/sessions/resolve?id=resolveuniq');
    assert.equal(prefix.status, 200, JSON.stringify(prefix.body));
    const session = (0, test_types_js_1.record)(prefix.body.session);
    ...
    const listed = (0, test_types_js_1.present)((0, test_types_js_1.records)((await get('/api/sessions')).body.previous).find((s) => s.id === RESOLVE_UNIQUE));
...
test('GET /api/sessions/resolve reports ambiguous prefixes with candidates', async () => {
    ...
    assert.match(text(body.error), /ambiguous/);
    const matches = (0, test_types_js_1.records)(body.matches);
    assert.ok(matches.length <= 10, 'candidate list is capped');
    ...
    assert.equal((0, test_types_js_1.record)(exactWins.body.session).id, RESOLVE_AMBIGUOUS_A);
});
...
const ALIAS_ROUTE_ID = encodeSessionKey('omp', (0, test_types_js_1.nativeId)(ALIAS_NATIVE_ID));
test('GET /api/sessions/resolve accepts a native id or uuid tail and answers with a short ref', async () => {
    ...
    for (const ref of [ALIAS_ROUTE_ID, ALIAS_NATIVE_ID, '01a070d2-43fb-7360-aaba-a4ddf8d1deb0', '01a070d2', '2026-09-05T09-07']) {
        ...
        assert.equal((0, test_types_js_1.record)(body.session).id, ALIAS_ROUTE_ID, ref);
    }
    ...
    const shortRef = text(body.ref);
    assert.ok(shortRef && shortRef.length <= 40, `short ref: ${shortRef}`);
    assert.notEqual(shortRef, ALIAS_ROUTE_ID);
    const viaShort = await get(`/api/sessions/resolve?id=${encodeURIComponent(shortRef)}`);
    assert.equal((0, test_types_js_1.record)(viaShort.body.session).id, ALIAS_ROUTE_ID);
...
test('GET /api/host advertises ref-alias resolution so clients can print short refs', async () => {
    ...
    assert.equal((0, test_types_js_1.record)(body.capabilities).refAliases, true);
});
...
test('GET /api/agent-docs lists the shipped topics with titles and descriptions', async () => {
    ...
    assert.equal(status, 200);
    const topics = (0, test_types_js_1.records)(body.topics);
    ...
    for (const topic of topics) {
        const title = text(topic.title), description = text(topic.description);
        assert.ok(title, `${topic.name} has a title`);
        assert.ok(!title.startsWith('#'), 'the title is the heading text, not the markup');
        assert.ok(description, `${topic.name} has a description`);
        assert.ok(description.length <= 161, 'descriptions are truncated');
    }
    assert.equal((0, test_types_js_1.present)(topics.find(topic => topic.name === 'refs')).title, 'Session refs');
});
...
test('GET /api/host advertises the resolve and docs capabilities', async () => {
    ...
    assert.equal(status, 200);
    assert.equal((0, test_types_js_1.record)(body.capabilities).resolve, true);
    assert.equal((0, test_types_js_1.record)(body.capabilities).docs, true);
});
