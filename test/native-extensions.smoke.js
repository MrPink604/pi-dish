#!/usr/bin/env node
// Generated tool from test/native-extensions.smoke.ts; edit that source and run npm run build:tools.
"use strict";
...
for (const directory of [home, work])
...
const env = (0, test_env_js_1.sanitizeTestEnv)({
...
let importedHtml;
let importReply = { status: 200, body: { path: '/s/native-smoke' } };
...
async function waitFor(predicate, label, timeout = 30000) {
    ...
    while (!predicate()) {
        if (failure)
            throw failure;
        if (exited)
            throw new Error(`Harness exited before ${label}\n${output}`);
...
function start(executable, args) {
    ...
    terminal.onData(data => { output += (0, node_util_1.stripVTControlCharacters)(data); });
    terminal.onExit(() => { exited = true; });
}
function submit(text) {
    terminal.write(`\x1b[200~${text}\x1b[201~\r`);
}
async function moodSmoke(base) {
...
async function shareSmoke(base) {
    const omp = process.env.PI_DISH_REAL_OMP_BIN;
    if (!omp || !path.isAbsolute(omp))
        throw new Error('PI_DISH_REAL_OMP_BIN must name an absolute real OMP executable');
    fs.accessSync(omp, fs.constants.X_OK);
    const agent = path.join(home, '.omp', 'agent');
    fs.mkdirSync(agent, { recursive: true });
    (0, node_child_process_1.execFileSync)(omp, ['config', 'set', 'startup.setupWizard', 'false'], { env, cwd: work, stdio: 'pipe' });
    (0, node_child_process_1.execFileSync)(omp, ['config', 'set', 'startup.checkUpdate', 'false'], { env, cwd: work, stdio: 'pipe' });
    ...
    const data = record(JSON.parse(Buffer.from(match[1].trim(), 'base64').toString('utf8')));
    ...
    try {
        const { default: share } = await import((0, node_url_1.pathToFileURL)(hook).href);
        ...
        if (savedUrl === undefined)
            ...
            process.env.PI_DISH_URL = savedUrl;
    }
...
`);
    ...
    const socketPath = String(record(JSON.parse(fs.readFileSync(registryFile, 'utf8'))).socketPath);
    ...
    const waitMessage = async (predicate, label) => {
        let found;
        await waitFor(() => {
    ...
    const isDialogRequest = (message, method) => message.type === 'event' && message.event === 'extension_ui_request' && record(message.data).method === method;
    ...
    submit('/selecttest');
    const localRequest = await waitMessage(message => isDialogRequest(message, 'select') && record(message.data).id !== selectId, 'second bridged select request');
...
(async () => {
    try {
        ...
        if (selection === 'mood')
            await moodSmoke(base);
        else if (selection === 'dialogs')
            await dialogsSmoke();
    }
    finally {
        if (terminal && !exited) {
