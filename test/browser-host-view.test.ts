...
test('host presentation narrows labels and capabilities without mutating discovery metadata', () => {
  ...
  const projectUnknown = (value: unknown) => record(Reflect.apply(project, undefined, [value]));
  ...
  assert.equal(view.label, '12'); assert.deepEqual(JSON.parse(JSON.stringify(record(view.capabilities))), { terminal: true, recovery: false });
  assert.equal(projectUnknown(host), view); assert.equal(host.label, 12); assert.equal(host.capabilities.malformed, 'yes');
  const changed = projectUnknown({ ...host, label: 'new' }); assert.notEqual(changed, view); assert.equal(changed.label, 'new');
  assert.deepEqual(JSON.parse(JSON.stringify(record(projectUnknown({ ...host, capabilities: {} }).capabilities))), {});
});
test('static shell markup uses registered typed actions and contains no executable event attributes', () => {
  ...
  const registered = new Set<string>(context.PiDishBrowser.APP_ACTION_NAMES);
