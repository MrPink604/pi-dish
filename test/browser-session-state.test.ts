import type { SessionState } from '../src/browser/session-state.js';
...
function fixture() {
  let selfHostId: string | null = 'self';
  const labels: Record<string, string> = { self: 'This host', peer: 'Other host' };
  ...
  return { state, renders, labels, identifySelf: (host: string | null) => { selfHostId = host; } };
}
const current = (state: SessionState) => present(state.currentSession);
const found = (state: SessionState, id: string, host?: string | null) => present(state.findSession(id, host));
...
test('browser transcript metadata stays detached from registry lists and cannot overwrite the host', () => {
  ...
  const owner = present(state.captureSelection());
  renders.length = 0;
...
test('state writers filter identity, capability, family and extras independently of permitted metadata', () => {
  ...
  assert.equal(record(current(state).extras?.custom).name, 'extension name');
...
test('publication detaches external rows, capabilities and list membership while retaining opaque extras', () => {
  ...
  assert.equal(record(found(state, 'one', 'peer').extras?.custom).note, 'opaque update');
});
