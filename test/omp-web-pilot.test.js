// Generated test/tool from test/omp-web-pilot.test.ts; edit that source and run npm run build:tests.
"use strict";
...
test.before(async () => {
    ...
    if (!address || typeof address === 'string')
        throw new Error('OMP pilot server lacks TCP address');
...
test('OMP catalog preserves the command\'s authoritative availability list', async () => {
    ...
    assert.equal(events.filter(event => event.startsWith('finish ')).length, 0, 'complete model JSON is accepted without waiting for a lingering CLI');
    const models = (0, test_types_js_1.records)(body);
    ...
    const restricted = (0, test_types_js_1.present)(models.find(model => model.selector === 'zai/glm-5.2'));
...
test('OMP config endpoint separates a project overlay from the global record', async () => {
    ...
    fs.writeFileSync(path.join(cwd, '.omp', 'fake-project-roles.json'), JSON.stringify({ vision: 'zai/glm-5.2' }));
    const query = new URLSearchParams({ cwd });
    const { status, body } = await get(`/api/harnesses/omp/config?${query}`);
    assert.equal(status, 200);
    const config = (0, test_types_js_1.record)(body);
    assert.equal((0, test_types_js_1.record)(config.modelRoles).vision, 'zai/glm-5.2');
    assert.equal(Object.hasOwn((0, test_types_js_1.record)(config.globalModelRoles), 'vision'), false);
});
test('OMP model-role PUT patches the global record without absorbing project overrides', async () => {
    ...
    const patchedBody = (0, test_types_js_1.record)(patched.body);
    ...
    assert.equal((0, test_types_js_1.record)(patchedBody.modelRoles).vision, 'zai/glm-5.2');
    assert.equal(Object.hasOwn((0, test_types_js_1.record)(patchedBody.globalModelRoles), 'vision'), false);
    const reread = await get(`/api/harnesses/omp/config?${new URLSearchParams({ cwd })}`);
    const rereadBody = (0, test_types_js_1.record)(reread.body);
    assert.equal((0, test_types_js_1.record)(rereadBody.globalModelRoles).default, 'zai/glm-5.2');
    assert.equal(Object.hasOwn((0, test_types_js_1.record)(rereadBody.globalModelRoles), 'vision'), false);
    const unset = await put('/api/harnesses/omp/model-roles', { cwd, roles: { smol: null } });
    assert.equal(unset.status, 200);
    assert.deepEqual((0, test_types_js_1.record)(unset.body).globalModelRoles, { default: 'zai/glm-5.2' });
    ...
    assert.equal((0, test_types_js_1.record)((0, test_types_js_1.record)(custom.body).globalModelRoles)['my-role'], 'zai/glm-4.7-flash');
});
test('OMP model-role PUT serializes concurrent read-modify-writes', async () => {
    ...
    const config = (0, test_types_js_1.record)(body);
    assert.equal((0, test_types_js_1.record)(config.globalModelRoles).commit, 'zai/glm-4.7-flash');
    assert.equal((0, test_types_js_1.record)(config.globalModelRoles).tiny, 'zai/glm-5.2');
});
...
test('OMP agents endpoint lists definitions per source with global and effective settings', async () => {
    ...
    fs.writeFileSync(path.join(cwd, '.omp', 'fake-project-agents.json'), JSON.stringify({ 'task.disabledAgents': ['porter'] }));
    const { status, body } = await get(`/api/harnesses/omp/agents?${new URLSearchParams({ cwd })}`);
    assert.equal(status, 200);
    const agents = (0, test_types_js_1.records)((0, test_types_js_1.record)(body).agents);
    assert.deepEqual(agents.map(agent => [agent.name, agent.source]), [
        ['porter', 'project'], ['reviewer', 'bundled'], ['scout', 'user'],
    ]);
    const scout = (0, test_types_js_1.present)(agents.find(agent => agent.name === 'scout'));
    assert.equal(scout.description, 'My scout');
    assert.equal(scout.model, 'openai/gpt-5');
    assert.equal((0, test_types_js_1.present)(agents.find(agent => agent.name === 'reviewer')).model, '@slow');
    assert.equal((0, test_types_js_1.present)(agents.find(agent => agent.name === 'porter')).thinkingLevel, 'high');
    ...
    const agentBody = (0, test_types_js_1.record)(body);
    assert.deepEqual((0, test_types_js_1.record)(agentBody.settings).disabled, ['porter']);
    assert.deepEqual((0, test_types_js_1.record)(agentBody.globalSettings).disabled, []);
});
test('OMP agents PUT patches only the records the caller moved', async () => {
    ...
    const firstBody = (0, test_types_js_1.record)(first.body);
    const firstGlobal = (0, test_types_js_1.record)(firstBody.globalSettings);
    ...
    assert.deepEqual((0, test_types_js_1.record)(firstBody.settings).disabled, ['scout', 'porter']);
    ...
    const secondGlobal = (0, test_types_js_1.record)((0, test_types_js_1.record)(second.body).globalSettings);
    ...
    assert.deepEqual((0, test_types_js_1.record)((0, test_types_js_1.record)(reenabled.body).globalSettings).disabled, []);
});
...
test('synchronous OMP launch rejects a thinking level outside the selected model catalog entry', async () => {
    ...
    assert.match(String((0, test_types_js_1.record)(invalid.body).error), /valid levels: high, max/i);
    assert.equal(fs.readFileSync(modelEventsFile, 'utf8').trim().split('\n').length, 1, 'launch validation reuses the catalog loaded by the new-session view');
});
test('shared catalogs preserve cwd, environment, config and credential freshness during discovery', async () => {
    const commands = require('../lib/harness-feature-commands');
    const harnesses = require('../lib/harnesses');
    const descriptor = (0, test_types_js_1.present)(harnesses.getHarness('omp'));
    ...
    for (const dir of [agentDir, path.join(project, '.omp'), path.join(otherProject, '.omp')])
  ...
  `);
    ...
    try {
        ...
        assert.equal((0, test_types_js_1.records)((0, test_types_js_1.record)(raw).models)[0].id, 'original');
        ...
        databaseBytes[100] = 2;
        fs.writeFileSync(database, databaseBytes);
        assert.equal((await commands.runHarnessModelCommand(descriptor, { cwd: project }))[0].name, 'original:2', 'changed database payload invalidates even when the header counters do not change');
        ...
        assert.equal((0, test_types_js_1.record)((0, test_types_js_1.records)((0, test_types_js_1.record)(forced[0]).models)[0].cost).input, 7);
        assert.equal((0, test_types_js_1.record)((0, test_types_js_1.records)((0, test_types_js_1.record)(forced[1]).models)[0].cost).input, 7);
        ...
        const started = (0, node_events_1.once)(watcher, 'change');
