// Generated tool from test/edge-build.test.ts; edit that source and run npm run build:tools.
"use strict";
...
test('edge delivery preserves executable CJS and ESM, rejects drift and leaves outputs intact on failure', async () => {
    ...
    try {
        for (const dir of ['scripts', 'skills/scripts', 'electron', 'extensions'])
        ...
        const env = (0, test_env_1.sanitizeTestEnv)(process.env);
        const run = (...args) => (0, node_child_process_1.spawnSync)(process.execPath, ['scripts/build-edges.js', ...args], { cwd: root, env, encoding: 'utf8', timeout: 30000 });
        ...
        const execution = (0, node_child_process_1.spawnSync)(cli, [], { env, encoding: 'utf8' });
        passes(execution);
        assert.equal(execution.stdout, '42\n');
        const share = await import((0, node_url_1.pathToFileURL)(path.join(root, 'extensions/share.mjs')).href);
        assert.ok(typeof share === 'object' && share !== null && 'default' in share);
        assert.ok(typeof share.default === 'function');
        const shared = await share.default();
        ...
        for (const args of [[], ['--check']]) {
            ...
            assert.deepEqual(outputs.map(file => fs.readFileSync(path.join(root, file), 'utf8')), before);
            const authoredValue = require(authored);
...
test('skill client bundle resolves aliases after installation without the source tree', () => {
    ...
    try {
        ...
        fs.symlinkSync(path.join(__dirname, '../node_modules'), path.join(root, 'node_modules'), 'junction');
        const env = (0, test_env_1.sanitizeTestEnv)(process.env);
        const built = (0, node_child_process_1.spawnSync)(process.execPath, ['scripts/build-edges.js'], { cwd: root, env, encoding: 'utf8', timeout: 30000 });
        ...
        const stableRef = client.stableSessionRef(id, [id]);
        const legacyRef = client.stableSessionRef('legacy-session-long', ['legacy-session-long']);
        const harnessId = client.sessionHarnessId(id);
