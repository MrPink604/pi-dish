...
const piEnv: NodeJS.ProcessEnv = { ...process.env, ...piSpec.env, TMUX: '', TMUX_PANE: '' };

let piOk = !process.env.PI_DISH_SKIP_INTEGRATION;
let hostPiVersion: string | null = null;
try {
...
let holdArrived: (() => void) | null = null; // resolves when a HOLD request is being served
...
const llm = http.createServer((req, res) => {
  ...
  req.on('end', async () => {
    ...
    try {
      ...
      const messages = records(parsed.messages);
      const lastUser = [...messages].reverse().find(message => message.role === 'user');
      const content = lastUser?.content;
      userText = typeof content === 'string'
        ? content
        : records(content).filter(block => block.type === 'text').map(block => String(block.text ?? '')).join('');
    } catch {}
...
const { getBridgeSession }: typeof import('../lib/bridge-session') = require('../lib/bridge-session')
const { processIdentity }: typeof import('../lib/process-identity') = require('../lib/process-identity')

let base = '';
let pi: ChildProcessWithoutNullStreams | null = null; // the spawned real-pi child
let piStdout: TestRecord[] = []; // parsed RPC JSONL lines from pi's stdout
let piStderr = '';
let sessionId = '';
...
const messageText = (value: unknown): string => records(record(value).content)
  .filter(block => block.type === 'text').map(block => String(block.text ?? '')).join('');

async function waitFor<T>(fn: () => T | Promise<T>, timeout = 10000, label = 'condition'): Promise<T> {
...
test.before(async () => {
  ...
  if (!serverAddress || typeof serverAddress === 'string') throw new Error('Test server lacks TCP address');
  ...
  if (!llmAddress || typeof llmAddress === 'string') throw new Error('Fake LLM lacks TCP address');
  ...
  pi.stdout.on('data', (chunk: Buffer) => {
    ...
    while ((i = buf.indexOf('\n')) >= 0) {
      ...
      try { piStdout.push(record(JSON.parse(line))); } catch {}
    }
  });
  pi.stderr.on('data', (chunk: Buffer) => { piStderr += chunk.toString(); });
...
test.after(async () => {
  if (pi && pi.exitCode === null) {
    pi.kill('SIGTERM');
    await new Promise<void>(resolve => { pi?.on('exit', () => resolve()); setTimeout(resolve, 3000); });
...
test('bundled pi SDK version matches the host pi', { skip: !piOk }, () => {
  const bundledPackage: unknown = require('../node_modules/@earendil-works/pi-coding-agent/package.json');
  const bundled = record(bundledPackage).version;
  if (typeof bundled !== 'string') throw new Error('Bundled Pi package lacks a version');
...
async function bridgeConfigFailure(socketOverride: string | null) {
  const env: NodeJS.ProcessEnv = { ...piEnv, HOME: tmpHome };
...
test('the real bridge migrates its owned default socket directory from 0755 to 0700', { skip: !piOk, timeout: 30000 }, async () => {
  ...
  const env: NodeJS.ProcessEnv = { ...piEnv, HOME: migrationHome };
  ...
  try {
    ...
    const deadline = Date.now() + 10000;
    let entry: TestRecord | null = null;
    while (!entry && child.exitCode === null && Date.now() < deadline) {
      try {
        for (const name of fs.readdirSync(registryDir)) {
          const candidate = record(JSON.parse(fs.readFileSync(path.join(registryDir, name), 'utf8')));
      ...
      if (!entry) await new Promise<void>(resolve => setTimeout(resolve, 100));
    }
...
test('the real bridge binds under a long HOME using PI_DISH_SOCKET_DIR and registers', { skip: !piOk, timeout: 60000 }, async () => {
  ...
  const entry = await waitFor(() => {
    try {
      for (const f of fs.readdirSync(registryDir)) {
        ...
        const reg = record(JSON.parse(fs.readFileSync(path.join(registryDir, f), 'utf8')));
        if (typeof reg.socketPath === 'string' && fs.existsSync(reg.socketPath)) return reg;
      }
    } catch {}
    return null;
  }, 30000, 'bridge registry entry (is the bridge extension loading in this pi version?)');

  const registered = present(entry), socketPath = String(registered.socketPath);
  sessionId = String(registered.sessionId ?? '');
  assert.ok(sessionId, 'registry entry carries the session id');
  const process = present(pi);
  assert.equal(registered.pid, process.pid, 'entry belongs to the pi we spawned');
  assert.equal(registered.startTime, processIdentity(process.pid)?.startTime,
  ...
  assert.ok(path.basename(socketPath).length <= 30,
    `socket basename must be hash-sized, got ${path.basename(socketPath)}`);
  assert.ok(!path.basename(socketPath).includes(sessionId),
    'socket path must not embed the literal session id');

  const bridge = await getBridgeSession(checkedNativeId(sessionId));
  ...
  const sess = await waitFor(async () => {
    ...
    return records(body.active).find(session => session.id === sessionId);
  }, 10000, 'session in the active list');
  assert.equal(present(sess).model, 'fakeprov/fake-model', 'model comes from the bridge state');
});
...
test('prompt round-trip: real agent turn → bridge events → SSE → JSONL', { skip: !piOk, timeout: 60000 }, async () => {
  ...
  try {
    await stream.waitFor((e: unknown) => eventName(e) === 'init');
    const { status, body } = await post(`/api/sessions/${sessionId}/prompt`, { message: 'hello integration' });
    assert.equal(status, 200, JSON.stringify(body));

    await stream.waitFor((e: unknown) => eventName(e) === 'turn_start', 20000);
    const end = await stream.waitFor((e: unknown) => eventName(e) === 'message_end' && record(eventData(e).message).role === 'assistant', 20000);
    ...
    await stream.waitFor((e: unknown) => eventName(e) === 'turn_end', 20000);
    ...
    const texts = records(msgs.messages).map(messageText);
...
test('steering queue cancellation preserves duplicate index and image identity', { skip: !piOk, timeout: 60000 }, async () => {
  ...
  try {
    ...
    const held = new Promise<void>(resolve => { holdArrived = resolve; });
    ...
    const update = await stream.waitFor(
      (e: unknown) => eventName(e) === 'queue_update' && JSON.stringify(eventData(e).steering) ===
    ...
    present(holdRelease)();
    await stream.waitFor((e: unknown) => eventName(e) === 'agent_end', 20000);
    ...
    const { body: msgs } = await get(`/api/sessions/${sessionId}/messages`);
    const queuedUsers = records(msgs.messages).filter(message => message.role === 'user').map(message => ({
      message,
    ...
    const firstQueued = present(queuedUsers[0]);
    const image = records(firstQueued.message.content).find(block => block.type === 'image');
    assert.equal(image?.mimeType, QUEUE_IMAGE_A.mimeType, 'the uncancelled duplicate kept its image metadata');
    const imageResponse = await fetch(base + String(record(image).url ?? ''));
...
test('navigate_tree: self-primes a command context via the captured AgentSession', { skip: !piOk, timeout: 60000 }, async () => {
  ...
  const target = present(records(tree.nodes).find(node =>
    node.type === 'message' && node.role === 'user' && node.text === 'hello integration'));
  ...
  try {
    ...
    await stream.waitFor((e: unknown) => eventName(e) === 'session_tree', 15000);
  } finally {
    stream.close();
  }
  ...
  assert.ok(!new Set(activePathIds).has(target.id),
    'the branched-away turn left the active path without another append');
  const { body: msgs } = await get(`/api/sessions/${sessionId}/messages`);
  const texts = records(msgs.messages).map(messageText);
  assert.ok(!texts.some(text => text.includes('hello integration')),
...
test('compaction: second /compact refused, prompts queue until compaction ends', { skip: !piOk, timeout: 60000 }, async () => {
  ...
  try {
    ...
    await stream.waitFor((e: unknown) => eventName(e) === 'compaction_start', 15000);
    await held;
    ...
    for (const message of ['compaction duplicate', 'compaction other', 'compaction duplicate']) {
      ...
      assert.equal(record(queued.body.result).queued, true, JSON.stringify(queued.body));
    }
    await stream.waitFor((e: unknown) => eventName(e) === 'queue_update' &&
    ...
    present(holdRelease)();
    const end = await stream.waitFor((e: unknown) => eventName(e) === 'compaction_end', 20000);
    ...
    const msgs = await waitFor(async () => {
      ...
      const texts = records(body.messages).filter(message => message.role === 'user').map(messageText);
      return texts.includes('compaction duplicate') ? body : null;
    }, 20000, 'compaction queue flush');
    const texts = records(present(msgs).messages).map(messageText);
    assert.deepEqual(texts.filter((text: string) => text === 'compaction duplicate' || text === 'compaction other'),
      ['compaction duplicate']);
    const { body: tree } = await get(`/api/sessions/${sessionId}/tree`);
    assert.ok(records(tree.nodes).some(node => node.type === 'compaction'),
...
test('guarded reload refuses unfinished work and reloads after the agent becomes idle',
  { skip: !piOk, timeout: 60000 }, async () => {
    ...
    await waitFor(async () => {
      const state = record(await bridge.send('get_state'));
    ...
    await waitFor(async () => {
      try {
        const fresh = await getBridgeSession(checkedNativeId(sessionId));
        return fresh.alive && record(record(await fresh.send('get_state')).lifecycle).idle === true;
      } catch { return false; }
...
test('/reload from the API: responds ok, re-evaluates extensions, session stays usable', { skip: !piOk, timeout: 60000 }, async () => {
  ...
  await waitFor(async () => {
    const { body } = await get('/api/sessions?active=1');
    return records(body.active).some(session => session.id === sessionId);
  }, 15000, 'session active again after reload');
  const p = await post(`/api/sessions/${sessionId}/prompt`, { message: 'after reload' });
  assert.equal(p.status, 200, JSON.stringify(p.body));
  await waitFor(async () => {
    const { body } = await get(`/api/sessions/${sessionId}/messages`);
    return records(body.messages).some(message => JSON.stringify(message.content || '').includes('echo: after reload'));
