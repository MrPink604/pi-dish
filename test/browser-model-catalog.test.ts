...
import type { CatalogModel } from '../src/core/session-api.js';
import type { ModelCatalogScope } from '../src/browser/model-catalog.js';
...
function deferred<T>(): Deferred<T> {
  let resolve: ((value: T) => void) | undefined, reject: ((reason?: unknown) => void) | undefined;
  const promise = new Promise<T>((done, fail) => { resolve = done; reject = fail; });
  return { promise, resolve: value => present(resolve)(value), reject: reason => present(reject)(reason) };
}
const model = (id: string, provider = 'fixture', enabled = true) => ({ id, provider, enabled });
const scope = (id: string, cwd = '') => ({ host: { hostId: id, base: '/' + id, token: 'fixture-' + id }, harnessId: 'pi', cwd });
function fixture() {
  const requests: Array<Deferred<unknown> & { scope: ModelCatalogScope }> = [];
  const persisted: Array<{ scope: ModelCatalogScope; rows: readonly Readonly<CatalogModel>[] }> = [];
  const errors: unknown[] = [], changes: number[] = [];
  const catalog = createModelCatalog({ read: scope => { const request = deferred<unknown>(); requests.push({ scope, ...request }); return request.promise; },
...
test('model catalog rejects superseded responses and captures the request endpoint', async () => {
  ...
  present(f.requests[1]).resolve([model('new')]); await next;
  present(f.requests[0]).resolve([model('old')]); await old;
  assert.equal(present(f.catalog.rows()[0]).id, 'new');
  assert.equal(present(f.requests[0]).scope.host.token, 'fixture-a');
  assert.equal(Object.isFrozen(present(f.requests[0]).scope.host), true);
...
test('retired or invalid model owners cannot publish success or failure', async () => {
  for (const action of ['retire', 'view', 'clear']) {
    ...
    present(f.requests[0]).reject(new Error('retired failure')); await old;
...
test('cached models remain visible during a cwd refresh while the stale request cannot publish', async () => {
  ...
  cwd = 'new';
  assert.equal(present(f.catalog.rows()[0]).id, 'cached');
  present(f.requests[0]).resolve([model('stale')]); await old;
  assert.equal(present(f.catalog.rows()[0]).id, 'cached');
  hostOwned = false;
  assert.equal(f.catalog.rows().length, 0);
...
test('model edit writers preserve persisted snapshots and provider filtering', async () => {
  ...
  present(f.requests[0]).resolve([model('first'), model('second'), model('outside', 'other')]); await load;
  const persisted = present(f.persisted[0]).rows;
  f.catalog.toggleProvider('fixture', 'first');
  assert.deepEqual(plain(f.catalog.enabledIds()), ['fixture/second', 'other/outside']);
  assert.equal(present(persisted[0]).enabled, true);
