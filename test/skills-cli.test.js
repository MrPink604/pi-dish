// Generated test/tool from test/skills-cli.test.ts; edit that source and run npm run build:tests.
"use strict";
...
async function settleIndex() {
    for (let i = 0; i < 100; i++) {
        ...
        if (!body.indexing && previous.length >= 3 + REFERENCE_FIXTURES.length)
            return;
    }
...
test('resolve expands a unique prefix to the full id', async () => {
    ...
    const parsed = (0, test_types_js_1.record)(JSON.parse((await run(['resolve', '2026-08-24', '--json'])).stdout));
    assert.equal(parsed.id, UNIQUE_ID);
    assert.equal(parsed.host, null, 'a local ref carries no proxy segment');
    assert.equal((0, test_types_js_1.record)(parsed.session).cwd, '/home/user/skillproj');
});
...
test('every host-qualified ref form reaches this host, and unknown ones do not', async () => {
    const identity = (0, test_types_js_1.record)(await (await fetch(`${base}/api/host`)).json());
    ...
    assert.equal((0, test_types_js_1.record)(JSON.parse((await run(['resolve', `${label}/2026-08-24`, '--json'])).stdout)).host, null, 'this host resolves to local, never to a proxy segment');
    // The provenance form carries a whole id by construction; prefix matching
    // there would silently retarget a stale reference.
    await assert.rejects(run(['resolve', `${hostId}:2026-08-24`]), (error) => {
        assert.match((0, test_types_js_1.errorMessage)(error), /Session not found/);
        return true;
    });
    await assert.rejects(run(['resolve', 'nosuchhost/2026-08-24']), (error) => {
        const message = (0, test_types_js_1.errorMessage)(error);
    ...
    await assert.rejects(run(['resolve', `self/${UNIQUE_ID}`, '--host', 'elsewhere']), (error) => {
        assert.match((0, test_types_js_1.errorMessage)(error), /names host "self" but --host elsewhere was also given/);
        return true;
    });
});
test('an ambiguous prefix fails with the candidates listed', async () => {
    await assert.rejects(run(['resolve', AMBIGUOUS_PREFIX]), (error) => {
...
test('printed references preserve admitted Pi/OMP identity and avoid shared UUID ambiguity', async () => {
    ...
    for (const [harness, native, name] of REFERENCE_FIXTURES) {
        ...
        const printedRef = (0, test_types_js_1.present)(row[0]);
        assert.equal((0, test_types_js_1.record)(JSON.parse((await run(['resolve', printedRef, '--json'])).stdout)).id, id, name);
        assert.equal((0, test_types_js_1.record)(await client.resolveSessionClientSide(base, null, printedRef)).id, id, name);
        const reply = (0, test_types_js_1.record)(await (await fetch(`${base}/api/sessions/resolve?id=${encodeURIComponent(id)}`)).json());
        const roundTrip = (0, test_types_js_1.record)(await (await fetch(`${base}/api/sessions/resolve?id=${encodeURIComponent(String(reply.ref))}`)).json());
        assert.equal((0, test_types_js_1.record)(roundTrip.session).id, id, name);
        if (name === 'Ordinary UUID')
            assert.equal(printedRef, '01a07264');
    }
    const ambiguous = await fetch(`${base}/api/sessions/resolve?id=${SHARED_UUID}`);
    assert.equal(ambiguous.status, 409);
    const ambiguousBody = (0, test_types_js_1.record)(await ambiguous.json());
    assert.deepEqual((0, test_types_js_1.records)(ambiguousBody.matches).map(row => row.name).sort(), ['Shared UUID morning', 'Shared UUID next day']);
    await assert.rejects(run(['resolve', SHARED_UUID]), (error) => (0, test_types_js_1.record)(error).code === 1 && /ambiguous/.test((0, test_types_js_1.errorMessage)(error)));
});
test('a prefix shorter than the minimum is rejected', async () => {
    await assert.rejects(run(['resolve', '20']), (error) => {
        assert.match((0, test_types_js_1.errorMessage)(error), /at least 4 characters/);
        return true;
    });
});
...
test('read prints the exact invocation that pages further back, and it works', async () => {
    ...
    const hint = (0, test_types_js_1.present)(first.stdout.trim().split('\n').at(-1));
...
test('show returns the same window as raw JSON', async () => {
    const shown = (0, test_types_js_1.record)(JSON.parse((await run(['show', '2026-08-24', '--limit', '5'])).stdout));
    assert.equal((0, test_types_js_1.record)(shown.session).id, UNIQUE_ID);
    assert.ok((0, test_types_js_1.records)(shown.messages).some(message => (0, test_types_js_1.records)(message.content).some(block => block.text === 'investigate the zarquon index stall')));
});
test('search finds the fixture session by a term from its transcript', async () => {
    ...
    const parsed = (0, test_types_js_1.record)(JSON.parse((await run(['search', 'zarquon', '--json'])).stdout));
    assert.ok((0, test_types_js_1.records)(parsed.results).some(result => result.id === UNIQUE_ID));
});
test('search rejects --all-hosts together with --host', async () => {
    await assert.rejects(run(['search', 'zarquon', '--all-hosts', '--host', 'nope']), (error) => {
...
test('search --all-hosts works on a fleet of one', async () => {
    ...
    const parsed = (0, test_types_js_1.record)(JSON.parse((await run(['search', 'zarquon', '--all-hosts', '--json'])).stdout));
...
test('docs lists the topics this server actually ships', async () => {
    ...
    const parsed = (0, test_types_js_1.record)(JSON.parse((await run(['docs', '--json'])).stdout));
    assert.deepEqual((0, test_types_js_1.records)(parsed.topics).map(topic => topic.name).sort(), ['fleet', 'refs', 'routines', 'search', 'sessions']);
});
...
test('an unknown docs topic names the ones that exist', async () => {
    await assert.rejects(run(['docs', 'nosuchtopic']), (error) => {
        const message = (0, test_types_js_1.errorMessage)(error);
...
test('an unknown command errors and points at the command list', async () => {
    await assert.rejects(run(['frobnicate']), (error) => {
        const detail = (0, test_types_js_1.record)(error), message = (0, test_types_js_1.errorMessage)(error);
        assert.equal(detail.code, 1);
        assert.match(message, /unknown command: frobnicate/);
        assert.match(message, /--help/);
        return true;
    });
    await assert.rejects(run(['help', 'frobnicate']), (error) => {
        assert.match((0, test_types_js_1.errorMessage)(error), /unknown command: frobnicate/);
        return true;
    });
});
test('a ref for a session that does not exist is a plain not-found', async () => {
    await assert.rejects(run(['read', 'zzzz-no-such-session']), (error) => {
        const message = (0, test_types_js_1.errorMessage)(error);
