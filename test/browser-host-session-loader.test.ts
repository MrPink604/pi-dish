import type { RequestOptions } from '../src/browser/api-client.js';
import type { HostConnectionEvent } from '../src/browser/host-connections.js';
import type { SessionHost } from '../src/browser/host-session-loader.js';
import type { SessionLists } from '../src/browser/session-state.js';
...
const { stripQueryField }: typeof import('../lib/helper-query') = require('../lib/helper-query')
const { decodeSessionList }: typeof import('../lib/session-api') = require('../lib/session-api')
...
interface LoaderCall {
  ...
  resolve: (wire: unknown) => void;
  reject: (reason?: unknown) => void;
}
interface LoaderEvent {
  type: 'connection' | 'indexing' | 'before' | 'publish' | 'error';
  host?: SessionHost;
...
const at = <T>(values: readonly T[], index: number): T => present(values[index]);
function fixture() {
  let sequence = 1;
  const calls: LoaderCall[] = [], events: LoaderEvent[] = [];
  const loader = createHostSessionLoader({
    requestList: (host, requestPath, options) => new Promise((resolve, reject) => calls.push({
      host, path: requestPath, options, resolve: (wire: unknown) => resolve(decodeSessionList(wire)), reject,
    })),
  ...
  return { loader, calls, events, setSequence: (value: number) => { sequence = value; } };
}
const host = { hostId: 'peer', base: '/hosts/peer' };
const rows = (name: string) => ({ active: [{ id: 'one', name }], previous: [] });
...
test('identical host polls share a request and publish for the newest joining query', async () => {
  ...
  assert.equal(at(present(loader.getCache(host)).active, 0).name, 'current');
});
...
test('superseded responses cannot publish or clear a newer in-flight request', async () => {
  ...
  await second;
  assert.equal(at(present(loader.getCache(host)).active, 0).name, 'fresh');
});
...
test('hosts publish independently and an obsolete fan-out cannot change cached rows', async () => {
  ...
  await fast;
  assert.equal(at(present(loader.getCache(self)).active, 0).name, 'local');
  assert.equal(loader.getCache(host), undefined);
  setSequence(2);
...
test('active polls preserve advisory family hints and merge live children until a full refresh', async () => {
  ...
  const incoming: { active: Array<{ id: string; name: string; parentId?: string }>; previous: never[]; children: Array<{ id: string; name?: string; subagentLive: boolean }> } = {
  ...
  await pending;
  const cached = present(loader.getCache(host));
  ...
  await pending;
  assert.equal(at(present(loader.getCache(host)).active, 0).parentId, undefined);
  assert.equal(present(loader.getCache(host)).previous.length, 0);
...
test('failed and unauthorized polls retain cached rows and report the current host state', async () => {
  ...
  await pending;
  assert.equal(at(present(loader.getCache(host)).active, 0).name, 'cached');
  ...
  assert.equal(record(at(events, 0).event).error, offline);
});
...
test('pruning retires old requests even when the same host is added again', async () => {
  ...
  assert.equal(at(present(loader.getCache(host)).active, 0).name, 'replacement');
  assert.equal(loader.isIndexing(), false);
});
...
test('a changed host endpoint starts a new request and a retired 401 cannot block its success', async () => {
  ...
  assert.equal(record(at(calls, 0).host).base, '/hosts/peer');
  assert.equal(record(at(calls, 0).host).token, 'fixture-old');
  ...
  await old;
  assert.equal(events.length, count);
  assert.equal(at(present(loader.getCache(mutable)).active, 0).name, 'replacement');
});
...
for (const hint of [undefined, null, '']) {
  test(`active-only hints preserve prior parent and family for ${String(hint)} while full scans replace them`, async () => {
    ...
    at(calls, 1).resolve(incoming);
    await pending;
    assert.equal(at(present(loader.getCache(host)).active, 0).parentId, 'parent');
    assert.equal(at(present(loader.getCache(host)).active, 0).parentSource, 'native');
    assert.equal(at(present(loader.getCache(host)).active, 0).familyParentId, 'family');
    pending = loader.load(host, undefined, true, 1);
    at(calls, 2).resolve(incoming);
    await pending;
    assert.equal(at(present(loader.getCache(host)).active, 0).parentId, hint);
    assert.equal(at(present(loader.getCache(host)).active, 0).familyParentId, hint);
  });
}
...
test('acknowledged metadata survives failed fan-out and partial child replay through sidebar publication', async () => {
  ...
  const lists = createSidebarLists({
    ...
    request: (target, requestPath) => new Promise<Response>((resolve, reject) => calls.push({
      target, path: requestPath, reject, resolve: (wire: unknown) => resolve(new Response(JSON.stringify(wire))),
    })),
  });
  try {
    ...
    await pending;
    assert.equal(present(state.findSession('same', 'self')).name, 'fresh local');
    assert.equal(present(state.findSession('same', 'peer')).name, 'remote acknowledgement');
    assert.equal(present(state.findSession('same', 'peer')).model, 'ack/model');
    assert.equal(at(present(lists.loader.getCache(host)).active, 0).model, 'ack/model');
    assert.equal(present(state.currentSession).model, 'ack/child');
    ...
    await pending;
    assert.equal(present(state.findSession('same', 'self')).name, 'fresh local');
    assert.equal(present(state.findSession('same', 'peer')).name, 'fresh remote', 'fresh HTTP facts remain authoritative');
    assert.equal(present(state.findSession('same', 'peer')).model, undefined, 'fresh active rows do not inherit omitted metadata');
    const child = present(state.findSession('child', 'peer'));
    ...
    assert.equal(present(state.currentSession).name, 'new child acknowledgement');
    assert.equal(at(present(lists.loader.getCache(host)).previous, 0).model, 'ack/child');
    state.mergeCurrentSession(state.captureSelection(), { model: 'transcript/only' });
    assert.equal(present(state.currentSession).model, 'transcript/only');
    assert.equal(at(present(lists.loader.getCache(host)).previous, 0).model, 'ack/child', 'transcript metadata cannot enter the borrowed list cache');
  } finally {
    lists.dispose();
  }
});
