// Generated test/tool from test/omp-tree-bridge.test.ts; edit that source and run npm run build:tests.
"use strict";
...
async function startHost(mode) {
    ...
    const child = (0, node_child_process_1.spawn)('bun', [fixture], {
    ...
    try {
        ...
        const registered = await waitForValue(() => {
            ...
            const entry = (0, test_types_js_1.bridgeEntry)((0, test_types_js_1.record)(JSON.parse(fs.readFileSync(registryPath, 'utf8'))));
        ...
        return {
            ...
            async stop() {
                ...
                await Promise.race([
                    ...
                    delay(2000).then(() => { if (child.exitCode === null)
                        child.kill('SIGKILL'); }),
                ]);
...
function readOperations(host) {
    return fs.readFileSync(host.log, 'utf8').trim().split('\n').filter(Boolean).map(line => {
        ...
        if (!(0, wire_protocol_js_1.isRecord)(value))
            throw new Error('Invalid operation log entry');
        return value;
    });
}
test('OMP bridge serializes tree reads and runs navigate/branch only in command context', { skip: !bunAvailable }, async () => {
    ...
    try {
        ...
        assert.equal(host.entry.treeServiceShortcut, 'ctrl+alt+shift+f12', 'the registry advertises the chord pi-dish presses instead of typing the command');
        const commands = (0, test_types_js_1.record)(await host.session.getCommands());
        assert.equal((0, test_types_js_1.records)(commands.commands).some(command => (0, test_types_js_1.record)(command).name === 'dish-tree-service'), false, 'the internal service command is not advertised to pi-dish clients');
    }
    finally {
        await host.stop();
    }
...
test('OMP bridge maps command-context acquisition and operation timeouts to distinct errors', { skip: !bunAvailable }, async (t) => {
    await t.test('acquisition timeout', async () => {
        ...
        try {
            await assert.rejects(host.session.treeNavigate('a1'), /tree navigation command context acquisition timed out after 100ms/);
        }
        finally {
            await host.stop();
        }
...
test('OMP bridge capability-gates missing read and command-context APIs', { skip: !bunAvailable }, async (t) => {
    await t.test('missing ReadonlySessionManager API degrades before registration', async () => {
        ...
        try {
            const capabilities = (0, test_types_js_1.record)(host.entry.capabilities);
...
test('OMP bridge without shortcut support keeps the typed service command', { skip: !bunAvailable }, async () => {
    ...
    try {
        ...
        assert.equal((0, test_types_js_1.record)(await operation).leafId, 'a1');
        assert.deepEqual(readOperations(host), [
