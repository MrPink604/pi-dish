...
import type os = require('node:os');
const { createHostHealth, parseMemAvailable, defaultHostHealthDeps }: typeof import('../lib/host-health.js') = require('../lib/host-health.js');
type HostHealthDeps = import('../lib/host-health.js').HostHealthDeps;

const GB = 1024 ** 3;
const MEMINFO = 'MemTotal:       16384000 kB\nMemFree:         1024000 kB\nMemAvailable:    8192000 kB\nBuffers:          100 kB\n';

/** Two cores whose cumulative busy/idle times the test advances explicitly. */
function fakeDeps(overrides: Partial<HostHealthDeps> = {}) {
  const clock = { now: 1_000_000 };
  const cpu = { busy: 0, idle: 0 };
  const calls = { sleep: [] as number[], cpus: 0, statfs: [] as string[] };
  // Work done while a short sample sleeps, so first reads measure something.
  let duringSleep = { busy: 0, idle: 0 };
  const core = (): os.CpuInfo => ({
    model: '  Fixture CPU  ', speed: 1000,
    times: { user: cpu.busy / 2, nice: 0, sys: cpu.busy / 2, idle: cpu.idle, irq: 0 },
  });
  const deps: HostHealthDeps = {
    ...
    sleep: async ms => {
      calls.sleep.push(ms);
      clock.now += ms;
      cpu.busy += duringSleep.busy; cpu.idle += duringSleep.idle;
    },
    ...overrides,
  };
  return {
    deps, clock, cpu, calls,
    work(busy: number, idle: number) { cpu.busy += busy; cpu.idle += idle; },
    sampleWork(busy: number, idle: number) { duringSleep = { busy, idle }; },
  };
}
...
test('a failed snapshot is not shared with later callers', async () => {
  let fail = true;
  const fake = fakeDeps({ loadavg: () => { if (fail) throw new Error('loadavg failed'); return [0, 0, 0]; } });
  const health = createHostHealth(fake.deps);
