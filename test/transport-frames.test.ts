...
import { decodeRPCFrame, decodeBridgeFrame, isRecord, type ProtocolRecord } from '../lib/wire-protocol.js';
...
function rpcFixture(t: test.TestContext) {
...
test('bridge socket ignores bad frames and continues through a valid response and event', async (t) => {
  ...
  const server = net.createServer(sock => {
    ...
    sock.on('data', (chunk: Buffer) => {
      const command: unknown = JSON.parse(chunk.toString());
  ...
  const forwarded = new Promise<unknown>(resolve => session.once('future_event', resolve));
...
test('RPC stdio ignores bad response status and still forwards subsequent agent events', async (t) => {
  ...
  const commandBytes: unknown = proc.stdin.read();
  assert.ok(Buffer.isBuffer(commandBytes));
  const command: unknown = JSON.parse(commandBytes.toString());
  assert.ok(isRecord(command));
...
test('RPC reconstructs text, thinking and tool-call deltas without mutating prior snapshots', (t) => {
  ...
  const snapshots: ProtocolRecord[] = [];
  session.on('message_update', data => {
    assert.ok(isRecord(data) && isRecord(data.message));
  ...
  const update = (event: unknown) => session._handleMessage({ type: 'message_update', assistantMessageEvent: event });
  ...
  const firstBlock: unknown = firstSnapshot.content[0];
