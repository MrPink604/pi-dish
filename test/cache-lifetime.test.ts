...
const CL: typeof import('../lib/cache-lifetime.js') = require('../lib/cache-lifetime.js');
const index: typeof import('../lib/session-index.js') = require('../lib/session-index.js');
const SM: typeof import('../lib/session-metadata.js') = require('../lib/session-metadata.js');
...
interface CacheUsage { read?: number; write?: number; write1h?: number }

function cacheMsg(t: number, usage: CacheUsage, provider = 'openai', model = 'gpt-4o', api = 'openai-responses') {
 ...
 */
function cliffEntries(startMs: number, ttlMs: number, probes: number,
  provider?: string, model?: string, api?: string): { entries: unknown[]; lastT: number } {
  const entries: unknown[] = [cacheMsg(startMs, { write: 100 }, provider, model, api)];
...
function cliffStart(ttlMs: number, probes: number): number {
  return Date.now() - Math.round(probes * ttlMs * 1.4) - 30 * MIN;
}

function observe(entries: unknown[], file = path.join(tmpHome, `learn-${fileSeq++}.jsonl`)) {
  CL.observeCacheLifetimeFromEntries(entries, file);
  return file;
}

function snapshotFor(identity: string) {
...
test('fit activates on a clean cliff and lands on the true TTL', () => {
  ...
  const entry = present(snapshotFor(openAiIdentity));
...
test('overlay substitutes learned TTL for estimates but never fixed or overridden', () => {
  ...
  const expiry = { refreshedAt, expiresAt: refreshedAt + 10 * MIN, retentionMs: 10 * MIN,
    retention: '~10m', basis: 'estimate' as const, identity: openAiIdentity };
  const learned = present(CL.applyLearnedCacheExpiry(expiry));
  ...
  const fixed = { ...expiry, basis: 'fixed' as const, retention: '5m', retentionMs: 5 * MIN };
...
test('gates reject thin and one-sided evidence', () => {
  ...
  assert.equal(present(snapshotFor(['acme-api', 'acme', 'model-a'].join(SEP))).active, false, 'six probes is not support');

  const t0 = Date.now() - 40 * MIN;
  const hitsOnly: unknown[] = [cacheMsg(t0, { write: 100 }, 'acme', 'model-b', 'acme-api')];
  for (let i = 1; i <= 30; i++) {
    hitsOnly.push(cacheMsg(t0 + i * MIN, { read: 90, write: 5 }, 'acme', 'model-b', 'acme-api'));
  }
  observe(hitsOnly);
  assert.equal(present(snapshotFor(['acme-api', 'acme', 'model-b'].join(SEP))).active, false, 'no misses: crossing never bracketed');
});
...
test('extended cache tier learns separately from the default tier', () => {
  ...
  const anthropic = (t: number, usage: CacheUsage) => cacheMsg(t, usage, 'anthropic', 'claude-opus-4', 'anthropic-messages');
  observe([
  ...
  assert.equal(present(snapshot.find(entry => entry.tier === '1h')).rawObservations, 2);
  assert.equal(present(snapshot.find(entry => entry.tier === null)).rawObservations, 1);
});
...
test('long-idle probes survive a window flooded by tool-loop chatter', () => {
  ...
  const report = present(CL.cacheLifetimeReport().find(entry => entry.model === 'model-d'));
  ...
  const entry = present(snapshotFor(identity));
...
test('a learned TTL serves providers with no built-in window, and only once active', () => {
  ...
  assert.equal(present(snapshotFor(identity)).active, true);
  const served = present(CL.applyLearnedCacheExpiry(unknown));
  assert.equal(served.basis, 'learned');
  assert.equal(served.expiresAt, refreshedAt + served.retentionMs);
  const report = present(CL.cacheLifetimeReport().find(entry => entry.model === 'model-e'));
...
test('the report explains precedence and the first failing gate', () => {
  const byModel = (model: string) => present(CL.cacheLifetimeReport().find(entry => entry.model === model && entry.tier === null));
  ...
  try {
    ...
    assert.equal(present(overridden.effective).retention, '2h');
  } finally {
    fs.rmSync(dishSettingsFile, { force: true });
  }
});
...
test('scanSessions mines observations from indexed sessions and their appends', () => {
  ...
  const source = sourceForIdentity('pi', nativeId('learned-scan'), file);
  index.scanSessions([source]);
  const mined = present(snapshotFor(openAiIdentity));
  ...
  index.scanSessions([source]);
  assert.equal(present(snapshotFor(openAiIdentity)).rawObservations, mined.rawObservations + 1, 'append extends the chain');
});
