...
async function setup(page: Page) {
  await page.route('**/api/harnesses', (route: Route) => route.fulfill({ json: { harnesses: [
  ...
  await page.route('**/api/harnesses/omp/config?*', (route: Route) => route.fulfill({ json: { defaultModel: 'ready', modelRoles: { smol: 'fixture/model' } } }));
...
test('a previous cwd config body cannot restore defaults or edit actions during debounce', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => {
    ...
    Response.prototype.json = async function () {
      ...
      if (this.url.includes('/harnesses/omp/config?cwd=%2Fold')) {
        ...
        await new Promise<void>(resolve => { window.releaseConfigBody = resolve; });
      }
      return value;
    };
  ...
  expect(await page.evaluate(() => fixtureElement(fixtureApp.features.newSessionController.config.config, 'new-session config').cwd)).toBe('/new');
});
...
test('old-host defaults cannot replace the selected peer readout', async ({ page, fleet }) => {
  ...
  await requiredRoute(held).fulfill({ json: { defaultModel: 'old-self' } });
  await page.evaluate(() => window.heldConfig);
  await expect(page.locator('#nsHarnessConfigValues')).toContainText('peer');
  expect(await page.evaluate(() => fixtureElement(fixtureApp.features.newSessionController.config.config, 'peer new-session config').defaultModel)).toBe('peer');
});
...
test('cwd blur keeps the spawn button under the pointer until mouse-up', async ({ page, fleet }) => {
  ...
  if (before === null) throw new Error('Missing before');
  let sent = false;
