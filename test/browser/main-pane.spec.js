...
async function openTakeover(page, name) {
  await page.evaluate(async name => {
    ...
    if (name === 'subagents') {
      ...
      document.querySelector('.session-relation-tree-link').click();
    } else {
...
test('every takeover excludes each other takeover without retiring its own opened surface', async ({ page, fleet }) => {
  ...
  for (const from of Object.keys(takeovers)) {
    for (const [to, className] of Object.entries(takeovers)) {
...
test('ordinary takeovers retain file and settings while recovery and subagents retire them and late file bodies', async ({ page, fleet }) => {
  ...
  for (const clearing of ['recovery', 'subagents']) {
    held = null;
    ...
    await held.fulfill({ json: { path: '/fixture/held.txt', relPath: 'held.txt', content: 'retired file body', size: 17, mtime: 1 } });
