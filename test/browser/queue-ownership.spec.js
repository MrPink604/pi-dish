...
test('identical queued prompts retain their own bubbles and cancellation owner', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => { window.editRequest = fixtureApp.features.promptDelivery.edit(document.querySelector('.queue-item-edit')); });
  ...
  expect(await page.evaluate(ids => [fixtureApp.features.promptDelivery.has(ids.self), fixtureApp.features.promptDelivery.has(ids.peer)], ids)).toEqual([true, false]);
