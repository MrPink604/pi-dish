...
for (const kind of ['prompt', 'command', 'steer', 'followUp']) {
  test(`failed ${kind} restores payload to the origin after switching hosts`, async ({ page, fleet }) => {
    ...
    await page.route(endpoint, route => {
      if (!receive) throw new Error('Composer route resolver not initialized');
      receive(route);
    });
...
test('an image finishing preparation after navigation belongs to its starting composer', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => {
    ...
    Object.defineProperty(window, 'createImageBitmap', { configurable: true, value(source: ImageBitmapSource) {
      return new Promise<ImageBitmap>((resolve, reject) => {
        window.finishImage = async () => {
          try { resolve(await prepare(source)); } catch (error) { reject(error); }
        };
      });
    } });
  });
