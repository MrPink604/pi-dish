...
async function trackLoads(page) {
  await page.evaluate(() => {
    const load = loadSpawnTargets;
    window.targetLoads = [];
    loadSpawnTargets = () => { const pending = load(); window.targetLoads.push(pending); return pending; };
  });
}
...
test('closing the takeover retires a tmux response held in its body reader', async ({ page, fleet }) => {
  ...
  await page.evaluate(async () => { closeNewSessionView(); window.releaseTargetBody(); await window.targetLoads[0]; });
  expect(await page.evaluate(() => selectedSpawnTarget())).toBeNull();
...
test('run-in keyboard actions validate new names and keep saved resumes on their host', async ({ page, fleet }) => {
  ...
  expect(await page.evaluate(() => { try { selectedSpawnTarget(); } catch (e) { return e.message; } })).toContain('Enter a name');
  await page.locator('#newSessionTmuxName').fill(' fresh ');
  expect(await page.evaluate(() => selectedSpawnTarget())).toEqual({ type: 'tmux', socket: '/self', newTmuxSession: 'fresh' });
  await input.focus();
  ...
  expect(await page.evaluate(() => selectedSpawnTarget())).toEqual({ type: 'tmux', socket: '/self', tmuxSession: 'work' });
  expect(await page.evaluate(id => savedResumeTarget(id), fleet.peer.hostId)).toBeNull();
  expect(await page.evaluate(id => savedResumeTarget(id), fleet.self.hostId)).toEqual({ type: 'tmux', socket: '/self', tmuxSession: 'work' });
});
