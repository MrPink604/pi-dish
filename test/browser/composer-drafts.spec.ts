...
async function pendingImage(page: Page) {
  await page.evaluate(png => {
    window.bitmapClosed = 0;
    Object.defineProperty(window, 'createImageBitmap', {
      configurable: true,
      value: () => new Promise<ImageBitmap>(resolve => {
...
test('late image conversion stays with the same-id originating host', async ({ page, fleet }) => {
  ...
  if (owner === null) throw new Error('Missing self composer owner');
  await fleet.select(fleet.peer); await page.evaluate(async () => { void window.finishBitmap(); await window.imagePending; });
...
test('startup draft migration flushes pending text and carries unfinished image conversion to the registered session', async ({ page, fleet }) => {
  ...
  if (owner === null) throw new Error('Missing startup composer owner');
...
test('retained attachment remove buttons cannot affect a replacement or another host', async ({ page, fleet }) => {
  ...
  await page.evaluate(png => { const owner = fixtureElement(fixtureApp.features.composerDrafts.key, 'composer owner'); fixtureApp.features.composerDrafts.images.replace(owner, [{ data: png, mimeType: 'image/png' }]); window.oldRemove = document.querySelector<HTMLElement>('.attachment-remove'); }, PNG);
...
test('clearing a foreign draft does not cancel the visible draft debounce or mix its history', async ({ page, fleet }) => {
  ...
  await expect.poll(() => page.evaluate(() => localStorage.getItem(fixtureApp.features.composerDrafts.draftKey(fixtureElement(fixtureApp.features.composerDrafts.key, 'composer key'))))).toBe('Visible pending draft');
  await page.evaluate(() => fixtureApp.features.composerDrafts.navigate(-1, fixtureElement(document.querySelector<HTMLTextAreaElement>('#promptInput'), '#promptInput'))); await expect(page.locator('#promptInput')).toHaveValue('local history');
  await page.evaluate(() => fixtureApp.features.composerDrafts.navigate(-1, fixtureElement(document.querySelector<HTMLTextAreaElement>('#promptInput'), '#promptInput'))); await expect(page.locator('#promptInput')).toHaveValue('local history');
  await page.evaluate(() => fixtureApp.features.composerDrafts.navigate(1, fixtureElement(document.querySelector<HTMLTextAreaElement>('#promptInput'), '#promptInput'))); await expect(page.locator('#promptInput')).toHaveValue('Visible pending draft');
});
...
test('malformed history is narrowed and disposal flushes text while closing late bitmap and lightbox resources', async ({ page, fleet }) => {
  ...
  if (owner === null) throw new Error('Missing disposal composer owner');
