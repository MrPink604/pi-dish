...
async function open(page) {
  await page.evaluate(() => openSkillsView());
...
test('retained skill controls retire on navigation and refine chooses the skill host', async ({ page, fleet }) => {
  ...
  await page.evaluate(host => newSessionController.setHostId(host), fleet.peer.hostId);
  await page.locator('.refine-btn').click();
  await expect(page.locator('.main')).toHaveClass(/new-session-open/);
  expect(await page.evaluate(() => newSessionController.hostId())).toBe(fleet.self.hostId);
  expect(await page.evaluate(() => newSessionController.pendingDraft)).toContain('/fixture/one/SKILL.md');
  await page.evaluate(() => window.retiredSkillRow.click());
  await expect(page.locator('.main')).toHaveClass(/new-session-open/);
});
...
test('disposal cancels directory indexing and ignores retained refresh controls', async ({ page, fleet }) => {
  ...
  await open(page);
  await page.evaluate(() => { window.retiredSkillsRefresh = document.querySelector('[data-skills-action="refresh"]'); skillsController.dispose(); window.retiredSkillsRefresh.click(); });
  const before = reads;
...
test('a late activation list lookup cannot hijack a newer selection', async ({ page, fleet }) => {
  ...
  const selected = await page.evaluate(async ({ peer, id }) => {
    const original = loadSessions;
    let release;
    loadSessions = () => new Promise(resolve => { release = resolve; });
    try {
      const pending = openSkillActivation('not-yet-listed', 'entry');
      await selectSession(id, { host: peer });
      release(); await pending;
      return sessionState.currentSession.host;
    } finally { loadSessions = original; }
