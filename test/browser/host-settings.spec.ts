  ...
  await page.evaluate(() => {
    ...
    Response.prototype.json = async function() {
      ...
      if (this.url.endsWith('/hosts/edit/api/host')) {
        ...
        await new Promise<void>(resolve => { window.releaseHostBody = resolve; });
      }
      return data;
    };
...
test('add-host controls validate a URL and persist the captured label and token', async ({ page, fleet }) => {
  ...
  const catalog: unknown = await page.evaluate(() => JSON.parse(localStorage.getItem('pi-dish-hosts') ?? '[]'));
  if (!Array.isArray(catalog)) throw new Error('Invalid stored host catalog');
  expect(catalog.find(host => isRecord(host) && host.hostId === fleet.peer.hostId)).toEqual({ base: fleet.peer.base,
