...
import type { AnchoredComment } from '../../src/browser/anchored-comment-data.js';
import { test, expect, ROOT, requiredRoute } from './fixtures.js';
const comment = (id: string, body = id): AnchoredComment => ({ id, sessionId: ROOT, body, target: { kind: 'file', path: '/fixture/a.txt', relPath: 'a.txt', anchor: { type: 'text', quote: 'beta', prefix: 'alpha ', suffix: ' gamma' } } });
async function setup(page: Page, fleet: FleetFixture, entries: AnchoredComment[] = []) {
  await fleet.select(fleet.self);
  await page.route('**/api/sessions/*/file?*', (route: Route) => route.fulfill({ json: { path: '/fixture/a.txt', relPath: 'a.txt', content: 'alpha beta gamma', size: 16, mtime: 1 } }));
  await page.route('**/api/comments/index?*', (route: Route) => route.fulfill({ json: { comments: entries } }));
  await page.route('**/api/comments/get', (route: Route) => route.fulfill({ json: { comments: entries } }));
...
async function selectText(page: Page) {
  await page.evaluate(() => {
    ...
    const text = fixtureElement(code.firstChild, 'file view text'), range = document.createRange(); range.setStart(text, 6); range.setEnd(text, 10);
    const selection = fixtureElement(window.getSelection(), 'window selection'); selection.removeAllRanges(); selection.addRange(range); fixtureApp.features.anchoredCommentController.captureFile();
  });
...
test('overlapping comment refreshes cannot replace newer marks with an older body response', async ({ page, fleet }) => {
  await setup(page, fleet); let held: Route | null | undefined; let index = 0;
  ...
  await page.evaluate(() => { window.oldComments = fixtureApp.features.anchoredCommentController.refresh(); }); await expect.poll(() => !!held).toBe(true);
  await page.evaluate(() => fixtureApp.features.anchoredCommentController.refresh()); await requiredRoute(held).fulfill({ json: { comments: [comment('old', 'Old body')] } }); await page.evaluate(() => window.oldComments);
...
test('late save failure cannot mark a new same-id host editor and duplicate submit is suppressed', async ({ page, fleet }) => {
  await setup(page, fleet); await selectText(page); await page.locator('#commentBody').fill('Old draft'); let held: Route | undefined, writes = 0;
  await page.route('**/api/comments', route => { held = route; writes++; });
  await page.evaluate(() => { window.oldSave = fixtureApp.features.anchoredCommentController.submit(); void fixtureApp.features.anchoredCommentController.submit(); }); await expect.poll(() => !!held).toBe(true); expect(writes).toBe(1);
  await fleet.select(fleet.peer); await page.evaluate(() => fixtureApp.features.fileViews.openFile('a.txt')); await selectText(page); await page.locator('#commentBody').fill('Peer draft');
  await requiredRoute(held).fulfill({ status: 500, json: { error: 'old save failed' } }); await page.evaluate(() => window.oldSave);
...
test('a pending delete cannot disable or close another comment editor', async ({ page, fleet }) => {
  await setup(page, fleet, [comment('first'), comment('second')]); await page.evaluate(() => fixtureApp.features.anchoredCommentController.focus('first'));
  let held: Route | null | undefined; await page.route('**/api/comments/first', route => { held = route; });
  await page.evaluate(() => { void fixtureApp.features.anchoredCommentController.remove(); window.oldDelete = fixtureApp.features.anchoredCommentController.remove(); }); await expect.poll(() => !!held).toBe(true);
  ...
  await requiredRoute(held).fulfill({ status: 500, json: { error: 'old delete failed' } }); await page.evaluate(() => window.oldDelete);
...
test('replaced and closed comment list rows cannot reopen obsolete editors', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => { window.oldCommentRow = document.querySelector<HTMLElement>('.comment-list-row'); });
  await page.evaluate(entry => fixtureApp.features.anchoredCommentController.set([entry]), comment('new'));
  await page.evaluate(() => fixtureElement(window.oldCommentRow, 'old comment row').click()); await expect(page.locator('#commentBubble')).toBeHidden();
  await page.evaluate(() => { window.closedCommentRow = document.querySelector<HTMLElement>('.comment-list-row'); fixtureApp.features.anchoredCommentController.closeList(); fixtureElement(window.closedCommentRow, 'closed comment row').click(); });
...
test('queued pointer selection is retired when a file view closes and reopens', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => {
    ...
    Object.defineProperty(window, 'setTimeout', { configurable: true, value(handler: TimerHandler, timeout = 0, ...args: unknown[]) {
      if (timeout === 0 && typeof handler === 'function') { window.delayedSelections.push(() => Reflect.apply(handler, window, args)); return 1234567; }
      const timer: unknown = Reflect.apply(native, window, [handler, timeout, ...args]);
      if (typeof timer !== 'number') throw new Error('Fixture timer did not return an id');
      return timer;
    } });
    ...
    Object.defineProperty(window, 'setTimeout', { configurable: true, value: native }); fixtureApp.features.fileViews.closeFile();
  });
  await page.evaluate(() => fixtureApp.features.fileViews.openFile('a.txt'));
  await page.evaluate(() => {
    const code = fixtureElement(document.querySelector('#fileViewBody code'), '#fileViewBody code');
    const text = fixtureElement(code.firstChild, 'file view text'), range = document.createRange(); range.setStart(text, 6); range.setEnd(text, 10); const selection = fixtureElement(window.getSelection(), 'window.getSelection()'); selection.removeAllRanges(); selection.addRange(range);
...
test('comment disposal removes pointer, chip, bubble and delayed-delete actions', async ({ page, fleet }) => {
  await setup(page, fleet, [comment('old')]); await page.evaluate(() => fixtureApp.features.anchoredCommentController.focus('old'));
