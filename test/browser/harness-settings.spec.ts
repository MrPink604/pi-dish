...
async function routes(page: Page) {
  await page.route('**/api/harnesses/omp/config?*', (route: Route) => route.fulfill({ json: {
  ...
  await page.route('**/api/harnesses/omp/agents?*', (route: Route) => route.fulfill({ json: agents }));
  await page.route('**/api/models?*', (route: Route) => route.fulfill({ json: [model] }));
}
async function open(page: Page, host: BrowserHost & { hostId: string }, cwd: string) {
...
test('late settings reads cannot overwrite a newly opened host and custom prototype names stay literal', async ({ page, fleet }) => {
  ...
  await requiredRoute(held).fulfill({ json: { defaultModel: 'old-response', globalModelRoles: {} } });
...
for (const resultStatus of [200, 500]) test(`submitted settings keep their endpoint and ${resultStatus} completion leaves a replacement modal open`, async ({ page, fleet }) => {
  ...
  const payloads = writes.map(write => {
    ...
    const payload: unknown = write.request().postDataJSON();
    if (!isRecord(payload)) throw new Error('Invalid harness settings request');
    expect(payload.cwd).toBe('/self');
    return payload;
  });
  expect(payloads[0]?.agents).toEqual({ scout: { disabled: true } });
  expect(payloads[1]?.roles).toEqual({ smol: null });
...
test('a save completed after the editor closes refreshes its matching takeover defaults', async ({ page, fleet }) => {
  ...
  let modelName = 'before-save', held: Route | undefined;
