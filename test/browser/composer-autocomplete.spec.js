...
test('file debounce captures the original host before selection changes', async ({ page, fleet }) => {
  ...
  await page.evaluate(async ({ id, host }) => { const input = document.getElementById('promptInput'); input.value = '@old'; input.setSelectionRange(4, 4); input.dispatchEvent(new Event('input')); await selectSession(id, { host }); }, { id: ROOT, host: fleet.peer.hostId });
...
test('file selection persists the draft and directory selection drills into its next query', async ({ page, fleet }) => {
  ...
  await expect.poll(() => page.evaluate(() => localStorage.getItem(draftKey(composerDrafts.key)))).toBe('@dir/file.txt ');
});
...
test('late command catalogs cannot replace the current host and command labels remain literal', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => { window.oldCommands = loadCommands(sessionState.currentSession.id); }); await expect.poll(() => !!held).toBe(true);
  await fleet.select(fleet.peer); await page.evaluate(() => loadCommands(sessionState.currentSession.id));
...
test('reference completions retain the target-host reference and persist it', async ({ page, fleet }) => {
  ...
  await expect.poll(() => page.evaluate(() => localStorage.getItem(draftKey(composerDrafts.key)))).toBe('#' + ref + ' ');
});
...
test('autocomplete disposal retires pending lookups and removed row controls', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => composerAutocomplete.dispose()); await held.fulfill({ json: { files: [{ path: 'late.txt' }] } });
  await expect(page.locator('#autocomplete')).toBeHidden(); await page.evaluate(() => handleAutocomplete('@other')); await expect(page.locator('#autocomplete')).toBeHidden();
});
...
test('moving the caret hides an already displayed completion immediately', async ({ page, fleet }) => {
  ...
  expect(await page.evaluate(() => composerAutocomplete.visible)).toBe(false);
