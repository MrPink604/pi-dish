...
async function selectText(page) {
  await page.evaluate(() => {
    ...
    const selection = window.getSelection(); selection.removeAllRanges(); selection.addRange(range); fixtureApp.features.anchoredCommentController.captureFile();
  });
...
test('overlapping comment refreshes cannot replace newer marks with an older body response', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => fixtureApp.features.anchoredCommentController.refresh()); await held.fulfill({ json: { comments: [comment('old', 'Old body')] } }); await page.evaluate(() => window.oldComments);
...
test('replaced and closed comment list rows cannot reopen obsolete editors', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => { window.closedCommentRow = document.querySelector('.comment-list-row'); fixtureApp.features.anchoredCommentController.closeList(); window.closedCommentRow.click(); });
...
test('queued pointer selection is retired when a file view closes and reopens', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => {
    ...
    window.setTimeout = native; fixtureApp.features.fileViews.closeFile();
  });
  await page.evaluate(() => fixtureApp.features.fileViews.openFile('a.txt'));
  await page.evaluate(() => {
...
test('comment disposal removes pointer, chip, bubble and delayed-delete actions', async ({ page, fleet }) => {
  ...
  await page.evaluate(async () => { await fixtureApp.features.anchoredCommentController.remove(); await fixtureApp.features.anchoredCommentController.submit(); await fixtureApp.features.anchoredCommentController.focus('old'); document.getElementById('fileViewComments').click(); });
