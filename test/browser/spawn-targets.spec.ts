...
const targets = (name: string) => ({ available: true, servers: [{ name, socket: '/' + name, sessions: [{ name: 'work' }] }] });
  ...
  await page.evaluate(url => {
    ...
    Response.prototype.json = async function() {
      ...
      if (this.url === url) {
        ...
        await new Promise<void>(resolve => { window.releaseTargetBody = resolve; });
      }
      return data;
    };
  }, url);
}
async function trackLoads(page: Page) {
  await page.evaluate(() => {
...
test('run-in keyboard actions validate new names and keep saved resumes on their host', async ({ page, fleet }) => {
  ...
  expect(await page.evaluate(() => { try { fixtureApp.features.newSessionController.selectedTarget(); } catch (error) { return error instanceof Error ? error.message : String(error); } })).toContain('Enter a name');
...
test('refresh disposes old picker rows and clears targets when tmux becomes unavailable', async ({ page, fleet }) => {
  ...
  await expect(page.locator('#newSessionTargetWrap')).toBeVisible();
  await page.locator('#newSessionTarget').focus();
  await page.evaluate(() => { window.retiredTargetRow = document.querySelector<HTMLElement>('#spawnTargetDropdown .cwd-option:last-child'); });
  available = false;
