...
test('retained relation chips and modal rows cannot retarget a same-id peer or a refreshed view', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => loadSessionRelations(sessionState.captureSelection()));
  ...
  await page.evaluate(() => loadSessionRelations(sessionState.captureSelection()));
  await page.evaluate(() => openRelationsModal());
  ...
  await page.evaluate(async () => { await loadSessionRelations(sessionState.captureSelection()); window.oldRelationRow.click(); });
...
test('relation disposal retires indexing and resize timers as well as rendered controls', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => { window.dispatchEvent(new Event('resize')); sessionRelationsController.dispose(); });
...
for (const close of [false, true]) test(`a late same-session search cannot overwrite ${close ? 'a closed bar' : 'a newer query'}`, async ({ page, fleet }) => {
  ...
  await page.evaluate(() => { openSearch(); window.oldNavigationSearch = runSessionSearch('old-query'); });
  await expect.poll(() => !!held).toBe(true);
  if (close) await page.evaluate(() => closeSearch());
  else await page.evaluate(() => runSessionSearch('self'));
  await held.fulfill({ json: { matches: [{ index: 0, role: 'user' }] } });
  await page.evaluate(() => window.oldNavigationSearch);
  expect(await page.evaluate(() => search.query)).toBe(close ? '' : 'self');
