...
const payload = (name: string) => ({ results: [{ id: ROOT, name, model: 'fixture/model', cwd: '/fixture/project', snippets: [name], matchCount: 0, searchScore: 5 }], total: 1 });
...
test('search facets and result cards retire on replacement while live cards retain their host', async ({ page, fleet }) => {
  ...
  await page.evaluate(host => {
    window.retiredSearchCard = document.querySelector<HTMLElement>(`.search-result[data-host="${host}"]`);
...
test('search disposal retires indexing, input listeners and retained facets', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => {
    ...
    const input = fixtureInput(document.getElementById('searchViewInput'), '#searchViewInput'); input.value = 'later'; input.dispatchEvent(new Event('input'));
  });
  const before = reads;
