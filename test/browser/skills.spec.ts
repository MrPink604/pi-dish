...
const coverage = (name = 'one', latest: { sessionId: string; entryId: string; name: string } | null = null) => ({ skill: `/fixture/${name}/SKILL.md`, numMapped: 2, unreadTokensEst: 3, sections: [{ heading: 'Cold section', neverRead: true, fraction: 0, reads: 0, startLine: 1, endLine: 2, lines: [{ text: 'Body', hits: 0 }] }], weeks26: [0, 1], latest });
async function routes(page: Page) {
  await page.route('**/api/skills', (route: Route) => route.fulfill({ json: { skills: [skill(), skill('two')], summary: { discovered: 2 }, refine: { mode: 'path', mdPath: '/refine.md' } } }));
  await page.route('**/api/skills/coverage?*', (route: Route) => route.fulfill({ json: coverage(new URL(route.request().url()).searchParams.get('skill')?.includes('/two/') ? 'two' : 'one') }));
}
...
test('late coverage cannot replace a newer skill or refine it with old evidence', async ({ page, fleet }) => {
  ...
  await requiredRoute(held).fulfill({ json: coverage('two') });
...
test('retained skill controls retire on navigation and refine chooses the skill host', async ({ page, fleet }) => {
  await routes(page);
  await open(page);
  await page.evaluate(() => { window.retiredSkillRow = document.querySelector<HTMLElement>('.sk-row'); });
  await page.evaluate(() => fixtureApp.features.skillsController.detail('/fixture/one/SKILL.md'));
...
test('disposal cancels directory indexing and ignores retained refresh controls', async ({ page, fleet }) => {
  ...
  await open(page);
  await page.evaluate(() => { window.retiredSkillsRefresh = document.querySelector<HTMLElement>('[data-skills-action="refresh"]'); fixtureApp.features.skillsController.dispose(); fixtureElement(window.retiredSkillsRefresh, 'retired skills refresh').click(); });
  const before = reads;
...
test('a late activation list lookup cannot hijack a newer selection', async ({ page, fleet }) => {
  ...
  const selected = await page.evaluate(async ({ peer, id }) => {
    ...
    try {
      ...
      if (!release) throw new Error('Sidebar load resolver not initialized');
