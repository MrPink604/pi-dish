...
for (const [action, field, value] of [
  ...
  test(`delayed ${action} response updates its host after selection changes`, async ({ page, fleet }) => {
    ...
    await select(peer);
    await page.evaluate(({ id, host }) => patchSession(id, {
    ...
    const before = await page.evaluate(({ id, host, field }) => findSession(id, host)[field],
    ...
    const outcome = await page.evaluate(({ id, self, peer, field }) => ({
      ...
      self: findSession(id, self)[field], peer: findSession(id, peer)[field],
