...
for (const kind of ['Model', 'Thinking']) {
  test(`closing a pending ${kind.toLowerCase()} menu retires its catalog completion on the same session`, async ({ page, fleet }) => {
    ...
    await page.evaluate(kind => {
      const load = fixtureApp.features.appModels.load; fixtureApp.features.appModels.load = () => { fixtureApp.features.appModels.load = load; return new Promise(resolve => { window.finishHeaderCatalog = resolve; }); };
      ...
      if (kind === 'Model') fixtureApp.features.sessionControls.closeModels(); else fixtureApp.features.sessionControls.closeThinking();
    }, kind);
...
test('a rename editor cannot commit its old text after selecting the same session id on another host', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => { fixtureApp.features.sessionControls.startRename(); document.getElementById('sessionNameInput').value = 'Old peer edit'; });
  await fleet.select(fleet.self);
  await page.evaluate(() => { document.getElementById('sessionNameInput').dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter' })); return fixtureApp.features.sessionControls.commitRename(); });
...
test('tokenless export uses navigation and disposal retires pending exports and header controls', async ({ page, fleet }) => {
  ...
  const navigation = await page.evaluate(async () => {
    const calls = []; window.open = (...args) => { calls.push(args); return null; }; await fixtureApp.features.sessionControls.export(); return calls;
  });
