...
test('queue decoder ignores malformed rows and displays valid text literally', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => window.fixtureSessionListPatch(fixtureApp.features.sessionState.currentSession.id, { capabilities: { queueCancel: true } }));
...
test('replaced queue controls are inert and a double click sends one cancellation', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => {
    ...
    fixtureApp.features.promptDelivery.render({ followUp: ['first'] }); window.oldQueueEdit = document.querySelector('.queue-item-edit');
    fixtureApp.features.promptDelivery.render({ followUp: ['replacement'] }); window.oldQueueEdit.click();
  });
  expect(await page.evaluate(() => window.queueCancels.length)).toBe(0);
  await page.evaluate(() => { const button = document.querySelector('.queue-item-edit'); window.firstQueueEdit = fixtureApp.features.promptDelivery.edit(button); void fixtureApp.features.promptDelivery.edit(button); });
  ...
  expect(await page.evaluate(() => localStorage.getItem(fixtureApp.features.composerDrafts.draftKey(fixtureApp.features.sessionState.currentSession.id)))).toBe('replacement');
});
...
test('late side-command success cannot replace a newer answer or status', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => {
    ...
    document.getElementById('promptInput').value = '/btw first'; window.firstCommand = fixtureApp.features.composerSubmit.sendPrompt();
    document.getElementById('promptInput').value = '/btw second'; window.secondCommand = fixtureApp.features.composerSubmit.sendPrompt();
...
test('disposing submit retires late command effects while preserving the next composer', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => {
    ...
    document.getElementById('promptInput').value = '/btw question'; window.disposedCommand = fixtureApp.features.composerSubmit.sendPrompt();
    fixtureApp.features.composerSubmit.dispose(); document.getElementById('promptInput').value = 'later draft';
