...
async function setup(page, fleet) {
  ...
  await page.evaluate(() => { window.infoCopies = []; copyTextToClipboard = async text => { window.infoCopies.push(text); }; });
}
...
test('stats controls from an earlier opening cannot copy publish revoke or close', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => {
    ...
    closeStatsModal(); openStatsModal();
  });
...
test('a late existing share response cannot overwrite the clipboard after changing hosts', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => { const button = document.createElement('button'); button.dataset.entryId = 'entry'; window.shareCopy = copyMessageShareLink(button); });
...
test('artifact controls retire on close and live same-token revokes use their answering host', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => openArtifactsModal());
  await expect(page.locator('#artifactsBody')).toContainText('Self plan');
  await page.evaluate(() => {
    window.oldArtifacts = [...document.querySelectorAll('#artifactsBody button')]; closeArtifactsModal(); window.oldArtifacts.forEach(button => button.click());
  });
  expect(writes).toEqual([]); expect(await page.evaluate(() => window.infoCopies)).toEqual([]);
  await fleet.select(fleet.peer); await page.evaluate(() => openArtifactsModal());
...
test('session-info disposal ignores held stats and artifact reads and retained copy timers', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => { openStatsModal(); openArtifactsModal(); });
  await expect.poll(() => !!stats && artifacts.length > 0).toBe(true);
  await page.evaluate(() => { sessionInfo.dispose(); window.disposedInfoBody = document.getElementById('statsBody').innerHTML; });
