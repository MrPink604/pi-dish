...
let delivery = 0;
async function show(page: Page, request: Record<string, unknown>, fleet: FleetFixture) {
  ...
  const host = await page.evaluate(() => {
    if (!window.extensionDeliveryHook) {
      ...
      fixtureApp.features.extensionUI.handle = (request, session) => {
        ...
        if (typeof request === 'object' && request !== null && '__fixtureNonce' in request && typeof request.__fixtureNonce === 'number') window.extensionDelivery = request.__fixtureNonce;
      };
    }
  ...
  if (['select', 'confirm', 'input', 'editor', 'ask'].includes(method)) fixtureHost.emit('turn_start', {});
...
test('stashed dialogs preserve edits while same-id peer cards cannot answer the current host', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => { window.peerDialog = document.querySelector<HTMLElement>('.ext-ui-docked-dialog'); window.peerSubmit = fixtureElement(window.peerDialog, 'peer dialog').querySelector<HTMLElement>('[data-action="submit"]'); });
...
test('resolved dialog controls cannot answer a replacement using the same request id', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => { window.oldConfirm = [...document.querySelectorAll<HTMLElement>('.ext-ui-docked-dialog button')]; fixtureApp.features.extensionUI.resolve('shared-dialog', fixtureElement(fixtureApp.features.sessionState.captureSelection(), 'extension UI selection')); });
...
test('ask answers retain typed labels, custom notes and selections through redocking', async ({ page, fleet }) => {
  ...
  await page.route('**/api/sessions/*/ui-response', route => {
    ...
    if (!isRecord(payload)) throw new Error('Invalid extension UI answer');
    answer = payload; return route.fulfill({ json: { ok: true } });
  });
  await page.locator('[data-action="submit-ask"]').click();
...
test('authoritative dialog reconciliation prunes only its owning host and ignores malformed state', async ({ page, fleet }) => {
  await fleet.select(fleet.peer); await show(page, editor, fleet);
  await fleet.select(fleet.self); await show(page, editor, fleet);
  await page.evaluate(({ id, host }) => { fixtureApp.features.extensionUI.reconcile({ dialogs: [] }, { id, host }); fixtureApp.features.extensionUI.reconcile({ dialogs: false }, fixtureElement(fixtureApp.features.sessionState.captureSelection(), 'extension UI selection')); }, { id: ROOT, host: fleet.peer.hostId });
...
test('widget reprojection cancels both removal phases and old headers cannot replace peer state', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => { window.oldWidget = document.querySelector<HTMLElement>('.ext-ui-widget'); window.oldWidgetHeader = fixtureElement(window.oldWidget, 'old widget').querySelector<HTMLElement>('.ext-ui-widget-header'); });
...
test('extension UI disposal retires toast, widget, status and dialog listeners and timers', async ({ page, fleet }) => {
  ...
  await page.route('**/api/sessions/*/ui-response', route => { writes++; return route.fulfill({ json: { ok: true } }); });
  await page.evaluate(() => {
    window.retiredExtensionButtons = [...document.querySelectorAll<HTMLElement>('.ext-ui-docked-dialog button, .ext-ui-toast button')];
