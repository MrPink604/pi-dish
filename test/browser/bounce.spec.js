// Generated test/tool from test/browser/bounce.spec.ts; edit that source and run npm run build:tests.
"use strict";
...
(0, fixtures_js_1.test)('late preview replies and retained target/cancel controls retire with their view', async ({ page, fleet }) => {
    await routes(page);
    let held;
    const deletes = [];
    ...
    await (0, fixtures_js_1.expect)(page.locator('.bounce-target')).toHaveCount(2);
    await (0, fixtures_js_1.requiredRoute)(held).fulfill({ json: preview('Old reload preview') });
    await (0, fixtures_js_1.expect)(page.locator('#bounceHosts')).not.toContainText('Old reload preview');
    await (0, fixtures_js_1.expect)(page.locator('[data-bounce-cancel]')).toHaveCount(2);
    await page.evaluate(async () => {
        ...
        const input = fixtureElement(window.retiredBounceInput, 'retired bounce input');
        input.checked = true;
        input.dispatchEvent(new Event('change'));
        fixtureElement(window.retiredBounceCancel, 'retired bounce cancel').click();
    });
    await (0, fixtures_js_1.expect)(page.locator('#bounceSubmit')).toHaveText('Queue Restart (0)');
    await (0, fixtures_js_1.expect)(page.locator('[data-bounce-cancel]')).toHaveCount(2);
...
(0, fixtures_js_1.test)('an older status read cannot erase a newly accepted operation', async ({ page, fleet }) => {
    await routes(page);
    let oldRead;
    await page.route(`${fleet.self.base}/api/session-bounces`, route => {
        if (route.request().method() === 'GET')
            ...
            return route.fulfill({ json: { operation: operation('accepted-operation') } });
    });
    await open(page);
    ...
    await (0, fixtures_js_1.expect)(page.locator('.bounce-host').first()).toContainText('accepted-operation');
    await (0, fixtures_js_1.requiredRoute)(oldRead).fulfill({ json: { operations: [] } });
    await (0, fixtures_js_1.expect)(page.locator('.bounce-host').first()).toContainText('accepted-operation');
});
(0, fixtures_js_1.test)('disposal stops polling and rejects retained controls after held status responses', async ({ page, fleet }) => {
    await routes(page);
    ...
    for (const route of reads)
        await route.fulfill({ json: { operations: [operation('late-operation')] } });
    await page.clock.runFor(6000);
    (0, fixtures_js_1.expect)(reads).toHaveLength(2);
    await (0, fixtures_js_1.expect)(page.locator('#bounceHosts')).not.toContainText('late-operation');
