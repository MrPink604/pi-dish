...
test('a failed old abort cannot release the gate of a subsequent turn', async ({ page, fleet }) => {
  const result = await page.evaluate(() => {
    ...
    const second = fixtureApp.features.sessionActivity.beginAbort('self same'); fixtureApp.features.sessionActivity.endAbort('self same', fixtureElement(first, 'first abort token'));
    const retained = fixtureApp.features.sessionActivity.isAborting('self same'); fixtureApp.features.sessionActivity.endAbort('self same', fixtureElement(second, 'second abort token'));
...
test('btw clipboard completion and detached dismissal cannot modify the replacement panel', async ({ page, fleet }) => {
  await fleet.select(fleet.self);
  await page.evaluate(() => {
    fixtureApp.features.btwPanel.dispose(); window.sideQuestion = PiDishBrowser.createBtwPanel({ document, sessionState: fixtureApp.features.sessionState, markdown: text => text, copy: () => new Promise<void>(resolve => { window.finishSideCopy = resolve; }) });
    const owner = window.sideQuestion.show('first'); window.sideQuestion.resolve('answer', owner);
    window.oldSideClose = document.querySelector<HTMLElement>('.btw-dismiss'); fixtureElement(document.querySelector<HTMLElement>('.btw-copy'), '.btw-copy').click();
    const next = window.sideQuestion.show('next'); window.sideQuestion.resolve('next answer', next);
    fixtureElement(window.oldSideClose, 'old BTW close').click(); window.finishSideCopy();
  });
