  ...
  await page.evaluate(host => {
    ...
    window.fetch = async (...args) => {
      ...
      if (new URL(String(args[0]), location.href).href === host + '/api/settings') {
        ...
        res.json = async () => { const body = await json(); window.recoveryBodyWaiting = true; await new Promise<void>(resolve => { window.releaseRecoveryBody = resolve; }); return { ...body, recoveryMode: 'continue' }; };
      }
      return res;
    };
  }, fleet.self.base);
...
test('old preference save failures and retained buttons cannot act on a replacement fleet view', async ({ page, fleet }) => {
  ...
  expect(write.request().postDataJSON()).toEqual({ recoveryMode: 'restore' });
});
...
test('report requests and retained restore actions retire with the selected host and view', async ({ page, fleet }) => {
  ...
  await requiredRoute(held).fulfill({ json: report('Old record') });
  await expect(page.locator('.recovery-list')).toContainText('Peer record');
  await expect(page.locator('#recoveryViewBody')).toContainText('constructor');
  await page.evaluate(async () => {
    window.retiredRestore = document.querySelector<HTMLElement>('.recovery-restore');
    await fixtureApp.features.recoveryController.load();
    fixtureElement(window.retiredRestore, 'retired restore').click();
    window.closedRestore = document.querySelector<HTMLElement>('.recovery-restore');
...
test('a queued exclusion keeps its host and late completion cannot reload another report', async ({ page, fleet }) => {
  ...
  await page.selectOption('#recoveryReportHost', fleet.peer.hostId);
  await expect(page.locator('.recovery-list')).toContainText('Peer record');
  const exclusionRoute = requiredRoute(held);
  ...
  expect(new URL(exclusionRoute.request().url()).origin).toBe(fleet.self.base);
  expect(exclusionRoute.request().postDataJSON()).toEqual({ excluded: true });
});

test('changed endpoint credentials retire displayed report actions and trigger an owned refresh', async ({ page, fleet }) => {
  const reads: Array<string | undefined> = [], writes: Route[] = [];
  ...
  await page.evaluate(host => {
    window.oldTokenRestore = document.querySelector<HTMLElement>('.recovery-restore');
