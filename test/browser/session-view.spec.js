// Generated test/tool from test/browser/session-view.spec.ts; edit that source and run npm run build:tests.
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const fixtures_js_1 = require("./fixtures.js");
(0, fixtures_js_1.test)('late resume-model options cannot replace the picker for a newly selected host', async ({ page, fleet }) => {
    await fleet.select(fleet.self);
    await page.evaluate(() => {
        window.fixtureSessionListPatch(fixtureCurrentSession().id, { harnessId: 'omp' });
        const original = fixtureApp.features.apiTransport.request;
        fixtureApp.features.apiTransport.request = (host, path, init) => path.startsWith('/api/models') ? new Promise(resolve => { window.finishResumeModels = resolve; }) : original(host, path, init);
    ...
    await (0, fixtures_js_1.expect)(page.locator('#resumeModelWrap')).toBeHidden();
    await (0, fixtures_js_1.expect)(page.locator('#resumeModelSelect')).not.toContainText('fixture/old');
});
(0, fixtures_js_1.test)('resume serializes a captured target and cannot navigate the replacement selection', async ({ page, fleet }) => {
    await fleet.select(fleet.self);
    await page.evaluate(() => {
        window.resumeCalls = [];
        window.resumeNavigations = [];
        window.resumeStatuses = [];
        ...
        window.resumeFirst = window.ownedResume.resume();
        void window.ownedResume.resume();
    });
    (0, fixtures_js_1.expect)(await page.evaluate(() => window.resumeCalls.length)).toBe(1);
    (0, fixtures_js_1.expect)(await page.evaluate(() => {
        ...
        if (typeof body !== 'string')
            throw new Error('Resume call lacks JSON body');
        return JSON.parse(body);
    })).toEqual({ target: { type: 'tmux', socket: 'fixture', tmuxSession: 'captured' } });
    await fleet.select(fleet.peer);
    await page.evaluate(id => { fixtureElement(window.resumeCalls[0], 'resume call').resolve(new Response(JSON.stringify({ id }))); return window.resumeFirst; }, fixtures_js_1.ROOT);
    (0, fixtures_js_1.expect)(await page.evaluate(() => window.resumeNavigations)).toEqual([]);
    (0, fixtures_js_1.expect)(await page.evaluate(() => window.resumeStatuses.length)).toBe(1);
});
(0, fixtures_js_1.test)('resume disposal retires late response effects and changed endpoints retire picker data', async ({ page, fleet }) => {
    await fleet.select(fleet.self);
    await page.evaluate(() => {
        window.fixtureSessionListPatch(fixtureCurrentSession().id, { harnessId: 'omp' });
        window.resumeEndpoint = { base: 'http://original' };
        window.resumeResponses = [];
        window.resumeEffects = [];
        ...
        window.oldPicker = window.ownedResume.load(fixtureCurrentSession());
        window.resumeEndpoint = { base: 'http://new' };
        fixtureElement(window.resumeResponses[0], 'resume response').resolve(new Response(JSON.stringify([{ provider: 'fixture', id: 'old' }])));
    });
    await page.evaluate(() => window.oldPicker);
    await (0, fixtures_js_1.expect)(page.locator('#resumeModelSelect')).not.toContainText('fixture/old');
    await page.evaluate(id => { window.disposedResume = window.ownedResume.resume(); window.ownedResume.dispose(); fixtureElement(window.resumeResponses[1], 'disposed resume response').resolve(new Response(JSON.stringify({ id }))); return window.disposedResume; }, fixtures_js_1.ROOT);
    (0, fixtures_js_1.expect)(await page.evaluate(() => window.resumeEffects)).toEqual([]);
});
(0, fixtures_js_1.test)('list ingress omits malformed presentation fields before header rendering and escapes names', async ({ page, fleet }) => {
    await page.route(`${fleet.self.base}/api/sessions?**`, route => route.fulfill({ json: {
            active: [], previous: [{ id: fixtures_js_1.ROOT, name: '<img src=x>', model: null, cwd: {}, contextPercent: {}, contextTokens: 'not numeric', turnInProgress: 1 }],
        } }));
    ...
    (0, fixtures_js_1.expect)(await page.evaluate(({ id, host }) => {
    ...
    }, { id: fixtures_js_1.ROOT, host: fleet.self.hostId })).toEqual({ cwd: undefined, contextPercent: undefined, contextTokens: undefined, turnInProgress: undefined });
    await page.route(`${fleet.self.base}/api/sessions/${fixtures_js_1.ROOT}/messages?**`, route => route.fulfill({ json: { messages: [], session: {} } }));
    await fleet.select(fleet.self);
    await (0, fixtures_js_1.expect)(page.locator('#sessionName')).toHaveText('<img src=x>');
...
(0, fixtures_js_1.test)('composer cache countdown aligns with context and becomes a cold-cache glyph after expiry', async ({ page, fleet }) => {
    ...
    await (0, fixtures_js_1.expect)(page.locator('#sessionCache')).toHaveText('❄');
...
(0, fixtures_js_1.test)('restored same-host tool panels retain their node and duration after a new selection generation', async ({ page, fleet }) => {
    ...
    await page.evaluate(() => {
        ...
        window.cachedTool = document.querySelector('[data-tool-call-id="cached-tool"]');
        const tool = fixtureElement(window.cachedTool, 'cached tool');
        tool.open = true;
        tool.remove();
        fixtureApp.features.liveToolsController.clear(fixtureElement(document.getElementById('messages'), '#messages'));
        fixtureApp.features.sessionState.advanceSelection();
        fixtureElement(document.getElementById('messages'), '#messages').append(tool);
    ...
    (0, fixtures_js_1.expect)(await page.evaluate(() => document.querySelector('[data-tool-call-id="cached-tool"]') === window.cachedTool)).toBe(true);
...
fixtures_js_1.test.describe('selection retirement', () => {
    ...
    (0, fixtures_js_1.test)('unknown real and provisional targets preserve the selected view and its live ownership', async ({ page, fleet }) => {
        ...
        }, { id: fixtures_js_1.ROOT, host: fleet.self.hostId });
        (0, fixtures_js_1.expect)(result).toEqual({ owned: true, sameStream: true, sameNode: true, host: fleet.self.hostId });
        await (0, fixtures_js_1.expect)(page.locator('#promptInput')).toHaveValue('Keep this composer');
        await (0, fixtures_js_1.expect)(page.locator('#searchBar')).toBeVisible();
        await (0, fixtures_js_1.expect)(page.locator('mark.search-mark')).toHaveText('self');
    });
    (0, fixtures_js_1.test)('provisional retirement stashes clean transcript nodes and drafts without clearing the old session activity', async ({ page, fleet }) => {
        await fleet.select(fleet.self);
        await fixtures_js_1.expect.poll(() => page.evaluate(() => fixtureApp.features.messageStreamController.source?.readyState)).toBe(1);
        ...
        (0, fixtures_js_1.expect)(await page.evaluate(({ id, host, spawnId }) => {
        ...
        }, { id: fixtures_js_1.ROOT, host: fleet.self.hostId, spawnId })).toEqual({
        ...
        await fleet.select(fleet.self);
        await (0, fixtures_js_1.expect)(page.locator('#promptInput')).toHaveValue('Original real draft');
        (0, fixtures_js_1.expect)(await page.evaluate(() => document.querySelector('#messages [data-msg-index]') === window.retainedSelectionMessage)).toBe(true);
        await (0, fixtures_js_1.expect)(page.locator('mark.search-mark')).toHaveCount(0);
        await page.evaluate(id => fixtureApp.features.sessionView.pending(id), spawnId);
        await (0, fixtures_js_1.expect)(page.locator('#promptInput')).toHaveValue('Separate provisional draft');
        await (0, fixtures_js_1.expect)(page.locator('#btnSend')).toBeDisabled();
        await (0, fixtures_js_1.requiredRoute)(status).fulfill({ json: { status: 'error', error: 'Fixture ends pending operation' } });
        await (0, fixtures_js_1.expect)(page.locator('#sessionName')).toHaveText('Session failed to start');
    });
});
