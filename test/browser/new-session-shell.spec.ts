...
test('workspace buttons retain their owning host and are retired on host change or close', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => { window.oldWorkspaceButton = document.querySelector<HTMLElement>('#nsWorkspaces button'); });
  ...
  await page.evaluate(() => { window.peerWorkspaceButton = document.querySelector<HTMLElement>('#nsWorkspaces button'); fixtureApp.features.newSessionController.close(); });
