...
test('late file response cannot replace a newer file or cross a same-id host selection', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => fixtureApp.features.fileViews.openFile('new.txt')); await held.fulfill({ json: preview('old.txt') }); await page.evaluate(() => window.oldFile);
...
test('retired page controls and clipboard completion cannot affect a reopened file', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => {
    ...
    window.copyCalls = []; navigator.clipboard.writeText = text => { window.copyCalls.push(text); return new Promise(resolve => { window.finishCopy = resolve; }); };
    fixtureApp.features.fileViews.copy(document.getElementById('fileViewCopy'));
  });
...
test('lazy patches reject retired rows and late responses after diff refresh', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => { window.oldDetails = document.querySelector('details.diff-file'); window.oldPatch = window.oldDetails.querySelector('.diff-patch'); window.patchPromise = fixtureApp.features.fileViews.loadPatch(window.oldDetails); });
  ...
  await page.evaluate(async () => { window.oldDetails.open = true; await fixtureApp.features.fileViews.loadPatch(window.oldDetails); });
...
test('stale diff snapshot refreshes once and failed patch remains retryable', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => fixtureApp.features.fileViews.openDiff()); await page.evaluate(() => fixtureApp.features.fileViews.loadPatch(document.querySelector('details.diff-file')));
  expect(reads).toBe(2); await page.evaluate(() => fixtureApp.features.fileViews.loadPatch(document.querySelector('details.diff-file')));
  await expect(page.locator('.diff-patch')).toContainText('try again');
  await page.evaluate(() => fixtureApp.features.fileViews.loadPatch(document.querySelector('details.diff-file')));
...
test('file view disposal retires publish, copy and diff patch actions', async ({ page, fleet }) => {
  ...
  await page.evaluate(async () => { fixtureApp.features.fileViews.dispose(); await fixtureApp.features.fileViews.publish(); await fixtureApp.features.fileViews.openFile('a.txt'); await fixtureApp.features.fileViews.openDiff(); document.getElementById('fileViewPublish').click(); });
