...
import type { BrowserHost, FleetFixture } from './fixtures.js';
import type { RoutineHost } from '../../src/browser/routines-view.js';
import { test, expect, ROOT, requiredRoute } from './fixtures.js';
const routine = (name: string, id = 'shared') => ({ id, name, harness: 'pi', cwd: '/fixture/project', prompt: name + ' prompt', promptVersion: 1, versions: [{ version: 1, prompt: name + ' prompt', savedAt: Date.now() }] });
const invocations = (label: string) => ({ invocations: [{ id: label, version: 1, status: 'completed', startedAt: Date.now(), sessionId: ROOT }], nextBefore: null });
async function setup(page: Page, fleet: FleetFixture) {
  await page.route('**/api/routines', (route: Route) => route.fulfill({ json: { routines: [routine(new URL(route.request().url()).origin === fleet.self.base ? 'Self' : 'Peer')] } }));
  await page.route('**/api/routines/shared', (route: Route) => route.fulfill({ json: { routine: routine(new URL(route.request().url()).origin === fleet.self.base ? 'Self' : 'Peer') } }));
  await page.route('**/api/routines/shared/invocations?*', (route: Route) => route.fulfill({ json: invocations(new URL(route.request().url()).origin === fleet.self.base ? 'self-run' : 'peer-run') }));
  await page.route('**/api/models**', (route: Route) => route.fulfill({ json: [] }));
  await page.route('**/api/harnesses', (route: Route) => route.fulfill({ json: { harnesses: [{ id: 'pi', label: 'Pi', available: true }] } }));
  page.on('dialog', dialog => dialog.accept());
  await page.evaluate(() => {
    const hosts: RoutineHost[] = fixtureApp.ports.hostDiscovery.hosts().map(host => Object.freeze({
    ...
    window.rt = PiDishBrowser.createRoutinesView({ root: fixtureElement(document.querySelector<HTMLElement>('.main'), '.main'), request: (host, path, options) => fixtureApp.features.apiTransport.request(host, path, options), storage: localStorage, sessionState: fixtureApp.features.sessionState,
...
async function select(page: Page, host: BrowserHost & { hostId: string }) { await page.locator(`.rt-row[data-host="${host.hostId}"]`).click(); }
...
test('routine detail and invocation reads remain with their host after a same-id selection change', async ({ page, fleet }) => {
  ...
  await requiredRoute(held).fulfill({ json: { routine: routine('Old self') } });
  await expect(page.locator('#rtName')).toHaveValue('Peer');
  await page.unroute(fleet.self.base + '/api/routines/shared');
  let heldRuns: Route | undefined;
  ...
  await requiredRoute(heldRuns).fulfill({ json: invocations('old-self-run') });
...
for (const operation of ['save', 'run', 'delete']) test(`late routine ${operation} cannot replace another host's form`, async ({ page, fleet }) => {
  ...
  await select(page, fleet.peer);
  await expect(page.locator('#rtName')).toHaveValue('Peer');
  await requiredRoute(held).fulfill({ json: { routine: routine('Saved self') } });
  await expect(page.locator('#rtName')).toHaveValue('Peer');
...
test('routine host catalog changes retire old options and reopening preserves the draft', async ({ page, fleet }) => {
  ...
  await requiredRoute(held).fulfill({ json: { harnesses: [{ id: 'old-agent', available: true }] } });
...
test('retained routine controls retire and disposal cancels polling and delete arming', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => {
    window.oldRoutineControls = [
      ...
      fixtureElement(document.querySelector<HTMLElement>('.rt-version-restore'), '.rt-version-restore'),
      fixtureElement(document.querySelector<HTMLElement>('.rt-row'), '.rt-row'),
    ];
  });
  await select(page, fleet.peer);
...
test('routine session navigation preserves the answering host and drops a delayed lookup after selection', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => {
    ...
    fixtureApp.features.sidebarLists.load = () => new Promise<void>(resolve => { window.finishRoutineLookup = () => { fixtureApp.features.sidebarLists.load = originalLoad; window.restoreRoutineLookup(); resolve(); }; });
  });
  await page.locator('.rt-session-link').click();
...
test('removed routine hosts retire form actions and pending polling safely', async ({ page, fleet }) => {
  ...
  await page.evaluate(async host => {
    ...
    fixtureElement(document.getElementById('rtRunBtn'), "document.getElementById('rtRunBtn')").click(); await window.rt.save(); await window.rt.delete();
...
test('a routine save preserves edits made while the submitted form is in flight', async ({ page, fleet }) => {
  ...
  await requiredRoute(held).fulfill({ json: { routine: { ...routine('Self'), prompt: 'Submitted prompt', promptVersion: 2 } } });
...
test('a changed routine base offers a working back control and same-row reload', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => fixtureElement(document.querySelector<HTMLElement>('.rt-back'), '.rt-back').click());
