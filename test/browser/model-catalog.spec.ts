...
const models = (id: string) => [{ id, provider: 'fixture', name: id, enabled: true }];
  ...
  await page.evaluate(url => {
    ...
    Response.prototype.json = async function() {
      ...
      if (this.url === url) {
        ...
        await new Promise<void>(resolve => { window.releaseModelBody = resolve; });
      }
      return data;
    };
  }, url);
...
test('a previous cwd model body cannot publish before the next debounced controller refresh', async ({ page, fleet }) => {
  ...
  expect(await page.evaluate(() => {
    const rows: unknown = JSON.parse(localStorage.getItem('pi-dish-models-cache') ?? '[]');
    if (!Array.isArray(rows)) throw new Error('Invalid model cache');
    return rows.map(row => {
      if (typeof row !== 'object' || row === null || !('id' in row) || typeof row.id !== 'string') throw new Error('Invalid cached model');
      return row.id;
    });
  })).toEqual(['ready']);
