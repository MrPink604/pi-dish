...
for (const blocked of [false, true]) {
  test(`retired host discovery ${blocked ? '401' : 'identity'} cannot overwrite a re-added host`, async ({ page, fleet }) => {
    ...
    expect(await page.evaluate(host => fixtureElement(fixtureApp.features.hostDiscovery.descriptor(host), 'peer descriptor').label, fleet.peer.hostId)).toBe('Fresh descriptor');
    expect(await page.evaluate(() => fixtureElement(fixtureApp.features.hostDirectory.catalog[0], 'peer catalog entry').hostId)).toBe(fleet.peer.hostId);
  });
}

test('saving an unchanged catalog retains pending discovery for its hosts', async ({ page, fleet }) => {
  let pendingRoute: Route | null | undefined;
  ...
  await requiredRoute(pendingRoute).fulfill({ json: { hostId: fleet.peer.hostId, label: 'Discovered after save' } });
  await page.evaluate(() => window.pendingIdentity);
  expect(await page.evaluate(host => fixtureElement(fixtureApp.features.hostDiscovery.descriptor(host), 'peer descriptor').label, fleet.peer.hostId)).toBe('Discovered after save');
});
