...
test('saving a scope cancels its absorbed query and retired chips cannot toggle a replacement', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => { window.oldScopeButton = document.querySelector<HTMLElement>('#scopeChips .scope-chip'); window.customQuery.setFilters([{ name: 'Saved', query: 'replacement' }]); window.customQuery.renderChips(); fixtureElement(window.oldScopeButton, 'old scope button').click(); });
...
test('retired query polls cannot mark a host blocked during the next debounce window', async ({ page, fleet }) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    const events: unknown[] = [], hosts = [{ hostId: 'self', base: '', self: true }]; let finish: ((response: Response) => void) | undefined;
    const lists = PiDishBrowser.createSidebarLists({ document, sessionState: state, activity, hosts: () => hosts, pollable: () => hosts, selfId: () => 'self', query: () => 'new', all: () => true,
      refreshFleet() {}, connection: (_host, event) => events.push(event), request: () => new Promise<Response>(resolve => { finish = resolve; }) });
    const pending = lists.load('old'); lists.invalidate(); if (!finish) throw new Error('List response resolver unavailable'); finish(new Response('{}', { status: 401 })); await pending;
...
test('seen markers migrate and prune only within the owning host', async ({ page, fleet }) => {
  ...
  const result = await page.evaluate(() => {
    const storage = new Map<string, string>([['pi-dish-seen', JSON.stringify({ bare: 'old', 'peer same': 'peer-at', bad: {} })]]);
    const state = PiDishBrowser.createSessionState({ getSelfHostId: () => 'self', getHostLabel: id => id, onListsChanged() {}, onCurrentChanged() {} });
