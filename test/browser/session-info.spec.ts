...
const pages = (label: string) => [{ token: 'shared-page', root: '/fixture/plan.html', path: '/p/shared-page', title: label + ' plan', createdAt: Date.now() }];
async function setup(page: Page, fleet: FleetFixture) {
  await page.route('**/api/sessions/*/stats', (route: Route) => route.fulfill({ json: { model: new URL(route.request().url()).origin === fleet.self.base ? 'Self model' : 'Peer model', cwd: '/fixture/project', hardCacheMisses: 3, costs: { total: 0.5 } } }));
  await page.route('**/api/pages?*', (route: Route) => route.fulfill({ json: pages(new URL(route.request().url()).origin === fleet.self.base ? 'Self' : 'Peer') }));
  await page.route('**/api/sessions/*/share', (route: Route) => route.request().method() === 'GET'
...
test('stats controls from an earlier opening cannot copy publish revoke or close', async ({ page, fleet }) => {
  ...
  await expect(page.locator('#statsBody')).toContainText('3 hard misses');
  ...
  await page.evaluate(() => {
    window.oldInfoControls = [
      ...
      fixtureElement(document.querySelector<HTMLElement>('.stats-page-revoke'), '.stats-page-revoke'),
      fixtureElement(document.querySelector<HTMLElement>('.stats-copy'), '.stats-copy'),
...
test('a late existing share response cannot overwrite the clipboard after changing hosts', async ({ page, fleet }) => {
  ...
  await requiredRoute(held).fulfill({ json: { url: 'https://fixture.invalid/old-peer' } });
...
test('artifact controls retire on close and live same-token revokes use their answering host', async ({ page, fleet }) => {
  ...
  await page.evaluate(() => {
    window.oldArtifacts = [...document.querySelectorAll<HTMLElement>('#artifactsBody button')]; fixtureApp.features.sessionInfo.closeArtifacts(); window.oldArtifacts.forEach(button => button.click());
  });
...
test('session-info disposal ignores held stats and artifact reads and retained copy timers', async ({ page, fleet }) => {
  ...
  let stats: Route | undefined;
  const artifacts: Route[] = [];
  ...
  await requiredRoute(stats).fulfill({ json: { model: 'Late stats' } });
  for (const route of artifacts) await route.fulfill({ json: pages('Late') });
  await page.clock.runFor(2000);
  expect(await page.evaluate(() => fixtureElement(document.getElementById('statsBody'), "document.getElementById('statsBody')").innerHTML)).toBe(await page.evaluate(() => window.disposedInfoBody));
