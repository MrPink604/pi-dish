...
const userMsg = (text: string, timestamp?: unknown) =>
  ({ type: 'message', message: { role: 'user', content: [{ type: 'text', text }], timestamp } });
const assistantMsg = (text: string, usage?: unknown) =>
  ({ type: 'message', message: { role: 'assistant', content: [{ type: 'text', text }], usage } });
const messageText = (message: unknown): unknown => record(present(records(record(message).content)[0])).text;
...
test('readSessionMessages returns the displayable stream in order', () => {
  ...
  assert.deepEqual(all.map((message: { role?: unknown }) => message.role), ['user', 'assistant', 'toolResult', 'user', 'custom', 'assistant']);
  assert.equal(record(present(records(record(present(all[3])).content)[0])).text, 'injected', 'session-message custom entries render as user messages');
  assert.equal(record(present(all[4])).customType, 'other', 'unknown visible custom entries get a generic row');
...
test('OMP entry tolerance renders async/interruption/unknown custom rows without leaking hidden state', () => {
  ...
  assert.deepEqual(messages.map((message: { role?: unknown; customType?: unknown }) => [message.role, message.customType]), [
    ['user', undefined],
    ['assistant', undefined],
  ...
  assert.deepEqual(record(present(messages[1])).content, [], 'empty aborted assistant messages remain parseable');
  assert.deepEqual(record(present(messages[2])).content, [], 'interrupted reasoning is projected only as a marker');
  const jobs = records(record(record(present(messages[3])).details).jobs);
  assert.equal(record(present(jobs[0])).jobId, 'bg_123');
  assert.equal(record(present(messages[4])).content, 'A future visible OMP notice', 'unknown visible custom types get a generic row');
...
test('cache telemetry preserves retention, refreshes expiry, and counts hard misses', () => {
  ...
  assert.equal(present(messages[0].usage).cacheWrite1h, 100);
  assert.equal(present(messages[0].cacheExpiry).retention, '1h');
  assert.equal(present(messages[0].cacheExpiry).expiresAt, firstStart + 60 * 60_000);
  assert.equal(present(messages[1].cacheExpiry).expiresAt, secondStart + 60 * 60_000,
    'a cache hit carries the earlier retention policy forward');
...
test('readSessionMessages follows the active tree path, not file order', () => {
  ...
  const tmsg = (id: string, parentId: unknown, role: unknown, text: string) => ({
...
test('readSessionMessagesAtLeaf follows a live leaf that is not the last JSONL entry', () => {
  ...
  assert.deepEqual(SF.readSessionMessages(file).map(messageText),
...
test('large snapcompact archives are projected without retaining frame payloads', () => {
  ...
  const content = entries.map((entry: unknown) => JSON.stringify(entry)).join('\n') + '\n';
  const parsed = SF.parseSessionEntries(content);
  const compact = parsed.find((entry: unknown) => record(entry).type === 'compaction');
  ...
  assert.deepEqual(SF.readSessionMessagesAtLeaf(file, 'u2').map(messageText),
  ...
  const unusualCompact = record(present(SF.parseSessionEntries(unusual)[0]));
  assert.equal(unusualCompact.summary, 'field serialized after preserveData');
  assert.ok(String(record(unusualCompact.preserveData).custom).startsWith(archiveMarker),
...
test('malformed raw counters contribute no tokens while retry and cost policies retain raw usage', () => {
  ...
  const rawUsage = record(record(present(parsed[2])).message);
  const rawCounters = record(rawUsage.usage);
...
test('OMP chronology keeps billing continuity separate from display identity and retries', () => {
  ...
  assert.equal(present(present(usage.models['provider/billed']).tokens).output, 2);
  assert.equal(present(present(usage.models['provider/selected']).tokens).output, 3);
...
test('typed metadata accumulation narrows malformed known fields without promoting later headers', () => {
  const { sessionInfoFromEntries, extendSessionInfoFromEntries }: typeof import('../lib/session-metadata.js') = require('../lib/session-metadata.js')
  ...
  ].map((entry: unknown) => JSON.stringify(entry)).join('\n'));
