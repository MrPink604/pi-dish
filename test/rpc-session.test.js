// Generated test/tool from test/rpc-session.test.ts; edit that source and run npm run build:tests.
"use strict";
...
const post = async (requestPath, body, signal) => {
    const response = await fetch(base + requestPath, {
...
const findActive = async (id) => {
    ...
    return (0, test_types_js_1.records)(body.active).find(session => session.id === id) || null;
};
...
test('GET /api/models lists what the host pi reports, not the vendored CLI', async () => {
    ...
    const models = (0, test_types_js_1.records)(await response.json());
    ...
    assert.equal((0, test_types_js_1.present)(models.find(model => model.id === 'fresh-model')).contextWindow, 200000, 'context window parsed from the host table');
});
test('POST /api/sessions/new spawns a headless RPC pi and lists it active', async () => {
    ...
    const start = (0, test_types_js_1.present)(readStarts().find(entry => text(entry.sessionFile).endsWith(`${sessionId}.jsonl`)));
    const startArgs = Array.isArray(start?.args) ? start.args : [];
    assert.deepEqual(startArgs.slice(startArgs.indexOf('--thinking'), startArgs.indexOf('--thinking') + 2), ['--thinking', 'high'], 'reasoning level is forwarded to the pi CLI');
    ...
    assert.ok(readLog().some(c => c.type === 'set_session_name' && c.name === 'Named from creation'), 'creation names the session through the live RPC backend');
...
test('RPC startup waits for a valid state object and session file before registering', async () => {
    for (const invalid of [[], { sessionFile: 123 }]) {
        ...
        try {
            ...
            assert.match(id, /^2026-07-10T00-00-00-/, 'a malformed first reply must not publish the provisional rpc id');
            const rpc = (0, test_types_js_1.record)(getRPCSession((0, test_types_js_1.nativeId)(id)));
            const rpcProcess = (0, test_types_js_1.record)(rpc.proc);
            assert.equal(fs.readFileSync(STATE_LOG, 'utf8').trim().split('\n').filter(pid => pid === String(rpcProcess.pid)).length, 2, 'the same child must receive a second get_state request after the malformed reply');
            assert.equal(path.basename(text(rpc.sessionFile), '.jsonl'), id);
            assert.equal((0, test_types_js_1.record)(rpc.state).sessionFile, rpc.sessionFile);
        }
        finally {
            delete process.env.PI_FIXTURE_INVALID_STATE_ONCE;
            delete process.env.PI_FIXTURE_STATE_LOG;
            if (id)
                getRPCSession((0, test_types_js_1.nativeId)(id))?.kill();
        }
    }
});
test('RPC startup rejects an unroutable session id without retrying and stops its child', async () => {
    ...
    try {
        ...
        const start = (0, test_types_js_1.present)(readStarts()[before]);
        assert.ok(start.pid);
        ...
        assert.equal(getAllRPCSessions().some((rpc) => (0, test_types_js_1.record)((0, test_types_js_1.record)(rpc).proc).pid === start.pid), false);
    }
    finally {
        delete process.env.PI_FIXTURE_INVALID_STATE_ONCE;
        delete process.env.PI_FIXTURE_STATE_LOG;
...
test('direct RPC file URL cwd metadata matches the actual child filesystem path', async () => {
    ...
    const rpc = await createRPCSession({ cwd: (0, node_url_1.pathToFileURL)(directory) });
    try {
        const actual = JSON.parse(fs.readFileSync((0, test_types_js_1.present)(rpc.sessionFile), 'utf8').split('\n')[0]).cwd;
...
test('unrepresentable cwd metadata cleans its child and does not mask native spawn failure', async (t) => {
    ...
    assert.equal(getAllRPCSessions().some((rpc) => (0, test_types_js_1.record)((0, test_types_js_1.record)(rpc).proc).pid === spawned?.pid), false);
    const missing = (0, node_url_1.pathToFileURL)(path.join(tmpHome, 'missing-native-cwd'));
...
test('a bogus local caller id is rejected, a host-qualified one is advisory-accepted', async () => {
    ...
    assert.ok((0, test_types_js_1.records)(related.body.relations).every(relation => (0, test_types_js_1.record)(relation.session).id !== sessionId), 'a foreign-host id must not resolve to a local session of the same name');
...
test('source-aware spawn records advisory peer provenance', async () => {
    ...
    assert.ok((0, test_types_js_1.records)(sourceRelated.body.relations).some(relation => relation.kind === 'startedHere' && (0, test_types_js_1.record)(relation.session).id === peerId));
    const peerRelated = await get(`/api/sessions/${peerId}/related`);
    assert.ok((0, test_types_js_1.records)(peerRelated.body.relations).some(relation => relation.kind === 'startedFrom' && (0, test_types_js_1.record)(relation.session).id === sessionId));
    ...
    assert.equal((await findActive(operationId))?.name, 'Named async peer', 'an async spawn is not ready until its requested name is applied');
    const asyncRelated = await get(`/api/sessions/${operationId}/related`);
    assert.ok((0, test_types_js_1.records)(asyncRelated.body.relations).some(relation => relation.kind === 'startedFrom' && (0, test_types_js_1.record)(relation.session).id === sessionId), 'async spawn persists the same advisory provenance');
});
test('an unreachable bridge falls back to the existing RPC transport without spawning', async () => {
    const rpc = (0, test_types_js_1.present)(getRPCSession((0, test_types_js_1.nativeId)(sessionId)));
    ...
    try {
        ...
        assert.equal(getRPCSession((0, test_types_js_1.nativeId)(sessionId)), rpc, 'the already-owned RPCSession was reused');
...
test('prompt round-trips: RPC events stream over SSE and land in the JSONL', async () => {
    ...
    try {
        await sse.waitFor((e) => eventName(e) === 'init');
        const { status } = await post(`/api/sessions/${sessionId}/prompt`, { message: 'hello fixture' });
        assert.equal(status, 200);
        await sse.waitFor((e) => eventName(e) === 'turn_start');
        const update = await sse.waitFor((e) => eventName(e) === 'message_update');
        const updateMessage = (0, test_types_js_1.record)(eventData(update).message);
        assert.equal(updateMessage.role, 'assistant');
        assert.ok(text((0, test_types_js_1.records)(updateMessage.content)[0]?.text).length > 0, 'Pi 0.84 delta-only updates are reassembled into full SSE messages');
        const end = await sse.waitFor((e) => eventName(e) === 'message_end');
        assert.equal(text((0, test_types_js_1.records)((0, test_types_js_1.record)(eventData(end).message).content)[0]?.text), 'reply to: hello fixture');
        await sse.waitFor((e) => eventName(e) === 'turn_end');
        // The turn's final message was appended to the session JSONL.
        const { body } = await get(`/api/sessions/${sessionId}/messages`);
        const texts = (0, test_types_js_1.records)(body.messages).map(message => text((0, test_types_js_1.records)(message.content)[0]?.text));
...
test('RPC SSE reconnect replays a running tool start and latest update', async () => {
    ...
    try {
        await sse.waitFor((e) => eventName(e) === 'init');
        const start = await sse.waitFor((e) => eventName(e) === 'tool_execution_start' && eventData(e).toolCallId === 'rpc-replay-tool');
        const update = await sse.waitFor((e) => eventName(e) === 'tool_execution_update' && eventData(e).toolCallId === 'rpc-replay-tool');
        ...
        assert.equal(text((0, test_types_js_1.records)((0, test_types_js_1.record)(eventData(update).partialResult).content)[0]?.text), 'partial read');
        rpc._handleMessage({
        ...
        await sse.waitFor((e) => eventName(e) === 'tool_execution_end' && eventData(e).toolCallId === 'rpc-replay-tool');
    }
    finally {
...
test('RPC remembers dialogs before observers attach and HTTP answers retire their SSE replay without a resolved event', async () => {
    ...
    const observed = [];
    const resolved = [];
    const offRequest = rpc.on('extension_ui_request', (data) => observed.push(rpc.extUIState.dialogs.get((0, test_types_js_1.record)(data).id)));
    ...
    try {
        ...
        const replay = await first.waitFor((e) => eventName(e) === 'extension_ui_request' && eventData(e).id === dialog.id);
        assert.deepEqual(eventData(replay), dialog);
        const pending = await first.waitFor((e) => eventName(e) === 'extension_ui_state');
        ...
        const remaining = await reconnect.waitFor((e) => eventName(e) === 'extension_ui_state');
        assert.deepEqual(eventData(remaining).dialogs, []);
        assert.equal(reconnect.events.some((e) => eventName(e) === 'extension_ui_request' && eventData(e).id === dialog.id), false, 'an answered RPC dialog must not return on reconnect');
    }
    finally {
        first?.close();
        reconnect?.close();
...
test('a prompt sent mid-turn is delivered with steer behavior', async () => {
    ...
    try {
        ...
        assert.equal((0, test_types_js_1.present)(queued).streamingBehavior, 'followUp', 'dedicated endpoint requests Pi follow-up delivery');
        await sse.waitFor((e) => eventName(e) === 'turn_end');
    }
    finally {
        sse.close();
...
test('abort mid-turn ends the turn via agent_end (no paired turn_end)', async () => {
    ...
    try {
        ...
        assert.equal(status, 200);
        await sse.waitFor((e) => eventName(e) === 'agent_end');
        assert.ok(!sse.events.some((e) => eventName(e) === 'turn_end'), 'aborted turn has no turn_end');
        // The backend must not think a turn is still running.
        const sess = await findActive(sessionId);
        assert.equal((0, test_types_js_1.present)(sess).turnInProgress, false);
    }
    finally {
        sse.close();
    }
...
test('slash commands map onto RPC protocol commands', async () => {
    ...
    assert.equal((0, test_types_js_1.present)(await findActive(sessionId)).name, 'renamed via rpc');
    ...
    assert.equal((0, test_types_js_1.present)(await findActive(sessionId)).model, 'test/other-model');
...
test('POST /thinking validates the level against the session harness vocabulary', async () => {
    ...
    assert.equal(readLog().filter(c => c.type === 'set_thinking_level').length, before, 'rejected levels never reach pi');
...
test('a /compact issued while one runs is refused, not forwarded to pi', { timeout: 10000 }, async () => {
    ...
    try {
        ...
        assert.equal((0, test_types_js_1.present)(await findActive(sessionId)).compacting, true, 'list reflects compacting');
    ...
    assert.equal((0, test_types_js_1.present)(await findActive(sessionId)).compacting, false, 'flag clears when compaction ends');
...
test('overlapping RPC resumes launch exactly one process for the JSONL', async () => {
    ...
    try {
        ...
        assert.equal([a.body, b.body].filter((body) => body.sharedResume).length, 1, 'one caller owns the launch and one reports sharing it');
        ...
        assert.equal(path.resolve(text((0, test_types_js_1.present)(starts[0]).sessionFile)), path.resolve(path.join(dir, `${id}.jsonl`)));
        const closed = await post(`/api/sessions/${id}/close`, {});
        assert.equal(closed.status, 200, JSON.stringify(closed.body));
        const recoveryBody = await get('/api/recovery');
        const recovery = (0, test_types_js_1.present)((0, test_types_js_1.records)(recoveryBody.body.sessions).find(row => row.id === id));
        ...
        const nextRecovery = (0, test_types_js_1.records)((await get('/api/recovery')).body.sessions).find(row => row.id === id);
        assert.notEqual(nextRecovery?.status, 'closed', 'an explicit successful resume clears the prior close intent');
...
test('POST /resume preserves a nested generic session header id', async () => {
    ...
    assert.equal(getRPCSession((0, test_types_js_1.nativeId)('session')), null, 'generic basename does not create a duplicate live identity');
    const closed = await post(`/api/sessions/${id}/close`, {});
    assert.equal(closed.status, 200, JSON.stringify(closed.body));
});
...
test('POST /close shuts down an RPC child and removes it from the active list', async () => {
    ...
    const runtime = (0, test_types_js_1.record)(stats.body.runtime);
...
test('POST /restart replaces an RPC child without changing backend or session id', async () => {
    ...
    const before = (0, test_types_js_1.present)(await findActive(id));
    assert.equal((0, test_types_js_1.record)(before.capabilities).restart, true);
    assert.ok(before?.pid);
    const exited = new Promise(resolve => (0, test_types_js_1.present)(getRPCSession((0, test_types_js_1.nativeId)(id))).on('exit', () => resolve()));
    ...
    try {
        ...
        if (previousDelay === undefined)
            delete process.env.PI_FIXTURE_STARTUP_DELAY_MS;
        else
            process.env.PI_FIXTURE_STARTUP_DELAY_MS = previousDelay;
    }
    const after = (0, test_types_js_1.present)(await findActive(id));
    assert.notEqual(after.pid, before.pid, 'restart launched a fresh process');
    assert.equal(getRPCSession((0, test_types_js_1.nativeId)(id))?.proc.pid, after.pid, 'replacement remains managed by the RPC backend');
    const recovery = (0, test_types_js_1.records)((await get('/api/recovery')).body.sessions).find(row => row.id === id);
...
test('bulk API previews ownership, validates requests, cancels waiting targets and restarts an idle RPC runtime', async () => {
    ...
    const id = text(created.body.id);
    const original = (0, test_types_js_1.present)(getRPCSession((0, test_types_js_1.nativeId)(id)));
    try {
        assert.equal((0, test_types_js_1.record)((await get('/api/host')).body.capabilities).sessionBounces, true);
        ...
        const previewTarget = (0, test_types_js_1.present)((0, test_types_js_1.records)(preview.body.targets).find(target => target.sessionId === id));
        assert.equal(previewTarget.eligible, true);
        const reload = await get('/api/session-bounces/preview?mode=reload');
        assert.equal((0, test_types_js_1.present)((0, test_types_js_1.records)(reload.body.targets).find(target => target.sessionId === id)).eligible, false, 'an RPC runtime without a bridge never falls back to typing reload');
        ...
        const queuedOperation = (0, test_types_js_1.record)(queued.body.operation);
        assert.equal((0, test_types_js_1.present)((0, test_types_js_1.records)(queuedOperation.targets)[1]).status, 'skipped');
        const operations = (0, test_types_js_1.records)((await get('/api/session-bounces')).body.operations);
        const waiting = (0, test_types_js_1.present)(operations.find(operation => operation.id === queuedOperation.id));
        assert.match(text((0, test_types_js_1.present)((0, test_types_js_1.records)(waiting.targets)[0]).reason), /turn/);
        const duplicate = await post('/api/session-bounces', { mode: 'restart', sessionIds: [id] });
        assert.equal((0, test_types_js_1.present)((0, test_types_js_1.records)((0, test_types_js_1.record)(duplicate.body.operation).targets)[0]).status, 'skipped');
        const cancelledResponse = await fetch(base + `/api/session-bounces/${queuedOperation.id}`, { method: 'DELETE' });
        assert.equal(cancelledResponse.status, 200);
        const cancelled = (0, test_types_js_1.record)((await cancelledResponse.json()));
        assert.equal((0, test_types_js_1.present)((0, test_types_js_1.records)((0, test_types_js_1.record)(cancelled.operation).targets)[0]).status, 'cancelled');
        ...
        do {
            const operations = (0, test_types_js_1.records)((await get('/api/session-bounces')).body.operations);
            const operation = (0, test_types_js_1.present)(operations.find(candidate => candidate.id === (0, test_types_js_1.record)(ready.body.operation).id));
            result = (0, test_types_js_1.present)((0, test_types_js_1.records)(operation.targets)[0]);
            if (!['waiting', 'executing'].includes(text(result.status)))
                break;
            await new Promise(resolve => setTimeout(resolve, 50));
        } while (Date.now() < deadline);
        assert.equal(result.status, 'completed', text(result.reason));
        assert.equal(original.alive, false);
        assert.notEqual(getRPCSession((0, test_types_js_1.nativeId)(id)), original);
