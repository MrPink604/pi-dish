// Generated tool from test/skills-coverage.test.ts; edit that source and run npm run build:tools.
"use strict";
...
test('coverage counts overlapping ranges once per read and excludes old or untimed reads', () => {
    const coverage = (0, skills_js_1.projectSkillCoverage)('# A\na\n## B\nb\n## C\nc\n', 100, [
...
test('coverage keeps whitespace intro out of sections without dropping it from full-read or unread accounting', () => {
    const content = ' \n# A\na\n';
    const partial = (0, skills_js_1.projectSkillCoverage)(content, 100, [read({ ranges: [[2, -1]] })]);
    ...
    const explicit = (0, skills_js_1.projectSkillCoverage)(content, 100, [read({ kind: 'explicit', ranges: null, truncatedTo: 0 })]);
