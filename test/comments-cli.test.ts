...
import { nativeId as checkedNativeId, record, records } from './test-types.js';
...
test('comments CLI discovers its ancestor pi session, pages, and acknowledges', async (t) => {
  ...
  if (address === null || typeof address === 'string') throw new Error('Expected comments fixture TCP address');
  ...
  const parsedIndex = record(JSON.parse(index.stdout));
  const comments = records(parsedIndex.comments); const indexed = record(comments[0]);
  const target = record(indexed.target); const anchor = record(target.anchor);
  ...
  const selectedBody = record(JSON.parse(selected.stdout)); const selectedComments = records(selectedBody.comments);
  assert.equal(record(selectedComments[0]).body, 'Tighten this sentence.');
...
test('comments CLI reports the canonical route for an alternative registry entry', async (t) => {
  ...
  const session = await run(process.execPath, [cli, 'session'], { env });
  assert.equal(session.stdout.trim(), encodeSessionKey('omp', checkedNativeId('omp-native')));
});
