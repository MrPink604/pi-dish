// Generated test/tool from test/omp-export.test.ts; edit that source and run npm run build:tests.
"use strict";
...
const { exportOmpSessionHtml, injectOmpExportSnapshot, readOmpExportData, } = require('../lib/omp-export');
...
test('OMP export uses its native renderer, preserves the pre-header, and injects a live snapshot', async () => {
    ...
    try {
        ...
        assert.equal((0, test_types_js_1.record)(data.header).id, 'omp-export-test');
        assert.deepEqual(data.entries.map((entry) => (0, test_types_js_1.record)(entry).type), ['title', 'session_init', 'message']);
