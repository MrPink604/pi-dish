...
test('browser build detects stale output and preserves it on type failure', () => {
  ...
  try {
    ...
    const run = (...args: string[]) => spawnSync(process.execPath, ['scripts/build-browser.js', ...args], {
    ...
    const context = vm.createContext({ console: { log: (value: unknown) => logged.push(value) } }); vm.runInContext(appOriginal, context);
