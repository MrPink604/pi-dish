// Generated test/tool from test/helpers.test.ts; edit that source and run npm run build:tests.
"use strict";
...
(0, browser_vm_js_1.assertBrowserApiContext)(browserContext);
const B = browserContext.PiDishBrowser;
const property = (value, key) => (0, test_types_js_1.record)(value)[key];
const arrayProperty = (value, key) => {
    const result = property(value, key);
    return Array.isArray(result) ? result : [];
};
let workspaceSessionId = 0;
const workspaceSession = (cwd, lastActivity) => ({ id: `workspace-${++workspaceSessionId}`, cwd, lastActivity });
function renderMarkdown(marked, source) {
    const result = marked.parse(source);
    if (typeof result !== 'string')
        throw new Error('Expected synchronous marked output');
    return result;
}
...
test('stripAnsi removes CSI colors, OSC sequences, and stray escapes', () => {
    ...
    assert.equal(H.stripAnsi('\x1b[38;2;102;102;102mprocesses: \x1b[39m\x1b[38;2;138;190;183mticker2\x1b[39m \x1b[38;2;102;102;102mrunning\x1b[39m'), 'processes: ticker2 running');
...
test('formatRuntime names each backend and degrades to partial tmux info', () => {
    ...
    assert.equal(H.formatRuntime({ kind: 'tmux', pid: 99, server: 'default', tmuxSession: 'work', windowIndex: 3, windowName: 'pi' }), 'tmux default · work:3 pi · pid 99');
    // Pane query failed (dead server, no tmux): the socket name still locates it.
    assert.equal(H.formatRuntime({ kind: 'tmux', pid: 99, server: 'default', tmuxSession: null }), 'tmux default · pid 99');
    // The hidden headless placement reads as headless, not as tmux plumbing.
    assert.equal(H.formatRuntime({ kind: 'tmux', pid: 7, server: 'pi-dish', tmuxSession: 'headless', windowIndex: 1, windowName: 'pi' }), 'headless (hidden tmux — survives restarts) · pid 7');
...
test('sessionMetaText joins the searchable fields lowercased', () => {
    assert.equal(H.sessionMetaText({ name: 'Fix Login', cwd: '/home/U/App', model: 'GPT-5', id: 'S1' }), 'fix login /home/u/app gpt-5 s1');
...
test('parseIpythonResult unwraps BashResult reprs only when complete', () => {
    ...
    assert.deepEqual(H.parseIpythonResult("BashResult(exit_code=0, output='no duration')"), { exitCode: 0, output: 'no duration', durationMs: null });
...
test('groupByWorkspace groups by cwd and sorts by recency', () => {
    ...
    assert.deepEqual(groups.map((group) => Array.isArray(group) ? group[0] : undefined), ['~', '/b', '/a']);
    // within /a, newest first
    const groupA = Array.isArray(groups[2]) && Array.isArray(groups[2][1]) ? groups[2][1] : [];
    assert.deepEqual(groupA.map((session) => property(session, 'lastActivity')), ['2026-01-02T00:00:00Z', '2026-01-01T00:00:00Z']);
});
test('groupByWorkspace sinks collapsed groups below expanded ones', () => {
    ...
    assert.deepEqual(groups.map((group) => Array.isArray(group) ? group[0] : undefined), ['/c', '/a', '/b']);
});
test('buildWorkspaceTree shows a shared prefix once with distinguishing tails as children', () => {
    ...
    assert.deepEqual(tree[0].children.map((child) => [
        property(child, 'label'), property(child, 'path'), property(child, 'count'),
    ]), [
...
test('buildWorkspaceTree flattens multi-segment chains below a divergence point', () => {
    ...
    assert.deepEqual(tree[0].children.map((c) => property(c, 'label')), ['deep/nested/proj', 'other']);
});
test('buildWorkspaceTree keeps unrelated workspaces as flat single nodes', () => {
    ...
    assert.deepEqual(tree.map((node) => [property(node, 'label'), property(node, 'path'), arrayProperty(node, 'children').length]), [
        ['~', '~', 0],
        ['~/app', '/home/u/app', 0],
        ['/etc/x', '/etc/x', 0],
    ]);
    assert.equal((0, test_types_js_1.present)((0, test_types_js_1.present)(tree[1]).sessions).length, 1);
});
test('buildWorkspaceTree keeps sessions living at a prefix that also has children', () => {
    ...
    assert.equal(tree.length, 1);
    const root = (0, test_types_js_1.present)(tree[0]);
    assert.equal(root.path, '/w/app');
    assert.equal((0, test_types_js_1.present)(root.sessions).length, 1);
    assert.equal(root.count, 2);
    assert.deepEqual(root.children.map(child => child.label), ['sub']);
});
test('buildWorkspaceTree sinks collapsed nodes below expanded siblings per level', () => {
    ...
    assert.deepEqual(tree[0].children.map((c) => property(c, 'label')), ['older', 'newest']);
});
test('buildWorkspaceTree hoists a bare home root — ~/x groups stay top-level', () => {
    ...
    assert.deepEqual(tree.map((node) => property(node, 'label')), ['~/workspace', '~/src/dotfiles']);
    assert.deepEqual(tree[0].children.map((c) => property(c, 'label')), ['a', 'b']);
});
...
test('buildSessionFamilies keeps parents first and sorts whole blocks by newest descendant', () => {
    ...
    assert.deepEqual(roots.map((root) => property(property(root, 'session'), 'id')), ['other-cwd', 'parent', 'standalone']);
    assert.equal(roots[1].activity, Date.parse('2026-01-05T00:00:00Z'));
    assert.equal(roots[1].size, 3);
    assert.deepEqual(roots[1].children.map((child) => property(property(child, 'session'), 'id')), ['child']);
    assert.deepEqual(H.flattenSessionFamilies([roots[1]]).map((s) => property(s, 'id')), ['parent', 'child', 'grandchild']);
});
test('session family cycles degrade to standalone roots', () => {
    ...
    assert.deepEqual(roots.map((root) => property(property(root, 'session'), 'id')), ['b', 'a']);
    assert.ok(roots.every((root) => arrayProperty(root, 'children').length === 0));
});
...
test('partitionPinnedFamilies pins and orders the whole family from any member id', () => {
    ...
    assert.deepEqual(pinned.map((root) => property(property(root, 'session'), 'id')), ['p']);
    assert.deepEqual(rest.map((root) => property(property(root, 'session'), 'id')), ['other']);
    const activeFragment = H.buildSessionFamilies([
        { id: 'c', parentId: 'p', familyParentId: 'p', cwd: '/w', lastActivity: 2 },
    ]);
    assert.deepEqual(H.partitionPinnedFamilies(activeFragment, ['p'])[0].map((root) => property(property(root, 'session'), 'id')), ['c'], 'missing inactive parent aliases its visible child fragment');
    const crossCwdFragment = H.buildSessionFamilies([
        { id: 'x', parentId: 'p', familyParentId: null, cwd: '/other', lastActivity: 3 },
    ]);
    assert.equal(H.partitionPinnedFamilies(crossCwdFragment, ['p'])[0].length, 0, 'confirmed cross-workspace lineage never aliases the parent pin');
});
test('partitionPinnedFamilies matches pins by host + session key', () => {
    ...
    assert.deepEqual(H.partitionPinnedFamilies(families, ['hostA c'])[0]
        .map((root) => `${property(property(root, 'session'), 'host')} ${property(property(root, 'session'), 'cwd')}`), ['hostA /w']);
    ...
    assert.deepEqual(H.partitionPinnedFamilies(fragment, ['hostA p'])[0].map((root) => property(property(root, 'session'), 'id')), ['c'], 'missing parent aliases within its own host');
...
test('sortRelations ranks singular lineage links ahead of child lists, stable within a kind', () => {
    ...
    assert.deepEqual(sorted.map((r) => property(property(r, 'session'), 'id')), ['p', 'sf', 'c1', 'c2', 'c3', 'sh1', 'm']);
    assert.deepEqual(input.map(r => r.session.id), ['c1', 'c2', 'sh1', 'p', 'sf', 'c3', 'm'], 'input array is not mutated');
...
test('groupRelations groups by kind in rank order and skips nothing', () => {
    ...
    assert.deepEqual(groups.map(group => group.kind), ['parent', 'child', 'startedHere']);
    assert.deepEqual((0, test_types_js_1.present)(groups[1]).relations.map(relation => relation.session.id), ['c1', 'c2']);
    assert.deepEqual(H.groupRelations(undefined), []);
    const kindless = [{ session: { id: 'x' } }];
    const generic = Reflect.apply(H.groupRelations, H, [kindless]);
    assert.equal((0, test_types_js_1.present)(generic[0]).kind, 'related', 'kind-less relations fall into a generic group');
});
test('partitionPinned splits in pinned order and skips unknown ids', () => {
    const list = [{ id: 'a' }, { id: 'b' }, { id: 'c' }];
    const [pinned, rest] = H.partitionPinned(list, ['c', 'gone', 'a']);
    assert.deepEqual(pinned.map((s) => property(s, 'id')), ['c', 'a']);
    assert.deepEqual(rest.map((s) => property(s, 'id')), ['b']);
...
test('applyLocalFilter requires every token across name/cwd/model/id', () => {
    ...
    assert.deepEqual(H.applyLocalFilter(list, 'webapp gpt').map((s) => property(s, 'id')), ['s1']);
...
test('parseSessionQuery accepts only real ISO calendar dates', () => {
    const p = H.parseSessionQuery('since:2024-02-29 before:2026-02-31 since:2026-02-29 before:2026-13-01 since:2026-04-31 before:2026-01-00');
    assert.equal(p.since, new Date('2024-02-29T00:00:00').getTime(), 'a valid leap day keeps local-midnight semantics');
...
test('host: composes with is:active, since: and plain terms', () => {
    ...
    assert.deepEqual(H.positiveQueryTokens(H.parseSessionQuery('host:beelink login')), ['login'], 'host: never reaches content search');
});
test('routine: is a field term over the session\'s routine stamp', () => {
    ...
    assert.equal(H.scoreSessionMatch(H.parseSessionQuery('routine:nightly-review'), stamped), 0, 'field terms filter, they never score');
    assert.deepEqual(H.positiveQueryTokens(H.parseSessionQuery('routine:nightly login')), ['login'], 'routine: never reaches content search');
});
test('is:automation tests routine provenance; queryAsksForAutomation is positive-only', () => {
    ...
    assert.equal(H.isAutomationSession({ id: 'routine-only', routineId: 'r1' }), true, 'the id alone stamps it too');
...
test('stripQueryField removes a field\'s tokens and leaves the rest tokenized as parsed', () => {
    ...
    assert.deepEqual(rest.terms, full.terms.filter((term) => property(term, 'field') !== 'host'));
});
test('applyHostTerms filters on host terms alone, ignoring the rest of the query', () => {
    ...
    assert.deepEqual(H.applyHostTerms(list, 'host:beelink').map((s) => property(s, 'id')), ['s1']);
    assert.deepEqual(H.applyHostTerms(list, '-host:beelink').map((s) => property(s, 'id')), ['s2', 's3']);
    assert.deepEqual(H.applyHostTerms(list, 'host:uuid-3').map((s) => property(s, 'id')), ['s3']);
    // Non-host terms are the server's job on this path — they must not narrow.
    assert.deepEqual(H.applyHostTerms(list, 'host:beelink zzz name:nope').map((s) => property(s, 'id')), ['s1']);
...
test('applyLocalFilter understands the query grammar', () => {
    ...
    assert.deepEqual(H.applyLocalFilter(list, '-name:subagent').map((s) => property(s, 'id')), ['s2']);
    assert.deepEqual(H.applyLocalFilter(list, 'cwd:webapp').map((s) => property(s, 'id')), ['s1']);
    assert.equal(H.applyLocalFilter(list, ''), list);
});
...
test('scoreSessionMatch: metadata outranks content, name outranks the rest', () => {
    ...
    const contentOnly = H.scoreSessionMatch(p, { name: 'n', cwd: '/c', model: 'm', id: 'i' }, 'alpha '.repeat(200));
...
test('applyLocalFilter orders matches by relevance, then recency', () => {
    ...
    assert.deepEqual(H.applyLocalFilter(list, 'alpha').map((s) => property(s, 'id')), ['s3', 's2', 's1']);
    // No positive plain term → the incoming order stands.
    assert.deepEqual(H.applyLocalFilter(list, 'cwd:api').map((s) => property(s, 'id')), ['s2', 's3']);
});
test('groupSessionsByDate buckets by recency with undated sunk last', () => {
    ...
    assert.deepEqual(buckets.map((bucket) => property(bucket, 'key')), ['today', 'yesterday', 'lastweek', 'm:2026-05', 'undated']);
    assert.deepEqual(buckets[0].sessions.map((s) => property(s, 'id')), ['today', 'today2']);
...
test('groupSessionsByDate: Monday belongs to This week, Sunday before to Last week', () => {
    ...
    assert.deepEqual(buckets.map((bucket) => property(bucket, 'key')), ['yesterday', 'lastweek']);
});
...
test('highlightFuzzy wraps matched chars and escapes the rest', () => {
    assert.equal(H.highlightFuzzy('a<b', [0]), '<span class="cwd-match">a</span>&lt;b');
...
test('isUnreadSession keys on host + session, so ids may collide across hosts', () => {
    ...
    assert.equal(H.isUnreadSession(sess('hostB'), seen, null, true), true, "another host's same-id session has its own seen state");
...
test('sanitizeHostCatalog drops broken entries instead of throwing', () => {
    ...
    assert.deepEqual(B.sanitizeHostCatalog([{ base: 'http://a:1', label: 7, token: 7 }]), [{ base: 'http://a:1' }]);
});
...
test('highlightTokens escapes HTML and merges overlapping marks', () => {
    ...
    assert.equal(H.highlightTokens('the Needle here', ['needle']), 'the <mark>Needle</mark> here', 'case-insensitive, original casing kept');
...
test('findPathTokens picks file mentions out of prose, skipping URLs and word pairs', () => {
    ...
    const tokens = H.findPathTokens(text).map((token) => property(token, 'token'));
...
test('aggregateUsageWeekly chunks from the end and merges model rows by ref', () => {
    const mkDay = (day, cost, ref) => ({
        ...
        models: [{ ref, provider: ref.split('/')[0], model: ref.split('/')[1], calls: 1, cost,
                costUnavailable: { total: 0 }, tokens: { input: 10, output: 5, cacheRead: 0, cacheWrite: 0, reasoning: 0 } }],
    });
    const daily = [];
    for (let i = 0; i < 10; i++)
        daily.push(mkDay(`2026-07-${String(i + 1).padStart(2, '0')}`, 1, i % 2 ? 'a/m1' : 'b/m2'));
    const weeks = H.aggregateUsageWeekly(daily);
    assert.equal(weeks.length, 2);
    assert.deepEqual(weeks.map(week => week.days), [3, 7]);
    const old = (0, test_types_js_1.present)(weeks[0]), recent = (0, test_types_js_1.present)(weeks[1]);
    assert.equal(old.day, '2026-07-01');
    assert.equal(recent.day, '2026-07-04');
    assert.equal(recent.calls, 7);
    assert.equal(recent.costs.total, 7);
    assert.equal(recent.tokens.input, 70);
    assert.deepEqual(recent.models.map(model => model.ref).sort(), ['a/m1', 'b/m2']);
    assert.equal(recent.models.reduce((sum, model) => sum + (model.calls ?? 0), 0), 7);
});
...
test('mergeUsageLimits collapses agreeing rows and qualifies differing ones', () => {
    ...
    const limit = (usedFraction, resetsAt, extra = {}) => ({
        label: '7 days', windowLabel: '7 days', resetsAt, usedFraction, ...extra,
    });
    const entry = (hostLabel, reports) => {
        const payload = { generatedAt: now, harnesses: [{ harness: 'omp', label: 'Oh My Pi', reports }] };
        return { hostLabel, payload };
    };
    const report = (limits, { planType = 'pro', fetchedAt = now, provider = 'fakeprov' } = {}) => ({ provider, planType, fetchedAt, limits });
    ...
    const shared = (0, test_types_js_1.present)(merged.reports[0]), sharedLimit = (0, test_types_js_1.present)(shared.limits[0]);
    assert.equal(shared.limits.length, 1, 'near-identical rows collapse');
    assert.equal(sharedLimit.hosts, null, 'a fleet-wide row carries no qualifier');
    assert.equal(sharedLimit.usedFraction, 0.42, 'the freshest report wins');
    assert.equal(shared.planType, 'pro');
    merged = H.mergeUsageLimits([
        entry('host-a', [report([limit(0.1, now + 86400000)])]),
        entry('host-b', [report([limit(0.9, now + 86400000)])]),
    ]);
    const differing = (0, test_types_js_1.present)(merged.reports[0]);
    assert.equal(differing.limits.length, 2, 'disagreeing rows do not merge');
    assert.deepEqual(differing.limits.map(row => row.hosts), [['host-a'], ['host-b']]);
    assert.equal(differing.planType, 'pro', 'a uniform plan still shows at provider level');
    merged = H.mergeUsageLimits([
        entry('host-a', [report([limit(0.42, now + 86400000)], { planType: 'pro' })]),
        entry('host-b', [report([limit(0.42, now + 86400000)], { planType: 'max' })]),
    ]);
    const split = (0, test_types_js_1.present)(merged.reports[0]);
    assert.equal(split.limits.length, 2, 'plan mismatch keeps rows separate');
    assert.equal(split.planType, null, 'a split plan leaves the provider header');
    merged = H.mergeUsageLimits([
        entry('host-a', [report([limit(0.42, now + 86400000)])]),
    ...
    assert.equal((0, test_types_js_1.present)((0, test_types_js_1.present)(merged.reports[0]).limits[0]).hosts, null, 'the surviving row needs no qualifier');
});
test('usageLimitsHtml renders the merged provider quota view', () => {
    const now = 1700000000000;
    const entry = (hostLabel, harnesses) => {
        const payload = { generatedAt: now, harnesses };
        return { hostLabel, payload };
    };
    ...
    const base = [
        { label: '5 hours', windowLabel: '5 hours', resetsAt: now + 3600000, usedFraction: 0.42 },
        { label: '<7> days', windowLabel: '7 days', resetsAt: null, usedFraction: 0.9 },
    ];
    ...
    const two = H.usageLimitsHtml([
        ...
        entry('host-b', [omp([{ ...(0, test_types_js_1.present)(base[0]) }, { ...(0, test_types_js_1.present)(base[1]), usedFraction: 0.2 }])]),
    ], { now });
    const fiveHour = (0, test_types_js_1.present)(two.match(/5 hours<\/span><small>42% used[^<]*<\/small>/))[0];
    ...
    assert.equal(H.usageLimitsHtml([entry('host-a', [])], { now }), '', 'a host with no supporting harness contributes no section');
});
...
test('OMP_MODEL_ROLES lists the canonical roles in harness order', () => {
    assert.deepEqual(H.OMP_MODEL_ROLES.map((r) => property(r, 'key')), ['default', 'smol', 'slow', 'vision', 'plan', 'designer', 'commit', 'tiny', 'task', 'advisor']);
    assert.ok(H.OMP_MODEL_ROLES.every((role) => property(role, 'name') && property(role, 'description')));
});
test('buildModelRoleRows keeps global values editable and flags project overrides', () => {
    const rows = H.buildModelRoleRows({ default: 'zai/glm-4.7', 'my-role': 'zai/glm-5.2' }, { default: 'zai/glm-4.7', vision: 'anthropic/claude-opus-4', 'my-role': 'zai/glm-5.2' });
    assert.equal(rows.length, H.OMP_MODEL_ROLES.length + 1);
    const def = (0, test_types_js_1.present)(rows.find(row => row.key === 'default'));
    ...
    const vision = (0, test_types_js_1.present)(rows.find(row => row.key === 'vision'));
    assert.equal(vision.value, '');
    assert.equal(vision.override, 'anthropic/claude-opus-4');
    const custom = (0, test_types_js_1.present)(rows.at(-1));
    assert.deepEqual({ key: custom.key, custom: custom.custom, value: custom.value, override: custom.override }, { key: 'my-role', custom: true, value: 'zai/glm-5.2', override: null });
    assert.ok(rows.slice(0, H.OMP_MODEL_ROLES.length).every(row => row.custom === false));
});
test('buildModelRoleRows tolerates missing or malformed records', () => {
    ...
    assert.ok(rows.every((r) => property(r, 'value') === '' && property(r, 'override') === null));
    assert.equal(H.buildModelRoleRows({ default: 42, ok: 'zai/glm-5.2' }, ['x']).length, H.OMP_MODEL_ROLES.length + 1, 'non-string values are dropped, arrays are not records');
});
test('formatModelRoleSummary orders canonically and truncates gracefully', () => {
    ...
    assert.equal(H.formatModelRoleSummary({ default: 'a/b', smol: 'a/b', slow: 'a/b', vision: 'a/b', plan: 'a/b' }), 'default a/b · smol a/b · slow a/b · vision a/b · +1 more');
    assert.equal(H.formatModelRoleSummary({ zeta: 'a/b', default: 'c/d' }, 2), 'default c/d · zeta a/b', 'custom keys sort after the canonical ones');
});
test('parseModelRoleRef splits the ":level" suffix like OMP resolves it', () => {
    const known = ['zai/glm-5.2', 'deepseek/deepseek-v3.2:max'];
    assert.deepEqual(H.parseModelRoleRef('openai-codex/gpt-5.6-sol:xhigh', known), { model: 'openai-codex/gpt-5.6-sol', level: 'xhigh' });
    ...
    assert.deepEqual(H.parseModelRoleRef('deepseek/deepseek-v3.2:max', known), { model: 'deepseek/deepseek-v3.2:max', level: '' });
    // …but it is one when the catalog doesn't list the full string.
    assert.deepEqual(H.parseModelRoleRef('deepseek/deepseek-v3.2:max', []), { model: 'deepseek/deepseek-v3.2', level: 'max' });
...
test('modelRoleLevels follows OMP\'s roles order off/auto then the model ladder', () => {
    ...
    assert.deepEqual(H.modelRoleLevels({ thinking: null }), ['off', 'auto', 'minimal', 'low', 'medium', 'high', 'xhigh', 'max']);
    assert.deepEqual(H.modelRoleLevels(null), ['off', 'auto', 'minimal', 'low', 'medium', 'high', 'xhigh', 'max']);
...
test('sessionRef qualifies with the fleet name when the host has one', () => {
    ...
    assert.equal(H.sessionRef(session, { name: 'tycho', base: '/hosts/tycho', hostId: 'tycho-id' }), 'tycho/abcdefgh');
...
test('sessionRef falls back to hostId plus the full id for an unnamed host', () => {
    const session = { id: 'abcdefghijklmnop' };
    assert.equal(H.sessionRef(session, { hostId: 'host-uuid', base: 'http://ganymede:3333' }), 'host-uuid:abcdefghijklmnop');
});
test('sessionRef degrades to the bare prefix rather than throwing', () => {
    ...
    const invalidSession = { id: 42 };
    assert.equal(Reflect.apply(H.sessionRef, H, [invalidSession, null]), '');
});
...
test('sessionRef takes a widened prefix in every host form', () => {
    ...
    assert.equal(H.sessionRef(session, { name: 'tycho', base: '/hosts/tycho' }, '2026-08-20'), 'tycho/2026-08-20');
    // The uuid form is already unambiguous — it carries the whole id.
    assert.equal(H.sessionRef(session, { hostId: 'host-uuid', base: 'http://x:1' }, '2026-08-20'), 'host-uuid:2026-08-20T10-00-00-aaaa1111');
...
test('sessionRefAliases derives the native id and uuid tail from the id alone', () => {
    ...
    assert.equal((0, test_types_js_1.present)(H.decodeRouteSessionId(OMP_A)).harnessId, 'omp');
});
...
test('resolveSessionRefAmong keeps route-id resolution ahead of aliases', () => {
    ...
    assert.equal((0, test_types_js_1.present)(H.resolveSessionRefAmong(shadow, '01a07264').session).name, 'literal id');
    ...
    assert.deepEqual(ambiguous.matches.map((s) => property(s, 'name')), ['orchestrator', 'reviewer']);
    ...
    assert.equal((0, test_types_js_1.present)(H.resolveSessionRefAmong(CORPUS, OMP_A, { exactOnly: true }).session).name, 'orchestrator');
...
test('shortSessionRef prefers the uuid tail and widens only when it must', () => {
    ...
    for (const session of CORPUS) {
        ...
        assert.equal((0, test_types_js_1.present)(H.resolveSessionRefAmong(CORPUS, ref).session).name, session.name, ref);
    }
...
test('parseSessionRefTokens finds every ref form and dedupes them', () => {
    ...
    assert.deepEqual(H.parseSessionRefTokens(text).map((token) => property(token, 'ref')), [
        '8f3ab2c1',
        'tycho/8f3ab2c1',
...
test('parseSessionRefTokens trims sentence punctuation but never id characters', () => {
    ...
    assert.deepEqual(H.parseSessionRefTokens('resume #2026-08- now').map((token) => property(token, 'ref')), ['2026-08-']);
    assert.deepEqual(H.parseSessionRefTokens('resume #8f3ab2c1.').map((token) => property(token, 'ref')), ['8f3ab2c1']);
    assert.deepEqual(H.parseSessionRefTokens('resume #8f3ab2c1:').map((token) => property(token, 'ref')), ['8f3ab2c1']);
    assert.deepEqual(H.parseSessionRefTokens('resume #tycho/').map((token) => property(token, 'ref')), ['tycho']);
});
test('parseSessionRefTokens leaves markdown, short tokens and glued hashes alone', () => {
    ...
    assert.deepEqual(H.parseSessionRefTokens('`#abcdefgh`').map((token) => property(token, 'ref')), []);
    assert.deepEqual(H.parseSessionRefTokens('(#abcdefgh)').map((token) => property(token, 'ref')), ['abcdefgh']);
});
...
test('appendSessionRefContext is a no-op without resolved entries', () => {
    ...
    const unresolvedEntries = [{ name: 'x' }];
    assert.equal(Reflect.apply(H.appendSessionRefContext, H, ['#nothing-resolved', unresolvedEntries]), '#nothing-resolved');
...
test('session ref fields cannot break out of the block', () => {
    ...
    assert.equal((0, test_types_js_1.present)(block.match(/<session-refs>/g)).length, 1);
    assert.equal((0, test_types_js_1.present)(block.match(/<\/session-refs>/g)).length, 1);
...
test('searchSessionsForRef ranks name matches over cwd, live over historical', () => {
    ...
    assert.deepEqual(H.searchSessionsForRef(list, 'index').map((r) => property(property(r, 'session'), 'id')), ['aaaa1111', 'cccc3333']);
    assert.deepEqual(H.searchSessionsForRef(list, 'ttr').map((r) => property(property(r, 'session'), 'id')), ['bbbb2222']);
    // A pasted prefix finds its own session even when the name says nothing.
    assert.deepEqual(H.searchSessionsForRef(list, 'cccc').map((r) => property(property(r, 'session'), 'id')), ['cccc3333']);
    // No query is "most recent, live first".
    assert.deepEqual(H.searchSessionsForRef(list, '').map((r) => property(property(r, 'session'), 'id')), ['cccc3333', 'bbbb2222', 'aaaa1111']);
    ...
    assert.deepEqual(H.searchSessionsForRef(omp, '01a070d2').map((r) => property(property(r, 'session'), 'id')), [OMP_A]);
    assert.deepEqual(H.searchSessionsForRef(omp, '2026-09-05T09').map((r) => property(property(r, 'session'), 'id')), [OMP_A]);
    assert.deepEqual(H.searchSessionsForRef(omp, '~sk1_WyJvbXAi').map((r) => property(property(r, 'session'), 'id')), [OMP_A]);
    assert.deepEqual(H.searchSessionsForRef(omp, 'ffff'), []);
});
test('mergeHostEntries puts self first and keys on hostId, then base', () => {
    const hosts = B.mergeHostEntries({ hostId: 'self-id', label: 'laptop', capabilities: { tmux: true } }, [{ name: 'tycho', base: '/hosts/tycho', kind: 'ssh', hostId: 'tycho-id', reachable: true },
        { self: true, base: '', hostId: 'self-id' }], [{ base: 'http://ganymede:3333/', label: 'ganymede', token: 'tok' }]);
    assert.deepEqual(hosts.map((h) => property(h, 'key')), ['self-id', 'tycho-id', 'http://ganymede:3333']);
    ...
    assert.equal(hosts.filter((h) => property(h, 'hostId') === 'self-id').length, 1, 'the fleet listing of ourselves folds into the self entry');
});
test('mergeHostEntries folds duplicates but keeps the fields only the loser had', () => {
    const hosts = B.mergeHostEntries({ hostId: 'self-id' }, [{ name: 'tycho', base: '/hosts/tycho', hostId: 'tycho-id' }], [{ base: 'http://tycho:3333', hostId: 'tycho-id', token: 'tok', label: 'Tycho (direct)' }]);
...
test('mergeHostEntries drops garbage bases instead of guessing', () => {
    const hosts = B.mergeHostEntries({ hostId: 'self-id' }, [{ name: 'bad', base: 'tycho:3333' }, null, { name: 'ok', base: 'http://ok:1' }], [{ base: 'not a url' }, { nope: true }]);
    assert.deepEqual(hosts.map((host) => property(host, 'base')), ['', 'http://ok:1']);
});
// --- per-host capability gating -------------------------------------------
test('hostSupportsTerminal reads the owning host\'s advertised capabilities', () => {
    const remote = (capabilities) => ({ hostId: 'tycho-id', base: 'http://tycho:3333', capabilities });
    assert.equal(H.hostSupportsTerminal(remote({ terminal: true, tmux: true }), { terminal: false }), true, 'a peer that advertises the terminal is usable from a terminal-less entry host');
    assert.equal(H.hostSupportsTerminal(remote({ terminal: false }), { terminal: true }), false);
    assert.equal(H.hostSupportsTerminal(remote({ sessions: true }), { terminal: true }), false, 'absent capability in an advertised set means unsupported');
});
test('hostSupportsTerminal falls back to /api/config for self only', () => {
    const self = { hostId: 'self-id', base: '', self: true };
    assert.equal(H.hostSupportsTerminal(self, { terminal: true }), true, 'self before /api/host answers keeps single-host behavior');
    assert.equal(H.hostSupportsTerminal(self, { terminal: false }), false);
    const legacySelf = { base: '', capabilities: null };
    assert.equal(Reflect.apply(H.hostSupportsTerminal, H, [legacySelf, { terminal: true }]), true, 'the serving origin is self even without the flag');
    assert.equal(H.hostSupportsTerminal({ ...self, capabilities: { sessions: true } }, { terminal: true }), false, 'self capabilities win over the config fallback once advertised');
    assert.equal(H.hostSupportsTerminal({ hostId: 'old-id', base: 'http://old:3333' }, { terminal: true }), false, 'a remote of unknown build hides the button rather than serving a dead one');
    assert.equal(H.hostSupportsTerminal(null, { terminal: true }), false, 'no entry at all (a host that left the list) is not this host');
});
...
test('sttUnavailableReason reports the first blocking condition, secure origin first', () => {
    ...
    const insecure = (0, test_types_js_1.present)(H.sttUnavailableReason({ ...ok, isSecureContext: false, hasGetUserMedia: false, hasMediaRecorder: false }));
    ...
    assert.deepEqual(H.sttUnavailableReason({ ...ok, hasGetUserMedia: false }), { code: 'no-getusermedia', message: 'This browser exposes no microphone API.' });
    assert.deepEqual(H.sttUnavailableReason({ ...ok, hasMediaRecorder: false }), { code: 'no-recorder', message: "This browser can't record audio (no MediaRecorder)." });
    // Called with nothing known: still a reason, never a crash.
    assert.equal((0, test_types_js_1.present)(H.sttUnavailableReason()).code, 'insecure');
    assert.match((0, test_types_js_1.present)(H.sttUnavailableReason({ isSecureContext: false })).message, /this origin/);
});
...
const hostState = (previous, event, at) => (0, test_types_js_1.present)(B.hostConnReduce(previous, event, at));
test('hostConnReduce climbs the backoff ladder one rung per failure', () => {
    const fail = (previous, at) => hostState(previous, { type: 'failure', error: new Error('EHOSTUNREACH') }, at);
    let s = fail(null, T0);
...
test('hostConnReduce takes the failure error as text, from Error or string', () => {
    assert.equal(hostState(null, { type: 'failure', error: 'boom' }, T0).error, 'boom');
    assert.equal(hostState(null, { type: 'failure', error: new Error('nope') }, T0).error, 'nope');
    assert.equal(hostState(null, { type: 'failure' }, T0).error, null);
});
test('hostConnReduce success clears the error but keeps the rung until it settles', () => {
    const down = hostState(hostState(null, { type: 'failure', error: 'x' }, T0), { type: 'failure', error: 'x' }, T0);
    assert.equal(down.failures, 2);
    const up = hostState(down, 'success', T0 + 100);
...
test('hostConnReduce forgives the ladder only after 30s of unbroken reachability', () => {
    const down = hostState(null, { type: 'failure', error: 'x' }, T0);
    const up = hostState(down, 'success', T0);
    // One millisecond short of the window: still carrying the rung.
    const almost = hostState(up, 'success', T0 + 29_999);
    ...
    const settled = hostState(up, 'success', T0 + 30_000);
...
test('hostConnReduce restarts the hysteresis clock after a flap', () => {
    let s = hostState(null, 'success', T0);
    s = hostState(s, { type: 'failure', error: 'flap' }, T0 + 10_000);
    assert.equal(s.failures, 1);
    s = hostState(s, 'success', T0 + 20_000);
    // 20s after the first success, but only just back up: not forgiven.
    assert.equal(s.failures, 1);
    assert.equal(s.reachableSince, T0 + 20_000);
    s = hostState(s, { type: 'failure', error: 'flap' }, T0 + 25_000);
...
test('hostConnReduce blocked is sticky: neither failure nor success demotes it', () => {
    const blocked = hostState(null, 'blocked', T0);
    assert.deepEqual({ state: blocked.state, failures: blocked.failures, retryAt: blocked.retryAt, error: blocked.error }, { state: 'blocked', failures: 0, retryAt: 0, error: 'Unauthorized' });
    // A failure while blocked is a no-op, identity included (callers skip the
    // re-render on it).
    assert.equal(hostState(blocked, { type: 'failure', error: 'timeout' }, T0 + 5), blocked);
    assert.equal(hostState(blocked, 'blocked', T0 + 5), blocked);
    // Only a real success (the token was entered) can leave it.
    assert.equal(hostState(blocked, 'success', T0 + 5).state, 'reachable');
});
test('hostConnReduce blocked replaces a backoff state outright', () => {
    const down = hostState(null, { type: 'failure', error: 'x' }, T0);
    const blocked = hostState(down, 'blocked', T0 + 1);
...
test('hostConnReduce seed-down only applies to a host with no state at all', () => {
    const seeded = hostState(null, { type: 'seed-down', error: 'ssh: connect timed out' }, T0);
    assert.deepEqual([seeded.state, seeded.failures, seeded.retryAt, seeded.error], ['backoff', 1, T0 + 3000, 'ssh: connect timed out']);
    // Everything this client observed itself outranks the server's probe.
    for (const prev of [
        hostState(null, 'success', T0),
        hostState(null, 'blocked', T0),
        hostState(null, { type: 'failure', error: 'mine' }, T0),
    ]) {
        assert.equal(hostState(prev, { type: 'seed-down', error: 'theirs' }, T0 + 1), prev);
    }
});
...
test('mergeUsageSummaries sums totals, days, and per-model day buckets', () => {
    ...
    const merged = (0, test_types_js_1.present)(H.mergeUsageSummaries([
        { hostId: 'host-a', hostLabel: 'laptop', summary: a },
        { hostId: 'host-b', hostLabel: 'tycho', summary: b },
    ]));
    const totals = (0, test_types_js_1.present)(merged.totals), tokens = (0, test_types_js_1.present)(totals.tokens), costs = (0, test_types_js_1.present)(totals.costs);
    const headline = (0, test_types_js_1.present)((0, test_types_js_1.present)(merged.headlineCosts).month);
    const headlineBuckets = (0, test_types_js_1.present)((0, test_types_js_1.present)(merged.headlineCostsByBucket).month);
    const daily = (0, test_types_js_1.present)(merged.daily);
    assert.equal(totals.calls, 4);
    assert.equal(tokens.input, 20);
    assert.ok(Math.abs((0, test_types_js_1.present)(costs.total) - 0.6) < 1e-9);
    assert.equal(headline.toFixed(2), '0.60', 'KPI headlines sum');
    assert.ok(Math.abs((0, test_types_js_1.present)(headlineBuckets.total) - 0.6) < 1e-9 &&
        Math.abs((0, test_types_js_1.present)(headlineBuckets.input) - 0.2) < 1e-9, 'headline cost buckets sum per component');
    assert.deepEqual(daily.map(day => day.day), ['2026-08-20', '2026-08-21'], 'days merge chronologically');
    const shared = (0, test_types_js_1.present)(daily.find(day => day.day === '2026-08-21'));
    assert.equal(shared.calls, 4);
    assert.equal((0, test_types_js_1.present)(shared.models).length, 1, 'the same model ref on two hosts is one series');
    assert.ok(Math.abs(((0, test_types_js_1.present)((0, test_types_js_1.present)(shared.models)[0]).cost ?? 0) - 0.6) < 1e-9);
...
test('mergeUsageSummaries keeps per-host workspaces and sessions apart, merges models', () => {
    ...
    const groups = (0, test_types_js_1.present)(merged.groups);
    const workspaces = (0, test_types_js_1.present)(groups.workspaces), sessions = (0, test_types_js_1.present)(groups.sessions), models = (0, test_types_js_1.present)(groups.models);
    assert.equal(workspaces.length, 2, 'the same path on two machines is two workspaces');
    assert.deepEqual(workspaces.map(workspace => workspace.hostLabel), ['laptop', 'tycho']);
    assert.equal(sessions.length, 2, 'session ids are only unique within a host');
    assert.deepEqual(sessions.map(session => session.host), ['host-a', 'host-b']);
    assert.equal(models.length, 1, 'a model ref means the same thing everywhere');
    assert.equal((0, test_types_js_1.present)(models[0]).calls, 4);
});
test('mergeUsageSummaries preserves known subtotals and unavailable counts', () => {
    ...
    const totals = (0, test_types_js_1.present)(merged.totals), costs = (0, test_types_js_1.present)(totals.costs);
    assert.ok(Math.abs((0, test_types_js_1.present)(costs.total) - 0.7) < 1e-9);
    assert.equal(totals.priced, false);
    assert.equal(totals.unpricedCalls, 3);
    assert.ok(Math.abs((0, test_types_js_1.present)((0, test_types_js_1.present)(merged.headlineCosts).month) - 0.7) < 1e-9);
    assert.equal((0, test_types_js_1.present)(merged.headlineCostUnavailable).month, 3);
...
test('mergeUsageSummaries ranks merged groups by the requested metric', () => {
    ...
    const byCost = (0, test_types_js_1.present)(H.mergeUsageSummaries([
        { hostId: 'a', summary: usagePayload() }, { hostId: 'b', summary: small },
    ]));
    assert.deepEqual((0, test_types_js_1.present)((0, test_types_js_1.present)(byCost.groups).models).map(model => model.key), ['anthropic/opus', 'zai/glm']);
    const byTokens = (0, test_types_js_1.present)(H.mergeUsageSummaries([
        { hostId: 'a', summary: usagePayload({ sort: 'tokens' }) },
        { hostId: 'b', summary: { ...small, sort: 'tokens' } },
    ]));
    assert.deepEqual((0, test_types_js_1.present)((0, test_types_js_1.present)(byTokens.groups).models).map(model => model.key), ['zai/glm', 'anthropic/opus']);
});
...
test('sortHostSections puts self first, then labels alphabetically', () => {
    ...
    assert.deepEqual(H.sortHostSections(hosts).map((host) => property(host, 'hostId')), ['s', 'b', 'a', 'z']);
...
test('sortHostSections breaks label ties on id so the order never jitters', () => {
    ...
    assert.deepEqual(H.sortHostSections(hosts).map((host) => property(host, 'hostId')), ['a', 'b']);
...
test('filenameFromContentDisposition names the export download', () => {
    ...
    assert.equal(F('attachment; filename="2026-08-22T11-00-00-artfix01.html"', 'fb'), '2026-08-22T11-00-00-artfix01.html');
...
Those match. **The transport now accounts for essentially 100% of what remains**`;
    const html = renderMarkdown(marked, sample);
...
test('createMathExtensions renders LaTeX inline math with KaTeX', () => {
    ...
    const html = renderMarkdown(marked, inline);
...
test('createMathExtensions preserves plain currency and code blocks', () => {
    ...
    const currencyHtml = renderMarkdown(marked, currency);
...
test('thinkingLevelsFor trims OMP options to the model catalog entry', () => {
    ...
    assert.deepEqual(H.thinkingLevelsFor('omp', { thinking: ['low', 'high', 'max'] }), ['off', 'low', 'high', 'max', 'auto']);
    // No catalog entry (or an empty list): the full OMP vocabulary.
    assert.deepEqual(H.thinkingLevelsFor('omp', { thinking: null }), ['off', 'minimal', 'low', 'medium', 'high', 'xhigh', 'max', 'auto']);
    assert.deepEqual(H.thinkingLevelsFor('omp', null), ['off', 'minimal', 'low', 'medium', 'high', 'xhigh', 'max', 'auto']);
    // A catalog entry listing off can't produce duplicates.
    assert.deepEqual(H.thinkingLevelsFor('omp', { thinking: ['off', 'low'] }), ['off', 'low', 'auto']);
    // Pi keeps pi's fixed vocabulary; Prime's adds 'max' (and has no 'auto').
    assert.deepEqual(H.thinkingLevelsFor('pi', { thinking: ['low'] }), ['off', 'minimal', 'low', 'medium', 'high', 'xhigh']);
    assert.deepEqual(H.thinkingLevelsFor('prime', null), ['off', 'minimal', 'low', 'medium', 'high', 'xhigh', 'max']);
});
