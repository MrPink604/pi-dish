...
function writePageDir(home: string, name: string, body: string) {
...
interface BootedServer { base: string; port: number; child: ChildProcess; home: string }
...
async function json(response: Response): Promise<Record<string, unknown>> {
  ...
  try { return record(JSON.parse(text)); } catch { throw new Error(`not JSON (${response.status}): ${text.slice(0, 200)}`); }
}

let peer!: BootedServer;
let hub!: BootedServer;
let hubPub!: BootedServer;
let hubShareBase = '';
...
test('a session export downloads through the proxy, and only through it', async () => {
  ...
  const payload = Buffer.from(present(present(html.match(/id="session-data"[^>]*>([^<]+)</))[1]), 'base64').toString('utf8');
  ...
  assert.equal(records(artifactBody.artifacts).length, 0, 'exporting publishes nothing');
});
...
test('a share created through the proxy is mapped to the peer and re-addressed to the hub', async () => {
  ...
  const artifactBody = await json(await fetch(`${hub.base}/api/fleet-artifacts`));
  const entry = present(records(artifactBody.artifacts).find(artifact => artifact.token === shareToken));
...
test('a hostId no configured remote claims is a 404', async () => {
  ...
  const artifactBody = await json(await fetch(`${hub.base}/api/fleet-artifacts`));
  assert.equal(records(artifactBody.artifacts).some(artifact => artifact.token === 'unmappable-token'), false);
...
test('a comment on a fleet page files on the peer, where its agent reads it', async () => {
  ...
  assert.equal(record(comment.target).pageToken, pageToken);

  const peerComments = record(JSON.parse(fs.readFileSync(path.join(peer.home, '.pi', 'dish', 'comments.json'), 'utf8')));
  const commentId = String(comment.id), storedComment = record(peerComments[commentId]);
  ...
  const indexedComments = records(index.comments);
  ...
  const full = await json(await fetch(`${hub.base}/api/comments/get`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ sessionId: present(indexedComments[0]).sessionId, ids: [comment.id], pageToken }),
  }));
  assert.equal(present(records(full.comments)[0]).body, 'tighten this paragraph');
  ...
  const afterPatch = record(JSON.parse(fs.readFileSync(path.join(peer.home, '.pi', 'dish', 'comments.json'), 'utf8')));
  assert.equal(record(afterPatch[commentId]).body, 'tighten this paragraph, please');
...
test('revoking on the owner 404s the hub and prunes the mapping', async () => {
  ...
  const artifactBody = await json(await fetch(`${hub.base}/api/fleet-artifacts`));
  assert.equal(records(artifactBody.artifacts).some(artifact => artifact.token === pageToken), false, 'the dead mapping was pruned lazily');
});
...
test('a revoke proxied through the hub prunes the mapping directly', async () => {
  ...
  assert.equal(revoked.revoked, true);
  const artifactBody = await json(await fetch(`${hub.base}/api/fleet-artifacts`));
  assert.equal(records(artifactBody.artifacts).some(artifact => artifact.token === page.token), false);
  ...
  assert.equal(records(after.artifacts).some(artifact => artifact.token === shareToken), false);
...
test('the pages skill publishes locally and maps onto a hub with --via', async () => {
  ...
  const hubSettings = record(JSON.parse(fs.readFileSync(path.join(hub.home, '.pi', 'dish', 'settings.json'), 'utf8')));
  ...
  const run = (args: string[], extraEnv: NodeJS.ProcessEnv = {}) => new Promise<RunResult>(resolve => {
