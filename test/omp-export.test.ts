...
const {
  ...
  readOmpExportData,
}: typeof import('../lib/omp-export') = require('../lib/omp-export')
...
test('OMP export uses its native renderer, preserves the pre-header, and injects a live snapshot', async () => {
  ...
  try {
    ...
    assert.equal(record(data.header).id, 'omp-export-test');
    assert.deepEqual(data.entries.map((entry: unknown) => record(entry).type), ['title', 'session_init', 'message']);
