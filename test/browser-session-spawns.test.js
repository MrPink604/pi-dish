// Generated test/tool from test/browser-session-spawns.test.ts; edit that source and run npm run build:tests.
"use strict";
...
const settle = async () => { for (let i = 0; i < 20; i++)
    await Promise.resolve(); };
function fixture() {
    const reads = [];
    const events = [], drafts = new Map();
    let current = null, available = true;
    ...
    return { controller, reads, events, drafts, select: (key) => { current = key; }, present: (value) => { available = value; } };
}
const host = { hostId: 'a', base: '/a', token: 'fixture' };
test('spawn wire decoders reject untyped ids and narrow readiness', () => {
    for (const wire of [null, {}, { spawnId: 4 }, { spawnId: '' }])
        assert.throws(() => decodeSpawnId(wire), /Failed/);
...
test('submitted spawns copy their host, target and draft before acceptance and keep replacement views', async () => {
    ...
    const target = { type: 'tmux', socket: '/sock', tmuxSession: 'original' };
    let owned = true;
    const input = { host: endpoint, cwd: '/old', draft: 'old draft', target, ownsView: () => owned };
    const submit = f.controller.submit(input);
    endpoint.base = '/changed';
    endpoint.token = 'changed';
    target.tmuxSession = 'changed';
    input.draft = 'new draft';
    owned = false;
    (0, test_types_js_1.present)(f.reads[0]).resolve(response({ spawnId: 'op' }));
    const key = await submit;
    const statusRead = (0, test_types_js_1.present)(f.reads[1]), kickoff = (0, test_types_js_1.present)(f.reads[0]);
    ...
    if (typeof kickoff.init?.body !== 'string')
        throw new Error('Spawn kickoff lacks JSON body');
    assert.equal(plain(JSON.parse(kickoff.init.body)).target.tmuxSession, 'original');
    assert.equal(f.drafts.get(key), 'old draft');
    assert.ok(!f.events.some(event => event[0] === 'show'));
    statusRead.resolve(response({ status: 'ready', sessionId: 'session' }));
    await settle();
...
test('identical operation ids from different hosts remain distinct and readiness selects only the current one', async () => {
    ...
    (0, test_types_js_1.present)(f.reads[0]).resolve(response({ spawnId: 'same' }));
    const a = await first;
    const second = f.controller.submit({ host: { hostId: 'b', base: '/b' }, ownsView: () => true });
    (0, test_types_js_1.present)(f.reads[2]).resolve(response({ spawnId: 'same' }));
    const b = await second;
    assert.notEqual(a, b);
    assert.equal(a, sessionSpawnKey('a', 'same'));
    assert.equal(f.controller.entries().length, 2);
    (0, test_types_js_1.present)(f.reads[1]).resolve(response({ status: 'ready', sessionId: 'same-session' }));
    await settle();
    assert.ok(!f.events.some(event => event[0] === 'select'));
    (0, test_types_js_1.present)(f.reads[3]).resolve(response({ status: 'ready', sessionId: 'same-session' }));
    await settle();
...
test('a failed background spawn discards only its composer while the current failure preserves its draft', async () => {
    const f = fixture();
    const first = f.controller.submit({ host, ownsView: () => true });
    (0, test_types_js_1.present)(f.reads[0]).resolve(response({ spawnId: 'old' }));
    const old = await first;
    const second = f.controller.submit({ host, ownsView: () => true });
    (0, test_types_js_1.present)(f.reads[2]).resolve(response({ spawnId: 'new' }));
    const current = await second;
    (0, test_types_js_1.present)(f.reads[1]).resolve(response({ status: 'error', error: 'old failure' }));
    await settle();
    assert.ok(f.events.some(event => event[0] === 'discard' && event[1] === old));
    assert.ok(!f.events.some(event => event[0] === 'failure'));
    (0, test_types_js_1.present)(f.reads[3]).resolve(response({ status: 'error', error: 'current failure' }));
    await settle();
