// Generated test/tool from test/browser-display.test.ts; edit that source and run npm run build:tests.
"use strict";
...
test('display payloads narrow theme tokens, filters and supported metadata modes', () => {
    ...
    assert.equal(responseMode('unknown'), 'compact');
    assert.equal(responseMode('performance-cost'), 'performance-cost');
});
test('pre-paint output tolerates unavailable storage and uses only custom-property strings', () => {
    ...
    assert.equal(document.documentElement.dataset.theme, 'custom');
    assert.equal(properties['--accent'], '#123456');
    assert.equal(properties.color, undefined);
    assert.equal(properties['--bad'], undefined);
    assert.equal(clampSidebarWidth(1000, 1200), 600);
    assert.equal(clampSidebarWidth(20, 300), 220);
    assert.equal(clampTerminalHeight(1000, 500), 400);
    assert.equal(clampTerminalHeight(20, 500), 140);
});
