 ...
 * Run with: npm test
 */
import { nativeId as checkedNativeId, present, record, records, type TestRecord } from './test-types.js';
...
const OMP_ROUTE_ID = encodeSessionKey('omp', checkedNativeId(OMP_SESSION_ID));
...
const OMP_BLOB_ROUTE_ID = encodeSessionKey('omp', checkedNativeId(OMP_BLOB_SESSION_ID));
...
const tmux: typeof import('../lib/tmux') = require('../lib/tmux')
const harnessPricing: typeof import('../lib/harness-pricing') = require('../lib/harness-pricing')
const { invalidateRegistryCache }: typeof import('../lib/bridge-session') = require('../lib/bridge-session')
const { processIdentity, processIdentityAlive }: typeof import('../lib/process-identity') = require('../lib/process-identity')
const sessionProvenance: typeof import('../lib/session-provenance') = require('../lib/session-provenance')

function responseObject(value: unknown): Record<string, unknown> {
  return record(value);
}

function responseArray(value: unknown): Record<string, unknown>[] {
...
const getArray = async (requestPath: string) => {
  const response = await fetch(base + requestPath);
  return { status: response.status, body: responseArray(await response.json()) };
};

const post = async (requestPath: string, body?: unknown) => {
  const response = await fetch(base + requestPath, {
...
const put = async (requestPath: string, body?: unknown) => {
  const response = await fetch(base + requestPath, {
...
const patch = async (requestPath: string, body?: unknown) => {
  const response = await fetch(base + requestPath, {
...
const del = async (requestPath: string, body?: unknown) => {
...
const rawGet = (requestPath: string, headers: http.OutgoingHttpHeaders = {}) => new Promise<RawResponse>((resolve, reject) => {
...
interface UsageRowView extends TestRecord {
  id: string; key: string; ref: string; day: string;
  calls: number; unpricedCalls: number; priced: boolean; cost: number;
  costs: Record<string, number>;
  costUnavailable: Record<string, number>;
  models: UsageRowView[];
}
interface UsageSummaryView extends TestRecord {
  range: string; sort: string; models: string[]; unpricedModelCalls: number;
  totals: UsageRowView;
  groups: TestRecord & { models: UsageRowView[]; workspaces: UsageRowView[]; sessions: UsageRowView[] };
  daily: UsageRowView[];
  headlineCosts: Record<string, number>;
  headlineCostsByBucket: Record<string, Record<string, number>>;
  headlineCostUnavailable: Record<string, number>;
}


function usageRow(value: unknown): UsageRowView {
...
function usageSummary(value: unknown): UsageSummaryView {
  const summary = record(value), groups = record(summary.groups);
  return {
    ...summary,
    ...
    headlineCostsByBucket: Object.fromEntries(
      Object.entries(record(summary.headlineCostsByBucket)).map(([key, row]) => [key, numberRecord(row)]),
    ),
...
test('GET /api/sessions lists the fixture session with derived metadata', async () => {
  ...
  const sess = present(records(body.previous).find((s) => s.id === SESSION_ID))
...
test('GET /api/sessions client view omits server-only routing metadata', async () => {
  ...
  const nested = present(records(body.previous).find((s) => s.id === NESTED_SESSION_ID))
...
test('malformed historical metadata falls back before client projection', async () => {
  ...
  try {
    ...
    assert.ok(records(full.body.previous).some((row) => row.id === badId), 'fixture reaches the projection');
    const client = await get('/api/sessions?view=client');
    assert.equal(client.status, 200);
    assert.ok(records(client.body.previous).some((row) => row.id === SESSION_ID));
    assert.equal(records(client.body.previous).find((row) => row.id === badId)?.name, badId.slice(0, 8));
    assert.equal(records(client.body.previous).find((row) => row.id === badId)?.model, 'unknown');
...
test('encoded alternative-harness routes never fall back to a partial native id', async () => {
  ...
  const partial = await get(`/api/sessions/${encodeURIComponent(encodeSessionKey('omp', checkedNativeId('omp-prefix')))}/stats`);
...
test('OMP sibling-directory subagents are addressable and appear as session relations', async () => {
  ...
  const childRouteId = encodeSessionKey('omp', checkedNativeId(childNativeId));
  ...
  const child = present(records(listed.body.previous).find((session) => session.id === childRouteId))
  ...
  assert.ok(records(childRelated.body.relations).some((relation) => relation.kind === 'parent' && relation.source === 'omp-subsession-layout' && record(relation.session).id === OMP_ROUTE_ID));
  const parentRelated = await get(`/api/sessions/${encodeURIComponent(OMP_ROUTE_ID)}/related`);
  assert.ok(records(parentRelated.body.relations).some((relation) => relation.kind === 'child' && relation.source === 'omp-subsession-layout' && record(relation.session).id === childRouteId));

  const messages = await get(`/api/sessions/${encodeURIComponent(childRouteId)}/messages?limit=10`);
  assert.equal(messages.status, 200);
  const firstMessage = present(records(messages.body.messages)[0]);
  assert.equal(present(records(firstMessage.content)[0]).text, 'Inspect the OMP code');
});
...
test('live subagent sessions surface under their live parent on the Active tab', async () => {
  ...
  const runningRoute = encodeSessionKey('omp', checkedNativeId(runningId));
  const disposedRoute = encodeSessionKey('omp', checkedNativeId(disposedId));
  const subagent = (id: string, tail: unknown[] = []) => [
  ...
  try {
    ...
    const children = records(activeOnly.body.children).map((session) => session.id);
    ...
    const running = present(records(activeOnly.body.children).find((session) => session.id === runningRoute))
    assert.equal(running.subagentLive, true);
    assert.equal(running.isActive, false, 'its parent owns it — pi-dish cannot drive it');
    assert.equal(record(running.capabilities).prompt, false);
    assert.equal(record(running.capabilities).resume, false, 'resuming would append to a file the parent holds');
    ...
    const fullRunning = present(records(full.body.previous).find((session) => session.id === runningRoute))
    assert.equal(fullRunning.subagentLive, true, 'the All tab marks the same row live');
    assert.equal(record(fullRunning.capabilities).resume, false);
    const fullDisposed = present(records(full.body.previous).find((session) => session.id === disposedRoute))
    assert.ok(fullDisposed, 'the disposed subagent stays in history');
    assert.ok(!fullDisposed.subagentLive);
    assert.equal(record(fullDisposed.capabilities).resume, true);
...
test('a wrapper bridge claiming a session hides the stock pi bridge riding in the same process', async () => {
  ...
  const ompRouteId = encodeSessionKey('omp', checkedNativeId(nativeId));
  ...
  try {
    ...
    assert.ok(records(active.body.active).some((s) => s.harnessId === 'pi' && s.nativeSessionId === nativeId),
    ...
    assert.ok(!records(active.body.active).some((s) => s.nativeSessionId === nativeId),
    ...
    active = await get('/api/sessions?active=1');
    const claimed = records(active.body.active).filter((s) => s.nativeSessionId === nativeId);
...
test('alternative harnesses do not decode Pi workspace directory names as cwd', async () => {
  ...
  const routeId = encodeSessionKey('omp', checkedNativeId(nativeId));
  const listed = present(records(sessions.body.previous).find((entry) => entry.id === routeId))
...
test('nested generic sessions use the core header id and remain fully addressable', async () => {
  const listed = await get('/api/sessions');
  const nested = present(records(listed.body.previous).find((s) => s.id === NESTED_SESSION_ID))
  ...
  const filteredChild = present(records(filtered.body.previous).find((s) => s.id === NESTED_SESSION_ID))
  ...
  assert.equal(messages.status, 200);
  const firstMessage = present(records(messages.body.messages)[0]);
  assert.equal(present(records(firstMessage.content)[0]).text, 'nested peer question');

  const related = await get(`/api/sessions/${NESTED_SESSION_ID}/related`);
  assert.equal(related.status, 200);
  const parent = present(records(related.body.relations).find((r) => r.kind === 'parent' && r.source === 'pi-session-header'))
  assert.equal(record(parent.session).id, SESSION_ID);

  const inverse = await get(`/api/sessions/${SESSION_ID}/related`);
  const child = present(records(inverse.body.relations).find((r) => r.kind === 'child' && r.source === 'pi-session-header'))
  assert.equal(record(child.session).id, NESTED_SESSION_ID);
});
...
test('stale native parent paths do not attach by basename alone', async () => {
  ...
  try {
    const listed = await get('/api/sessions');
    const stale = present(records(listed.body.previous).find((s) => s.id === id))
    assert.ok(stale);
...
test('alien JSONL basenames are skipped without breaking sessions or usage APIs', async () => {
  ...
  try {
    ...
    assert.equal(records(listed.body.previous).some((session) => session.sessionFile === alien), false);
...
test('a newly ambiguous nested header id invalidates route lookup', async () => {
  ...
  try {
    ...
    const listed = await get('/api/sessions');
    assert.equal(records(listed.body.previous).some((s) => s.id === NESTED_SESSION_ID), false);
  } finally {
...
test('pi-dish launch provenance adds neutral related-session navigation', async () => {
  ...
  const listed = await get('/api/sessions');
  const launched = present(records(listed.body.previous).find((s) => s.id === REAL_CWD_ID))
  ...
  assert.ok(records(source.body.relations).some((r) => r.kind === 'startedHere' && record(r.session).id === REAL_CWD_ID));
  const child = await get(`/api/sessions/${REAL_CWD_ID}/related`);
  assert.ok(records(child.body.relations).some((r) => r.kind === 'startedFrom' && record(r.session).id === SESSION_ID));
});
...
test('lineage endpoint assembles the recursive family tree', async () => {
  ...
  const writeRow = (file: string, id: string, parentSession: unknown) => fs.writeFileSync(file, [
  ...
  try {
    ...
    assert.equal(record(res.body.session).id, rootId);
    assert.equal(res.body.members, 3);
    assert.equal(res.body.truncated, false);
    const tree = record(res.body.tree);
    assert.equal(record(tree.session).id, rootId, 'tree roots at the top ancestor');
    assert.equal(tree.edge, null);
    const children = records(tree.children);
    assert.equal(children.length, 1);
    const child = present(children[0]), grandchildren = records(child.children);
    assert.equal(record(child.session).id, childId);
    assert.deepEqual(child.edge, { kind: 'child', source: 'pi-session-header' });
    assert.equal(record(present(grandchildren[0]).session).id, gcId, 'grandchild nests under the child');
    assert.equal(record(child.session).turnInProgress, false, 'summaries carry busy state for the signal UI');
    assert.ok(record(child.session).capabilities, 'summaries carry capability advice');
    assert.equal(record(record(child.session).capabilities).prompt, false, 'an inactive relative cannot be prompted');
    assert.equal(typeof record(record(child.session).capabilities).resume, 'boolean');

    const fromGc = await get(`/api/sessions/${gcId}/lineage`);
    const fromGcTree = record(fromGc.body.tree);
    assert.equal(record(fromGcTree.session).id, rootId, 'a descendant sees the same root');
    assert.equal(record(fromGc.body.session).id, gcId);
...
test('lineage terminates on cyclic parent headers', async () => {
  ...
  const writeRow = (file: string, id: string, parentSession: unknown) => fs.writeFileSync(file, JSON.stringify(
...
test('lineage marks pi-dish launch edges and respects the node cap', async () => {
  ...
  try {
    ...
    const fullTree = record(full.body.tree);
    const childNode = present(records(fullTree.children).find(node => record(node.session).id === childId));
    assert.ok(childNode, 'launched session joins the tree below its source');
    const childEdge = record(childNode.edge);
...
test('prime RLM subagents join the catalog and the lineage tree', async () => {
  ...
  const writeRow = (file: string, id: string, parentSession: unknown, rlmDepth: unknown) => {
  ...
  const rootRoute = encodeSessionKey('prime', checkedNativeId(rootNative));
  const childRoute = encodeSessionKey('prime', checkedNativeId(childNative));
  const gcRoute = encodeSessionKey('prime', checkedNativeId(gcNative));
  try {
    const listed = await get('/api/sessions');
    const child = present(records(listed.body.previous).find((session) => session.id === childRoute))
    assert.ok(child, 'prime subagent is listed from the artifacts tree');
    assert.equal(child.parentId, rootRoute);
    const gc = present(records(listed.body.previous).find((session) => session.id === gcRoute))
    ...
    const lineageTree = record(lineage.body.tree);
    const lineageChild = present(records(lineageTree.children)[0]);
    const lineageGrandchild = present(records(lineageChild.children)[0]);
    assert.equal(record(lineageTree.session).id, rootRoute);
    assert.equal(record(lineageChild.session).id, childRoute);
    assert.equal(record(lineageGrandchild.session).id, gcRoute);
    assert.equal(lineage.body.members, 3);

    const childRelated = await get(`/api/sessions/${encodeURIComponent(childRoute)}/related`);
    assert.ok(records(childRelated.body.relations).some((relation) => relation.kind === 'parent' && record(relation.session).id === rootRoute),
...
test('live prime subagents surface under their live parent via the display entry', async () => {
  ...
  const runningRoute = encodeSessionKey('prime', checkedNativeId(runningNative));
  const doneRoute = encodeSessionKey('prime', checkedNativeId(doneNative));
  const writeRow = (file: string, id: string, parentSession: unknown) => {
    fs.mkdirSync(path.dirname(file), { recursive: true });
    fs.writeFileSync(file, [
  ...
  const display = (dir: string, file: string, id: string, status: string) => fs.writeFileSync(path.join(dir, 'rlm-subagent.json'), JSON.stringify({
  ...
  try {
    ...
    const running = present(records(activeOnly.body.children).find((session) => session.id === runningRoute))
    assert.equal(running.subagentLive, true);
    assert.equal(running.parentId, encodeSessionKey('prime', checkedNativeId(rootNative)));

    const related = await get(`/api/sessions/${encodeURIComponent(encodeSessionKey('prime', checkedNativeId(rootNative)))}/related`);
    const runningRelation = present(records(related.body.relations).find((relation) => record(relation.session).id === runningRoute))
    assert.equal(record(runningRelation.session).subagentLive, true, 'relation summaries carry the live-subagent flag');
  } finally {
    fs.rmSync(registryPath, { force: true });
    fs.rmSync(primeDir, { recursive: true, force: true });
    invalidateRegistryCache();
...
test('cwd falls back to the dir-name decode only when the decoded path exists', async () => {
  ...
  const sess = present(records(body.previous).find((s) => s.id === NO_CWD_ID))
...
test('GET /api/sessions?q= filters on message content', async () => {
  ...
  const sess = present(records(hit.body.previous).find((s) => s.id === SESSION_ID))
  assert.ok(sess);
  ...
  assert.ok(!records(miss.body.previous).some((s) => s.id === SESSION_ID));

  // …metadata matches (name/cwd/model/id) don't need one.
  const byName = await get('/api/sessions?q=hello');
  const metaMatch = present(records(byName.body.previous).find((s) => s.id === SESSION_ID))
...
test('GET /api/sessions?active=1&q= content-matches registered sessions', async () => {
  ...
  try {
    ...
    const sess = present(records(hit.body.active).find((s) => s.id === ID))
    ...
    assert.ok(!records(miss.body.active).some((s) => s.id === ID), 'non-matching query filters it out');
  } finally {
...
test('GET /api/sessions?q= speaks the filter grammar: negation, fields, dates', async () => {
  ...
  assert.ok(records(negContent.body.previous).some((s) => s.id === SESSION_ID), 'content-only word must not exclude via negation');
  const negName = await get(`/api/sessions?q=${encodeURIComponent('-name:hello')}`);
  assert.ok(!records(negName.body.previous).some((s) => s.id === SESSION_ID), 'name negation excludes');
  ...
  assert.ok(records(byCwd.body.previous).some((s) => s.id === SESSION_ID));
  const byWrongField = await get(`/api/sessions?q=${encodeURIComponent('name:proj')}`);
  assert.ok(!records(byWrongField.body.previous).some((s) => s.id === SESSION_ID));
  ...
  assert.ok(records(inRange.body.previous).some((s) => s.id === DATED_ID));
  const tooOld = await get(`/api/sessions?q=${encodeURIComponent('dated since:2026-07-01')}`);
  assert.ok(!records(tooOld.body.previous).some((s) => s.id === DATED_ID));
  ...
  const sess = present(records(combo.body.previous).find((s) => s.id === SESSION_ID))
...
{
  const write = (id: string, entries: unknown[], mtime: string | number | Date) => {
    const file = path.join(sessionDir, `${id}.jsonl`);
...
test('GET /api/sessions?q= ranks by relevance, recency only as tiebreaker', async () => {
  ...
  const ids = records(body.previous).map(session => session.id).filter(id => id === RANK_NAME_ID || id === RANK_CONTENT_ID);
  assert.deepEqual(ids, [RANK_NAME_ID, RANK_CONTENT_ID],
    'the name match outranks the newer session that only repeats the word');
  const named = present(records(body.previous).find((s) => s.id === RANK_NAME_ID))
  const shouted = present(records(body.previous).find((s) => s.id === RANK_CONTENT_ID))
  assert.ok(Number(named.searchScore) > Number(shouted.searchScore),
  ...
  const metaTimes = records(meta.body.previous).map(session => new Date(text(session.lastActivity) || 0).getTime());
  assert.ok(metaTimes.every((timestamp, index) => index === 0 || timestamp <= present(metaTimes[index - 1])), 'recency order');
  assert.equal(present(records(meta.body.previous)[0]).searchScore, undefined);
});
...
test('GET /api/search returns flat results with multi-snippets and match counts', async () => {
  ...
  const sess = present(records(body.results).find((s) => s.id === SESSION_ID));
  ...
  assert.ok(snippets.length >= 1 && snippets.every(snippet => snippet.includes('alpha')),
  ...
  const metaSess = present(records(byCwd.body.results).find((s) => s.id === SESSION_ID))
  assert.ok(metaSess);
  ...
  assert.ok(!records(neg.body.results).some((s) => s.id === SESSION_ID));
  const liveOnly = await get(`/api/search?q=${encodeURIComponent('alpha is:active')}`);
  assert.ok(!records(liveOnly.body.results).some((s) => s.id === SESSION_ID), 'historical session excluded by is:active');
  ...
  const rankIds = records(ranked.body.results).map(session => session.id);
  assert.deepEqual(rankIds, [RANK_NAME_ID, RANK_CONTENT_ID]);
  const rankedResults = records(ranked.body.results);
  assert.ok(Number(present(rankedResults[0]).searchScore) > Number(present(rankedResults[1]).searchScore),
    'result objects carry the score they were ranked by');
  assert.ok(present(rankedResults[0]).matchCount === 1 && Number(present(rankedResults[1]).matchCount) > 50,
    `occurrence counts stay independent of the ranking ${JSON.stringify(rankedResults.map(result => [result.id, result.matchCount]))}`);

  // Empty query browses everything, recency-first.
  const all = await get('/api/search?q=');
  assert.ok(records(all.body.results).length >= 3);
  const times = records(all.body.results).map(session => new Date(text(session.lastActivity) || 0).getTime());
  assert.ok(times.every((timestamp, index) => index === 0 || timestamp <= present(times[index - 1])), 'recency order');
});
...
test('GET /api/search applies saved scope before the 100-result cap', async (t) => {
  ...
  const writeCapSession = (id: string, cwd: string, timestamp: string | number | Date) => {
  ...
  assert.equal(records(unscoped.body.results).length, 100);
  assert.ok(!records(unscoped.body.results).some((s) => s.id === targetId),
  ...
  const kept = records(unscoped.body.results);
  assert.ok(kept.every((s) => Number(s.matchCount) === 1
      && Array.isArray(s.snippets) && s.snippets.map(text).some((snippet) => snippet.includes('scope-cap-boundary'))),
    'kept rows keep their occurrence count and snippet');
  const keptTimes = kept.map((s) => new Date(text(s.lastActivity) || 0).getTime());
  assert.ok(keptTimes.every((timestamp, index) => index === 0 || timestamp <= present(keptTimes[index - 1])),
    'recency order survives the capped slice');
  ...
  assert.deepEqual(records(scoped.body.results).map((s) => s.id), [targetId]);
...
test('GET /messages returns the tail with indexes', async () => {
  ...
  assert.deepEqual(records(body.messages).map((m) => m.index), [0, 1, 2, 3, 4]);
  assert.deepEqual(records(body.messages).map((m) => m.role), ['user', 'assistant', 'toolResult', 'user', 'assistant']);
  // Entry id + generation stats ride the message (share deep links, tok/s).
  const last = present(records(body.messages)[4]);
...
test('GET /messages honors limit / before / after cursors', async () => {
  ...
  assert.deepEqual(records(bogus.body.messages).map((m) => m.index), [3, 4]);
  assert.ok(records(bogus.body.messages).every((m) => Number.isFinite(m.index)), 'indexes stay numeric');
});

test('GET /messages moves historical image bytes to a cacheable resource', async () => {
  const { body } = await get(`/api/sessions/${SESSION_ID}/messages`);
  const messages = records(body.messages);
  const thirdMessage = present(messages[2]);
  const image = present(records(thirdMessage.content).find(block => block.type === 'image'));
  assert.ok(image.url, 'message payload carries an image resource URL');
  assert.match(text(image.url), /\/messages\/img00001\/images\/1$/,
...
test('OMP blob-store image refs resolve to the stored bytes', async () => {
  ...
  const images = records(body.messages).flatMap(message => records(message.content))
  ...
  const ok = await fetch(base + text(present(images[0]).url));
  ...
  const missing = await fetch(base + text(present(images[1]).url));
...
test('historical image resource stays stable after active branch navigation', async () => {
  ...
  const image = present(records(before.body.messages).flatMap(message => records(message.content))
  ...
  assert.equal(records(after.body.messages).some(message => records(message.content).some(block => block.type === 'image')), false,
...
test('GET /search requires all tokens to match within one message', async () => {
  ...
  assert.deepEqual(records(both.body.matches).map((m) => m.index), [3]);
...
test('GET /search mode=any returns relevant messages for session-wide advanced matches', async () => {
  ...
  assert.ok(records(advanced.body.results).some((s) => s.id === SESSION_ID),
...
test('GET /api/dirs fuzzy-finds directories under $HOME', async () => {
  ...
  assert.equal(status, 200);
  const hit = present(body.find(directory => directory.path === realCwd));
...
test('GET /files fuzzy-searches the session cwd', async () => {
  ...
  assert.ok(records(body.files).some((f) => f.path === 'src/main.js'));
});
...
test('GET /files completes absolute paths outside the session cwd', async () => {
  ...
  assert.equal(status, 200);
  const dirHit = present(records(body.files).find((f) => f.path === path.join(tmpHome, 'scratch')))
  ...
  const fileHit = present(records(body.files).find((f) => f.path === path.join(tmpHome, 'scratch', 'notes.md')))
...
test('GET /file returns image metadata while a resource route serves the bytes', async () => {
  ...
  assert.equal(status, 200);
  assert.equal(record(body.image).mimeType, 'image/png');
  assert.ok(record(body.image).url, 'viewer JSON carries a resource URL');
  assert.equal(record(body.image).data, undefined, 'viewer JSON does not carry base64 bytes');

  const res = await fetch(base + text(record(body.image).url));
  assert.equal(res.status, 200);
  assert.equal(res.headers.get('content-type'), 'image/png');
  assert.deepEqual(Buffer.from(await res.arrayBuffer()), TINY_PNG);
});
...
test('GET /diff aggregates uncommitted changes across repos under the session cwd', async (t) => {
  ...
  const entry = present(records(body.repos).find((r) => r.path === 'repo-x'))
  assert.ok(entry, 'repo under the cwd is discovered');
  assert.equal(entry.branch, 'main');
  const byPath = new Map(records(entry.files).map(file => [text(file.path), file]));
  assert.equal(record(byPath.get('a.txt')).status, 'M');
  assert.ok(text(record(byPath.get('a.txt')).patch).includes('+two'));
  assert.equal(record(byPath.get('new.txt')).status, '?');
  assert.ok(text(record(byPath.get('new.txt')).patch).includes('+fresh'), 'untracked files get synthesized patches');
});
...
test('large diff summaries defer collapsed patches and serve one on demand', async (t) => {
  ...
  const entry = present(records(body.repos).find((r) => r.path === 'repo-big'))
  assert.ok(entry, 'large repo is present in the summary');
  const entryFiles = records(entry.files);
  assert.equal(entryFiles.length, 7);
  assert.ok(entryFiles.every(file => file.patch === undefined && file.patchDeferred === true),
...
test('file-viewer pages render as standalone read-only documents', async () => {
  ...
  const imageMatch = present(imageHtml.match(/<img[^>]+src="([^"]+)"/));
  const imageSrc = present(imageMatch[1]).replace(/&amp;/g, '&');
...
test('POST /api/pages with an invalid explicit sessionId cannot mutate an existing page association', async () => {
  ...
  const list = await getArray('/api/pages');
  const preserved = present(list.body.find(page => page.token === published.body.token));
  assert.ok(preserved);
...
test('comments support unpaginated indexing, selected reads, and acknowledgment', async () => {
  ...
  const selectedComments = records(selected.body.comments);
  assert.deepEqual(selectedComments.map(comment => comment.id), [second.body.id, first.body.id],
    'the agent can fetch any inferred group in its requested order without acking earlier comments');
  assert.equal(record(record(present(selectedComments[1]).target).anchor).prefix, '# ', 'selected fetch returns the full anchor');
  ...
  assert.deepEqual(records(indexAfterAck.body.comments).map((c) => c.id), [second.body.id]);
...
test('open comments are editable and deletable, acknowledged ones are not', async () => {
  ...
  assert.equal(present(records(readBack.body.comments)[0]).body, 'Second wording.');
  ...
  assert.ok(!records(afterDelete.body.comments).some((c) => c.id === disposable.body.id),
...
test('published-page comments inherit the page session and artifact identity', async () => {
  ...
  assert.equal(record(created.body.target).root, artifact);

  const selected = await post('/api/comments/get', { sessionId: REAL_CWD_ID, ids: [created.body.id] });
  assert.deepEqual(records(selected.body.comments).map((c) => c.id), [created.body.id]);
  ...
  const pageComments = records(byToken.body.comments);
  assert.deepEqual(pageComments.map(comment => comment.id), [created.body.id]);
  assert.equal(present(pageComments[0]).sessionId, REAL_CWD_ID);
...
test('encoded Pi aliases share the canonical raw Pi comment identity', async () => {
  const alias = encodeSessionKey('pi', checkedNativeId(REAL_CWD_ID));
  const created = await post('/api/comments', {
...
test('publishing without a sessionId infers the most specific containing cwd', async () => {
  ...
  try {
    ...
    assert.ok(inner.body.some(page => page.token === body.token),
...
test('inferred alternative-harness pages and comments keep the encoded harness identity', async () => {
  ...
  fs.writeFileSync(path.join(registryDir, `${OMP_SESSION_ID}.json`), JSON.stringify({
    ...
    startTime: present(processIdentity(process.pid)).startTime,
...
test('the session list forwards the registry compacting flag', async () => {
  ...
  try {
    const { body } = await get('/api/sessions');
    const sess = present(records(body.active).find((s) => s.id === ID))
...
test('OMP /compact routes through the advertised bridge operation and rejects overlap', async () => {
  ...
  await new Promise<void>((resolve, reject) => { bridge.once('error', reject); bridge.listen(socketPath, resolve); });
...
test('OMP command discovery includes pane-backed host commands and API aliases stay capability-first', async () => {
  ...
  try {
    ...
    tmux.paneExists = async (socket: string, paneId: string) => socket === '/fake/owned-omp.sock' && paneId === '%88';
    tmux.sendKeys = async (socket: string, paneId: string, keyText: string) => { injected.push({ socket, paneId, text: keyText }); };
    ...
    assert.equal(present(commands.body.find(command => command.name === 'shake')).args, '[images]');
    assert.equal(present(commands.body.find(command => command.name === 'guided-goal')).args, '<rough objective>');
    ...
    const send = (message: string) => post(`/api/sessions/${encodeURIComponent(routeId)}/command`, { message });
    ...
    invalidateRegistryCache();
    tmux.paneExists = async (socket: string, paneId: string) => socket === '/fake/stamped-omp.sock' && paneId === '%77';
...
test('PI_DISH_URL served to spawned agents always answers', async () => {
  ...
  let out = '';
  try {
    await new Promise<void>(resolve => child.stdout.on('data', function onChunk(chunk: Buffer) {
      out += chunk.toString();
...
test('PUT /api/models/enabled preserves a concurrent pi settings write', async (t) => {
  ...
  const createRequire: typeof import('node:module').createRequire = require('node:module').createRequire;
  const sdkRequire = createRequire(path.join(__dirname, '..',
    'node_modules', '@earendil-works', 'pi-coding-agent', 'package.json'));
  const lockfile: typeof import('proper-lockfile') = sdkRequire('proper-lockfile');
  const lockSync = lockfile.lockSync;
  let release: ReturnType<typeof lockSync> | null = lockSync(settingsFile, { realpath: false });
  const snapshot = JSON.parse(fs.readFileSync(settingsFile, 'utf8'));
  t.mock.method(lockfile, 'lockSync', function (file: string, options: Parameters<typeof lockSync>[1]) {
    try {
      return lockSync(file, options);
    } catch (error) {
      if (file === settingsFile && release && record(error).code === 'ELOCKED') {
...
test('GET /stats aggregates tokens, cost, and message counts from the JSONL', async () => {
  ...
  assert.equal(record(body.contextUsage).tokens, 1234);
...
test('usage summary preserves cost availability through every grouping and filter', async () => {
  ...
  const filterUrl = (models: string, days = 'all', sort = 'cost') =>
  ...
  try {
    ...
    assert.equal(present(free.groups.models.find(model => model.key === 'audit/free')).priced, true,
    ...
    const workspace = present(all.groups.workspaces.find(row => row.key === '/workspace/usage'));
    const session = present(all.groups.sessions.find(row => row.id === usageId));
    ...
    const mixed = present(all.groups.models.find(model => model.key === 'audit/mixed'));
    ...
    const todayEntry = present(usageSummary((await get(filterUrl(refs.join(','), '7'))).body).daily.at(-1))
    assert.ok(Math.abs(todayEntry.costs.total - 1.7) < 1e-9);
    const mixedDay = present(todayEntry.models.find(model => model.ref === 'audit/mixed'));
    assert.equal(mixedDay.cost, 0.2);
    assert.equal(mixedDay.costUnavailable.total, 1);
    assert.ok(todayEntry.models.some(model => model.ref === 'audit/paid' && model.cost === 1),
    ...
    assert.ok(thirty.daily.some(day => day.models.some(model => model.ref === 'audit/old-paid' && model.cost === 2)),
    ...
    const modelPos = (body: ReturnType<typeof usageSummary>, ref: string) =>
      body.groups.models.findIndex(model => model.key === ref);
    ...
    assert.ok(paid.groups.models.some(model => model.key === 'audit/dateless-paid'),
      'facet list keeps deselected models');
    const paidSession = present(paid.groups.sessions.find(row => row.id === usageId));
    assert.ok(paidSession.calls === 1 && paidSession.costs.total === 1,
      'session group carries only the filtered model usage');
    const wsPaid = present(paid.groups.workspaces.find(row => row.key === '/workspace/usage'));
    assert.equal(wsPaid.calls, 1, 'workspace group carries only filtered usage');
    assert.ok(paid.daily.every(day => day.models.every(model => model.ref === 'audit/paid')),
...
test('usage limits come from the harness CLI, sanitized and failure-tolerant', async () => {
  ...
  const omp = present(records(body.harnesses).find((h) => h.harness === 'omp'))
  ...
  assert.equal(record((await get('/api/host')).body.capabilities).usageLimits, true);
});
...
test('cache lifetimes report the learner window with precedence and gates', async () => {
  ...
  try {
    ...
    assert.equal(status, 200);
    assert.ok(Number.isFinite(body.generatedAt));
    const rows = records(body.identities);
    const openai = present(rows.find(row => row.model === 'gpt-report'));
    assert.equal(openai.source, 'builtin', 'two probes learn nothing; the ~10m estimate stands');
    assert.equal(record(openai.effective).retention, '~10m');
    assert.equal(record(present(records(openai.gates)[0])).id, 'support');
    assert.deepEqual(record(openai.probes).total, 2);
    assert.ok(Array.isArray(openai.points) && openai.points.length === 2, 'probes plotted as [gapMs, hit] pairs');
    const mystery = present(rows.find(row => row.model === 'mystery'));
    assert.equal(mystery.source, 'none');
    assert.equal(mystery.effective, null);
    assert.equal(record((await get('/api/host')).body.capabilities).cacheLifetimes, true);
  } finally {
...
test('an unknown-provider cache anchor never reaches a client projection', async () => {
  ...
  assert.equal(status, 200);
  assert.equal(record(body.session).cacheExpiry, null, 'no learned window, no projection');
  const listed = [...records((await get('/api/sessions')).body.previous)].find(session => session.id === id);
  assert.equal(present(listed).cacheExpiry, null);
  assert.ok(records(body.messages).every(message => message.cacheExpiry === undefined), 'per-message projection omitted too');
});
...
test('usage limits degrade to an error entry when the harness command fails', async () => {
  ...
  try {
    const { status, body } = await get('/api/usage-limits');
    assert.equal(status, 200);
    assert.equal(records(body.harnesses).length, 0,
      'an uninstalled harness is skipped, not an error — the capability check agrees');
    assert.equal('usageLimits' in record((await get('/api/host')).body.capabilities), false);
  } finally {
...
function withSttSettings(block: unknown) {
...
const postAudio = async (requestPath: string, contentType: string, body: string | Buffer) => {
...
test('speech to text is invisible and refused while unconfigured', async () => {
  ...
  try {
    ...
    assert.equal('stt' in record((await get('/api/host')).body.capabilities), false,
...
test('POST /api/stt relays audio to the configured endpoint and keeps its credentials server-side', async () => {
  const seen: TestRecord[] = [];
  const upstream = http.createServer(async (req, res) => {
    const Readable: typeof import('node:stream').Readable = require('node:stream').Readable;
  ...
  if (!upstreamAddress || typeof upstreamAddress === 'string') throw new Error('STT upstream lacks TCP address');
  ...
  try {
    ...
    assert.equal(record((await get('/api/host')).body.capabilities).stt, true);
...
test('POST /api/stt reports an upstream failure and validates its own body', async () => {
  ...
  const upstreamAddress = upstream.address();
  if (!upstreamAddress || typeof upstreamAddress === 'string') throw new Error('STT failure upstream lacks TCP address');
...
test('recovery mode and exclusions are partial host settings with validated values', async () => {
  ...
  assert.equal(record((await get('/api/host')).body.capabilities).recovery, true);
...
test('POST /close prunes a reused registry PID without signaling the new process', async () => {
  ...
  const actual = present(processIdentity(child.pid))
...
test('POST /close SIGTERMs the registry pid; /stats reports where it runs', async () => {
  ...
  fs.writeFileSync(sockStub, '');
  const identity = present(processIdentity(child.pid))
  ...
  try {
    ...
    assert.equal(record(stats.body.runtime).kind, 'tmux');
    assert.equal(record(stats.body.runtime).server, 'work');
    assert.equal(record(stats.body.runtime).pid, child.pid);
    assert.equal(record(stats.body.runtime).tmuxSession, null);

    const exited = new Promise(r => child.once('exit', (code: number, signal: string) => r(signal)));
...
test('POST /close refuses an unreachable legacy registry PID without signaling it', async () => {
  ...
  try {
    ...
    assert.ok(records(listed.body.active).some((session) => session.id === id),
...
test('POST /branch on a user message returns its text and persists the leaf move', async () => {
  ...
  const texts = records(msgs.body.messages).map(message => text(records(message.content)[0]?.text));
...
test('branch_summary entries surface in /messages as branchSummary role', async () => {
  ...
  assert.equal(status, 200);
  const bs = present(records(body.messages).find((m) => m.role === 'branchSummary'))
  assert.ok(bs, 'branch summary appears in the message stream');
  assert.equal(text(records(bs.content)[0]?.text), 'Explored **X**; conclusion Y.');
});
...
test('OMP transcript loading does not wait for the optional pricing catalog', async (t) => {
  ...
  let catalogCallback: (error: Error | null, stdout: string, stderr: string) => void = () => {};
  let catalogStartedResolve: () => void = () => {};
  const catalogStarted = new Promise<void>(resolve => { catalogStartedResolve = resolve; });
  t.mock.method(childProcess, 'execFile', (_file: unknown, _args: unknown, _options: unknown, callback: typeof catalogCallback) => {
  ...
  const raced = await Promise.race([
    ...
    new Promise<{ timedOut: true }>(resolve => { timeout = setTimeout(() => resolve({ timedOut: true }), 500); }),
  ]);
  ...
  assert.ok(records(result.body.messages).length > 0);
});
...
test('live OMP tree routes use tree_read/tree_navigate and the transcript follows the bridge leaf', async () => {
  ...
  const originalSendKeys = tmux.sendKeys;
  let sdkCalls = 0;
  const serviceCommands: TestRecord[] = []
  piSDK.getSessionTree = piSDK.branchSession = async () => {
    sdkCalls++;
  ...
  tmux.sendKeys = async (socket: string, paneId: string, keyText: string) => {
    serviceCommands.push({ socket, paneId, text: keyText });
    const queued = present(pendingNavigation);
  ...
  try {
    ...
    const wireNavigate = present(received.find(command => command.command === 'tree_navigate'));
    ...
    assert.deepEqual(records(messages.body.messages).map(message => text(records(message.content)[0]?.text)), ['OMP shared prompt']);
...
test('live OMP tree navigation presses the advertised service chord and surfaces bridge refusals', async () => {
  ...
  const typed: TestRecord[] = []
  const pressed: TestRecord[] = []
  tmux.findPaneByPid = async () => ({ socket: '/fake/omp-tmux.sock', paneId: '%42', tmuxSession: 'work', windowIndex: 0, windowName: 'fixture' });
  tmux.sendKeys = async (socket: string, paneId: string, keyText: string) => { typed.push({ socket, paneId, text: keyText }); };
  tmux.sendKey = async (socket: string, paneId: string, key: string) => {
    pressed.push({ socket, paneId, key });
    const { sock, command } = present(pendingNavigation);
    pendingNavigation = null;
    sock.write(JSON.stringify({
      type: 'response', id: command.id, command: command.command, success: true,
...
test('POST /branch on a live bridge session forwards navigate_tree', async () => {
  ...
  const received: TestRecord[] = []
  let reply: { success: boolean; data?: { editorText: string }; error?: string } = { success: true, data: { editorText: 'from bridge' } };
  const socks: net.Socket[] = []
  const bridge = net.createServer((sock) => {
    socks.push(sock);
    ...
    sock.on('data', (chunk) => {
      ...
      while ((i = buf.indexOf('\n')) >= 0) {
        ...
        if (!line.trim()) continue;
        const cmd = record(JSON.parse(line))
        received.push(cmd);
  ...
  await new Promise<void>(resolve => setTimeout(resolve, 600)); // registry scan memo TTL
  ...
  try {
    ...
    const nav = present(received.find(command => command.command === 'navigate_tree'));
    ...
    for (const socket of socks) socket.destroy();
    await new Promise<void>((resolve, reject) => bridge.close(error => error ? reject(error) : resolve()));
  }
});
...
test('GET /api/themes without a themes dir serves the built-ins', async () => {
  ...
  const { status, body } = await get('/api/themes');
  assert.equal(status, 200);
  assert.deepEqual(records(body.themes).map(theme => theme.id), ['solarized', 'graphite']);
});
...
test('POST /share is idempotent and 404s for an unknown session', async () => {
  ...
  const piAlias = encodeSessionKey('pi', checkedNativeId(TREE_ID));
...
test('GET /share/:token renders the exported HTML inline; unknown token 404s', async () => {
  ...
  const dataMatch = present(html.match(/id="session-data"[^>]*>([^<]+)</));
  const payload = Buffer.from(present(dataMatch[1]), 'base64').toString('utf8');
...
test('OMP sessions can create and render public share links', async () => {
  ...
  const dataMatch = present(html.match(/id="session-data"[^>]*>([^<]+)</));
  const payload = record(JSON.parse(Buffer.from(present(dataMatch[1]), 'base64').toString('utf8')));
  assert.equal(record(payload.header).id, OMP_SESSION_ID);
  assert.ok(records(payload.entries).some(entry => entry.id === 'omp-a1'), 'OMP transcript entries reach the export');
  assert.ok(records(payload.entries).some(entry => entry.type === 'title'), 'OMP pre-header records are preserved');
  ...
  const bridge = net.createServer(sock => {
    ...
    sock.on('data', chunk => {
      ...
      for (;;) {
        ...
        if (newline < 0) break;
        const command = record(JSON.parse(pending.slice(0, newline)));
...
test('OMP custom /share imports and serves its exact native live HTML', async () => {
  ...
  const result = record(await imported.json());
...
test('OMP import retains permissive native bytes but still requires a header', async () => {
  const wrap = (data: unknown) => `<html><script id='session-data'>!${Buffer.from(JSON.stringify(data)).toString('base64')}</script></html>`;
...
test('dedicated share listener serves shared sessions, pages, and standalone file styles', async () => {
  ...
  const sharePort = await new Promise<number>((resolve, reject) => {
    const srv = net.createServer();
    srv.once('error', reject);
    srv.listen(0, '127.0.0.1', () => {
      const address = srv.address();
      if (!address || typeof address === 'string') return reject(new Error('Share port probe lacks TCP address'));
...
test('session caches pick up JSONL appends (mtime/size revalidation)', async () => {
  ...
  const sess = present(records(list.body.previous).find((s) => s.id === SESSION_ID))
  ...
  assert.ok(records(listSearch.body.previous).some((s) => s.id === SESSION_ID), 'list search text refreshed');
...
test('OMP session_switch re-keys the owned pane, active route, history source, and old-route SSE', async () => {
  ...
  const oldRouteId = encodeSessionKey('omp', checkedNativeId(oldNativeId));
  const newRouteId = encodeSessionKey('omp', checkedNativeId(newNativeId));
  ...
  try {
    await stream.waitFor((event: unknown) => eventName(event) === 'init');
    tmux.recordSpawn(oldRouteId, {
      socket: '/fake/omp-session-switch.sock', paneId: '%91', bridgeInstanceId: instanceId,
    });
    tmux.paneExists = async (socket: string, paneId: string) => socket === '/fake/omp-session-switch.sock' && paneId === '%91';
    tmux.sendKeys = async (socket: string, paneId: string, keyText: string) => { injected.push({ socket, paneId, text: keyText }); };
    ...
    const switchEvent = await stream.waitFor((event: unknown) => eventName(event) === 'session_switch');
    ...
    const listed = await get('/api/sessions');
    assert.ok(records(listed.body.active).some((session) => session.id === newRouteId), 'new route is active');
    assert.ok(records(listed.body.previous).some((session) => session.id === oldRouteId), 'old route remains historical');
    assert.equal(records(listed.body.active).some((session) => session.id === oldRouteId), false);
...
test('SSE replays remembered extension UI state to new connections', async () => {
  ...
  const emit = (event: string, data: unknown) => {
    for (const socket of bridgeSocks) socket.write(JSON.stringify({ type: 'event', event, data }) + '\n');
  };
  ...
  try {
    ...
    await s1.waitFor((e: unknown) => eventName(e) === 'init');
    assert.equal(s1.events.some((e: unknown) => eventData(e).id === 'stale-ask'), false,
    ...
    await s1.waitFor((e: unknown) => eventName(e) === 'extension_ui_request' && eventData(e).method === 'confirm');
    await s1.waitFor((e: unknown) => eventName(e) === 'extension_ui_request' && eventData(e).method === 'ask');
    await s1.waitFor((e: unknown) => eventName(e) === 'tool_execution_update' && eventData(e).toolCallId === 'replay-tool');
    const interrupted = await s1.waitFor((e: unknown) =>
      eventName(e) === 'message_end' && record(eventData(e).message).customType === 'interrupted-thinking');
    assert.deepEqual(record(eventData(interrupted).message).content, [], 'hidden interrupted reasoning is projected to a marker');
    await s1.waitFor((e: unknown) => eventName(e) === 'message_end' && record(eventData(e).message).customType === 'visible-sentinel');
    assert.equal(s1.events.some((e: unknown) => eventName(e) === 'message_end' && record(eventData(e).message).customType === 'private-host-state'), false,
    ...
    const replayStart = await s2.waitFor((e: unknown) => eventName(e) === 'tool_execution_start' && eventData(e).toolCallId === 'replay-tool');
    const replayUpdate = await s2.waitFor((e: unknown) => eventName(e) === 'tool_execution_update' && eventData(e).toolCallId === 'replay-tool');
    assert.deepEqual(eventData(replayStart), {
      toolCallId: 'replay-tool', toolName: 'Bash', args: { command: 'sleep 12' }, startedAt: 123,
    });
    const partialResult = record(eventData(replayUpdate).partialResult);
    assert.equal(text(records(partialResult.content)[0]?.text), 'still running');
    assert.ok(s2.events.findIndex((e: unknown) => eventName(e) === 'init') <
      s2.events.findIndex((e: unknown) => eventName(e) === 'tool_execution_start'), 'tool replay follows init');
    const widget = await s2.waitFor((e: unknown) => eventName(e) === 'extension_ui_request' && eventData(e).method === 'setWidget');
    assert.deepEqual(eventData(widget).widgetLines, ['one', 'two']);
    assert.equal(eventData(widget).widgetKey, 'procs');
    await s2.waitFor((e: unknown) => eventName(e) === 'extension_ui_request' && eventData(e).id === 'dlg1');
    const ask = await s2.waitFor((e: unknown) => eventName(e) === 'extension_ui_request' && eventData(e).id === 'ask1');
    assert.equal(record(records(eventData(ask).questions)[0]).question, 'Which release?');
    ...
    emit('turn_end', {});
    await s2.waitFor((e: unknown) => eventName(e) === 'tool_execution_end' && eventData(e).toolCallId === 'replay-tool');
    await s2.waitFor((e: unknown) => eventName(e) === 'extension_ui_resolved' && eventData(e).id === 'ask1');
    s2.close();
    ...
    await s3.waitFor((e: unknown) => eventName(e) === 'init');
    emit('extension_ui_request', { method: 'notify', message: 'sentinel' });
    await s3.waitFor((e: unknown) => eventName(e) === 'extension_ui_request' && eventData(e).method === 'notify');
    const extEvents = records(s3.events).filter((e: TestRecord) => eventName(e) === 'extension_ui_request');
    assert.equal(extEvents.length, 1, 'cleared widget / resolved dialog must not be replayed');
    assert.equal(records(s3.events).some(e => text(e.event).startsWith('tool_execution_')), false,
      'completed tools are absent from later reconnect snapshots');
    s3.close();
  } finally {
    fs.rmSync(path.join(registryDir, `${BRIDGE_ID}.json`), { force: true });
    for (const socket of bridgeSocks) socket.destroy();
    await new Promise<void>((resolve, reject) => bridge.close(error => error ? reject(error) : resolve()));
  }
});
...
test('/reload maps a run_command socket teardown to success (old-bridge race)', async () => {
  ...
  await new Promise<void>(resolve => setTimeout(resolve, 600)); // registry memo TTL
...
test('GET /api/skills discovers the fixture skill with mined usage rollups', async () => {
  ...
  assert.ok(Number(record(body.summary).discovered) >= 1);
  const demo = present(records(body.skills).find(skill => skill.name === 'demo'));
  ...
  const usage = record(demo.usage), kindSplit = record(usage.kindSplit);
...
test('disable-model-invocation skills would be manual with zero advertised cost', async () => {
  ...
  const skills = records(body.skills);
...
test('GET /api/skills/coverage maps ranged reads since mtime; targeted are touches', async () => {
  ...
  const sections = records(body.sections);
  ...
  assert.equal(record(body.latest).sessionId, SKILLS_SESSION_ID, 'latest activation deep-links its session');
...
const writeResolveFixture = (id: string, text: string) => fs.writeFileSync(path.join(sessionDir, `${id}.jsonl`), [
...
const waitForSessions = async (ids: string[], tries = 60): Promise<void> => {
  for (let i = 0; i < tries; i++) {
    const { body } = await get('/api/sessions');
    const rows = [...records(body.active), ...records(body.previous)];
...
test('GET /api/sessions/resolve resolves exact ids and unique prefixes', async () => {
  ...
  const listed = present(records((await get('/api/sessions')).body.previous).find((s) => s.id === RESOLVE_UNIQUE))
...
test('GET /api/sessions/resolve reports ambiguous prefixes with candidates', async () => {
  ...
  const matches = records(body.matches);
  ...
  assert.equal(record(exactWins.body.session).id, RESOLVE_AMBIGUOUS_A);
});
...
const ALIAS_ROUTE_ID = encodeSessionKey('omp', checkedNativeId(ALIAS_NATIVE_ID));
...
test('GET /api/sessions/resolve accepts a native id or uuid tail and answers with a short ref', async () => {
  ...
  for (const ref of [ALIAS_ROUTE_ID, ALIAS_NATIVE_ID, '01a070d2-43fb-7360-aaba-a4ddf8d1deb0', '01a070d2', '2026-09-05T09-07']) {
    ...
    assert.equal(record(body.session).id, ALIAS_ROUTE_ID, ref);
  }
  ...
  assert.equal(record(viaShort.body.session).id, ALIAS_ROUTE_ID);
...
test('GET /api/host advertises ref-alias resolution so clients can print short refs', async () => {
  ...
  assert.equal(record(body.capabilities).refAliases, true);
});
...
test('GET /api/agent-docs lists the shipped topics with titles and descriptions', async () => {
  ...
  assert.equal(status, 200);
  const topics = records(body.topics);
  ...
  assert.equal(present(topics.find(topic => topic.name === 'refs')).title, 'Session refs');
});
...
test('GET /api/host advertises the resolve and docs capabilities', async () => {
  ...
  assert.equal(status, 200);
  assert.equal(record(body.capabilities).resolve, true);
  assert.equal(record(body.capabilities).docs, true);
});
