// Generated test/tool from test/fleet-artifacts.test.ts; edit that source and run npm run build:tests.
"use strict";
...
async function json(response) {
    ...
    try {
        return (0, test_types_js_1.record)(JSON.parse(text));
    }
    catch {
...
test('boot a peer, a counting remote and two hubs that know about them', async () => {
    ...
    peerHostId = String((await json(await fetch(`${peer.base}/api/host`))).hostId ?? '');
    counterHits = [];
    ...
    const counterPort = await listenHttp(counter);
    const remotes = [
...
test('a session export downloads through the proxy, and only through it', async () => {
    ...
    assert.equal((0, test_types_js_1.records)(artifactBody.artifacts).length, 0, 'exporting publishes nothing');
});
...
test('a share created through the proxy is mapped to the peer and re-addressed to the hub', async () => {
    ...
    shareToken = String(created.token);
    ...
    const artifactBody = await json(await fetch(`${hub.base}/api/fleet-artifacts`));
    const entry = (0, test_types_js_1.present)((0, test_types_js_1.records)(artifactBody.artifacts).find(artifact => artifact.token === shareToken));
...
test('a hostId no configured remote claims is a 404', async () => {
    ...
    const artifactBody = await json(await fetch(`${hub.base}/api/fleet-artifacts`));
    assert.equal((0, test_types_js_1.records)(artifactBody.artifacts).some(artifact => artifact.token === 'unmappable-token'), false);
...
test('a comment on a fleet page files on the peer, where its agent reads it', async () => {
    ...
    assert.equal((0, test_types_js_1.record)(comment.target).pageToken, pageToken);
    const peerComments = (0, test_types_js_1.record)(JSON.parse(fs.readFileSync(path.join(peer.home, '.pi', 'dish', 'comments.json'), 'utf8')));
    const commentId = String(comment.id), storedComment = (0, test_types_js_1.record)(peerComments[commentId]);
    assert.ok(peerComments[commentId], 'the comment landed in the owning host\'s store');
    assert.equal(storedComment.body, 'tighten this paragraph');
    assert.equal(fs.existsSync(path.join(hub.home, '.pi', 'dish', 'comments.json')), false, 'the hub keeps no copy of a peer\'s feedback');
    ...
    const indexedComments = (0, test_types_js_1.records)(index.comments);
    assert.deepEqual(indexedComments.map(indexed => indexed.id), [comment.id]);
    const full = await json(await fetch(`${hub.base}/api/comments/get`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sessionId: (0, test_types_js_1.present)(indexedComments[0]).sessionId, ids: [comment.id], pageToken }),
    }));
    assert.equal((0, test_types_js_1.present)((0, test_types_js_1.records)(full.comments)[0]).body, 'tighten this paragraph');
    ...
    const afterPatch = (0, test_types_js_1.record)(JSON.parse(fs.readFileSync(path.join(peer.home, '.pi', 'dish', 'comments.json'), 'utf8')));
    assert.equal((0, test_types_js_1.record)(afterPatch[commentId]).body, 'tighten this paragraph, please');
...
test('revoking on the owner 404s the hub and prunes the mapping', async () => {
    ...
    assert.equal((await res.text()).trim(), 'Not found');
    const artifactBody = await json(await fetch(`${hub.base}/api/fleet-artifacts`));
    assert.equal((0, test_types_js_1.records)(artifactBody.artifacts).some(artifact => artifact.token === pageToken), false, 'the dead mapping was pruned lazily');
});
test('a revoke proxied through the hub prunes the mapping directly', async () => {
    ...
    assert.equal(revoked.revoked, true);
    const artifactBody = await json(await fetch(`${hub.base}/api/fleet-artifacts`));
    assert.equal((0, test_types_js_1.records)(artifactBody.artifacts).some(artifact => artifact.token === page.token), false);
    ...
    assert.equal((0, test_types_js_1.records)(after.artifacts).some(artifact => artifact.token === shareToken), false);
...
test('the pages skill publishes locally and maps onto a hub with --via', async () => {
    ...
    const hubSettings = (0, test_types_js_1.record)(JSON.parse(fs.readFileSync(path.join(hub.home, '.pi', 'dish', 'settings.json'), 'utf8')));
    const hubRemotes = Array.isArray(hubSettings.remotes) ? hubSettings.remotes : [];
    hubRemotes.push({ name: 'agenthost', url: agentHost.base });
    hubSettings.remotes = hubRemotes;
    ...
    const run = (args, extraEnv = {}) => new Promise(resolve => {
        const child = (0, node_child_process_1.spawn)(process.execPath, [CLI, ...args], {
            env: { ...process.env, HOME: home, PI_DISH_URL: agentHost.base, PI_DISH_PUBLIC_VIA: '', ...extraEnv },
