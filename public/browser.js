...
${restored}`;
  ...
  function createSidebarControls(options2) {
    ...
    function familyRoots() {
      ...
      rootLists = sessionState.sessions;
      rootMap = map;
      return map;
    }
...
${row.id}`;
  ...
  function createTranscript(options2) {
    ...
    const owns = (owner) => {
      const endpoint = options2.host(owner.selection.host);
      return !disposed && owner.generation === generation && sessionState.ownsSelection(owner.selection) && endpoint?.base === owner.endpoint.base && (endpoint.token || "") === (owner.endpoint.token || "");
    };
    function retire() {
      generation++;
      catchupSequence++;
  ...
  function createComposerSubmit(options2) {
    ...
    async function sendPrompt() {
      ...
      if (sessionState.currentSession.harnessId === "omp" && OMP_MODEL_MENTION.test(message3) && (sessionActivity.turn || sessionActivity.compacting)) {
    ...
    async function sendQueuedMessage(kind) {
      ...
      if (sessionState.currentSession.harnessId === "omp" && OMP_MODEL_MENTION.test(message3)) {
