...
(() => {
  ...
  function decodeKnownDirectories(value) {
    ...
    const rows = value;
    return rows.flatMap((row) => record(row) && typeof row.path === "string" && typeof row.short === "string" ? [{ path: row.path, short: row.short }] : []);
  }
  function decodeDirectoryChildren(value) {
    ...
    return { dirs: rows.flatMap((row) => record(row) && typeof row.path === "string" && typeof row.name === "string" ? [{ path: row.path, name: row.name }] : []), error: !!value.error };
  }
  ...
  function formatCacheStat(cacheRead, cacheWrite, input) {
    ...
    const prompt2 = read + write + (input || 0);
    if (prompt2 === 0) return "\u2014";
    let s = `${formatTokens(read)} read (${Math.round(read / prompt2 * 100)}% hit)`;
  ...
  function createHostDirectory(options2) {
    ...
    return {
      ...
      effectiveHosts: effectiveHosts2,
      entryFor,
      hostById,
      resolveHost,
      sourceFor,
  ...
  var optionalNumber = (value) => finite2(value) ? value : void 0;
  ...
  var fieldDecoders = {
    ...
    thinkingLevel: nullableString,
    harnessId: optionalString,
    harnessLabel: optionalString,
    capabilities: decodeCapabilities,
    isActive: optionalBoolean,
    closeMode: optionalString,
    conflicted: optionalBoolean,
    liveInstanceCount: optionalNumber,
    ...
    parentSource: nullableString,
    familyParentId: nullableString,
    routine: optionalString,
    routineId: optionalString,
    routineInvocationId: optionalString,
    searchSnippet: optionalString,
    searchScore: optionalNumber
  };
  function decodeCapabilities(value) {
    if (value === void 0) return void 0;
    if (!record3(value)) return invalid("session capabilities");
    const capabilities = {};
  ...
  function decodeFields(value) {
    ...
    for (const key of Object.keys(fieldDecoders)) {
      ...
      if (decoded !== void 0) Object.defineProperty(fields, key, { value: decoded, enumerable: true, configurable: true, writable: true });
    }
    return fields;
  }
  function decodeSessionRow(value) {
    if (!record3(value) || !Object.hasOwn(value, "id")) return invalid("session");
  ...
  function decodePatch(value, keys) {
    if (!record3(value)) return invalid("session patch");
    const patch = {};
    for (const key of keys) {
      if (!Object.hasOwn(value, key) || value[key] === void 0) continue;
      const decoded = fieldDecoders[key](value[key]);
      if (decoded === void 0) return invalid("session patch");
  ...
  function validateSessionControls(value) {
    if (!record3(value) || !text2(value.id)) return invalid("session");
    ...
    if (value.harnessId !== void 0 && !text2(value.harnessId)) return invalid("session");
    if (value.isActive !== void 0 && typeof value.isActive !== "boolean") return invalid("session");
  }
  function decodeSessionList(value) {
    if (!record3(value) || !Array.isArray(value.active) || !Array.isArray(value.previous) || value.children !== void 0 && !Array.isArray(value.children)) return invalid("session list");
    return {
      active: value.active.map(decodeSessionRow),
      previous: value.previous.map(decodeSessionRow),
      ...
      ...finite2(value.discoverySkipped) ? { discoverySkipped: value.discoverySkipped } : {}
    };
  }
  function pricing(value) {
    if (!record3(value) || !finite2(value.input) || !finite2(value.output)) return null;
    return {
      input: value.input,
      output: value.output,
      ...finite2(value.cacheRead) ? { cacheRead: value.cacheRead } : {},
      ...finite2(value.cacheWrite) ? { cacheWrite: value.cacheWrite } : {}
    };
  }
  ...
  function decodeModelCatalog(value) {
    ...
    return value.map((item) => {
      if (!record3(item) || !text2(item.id) || !text2(item.provider)) return invalid("model catalog");
  ...
  function decodeMutationResult(value) {
    if (!record3(value) || value.success !== true) return invalid("mutation");
    return { ...value, success: true };
  }
  function decodeThinkingResult(value) {
    const result = decodeMutationResult(value);
    if (!text2(result.level)) return invalid("thinking");
    return { ...result, level: result.level };
  }
  function decodeEnabledModelsResult(value) {
    const result = decodeMutationResult(value);
    if (result.enabledModels !== null && (!Array.isArray(result.enabledModels) || !result.enabledModels.every(text2))) return invalid("enabled models");
  ...
  function decodeHostDescriptor(value) {
    if (!record4(value) || typeof value.hostId !== "string" || !value.hostId) return null;
    return {
      hostId: value.hostId,
  ...
  function createHostDiscovery(options2) {
    ...
    async function performFleet(sequence) {
      try {
        ...
        if (!response.ok) return;
        const data = await response.json();
        if (sequence !== fleetSequence || !record4(data) || !Array.isArray(data.hosts)) return;
        const rows = data.hosts;
        const hosts = rows.filter(record4);
  ...
  function createSessionState(options2) {
    ...
    function stampSessionHost(session, hostId = options2.getSelfHostId()) {
      ...
      const label = options2.getHostLabel(session.host);
    ...
    function setCurrentSession(id, host) {
      ...
      currentSession = entry ? stampSessionHost({ ...entry }, entry.host || options2.getSelfHostId()) : null;
      return currentSession;
    }
    function applyPatch(id, patch, host) {
      const matches = (session) => session !== null && session.id === id && (session.host || null) === (host || null);
      for (const list of [sessions.active, sessions.previous]) {
        const session = list.find(matches);
        if (session) stampSessionHost(Object.assign(session, patch), session.host || options2.getSelfHostId());
      }
      if (matches(currentSession) && currentSession) stampSessionHost(Object.assign(currentSession, patch), currentSession.host || options2.getSelfHostId());
      options2.onListsChanged();
    ...
    function patchSessionActivity(id, patch, host = sessionHostId(id)) {
    ...
    function mergeCurrentSession(owner, fields) {
      ...
      stampSessionHost(currentSession, currentSession.host || options2.getSelfHostId());
      options2.onCurrentChanged();
    }
  ...
  function createResponseDetails(options2) {
    ...
    function openResponseDetails(id) {
      ...
      const prompt2 = (u.input || 0) + (u.cacheRead || 0) + (u.cacheWrite || 0);
      ...
      const rows = [
        ...
        ["Cache", `${formatTokens(u.cacheRead)} read \xB7 ${formatTokens(u.cacheWrite)} write${prompt2 ? ` \xB7 ${Math.round((u.cacheRead || 0) / prompt2 * 100)}% hit` : ""}`],
  ...
  function createSessionReferences(options2) {
    const { sessionState: sessionState2 } = options2;
    function hostId(session) {
      return session?.host || options2.selfId();
    }
    function all() {
      return [...sessionState2.sessions.active, ...sessionState2.sessions.previous];
    }
    function candidates() {
      const current = sessionState2.currentSession;
    ...
    function prefix(session) {
      ...
      const aliases = endpoint && (record2(endpoint.capabilities) ? endpoint.capabilities.refAliases === true : (endpoint.self === true || endpoint.base === "") && !!options2.config().refAliases);
    ...
    function match(value, localHostId = sessionState2.currentSession ? hostId(sessionState2.currentSession) : options2.selfId()) {
    ...
    function hints(message3) {
      return parseSessionRefTokens(message3).flatMap(({ ref: ref2 }) => {
        ...
        return session ? [{ ref: ref2, name: session.name || "", host: options2.hostLabel(session.host || null) || "", cwd: session.cwd || "", isActive: !!session.isActive }] : [];
      });
    }
  ...
  function decodeSlashCommands(value) {
    return Array.isArray(value) ? value.flatMap((v) => record2(v) && typeof v.name === "string" && v.name ? [{ name: v.name, description: text3(v.description), source: text3(v.source), args: text3(v.args) }] : []) : [];
  }
  function decodeFileCompletions(value) {
    return Array.isArray(value) ? value.flatMap((v) => record2(v) && typeof v.path === "string" ? [{ path: v.path, isDir: v.isDir === true, gitStatus: text3(v.gitStatus) }] : []) : [];
  }
  ...
  function createComposerAutocomplete(options2) {
    ...
    function captureRequest() {
      const selection = sessionState2.captureSelection();
    ...
    function ownsRequest(owner) {
      return !!owner && !disposed && sessionState2.ownsSelection(owner.selection) && options2.host(owner.selection.host)?.base === owner.endpoint.base;
    }
    function capture() {
    ...
    function queueFile(token) {
      ...
      fileTimer = setTimeout(() => {
        ...
        void options2.request({ ...owner.endpoint, token: endpoint.token }, `/api/sessions/${encodeURIComponent(owner.selection.id)}/files?q=${encodeURIComponent(token)}`).then(async (response) => {
          ...
          if (response.ok && record2(value)) showFiles(value.files, owner);
          else hide();
        }).catch(() => {
  ...
  function createSidebarActivity(options2) {
    const { document: document2, storage, sessionState: sessionState2 } = options2;
  ...
  function decodeSavedFilters(value) {
    return Array.isArray(value) ? value.flatMap((row) => record2(row) && typeof row.name === "string" && typeof row.query === "string" ? [{ name: row.name, query: row.query }] : []) : [];
  }
  ...
  function createDisplayPreferences(options2) {
    ...
    <div class="preference-row"><label><strong>Saved sidebar filters</strong><small>Server-global. Chips under the sidebar filter toggle these per device; type a query there and hit \u201C+ save filter\u201D to add one.</small></label><div id="savedFiltersList" class="saved-filters-list"></div></div>`;
          ...
          button.addEventListener("click", async () => {
            ...
            try {
              ...
              if (owns()) {
                ...
                options2.alert("Could not delete filter: " + message(error));
              }
            }
      ...
      try {
        ...
        const data = await response.json();
        if (!owns()) return;
        if (!response.ok || !record2(data)) throw new Error("Could not load server setting.");
      ...
      save.addEventListener("click", async () => {
        ...
        try {
          ...
          const data = await response.json();
          if (!response.ok) throw new Error(record2(data) && typeof data.error === "string" ? data.error : "Save failed");
          if (owns()) status.textContent = "Saved for all devices.";
        } catch (error) {
          if (owns()) status.textContent = "Save failed: " + message(error);
        } finally {
  ...
  function createSidebarQuery(options2) {
    ...
    async function loadFilters() {
      ...
      try {
        ...
        if (!response.ok) throw new Error(record2(data) && typeof data.error === "string" ? data.error : "Failed to load filters");
        commitFilters(decodeSavedFilters(record2(data) ? data.savedFilters : void 0));
        refreshViews(false);
      } catch (error) {
    ...
    async function run(host, key, wireQuery, withPrevious, owner) {
      try {
        ...
        const next = {
          ...
          previous: withPrevious ? data.previous.map(sessionEntryFromRow) : mergeLiveSubagents(cached.previous, data.children?.map(sessionEntryFromRow))
        };
  ...
  function createSidebarLists(options2) {
    ...
    const loader = createHostSessionLoader({
      ...
      beforePublish: (host, next, wireQuery) => {
        if (disposed) return;
        const hostId = host.hostId || null, selected = sessionState2.currentSession;
...
${row.id}`;
  ...
  function createSidebarControls(options2) {
    const { document: document2, storage, sessionState: sessionState2 } = options2, window2 = document2.defaultView;
    ...
    function toggleFamily(id, host = sessionState2.sessionHostId(id)) {
      if (disposed) return;
    ...
    function togglePin(id, displayedRoot = id, members = [id], host = sessionState2.sessionHostId(id)) {
      if (disposed) return;
      ...
      for (const member of aliases) {
        const { hostId, sessionId } = parseSessionKey(member), parentId = sessionState2.findSession(sessionId, hostId)?.familyParentId;
    ...
    let confirm2 = null, busy = null, confirmTimer = null;
    function captureClose(id, host) {
      const endpoint2 = options2.host(host);
      return disposed || !endpoint2 ? null : { id, host, key: sessionKey(host, id), endpoint: Object.freeze({ ...endpoint2 }), owner: sessionState2.captureSelection(), generation: sessionState2.selectionGeneration };
    }
    ...
    const ownsFeedback = (entry) => entry.owner ? sessionState2.ownsSelection(entry.owner) : sessionState2.selectionGeneration === entry.generation && !sessionState2.currentSession;
    function clearConfirm() {
      if (confirmTimer) clearTimeout(confirmTimer);
      confirmTimer = null;
      confirm2 = null;
    }
    async function performClose(id, host = sessionState2.sessionHostId(id), captured) {
    ...
    function closeClick(id, host = sessionState2.sessionHostId(id)) {
      if (disposed || busy) return;
      const key = sessionKey(host, id);
      if (confirm2?.key === key && endpoint(confirm2)) {
        void performClose(id, host, confirm2);
        return;
      }
      clearConfirm();
      confirm2 = captureClose(id, host);
      if (!confirm2) return;
      const entry = confirm2;
      confirmTimer = setTimeout(() => {
        if (confirm2 !== entry || disposed) return;
        clearConfirm();
        render();
      }, 3e3);
      render();
    ...
    function openMenu(session, x2, y) {
      ...
      window2.addEventListener("resize", closeMenu, { signal });
      later(() => {
        if (!current()) return;
        document2.addEventListener("click", (event) => {
    ...
    function mount() {
      ...
      list.addEventListener("click", (event) => {
        ...
        if (window2.innerWidth <= 768) options2.closeSidebar();
      }, { signal });
      list.addEventListener("contextmenu", (event) => {
        if (!(event.target instanceof Element)) return;
        const item = event.target.closest(".session-item[data-id]");
        if (!item || !list.contains(item)) return;
        const session = sessionState2.findSession(item.dataset.id, item.dataset.host || null);
        if (!session) return;
        event.preventDefault();
  ...
  function usageUnattributedCost(costs2) {
    if (!finite(costs2?.total)) return 0;
    const attributed = ["input", "output", "cacheRead", "cacheWrite"].reduce((sum, key) => sum + (finite(costs2[key]) ? costs2[key] : 0), 0);
  ...
  function mergeUsageSummaries(list) {
    ...
    for (const { summary, hostId = null, hostLabel = null } of entries) {
      ...
      for (const [key, costs2] of Object.entries(summary.headlineCostsByBucket || {})) {
        ...
        for (const k of USAGE_MERGE_COST_KEYS) if (finite(costs2?.[k])) row[k] += costs2[k];
      }
  ...
  `;
    ...
    function renderSessions2() {
      ...
      const { active, previous } = options2;
  ...
  function createSessionView(options2) {
    ...
    <small>You can write your prompt while it starts.</small>
      ...
      options2.drafts.restore(pendingComposerKey2(spawnId));
    ...
    async function selectSession(id, { forceTranscriptReload = false, host = null, keepBounceView = false } = {}) {
      ...
      const owns = () => !disposed && sessionState2.ownsSelection(owner) && options2.endpoint(owner.host).base === endpoint.base;
      ...
      if (sessionState2.currentSession?.isActive) {
  ...
  function createSessionResume(options2) {
    ...
    async function load(session) {
      ...
      const owns = () => !disposed && generation === sequence && sessionState2.ownsSelection(owner) && options2.endpoint(owner.host).base === endpoint.base;
      wrap.style.display = "flex";
      select.title = "Loading Oh My Pi models\u2026";
      try {
        const models = await api.models(endpoint, { harnessId: "omp", cwd: session.cwd });
        if (!owns()) return;
    ...
    async function resume() {
      ...
      if (sessionState2.currentSession?.capabilities?.resume === false) return;
      ...
      const model = sessionState2.currentSession?.harnessId === "omp" ? select?.value || void 0 : void 0;
      const owns = () => !disposed && pending.get(key) === token && sessionState2.ownsSelection(owner) && options2.endpoint(owner.host).base === endpoint.base;
      ...
      try {
        ...
        if (disposed) return;
        if (!record2(data) || typeof data.id !== "string" || !data.id) throw new Error("Resume returned an invalid session");
  ...
  function decodeLineageSession(value) {
    if (!record2(value) || typeof value.id !== "string" || !value.id) return null;
    return {
      id: value.id,
      name: text5(value.name),
      cwd: text5(value.cwd),
      harnessId: text5(value.harnessId),
  ...
  function decodeLineageNode(value, budget) {
    if (!record2(value) || budget.left <= 0) return null;
    const session = decodeLineageSession(value.session);
    if (!session) return null;
    budget.left -= 1;
    const edge = record2(value.edge) ? { kind: text5(value.edge.kind), source: text5(value.edge.source) } : null;
  ...
  function createSessionRelations(options2) {
    ...
    const owns = (owner, endpoint = renderEndpoint) => !disposed && sessionState2.ownsSelection(owner) && !!owner && sameEndpoint(owner.host, endpoint);
  ...
  function extractTextContent(content) {
    ...
    if (Array.isArray(content)) {
      ...
      return blocks.map((c) => typeof c === "string" ? c : record2(c) && c.type === "text" && typeof c.text === "string" ? c.text : "").join("\n");
    }
    return "";
  }
  function extractTextBlocks(content) {
    ...
    const blocks = content;
    return blocks.filter((c) => typeof c === "string" || record2(c) && c.type === "text").map((c) => typeof c === "string" ? c : record2(c) && typeof c.text === "string" ? c.text : "").join("\n");
  }
  ...
  function messageHasVisibleText(msg) {
    if (!record2(msg)) return false;
    if (msg.errorMessage) return true;
    if (typeof msg.content === "string") return !!msg.content;
    return Array.isArray(msg.content) && msg.content.some((b) => record2(b) && b.type === "text" && typeof b.text === "string" && !!b.text);
  }
  function getToolOutputText(partialResult) {
    if (!record2(partialResult) || !Array.isArray(partialResult.content)) return "";
    const blocks = partialResult.content;
    return blocks.filter((c) => record2(c) && c.type === "text").map((c) => typeof c.text === "string" ? c.text : "").join("");
  }
  function extractImageBlocks(content) {
    ...
    for (const block of blocks) {
      if (!record2(block) || block.type !== "image") continue;
  ...
  function decodeMessageUsage(value) {
    ...
    return {
      ...
      cacheWrite1h: number2(value.cacheWrite1h),
  ...
  function decodeCacheExpiry(value) {
    if (!record2(value) || !finite(value.refreshedAt) || !finite(value.expiresAt) || !finite(value.retentionMs) || typeof value.retention !== "string" || !["fixed", "minimum", "estimate"].includes(String(value.basis))) return void 0;
    return {
      refreshedAt: value.refreshedAt,
      expiresAt: value.expiresAt,
      retentionMs: value.retentionMs,
  ...
  function createSubagentsView(options2) {
    ...
    function renderTree() {
      ...
      const currentId = lineage.session?.id || sessionState2.currentSession?.id || null;
      const rows = [];
  ...
  function createSessionHeader(options2) {
    ...
    function updateCacheExpiry() {
      ...
      const presentation = cacheExpiryPresentation(sessionState2.currentSession?.cacheExpiry);
    ...
    function updateSessionHeader() {
      if (disposed || !sessionState2.currentSession) return;
      const current = sessionState2.currentSession;
      ...
      if (showHarness) {
        ...
        const configurable = options2.settings(current);
      ...
      if (cpHarness) cpHarness.style.display = options2.settings(current) ? "" : "none";
  ...
  function createSessionControls(options2) {
    const { document: document2, sessionState: sessionState2, catalog } = options2, window2 = document2.defaultView;
    const element = (id) => {
      const value = document2.getElementById(id);
    ...
    function header() {
      return sessionState2.currentSession;
    }
    ...
    async function exportSession() {
      ...
      if (!endpoint.token) {
        window2.open(endpoint.base + path, "_blank");
        return;
      }
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(record2(data) && typeof data.error === "string" ? data.error : `HTTP ${response.status}`);
        }
  ...
  function decodeSessionSearch(value) {
    if (!record2(value) || !Array.isArray(value.matches)) throw new Error("Invalid session search response");
    return value.matches.flatMap((match) => record2(match) && typeof match.index === "number" && Number.isInteger(match.index) && match.index >= 0 ? [{ index: match.index, role: typeof match.role === "string" ? match.role : "" }] : []);
  }
  function createSessionSearch(options2) {
    ...
    async function run(value, { mode = "message", closeIfEmpty = false } = {}) {
      ...
      try {
        ...
        const data = await response.json();
        if (!owns()) return;
        if (!response.ok || record2(data) && typeof data.error === "string" && data.error) throw new Error(record2(data) && typeof data.error === "string" ? data.error : `HTTP ${response.status}`);
        query = value;
    ...
    async function jump() {
      const owner = sessionState2.captureSelection(), match = matches[pos], seq = sequence, tokens2 = query.split(/\s+/).filter(Boolean);
      const resolved = owner && options2.endpoint(owner.host);
  ...
  function decodeRecoveryReport(value) {
    if (!record2(value)) throw new Error("Invalid recovery report");
    const rows = Array.isArray(value.sessions) ? value.sessions : [];
    const sessions = rows.flatMap((row) => record2(row) && typeof row.id === "string" && row.id ? [{
      id: row.id,
      name: text6(row.name),
      harnessId: text6(row.harnessId),
      cwd: text6(row.cwd),
      reason: text6(row.reason),
      status: text6(row.status),
      excluded: row.excluded === true,
      updatedAt: typeof row.updatedAt === "string" || typeof row.updatedAt === "number" ? row.updatedAt : null
    }] : []);
    return {
      mode: text6(value.mode) || "off",
      sessions,
  ...
  function createRecovery(options2) {
    ...
    const apiSend2 = (host, path, payload, method) => sendJson(apiFetch, host, path, payload, method);
    const effectiveHosts2 = options2.hosts;
    const hostIsDown = options2.down;
    const confirm2 = options2.confirm;
    let disposed = false;
    ...
    function recoveryCapableHosts() {
      return effectiveHosts2().filter((host) => options2.supports(host));
    }
    ...
    function refreshRecoveryHosts() {
      ...
      if (unavailable) {
        const missing = effectiveHosts2().filter((host) => !options2.supports(host));
    ...
    async function renderRecoveryPreferences() {
      ...
      const load = async () => {
        ...
        try {
          ...
          const data = await res.json();
          if (!res.ok) throw new Error(record2(data) && text6(data.error) || `HTTP ${res.status}`);
          if (!owns(request, host)) return;
          mode.value = decodeRecoveryMode(record2(data) ? data.recoveryMode : null);
          mode.disabled = save.disabled = false;
          status.textContent = "";
        } catch (error) {
          if (owns(request, host)) status.textContent = "Could not load: " + message2(error);
        }
      };
      ...
      save.addEventListener("click", async () => {
        ...
        if (value === "continue" && !confirm2("On future server launches, continue interrupted work automatically? This may incur cost and repeat external actions. Tool execution is not exactly-once.")) return;
        const request = ++seq;
        mode.disabled = save.disabled = true;
        status.textContent = "Saving\u2026";
        try {
          await apiSend2(host, "/api/settings", { recoveryMode: value }, "PUT");
          if (owns(request, host)) status.textContent = "Saved on " + hostDisplayLabel(host) + " for future server launches.";
        } catch (error) {
          if (owns(request, host)) status.textContent = "Save failed: " + message2(error);
        } finally {
      ...
      try {
        ...
        for (const record9 of records) {
          ...
          const action = async (path, payload, method) => {
            ...
            try {
              await apiSend2(host, path, payload, method);
              if (owns()) await loadRecoveryView();
            } catch (error) {
              if (!owns()) return;
              status.textContent = "Action failed: " + message2(error) + ". Refresh the report to check the host\u2019s outcome before trying again.";
          ...
          row.querySelector(".recovery-restore")?.addEventListener("click", () => {
            if (owns() && !actionBusy && confirm2("Restore " + (record9.name || record9.id) + " idle? This will not replay uncertain work. Review its transcript before sending another prompt.")) {
              void action("/api/recovery/retry", { id: record9.id }, "POST");
            }
          }, listener);
        }
      } catch (error) {
        if (owns()) body.textContent = "Could not load recovery report: " + message2(error);
      }
    }
    function dispose() {
      unmountPreferences();
  ...
  function createSearchView(options2) {
    ...
    const effectiveHosts2 = options2.hosts, fanoutHosts2 = options2.fanout, scopeQuery = options2.scope;
    const hostChipHtml = (host) => options2.hostChip(host || null);
    const isMultiHost2 = () => effectiveHosts2().length > 1;
    let disposed = false;
    ...
    async function runSearchView() {
      ...
      const hosts = queryHosts(queryHosts(fanoutHosts2(), query), scope).map((host) => Object.freeze({ ...host }));
      ...
      try {
        await Promise.all(hosts.map(async (host, i) => {
          try {
            ...
            if (!r.ok) throw new Error(record2(data) && typeof data.error === "string" ? data.error : `HTTP ${r.status}`);
    ...
    function searchFacetOptions() {
      const all = [...sessionState2.sessions.active, ...sessionState2.sessions.previous];
    ...
    function renderSearchFacetsHtml() {
      ...
      const hostSelect = !isMultiHost2() ? "" : `<select class="search-facet-select" id="searchFacetHost">${['<option value="">All hosts</option>', ...effectiveHosts2().map((h) => hostDisplayLabel(h)).filter(Boolean).map((label) => `<option value="${escapeHtml(label)}"${label.toLowerCase() === st.host ? " selected" : ""}>${escapeHtml(label)}</option>`)].join("")}</select>`;
    ...
    async function openSearchResult(id, hasContentMatches, host = null, renderedQuery = searchViewRenderedQuery, endpoint = options2.host(host)) {
      ...
      if (!sessionState2.findSession(id, host)) await options2.loadPrevious();
      if (disposed || navigation !== searchViewSeq || !sameHost(captured)) return;
      const entry = sessionState2.findSession(id, host);
      if (!entry) return;
      const selecting = options2.selectSession(id, { host: entry.host || null });
      const owner = sessionState2.captureSelection(), selectedView = view;
      await selecting;
      if (tokens2.length && hasContentMatches && owner && selectedView === view && sessionState2.ownsSelection(owner) && owner.id === id && owner.host === (entry.host || null)) {
  ...
  var number3 = (value) => finite(value) ? value : 0;
  var object2 = (value) => record2(value) ? value : {};
  var numbers = (value) => Array.isArray(value) ? value.map(number3) : [];
  function decodeSkillDirectory(value) {
    if (!record2(value) || !Array.isArray(value.skills)) throw new Error("Invalid skills directory");
    ...
    return {
      ...
      summary: { discovered: number3(summary.discovered), advertised: number3(summary.advertised), catalogTokensEst: number3(summary.catalogTokensEst), activations30d: number3(summary.activations30d), quiet60d: number3(summary.quiet60d) },
      skills: value.skills.flatMap((row) => {
        if (!record2(row) || typeof row.skill !== "string" || !row.skill) return [];
        ...
        return [{
          ...
          bodyTokensEst: number3(row.bodyTokensEst),
          bodyBytes: number3(row.bodyBytes),
          catalogTokensEst: number3(row.catalogTokensEst),
          usage: {
            lastUsedTs: finite(usage.lastUsedTs) ? usage.lastUsedTs : null,
            count30d: number3(usage.count30d),
            total: number3(usage.total),
            sessionCount: number3(usage.sessionCount),
            cwdCount: number3(usage.cwdCount),
  ...
  function decodeSkillCoverage(value) {
    if (!record2(value) || typeof value.skill !== "string" || !value.skill) throw new Error("Invalid skill coverage");
    const kinds = object2(value.kindSplit), latest = object2(value.latest);
    const sections = Array.isArray(value.sections) ? value.sections : [];
    return {
      skill: value.skill,
      numMapped: number3(value.numMapped),
      excludedBeforeMtime: number3(value.excludedBeforeMtime),
      flatFullRead: value.flatFullRead === true,
      unreadTokensEst: number3(value.unreadTokensEst),
      targetedTouches: number3(value.targetedTouches),
      cwdCount: number3(value.cwdCount),
      topCwd: text7(value.topCwd),
      mtimeMs: number3(value.mtimeMs),
      weeks26: numbers(value.weeks26),
      kindSplit: { read: number3(kinds.read), explicit: number3(kinds.explicit) },
      sessionCount: number3(value.sessionCount),
      latest: typeof latest.sessionId === "string" && latest.sessionId ? { sessionId: latest.sessionId, entryId: text7(latest.entryId), name: text7(latest.name), ts: number3(latest.ts), model: text7(latest.model) } : null,
      sections: sections.flatMap((section) => {
        if (!record2(section)) return [];
        const lines = Array.isArray(section.lines) ? section.lines : [];
        return [{
          heading: text7(section.heading),
          startLine: number3(section.startLine),
          endLine: number3(section.endLine),
          reads: number3(section.reads),
          fraction: Math.max(0, Math.min(1, number3(section.fraction))),
          neverRead: section.neverRead === true,
          lines: lines.flatMap((line) => record2(line) ? [{ text: text7(line.text), hits: number3(line.hits) }] : [])
        }];
      })
    };
  }
  ...
  ${escapeHtml(covUrl)}</div>
    ...
    async function openSkillActivation(id, entryId) {
      ...
      if (!sessionState2.findSession(id, endpoint.hostId)) await options2.loadPrevious();
      if (!current() || !sessionState2.findSession(id, endpoint.hostId)) return;
      const selecting = options2.selectSession(id, { host: endpoint.hostId });
      const owner = sessionState2.captureSelection(), selectedView = skillsSeq;
      await selecting;
      if (!entryId || !owner || !sessionState2.ownsSelection(owner) || owner.id !== id || owner.host !== endpoint.hostId) return;
      activationTimer = setTimeout(() => {
        if (disposed || selectedView !== skillsSeq || !sessionState2.ownsSelection(owner)) return;
  ...
  var object3 = (value) => record2(value) ? value : {};
  var text8 = (value) => typeof value === "string" ? value : "";
  var number4 = (value) => finite(value) ? value : 0;
  function costs(value) {
    const row = object3(value);
    return Object.fromEntries(USAGE_MERGE_COST_KEYS.map((key) => [key, finite(row[key]) ? row[key] : null]));
  }
  ...
  function decodeUsageSummary(value, host) {
    if (!record2(value)) throw new Error("Invalid usage summary");
    const groups = object3(value.groups);
    const decodeGroups = (value2, kind) => Array.isArray(value2) ? value2.flatMap((row) => {
      if (!record2(row) || (kind === "sessions" ? typeof row.id !== "string" : typeof row.key !== "string")) return [];
      return [{
        ...bucket(row),
        key: text8(row.key),
        id: text8(row.id),
        name: text8(row.name),
        workspace: text8(row.workspace),
        provider: text8(row.provider),
    ...
    const daily = Array.isArray(value.daily) ? value.daily.flatMap((row) => {
      if (!record2(row) || typeof row.day !== "string") return [];
      return [{ ...bucket(row), day: row.day, days: number4(row.days) || 1, models: Array.isArray(row.models) ? row.models.flatMap((model) => {
        if (!record2(model) || typeof model.ref !== "string") return [];
        return [{
          ref: model.ref,
          provider: text8(model.provider),
          model: text8(model.model),
          calls: number4(model.calls),
          cost: finite(model.cost) ? model.cost : null,
    ...
    return {
      range: text8(value.range),
      sort: text8(value.sort),
      ...
      daily,
      headlineCosts: Object.fromEntries(Object.entries(object3(value.headlineCosts)).map(([key, value2]) => [key, finite(value2) ? value2 : null])),
  ...
  function decodeUsageLimits(value) {
    if (!record2(value) || !Array.isArray(value.harnesses)) return { harnesses: [] };
    return { harnesses: value.harnesses.flatMap((harness) => {
      if (!record2(harness)) return [];
      return [{ harness: text8(harness.harness), label: text8(harness.label), error: text8(harness.error), reports: Array.isArray(harness.reports) ? harness.reports.flatMap((report) => {
        if (!record2(report) || typeof report.provider !== "string") return [];
        return [{ provider: report.provider, planType: text8(report.planType), fetchedAt: number4(report.fetchedAt), limits: Array.isArray(report.limits) ? report.limits.flatMap((limit) => {
          if (!record2(limit) || typeof limit.label !== "string" || !finite(limit.usedFraction)) return [];
          return [{ label: limit.label, windowLabel: text8(limit.windowLabel), usedFraction: limit.usedFraction, resetsAt: finite(limit.resetsAt) ? limit.resetsAt : null }];
        }) : [] }];
      }) : [] }];
    }) };
  }
  ...
  function createUsageView(options2) {
    ...
    const localStorage2 = options2.storage, isMultiHost2 = options2.multiHost;
    const element = (id) => {
      const value = document2.getElementById(id);
    ...
    function setUsageSort(sort) {
      ...
      usageSort = sort;
      localStorage2.setItem("pi-dish-usage-sort", sort);
      loadUsageView();
    }
    function setUsageStack(stack) {
      ...
      usageStack = stack;
      localStorage2.setItem("pi-dish-usage-stack", stack);
    ...
    function usageMetricValue(bucket2, metric) {
      if (metric === "cost") return finite(bucket2.costs?.total) ? bucket2.costs.total : 0;
    ...
    function usageCostBreakdown(costs2) {
      if (!costs2 || !finite(costs2.total)) return null;
    ...
    function usageTokensDetail(tokens2) {
      ...
      const prompt2 = (t.input || 0) + (t.cacheRead || 0) + (t.cacheWrite || 0);
      if (prompt2 > 0 && (t.cacheRead || 0) > 0) parts.push(`${Math.round((t.cacheRead || 0) / prompt2 * 100)}% cached`);
      return parts.join(" \xB7 ");
    }
    ...
    function showUsageTooltip(bucket2, e) {
      ...
      if (usageChart?.stack === "buckets" && metric === "cost") {
        USAGE_COST_BUCKETS.forEach(([key, label, cls]) => {
          rows.push([cls, label, finite(bucket2.costs?.[key]) ? bucket2.costs[key] : 0]);
        });
  ...
  function decodeSessionStats(value) {
    if (!record2(value)) throw new Error("Invalid session stats");
    ...
    return {
      ...
      hardCacheMisses: number5(value.hardCacheMisses),
  ...
  function createSessionInfo(options2) {
    const { document: document2, sessionState: sessionState2 } = options2, location2 = document2.defaultView.location;
    const copyTextToClipboard3 = options2.copy, setStatus2 = options2.status, confirm2 = options2.confirm, sessionRefFor2 = options2.reference;
    ...
    function owns(owner, host) {
      if (disposed || !owner || !host || !sessionState2.ownsSelection(owner)) return false;
    ...
    async function shareFor(host, id) {
      ...
      const value = await response.json();
      if (!response.ok) throw new Error(record2(value) && typeof value.error === "string" ? value.error : `HTTP ${response.status}`);
      return decodeSessionShare(value);
    }
    async function apiSend2(host, path) {
    ...
    function openStatsModal() {
      ...
      closeStatsModal();
      statsEndpoint = host;
      const ref = sessionRefFor2(sessionState2.currentSession);
      ...
      body.addEventListener("click", (event) => {
        ...
        void copyTextToClipboard3(button.dataset.copy || "").then(() => {
          ...
          if (ownsStatsModal(owner, generation)) setStatus2("Copy failed (clipboard blocked)", "error");
        });
    ...
    async function loadShareSection(owner, generation) {
      ...
      if (!element2) return;
      if (!sessionSupports2(sessionState2.findSession(owner.id, owner.host), "export")) {
        element2.remove();
        return;
      }
    ...
    function renderShareSection(owner, share, generation) {
      ...
      const link = share ? share.url || location2.origin + share.path : "";
      ...
      button.addEventListener("click", async () => {
        ...
        try {
          ...
          if (current()) {
            button.disabled = false;
            setStatus2(`Failed to ${share ? "revoke" : "create"} share: ` + errorMessage(error), "error");
          }
        }
      }, { signal: shareEvents.signal });
    }
    async function copyMessageShareLink(button) {
      const owner = sessionState2.captureSelection(), id = button.dataset.entryId;
      ...
      try {
        ...
        if (!current()) return;
        if (!share) {
          if (!confirm2("No share link exists for this session yet \u2014 create one? Anyone with the link can view the whole session read-only.") || !current()) return;
        ...
        if (!current()) return;
        const base = share.url || location2.origin + share.path;
        await copyTextToClipboard3(`${base}?targetId=${encodeURIComponent(id)}`);
        if (!current()) return;
        button.classList.add("copied");
        later(messageTimers, () => {
          if (owns(owner, host)) button.classList.remove("copied");
        });
        setStatus2("Message share link copied");
      } catch (error) {
        if (current()) setStatus2("Share link failed: " + errorMessage(error), "error");
      } finally {
    ...
    async function finishSessionClose(sessionId, host, owner) {
      if (disposed) return;
      if (!owner || sessionState2.ownsSelection(owner)) setStatus2("Session closed");
      await options2.loadPrevious();
      if (!disposed && owner && sessionState2.ownsSelection(owner) && owner.id === sessionId && owner.host === (host || null)) {
    ...
    function renderCloseSection(owner, generation) {
      ...
      if (!el) return;
      const session = sessionState2.findSession(sessionId, owner.host);
      ...
      closeBtn.addEventListener("click", async () => {
        ...
        const warn = detach ? "Detach this client? The logical agent will continue independently." : ownedAgent ? "Stop this agent and its children? Any work in progress will be aborted; the transcript stays resumable." : sessionState2.findSession(sessionId, host)?.turnInProgress ? "A turn is in progress \u2014 closing will abort it. Close this session?" : "Close this session? The agent process will shut down (the transcript stays resumable).";
        if (!confirm2(warn) || !ownsStatsModal(owner, generation)) return;
        busy = true;
        closeBtn.disabled = true;
        closeBtn.textContent = detach ? "Detaching\u2026" : "Closing\u2026";
        try {
          await apiSend2(endpoint2, `/api/sessions/${encodeURIComponent(sessionId)}/close`);
          if (!ownsStatsModal(owner, generation)) return;
          closeStatsModal();
          ...
          setStatus2("Close failed: " + errorMessage(e), "error");
        }
      }, listener);
      ...
      restartBtn.addEventListener("click", async () => {
        if (!ownsStatsModal(owner, generation) || busy) return;
        const active = sessionState2.findSession(sessionId, host);
        ...
        setStatus2("Restarting agent\u2026", "working");
        try {
          const data = await apiSend2(endpoint2, `/api/sessions/${encodeURIComponent(sessionId)}/restart`);
          if (!record2(data) || typeof data.id !== "string" || !data.id) throw new Error("Invalid restart response");
          if (ownsStatsModal(owner, generation)) closeStatsModal();
          if (owns(owner, endpoint2)) setStatus2("Agent restarted");
          await refreshSessions();
          if (owns(owner, endpoint2) && record2(data) && typeof data.id === "string") {
          ...
          if (owns(owner, endpoint2)) {
            ...
            setStatus2("Restart failed: " + errorMessage(e), "error");
          }
        }
      }, listener);
    }
    ...
    async function loadPagesSection(owner, generation) {
      ...
      try {
        ...
        element2.innerHTML = pages.length ? '<div class="stats-share-title">Published pages</div>' + pages.map((page) => {
          const link = page.url || location2.origin + page.path, label = page.title || page.root.split("/").pop();
        ...
        element2.querySelectorAll(".stats-page-revoke").forEach((button) => {
          ...
          button.addEventListener("click", async () => {
            ...
            try {
              ...
              if (current()) {
                button.disabled = false;
                setStatus2("Failed to revoke: " + errorMessage(error), "error");
              }
            }
    ...
    let artifactsSeq = 0;
    async function refreshArtifacts(owner = sessionState2.captureSelection()) {
      if (disposed || !owner || !sessionState2.ownsSelection(owner)) return;
      const sessionId = owner.id;
      const host = endpoint(owner);
      if (!host) return;
    ...
    function renderArtifactsModal() {
      ...
      if (pages.length) {
        html += '<div class="stats-share-title">Published pages</div>' + pages.map((p) => {
          const link = p.url || location2.origin + p.path;
      ...
      if (share) {
        const link = share.url || location2.origin + share.path;
...
`;
  ...
  function createFileViews(options2) {
    ...
    function owns(view, kind, id, generation) {
      if (disposed || !view.owner || !view.endpoint || view.sessionId !== id || view.generation !== generation || !isOpen(kind) || !sessionState2.ownsSelection(view.owner)) return false;
    ...
    async function loadDiff() {
      if (disposed || !sessionState2.currentSession || !isOpen("diff")) return;
  ...
  function decodeAnchoredComments(value) {
    return Array.isArray(value) ? value.flatMap((v) => {
      if (!record2(v) || typeof v.id !== "string" || typeof v.sessionId !== "string" || typeof v.body !== "string") return [];
  ...
  function decodeCommentIndex(value) {
    return record2(value) && Array.isArray(value.comments) ? value.comments.flatMap((v) => {
      if (!record2(v) || typeof v.id !== "string") return [];
      const target = decodeCommentTarget(v.target);
  ...
  function createAnchoredComments(options2) {
    ...
    function position() {
      ...
      const viewport = window2.visualViewport, left = viewport?.offsetLeft || 0, top = viewport?.offsetTop || 0;
      const width = viewport?.width || window2.innerWidth, height = viewport?.height || window2.innerHeight, margin = 8, gap = 8;
    ...
    async function refresh() {
      ...
      try {
        ...
        if (!current()) return;
        set(record2(full) ? full.comments : []);
      } catch {
      }
    }
    ...
    function toggleList(chip) {
      ...
      popover.style.left = `${Math.max(8, Math.min(window2.innerWidth - popover.offsetWidth - 8, rect.left))}px`;
    ...
    function mount() {
      ...
      document2.addEventListener("click", (event) => {
        ...
        const marked2 = target.closest("mark.comment-mark, .diff-line.comment-line");
        const entry = marked2 ? marks.get(marked2) : null;
        if (marked2 && entry && marked2.isConnected && owns(entry.view) && comments.includes(entry.comment) && window2.getSelection()?.isCollapsed !== false) {
          openEditor(entry.comment, marked2);
          return;
        }
      ...
      window2.addEventListener("resize", reposition, { signal });
      window2.visualViewport?.addEventListener("resize", reposition, { signal });
      window2.visualViewport?.addEventListener("scroll", reposition, { signal });
  ...
  var cursor = (value) => finite(value) && Number.isInteger(value) && value >= 0 ? value : null;
  function decodeTranscriptPage(value) {
    if (!record2(value) || !Array.isArray(value.messages)) throw new Error("Invalid transcript page");
    const session = record2(value.session) ? decodeSessionTranscriptPatch(value.session) : {};
  ...
  function createTranscript(options2) {
    ...
    async function page(owner, suffix) {
      ...
      try {
        ...
        const value = await response.json();
        if (!response.ok) throw new Error(record2(value) && typeof value.error === "string" ? value.error : `Transcript request failed (${response.status})`);
    ...
    async function load(selection = sessionState2.captureSelection()) {
    ...
    async function catchup(selection = sessionState2.captureSelection()) {
      const owner = capture(selection);
      if (!owner) return;
  ...
  function decode(value) {
    if (!record2(value) || typeof value.toolCallId !== "string" || !value.toolCallId) return null;
    return {
      toolCallId: value.toolCallId,
      toolName: typeof value.toolName === "string" ? value.toolName : void 0,
      args: value.args,
      startedAt: typeof value.startedAt === "string" || finite(value.startedAt) ? value.startedAt : void 0,
      partialResult: record2(value.partialResult) ? { content: value.partialResult.content } : void 0,
      result: record2(value.result) ? { content: value.result.content } : void 0,
  ...
  function createLiveTools(options2) {
    ...
    const owns = (owner) => !disposed && sessionState2.ownsSelection(owner);
    function lookup(id) {
      const owner = sessionState2.captureSelection(), container = document2.getElementById("messages");
    ...
    function appendLiveToolPanel(data, { completionOnly = false } = {}) {
      ...
      const parsedStartedAt = finite(data.startedAt) ? data.startedAt : typeof data.startedAt === "string" ? Date.parse(data.startedAt) : NaN;
      const entry = {
        owner,
        el,
    ...
    function updateLiveToolPanel(data) {
      if (disposed || !sessionState2.captureSelection()) return;
  ...
  function createMessageStream(options2) {
    const { document: document2, sessionState: sessionState2, renderer, tools } = options2;
    let source = null, reconnect = null, connectionGeneration = 0, disposed = false;
    function ownsConnection(owner, target, generation) {
      return !disposed && generation === connectionGeneration && sessionState2.ownsSelection(owner) && options2.endpoint(owner.host).base === target.base;
    }
    function stop() {
    ...
    async function start(owner = sessionState2.captureSelection()) {
      if (disposed || !owner || !sessionState2.ownsSelection(owner)) return;
      stop();
      ...
      try {
        ...
        addOwnedListener("compaction_end", (e) => {
          ...
          try {
            ...
            const r = record2(data.result) ? data.result : null;
        ...
        addOwnedListener("session_tree", () => {
          if (sessionState2.currentSession && sessionState2.currentSession.id === sessionId) {
        ...
        addOwnedListener("session_switch", (e) => {
          ...
          const nextId = record2(data) && typeof data.sessionId === "string" ? data.sessionId : "";
          ...
          void options2.loadSessions(void 0, { withPrevious: true }).then(() => {
            if (!ownsStream() || switchOwner !== switchSequence || !sessionState2.findSession(nextId, hostId)) return;
  ...
  function decodeComposerImages(value) {
    return Array.isArray(value) ? value.flatMap((image) => record2(image) && typeof image.data === "string" && typeof image.mimeType === "string" && image.mimeType.startsWith("image/") ? [{ data: image.data, mimeType: image.mimeType }] : []) : [];
  }
...
${restored}`;
  ...
  function createComposerSpeech(options2) {
    const { document: document2, sessionState: sessionState2 } = options2, window2 = document2.defaultView, navigator2 = window2.navigator;
    ...
    const errorName = (e) => record2(e) && typeof e.name === "string" ? e.name : "unknown";
    function capture() {
      const key = options2.composerKey();
      return !disposed && key ? { key, selection: sessionState2.captureSelection() } : null;
    }
    function owns(owner) {
      return !disposed && options2.composerKey() === owner.key && (!owner.selection || sessionState2.ownsSelection(owner.selection));
    }
    function host() {
      return options2.hosts().find((entry) => record2(entry.capabilities) ? entry.capabilities.stt === true : (entry.self === true || entry.base === "") && !!options2.config().stt) || null;
    }
    function reason() {
      return sttUnavailableReason({ isSecureContext: !!window2.isSecureContext, hasGetUserMedia: !!navigator2.mediaDevices?.getUserMedia, hasMediaRecorder: typeof window2.MediaRecorder === "function", origin: window2.location.origin });
    }
    ...
    async function start() {
      ...
      let stream;
      try {
        stream = await navigator2.mediaDevices.getUserMedia({ audio: true });
      } catch (error) {
      ...
      entry.stream = stream;
      const mime = MIME_TYPES.find((type) => typeof window2.MediaRecorder.isTypeSupported === "function" && window2.MediaRecorder.isTypeSupported(type));
      let recorder;
      try {
        recorder = mime ? new window2.MediaRecorder(stream, { mimeType: mime }) : new window2.MediaRecorder(stream);
      } catch {
        try {
          recorder = new window2.MediaRecorder(stream);
        } catch (error) {
          retire(entry);
  ...
  function decodeQueueData(value) {
    ...
    return { steering: strings(record2(value) ? value.steering : null), followUp: strings(record2(value) ? value.followUp : null) };
  }
  function createPromptDelivery(options2) {
    const { document: document2, sessionState: sessionState2 } = options2, pending = /* @__PURE__ */ new Map();
    ...
    function canCancelQueue() {
      const caps = sessionState2.currentSession?.capabilities;
      return !record2(caps) || caps.queueCancel !== false;
    }
    function owns(row) {
      return !disposed && row.generation === generation && sessionState2.ownsSelection(row.owner) && options2.endpoint(row.owner.host).base === row.endpoint.base;
    }
    ...
    async function edit(button) {
      ...
      cancelling.add(row);
      const prompt2 = row.clientId ? pending.get(row.clientId) : void 0, previous = prompt2?.status;
      if (prompt2) prompt2.status = "cancelling";
      ...
      try {
        ...
        if (disposed) return;
        if (prompt2 && row.clientId && pending.get(row.clientId) === prompt2 && previous) prompt2.status = previous;
        if (sessionState2.ownsSelection(row.owner) && options2.endpoint(row.owner.host).base === row.endpoint.base) {
          render(data);
  ...
  function createSessionActivity(options2) {
    ...
    function setTurnInProgress(active) {
      if (disposed) return;
      owner = sessionState2.captureSelection();
      ...
      if (sessionState2.currentSession && !!sessionState2.currentSession.turnInProgress !== !!active) {
        sessionState2.patchSessionActivity(sessionState2.currentSession.id, { turnInProgress: !!active }, owner?.host);
      }
    ...
    function setCompacting(active) {
      ...
      syncActivityIndicator();
      if (sessionState2.currentSession && !!sessionState2.currentSession.compacting !== on) {
        sessionState2.patchSessionActivity(sessionState2.currentSession.id, { compacting: on }, owner?.host);
      }
    }
  ...
  function createComposerSubmit(options2) {
    ...
    async function send(endpoint, path, body) {
      const value = await sendJson(options2.request, endpoint, path, body), data = record2(value) ? value : {};
      return { info: typeof data.info === "string" ? data.info : "", answer: typeof data.answer === "string" ? data.answer : "", result: { queued: record2(data.result) && data.result.queued === true } };
    }
    async function sendPrompt() {
      ...
      if (options2.spawnId()) {
        if (message3 || composerDrafts2.images.current().length) {
      ...
      if (!message3 && !composerDrafts2.images.current().length || !sessionState2.currentSession) return;
      const owner = sessionState2.captureSelection();
      if (!owner || disposed) return;
      const endpoint = Object.freeze({ ...options2.endpoint(owner.host) }), sequence = ++feedbackSequence;
      const owns = () => !disposed && sequence === feedbackSequence && sessionState2.ownsSelection(owner) && options2.endpoint(owner.host).base === endpoint.base;
      ...
      if (message3.startsWith("/")) {
        if (sessionActivity2.compacting && /^\/compact(\s|$)/.test(message3)) {
          options2.status("Compaction already in progress", "error");
          return;
        }
        input.value = "";
        input.style.height = "";
        composerDrafts2.record(message3, ownerKey);
        composerDrafts2.clearDraft(ownerKey);
        options2.status("Running " + message3.split(" ")[0] + "...", "working");
        const btwQuestion = message3.match(/^\/btw\s+([\s\S]*)$/)?.[1]?.trim();
        const btwOwner = btwQuestion ? btwPanel2.show(btwQuestion) : null;
        try {
          const data = await send(endpoint, `/api/sessions/${encodeURIComponent(sessionId)}/command`, { message: message3 });
          if (disposed || !sessionState2.ownsSelection(owner) || options2.endpoint(owner.host).base !== endpoint.base) return;
          if (btwQuestion) {
            if (typeof data.answer === "string" && data.answer) btwPanel2.resolve(data.answer, btwOwner);
            else btwPanel2.fail("(no answer)", btwOwner);
          }
          ...
          composerDrafts2.restorePayload(ownerKey, message3, null);
          if (btwQuestion && options2.endpoint(owner.host).base === endpoint.base) btwPanel2.fail(e.message, btwOwner);
          if (owns()) {
      ...
      composerDrafts2.record(message3, ownerKey);
      composerDrafts2.clearDraft(ownerKey);
      const images = composerDrafts2.images.take();
      ...
      try {
        ...
        if (resp?.result?.queued) {
          sessionActivity2.setCompacting(true);
          sessionActivity2.setTurn(false);
        ...
        composerDrafts2.restorePayload(ownerKey, message3, images);
        if (owns()) {
    ...
    async function sendQueuedMessage(kind) {
      ...
      if (!message3 && !composerDrafts2.images.current().length || !sessionState2.currentSession || !sessionState2.currentSession.isActive) return;
      const owner = sessionState2.captureSelection();
      if (!owner || disposed) return;
      const endpoint = Object.freeze({ ...options2.endpoint(owner.host) }), sequence = ++feedbackSequence;
      const owns = () => !disposed && sequence === feedbackSequence && sessionState2.ownsSelection(owner) && options2.endpoint(owner.host).base === endpoint.base;
    ...
    async function abortTurn() {
      if (disposed) return;
      if (!sessionState2.currentSession || !sessionActivity2.turn && !sessionActivity2.compacting) return;
      const owner = sessionState2.captureSelection();
      if (!owner || disposed) return;
      const endpoint = Object.freeze({ ...options2.endpoint(owner.host) }), sequence = ++feedbackSequence;
      const owns = () => !disposed && sequence === feedbackSequence && sessionState2.ownsSelection(owner) && options2.endpoint(owner.host).base === endpoint.base;
      const { id: sessionId } = owner;
      const ownerKey = sessionRefKey(owner);
      if (sessionActivity2.isAborting(ownerKey)) return;
      const abortOwner = sessionActivity2.beginAbort(ownerKey);
      ...
      try {
        ...
        if (disposed) return;
        const e = { message: error instanceof Error ? error.message : String(error) };
        sessionActivity2.endAbort(ownerKey, abortOwner);
  ...
  function decodeSpawnId(value) {
    if (!record5(value) || typeof value.spawnId !== "string" || !value.spawnId) throw new Error("Failed to start session");
    ...
    async function monitor(key, spawn) {
      try {
        ...
        for (let attempt = 0; ; attempt++) {
          ...
          const data = await response.json().catch(() => null);
          if (!response.ok && response.status !== 202) throw new Error(record5(data) && typeof data.error === "string" && data.error ? data.error : `spawn status failed (${response.status})`);
  ...
  function decodeSpawnChoices(value) {
    if (!record6(value) || !value.available || !Array.isArray(value.servers)) return [HEADLESS];
    const values = value.servers;
    const servers = values.flatMap((server) => record6(server) && typeof server.name === "string" && typeof server.socket === "string" && server.socket ? [{ name: server.name, socket: server.socket, sessions: server.sessions }] : []);
    ...
    for (const server of servers) {
      ...
      for (const session of sessions) if (record6(session) && typeof session.name === "string" && session.name) {
        choices.push({
  ...
  function decodeHarnessConfigPreview(value, cwd) {
    if (!record7(value)) throw new Error("Invalid harness defaults");
    const roles = record7(value.modelRoles) ? Object.fromEntries(Object.entries(value.modelRoles).filter((entry) => typeof entry[1] === "string")) : {};
    return {
      cwd,
  ...
  function createNewSessionConfigPreview(options2) {
    ...
    async function load(cwd) {
      ...
      try {
        ...
        const data = await response.json();
        if (!owns()) return;
        if (!response.ok) throw new Error(record7(data) && typeof data.error === "string" && data.error ? data.error : `HTTP ${response.status}`);
  ...
  function booleanRecord(value) {
    const result = Object.fromEntries(record8(value) ? Object.entries(value).filter((entry) => typeof entry[1] === "boolean") : []);
    Object.setPrototypeOf(result, null);
    return result;
  }
  ...
  function createHarnessSettings(options2) {
    const { root, escapeHtml: escapeHtml2, shortCwd: shortCwd2, parseModelRoleRef: parseModelRoleRef2, composeModelRoleRef: composeModelRoleRef2, modelRoleLevels: modelRoleLevels2 } = options2;
    ...
    function modelRoleLevelOptions(level, modelSelector, keepUnknown) {
      ...
      const levels = modelRoleLevels2(entry);
    ...
    function renderModelRoles() {
      ...
      body.innerHTML = rows.map((row) => {
        const { model, level } = parseModelRoleRef2(row.value, known);
    ...
    function collectHarnessSettingsPatch() {
      ...
      for (const select of Array.from(root.querySelectorAll("#modelRolesBody .model-role-select"))) {
        ...
        roles[select.dataset.role] = select.value ? composeModelRoleRef2(select.value, level) : null;
      }
  ...
  function createStreamingRenderer(options2) {
    ...
    function flush() {
      ...
      const frame = pending;
      pending = null;
      if (disposed || !frame || !sessionState2.ownsSelection(frame.owner)) return;
      try {
    ...
    function renderStreamingMessage(message3, owner = sessionState2.captureSelection()) {
      if (disposed || !sessionState2.ownsSelection(owner)) return;
      const container = document2.getElementById("messages");
      if (!container) return;
      const wasPinned = options2.pinned(container);
  ...
  function createExtensionDialogs(options2) {
    ...
    function matches(entry) {
      const selected = sessionState2.currentSession, host = options2.host(entry.hostId);
    ...
    function currentCard(key, card) {
      ...
      return !disposed && !!entry && entry.el === card && card.isConnected && matches(entry) && sessionState2.ownsSelection(entry.owner);
    }
    ...
    function dockExtDialog(entry) {
      ...
      if (!dock) return;
      entry.owner = sessionState2.captureSelection();
    ...
    function sendExtDialogResponse(dialogKey, response, card) {
      ...
      void sendJson(options2.request, Object.freeze({ ...host }), `/api/sessions/${encodeURIComponent(entry.sessionId)}/ui-response`, { requestId: entry.requestId, ...response }).catch((error) => {
        if (!disposed && sessionState2.ownsSelection(owner)) options2.status("Dialog response failed: " + (error instanceof Error ? error.message : String(error)), "error");
      });
    ...
    <div class="ext-ui-dialog-actions">
      ...
      const entry = { el: card, events, endpoint: Object.freeze({ ...endpoint }), owner: sessionState2.captureSelection(), sessionId, hostId, sessionKey: sessionKey(hostId, sessionId), requestId: req.id, minimized: false, sig: extDialogSig(req) };
    ...
    return {
      ...
      reconcile(value, session) {
        if (disposed || !record2(value) || !Array.isArray(value.dialogs)) return;
  ...
  function createExtensionDisplay(options2) {
    const { document: document2, sessionState: sessionState2, storage } = options2;
    ...
    const current = (owner) => !disposed && sessionState2.ownsSelection(owner);
    ...
    function widget(key, lines, placement) {
      ...
      const owner = sessionState2.captureSelection(), collapsedKey = sessionRefKey(owner) + "|" + key;
      if (!entry) {
...
(status from ${key})`;
  ...
  function createDiagrams(options2) {
    const { document: document2 } = options2, window2 = document2.defaultView;
    ...
    function loadMermaid() {
      ...
      mermaidPromise = options2.assets.load("script", { src: "vendor/mermaid.min.js" }).then(() => {
        const mermaid2 = options2.runtime();
        if (!mermaid2) throw new Error("mermaid did not load");
        if (disposed) throw new Error("Diagrams disposed");
        mermaid2.initialize(mermaidConfig());
        return mermaid2;
      }).catch((err) => {
        mermaidPromise = null;
        throw err;
    ...
    function mermaidConfig() {
      const css = window2.getComputedStyle(document2.documentElement);
      ...
      return {
        ...
        theme: "base",
        fontFamily: window2.getComputedStyle(document2.body).fontFamily,
  ...
  function createRichText(options2) {
    ...
    document2.addEventListener("click", (event) => {
      ...
      if (copy) {
        const owner = sessionState2.captureSelection(), token = /* @__PURE__ */ Symbol();
        copies.set(copy, token);
        const current = () => !disposed && copy.isConnected && copies.get(copy) === token && (owner ? sessionState2.ownsSelection(owner) : !sessionState2.currentSession);
    ...
    options2.marked?.use({
      ...
      renderer: {
        html(html) {
          return escapeHtml(typeof html === "string" ? html : record2(html) && typeof html.text === "string" ? html.text : "");
        }
      },
      walkTokens(token) {
    ...
    function applyHighlight(el) {
      ...
      root.querySelectorAll(".markdown-body pre code").forEach((code) => {
        ...
        const hljs2 = options2.highlight();
        if (!hljs2) {
          pendingHighlight.push({ code, source: code.textContent || "" });
          return;
        }
        try {
          hljs2.highlightElement(code);
        } catch (e) {
        }
      });
      if (pendingHighlight.length) {
        loadHighlightAssets().then((hljs2) => {
          if (disposed) return;
  ...
  function decodeTranscriptTree(value) {
    ...
    return {
      ...
      nodes: value.nodes.flatMap((node) => {
        if (!record2(node) || typeof node.id !== "string" || !node.id) return [];
        return [{
          id: node.id,
  ...
  function createTranscriptTree(options2) {
    const { document: document2, sessionState: sessionState2, storage, status: setStatus2 } = options2;
    const element = (id) => {
      const value = document2.getElementById(id);
    ...
    async function openTreeModal() {
      ...
      treeOwner = owner;
      treeEndpoint = host;
      setStatus2("Loading tree...", "working");
      try {
        ...
        if (ownsView(owner, host, generation)) setStatus2("Failed to load tree: " + errorMessage(error), "error");
      }
    }
    ...
    function selectTreeNode(entryId) {
      ...
      const allowInstructions = sessionState2.currentSession?.harnessId !== "omp";
    ...
    async function confirmBranch() {
      ...
      setStatus2(summarize ? "Summarizing abandoned branch\u2026" : "Branching...", "working");
      try {
        ...
        if (!disposed && record2(data) && typeof data.editorText === "string" && data.editorText) options2.saveEditorDraft(owner, data.editorText);
        if (operation !== operationGeneration || !ownsSelection(owner, host)) return;
        closeTreeModal();
        setStatus2("Branched \u2014 reloading");
        await options2.selectSession(owner.id, { host: owner.host, forceTranscriptReload: true });
      } catch (error) {
        if (operation !== operationGeneration || !ownsSelection(owner, host)) return;
        setStatus2("Branch failed: " + errorMessage(error), "error");
  ...
  function decodeThemes(value) {
    if (!record2(value) || !Array.isArray(value.themes)) return [];
    return value.themes.flatMap((row) => record2(row) && typeof row.id === "string" && row.id ? [{ id: row.id, builtin: row.builtin === true, tokens: decodeThemeTokens(row.tokens) }] : []);
  }
  ...
  function decodeTerminalOutput(value) {
    ...
    switch (value.type) {
      ...
      case "exit":
        return { type: "exit", code: finite(value.code) ? value.code : null };
      case "error":
  ...
  function createTerminalController(options2) {
    ...
    function owns(value) {
      return !disposed && value === state && sessionState2.ownsSelection(value.owner) && sameHost(value.owner, value.endpoint);
    }
    function modeKey(id, host = sessionState2.sessionHostId(id)) {
    ...
    function updateButtons() {
      if (disposed) return;
      const show = options2.supportsTerminal(sessionState2.currentSession) && sessionState2.currentSession?.isActive === true;
    ...
    function updateMode() {
      ...
      if (button) {
        button.style.display = state && options2.supportsTmux(sessionState2.currentSession) && sessionState2.currentSession?.isActive === true ? "" : "none";
    ...
    async function open(mode) {
      if (disposed || state || !sessionState2.currentSession || !options2.supportsTerminal(sessionState2.currentSession)) return;
      cancelOpen?.();
      const own = ++generation;
      const session = sessionState2.currentSession, owner = sessionState2.captureSelection();
      if (!owner) return;
      ...
      cancelOpen = cancel;
      const current = () => !disposed && own === generation && !state && sessionState2.ownsSelection(owner) && sameHost(owner, endpoint);
      let fontTimer;
      try {
        try {
        ...
        if (!current()) return;
        const css = window2.getComputedStyle(document2.documentElement);
        const term = options2.createTerminal({ fontFamily: css.getPropertyValue("--font-mono").trim() + ", 'Symbols Nerd Font Mono'", fontSize: window2.innerWidth <= 768 ? 12 : 13, theme: options2.theme(), scrollback: 5e3, cursorBlink: true });
        if (!term) return;
        ...
        window2.addEventListener("resize", fit, { signal: next.events.signal });
        window2.visualViewport?.addEventListener("resize", fit, { signal: next.events.signal });
        connect();
        term.focus();
      } finally {
        clearTimeout(fontTimer);
    ...
    function switchMode() {
      ...
      if (mode === "tmux" && !options2.supportsTmux(sessionState2.currentSession)) return;
  ...
  function createPanelResize(options2) {
    const { document: document2, storage } = options2, window2 = document2.defaultView;
    const events = new AbortController();
    ...
    function sidebarWidth() {
      ...
      sidebar.style.width = Number.isFinite(saved) ? clampSidebarWidth(saved, window2.innerWidth) + "px" : "";
    }
    ...
    function mount(kind) {
      ...
      handle.addEventListener("pointerdown", (event) => {
        ...
        handle.addEventListener("pointermove", (move) => {
          ...
          if (kind === "sidebar") panel.style.width = clampSidebarWidth(startWidth + move.clientX - startX, window2.innerWidth) + "px";
          else {
  ...
  function decodeRoutine(value, host) {
    if (!record2(value)) throw new Error("Invalid routine response");
    const row = record2(value.routine) ? value.routine : value;
    if (typeof row.id !== "string" || !row.id) throw new Error("Invalid routine identity");
    const stats = record2(row.stats) ? row.stats : {};
  ...
  function decodeRoutineList(value, host) {
    if (!record2(value) || !Array.isArray(value.routines)) throw new Error("Invalid routines response");
  ...
  function createRoutinesView(options2) {
    const document2 = options2.root.ownerDocument, window2 = document2.defaultView, location2 = window2.location;
    const localStorage2 = options2.storage, sessionState2 = options2.sessionState;
    ...
    const isMultiHost2 = options2.multiHost, hostChipHtml = options2.hostChip;
    const copyTextToClipboard3 = options2.copy, setStatus2 = options2.status, confirm2 = options2.confirm;
    const createCwdAutocomplete3 = options2.autocomplete;
    ...
    async function httpError(response) {
      const data = await response.json().catch(() => null);
      return record2(data) && typeof data.error === "string" ? data.error : `HTTP ${response.status}`;
    }
    function payloadError(data, status) {
      return record2(data) && typeof data.error === "string" ? data.error : `HTTP ${status}`;
    }
    let routinesList = [];
    ...
    function routinesViewEscape() {
      ...
      if (view && view.classList.contains("detail-open") && window2.matchMedia("(max-width: 768px)").matches) {
        backToRoutinesList();
        return true;
      }
      closeRoutinesView();
    ...
    function confirmLeaveRoutineForm() {
      ...
      return confirm2("This routine has unsaved changes. Discard them?");
    }
    ...
    function routineKnownCwds() {
      ...
      const seen = /* @__PURE__ */ new Set();
      const out = [];
      for (const s of [...sessionState2.sessions.active, ...sessionState2.sessions.previous]) {
        if (isMultiHost2() && (s.host || null) !== hostId) continue;
    ...
    async function loadRoutineHarnesses(hostId) {
      ...
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          if (record2(data) && Array.isArray(data.harnesses) && data.harnesses.length) list = data.harnesses.flatMap((row) => record2(row) && typeof row.id === "string" ? [{ id: row.id, label: typeof row.label === "string" ? row.label : row.id, available: row.available !== false }] : []);
        }
      } catch {
      }
    ...
    function routineFormDefaults() {
      ...
      return {
        ...
        description: "",
        harness: "pi",
        cwd: localStorage2.getItem("pi-dish-cwd") || "",
        model: "",
        thinking: "",
        cron: "",
        enabled: true,
    ...
    function wireRoutineForm(values, baseline = true) {
      ...
      if (cwdInput && cwdDropdown) {
        routineCwdAutocomplete = createCwdAutocomplete3({
          input: cwdInput,
    ...
    function routineInvokeUrl(routine) {
      ...
      try {
        return new URL(base + path, location2.origin).href;
      } catch {
        return base + path;
      }
    }
    ...
    function copyRoutineCurl() {
      ...
      copyTextToClipboard3(routineInvokeCurl(routineSelected));
      setStatus2("Invoke command copied");
    }
    ...
    function routineSessionLabel(sessionId) {
      const known = sessionState2.findSession(sessionId, routineSelected?.host || null);
    ...
    async function openRoutineSession(sessionId, host) {
      ...
      const resolved = options2.host(host);
      if (!resolved) return;
      const selection = sessionState2.captureSelection(), endpoint = Object.freeze({ ...resolved });
      closeRoutinesView();
      const view = viewGeneration;
      if (!sessionState2.findSession(sessionId, host)) await options2.loadPrevious();
      if (disposed || view !== viewGeneration || isRoutinesViewOpen() || !sameHost(host, endpoint) || (selection ? !sessionState2.ownsSelection(selection) : sessionState2.currentSession !== null)) return;
    ...
    async function runRoutineNow() {
      ...
      try {
        ...
        const data = await res.json().catch(() => ({}));
        if (!res.ok) {
          const extra = res.status === 429 && record2(data) && typeof data.retryAfterSec === "number" ? ` (retry in ${data.retryAfterSec}s)` : "";
  ...
  function decodeBounceOperation(value) {
    if (!record2(value) || typeof value.id !== "string" || !value.id || !Array.isArray(value.targets) || value.mode !== "reload" && value.mode !== "restart") throw new Error("Invalid operation response");
    return {
      id: value.id,
      mode: value.mode,
  ...
  function decodeBounceOperations(value) {
    if (!record2(value) || !Array.isArray(value.operations)) throw new Error("Invalid operations response");
    return value.operations.map(decodeBounceOperation);
  }

  // src/browser/bounce.ts
  function createBounce(options2) {
    const document2 = options2.document, apiFetch = options2.request, sessionState2 = options2.sessionState;
    const effectiveHosts2 = options2.hosts, refreshSessions = options2.refreshSessions, selectSession = options2.selectSession;
    const element = (id) => {
      const value = document2.getElementById(id);
    ...
    function sameHost(host) {
      return effectiveHosts2().some((current) => current.hostId === host.hostId && current.base === host.base && (current.token || "") === (host.token || ""));
    }
    let bounceHosts = [];
    ...
    async function refreshBounceView() {
      ...
      bounceHosts = effectiveHosts2().map((host) => ({
    ...
    async function submitBounceTargets() {
      ...
      await Promise.allSettled(snapshot.map(async ({ state, sessionIds }) => {
        try {
          ...
          const data = await res.json().catch(() => ({}));
          if (!res.ok) throw new Error(record2(data) && typeof data.error === "string" && data.error || `HTTP ${res.status}`);
          const operation = decodeBounceOperation(record2(data) ? data.operation : null);
          ++state.readSeq;
    ...
    async function cancelBounceWaiting(state, operation) {
      ...
      try {
        ...
        const data = await res.json().catch(() => ({}));
        if (!res.ok) throw new Error(record2(data) && typeof data.error === "string" && data.error || `HTTP ${res.status}`);
        const updated = decodeBounceOperation(record2(data) ? data.operation : null);
        ++state.readSeq;
    ...
    async function reconcileBounceRestarts(state, operations, submitted = false) {
      ...
      if (affected && owner && id && !sessionState2.findSession(id, owner.host)) await options2.loadPrevious();
      if (!bounceHostElement(state)) return;
      for (const { key } of completed) bouncePendingRestarts.delete(key);
      if (!affected || !id || !owner || !sessionState2.ownsSelection(owner)) return;
  ...
  // src/browser/app.ts
  var hostView = createHostView();
  function appRecord(value) {
    return !!value && typeof value === "object" && !Array.isArray(value);
  }
  var HOSTS_KEY = "pi-dish-hosts";
  var KEYS_MIGRATED_KEY = "pi-dish-keys-migrated";
  var hostDirectory = createHostDirectory({
  ...
  var apiTransport = createHostTransport({ resolveHost: (...args) => hostDirectory.resolveHost(...args), fetch: (...args) => fetch(...args) });
  var sessionApi = createSessionApi((...args) => apiTransport.request(...args));
  ...
  var hostConnections = createHostConnections({ onChange: () => renderHostsSection() });
  ...
  var HOST_COLORS_KEY = "pi-dish-host-colors";
  var HOST_COLOR_ORDER_KEY = "pi-dish-host-color-order";
  var hostPresentation = createHostPresentation({
    ...
    isDown: (...args) => hostConnections.isDown(...args)
  });
  ...
  var responseDetailsController = createResponseDetails({ document, sessionState, mode: () => displayPreferences.responseMode });
  var appChrome = createAppChrome({ document, storage: localStorage, older: (container) => transcriptController.maybeOlder(container) });
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    promptInput.addEventListener("keydown", (e) => {
      ...
      if (!composerAutocomplete.visible && e.key === "ArrowUp" && promptInput.selectionStart === 0 && promptInput.selectionEnd === 0) {
        if (composerDrafts.navigate(-1, promptInput)) {
          e.preventDefault();
          return;
        }
      }
      ...
      if (e.key === "Enter" && !e.shiftKey) {
        if (e.ctrlKey) {
          e.preventDefault();
          composerSubmit.sendSteer();
        } else {
          e.preventDefault();
          composerSubmit.sendPrompt();
        }
      }
      if (e.key === "Escape" && !composerAutocomplete.visible && !composerSpeech.isRecording() && sessionActivity.turn) {
        e.preventDefault();
        composerSubmit.abortTurn();
      }
    });
    ...
    document.getElementById("imageFileInput").addEventListener("change", (e) => {
      ...
      composerDrafts.images.add(input.files || []);
      input.value = "";
    });
  ...
  var composerAutocomplete = createComposerAutocomplete({
    document,
    sessionState,
  ...
  var sidebarQuery = createSidebarQuery({
    ...
    reload: (query) => sidebarLists.load(query),
    queriedFor: () => sidebarLists.queriedFor,
    invalidateLists: () => sidebarLists.invalidate(),
    busy: (value) => sidebarLists.busy(value),
    searchChanged: () => {
      if (searchViewController.isOpen()) searchViewController.run();
    },
    openSearch: (query) => searchViewController.open(query),
    prompt: (label, initial) => window.prompt(label, initial),
    alert: (message3) => alert(message3)
  });
  sidebarQuery.mount();
  var sidebarLists = createSidebarLists({
    document,
    request: (host, path, options2) => apiTransport.request(host, path, options2),
    sessionState,
  ...
  var sidebarControls = createSidebarControls({
    document,
    storage: localStorage,
    sessionState,
    request: (host, path, options2) => apiTransport.request(host, path, options2),
    host: hostEntryFor,
    render: () => renderSessions(),
    closeSidebar: () => sidebarQuery.close(),
    select: (id, host) => sessionView.select(id, { host }),
    pending: (id) => sessionView.pending(id),
    create: (cwd, host) => newSessionController.create(cwd, host),
  ...
  function sessionRefFor(session) {
    return session?.id ? sessionRef(session, hostEntryFor(session.host || null), sessionReferences.prefix(session)) : "";
  }
  ...
  var sessionView = createSessionView({
    ...
    cancelStreaming: () => streamingRenderer.cancel(),
    stopFollowing: () => {
    ...
    closeControls: () => appChrome.closePanel(),
    hideAutocomplete: () => composerAutocomplete.hide(),
    retireModels: () => modelCatalog.retire(),
    retireCommands: () => composerAutocomplete.retireCommands(),
    queue: (data) => promptDelivery.render(data),
    closeBtw: () => btwPanel.close(),
    resetArtifacts: () => sessionInfo.resetArtifacts(),
    thinking: () => sessionControls.updateThinking(),
    terminal: () => terminalController.updateButtons(),
    mic: () => composerSpeech.updateButton(),
    mood: (description, face) => moodController.set(description, face),
    status: (message3, type) => setStatus(message3, type),
    render: () => renderSessions(),
    cancelRecording: () => composerSpeech.cancel(),
    hideNote: () => composerNotes.hide(),
    math: () => richText.loadMath(),
    reveal: (id, host) => sidebarControls.reveal(id, host),
    seen: (session) => sidebarActivity.mark(session),
    artifacts: (owner) => sessionInfo.refreshArtifacts(owner),
    header: () => sessionHeader.update(),
    relations: (owner) => sessionRelationsController.load(owner),
    models: (id, harness) => appModels.load(id, harness),
    commands: (id) => composerAutocomplete.loadCommands(id)
  });
  var sessionResume = createSessionResume({
    document,
    sessionState,
    request: (...args) => apiTransport.request(...args),
    endpoint: (...args) => hostDirectory.resolveHost(...args),
    target: (host) => savedResumeTarget(host),
    refresh: () => sidebarLists.refresh(),
    select: (id, options2) => sessionView.select(id, options2),
    status: (message3, type) => setStatus(message3, type)
  });
  var modelCatalog = createModelCatalog({
    read: (scope) => sessionApi.models(scope.host, scope),
    persist: (scope, models) => localStorage.setItem(modelsCacheKey2(scope.harnessId, scope.host.hostId), JSON.stringify(models)),
  ...
  var sessionHeader = createSessionHeader({
    document,
    sessionState,
    multi: isMultiHost,
    host: hostEntryFor,
    down: hostIdIsDown,
    color: (...args) => hostPresentation.colorFor(...args),
    label: hostLabelFor,
  ...
  var sessionControls = createSessionControls({
    document,
    sessionState,
  ...
  var sessionSearch = createSessionSearch({
    document,
    sessionState,
    request: (host, path, init) => apiTransport.request(host, path, init),
    endpoint: hostEntryFor,
    ...
    updateJumpButton: (...args) => appChrome.jump(...args)
  });
  function searchPrev() {
  ...
  var displayPreferences = createDisplayPreferences({
    document,
    storage: localStorage,
    request: (host, url, options2) => apiTransport.request(host, url, options2),
    host: () => hostEntryFor(null),
    beforeOpen: () => {
      sidebarQuery.close();
    ...
    metadataChanged: () => responseDetailsController.update(),
  ...
  var recoveryController = createRecovery({
    ...
    down: (...args) => hostConnections.isDown(...args),
  ...
  var hostSettings = createHostSettings({
    ...
    escapeHtml,
    color: (...args) => hostPresentation.colorFor(...args),
    customColor: (...args) => hostPresentation.isCustom(...args),
    resolveColor: (...args) => resolveColorToHex(...args),
    ...
    renderNewSessionHosts: (...args) => newSessionController.renderHosts(...args)
  });
  ...
  var searchViewController = createSearchView({
    ...
    fanout: fanoutHosts,
    host: hostEntryFor,
    scope: (...args) => sidebarQuery.scope(...args),
  ...
  var skillsController = createSkills({
    root: document.querySelector(".main"),
    request: (...args) => apiTransport.request(...args),
    self: selfHostEntry,
    ...
    copy: copyTextToClipboard2,
    status: setStatus
  });
  var usageController = createUsageView({
    root: document.querySelector(".main"),
    request: (...args) => apiTransport.request(...args),
    storage: localStorage,
    fleetReady: () => hostFleetReady,
  ...
  var fileViews = createFileViews({
    ...
    clearComments: () => anchoredCommentController.set([]),
    refreshComments: () => anchoredCommentController.refresh(),
    markComments: () => anchoredCommentController.applyMarks()
  });
  var anchoredCommentController = createAnchoredComments({
    document,
    sessionState,
    views: fileViews,
    request: (host, path, options2) => apiTransport.request(host, path, options2),
    host: hostEntryFor,
    status: (message3, type) => setStatus(message3, type),
    loadPatch: (details) => fileViews.loadPatch(details)
  });
  var transcriptController = createTranscript({
    document,
    sessionState,
    request: (host, path, options2) => apiTransport.request(host, path, options2),
    host: hostEntryFor,
    renderMessage: (message3) => messageRenderer.message(message3),
    finalize: (root, options2) => finalizeRender(root, options2),
    closeSearch: () => sessionSearch.close(),
    cancelStreaming: () => streamingRenderer.cancel(),
    mood: (description, face) => moodController.set(description, face),
    updateMood: (messages) => moodController.fromMessages(messages),
  ...
  var liveToolsController = createLiveTools({
    ...
    scroll: (...args) => appChrome.scroll(...args),
    jump: (...args) => appChrome.jump(...args),
    images: (content, alt) => messageRenderer.images(content, alt),
    mood: (name, args) => moodController.fromTool(name, args)
  });
  var messageStreamController = createMessageStream({
    document,
    sessionState,
    endpoint: (...args) => hostDirectory.resolveHost(...args),
    ...
    status: (message3, type) => setStatus(message3, type),
    catchup: (owner) => transcriptController.catchup(owner),
    refresh: () => sidebarLists.refresh(),
    artifacts: (owner) => sessionInfo.refreshArtifacts(owner),
    pinned: (...args) => appChrome.pinned(...args),
    follow: () => appChrome.following,
    ...
    loadSessions: (query, options2) => sidebarLists.load(query, options2)
  });
  var composerDrafts = createComposerDrafts({
    document,
    storage: localStorage,
  ...
  var composerNotes = createComposerNotes(document);
  var composerSpeech = createComposerSpeech({
    document,
    sessionState,
    composerKey: () => composerDrafts.key,
    hosts: effectiveHosts,
  ...
  var composerSubmit = createComposerSubmit({
    ...
    refs: (message3) => sessionReferences.hints(message3),
    status: (message3, type) => setStatus(message3, type),
    openTree: () => transcriptTree.open(),
    hideAutocomplete: () => composerAutocomplete.hide(),
    refresh: () => sidebarLists.refresh(),
    follow: () => {
      appChrome.follow();
    ...
    scroll: (...args) => appChrome.scroll(...args),
    renderUser: (message3, time, attrs) => messageRenderer.user(message3, time, attrs)
  });
  var pendingSessionSpawns = createSessionSpawns({
    request: (...args) => apiTransport.request(...args),
    delay: (attempt) => new Promise((resolve) => setTimeout(resolve, attempt < 50 ? 100 : 250)),
    harnessLabel: (...args) => newSessionController.harnessLabel(...args),
    current: () => sessionView.spawnId,
    changed: renderSessions,
    showPending: (key) => {
      sidebarQuery.switchTab("active");
      sessionView.pending(key);
      if (window.innerWidth <= 768) sidebarQuery.close();
    },
    loadHost: (host) => sidebarLists.loadHost(host),
    hasSession: (id, host) => !!sessionState.findSession(id, host),
    selectSession: (id, host) => {
      void sessionView.select(id, { host });
    },
    stashPrompt: (...args) => composerDrafts.stash(...args),
    saveDraft: (key, draft) => {
      try {
        localStorage.setItem(composerDrafts.draftKey(pendingComposerKey(key)), draft);
      } catch {
      }
    },
    ...
    discardPrompt: (key) => {
      ...
      composerDrafts.clearDraft(owner);
      composerDrafts.images.discard(owner);
    },
    showFailure: (...args) => sessionView.failure(...args),
    status: setStatus
  });
  var newSessionController = createNewSession({
    root: document.querySelector(".main"),
    storage: localStorage,
    request: (...args) => apiTransport.request(...args),
    self: selfHostEntry,
    host: hostEntryFor,
    hosts: effectiveHosts,
    hostDown: (...args) => hostConnections.isDown(...args),
    multiHost: isMultiHost,
    sessionState,
  ...
  var harnessDiscovery = newSessionController.harnesses;
  var newSessionConfigPreview = newSessionController.config;
  var spawnTargetsController = newSessionController.targets;
  function savedResumeTarget(host) {
    return spawnTargetsController.resume(hostEntryFor(host));
  }
  function loadNsHarnessConfig(cwd = newSessionController.cwd()) {
  ...
  var harnessSettingsController = createHarnessSettings({
    root: document.getElementById("harnessSettingsModal"),
    host: hostEntryFor,
    request: (...args) => apiTransport.request(...args),
    fallbackModels: (host, harness) => modelCatalog.scope?.harnessId === harness && sameDirectoryHost(modelCatalog.scope?.host || null, host) ? modelCatalog.rows() : [],
    escapeHtml,
    shortCwd,
    ...
    modelRoleLevels,
    onSaved: (scope) => {
      if (newSessionController.isOpen() && newSessionController.selectedHarness() === scope.harnessId && newSessionController.hostId() === scope.hostId && newSessionController.cwd() === scope.cwd) void loadNsHarnessConfig();
    }
  });
  function openSessionHarnessSettings() {
    ...
    return openHarnessSettings({
      ...
      hostId: sessionReferences.hostId(session),
      cwd: session.cwd ?? "",
  ...
  function openHarnessSettings(opts = {}) {
    ...
    return harnessSettingsController.open({
      harnessId,
      hostId: opts.hostId !== void 0 ? opts.hostId : newSessionController.hostId(),
      cwd: (opts.cwd !== void 0 ? opts.cwd : newSessionConfigPreview.config?.cwd ?? newSessionController.cwd()) || "",
      label: opts.label || newSessionController.harnessLabel(harnessId),
      tab: opts.tab
    });
  }
  function createCwdAutocomplete2({
    input,
    dropdown,
    hostId = (...args) => newSessionController.hostId(...args),
    known = () => [],
    onPick = () => {
    },
    onSubmit = null,
  ...
  function readJSONPref(key, fallback2) {
    try {
      return JSON.parse(localStorage.getItem(key) || "null") ?? fallback2;
    } catch {
      return fallback2;
    }
  }
  ...
  function setStatus(message3, type = "") {
    const status = document.getElementById("status");
    status.textContent = message3;
    status.className = `status ${type}`;
  }
  var moodController = createMood(document);
  var extensionUI = createExtensionUI({
    document,
    sessionState,
    storage: localStorage,
    request: (host, path, options2) => apiTransport.request(host, path, options2),
  ...
  var browserAssets = createBrowserAssets(document);
  var diagramRenderer = createDiagrams({
    document,
    assets: browserAssets,
    runtime: () => typeof mermaid === "undefined" ? null : mermaid,
    retainedRoots: () => transcriptController.retainedRoots(),
    isPinned: (feed) => appChrome.pinned(feed),
    scrollBottom: (feed) => appChrome.scroll(feed)
  });
  ...
  var themesController = createThemes({
    document,
    storage: localStorage,
    request: (host, url, options2) => apiTransport.request(host, url, options2),
    host: () => hostEntryFor(null),
  ...
  function terminalTheme2() {
    return terminalTheme(document);
  }
  var terminalController = createTerminalController({
    document,
    storage: localStorage,
    sessionState,
    ...
    createTerminal: (options2) => typeof Terminal === "undefined" ? null : new Terminal(options2),
    createFitAddon: () => {
    ...
    theme: () => terminalTheme2(),
    applySize: (panel) => panelResize.terminalSize(panel),
    confirm: (message3) => confirm(message3)
  });
  var panelResize = createPanelResize({ document, storage: localStorage, fitTerminal: () => terminalController.fit() });
  var routinesController = createRoutinesView({
    root: document.querySelector(".main"),
    request: (host, url, options2) => apiTransport.request(host, url, options2),
    storage: localStorage,
    sessionState,
  ...
  var bounceController = createBounce({
    document,
    request: (...args) => apiTransport.request(...args),
    hosts: effectiveHosts,
    fleetReady: () => hostFleetReady,
  ...
  createAppBindings({ document, actions: {
    ...
    searchKey: (event) => {
      if (event instanceof KeyboardEvent) sessionSearch.key(event);
    },
    ...
    onNsHostChange: (_event, node) => {
      if (node instanceof HTMLSelectElement) newSessionController.changeHost(node.value);
    },
    ...
    backdropCloseTreeModal: (event, node) => {
      if (event.target === node) transcriptTree.close();
    },
    ...
    bounceToggle: (_event, node) => {
      if (node instanceof HTMLDetailsElement) {
        if (node.open) bounceController.refresh();
        else bounceController.close();
      }
    },
    ...
    backdropCloseHarnessSettings: (event, node) => {
      if (event.target === node) harnessSettingsController.close();
    },
    ...
    backdropCloseResponseDetails: (event, node) => {
      if (event.target === node) responseDetailsController.close();
    },
