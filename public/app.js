...
`;
  ...
  function decodeSessionRelations(value) {
    ...
    return value.flatMap((row) => {
      if (!record2(row) || !record2(row.session) || typeof row.session.id !== "string" || !row.session.id) return [];
      const session = row.session;
      return [{ kind: text5(row.kind), source: text5(row.source), session: {
        id: session.id,
        name: text5(session.name),
        cwd: text5(session.cwd),
  ...
  function createSessionRelations(options2) {
    const { document: document2, window: window2, sessionState: sessionState2 } = options2;
    const element = (id) => document2.getElementById(id);
    ...
    window2.addEventListener("resize", () => {
      ...
      relationResizeTimer = setTimeout(() => {
        const el = document2.getElementById("sessionRelations");
        if (sessionState2.currentSession && el?.style.display !== "none") renderSessionRelations(sessionRelations);
      }, 100);
    ...
    async function loadSessionRelations(owner) {
      ...
      try {
        ...
        renderSessionRelations(decodeSessionRelations(record2(data) ? data.relations : null), owner, endpoint);
        if (record2(data) && data.indexing === true) indexingTimer = setTimeout(() => {
