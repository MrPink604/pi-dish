...
export function createSidebarActivity(options: { document: Document; storage: Pick<Storage, 'getItem' | 'setItem'>; sessionState: SessionState }) {
  ...
  function unread(session: SessionEntry) { return isUnreadSession(sidebarSession(session), seen, sessionState.currentSession ? sessionRefKey(sessionState.currentSession) : null, !document.hidden); }
