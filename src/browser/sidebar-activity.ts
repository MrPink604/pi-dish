...
export function createSidebarActivity(options: { document: Document; storage: Pick<Storage, 'getItem' | 'setItem'>; sessionState: SessionState }) {
  ...
  function mark(session: SessionEntry | null, at: SessionEntry['lastActivity'] = session?.lastActivity) { if (!session || !at) return; seen[sessionRefKey(session)] = at; save(); }
  function unread(session: SessionEntry) { return isUnreadSession(session, seen, sessionState.currentSession ? sessionRefKey(sessionState.currentSession) : null, !document.hidden); }
