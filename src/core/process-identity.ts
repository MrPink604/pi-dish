...
function processRecord(pid: number | string | undefined): (ProcessIdentity & { ppid: number }) | null {
 ...
 */
function processIdentity(pid: number | string | undefined): ProcessIdentity | null {
  const record = processRecord(pid);
  return record ? { pid: record.pid, startTime: record.startTime } : null;
}

function processIdentityAlive(identity: ProcessIdentityInput | null | undefined): boolean {
 ...
 */
function inspectProcessAncestry(identity: ProcessIdentityInput | null | undefined, { maxDepth = 128 } = {}): ProcessAncestry {
...
export = { processIdentity, processIdentityAlive, inspectProcessAncestry };
