...
const registry: Readonly<Record<HarnessId, HarnessDescriptor>> = {
  ...
  omp: {
    ...
    taskAgents: {
      ...
      userDir: (env = process.env) => path.join(
        env.PI_CODING_AGENT_DIR || env.OMP_AGENT_DIR
  ...
  prime: {
    ...
    discoveryExtensionsDir: (env = process.env) =>
      path.join(env.PRIME_AGENT_DIR || path.join(env.HOME || os.homedir(), '.prime', 'agent'), 'extensions'),
...
function splitShellWords(input: unknown): string[] {
  const words = [];
  let current = '', quote: string | null = null, escaped = false;
...
function resolveLaunchSpec(descriptor: HarnessDescriptor, env: NodeJS.ProcessEnv = process.env) {
...
export = { registry, getHarness, listHarnesses, resolveLaunchSpec };
