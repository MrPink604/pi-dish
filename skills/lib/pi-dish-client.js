...
exports.fleetRows = fleetRows;
exports.fleetHost = fleetHost;
exports.stringValue = stringValue;
exports.makeFail = makeFail;
exports.print = print;
exports.parentPid = parentPid;
exports.ancestorPids = ancestorPids;
exports.pidAlive = pidAlive;
exports.registryEntries = registryEntries;
exports.registryRouteId = registryRouteId;
exports.sessionHarnessId = sessionHarnessId;
exports.nativeSessionId = nativeSessionId;
exports.discoverSession = discoverSession;
exports.discoverSessionQuietly = discoverSessionQuietly;
exports.defaultBase = defaultBase;
exports.request = request;
exports.requestText = requestText;
exports.hostPath = hostPath;
exports.api = api;
exports.unknownHostError = unknownHostError;
exports.jsonInit = jsonInit;
exports.fleetHosts = fleetHosts;
exports.resetFleetCache = resetFleetCache;
exports.hostSupports = hostSupports;
exports.hostLabelOf = hostLabelOf;
exports.parseRef = parseRef;
exports.resolveHostPart = resolveHostPart;
exports.entryForHostName = entryForHostName;
exports.resolveSessionRef = resolveSessionRef;
exports.resolveSessionClientSide = resolveSessionClientSide;
exports.sessionRefAliases = sessionRefAliases;
exports.resolveRefAmong = resolveRefAmong;
exports.shortSessionRef = shortSessionRef;
exports.stableSessionRef = stableSessionRef;
...
exports.truncateResult = truncateResult;
/**
...
function parentPid(pid) {
    try {
        ...
        try {
            const value = (0, node_child_process_1.execFileSync)('ps', ['-o', 'ppid=', '-p', String(pid)], { encoding: 'utf8' }).trim();
...
function registryEntries() {
    ...
    return names.filter((name) => name.endsWith('.json')).flatMap((name) => {
        try {
            ...
            if (entry.socketPath && (typeof entry.socketPath !== 'string' || !fs.existsSync(entry.socketPath)))
                return [];
            if (typeof entry.pid === 'number' && Number.isInteger(entry.pid) && !pidAlive(entry.pid))
                return [];
            return [entry];
        }
        catch {
...
function decodeRouteId(routeId) {
    ...
    if (!raw.startsWith('~sk1_'))
        return null;
    try {
        const tuple = JSON.parse(Buffer.from(raw.slice(5), 'base64url').toString('utf8'));
        if (!Array.isArray(tuple) || typeof tuple[0] !== 'string' || !tuple[0])
            return null;
...
function discoverSession(explicit, options = {}) {
    if (explicit)
        return explicit;
    if (process.env.PI_DISH_SESSION_ID)
        return process.env.PI_DISH_SESSION_ID;
    ...
    const byCwd = entries.filter((entry) => {
        ...
        const resolved = typeof entry.cwd === 'string'
            ? path.resolve(entry.cwd) : Reflect.apply(path.resolve, path, [entry.cwd]);
        return resolved === cwd;
    });
    if (byCwd.length === 1)
        return registryRouteId(byCwd[0]);
    if (!entries.length)
        throw new Error(options.noneMessage || 'no live pi-dish bridge sessions found');
...
function parseRef(raw) {
    ...
    if (!ref)
        throw new Error('a session ref is required');
    const slash = ref.indexOf('/');
    if (slash !== -1) {
        const hostPart = ref.slice(0, slash);
        const id = ref.slice(slash + 1);
        if (!hostPart)
            throw new Error(`invalid ref "${ref}": nothing before the "/" to name a host`);
        if (!id)
            throw new Error(`invalid ref "${ref}": nothing after the "/" to name a session`);
    ...
    if (colon > 0) {
        ...
        if (UUID_RE.test(head) && rest)
            return { hostPart: head, hostIdForm: true, id: rest };
    }
...
function resolveRefAmong(sessions, ref, exactOnly) {
    ...
    for (let stage = 0; stage < (exactOnly ? 2 : stages.length); stage++) {
        ...
        if (matches.length === 1)
            return { session: matches[0], matches };
...
function shortSessionRef(id, peerIds, minLen = 8) {
    ...
    for (const alias of sessionRefAliases(self).slice().reverse()) {
        for (let len = Math.min(minLen, alias.length); len <= alias.length; len++) {
            ...
            if (peers.some((peer) => peer.startsWith(candidate)))
                continue;
...
function ambiguousRefError(ref, matches) {
    ...
    return new Error(`ambiguous session id prefix "${ref}" — ${matches?.length || 0} sessions match; use a longer prefix or the full id:\n`
        + lines.join('\n'));
}
    ...
    if (hostSupports(entry, 'resolve')) {
        try {
            ...
            if (isSessionRow(data.session)) {
                ...
                if (ref.hostIdForm && data.session.id !== ref.id)
                    throw new Error(`Session not found: ${ref.id}`);
            ...
            if (errorBody(e).error) {
                if (Array.isArray(errorBody(e).matches))
                    throw ambiguousRefError(ref.id, sessionRows(errorBody(e).matches));
                ...
                if (errorStatus(e) !== 404 || hostSupports(entry, 'refAliases'))
                    throw e;
            }
...
function mergeSearchResults(hostResults, limit) {
    ...
    for (const bucket of hostResults || []) {
        for (const session of bucket?.results || []) {
            if (session)
                rows.push({ host: bucket.host ?? null, session });
        }
    }
    rows.sort((a, b) => {
...
function renderTranscript(input, options = {}) {
    ...
    if (session.lastActivity)
        out.push(`- last activity: ${stamp(session.lastActivity) || session.lastActivity}`);
    ...
    for (const value of messages) {
        ...
        if (message.role === 'toolResult' && message.toolName)
            extra.push(message.toolName);
        if (message.isError)
            extra.push('error');
        if (message.role === 'custom' && message.customType)
            extra.push(message.customType);
        ...
        if (message.errorMessage)
            body.push(`! ${oneLine(message.errorMessage)}`);
        if (body.length)
            out.push(body.join('\n\n'));
        else
            out.push('_(no displayable content)_');
    }
