...
import type { FleetHost, SessionRow, SessionCatalog, SearchResponse, SearchBucket, RequestOptions, HttpResult } from '../../lib/pi-dish-client.js';
...
interface ValueArgs {
  url?: string; session?: string; cwd?: string; harness?: string;
  model?: string; name?: string; prompt?: string; limit?: string;
  host?: string; before?: string;
}
interface BooleanArgs {
  json?: boolean; active?: boolean; no_wait?: boolean;
  thinking?: boolean; all_hosts?: boolean;
}
interface SessionArgs extends ValueArgs, BooleanArgs {
  command: string;
...
interface DocsTopic { name?: unknown; title?: unknown; description?: unknown }
interface Relation { kind?: unknown; source?: unknown; session?: unknown }
interface FleetSearchStatus {
  status: 'ok' | 'skipped' | 'error';
  error?: unknown;
  total?: unknown;
  indexing?: boolean;
}
interface TmuxTarget { socket: unknown; pane: unknown }
interface TmuxEntry extends TmuxTarget {
  harness: unknown; name: unknown; state: string; cwd: string; model: unknown;
  sessionId: unknown; routeId: unknown; pid: unknown; updatedAt: unknown;
}
function wireArray(value: unknown): unknown[] { return Array.isArray(value) ? value : []; }
...
function parseArgs(argv: string[]): SessionArgs {
  ...
  const result: SessionArgs = { command: hasCommand ? leading : 'list', explicitCommand: hasCommand, positional: [] };
  for (let i = hasCommand ? 1 : 0; i < argv.length; i++) {
    const arg = argv[i];
    if (arg === '--help' || arg === '-h') result.help = true;
    else if (BOOL_FLAGS.has(arg)) result[BOOL_FLAGS.get(arg)!] = true;
...
function intArg(value: string | undefined, fallback: number, max = 100) {
...
function sessionLine(session: SessionRow, ref?: string) {
 ...
 */
function refIndex(sessions: readonly SessionRow[] | null | undefined, entry: unknown): Map<string, string> {
  const rows = sessions || [];
...
function searchRow(session: SessionRow, ref?: string) {
...
function writeSnippets(session: SessionRow) {
...
async function resolveSpawnHarness(base: string, host: string | null, explicit: string | undefined, callerSessionId: unknown): Promise<string> {
  ...
  try { harnesses = core.record((await api(base, host, '/api/harnesses')).data).harnesses; }
  ...
  const entries: unknown[] = harnesses;
...
async function createSpawn(base: string, host: string | null, body: SpawnBody, callerId: unknown): Promise<HttpResult> {
  ...
  try {
    ...
    if (!host || !/requestedBySessionId/i.test(core.errorMessage(e) || '')) throw e;
...
async function pollSpawn(base: string, host: string | null, spawnId: string, timeoutMs = 45000): Promise<SpawnOperation & Record<string, unknown>> {
...
async function listDocs(base: string, host: string | null, json?: boolean) {
  ...
  for (const value of topics) {
    const topic: DocsTopic = core.record(value);
...
async function showDoc(base: string, host: string | null, topic: string, json?: boolean) {
  const entry = await entryForHostName(base, host);
  if (entry && !hostSupports(entry, 'docs')) throw new Error(DOCS_UNSUPPORTED);
  let text;
  try {
...
function abortInit(): RequestOptions {
...
async function fleetSearch(base: string, query: string, limit: number, json?: boolean) {
  const hosts = await fleetHosts(base);
  if (!hosts) throw new Error('--all-hosts needs GET /api/hosts; this server did not answer it');

  const targets: { label: unknown; host: unknown }[] = [];
  const skipped: { label: unknown; reason: unknown }[] = [];
  ...
  const status: Record<string, FleetSearchStatus> = {};
  ...
  const refsByHost = new Map<unknown, Map<string, string>>();
  for (const skip of skipped) status[String(skip.label)] = { status: 'skipped', error: skip.reason };
...
function activeTmuxEntries(): TmuxEntry[] {
  ...
  for (const entry of entries) {
    ...
    let winName = null;
    try {
      winName = execFileSync('tmux', ['-S', String(tmux.socket), 'display-message', '-p', '-t', String(tmux.pane), '#{window_name}'], { encoding: 'utf8', timeout: 1000 }).trim() || null;
    } catch {}
...
function attachToTmux(socket: unknown, pane: unknown) {
  const isInsideTmux = Boolean(process.env.TMUX);
  const currentTmuxSocket = isInsideTmux ? process.env.TMUX?.split(',')[0] : null;

  try {
    execFileSync('tmux', ['-S', String(socket), 'select-window', '-t', String(pane)], { stdio: 'ignore', timeout: 2000 });
    execFileSync('tmux', ['-S', String(socket), 'select-pane', '-t', String(pane)], { stdio: 'ignore', timeout: 2000 });
  } catch {}
  ...
  const resolvedSocket: unknown = isInsideTmux && currentTmuxSocket
  ...
  if (isInsideTmux && currentTmuxSocket && path.resolve(currentTmuxSocket) === resolvedSocket) {
    execFileSync('tmux', ['-S', String(socket), 'switch-client', '-t', String(pane)], { stdio: 'inherit' });
  } else {
    // A nested attach needs $TMUX gone, same as the server's pane-view PTY.
    const env = { ...process.env };
    delete env.TMUX;
    delete env.TMUX_PANE;
    spawnSync('tmux', ['-S', String(socket), 'attach-session', '-t', String(pane)], { stdio: 'inherit', env });
  }
}

function pickSessionWithFzf(entries: readonly TmuxEntry[]): Promise<TmuxTarget | null> {
  return new Promise<TmuxTarget | null>((resolve, reject) => {
    const fzf = spawn('fzf', [
      '--exit-0',
      '--delimiter=\t',
      '--with-nth=1',
    ...
    fzf.on('error', err => {
      if (core.record(err).code === 'ENOENT') resolve(null);
      else reject(err);
    });
...
async function runAttach(args: SessionArgs) {
  ...
  let selected: TmuxTarget | null | undefined = null;
  if (targetQuery) {
...
async function main() {
  ...
  catch (e) { return fail(core.errorMessage(e)); }
  ...
  try {
    ...
    if (spec.name === 'hosts') {
      ...
      for (const entry of hosts) process.stdout.write(hostLine(entry) + '\n');
    ...
    if (spec.name === 'list') {
      ...
      const catalog: SessionCatalog = core.record(data);
      ...
      if (catalog.indexing) process.stdout.write('# Session index is still building; repeat list for more.\n');
      if (catalog.discoveryTruncated) process.stdout.write('# Nested session discovery reached its safety limit.\n');
      return;
    }
    ...
    if (spec.name === 'spawn') {
      ...
      const body: SpawnBody = { async: true, requestedBySessionId: callerId, harness };
      ...
      if (operation.status === 'error') throw new Error(String(operation.error || 'session spawn failed'));
      const id = operation.sessionId;
      if (typeof id !== 'string') throw new TypeError('spawn response did not report a sessionId');
    ...
    if (spec.name === 'search') {
      ...
      const search: SearchResponse & Record<string, unknown> = core.record(data);
      const results = core.sessionRows(search.results).slice(0, limit);
      if (args.json) return print({ ...search, results }, true);
      ...
      if (Number(search.total) > results.length) process.stdout.write(`# ${Number(search.total) - results.length} more results not shown; refine the query or raise --limit.\n`);
      if (search.indexing) process.stdout.write('# Session index is still building; results may be partial — retry shortly.\n');
      return;
    }
    ...
    if (spec.name === 'related') {
      ...
      for (const value of relations) {
        const relation: Relation = core.record(value);
      ...
      if (!relations.length) process.stdout.write('No related sessions.\n');
      return;
    }
