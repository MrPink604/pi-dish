...
Last confirmed CI checkpoint, `b1cc60d` (newer local work is in the checkpoint log):

- Fable 5.1 reviewed the host-settings commit before push with no blocking
  findings. Earlier host-directory/discovery checkpoints were also reviewed before push.
- 862 backend tests and 71 browser regressions passed, along with independent UI
