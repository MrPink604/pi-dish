#!/usr/bin/env node
// Generated test/tool from scripts/session-catalog-baseline.ts; edit that source and run npm run build:tests.
"use strict";
    ...
    const runs = ['routes', 'active', 'lists'].map(name => {
        ...
        try {
            ...
            if (run.error || run.status !== 0)
                throw run.error || new Error(run.stderr || run.stdout);
...
if (!['routes', 'active', 'lists'].includes(scenario))
    throw new Error('Usage: node scripts/session-catalog-baseline.js [routes|active|lists]');
...
function write(file, id, omp = false, count = 24) {
    ...
    for (let n = 0; n < count; n++)
        entries.push({ type: 'message', id: `m${n}`, parentId: n ? `m${n - 1}` : null,
...
function createCorpus() {
    for (let h = 0; h < roots.length; h++) {
        for (let n = 0; n < 80; n++)
            write(path.join(roots[h], h === 2 ? '' : `workspace-${n % 4}`, `fixture-${n}.jsonl`), `fixture-${n}`, h === 1);
    }
    for (let n = 0; n < 8; n++)
        write(path.join(roots[0], 'workspace-0', `launcher-${n}`, 'session.jsonl'), `generic-${n}`);
    for (let n = 0; n < 8; n++)
        write(path.join(live.slice(0, -6), `child-${n}.jsonl`), `child-${n}`, true);
    ...
    if (!identity)
        throw new Error('Baseline process identity is unavailable');
...
wrap('readFileSync', (original, receiver, args) => {
    ...
    try {
        ...
        if (counts && corpusPath(args[0])) {
            counts.fullReads++;
            counts.bytes += Buffer.byteLength(String(result));
        }
        return result;
    }
    finally {
...
wrap('openSync', (original, receiver, args) => {
    ...
    if (typeof fd === 'number' && corpusPath(args[0]))
        fds.set(fd, args[0]);
    return fd;
});
...
async function measure(name, action) {
    ...
    results.push({ name, ms: Number((node_perf_hooks_1.performance.now() - start).toFixed(3)), ...observed, ...detail });
    counts = null;
}
async function main() {
    const activeServer = require('../server');
    server = activeServer;
    if (!activeServer.listening)
        await (0, node_events_1.once)(activeServer, 'listening');
    ...
    if (!address || typeof address === 'string')
        throw new Error('Baseline server lacks TCP address');
    ...
    if (scenario === 'lists') {
        ...
        const sourceApi = require('../lib/session-source');
