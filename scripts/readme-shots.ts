...
interface BridgeOptions {
  ...
  onPrompt?: (message: Record<string, unknown>) => void;
}
...
const ts = (minAgo: number, secOffset = 0): string =>
  new Date(Date.now() - minAgo * 60_000 + secOffset * 1000).toISOString();

function writeSession(dirName: string, id: string, entries: readonly FixtureEntry[]): string {
...
const user = (text: string, timestamp: string): FixtureEntry => ({ type: 'message', message: { role: 'user', content: [{ type: 'text', text }], timestamp } });
const asst = (content: unknown, timestamp: string): FixtureEntry => ({ type: 'message', message: { role: 'assistant', content, timestamp } });
const toolCall = (id: string, name: string, args: Record<string, unknown>, timestamp: string): FixtureEntry =>
  asst([{ type: 'toolCall', id, name, arguments: args }], timestamp);
const toolResult = (name: string, text: string, timestamp: string): FixtureEntry => ({ type: 'message', message: { role: 'toolResult', toolName: name, content: [{ type: 'text', text }], timestamp } });
...
// --- fake bridges ----------------------------------------------------------------
function makeBridge(socketPath: string, { models, onPrompt }: BridgeOptions = {}) {
  ...
  const server = net.createServer(sock => {
    ...
    sock.on('data', chunk => {
      ...
      while ((idx = buf.indexOf('\n')) >= 0) {
        ...
        let data: unknown = {};
        if (message.command === 'get_available_models') data = { models };
        else if (message.command === 'get_commands') data = [
          { name: 'compact', description: 'compact the session', source: 'builtin' },
          { name: 'mood', description: 'set the mood', source: 'extension' },
        ];
        sock.write(JSON.stringify({ type: 'response', id: message.id, success: true, data }) + '\n');
        if (message.command === 'prompt' && onPrompt) onPrompt(message);
      }
    });
  });
...
const appendMain = (entry: FixtureEntry): void => fs.appendFileSync(mainFile, JSON.stringify(entry) + '\n');
const sleep = (ms: number): Promise<void> => delay(ms);

(async () => {
  main.server.listen(mainSock);
  await once(main.server, 'listening');
  api.server.listen(apiSock);
  await once(api.server, 'listening');

  const server: import('node:http').Server = require(path.join(REPO, 'server.js'));
  if (!server.listening) await once(server, 'listening');
  const address = server.address();
  if (address === null || typeof address === 'string') throw new Error('Expected server TCP address');
  ...
  try {
    ...
    await desktop.evaluate(() => { const messages = document.getElementById('messages'); if (messages) messages.scrollTop = messages.scrollHeight; });
    ...
    await mobile.evaluate(() => { const messages = document.getElementById('messages'); if (messages) messages.scrollTop = messages.scrollHeight; });
    await mobile.waitForTimeout(300);
