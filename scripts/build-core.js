#!/usr/bin/env node
// Generated tool from scripts/build-core.ts; edit that source and run npm run build:tools.
"use strict";
...
try {
    ...
    const result = (0, node_child_process_1.spawnSync)(process.execPath, [compiler, '-p', 'tsconfig.core.json', '--outDir', out], {
    ...
    if (result.status !== 0) {
        ...
        if (orphaned.length)
            throw new Error(`Remove obsolete core output before building:\n${orphaned.map(file => `lib/${file}`).join('\n')}`);
        const stale = [];
