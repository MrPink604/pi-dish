...
const declarationProjects = new Set<string>(['tsconfig.node-tests.json', 'tsconfig.dev-tools.json']);
const banner = '// Generated test/tool from ';
const outputs = new Map<string, { source: string; project: string }>();

function sourceFiles(project: string): string[] {
  const parsed: unknown = JSON.parse(fs.readFileSync(path.join(root, project), 'utf8'));
  if (typeof parsed !== 'object' || parsed === null || !('files' in parsed)
      || !Array.isArray(parsed.files) || !parsed.files.every((file: unknown) => typeof file === 'string')) {
...
for (const project of projects) {
  for (const source of sourceFiles(project)) {
    if (source === 'test/browser/globals.d.ts') continue;
    ...
    for (const suffix of suffixes) {
      ...
      if (outputs.has(file)) throw new Error(`Duplicate test output: ${file}`);
...
function generatedOutputs(): string[] {
  ...
  for (const directory of ['test', 'scripts']) {
    ...
    for (const entry of fs.readdirSync(base, { recursive: true, withFileTypes: true })) {
      ...
      if (content.startsWith(banner)) files.push(path.relative(root, target).split(path.sep).join('/'));
    }
  }
  return files;
}
...
try {
  ...
  for (const project of projects) {
    ...
    const result = spawnSync(process.execPath, [compiler, '-p', project, '--outDir', projectOut], {
      cwd: root,
      stdio: 'inherit',
    });
    ...
    for (const entry of fs.readdirSync(projectOut, { recursive: true, withFileTypes: true })) {
      ...
      if (outputs.get(file)?.project !== project) continue;
      if (emitted.has(file)) throw new Error(`Multiple compiler programs emitted owned output ${file}`);
  ...
  if (failed) {
    ...
    if (!check) for (const file of orphaned) fs.rmSync(path.join(root, file));
    const prepared = [];
