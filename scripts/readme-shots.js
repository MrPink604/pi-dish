#!/usr/bin/env node
// Generated test/tool from scripts/readme-shots.ts; edit that source and run npm run build:tests.
"use strict";
...
const ts = (minAgo, secOffset = 0) => new Date(Date.now() - minAgo * 60_000 + secOffset * 1000).toISOString();
...
const user = (text, timestamp) => ({ type: 'message', message: { role: 'user', content: [{ type: 'text', text }], timestamp } });
const asst = (content, timestamp) => ({ type: 'message', message: { role: 'assistant', content, timestamp } });
const toolCall = (id, name, args, timestamp) => asst([{ type: 'toolCall', id, name, arguments: args }], timestamp);
const toolResult = (name, text, timestamp) => ({ type: 'message', message: { role: 'toolResult', toolName: name, content: [{ type: 'text', text }], timestamp } });
        ...
        sock.on('data', chunk => {
            ...
            while ((idx = buf.indexOf('\n')) >= 0) {
                ...
                if (!(0, wire_protocol_js_1.isRecord)(message))
                    continue;
                let data = {};
                if (message.command === 'get_available_models')
                    data = { models };
                else if (message.command === 'get_commands')
                    data = [
                ...
                if (message.command === 'prompt' && onPrompt)
                    onPrompt(message);
            }
        });
    });
...
const appendMain = (entry) => fs.appendFileSync(mainFile, JSON.stringify(entry) + '\n');
const sleep = (ms) => (0, promises_1.setTimeout)(ms);
(async () => {
    main.server.listen(mainSock);
    await (0, node_events_1.once)(main.server, 'listening');
    api.server.listen(apiSock);
    await (0, node_events_1.once)(api.server, 'listening');
    ...
    const browser = await playwright_1.chromium.launch({ executablePath: process.env.CHROME_BIN || undefined, headless: true });
    try {
        ...
        await desktop.evaluate(() => { const messages = document.getElementById('messages'); if (messages)
        ...
        await mobile.evaluate(() => { const messages = document.getElementById('messages'); if (messages)
            messages.scrollTop = messages.scrollHeight; });
        await mobile.waitForTimeout(300);
